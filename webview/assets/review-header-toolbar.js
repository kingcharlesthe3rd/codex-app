import { s as e } from "./chunk-Bj-mKKzh.js";
import { ka as t } from "./app-server-manager-signals.js";
import { n, t as r } from "./jsx-runtime.js";
import { A as i, F as a, I as o, lt as s, r as c, s as l } from "./vscode-api.js";
import { c as u, o as d } from "./lib-2.js";
import { t as f } from "./tooltip.js";
import { t as p } from "./route-scope.js";
import { t as m } from "./button.js";
import { t as h } from "./spinner.js";
import { J as g, r as _ } from "./review-navigation-model.js";
import { n as v, t as y } from "./json-2.js";
import { t as b } from "./image-square.js";
import { t as x } from "./check-md.js";
import { t as S } from "./clock.js";
import { r as C, t as w } from "./dropdown.js";
import { t as T } from "./three-dots.js";
import { t as E } from "./warning.js";
import { c as D, l as O, o as k, p as A, u as j } from "./pull-request-status.js";
import { n as M, r as N } from "./code-snippet.js";
import { t as P } from "./regenerate.js";
import { t as F } from "./comment.js";
import { t as I } from "./score-query-match.js";
import { n as L, t as R } from "./diff-unified.js";
import { t as z } from "./middle-truncated-text.js";
import { t as B } from "./document-search.js";
var V = e(n()),
  H = r(),
  U = (e) =>
    (0, H.jsxs)(`svg`, {
      xmlns: `http://www.w3.org/2000/svg`,
      width: 20,
      height: 20,
      fill: `currentColor`,
      viewBox: `0 0 20 20`,
      ...e,
      children: [
        (0, H.jsx)(`path`, {
          d: `M12.084 12.668a.666.666 0 0 1 0 1.33H7.917a.665.665 0 1 1 0-1.33h4.167ZM10 5.585c.367 0 .665.298.665.665v1.418h1.419a.666.666 0 0 1 0 1.33h-1.419v1.419a.666.666 0 0 1-1.33 0V8.998H7.917a.665.665 0 0 1 0-1.33h1.418V6.25c0-.367.298-.665.665-.665Z`,
        }),
        (0, H.jsx)(`path`, {
          fillRule: `evenodd`,
          d: `M12.667 2.668c.689 0 1.246 0 1.696.036.458.038.865.117 1.242.309a3.163 3.163 0 0 1 1.382 1.383c.192.377.272.783.309 1.24.037.45.036 1.008.036 1.697v5.333c0 .689 0 1.246-.036 1.696-.037.458-.117.865-.309 1.242a3.166 3.166 0 0 1-1.382 1.382c-.377.192-.784.271-1.242.309-.45.037-1.007.036-1.696.036H7.334c-.689 0-1.246 0-1.696-.036-.458-.038-.864-.117-1.24-.309a3.166 3.166 0 0 1-1.384-1.383c-.192-.376-.271-.783-.309-1.24-.037-.45-.036-1.008-.036-1.697V7.333c0-.689 0-1.246.036-1.696.038-.458.117-.864.309-1.24a3.17 3.17 0 0 1 1.383-1.384c.377-.192.783-.272 1.24-.309.45-.037 1.008-.036 1.697-.036h5.333Zm-5.333 1.33c-.71 0-1.204.001-1.588.032-.375.03-.587.088-.745.168A1.836 1.836 0 0 0 4.199 5c-.08.158-.137.37-.168.745C4 6.13 4 6.622 4 7.333v5.333c0 .71.001 1.204.032 1.588.03.375.088.587.168.745.176.345.457.627.802.803.158.08.37.137.745.168.384.031.877.031 1.588.031h5.333c.71 0 1.204 0 1.588-.031.375-.031.587-.088.745-.168a1.84 1.84 0 0 0 .803-.803c.08-.158.137-.37.168-.745.031-.383.031-.877.031-1.588V7.333c0-.71 0-1.204-.031-1.588-.031-.375-.088-.587-.168-.745A1.838 1.838 0 0 0 15 4.198c-.158-.08-.37-.137-.745-.168-.384-.031-.877-.032-1.588-.032H7.334Z`,
          clipRule: `evenodd`,
        }),
      ],
    }),
  W = (e) =>
    (0, H.jsxs)(`svg`, {
      width: 21,
      height: 21,
      viewBox: `0 0 21 21`,
      fill: `none`,
      xmlns: `http://www.w3.org/2000/svg`,
      ...e,
      children: [
        (0, H.jsx)(`path`, {
          fillRule: `evenodd`,
          clipRule: `evenodd`,
          d: `M10.8 7.20081C12.6973 7.20093 14.2363 8.73902 14.2365 10.6364C14.2363 12.5337 12.6973 14.0728 10.8 14.0729C8.90266 14.0727 7.36459 12.5336 7.36441 10.6364C7.36458 8.73905 8.90265 7.20098 10.8 7.20081ZM10.8 8.53088C9.63719 8.53106 8.69466 9.47359 8.69449 10.6364C8.69467 11.7991 9.6372 12.7426 10.8 12.7428C11.9628 12.7427 12.9062 11.7991 12.9064 10.6364C12.9062 9.47356 11.9628 8.53101 10.8 8.53088Z`,
          fill: `currentColor`,
        }),
        (0, H.jsx)(`path`, {
          fillRule: `evenodd`,
          clipRule: `evenodd`,
          d: `M10.8 3.47131C13.2616 3.47131 15.1979 4.65538 16.5998 6.00061C17.999 7.34335 18.9111 8.88809 19.3478 9.7262L19.4464 9.94495C19.6143 10.3912 19.6143 10.8815 19.4464 11.3278L19.3478 11.5465C18.9111 12.3846 17.9989 13.9294 16.5998 15.2721C15.1979 16.6173 13.2616 17.8014 10.8 17.8014C8.33838 17.8013 6.40196 16.6173 5.00015 15.2721C3.7758 14.0971 2.92396 12.7676 2.43765 11.8922L2.2521 11.5465C1.95397 10.9742 1.95397 10.2985 2.2521 9.7262L2.43765 9.38049C2.92396 8.50512 3.7758 7.17559 5.00015 6.00061C6.40196 4.65537 8.33838 3.47138 10.8 3.47131ZM10.8 4.80139C8.79567 4.80146 7.17197 5.76012 5.92105 6.96057C4.8244 8.01304 4.04928 9.21984 3.60269 10.0231L3.43081 10.3405C3.33334 10.5276 3.33334 10.7451 3.43081 10.9323L3.60269 11.2496C4.04928 12.0529 4.8244 13.2597 5.92105 14.3121C7.17197 15.5126 8.79567 16.4712 10.8 16.4713C12.8043 16.4713 14.4279 15.5126 15.6789 14.3121C16.9321 13.1094 17.7661 11.7056 18.1691 10.9323L18.2003 10.86C18.2551 10.7149 18.2551 10.5578 18.2003 10.4127L18.1691 10.3405C17.7661 9.56708 16.9321 8.16328 15.6789 6.96057C14.4279 5.76012 12.8043 4.80139 10.8 4.80139Z`,
          fill: `currentColor`,
        }),
      ],
    });
function G({
  canFix: e,
  checks: t,
  ciStatus: n,
  commentAttachments: r,
  commentsWorkflow: i,
  fixTooltipText: a,
  intl: o,
  mergeWorkflow: s,
  mergeTooltipText: c,
  onFix: l,
  onFixCheck: u,
  onOpenCheck: f,
  reviewStatus: p,
  reviewers: m,
  statusKinds: g,
}) {
  if (g.length === 0) return [];
  let _ = r.length,
    v = [],
    y = k({ canFix: e, checks: t, intl: o, onFixCheck: u, onOpenCheck: f }),
    b = D({ checks: t, ciStatus: n }),
    S = O(n);
  return (
    g.includes(`checks-none`) && v.push({ icon: b, key: `checks-none`, label: S }),
    g.includes(`checks-failing`) &&
      v.push({
        accessory: (0, H.jsx)(d, {
          id: `localConversation.pullRequest.actions.fixAll`,
          defaultMessage: `Fix all`,
          description: `Trailing label for the failing checks row in the pull request actions dropdown`,
        }),
        actionDisabled: !e,
        disabled: !e,
        icon: b,
        flyoutContent: y,
        key: `checks-failing`,
        label: S,
        onSelect: l,
        tooltipText: a,
      }),
    g.includes(`checks-pending`) &&
      v.push({ flyoutContent: y, icon: b, key: `checks-pending`, label: S }),
    g.includes(`checks-successful`) &&
      v.push({ flyoutContent: y, icon: b, key: `checks-successful`, label: S }),
    v.push(J({ flyoutContent: Y(m, o), intl: o, reviewStatus: p })),
    v.push(
      K({
        attachedCommentKeys: i.attachedCommentKeys,
        canAddressComments: i.canAddressComments,
        commentAttachments: r,
        commentCount: _,
        intl: o,
        onAddressComment: (e) => {
          i.startAddressCommentsForAttachments([e]);
        },
        onAddressComments: i.startAddressComments,
        onRemoveComment: i.removeAddressCommentsForAttachment,
        tooltipText: i.tooltipText,
      }),
    ),
    g.includes(`merging-blocked`) &&
      v.push({
        disabled: !0,
        icon: (0, H.jsx)(E, { className: `icon-sm shrink-0 text-token-charts-red` }),
        key: `merging-blocked`,
        label: (0, H.jsx)(d, {
          id: `localConversation.pullRequest.actions.mergingBlocked`,
          defaultMessage: `Merging blocked`,
          description: `Status row shown when the pull request cannot yet be merged`,
        }),
        tooltipText: c,
      }),
    g.includes(`ready-to-merge`) &&
      v.push({
        accessory: (0, H.jsx)(d, {
          id: `localConversation.pullRequest.actions.merge`,
          defaultMessage: `Merge`,
          description: `Label for the merge action in the pull request actions dropdown`,
        }),
        disabled: s.mergeDisabledReason != null || s.isPending,
        icon: s.isPending
          ? (0, H.jsx)(h, { className: `icon-sm shrink-0` })
          : (0, H.jsx)(x, { className: `icon-sm shrink-0 text-token-description-foreground` }),
        key: `ready-to-merge`,
        label: (0, H.jsx)(d, {
          id: `localConversation.pullRequest.actions.readyToMerge`,
          defaultMessage: `Ready to merge`,
          description: `Status row shown when the pull request is ready to merge`,
        }),
        onSelect: s.mergePullRequest,
        tooltipDisabled: s.mergeDisabledReason == null,
        tooltipText: c,
      }),
    v
  );
}
function K({
  attachedCommentKeys: e,
  canAddressComments: n,
  commentAttachments: r,
  commentCount: i,
  intl: a,
  onAddressComment: o,
  onAddressComments: s,
  onRemoveComment: c,
  tooltipText: l,
}) {
  let u = r.some((n) => !e.has(t(n)));
  return {
    accessory: u
      ? (0, H.jsx)(d, {
          id: `localConversation.pullRequest.actions.comments.addressAll`,
          defaultMessage: `Add all`,
          description: `Trailing label for the review comments row in the pull request actions dropdown`,
        })
      : void 0,
    actionDisabled: !n,
    disabled: i === 0,
    flyoutContent: q(e, n, r, a, o, c),
    icon: (0, H.jsx)(F, { className: `icon-sm shrink-0` }),
    key: `comments`,
    label:
      i > 0
        ? a.formatMessage(
            {
              id: `localConversation.pullRequest.actions.comments.count`,
              defaultMessage: `{count, plural, one {# comment} other {# comments}}`,
              description: `Label for the review comments row when comments are present`,
            },
            { count: i },
          )
        : a.formatMessage({
            id: `localConversation.pullRequest.actions.comments.none`,
            defaultMessage: `No comments`,
            description: `Label for the review comments row when there are no comments`,
          }),
    onSelect: u ? s : void 0,
    tooltipDisabled: n,
    tooltipText: l,
  };
}
function q(e, n, r, i, a, o) {
  if (r.length !== 0)
    return (0, H.jsx)(j, {
      groups: [
        {
          comments: r.map((r) => {
            let s = t(r),
              c = e.has(s);
            return {
              action: {
                color: c ? `ghost` : `primary`,
                disabled: !c && !n,
                label: c
                  ? i.formatMessage({
                      id: `localConversation.pullRequest.actions.comments.remove`,
                      defaultMessage: `Remove`,
                      description: `Action button shown on an attached pull request comment in a diff`,
                    })
                  : i.formatMessage({
                      id: `localConversation.pullRequest.actions.comments.address`,
                      defaultMessage: `Add to chat`,
                      description: `Action button shown on an individual review comment in the comments flyout`,
                    }),
                onSelect: () => {
                  if (c) {
                    o(r);
                    return;
                  }
                  a(r);
                },
              },
              comment: r,
            };
          }),
          title: i.formatMessage(
            {
              id: `localConversation.pullRequest.actions.comments.flyoutTitle`,
              defaultMessage: `{count, plural, one {# comment} other {# comments}}`,
              description: `Title for the pull request comments flyout in the git actions dropdown`,
            },
            { count: r.length },
          ),
        },
      ],
    });
}
function J({ flyoutContent: e, intl: t, reviewStatus: n }) {
  let r = e != null,
    i = r
      ? void 0
      : t.formatMessage({
          id: `localConversation.pullRequest.actions.noRequestedReviews`,
          defaultMessage: `No requested reviews`,
          description: `Tooltip shown when the approvals row is disabled because the pull request has no requested or completed reviews`,
        });
  switch (n) {
    case `approved`:
      return {
        disabled: !r,
        flyoutContent: e,
        icon: (0, H.jsx)(x, { className: `icon-sm shrink-0 text-token-description-foreground` }),
        key: `approvals-complete`,
        label: (0, H.jsx)(d, {
          id: `localConversation.pullRequest.actions.approvalsComplete`,
          defaultMessage: `Approved`,
          description: `Status row shown when the pull request has the required approvals`,
        }),
        tooltipText: i,
      };
    case `changes_requested`:
      return {
        disabled: !r,
        flyoutContent: e,
        icon: (0, H.jsx)(E, { className: `icon-sm shrink-0 text-token-charts-red` }),
        key: `changes-requested`,
        label: (0, H.jsx)(d, {
          id: `localConversation.pullRequest.actions.changesRequested`,
          defaultMessage: `Changes requested`,
          description: `Status row shown when reviewers requested changes on the pull request`,
        }),
        tooltipText: i,
      };
    case `review_required`:
      return {
        disabled: !r,
        flyoutContent: e,
        icon: (0, H.jsx)(S, { className: `icon-sm shrink-0 text-token-description-foreground` }),
        key: `awaiting-review`,
        label: (0, H.jsx)(d, {
          id: `localConversation.pullRequest.actions.awaitingReview`,
          defaultMessage: `Awaiting review`,
          description: `Status row shown when the pull request is still awaiting review`,
        }),
        tooltipText: i,
      };
    case `none`:
      return {
        disabled: !r,
        flyoutContent: e,
        icon: (0, H.jsx)(S, { className: `icon-sm shrink-0 text-token-description-foreground` }),
        key: `no-approvals`,
        label: (0, H.jsx)(d, {
          id: `localConversation.pullRequest.actions.noApprovals`,
          defaultMessage: `No approvals yet`,
          description: `Status row shown when the pull request has no approvals yet`,
        }),
        tooltipText: i,
      };
  }
}
function Y(e, t) {
  let n = ee(e, t);
  if (n.length !== 0)
    return (0, H.jsx)(A, {
      groups: n,
      title: t.formatMessage({
        id: `localConversation.pullRequest.actions.reviewers.title`,
        defaultMessage: `Approvals`,
        description: `Title shown at the top of the approvals flyout`,
      }),
    });
}
function ee(e, t) {
  return [
    X(`approved`, e.approved, t),
    X(`requested`, e.requested, t),
    X(`changes-requested`, e.changesRequested, t),
    X(
      `commented`,
      e.commented.filter(
        (t) =>
          !e.approved.includes(t) && !e.requested.includes(t) && !e.changesRequested.includes(t),
      ),
      t,
    ),
  ].filter((e) => e != null);
}
function X(e, t, n) {
  return t.length === 0
    ? null
    : { reviewers: t.map((t) => ({ accessoryLabel: te(e, n), icon: ne(e), label: t })) };
}
function te(e, t) {
  switch (e) {
    case `approved`:
      return t.formatMessage({
        id: `localConversation.pullRequest.actions.reviewers.approved`,
        defaultMessage: `Approved`,
        description: `Group title for approved reviewers in the approvals flyout`,
      });
    case `requested`:
      return t.formatMessage({
        id: `localConversation.pullRequest.actions.reviewers.requested`,
        defaultMessage: `Awaiting approval`,
        description: `Group title for requested reviewers in the approvals flyout`,
      });
    case `changes-requested`:
      return t.formatMessage({
        id: `localConversation.pullRequest.actions.reviewers.changesRequested`,
        defaultMessage: `Changes requested`,
        description: `Group title for reviewers who requested changes in the approvals flyout`,
      });
    case `commented`:
      return t.formatMessage({
        id: `localConversation.pullRequest.actions.reviewers.commented`,
        defaultMessage: `Commented`,
        description: `Group title for commenting reviewers in the approvals flyout`,
      });
  }
}
function ne(e) {
  switch (e) {
    case `approved`:
      return (0, H.jsx)(x, { className: `icon-sm shrink-0 text-token-description-foreground` });
    case `requested`:
      return (0, H.jsx)(S, { className: `icon-sm shrink-0 text-token-description-foreground` });
    case `commented`:
      return (0, H.jsx)(F, { className: `icon-sm shrink-0 text-token-description-foreground` });
    case `changes-requested`:
      return (0, H.jsx)(E, { className: `icon-sm shrink-0 text-token-charts-red` });
  }
}
var Z = s();
function re(e, t, n) {
  return [`vscode`, `gh-pr-merge`, e ?? null, t ?? null, n ?? null];
}
function ie(e) {
  let t = (0, Z.c)(20),
    { cwd: n, headBranch: r, hostId: a, operationSource: o, onSuccess: s } = e,
    u = i(),
    d;
  t[0] !== n || t[1] !== r || t[2] !== a
    ? ((d = re(n, r, a)), (t[0] = n), (t[1] = r), (t[2] = a), (t[3] = d))
    : (d = t[3]);
  let f;
  t[4] !== r || t[5] !== s || t[6] !== u
    ? ((f = (e, t) => {
        if (e.status === `success`) {
          if (r != null) {
            let e = (e) => {
              u.setQueryData(c(`gh-pr-status`, e), ae);
            };
            (e({ cwd: t.cwd, headBranch: r, hostId: t.hostId }),
              e({ cwd: t.cwd, headBranch: r, hostId: t.hostId, number: t.number, repo: t.repo }),
              Promise.all([
                u.invalidateQueries({ queryKey: c(`gh-pr-body`) }),
                u.invalidateQueries({ queryKey: c(`gh-pr-checks`) }),
                u.invalidateQueries({ queryKey: c(`gh-pr-comments`) }),
                u.invalidateQueries({
                  queryKey: c(`gh-pr-status`, { cwd: t.cwd, headBranch: r, hostId: t.hostId }),
                }),
                u.invalidateQueries({
                  queryKey: c(`gh-pr-status`, {
                    cwd: t.cwd,
                    headBranch: r,
                    hostId: t.hostId,
                    number: t.number,
                    repo: t.repo,
                  }),
                }),
              ]));
          }
          s?.(e, t);
        }
      }),
      (t[4] = r),
      (t[5] = s),
      (t[6] = u),
      (t[7] = f))
    : (f = t[7]);
  let p;
  t[8] !== o || t[9] !== d || t[10] !== f
    ? ((p = { source: o, mutationKey: d, onSuccess: f }),
      (t[8] = o),
      (t[9] = d),
      (t[10] = f),
      (t[11] = p))
    : (p = t[11]);
  let m = l(`gh-pr-merge`, p),
    h,
    g;
  t[12] !== a || t[13] !== m
    ? ((h = (e, t) => {
        m.mutate({ ...e, hostId: a }, t);
      }),
      (g = (e, t) => m.mutateAsync({ ...e, hostId: a }, t)),
      (t[12] = a),
      (t[13] = m),
      (t[14] = h),
      (t[15] = g))
    : ((h = t[14]), (g = t[15]));
  let _;
  return (
    t[16] !== m || t[17] !== h || t[18] !== g
      ? ((_ = { ...m, mutate: h, mutateAsync: g }),
        (t[16] = m),
        (t[17] = h),
        (t[18] = g),
        (t[19] = _))
      : (_ = t[19]),
    _
  );
}
function ae(e) {
  return e == null || e.status !== `success`
    ? e
    : {
        ...e,
        canMerge: !1,
        checks: [],
        ciStatus: `none`,
        commentAttachments: [],
        hasOpenPr: !1,
        isDraft: !1,
        mergeBlocker: null,
        reviewers: {
          approved: [],
          commentCounts: [],
          commented: [],
          changesRequested: [],
          requested: [],
          unresolvedCommentCount: 0,
        },
      };
}
var oe = (e) =>
  (0, H.jsx)(`svg`, {
    xmlns: `http://www.w3.org/2000/svg`,
    width: 20,
    height: 20,
    fill: `currentColor`,
    viewBox: `0 0 20 20`,
    ...e,
    children: (0, H.jsx)(`path`, {
      d: `M10 2.085a7.915 7.915 0 1 1 0 15.83 7.915 7.915 0 0 1 0-15.83Z`,
    }),
  });
function se(e, t) {
  let n = t.trim();
  return n.length === 0
    ? [...e].sort(ce)
    : e
        .map((e) => {
          let t = I(Q(e.displayPath).fileName, n);
          return { entry: e, score: t > 0 ? t : I(e.displayPath, n) };
        })
        .filter(({ score: e }) => e > 0)
        .sort((e, t) => t.score - e.score)
        .map(({ entry: e }) => e);
}
function ce(e, t) {
  let n = Q(e.displayPath),
    r = Q(t.displayPath);
  return n.fileName.localeCompare(r.fileName) || n.parentPath.localeCompare(r.parentPath);
}
function Q(e) {
  let t = e.lastIndexOf(`/`);
  return t === -1
    ? { fileName: e, parentPath: `` }
    : { fileName: e.slice(t + 1), parentPath: e.slice(0, t) };
}
function le() {
  let e = (0, Z.c)(5),
    t = a(p),
    n = o(g),
    r;
  e[0] === t ? (r = e[1]) : ((r = (e) => _(t, e)), (e[0] = t), (e[1] = r));
  let i;
  return (
    e[2] !== n || e[3] !== r
      ? ((i = (0, H.jsx)(ue, { entries: n, onSelectPath: r })), (e[2] = n), (e[3] = r), (e[4] = i))
      : (i = e[4]),
    i
  );
}
function ue(e) {
  let t = (0, Z.c)(19),
    { entries: n, onSelectPath: r } = e,
    i = u(),
    [a, o] = (0, V.useState)(``),
    [s, c] = (0, V.useState)(!1),
    l;
  if (t[0] !== n || t[1] !== i || t[2] !== r || t[3] !== s || t[4] !== a) {
    let e = se(n, a),
      u;
    t[6] === i
      ? (u = t[7])
      : ((u = i.formatMessage({
          id: `codex.review.jumpToFile`,
          defaultMessage: `Jump to file`,
          description: `Button label for opening diff file jump search`,
        })),
        (t[6] = i),
        (t[7] = u));
    let p = u,
      h;
    t[8] === Symbol.for(`react.memo_cache_sentinel`)
      ? ((h = (e) => {
          (e && o(``), c(e));
        }),
        (t[8] = h))
      : (h = t[8]);
    let g;
    t[9] === Symbol.for(`react.memo_cache_sentinel`)
      ? ((g = (0, H.jsx)(B, { className: `icon-xs text-token-description-foreground` })),
        (t[9] = g))
      : (g = t[9]);
    let _;
    t[10] === p
      ? (_ = t[11])
      : ((_ = (0, H.jsx)(m, {
          "aria-label": p,
          color: `ghost`,
          size: `toolbar`,
          uniform: !0,
          children: g,
        })),
        (t[10] = p),
        (t[11] = _));
    let v;
    t[12] !== p || t[13] !== _
      ? ((v = (0, H.jsx)(f, { tooltipContent: p, delayOpen: !0, children: _ })),
        (t[12] = p),
        (t[13] = _),
        (t[14] = v))
      : (v = t[14]);
    let y;
    t[15] === Symbol.for(`react.memo_cache_sentinel`)
      ? ((y = (e) => o(e.currentTarget.value)), (t[15] = y))
      : (y = t[15]);
    let b;
    (t[16] !== p || t[17] !== a
      ? ((b = (0, H.jsx)(C.SearchInput, { autoFocus: !0, placeholder: p, value: a, onChange: y })),
        (t[16] = p),
        (t[17] = a),
        (t[18] = b))
      : (b = t[18]),
      (l = (0, H.jsxs)(w, {
        open: s,
        onOpenChange: h,
        align: `end`,
        contentWidth: `panelWide`,
        contentMaxHeight: `list`,
        triggerButton: v,
        children: [
          b,
          e.length === 0
            ? (0, H.jsx)(C.Message, {
                compact: !0,
                children: (0, H.jsx)(d, {
                  id: `codex.review.jumpToFile.empty`,
                  defaultMessage: `No matching files`,
                  description: `Empty state for review file jump search`,
                }),
              })
            : e.map((e) =>
                (0, H.jsx)(
                  C.Item,
                  {
                    allowWrap: !0,
                    onSelect: () => {
                      (r(e.path), c(!1));
                    },
                    children: (0, H.jsx)(de, { displayPath: e.displayPath }),
                  },
                  e.path,
                ),
              ),
        ],
      })),
      (t[0] = n),
      (t[1] = i),
      (t[2] = r),
      (t[3] = s),
      (t[4] = a),
      (t[5] = l));
  } else l = t[5];
  return l;
}
function de(e) {
  let t = (0, Z.c)(9),
    { displayPath: n } = e,
    r;
  t[0] === n ? (r = t[1]) : ((r = Q(n)), (t[0] = n), (t[1] = r));
  let { fileName: i, parentPath: a } = r,
    o;
  t[2] === i
    ? (o = t[3])
    : ((o = (0, H.jsx)(`span`, { className: `shrink-0 text-token-foreground`, children: i })),
      (t[2] = i),
      (t[3] = o));
  let s;
  t[4] === a
    ? (s = t[5])
    : ((s =
        a.length > 0
          ? (0, H.jsx)(z, {
              className: `min-w-0 flex-1 text-token-description-foreground`,
              text: a,
            })
          : null),
      (t[4] = a),
      (t[5] = s));
  let c;
  return (
    t[6] !== o || t[7] !== s
      ? ((c = (0, H.jsxs)(`span`, {
          className: `flex min-w-0 items-center gap-2`,
          children: [o, s],
        })),
        (t[6] = o),
        (t[7] = s),
        (t[8] = c))
      : (c = t[8]),
    c
  );
}
var fe = (e) =>
    (0, H.jsx)(`svg`, {
      width: 20,
      height: 20,
      viewBox: `0 0 20 20`,
      fill: `currentColor`,
      xmlns: `http://www.w3.org/2000/svg`,
      ...e,
      children: (0, H.jsx)(`path`, {
        d: `M11.8352 4.58331C11.8352 3.56995 11.0136 2.74848 10.0002 2.74835C8.9868 2.74835 8.16528 3.56987 8.16528 4.58331C8.16528 4.67625 8.24033 4.75123 8.33325 4.75128H11.6663C11.7592 4.75128 11.8352 4.67628 11.8352 4.58331ZM16.7063 13.0833C16.7063 13.7724 16.707 14.3294 16.6702 14.7796C16.6374 15.1802 16.5731 15.541 16.429 15.8773L16.3616 16.0198C16.096 16.541 15.6924 16.9775 15.1965 17.2816L14.9788 17.4036C14.602 17.5956 14.1956 17.6748 13.7376 17.7122C13.2874 17.749 12.7302 17.7484 12.0413 17.7484H7.95825C7.26916 17.7484 6.71221 17.749 6.26196 17.7122C5.86132 17.6795 5.5006 17.6143 5.16431 17.47L5.02173 17.4036C4.50069 17.1381 4.06411 16.7343 3.76001 16.2386L3.63794 16.0198C3.44614 15.6432 3.36676 15.2374 3.32935 14.7796C3.29256 14.3294 3.29321 13.7724 3.29321 13.0833V7.4339C3.29321 6.89108 3.2926 6.45196 3.31568 6.09503C3.33912 5.73276 3.38938 5.40917 3.51001 5.09991C3.83166 4.27546 4.48341 3.62273 5.30786 3.30109L5.54321 3.22394C5.78184 3.15798 6.03209 3.12531 6.30396 3.10773C6.56114 3.0911 6.86075 3.08745 7.21118 3.08624C7.74527 2.0934 8.79389 1.41827 10.0002 1.41827C11.2064 1.41836 12.2542 2.09359 12.7883 3.08624C13.139 3.08744 13.4392 3.09109 13.6965 3.10773C14.0587 3.13116 14.3825 3.18053 14.6917 3.30109L14.844 3.36554C15.5964 3.7052 16.1889 4.32679 16.4905 5.09991L16.5686 5.33527C16.6344 5.5736 16.6663 5.82355 16.6838 6.09503C16.7069 6.45195 16.7063 6.89108 16.7063 7.4339V13.0833ZM13.1653 4.58331C13.1653 5.41082 12.4938 6.08136 11.6663 6.08136H8.33325C7.50579 6.08131 6.83521 5.41079 6.83521 4.58331C6.83521 4.52829 6.83634 4.47359 6.83911 4.41925C6.66887 4.42205 6.52147 4.42638 6.38989 4.43488C6.16579 4.44937 6.01318 4.47293 5.89673 4.50519L5.79126 4.54034C5.31326 4.72683 4.93477 5.10531 4.74829 5.58331C4.69813 5.71204 4.66211 5.88293 4.64282 6.18097C4.62316 6.48503 4.62329 6.87387 4.62329 7.4339V13.0833C4.62329 13.7944 4.62418 14.2877 4.65552 14.6712C4.68622 15.0469 4.74298 15.2583 4.82349 15.4163L4.8938 15.5423C5.07011 15.8299 5.32299 16.0641 5.62525 16.2181L5.75513 16.2747C5.89888 16.3262 6.08851 16.363 6.37036 16.386C6.75388 16.4174 7.24721 16.4183 7.95825 16.4183H12.0413C12.7521 16.4183 13.2457 16.4174 13.6292 16.386C14.0047 16.3554 14.2163 16.2985 14.3743 16.2181L14.5012 16.1478C14.7887 15.9715 15.0231 15.7185 15.177 15.4163L15.2327 15.2864C15.2841 15.1427 15.3219 14.9532 15.345 14.6712C15.3763 14.2877 15.3762 13.7944 15.3762 13.0833V7.4339C15.3762 6.87387 15.3764 6.48503 15.3567 6.18097C15.3422 5.95722 15.3186 5.80513 15.2864 5.68878L15.2512 5.58331C15.0881 5.16509 14.7781 4.8233 14.3831 4.61945L14.2083 4.54034C14.0795 4.49017 13.9087 4.45416 13.6106 4.43488C13.4788 4.42636 13.331 4.42205 13.1604 4.41925C13.1632 4.47359 13.1653 4.52828 13.1653 4.58331Z`,
      }),
    }),
  pe = (e) =>
    (0, H.jsxs)(`svg`, {
      width: 24,
      height: 24,
      viewBox: `0 0 24 24`,
      fill: `none`,
      xmlns: `http://www.w3.org/2000/svg`,
      ...e,
      children: [
        (0, H.jsx)(`path`, {
          d: `M12.8 8L19.8 8`,
          stroke: `currentColor`,
          strokeWidth: 2,
          strokeLinecap: `round`,
        }),
        (0, H.jsx)(`path`, {
          d: `M12.8 12L19.8 12.002`,
          stroke: `currentColor`,
          strokeWidth: 2,
          strokeLinecap: `round`,
        }),
        (0, H.jsx)(`path`, {
          d: `M12.8 16L16.8 16`,
          stroke: `currentColor`,
          strokeWidth: 2,
          strokeLinecap: `round`,
        }),
        (0, H.jsx)(`path`, {
          d: `M6.24013 6V18`,
          stroke: `currentColor`,
          strokeWidth: 1.8,
          strokeLinecap: `round`,
          strokeLinejoin: `round`,
        }),
        (0, H.jsx)(`path`, {
          d: `M3.74022 7.50012L6.24022 5.00012L8.74022 7.50012`,
          stroke: `currentColor`,
          strokeWidth: 2,
          strokeLinecap: `round`,
          strokeLinejoin: `round`,
        }),
        (0, H.jsx)(`path`, {
          d: `M8.74022 16.5L6.24022 19L3.74022 16.5`,
          stroke: `currentColor`,
          strokeWidth: 2,
          strokeLinecap: `round`,
          strokeLinejoin: `round`,
        }),
      ],
    }),
  me = (e) =>
    (0, H.jsxs)(`svg`, {
      width: 24,
      height: 24,
      viewBox: `0 0 24 24`,
      fill: `none`,
      xmlns: `http://www.w3.org/2000/svg`,
      ...e,
      children: [
        (0, H.jsx)(`path`, {
          d: `M13 9.50004L20 9.5`,
          stroke: `currentColor`,
          strokeWidth: 2,
          strokeLinecap: `round`,
        }),
        (0, H.jsx)(`path`, {
          d: `M13 14.5L17 14.5`,
          stroke: `currentColor`,
          strokeWidth: 2,
          strokeLinecap: `round`,
        }),
        (0, H.jsx)(`path`, {
          d: `M6.23999 4V9`,
          stroke: `currentColor`,
          strokeWidth: 1.8,
          strokeLinecap: `round`,
          strokeLinejoin: `round`,
        }),
        (0, H.jsx)(`path`, {
          d: `M6.23999 15V20`,
          stroke: `currentColor`,
          strokeWidth: 1.8,
          strokeLinecap: `round`,
          strokeLinejoin: `round`,
        }),
        (0, H.jsx)(`path`, {
          d: `M3.74009 16.5L6.24009 14L8.74009 16.5`,
          stroke: `currentColor`,
          strokeWidth: 2,
          strokeLinecap: `round`,
          strokeLinejoin: `round`,
        }),
        (0, H.jsx)(`path`, {
          d: `M8.74009 7.5L6.24009 10L3.74009 7.5`,
          stroke: `currentColor`,
          strokeWidth: 2,
          strokeLinecap: `round`,
          strokeLinejoin: `round`,
        }),
      ],
    }),
  he = (e) =>
    (0, H.jsx)(`svg`, {
      xmlns: `http://www.w3.org/2000/svg`,
      width: 20,
      height: 20,
      fill: `currentColor`,
      viewBox: `0 0 20 20`,
      ...e,
      children: (0, H.jsx)(`path`, {
        fill: `currentColor`,
        d: `M14.375 9.502a3.165 3.165 0 0 1 0 6.33h-2.77l.949.948a.666.666 0 0 1-.942.94L9.53 15.639a.667.667 0 0 1 0-.942l2.083-2.083a.666.666 0 0 1 .942.94l-.949.949h2.77a1.836 1.836 0 0 0 0-3.67H3.333a.666.666 0 0 1 0-1.33h11.042Zm-7.709 5a.665.665 0 1 1 0 1.33H3.333a.666.666 0 0 1 0-1.33h3.333Zm10-10a.665.665 0 1 1 0 1.33H3.333a.666.666 0 0 1 0-1.33h13.333Z`,
      }),
    });
function ge(e) {
  let t = (0, Z.c)(57),
    {
      copyGitApplyCommandDisabled: n,
      diffControls: r,
      onClickCopyGitApplyCommand: i,
      onRefreshGitQueries: a,
      refreshGitQueriesDisabled: o,
      showCopyGitApplyCommand: s,
      showHideWhitespace: c,
      showLoadFullFiles: l,
      showRefreshGitQueries: d,
    } = e,
    p = c === void 0 ? !0 : c,
    h = l === void 0 ? !0 : l,
    g = u(),
    [_, v] = (0, V.useState)(!1),
    y;
  t[0] === g
    ? (y = t[1])
    : ((y = g.formatMessage({
        id: `codex.review.refreshGitQueries`,
        defaultMessage: `Refresh`,
        description: `Menu item to refresh git queries for the current repository`,
      })),
      (t[0] = g),
      (t[1] = y));
  let b = y,
    x;
  t[2] !== r.wrap || t[3] !== g
    ? ((x = r.wrap
        ? g.formatMessage({
            id: `codex.review.wrap.disable`,
            defaultMessage: `Disable word wrap`,
            description: `Menu item to disable word wrap in diff view`,
          })
        : g.formatMessage({
            id: `codex.review.wrap.enable`,
            defaultMessage: `Enable word wrap`,
            description: `Menu item to enable word wrap in diff view`,
          })),
      (t[2] = r.wrap),
      (t[3] = g),
      (t[4] = x))
    : (x = t[4]);
  let S = x,
    C;
  t[5] !== r.expanded || t[6] !== g
    ? ((C = r.expanded
        ? g.formatMessage({
            id: `codex.review.expandOrCollapseDiffMenu.collapse`,
            defaultMessage: `Collapse all diffs`,
            description: `Menu item to collapse all diffs in the review options menu`,
          })
        : g.formatMessage({
            id: `codex.review.expandOrCollapseDiffMenu.expand`,
            defaultMessage: `Expand all diffs`,
            description: `Menu item to expand all diffs in the review options menu`,
          })),
      (t[5] = r.expanded),
      (t[6] = g),
      (t[7] = C))
    : (C = t[7]);
  let E = C,
    D;
  t[8] === g
    ? (D = t[9])
    : ((D = g.formatMessage({
        id: `codex.review.header.moreOptions`,
        defaultMessage: `Review options`,
        description: `Aria label for review header dropdown menu button`,
      })),
      (t[8] = g),
      (t[9] = D));
  let O = D,
    k;
  t[10] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((k = () => {
        v(!1);
      }),
      (t[10] = k))
    : (k = t[10]);
  let A = k,
    j;
  t[11] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((j = (0, H.jsx)(T, { className: `icon-xs text-token-description-foreground` })), (t[11] = j))
    : (j = t[11]);
  let F;
  t[12] === O
    ? (F = t[13])
    : ((F = (0, H.jsx)(m, {
        "aria-label": O,
        color: `ghost`,
        size: `toolbar`,
        uniform: !0,
        children: j,
      })),
      (t[12] = O),
      (t[13] = F));
  let I;
  t[14] !== O || t[15] !== F
    ? ((I = (0, H.jsx)(f, { tooltipContent: O, delayOpen: !0, children: F })),
      (t[14] = O),
      (t[15] = F),
      (t[16] = I))
    : (I = t[16]);
  let L;
  t[17] !== n ||
  t[18] !== r.hideWhitespace ||
  t[19] !== r.loadFullFilesEnabled ||
  t[20] !== r.onToggleHideWhitespace ||
  t[21] !== r.onToggleLoadFullFiles ||
  t[22] !== r.onToggleRichPreview ||
  t[23] !== r.onToggleWordDiffs ||
  t[24] !== r.richPreviewEnabled ||
  t[25] !== r.wordDiffsEnabled ||
  t[26] !== i ||
  t[27] !== s ||
  t[28] !== p ||
  t[29] !== h
    ? ((L = (0, H.jsx)(ye, {
        onClose: A,
        wordDiffsEnabled: r.wordDiffsEnabled,
        showCopyGitApplyCommand: s,
        showHideWhitespace: p,
        showLoadFullFiles: h,
        copyGitApplyCommandDisabled: n,
        onClickCopyGitApplyCommand: i,
        richPreviewEnabled: r.richPreviewEnabled,
        onToggleRichPreview: r.onToggleRichPreview,
        onToggleWordDiffs: r.onToggleWordDiffs,
        hideWhitespace: r.hideWhitespace,
        onToggleHideWhitespace: r.onToggleHideWhitespace,
        loadFullFilesEnabled: r.loadFullFilesEnabled,
        onToggleLoadFullFiles: r.onToggleLoadFullFiles,
      })),
      (t[17] = n),
      (t[18] = r.hideWhitespace),
      (t[19] = r.loadFullFilesEnabled),
      (t[20] = r.onToggleHideWhitespace),
      (t[21] = r.onToggleLoadFullFiles),
      (t[22] = r.onToggleRichPreview),
      (t[23] = r.onToggleWordDiffs),
      (t[24] = r.richPreviewEnabled),
      (t[25] = r.wordDiffsEnabled),
      (t[26] = i),
      (t[27] = s),
      (t[28] = p),
      (t[29] = h),
      (t[30] = L))
    : (L = t[30]);
  let R;
  t[31] !== _ || t[32] !== I || t[33] !== L
    ? ((R = (0, H.jsx)(w, {
        open: _,
        onOpenChange: v,
        align: `end`,
        contentWidth: `menu`,
        triggerButton: I,
        children: L,
      })),
      (t[31] = _),
      (t[32] = I),
      (t[33] = L),
      (t[34] = R))
    : (R = t[34]);
  let z;
  t[35] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((z = (0, H.jsx)(le, {})), (t[35] = z))
    : (z = t[35]);
  let B;
  t[36] !== a || t[37] !== o || t[38] !== b || t[39] !== d
    ? ((B = d
        ? (0, H.jsx)($, { Icon: P, label: b, onClick: () => a?.(), disabled: o || !a })
        : null),
      (t[36] = a),
      (t[37] = o),
      (t[38] = b),
      (t[39] = d),
      (t[40] = B))
    : (B = t[40]);
  let U = r.wrap ? M : N,
    W;
  t[41] !== r.onToggleWrap || t[42] !== U || t[43] !== S
    ? ((W = (0, H.jsx)($, { Icon: U, label: S, onClick: r.onToggleWrap })),
      (t[41] = r.onToggleWrap),
      (t[42] = U),
      (t[43] = S),
      (t[44] = W))
    : (W = t[44]);
  let G = r.expanded ? me : pe,
    K;
  t[45] !== r.onToggleExpanded || t[46] !== E || t[47] !== G
    ? ((K = (0, H.jsx)($, { Icon: G, label: E, onClick: r.onToggleExpanded })),
      (t[45] = r.onToggleExpanded),
      (t[46] = E),
      (t[47] = G),
      (t[48] = K))
    : (K = t[48]);
  let q;
  t[49] === r ? (q = t[50]) : ((q = (0, H.jsx)(ve, { diffControls: r })), (t[49] = r), (t[50] = q));
  let J;
  return (
    t[51] !== R || t[52] !== B || t[53] !== W || t[54] !== K || t[55] !== q
      ? ((J = (0, H.jsxs)(`div`, {
          className: `flex items-center gap-1.5`,
          children: [R, z, B, W, K, q],
        })),
        (t[51] = R),
        (t[52] = B),
        (t[53] = W),
        (t[54] = K),
        (t[55] = q),
        (t[56] = J))
      : (J = t[56]),
    J
  );
}
function _e(e) {
  let t = (0, Z.c)(56),
    {
      copyGitApplyCommandDisabled: n,
      diffControls: r,
      onClickCopyGitApplyCommand: i,
      onRefreshGitQueries: a,
      refreshGitQueriesDisabled: o,
      showCopyGitApplyCommand: s,
      showHideWhitespace: c,
      showLoadFullFiles: l,
      showRefreshGitQueries: d,
    } = e,
    p = c === void 0 ? !0 : c,
    h = l === void 0 ? !0 : l,
    g = u(),
    [_, v] = (0, V.useState)(!1),
    y;
  t[0] === g
    ? (y = t[1])
    : ((y = g.formatMessage({
        id: `codex.review.refreshGitQueries`,
        defaultMessage: `Refresh`,
        description: `Menu item to refresh git queries for the current repository`,
      })),
      (t[0] = g),
      (t[1] = y));
  let b = y,
    x;
  t[2] !== r.wrap || t[3] !== g
    ? ((x = r.wrap
        ? g.formatMessage({
            id: `codex.review.wrap.disable`,
            defaultMessage: `Disable word wrap`,
            description: `Menu item to disable word wrap in diff view`,
          })
        : g.formatMessage({
            id: `codex.review.wrap.enable`,
            defaultMessage: `Enable word wrap`,
            description: `Menu item to enable word wrap in diff view`,
          })),
      (t[2] = r.wrap),
      (t[3] = g),
      (t[4] = x))
    : (x = t[4]);
  let S = x,
    E;
  t[5] !== r.expanded || t[6] !== g
    ? ((E = r.expanded
        ? g.formatMessage({
            id: `codex.review.expandOrCollapseDiffMenu.collapse`,
            defaultMessage: `Collapse all diffs`,
            description: `Menu item to collapse all diffs in the review options menu`,
          })
        : g.formatMessage({
            id: `codex.review.expandOrCollapseDiffMenu.expand`,
            defaultMessage: `Expand all diffs`,
            description: `Menu item to expand all diffs in the review options menu`,
          })),
      (t[5] = r.expanded),
      (t[6] = g),
      (t[7] = E))
    : (E = t[7]);
  let D = E,
    O;
  t[8] === g
    ? (O = t[9])
    : ((O = g.formatMessage({
        id: `codex.review.header.moreOptions`,
        defaultMessage: `Review options`,
        description: `Aria label for review header dropdown menu button`,
      })),
      (t[8] = g),
      (t[9] = O));
  let k = O,
    A;
  t[10] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((A = () => {
        v(!1);
      }),
      (t[10] = A))
    : (A = t[10]);
  let j = A,
    F;
  t[11] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((F = (0, H.jsx)(T, { className: `icon-xs` })), (t[11] = F))
    : (F = t[11]);
  let I;
  t[12] === k
    ? (I = t[13])
    : ((I = (0, H.jsx)(m, {
        "aria-label": k,
        color: `ghost`,
        size: `toolbar`,
        uniform: !0,
        children: F,
      })),
      (t[12] = k),
      (t[13] = I));
  let L;
  t[14] !== k || t[15] !== I
    ? ((L = (0, H.jsx)(f, { tooltipContent: k, delayOpen: !0, children: I })),
      (t[14] = k),
      (t[15] = I),
      (t[16] = L))
    : (L = t[16]);
  let R;
  t[17] !== a || t[18] !== o || t[19] !== b || t[20] !== d
    ? ((R = d
        ? (0, H.jsx)(C.Item, {
            onSelect: (e) => {
              (e.preventDefault(), a?.(), j());
            },
            LeftIcon: P,
            disabled: o || !a,
            children: b,
          })
        : null),
      (t[17] = a),
      (t[18] = o),
      (t[19] = b),
      (t[20] = d),
      (t[21] = R))
    : (R = t[21]);
  let z;
  t[22] === r
    ? (z = t[23])
    : ((z = (e) => {
        (e.preventDefault(), r.onToggleWrap(), j());
      }),
      (t[22] = r),
      (t[23] = z));
  let B = r.wrap ? M : N,
    U;
  t[24] !== z || t[25] !== B || t[26] !== S
    ? ((U = (0, H.jsx)(C.Item, { onSelect: z, LeftIcon: B, children: S })),
      (t[24] = z),
      (t[25] = B),
      (t[26] = S),
      (t[27] = U))
    : (U = t[27]);
  let W;
  t[28] === r
    ? (W = t[29])
    : ((W = (e) => {
        (e.preventDefault(), r.onToggleExpanded(), j());
      }),
      (t[28] = r),
      (t[29] = W));
  let G = r.expanded ? me : pe,
    K;
  t[30] !== D || t[31] !== W || t[32] !== G
    ? ((K = (0, H.jsx)(C.Item, { onSelect: W, LeftIcon: G, children: D })),
      (t[30] = D),
      (t[31] = W),
      (t[32] = G),
      (t[33] = K))
    : (K = t[33]);
  let q;
  t[34] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((q = (0, H.jsx)(C.Separator, {})), (t[34] = q))
    : (q = t[34]);
  let J;
  t[35] !== n ||
  t[36] !== r.hideWhitespace ||
  t[37] !== r.loadFullFilesEnabled ||
  t[38] !== r.onToggleHideWhitespace ||
  t[39] !== r.onToggleLoadFullFiles ||
  t[40] !== r.onToggleRichPreview ||
  t[41] !== r.onToggleWordDiffs ||
  t[42] !== r.richPreviewEnabled ||
  t[43] !== r.wordDiffsEnabled ||
  t[44] !== i ||
  t[45] !== s ||
  t[46] !== p ||
  t[47] !== h
    ? ((J = (0, H.jsx)(ye, {
        onClose: j,
        wordDiffsEnabled: r.wordDiffsEnabled,
        showCopyGitApplyCommand: s,
        showHideWhitespace: p,
        showLoadFullFiles: h,
        copyGitApplyCommandDisabled: n,
        onClickCopyGitApplyCommand: i,
        richPreviewEnabled: r.richPreviewEnabled,
        onToggleRichPreview: r.onToggleRichPreview,
        onToggleWordDiffs: r.onToggleWordDiffs,
        hideWhitespace: r.hideWhitespace,
        onToggleHideWhitespace: r.onToggleHideWhitespace,
        loadFullFilesEnabled: r.loadFullFilesEnabled,
        onToggleLoadFullFiles: r.onToggleLoadFullFiles,
      })),
      (t[35] = n),
      (t[36] = r.hideWhitespace),
      (t[37] = r.loadFullFilesEnabled),
      (t[38] = r.onToggleHideWhitespace),
      (t[39] = r.onToggleLoadFullFiles),
      (t[40] = r.onToggleRichPreview),
      (t[41] = r.onToggleWordDiffs),
      (t[42] = r.richPreviewEnabled),
      (t[43] = r.wordDiffsEnabled),
      (t[44] = i),
      (t[45] = s),
      (t[46] = p),
      (t[47] = h),
      (t[48] = J))
    : (J = t[48]);
  let Y;
  return (
    t[49] !== _ || t[50] !== L || t[51] !== R || t[52] !== U || t[53] !== K || t[54] !== J
      ? ((Y = (0, H.jsxs)(w, {
          open: _,
          onOpenChange: v,
          align: `end`,
          contentWidth: `menu`,
          triggerButton: L,
          children: [R, U, K, q, J],
        })),
        (t[49] = _),
        (t[50] = L),
        (t[51] = R),
        (t[52] = U),
        (t[53] = K),
        (t[54] = J),
        (t[55] = Y))
      : (Y = t[55]),
    Y
  );
}
function ve(e) {
  let t = (0, Z.c)(9),
    { diffControls: n } = e,
    r = u(),
    i;
  t[0] !== n.diffMode || t[1] !== r
    ? ((i =
        n.diffMode === `unified`
          ? r.formatMessage({
              id: `codex.review.switchToSplit`,
              defaultMessage: `Switch to split diff`,
              description: `Button label to switch to split diff view`,
            })
          : r.formatMessage({
              id: `codex.review.switchToUnified`,
              defaultMessage: `Switch to unified diff`,
              description: `Button label to switch to unified diff view`,
            })),
      (t[0] = n.diffMode),
      (t[1] = r),
      (t[2] = i))
    : (i = t[2]);
  let a = i,
    o = n.diffMode === `unified` ? L : R,
    s;
  t[3] === n
    ? (s = t[4])
    : ((s = () => n.onSelectDiffMode(n.diffMode === `unified` ? `right` : `left`)),
      (t[3] = n),
      (t[4] = s));
  let c;
  return (
    t[5] !== a || t[6] !== o || t[7] !== s
      ? ((c = (0, H.jsx)($, { Icon: o, label: a, onClick: s })),
        (t[5] = a),
        (t[6] = o),
        (t[7] = s),
        (t[8] = c))
      : (c = t[8]),
    c
  );
}
function $(e) {
  let t = (0, Z.c)(12),
    { Icon: n, label: r, onClick: i, disabled: a, pressed: o } = e,
    s = a === void 0 ? !1 : a,
    c = o === void 0 ? !1 : o,
    l = c ? `ghostActive` : `ghost`,
    u;
  t[0] === n ? (u = t[1]) : ((u = (0, H.jsx)(n, { className: `icon-xs` })), (t[0] = n), (t[1] = u));
  let d;
  t[2] !== s || t[3] !== r || t[4] !== i || t[5] !== c || t[6] !== l || t[7] !== u
    ? ((d = (0, H.jsx)(m, {
        "aria-label": r,
        "aria-pressed": c,
        color: l,
        size: `toolbar`,
        uniform: !0,
        onClick: i,
        disabled: s,
        children: u,
      })),
      (t[2] = s),
      (t[3] = r),
      (t[4] = i),
      (t[5] = c),
      (t[6] = l),
      (t[7] = u),
      (t[8] = d))
    : (d = t[8]);
  let p;
  return (
    t[9] !== r || t[10] !== d
      ? ((p = (0, H.jsx)(f, { tooltipContent: r, children: d })),
        (t[9] = r),
        (t[10] = d),
        (t[11] = p))
      : (p = t[11]),
    p
  );
}
function ye(e) {
  let t = (0, Z.c)(50),
    {
      onClose: n,
      wordDiffsEnabled: r,
      showCopyGitApplyCommand: i,
      showHideWhitespace: a,
      showLoadFullFiles: o,
      copyGitApplyCommandDisabled: s,
      onClickCopyGitApplyCommand: c,
      richPreviewEnabled: l,
      onToggleRichPreview: f,
      onToggleWordDiffs: p,
      hideWhitespace: m,
      onToggleHideWhitespace: h,
      loadFullFilesEnabled: g,
      onToggleLoadFullFiles: _,
    } = e,
    x = a === void 0 ? !0 : a,
    S = o === void 0 ? !0 : o,
    w = u(),
    T;
  t[0] !== w || t[1] !== l
    ? ((T = l
        ? w.formatMessage({
            id: `codex.diffView.richPreviewDisable`,
            defaultMessage: `Disable rich preview`,
            description: `Aria label for disabling rich previews in the diff view`,
          })
        : w.formatMessage({
            id: `codex.diffView.richPreviewEnable`,
            defaultMessage: `Enable rich preview`,
            description: `Aria label for enabling rich previews in the diff view`,
          })),
      (t[0] = w),
      (t[1] = l),
      (t[2] = T))
    : (T = t[2]);
  let E = T,
    D;
  t[3] !== n || t[4] !== f
    ? ((D = (e) => {
        (e.preventDefault(), f(), n());
      }),
      (t[3] = n),
      (t[4] = f),
      (t[5] = D))
    : (D = t[5]);
  let O = D,
    k;
  t[6] !== n || t[7] !== p
    ? ((k = (e) => {
        (e.preventDefault(), p(), n());
      }),
      (t[6] = n),
      (t[7] = p),
      (t[8] = k))
    : (k = t[8]);
  let A = k,
    j;
  t[9] !== n || t[10] !== h
    ? ((j = (e) => {
        (e.preventDefault(), h(), n());
      }),
      (t[9] = n),
      (t[10] = h),
      (t[11] = j))
    : (j = t[11]);
  let M = j,
    N;
  t[12] !== n || t[13] !== _
    ? ((N = (e) => {
        (e.preventDefault(), _(), n());
      }),
      (t[12] = n),
      (t[13] = _),
      (t[14] = N))
    : (N = t[14]);
  let P = N,
    F;
  t[15] !== c || t[16] !== n
    ? ((F = (e) => {
        (e.preventDefault(), c?.(), n());
      }),
      (t[15] = c),
      (t[16] = n),
      (t[17] = F))
    : (F = t[17]);
  let I = F,
    L;
  t[18] !== P || t[19] !== g || t[20] !== S
    ? ((L = S
        ? (0, H.jsx)(C.Item, {
            onSelect: P,
            LeftIcon: v,
            children: g
              ? (0, H.jsx)(d, {
                  id: `codex.review.loadFullFiles.disable`,
                  defaultMessage: `Don't load full files`,
                  description: `Menu item to avoid loading full file contents for partial diffs`,
                })
              : (0, H.jsx)(d, {
                  id: `codex.review.loadFullFiles.enable`,
                  defaultMessage: `Load full files`,
                  description: `Menu item to load full file contents for partial diffs`,
                }),
          })
        : null),
      (t[18] = P),
      (t[19] = g),
      (t[20] = S),
      (t[21] = L))
    : (L = t[21]);
  let R = l ? y : b,
    z;
  t[22] === l
    ? (z = t[23])
    : ((z = l
        ? (0, H.jsx)(d, {
            id: `codex.review.richPreview.disable`,
            defaultMessage: `Disable rich preview`,
            description: `Menu item to disable rich previews in the diff view`,
          })
        : (0, H.jsx)(d, {
            id: `codex.review.richPreview.enable`,
            defaultMessage: `Enable rich preview`,
            description: `Menu item to enable rich previews in the diff view`,
          })),
      (t[22] = l),
      (t[23] = z));
  let B;
  t[24] !== O || t[25] !== E || t[26] !== R || t[27] !== z
    ? ((B = (0, H.jsx)(C.Item, { onSelect: O, "aria-label": E, LeftIcon: R, children: z })),
      (t[24] = O),
      (t[25] = E),
      (t[26] = R),
      (t[27] = z),
      (t[28] = B))
    : (B = t[28]);
  let V = r ? U : he,
    G;
  t[29] === r
    ? (G = t[30])
    : ((G = r
        ? (0, H.jsx)(d, {
            id: `codex.review.wordDiffs.disable`,
            defaultMessage: `Disable word diffs`,
            description: `Menu item to disable word-level diff highlights`,
          })
        : (0, H.jsx)(d, {
            id: `codex.review.wordDiffs.enable`,
            defaultMessage: `Enable word diffs`,
            description: `Menu item to enable word-level diff highlights`,
          })),
      (t[29] = r),
      (t[30] = G));
  let K;
  t[31] !== A || t[32] !== V || t[33] !== G
    ? ((K = (0, H.jsx)(C.Item, { onSelect: A, LeftIcon: V, children: G })),
      (t[31] = A),
      (t[32] = V),
      (t[33] = G),
      (t[34] = K))
    : (K = t[34]);
  let q;
  t[35] !== M || t[36] !== m || t[37] !== x
    ? ((q = x
        ? (0, H.jsx)(C.Item, {
            onSelect: M,
            LeftIcon: W,
            children: m
              ? (0, H.jsx)(d, {
                  id: `codex.review.whitespace.show`,
                  defaultMessage: `Show white space`,
                  description: `Menu item to show whitespace-only diff changes`,
                })
              : (0, H.jsx)(d, {
                  id: `codex.review.whitespace.hide`,
                  defaultMessage: `Hide white space`,
                  description: `Menu item to hide whitespace-only diff changes`,
                }),
          })
        : null),
      (t[35] = M),
      (t[36] = m),
      (t[37] = x),
      (t[38] = q))
    : (q = t[38]);
  let J;
  t[39] !== s || t[40] !== I || t[41] !== c || t[42] !== i
    ? ((J = i
        ? (0, H.jsx)(C.Item, {
            onSelect: I,
            disabled: s || !c,
            LeftIcon: fe,
            children: (0, H.jsx)(d, {
              id: `codex.review.copyGitApplyCommand`,
              defaultMessage: `Copy git apply command`,
              description: `Menu item to copy a git apply command`,
            }),
          })
        : null),
      (t[39] = s),
      (t[40] = I),
      (t[41] = c),
      (t[42] = i),
      (t[43] = J))
    : (J = t[43]);
  let Y;
  return (
    t[44] !== B || t[45] !== K || t[46] !== q || t[47] !== J || t[48] !== L
      ? ((Y = (0, H.jsxs)(H.Fragment, { children: [L, B, K, q, J] })),
        (t[44] = B),
        (t[45] = K),
        (t[46] = q),
        (t[47] = J),
        (t[48] = L),
        (t[49] = Y))
      : (Y = t[49]),
    Y
  );
}
export {
  le as a,
  ee as c,
  U as d,
  fe as i,
  G as l,
  _e as n,
  oe as o,
  ge as r,
  ie as s,
  ve as t,
  W as u,
};
//# sourceMappingURL=review-header-toolbar.js.map
