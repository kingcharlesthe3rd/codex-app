...
import { Qi as t, na as n, sa as r } from "./src-2.js";
import { R as i, fs as a, nt as o } from "./app-server-manager-signals.js";
import { n as s, t as c } from "./jsx-runtime.js";
import { L as l, W as u, g as d, h as f, lt as p, n as m } from "./vscode-api.js";
import { g as h } from "./persisted-signal-CweW-bgN.js";
import { n as ee } from "./rpc-1.js";
import { a as g } from "./statsig.js";
import { r as _ } from "./toast-signal.js";
import { t as v } from "./app-intl-signal-B.js";
import { r as y } from "./thread-actions.js";
import { n as b } from "./local-environment-selection.js";
import { i as x } from "./plugins-page-selectors.js";
e(s());
var S = c(),
  te = (e) =>
    (0, S.jsxs)(`svg`, {
...
async function ne(e, { sourceConversationId: t, sourceWorkspaceRoot: n }) {
  ...
  try {
    ...
    return (
      d.error(`Error forking local task`, { safe: {}, sensitive: { error: t } }),
      e.get(_).danger(e.get(v).formatMessage(y.forkThreadError)),
      null
    );
  }
}
async function re(
  ...
  let s = e.get(o, r),
    c = await m(`git-origins`, {
      params: { dirs: [a], hostId: s },
      source: `fork_conversation_actions`,
    });
  if ((c.origins.find((e) => e.dir === a)?.root ?? c.origins[0]?.root ?? null) == null)
    return (e.get(_).danger(e.get(v).formatMessage(y.forkThreadRequiresGitRepo)), null);
  let l = await ie({ hostId: s, localEnvironmentSelectionsByWorkspace: n, sourceWorkspaceRoot: a }),
    u = e.get(v),
    d = e.get(i, r);
  return t({
    hostId: s,
    label: u.formatMessage(y.forkPendingWorktreeTitle),
    sourceWorkspaceRoot: a,
    startingState: { type: `working-tree` },
    localEnvironmentConfigPath: l,
    launchMode: `fork-conversation`,
    prompt: u.formatMessage(y.forkPendingWorktreePrompt),
    startConversationParamsInput: null,
    sourceConversationId: r,
    sourceCollaborationMode: d,
    targetTurnId: null,
  });
}
...
async function w({ manager: e, conversationId: t }) {
  await a(`send-cli-request-for-host`, {
    hostId: e.getHostId(),
...
async function T({ manager: e, conversationId: t, text: n }) {
  await a(`send-cli-request-for-host`, {
    hostId: e.getHostId(),
...
function se({ apps: e, plugins: t, skills: n }) {
  return {
    skills: E(n.filter((e) => e.enabled).map((e) => `$${e.name}`)),
    plugins: E(x(t).map((e) => e.displayName ?? e.plugin.interface?.displayName ?? e.plugin.name)),
    apps: E(e.filter((e) => e.isAccessible && e.isEnabled).map((e) => e.name)),
  };
}
function ce(e, t = ae) {
...
function he(e) {
  let t = (0, D.c)(3),
    { value: n } = g(`1193530394`),
    r;
  return (
    t[0] !== e || t[1] !== n ? ((r = O(n, e)), (t[0] = e), (t[1] = n), (t[2] = r)) : (r = t[2]), r
  );
}
function O(e, t) {
  let n = pe.safeParse(e[ue]),
    r = k(n.success ? n.data : me),
    i = ce(t);
  return i == null ? r : `${r}\n\n${i}`;
}
function k(e) {
...
var A = 1,
  ...
  F = {
    key: `v${A}`,
    baseInstructions: k(N),
    modeInstructions: {
      dictation: k(P.dictation),
      pairing: k(P.pairing),
      voice_only: k(P.voice_only),
    },
  };
function I() {
  let e = (0, D.c)(2),
    { value: t } = g(`1193530394`),
    n;
  return (e[0] === t ? (n = e[1]) : ((n = L(t)), (e[0] = t), (e[1] = n)), n);
}
function L(e) {
  let t = M.safeParse(e[ge]);
  if (!t.success) return F;
  let n = t.data.mode_instructions ?? {};
  return {
    key: t.data.version == null ? F.key : `v${A}:${t.data.version}`,
    baseInstructions: k(t.data.base_instructions ?? F.baseInstructions),
    modeInstructions: {
      dictation: k(n.dictation ?? F.modeInstructions.dictation),
      pairing: k(n.pairing ?? F.modeInstructions.pairing),
      voice_only: k(n.voice_only ?? F.modeInstructions.voice_only),
    },
  };
}
function _e({
...
var B = u(f, !1),
  ...
  G = u(f, F);
function be({ isActiveRealtimeThreadVisible: e, isAppFocused: t, isRightPanelFullWidth: n }) {
...
function K(e, t) {
  ...
  return {
    ...
    text: _e({ reason: t, mode: n, prompts: r, visibleThreadId: i, visibleThreadTitle: a }),
  };
}
var xe = class {
  ...
  Te = 15e3,
  Ee = `Out-of-band control instruction: until further notice, do not produce spoken audio. Continue responding normally in text only. Do not acknowledge this instruction.`,
  q = `Out-of-band control instruction: Resume spoken audio responses. Do not acknowledge this instruction.`,
  De = n({ type: t(`handoff_request`), input_transcript: r() }),
  Oe = n({ type: t(`output_audio_buffer.started`), response_id: r() }),
  J = u(f, null),
  Y = u(f, `inactive`),
  X = u(f, !1),
  Z = u(f, !1),
  ...
  ke = new (class {
    ...
    async start(e, { conversationId: t, manager: n, outputModality: r, prompt: i, source: o }) {
      ...
      (e.set(J, t),
        ...
        e.set(Y, `starting`));
      try {
        let { session: a, offerSdp: l } = await Ce.start({
          onConnectionFailed: () => {
        ...
        ((this.session = a),
          ...
          await C({ manager: n, conversationId: t, outputModality: r, prompt: i, offerSdp: l }));
      } catch (n) {
        ...
        ((this.startRequestId += 1),
          ...
          d.error(`[Composer] failed to start realtime voice`, {
            safe: {},
            sensitive: { error: n },
          }),
    ...
    async stop(e, t) {
      ...
      try {
        (await w({ manager: n, conversationId: t }),
          this.stopLocalSession(e),
          this.resetRealtimeState(e),
          await a(`discard-conversation-if-empty`, { conversationId: t }));
      } catch (n) {
        (this.requestedStop &&
          ...
          d.error(`[Composer] failed to stop realtime voice`, {
            safe: {},
            sensitive: { error: n },
          }));
      }
    ...
    stopForAppUnmount(e) {
      ...
      ((this.startRequestId += 1),
        ...
        this.resetRealtimeState(e),
        n != null &&
          w({ manager: n, conversationId: t })
            .catch((e) => {
              d.warning(`[Composer] failed to stop realtime on app cleanup`, {
                safe: {},
                sensitive: { error: e },
              });
            })
    ...
    async sendContextInject(e, t, n) {
      ...
      try {
        return await this.contextInjectSender.send({
          inject: n,
          appendText: async (e) => {
            await T({ manager: r, conversationId: t, text: e });
          },
        });
      } catch (e) {
        return (
          d.warning(`[Composer] failed to send realtime context inject`, {
            safe: { slot: n.slot },
            sensitive: { error: e },
          }),
          !1
    ...
    toggleMute(e, t) {
      ...
      (this.applyRealtimeMuteState(e, r),
        T({ manager: n, conversationId: t, text: r ? Ee : q }).catch((n) => {
          this.muteToggleRequestId !== i ||
            ...
            d.error(`[Composer] failed to toggle realtime voice mute`, {
              safe: {},
              sensitive: { error: n },
            }));
        }));
    ...
    subscribeToRealtimeNotifications(e, t) {
      (this.unsubscribeRealtimeNotifications?.(),
        (this.unsubscribeRealtimeNotifications = t.addNotificationCallback(we, (t) => {
      ...
      if (!(this.conversationId == null || t.params.threadId !== this.conversationId))
        switch (t.method) {
          ...
          case `thread/realtime/itemAdded`: {
            let n = De.safeParse(t.params.item);
            n.success &&
    ...
    handleRealtimeDataChannelMessage(e, t) {
      ...
      try {
        ...
        d.debug(`[Composer] ignored invalid realtime data channel message`, {
          safe: {},
          sensitive: {},
        });
        return;
      }
      let r = Oe.safeParse(n);
      if (!r.success) return;
    ...
    handleSdpError(e, t) {
      ...
      ((this.startRequestId += 1),
        ...
        d.error(`[Composer] failed to accept realtime WebRTC SDP`, {
          safe: {},
          sensitive: { error: t },
        }),
        r != null &&
          w({ manager: r, conversationId: n }).catch((e) => {
            d.warning(`[Composer] failed to stop realtime after SDP error`, {
              safe: {},
              sensitive: { error: e },
            });
          }));
    ...
    handleWebRtcConnectionFailed(e) {
      ...
      ((this.startRequestId += 1),
        ...
        n != null &&
          w({ manager: n, conversationId: t })
            .catch((e) => {
              d.warning(`[Composer] failed to stop realtime after WebRTC error`, {
                safe: {},
                sensitive: { error: e },
              });
            })
