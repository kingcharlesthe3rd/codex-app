...
import { f as se, h as O, n as k } from "./vscode-api.js";
import {
  Aa as A,
  Ar as j,
  Br as ce,
...
import { t as ge } from "./setting-storage.js";
import { F as _e } from "./rpc-1.js";
import { d as ve } from "./statsig.js";
import { t as ye } from "./v4.js";
import { t as be } from "./model-list-filter.js";
import { t as xe } from "./remote-projects.js";
import { t as z } from "./app-intl-signal.js";
import { t as Se } from "./windows-tabs-open-schema.js";
import { f as Ce } from "./sidebar-project-group-signals.js";
import { t as we } from "./read-service-tier-for-request.js";
import { S as Te, n as Ee } from "./realtime-controller.js";
import { t as De } from "./pending-worktree-store.js";
import { t as Oe } from "./threads-create.js";
import { r as ke } from "./thread-actions.js";
import { n as Ae } from "./web-search-favicon-icon.js";
import { t as je } from "./score-query-match.js";
import { r as Me } from "./thread-handoff-composer-block-state-C7-ef4-4.js";
import { t as Ne } from "./automation-host-support.js";
import { t as Pe } from "./get-default-branch-name.js";
import { t as Fe } from "./worktree-query-keys.js";
import { n as Ie, r as Le, t as Re } from "./run-app-action-in-primary-window.js";
import { t as ze } from "./get-move-to-local-targets.js";
async function B({ scope: e, threadId: t }) {
  let n = (
      await Promise.all(
        Be(e).map(async (e) => {
...
function Be(e) {
  return e.get(ae).filter((t) => {
    let n = t.getHostId();
    return n === `local` || e.get(te, n) === `connected`;
  });
}
async function Ve({ pinned: e, threadId: t }) {
  return (
    await k(`set-thread-pinned`, { params: { threadId: t, pinned: e } }), { threadId: t, pinned: e }
  );
}
async function He({ archived: e, scope: t, threadId: n }) {
  let { hostId: i } = await B({ scope: t, threadId: n }),
    a = F(n);
  return (
    e
...
async function Ue({ scope: e, threadId: t, title: n }) {
  let { hostId: i } = await B({ scope: e, threadId: t });
  return (
    await r(`set-thread-title`, { hostId: i, conversationId: F(t), title: n }),
    { threadId: t, title: n }
  );
}
async function We({
  ...
  let { hostId: o, manager: s } = await B({ scope: e, threadId: i }),
...
function Ke(e, t, n) {
  return {
    ...
    items: e.items.map((e) => qe(e, t, n)),
  };
}
function qe(e, t, n) {
  switch (e.type) {
    case `userMessage`:
      return { type: e.type, id: e.id, content: e.content.map(Je) };
    case `agentMessage`:
    ...
    case `reasoning`:
      return {
        ...
        id: e.id,
        summary: e.summary,
        ...(t ? { content: e.content.map((e) => Ye(e, n)) } : {}),
      };
    case `commandExecution`:
      return {
        ...
        exitCode: e.exitCode,
        durationMs: e.durationMs,
        ...(t && e.aggregatedOutput != null ? { output: Ye(e.aggregatedOutput, n) } : {}),
      };
    case `fileChange`:
      return {
        type: e.type,
...
function V({ sourceConversationId: e, targetConversationId: t, diffComments: n }) {
...
async function Xe({
  ...
  try {
    ...
    if (
      ...
      let e = await k(`feedback-desktop-log-archive`);
    ...
    if (s != null)
      try {
        await k(`delete-feedback-desktop-log-archive`, { params: { archiveId: s } });
      } catch (e) {
        O.warning(`Failed to delete feedback desktop log archive`, {
          safe: {},
          sensitive: { error: e },
        });
      }
...
async function tt({
  ...
  (d.invalidateQueries({ queryKey: Fe(T(c)) }), p?.());
  let E;
  try {
    E = await r(`fork-conversation-from-rollout-path`, {
      hostId: c.id,
      conversationId: e,
    ...
    try {
      await k(`worktree-set-owner-thread`, {
        params: { hostId: c.id, worktree: w.worktreeGitRoot, conversationId: e },
      });
    } catch {}
    return (
      O.warning(`Cross-host handoff moved git state, but conversation fork failed: {}`, {
        sensitive: { error: j(t) },
        safe: {},
      }),
      h?.(),
  ...
  return (
    ...
    await H(
      ...
      () => {
        u((t) => V({ sourceConversationId: e, targetConversationId: E, diffComments: t }));
      },
    ),
    await H(
      `Cross-host handoff succeeded, but pinned-thread state could not be transferred: {}`,
      () => $e({ sourceConversationId: e, targetConversationId: E }),
    ),
    m?.(),
    { status: `success`, conversationId: E }
  );
...
async function H(e, t) {
  try {
    ...
    O.warning(e, { sensitive: { error: j(t) }, safe: {} });
  }
}
async function nt({
  conversationId: e,
  ...
  if (b.status === `error`) {
    ...
    return {
      status: `error`,
      message: `${rt({ branch: t, error: b.error, intl: c })}${e}`,
      execOutput: b.execOutput,
    };
  }
  (b.warnings.includes(`drop-source-stash-failed`) &&
    O.warning(`Move to local succeeded, but source stash cleanup failed`),
    m?.());
  let x;
  try {
    x = await r(`fork-conversation-from-latest`, {
      hostId: l.getHostId(),
      conversationId: e,
      cwd: y,
      workspaceRoots: [y],
    ...
    return (
      O.warning(`Move to local git operations succeeded, but conversation fork failed: {}`, {
        sensitive: { error: j(t) },
        safe: {},
      }),
      g?.(),
  ...
  try {
    ...
    O.warning(`Move to local succeeded, but worktree ownership metadata could not be updated: {}`, {
      sensitive: { error: j(e) },
      safe: {},
    });
  }
  try {
    ...
    O.warning(`Move to local succeeded, but browser state could not be transferred: {}`, {
      sensitive: { error: j(e) },
      safe: {},
    });
  }
  try {
    ...
    O.warning(`Move to local succeeded, but thread title could not be transferred: {}`, {
      sensitive: { error: j(e) },
      safe: {},
    });
  }
  try {
    d((t) => V({ sourceConversationId: e, targetConversationId: x, diffComments: t }));
  } catch (e) {
    O.warning(`Move to local succeeded, but saved diff comments could not be transferred: {}`, {
      sensitive: { error: j(e) },
      safe: {},
    });
  }
  try {
    await $e({ sourceConversationId: e, targetConversationId: x });
  } catch (e) {
    O.warning(`Move to local succeeded, but pinned-thread state could not be transferred: {}`, {
      sensitive: { error: j(e) },
      safe: {},
    });
  }
...
async function it({
  ...
  try {
    ...
    ((v = t.worktreeGitRoot), (y = t.worktreeWorkspaceRoot), (x = t.hasUncommittedChanges));
  } catch {
    O.warning(
  ...
  if (v != null && y != null) ((S = `reuse-existing-worktree`), m?.(S, `running`), m?.(S, `done`));
    ...
    try {
      ...
      ((v = e.worktreeGitRoot),
        ...
        (x = !1),
        m?.(S, `done`),
        f.invalidateQueries({ queryKey: Fe(T(d)) }));
    } catch (e) {
      return (
        m?.(S, `failed`),
        {
  ...
  let C = await k(`git-create-branch`, {
  ...
  if (C.status === `error`) {
    if (b)
      try {
        ...
        O.warning(`Failed to clean up the newly created worktree: {}`, {
          sensitive: { error: j(e) },
          safe: {},
        });
      } finally {
  ...
  let E;
  try {
    E = await r(`fork-conversation-from-latest`, {
      hostId: s.getHostId(),
      conversationId: e,
      cwd: y,
      workspaceRoots: [y],
    ...
    try {
      await k(`worktree-set-owner-thread`, {
        params: { hostId: d.id, worktree: v, conversationId: e },
      });
    } catch {}
    return (
      O.warning(`Move to worktree git operations succeeded, but conversation fork failed: {}`, {
        sensitive: { error: j(t) },
        safe: {},
      }),
  ...
  try {
    ...
    O.warning(
      ...
      { sensitive: { error: j(e) }, safe: {} },
    );
  }
  try {
    l(e, E);
  } catch (e) {
    O.warning(`Move to worktree succeeded, but browser state could not be transferred: {}`, {
      sensitive: { error: j(e) },
      safe: {},
    });
  }
  try {
    ...
    O.warning(`Move to worktree succeeded, but thread title could not be transferred: {}`, {
      sensitive: { error: j(e) },
      safe: {},
    });
  }
  try {
    ...
    O.warning(`Move to worktree succeeded, but saved diff comments could not be transferred: {}`, {
      sensitive: { error: j(e) },
      safe: {},
    });
  }
  (w.warnings.includes(`stashed-target-worktree-changes`) &&
    O.warning(`Move to worktree stashed uncommitted changes in the reused worktree`),
    w.warnings.includes(`drop-source-stash-failed`) &&
      O.warning(`Move to worktree succeeded, but source stash cleanup failed`),
    w.warnings.includes(`drop-target-stash-failed`) &&
      O.warning(`Move to worktree succeeded, but target stash cleanup failed`));
  try {
    await $e({ sourceConversationId: e, targetConversationId: E });
  } catch (e) {
    O.warning(`Move to worktree succeeded, but pinned-thread state could not be transferred: {}`, {
      sensitive: { error: j(e) },
      safe: {},
    });
  }
  return (m?.(`switching-thread`, `done`), { status: `success`, conversationId: E });
}
var at = `open_in_codex`,
  ot = {
    ...
    inputSchema: fe().parse(ue(Se)),
  };
async function st({ argumentsValue: e, hostId: t, threadId: n }) {
  let r = Se.safeParse(e);
  if (!r.success) return E(`${at} received invalid arguments.`);
  try {
    let e = await Re(
    ...
    return E(e instanceof Error ? e.message : `Failed to open Codex tab.`);
  }
}
var ct = `send_realtime_voice_feedback`,
  lt = `[Realtime Voice]`,
  ut = I({
    feedback: A().trim().min(1).describe(`The user's realtime voice feedback, in their own words.`),
  }),
  dt = {
    name: ct,
    ...
    inputSchema: fe().parse(ue(ut)),
  };
async function ft({ argumentsValue: e, hostId: t, threadId: n }) {
  let r = ut.safeParse(e);
  if (!r.success) return E(`${ct} received invalid arguments.`);
  try {
    let e = await Xe({
    ...
    return (
      O.error(`Failed to submit realtime voice feedback`, {
        safe: { threadId: n },
        sensitive: { error: e },
      }),
      E(`Failed to submit realtime voice feedback.`)
    );
  }
}
var U = `speak_to_user`,
  pt = I({ message: A().min(1).describe(`The exact message to speak to the user.`) });
function mt(e = Te) {
  return { name: U, description: e, inputSchema: fe().parse(ue(pt)) };
}
async function ht({ argumentsValue: e, hostId: t, threadId: n }) {
  let i = pt.safeParse(e);
  if (!i.success) return E(`${U} received invalid arguments.`);
  try {
    return (
    ...
    return (
      O.error(`Failed to speak realtime voice message`, {
        safe: { threadId: n },
        sensitive: { error: e },
      }),
      E(`Failed to speak realtime voice message.`)
    );
  }
}
  ...
  switch (e.type) {
    ...
    case `worktree`: {
      ...
      return {
        ...
        pendingWorktreeId: De({
          hostId: o,
          label: n.formatMessage(ke.forkPendingWorktreeTitle),
          sourceWorkspaceRoot: s,
          startingState: e.startingState ?? { type: `working-tree` },
          localEnvironmentConfigPath: null,
          launchMode: `fork-conversation`,
          prompt: n.formatMessage(ke.forkPendingWorktreePrompt),
...
async function Lt({
  ...
  let c = e ?? (await B({ scope: i, threadId: o })).hostId,
    l = F(o),
  ...
  await r(`send-follow-up-message`, {
    ...
    serviceTier: await we(i, c, t ?? null),
  });
  let f = i.get(Ee);
  if (a != null && f != null) {
    let e = F(a),
...
var Ut = e(u(), 1),
  ...
  Kt = I({
    threadId: A().min(1),
    destinationHostId: A().min(1).optional(),
    followUpPrompt: A().trim().min(1).optional(),
  }),
  qt = I({
    operationId: A().min(1),
    afterRevision: L().int().min(0).optional(),
    waitMs: L().int().min(0).max(Wt).optional(),
  }),
  Jt = `handoff_thread`,
  Yt = `get_handoff_status`;
function Xt(e) {
  return (0, Ut.default)([h(e.get, `host_config`) ?? Gt, ...e.get(ne).map(he)], ({ id: e }) => e);
}
...
async function $t({
  ...
  callId: r,
  callingThreadId: i,
  crossHostHandoffEnabled: a = ve(e, `2256010998`),
  queryClient: o,
}) {
  let s = Kt.safeParse(n);
  if (!s.success) return E(`${Jt} received invalid arguments.`);
  let c = F(s.data.threadId);
  if (c === i) return E(`A thread cannot hand itself off. Choose another thread.`);
  try {
    let n = r,
      l = G(e, n);
    ...
    await Ze({ conversationId: c, sourceThreadRunning: b(d.conversation) });
    ...
    return (
      O.warning(`Thread handoff dynamic tool failed: {}`, {
        sensitive: { error: j(e) },
        safe: { threadId: c },
      }),
      E(e instanceof Error ? e.message : `Thread handoff failed.`)
    );
  }
}
async function en({ scope: e, argumentsValue: t }) {
  let n = qt.safeParse(t);
  if (!n.success) return E(`${Yt} received invalid arguments.`);
  let r = await Bt({
  ...
  return r == null
    ? E(`No thread handoff operation found for operationId ${n.data.operationId}.`)
    : mn(hn(r));
}
async function tn({
  scope: e,
  ...
  if (e.get(Me, o) > 0 || (C(e.get, ce.QUEUED_FOLLOW_UPS)?.[o]?.length ?? 0) > 0)
    throw Error(
...
async function nn(e) {
  let { codexHome: t } = await k(`codex-home`, { params: { hostId: e.hostConfig.id } });
  if (de(e.cwd, t)) {
    let n = await cn(e, t),
    ...
    return {
      ...
      progress: fn(e, {
        ...
        destinationHostId: e.hostConfig.id,
        destinationCwd: P(n.workspaceRoot),
        direction: `to-local`,
        localBranch: r,
        sourceBranch: r,
        steps: pn([
...
async function rn(e, t) {
  ...
  let n = un(e, e.currentBranch),
    [{ roots: r }, { origins: i }] = await Promise.all([
      ...
      k(`git-origins`, { params: { hostId: e.hostConfig.id, dirs: [e.cwd] }, source: q }),
    ]),
    { origins: a } = await k(`git-origins`, { params: { hostId: t.id, dirs: r }, source: q }),
    o = xe({
...
async function on({
  ...
  try {
    ...
    (O.warning(`Thread handoff failed unexpectedly: {}`, {
      sensitive: { error: j(e) },
...
function sn(e) {
  r(`archive-conversation`, {
    ...
    O.warning(`Thread handoff succeeded, but source archive failed: {}`, {
      sensitive: { error: j(t) },
...
async function cn(e, t) {
  let [{ worktrees: n }, { worktrees: r }] = await Promise.all([
    ...
    i = new Set(r.map(({ dir: e }) => _e(e))),
    a = ze({
      cwd: e.cwd,
      sourceWorktreeRoot: P(e.gitRoot),
      repoWorktreeEntries: n.filter(({ root: e }) => !i.has(_e(e)) && !de(e, t)),
    })[0];
...
function un(e, t) {
  ...
  let r = ge(e.scope.get, le.branchPrefix),
    i = Pe({ branchPrefix: r, conversationTitle: e.conversation.title });
  return i.length > 0 && !i.endsWith(`/`)
    ? i
    : Pe({ branchPrefix: r, conversationTitle: e.threadId });
}
function dn(e, t) {
...
var X = `received invalid arguments.`,
  ...
  Dn = me([`low`, `medium`, `high`, `xhigh`, `max`]),
  On = N(`type`, [
    M({ type: R(`working-tree`) }),
    M({ type: R(`branch`), branchName: A().min(1) }),
  ]),
  kn = N(`type`, [
    M({ type: R(`local`) }),
    M({ type: R(`worktree`), startingState: On.optional() }),
  ]),
  An = N(`type`, [
    M({ type: R(`project`), projectId: A().min(1), environment: kn }),
    M({ type: R(`projectless`), directoryName: A().min(1).optional() }),
  ]),
  jn = N(`type`, [
    M({ type: R(`same-directory`) }),
    M({ type: R(`worktree`), startingState: On.optional() }),
  ]),
  Z = `fork_thread`,
  Mn = `list_projects`,
  ...
  Rn = `set_thread_title`,
  zn = I({ prompt: A().min(1), target: An, model: A().min(1).optional(), thinking: Dn.optional() }),
  Bn = I({ threadId: A().min(1).optional(), environment: jn.optional() }),
  Vn = I({}),
  Hn = I({ query: A().optional(), limit: L().int().min(1).max(50).optional() }),
  Un = I({
    threadId: A().min(1),
    cursor: A().min(1).optional(),
    turnLimit: L().int().min(1).max(10).optional(),
    includeOutputs: pe().optional(),
    maxOutputCharsPerItem: L().int().min(0).max(2e4).optional(),
  }),
  Wn = I({
    threadId: A().min(1),
    prompt: A().min(1),
    model: A().min(1).optional(),
    thinking: Dn.optional(),
  }),
  Gn = I({ threadId: A().min(1), pinned: pe() }),
  Kn = I({ threadId: A().min(1).optional(), archived: pe() }),
  qn = I({ threadId: A().min(1), title: A().min(1) });
function Jn({
...
async function Yn({ scope: e, argumentsValue: t, sourceThreadId: n }) {
  if (n == null) return E(`${Z} missing calling thread id.`);
  let r = Bn.safeParse(t);
  if (!r.success) return E(`${Z} ${X}`);
  try {
    return $(
      await wt({
        scope: e,
...
async function Zn({ scope: e, argumentsValue: t }) {
  if (!Vn.safeParse(t).success) return E(`${Mn} ${X}`);
  try {
...
async function nr({ argumentsValue: e, sourceThreadId: t, scope: n }) {
  ...
  if (!r.success) return E(`${Q} ${X}`);
  let i = r.data.threadId ?? t;
  if (i == null) return E(`${Q} missing calling thread id.`);
  try {
    return $(await He({ ...r.data, threadId: i, scope: n }));
  } catch (e) {
    return E(e instanceof Error ? e.message : String(e));
  }
}
async function rr({ argumentsValue: e, scope: t }) {
  ...
  if (!n.success) return E(`${Rn} ${X}`);
  try {
    return $(await Ue({ ...n.data, scope: t }));
  } catch (e) {
    return E(e instanceof Error ? e.message : String(e));
  }
}
...
var gr = 100,
  ...
  vr = 1e3,
  yr = `codex_app`,
  br = new Set([ee.name, x.name, dt.name, U, c, l, i, o, Le]);
async function xr({
  availableHandoffHosts: e,
  authMethod: t,
  crossHostHandoffEnabled: n = !1,
  ...
  speakToUserToolDescription: f = Te,
  threadStartKind: m = `default`,
}) {
  let h = Ne(i),
    g = r?.[S] === !0,
    v = m === `realtime_voice` && r?.realtime_conversation === !0,
    b = r?.[y] === !0,
    C = r?.[s] === !0,
    [w, T] = await Promise.all([
      h ? Tr(c) : !1,
  ...
  return [
    {
      ...
      tools: [
        ...
        ...(v ? [dt, ...(d ? [mt(f)] : [])] : []),
        ...(g
          ? Jn({ availableHandoffHosts: e, availableModels: T, crossHostHandoffEnabled: n })
          : []),
        ...(h && b ? [vt, yt] : []),
        ...(m === `conversational_onboarding` ? [Ie] : []),
        ...(C && m !== `conversational_onboarding` ? [...p, a] : []),
...
async function Tr(e, t = null) {
  try {
    ...
    return Ae(n.data) ? !0 : n.nextCursor == null ? !1 : await Tr(e, n.nextCursor);
  } catch {
    return !1;
  }
}
...
//# sourceMappingURL=app-server-dynamic-tools-Gzn-GJ0M.js.map
