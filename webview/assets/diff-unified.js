...
import { $n as f, Xn as p, dr as m, ur as h } from "./thread-context-inputs.js";
...
function je(e) {
  ...
  let f = d,
    ...
    g;
  t[2] === p ? (g = t[3]) : ((g = (0, P.jsx)(Pe, { kind: p })), (t[2] = p), (t[3] = g));
  let _ = g,
    v,
    y;
  t[4] !== r || t[5] !== c || t[6] !== u
    ? ((v = m(r)),
      (y = h({ filePath: r, imagePreviewMode: v, isDeletion: c, richPreviewEnabled: u })),
      (t[4] = r),
      (t[5] = c),
      (t[6] = u),
      (t[7] = v),
...
function Je({
  ...
  return `${e.metadata.cacheKey ?? `${e.metadata.name}:${e.metadata.prevObjectId ?? `none`}:${e.metadata.newObjectId ?? `none`}:${e.additions}:${e.deletions}`}:${a ?? ``}:${p(t)}:${i ? `next-disk-fallback` : `next-object-only`}:${n ? `ignore-whitespace` : `exact-whitespace`}:${r ? `full` : `partial`}`;
}
async function Ye({
  diff: e,
  hostConfig: t,
  ...
  if (d.type === `error` || p.type === `error`) return { type: `error` };
  let m = Ve(e.metadata, d.lines, p.lines, { ignoreWhitespace: n });
