...
import { o as e } from "./thread-context-inputs.js";
import { n as t, r as n, u as r } from "./vscode-api.js";
import { or as i, un as a } from "./src-3.js";
import { i as o, r as s } from "./lib-1.js";
import { r as c } from "./platform.js";
import { t as l } from "./open-workspace-file.js";
import { t as u } from "./browser-sidebar-availability.js";
...
function h(e, { hostId: i, path: a }) {
  let o = i == null ? { path: a } : { hostId: i, path: a },
    s = n(`read-file`, o),
    c =
      e.getQueryData(s) ??
      (i == null ? void 0 : e.getQueryData(n(`read-file`, { path: a, hostId: i })));
  if (c != null) {
    m(c.contents);
    return;
  }
  e.fetchQuery({
    queryFn: ({ signal: e }) => t(`read-file`, { params: o, signal: e }),
    queryKey: s,
...
function _({ cwd: e, hostId: i, path: a }) {
  return {
    gcTime: r.INFINITE,
    queryKey: n(`open-in-targets`, { cwd: e, hostId: i, path: a }),
    queryFn: () => t(`open-in-targets`, { params: { cwd: e, hostId: i, path: a } }),
...
function x(
  ...
  let C = t.get(e, p ?? `local`),
    w = t.get(u),
    { primaryTarget: T, visibleTargets: E } = v(
      t.queryClient.getQueryData(_({ cwd: o, hostId: p, path: x }).queryKey),
    ),
    D = [],
    O = b,
    k = w && !a(C) && i(x),
  ...
  return (
    ...
    D.push({
      ...
      onSelect: () => {
        h(t.queryClient, { hostId: p, path: x });
      },
    }),
    a(C) ||
      D.push({
        id: `workspace-file-reveal-path`,
        message: S(t.get(c).data?.platform),
        onSelect: () => {
          j(`fileManager`);
        },
      }),
