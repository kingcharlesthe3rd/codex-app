import { s as e } from "./chunk.js";
import { n as t, t as n } from "./jsx-runtime.js";
import { R as r, a as i, o as a, s as o, t as s } from "./app-scope.js";
import { us as c } from "./app-server-manager-signals.js";
import { f as l, g as u, k as d, n as f, r as p } from "./vscode-api.js";
import "./isEqual.js";
import { gr as m } from "./src-1.js";
import "./react-dom.js";
import { c as h, o as g, r as _ } from "./lib.js";
import "./persisted-signal.js";
import { t as v } from "./clsx-Cir5-jBH.js";
import "./proxy.js";
import { t as y } from "./button-DO-oxX3-.js";
import "./setting-storage.js";
import "./reduced-motion-preference.js";
import "./use-reduced-motion-1.js";
import { t as b } from "./spinner.js";
import "./Combination.js";
import "./window-zoom-context.js";
import { t as x } from "./check-md.js";
import { t as S } from "./chevron-right.js";
import "./use-stable-callback.js";
import "./tooltip.js";
import "./rpc-1.js";
import { f as C, l as w, m as T, o as E, p as D, s as O } from "./browser-use-settings.js";
import "./statsig.js";
import "./request.js";
import "./platform-BrML-86Y.js";
import "./marked.esm.js";
import "./app-server-manager-hooks.js";
import "./invalidate-queries-and-broadcast.js";
import "./thread-context-inputs.js";
import "./config-queries.js";
import "./selectable-remote-connections-signal.js";
import "./remote-connection-visibility.js";
import "./sidebar-signals.js";
import "./use-is-copilot-api-available.js";
import "./use-global-state.js";
import "./use-auth.js";
import "./use-codex-home.js";
import "./use-os-info.js";
import { r as k } from "./toast-signal.js";
import "./remote-projects.js";
import "./check-circle-filled.js";
import "./chevron.js";
import { a as A, m as j, p as M } from "./chunk-109.js";
import "./known-app-icon.js";
import "./branch.js";
import "./github-mark.js";
import "./google-drive.js";
import "./notion.js";
import "./_baseEach-BPh-9Ri0.js";
import "./_baseOrderBy-Cz-sT1AW.js";
import "./codex-api.js";
import "./use-debounced-value.js";
import "./apps-queries.js";
import "./startCase.js";
import "./experimental-features-queries.js";
import "./use-is-dark.js";
import "./connector-logo.js";
import "./route-scope.js";
import { k as N } from "./mention-item-B.js";
import "./thread-detail-level.js";
import "./permissions-mode-defaults.js";
import "./use-permissions-mode.js";
import "./apps.js";
import "./chats.js";
import "./info.js";
import "./x.js";
import "./projectless-thread.js";
import "./plugin-install-store.js";
import { t as P } from "./use-platform.js";
import { i as F, r as I } from "./use-is-plugins-enabled.js";
import "./browser-sidebar-availability.js";
import { m as ee, y as L } from "./use-plugins.js";
import "./with-window.js";
import "./use-resolved-theme-variant.js";
import "./plugin-detail-route-utils.js";
import "./folder.js";
import "./settings.cog.js";
import "./openai-blossom.js";
import "./filesystem-media-src.js";
import "./use-register-command.js";
import "./modal-controller-state.js";
import { a as R, i as z, l as B, n as V, r as H } from "./dialog-layout.js";
import "./open-project-setup-dialog.js";
import "./local-projects.js";
import "./select-project.js";
import "./sidebar-project-groups.js";
import "./_defineProperty.js";
import "./run-command.js";
import "./use-skills.js";
import "./initial-route-atom.js";
import "./use-start-new-conversation.js";
import "./plugin-config-edits.js";
import "./plugin-prefill-prompt.js";
import "./bundled-plugin-auto-install-preference.js";
import "./upgrade-plan-dialog-launcher.js";
import { t as U } from "./arrow-left.js";
import "./codex-api-error.js";
import "./checkbox.js";
import "./minus.js";
import { m as W } from "./links.js";
import "./use-codex-pricing-url.js";
import "./plan-management-state.js";
import "./badge.js";
import "./search.js";
import { c as te, p as ne, s as re } from "./use-plugin-install-flow.js";
import { r as ie, t as ae } from "./dropdown.js";
import "./three-dots.js";
import { t as oe } from "./trash.js";
import "./plus.js";
import "./sortBy.js";
import "./use-connected-remote-connections.js";
import { t as G } from "./settings-host-context.js";
import "./check-plugin-availability.js";
import "./plugin-detail-queries.js";
import { t as se } from "./toggle.js";
import "./lock.js";
import "./arrow-top-right.js";
import "./connected-apps.js";
import "./regenerate.js";
import "./app-connect-oauth.js";
import {
  a as ce,
  i as le,
  o as ue,
  r as de,
  s as fe,
  t as K,
} from "./computer-use-app-approvals-query.js";
import "./hooks-settings-queries.js";
import "./shield-exclamation.js";
import { i as pe, t as me } from "./settings-shared.js";
import { t as q } from "./settings-content-layout.js";
import "./more-menu-trigger.js";
import { l as he, t as ge } from "./browser-use-origin-state-queries.js";
import "./plugins-availability.js";
import { n as J } from "./settings-row.js";
import { n as _e, t as Y } from "./settings-group.js";
import "./settings-empty-state.js";
import { t as ve } from "./use-home-directory.js";
import "./browser-use.js";
import "./statsig-url-config.js";
import "./browser-use-elevated-risk-learn-more-url.js";
import "./plugin-uninstall-dialog.js";
var ye = e(t(), 1),
  X = r(),
  Z = n();
function be(e) {
  let t = (0, X.c)(7),
    { computerImageDataUrl: n, lockImageDataUrl: r } = e,
    i;
  t[0] === n
    ? (i = t[1])
    : ((i = (0, Z.jsx)(`img`, {
        alt: ``,
        className: `absolute top-0 left-1 h-[53px] w-[53px] object-contain`,
        draggable: !1,
        src: n,
      })),
      (t[0] = n),
      (t[1] = i));
  let a;
  t[2] === r
    ? (a = t[3])
    : ((a = (0, Z.jsx)(`img`, {
        alt: ``,
        className: `absolute right-0 bottom-0 h-[33px] w-[27px] object-contain`,
        draggable: !1,
        src: r,
      })),
      (t[2] = r),
      (t[3] = a));
  let o;
  return (
    t[4] !== i || t[5] !== a
      ? ((o = (0, Z.jsxs)(`span`, {
          className: `relative inline-block h-[57px] w-[62px] shrink-0`,
          children: [i, a],
        })),
        (t[4] = i),
        (t[5] = a),
        (t[6] = o))
      : (o = t[6]),
    o
  );
}
var xe = _({
  connected: {
    id: `plugins.browserExtension.connected`,
    defaultMessage: `Connected to browser extension for additional control`,
    description: `Status text shown when the Chrome browser extension is installed`,
  },
  disconnected: {
    id: `plugins.browserExtension.disconnected`,
    defaultMessage: `Browser extension not connected`,
    description: `Status text shown when the Chrome browser extension is not installed`,
  },
  loading: {
    id: `plugins.browserExtension.loading`,
    defaultMessage: `Use the browser extension for additional control`,
    description: `Status text shown while the Chrome browser extension install state is loading`,
  },
});
function Se(e) {
  let t = (0, X.c)(10),
    { status: n } = e;
  if (n.isLoading) {
    let e;
    return (
      t[0] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((e = (0, Z.jsx)(g, { ...xe.loading })), (t[0] = e))
        : (e = t[0]),
      e
    );
  }
  let r = n.isConnected ? `bg-[var(--color-icon-success)]` : `bg-[var(--color-icon-error)]`,
    i;
  t[1] === r ? (i = t[2]) : ((i = v(`h-2 w-2 shrink-0 rounded-full`, r)), (t[1] = r), (t[2] = i));
  let a;
  t[3] === i ? (a = t[4]) : ((a = (0, Z.jsx)(`span`, { className: i })), (t[3] = i), (t[4] = a));
  let o;
  t[5] === n.isConnected
    ? (o = t[6])
    : ((o = (0, Z.jsx)(`span`, {
        className: `min-w-0 truncate`,
        children: n.isConnected
          ? (0, Z.jsx)(g, { ...xe.connected })
          : (0, Z.jsx)(g, { ...xe.disconnected }),
      })),
      (t[5] = n.isConnected),
      (t[6] = o));
  let s;
  return (
    t[7] !== a || t[8] !== o
      ? ((s = (0, Z.jsxs)(`span`, {
          className: `inline-flex max-w-full items-center gap-2`,
          children: [a, o],
        })),
        (t[7] = a),
        (t[8] = o),
        (t[9] = s))
      : (s = t[9]),
    s
  );
}
var Q = _({
    control: {
      id: `settings.computerUse.install.title`,
      defaultMessage: `Control`,
      description: `Title for computer use plugin install settings section`,
    },
    anyApp: {
      id: `settings.computerUse.anyApp.title`,
      defaultMessage: `Any App`,
      description: `Label for the Computer Use plugin row that lets Codex control 'Any App' on the user's computer`,
    },
    googleChrome: {
      id: `settings.computerUse.chrome.pluginTitle`,
      defaultMessage: `Google Chrome`,
      description: `Title for the Google Chrome plugin control row`,
    },
    alwaysAllowedApps: {
      id: `settings.computerUse.allowedApps.title`,
      defaultMessage: `Always-allowed apps`,
      description: `Title for computer use always allowed apps list`,
    },
    foregroundClickSounds: {
      id: `settings.computerUse.sounds.foregroundClicks`,
      defaultMessage: `Play sounds for foreground clicks`,
      description: `Computer use sound setting option for foreground click sounds`,
    },
    foregroundAndBackgroundClickSounds: {
      id: `settings.computerUse.sounds.foregroundAndBackgroundClicks`,
      defaultMessage: `Play sounds for foreground and background clicks`,
      description: `Computer use sound setting option for foreground and background click sounds`,
    },
    noClickSounds: {
      id: `settings.computerUse.sounds.off`,
      defaultMessage: `Don’t play sounds`,
      description: `Computer use sound setting option for no sounds`,
    },
  }),
  Ce = `` + new URL(`chrome-production-large-BLPyHeGg.png`, import.meta.url).href,
  we = `computer-use`,
  Te = `computer-use`,
  Ee = `chrome`,
  De = `chrome-dev`,
  Oe = `chrome-internal`,
  ke = `/settings/computer-use/google-chrome`,
  Ae = `foregroundClicks`,
  $ = [
    { value: `foregroundClicks`, label: Q.foregroundClickSounds },
    { value: `foregroundAndBackgroundClicks`, label: Q.foregroundAndBackgroundClickSounds },
    { value: `off`, label: Q.noClickSounds },
  ];
function je() {
  let e = (0, X.c)(20),
    { selectedHostId: t } = G(),
    n = i(ne, t),
    r;
  e[0] === t ? (r = e[1]) : ((r = { hostId: t }), (e[0] = t), (e[1] = r));
  let a = F(r),
    { platform: o } = P();
  if (M(ke) != null) {
    let t;
    return (
      e[2] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((t = (0, Z.jsx)(Pe, {})), (e[2] = t))
        : (t = e[2]),
      t
    );
  }
  let s, c;
  e[3] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((s = (0, Z.jsx)(pe, { slug: we })),
      (c = (0, Z.jsx)(g, {
        id: `settings.computerUse.subtitle`,
        defaultMessage: `Manage how Codex uses other applications on your computer`,
        description: `Subtitle for computer use settings`,
      })),
      (e[3] = s),
      (e[4] = c))
    : ((s = e[3]), (c = e[4]));
  let l;
  e[5] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((l = (0, Z.jsx)(Y.Header, { title: (0, Z.jsx)(g, { ...Q.control }) })), (e[5] = l))
    : (l = e[5]);
  let u;
  e[6] === a
    ? (u = e[7])
    : ((u = (0, Z.jsx)(Me, { computerUseAvailability: a })), (e[6] = a), (e[7] = u));
  let d;
  e[8] !== n || e[9] !== a.available || e[10] !== o
    ? ((d = o === `macOS` && a.available && n ? (0, Z.jsx)(Be, {}) : null),
      (e[8] = n),
      (e[9] = a.available),
      (e[10] = o),
      (e[11] = d))
    : (d = e[11]);
  let f;
  e[12] !== u || e[13] !== d
    ? ((f = (0, Z.jsxs)(Y, { children: [l, (0, Z.jsxs)(Y.Content, { children: [u, d] })] })),
      (e[12] = u),
      (e[13] = d),
      (e[14] = f))
    : (f = e[14]);
  let p;
  e[15] === a.available
    ? (p = e[16])
    : ((p = a.available
        ? (0, Z.jsxs)(Z.Fragment, {
            children: [
              (0, Z.jsxs)(Y, {
                children: [
                  (0, Z.jsx)(Y.Header, { title: (0, Z.jsx)(g, { ...Q.alwaysAllowedApps }) }),
                  (0, Z.jsx)(Y.Content, {
                    children: (0, Z.jsx)(_e, { children: (0, Z.jsx)(Ue, {}) }),
                  }),
                ],
              }),
              (0, Z.jsx)(Re, {}),
            ],
          })
        : null),
      (e[15] = a.available),
      (e[16] = p));
  let m;
  return (
    e[17] !== f || e[18] !== p
      ? ((m = (0, Z.jsxs)(q, { title: s, subtitle: c, children: [f, p] })),
        (e[17] = f),
        (e[18] = p),
        (e[19] = m))
      : (m = e[19]),
    m
  );
}
function Me(e) {
  let t = (0, X.c)(48),
    { computerUseAvailability: n } = e,
    r = j(),
    { selectedHostId: i } = G(),
    a = c(i).kind === `local`,
    o;
  t[0] === i ? (o = t[1]) : ((o = { hostId: i }), (t[0] = i), (t[1] = o));
  let s = I(o),
    l;
  t[2] === Symbol.for(`react.memo_cache_sentinel`) ? ((l = []), (t[2] = l)) : (l = t[2]);
  let u = ee(i, l),
    d = ve(i),
    f;
  t[3] !== d || t[4] !== u.availablePlugins
    ? ((f = C(u.availablePlugins, Te, d)), (t[3] = d), (t[4] = u.availablePlugins), (t[5] = f))
    : (f = t[5]);
  let p = f,
    m;
  t[6] !== d || t[7] !== u.availablePlugins
    ? ((m = Ke(u.availablePlugins, d)), (t[6] = d), (t[7] = u.availablePlugins), (t[8] = m))
    : (m = t[8]);
  let h = m,
    _ = h?.plugin.installed === !0,
    v = _ && h.plugin.enabled,
    b;
  t[9] !== h || t[10] !== _ || t[11] !== i
    ? ((b = { hostId: i, plugin: h, shouldReadBrowserExtension: _ }),
      (t[9] = h),
      (t[10] = _),
      (t[11] = i),
      (t[12] = b))
    : (b = t[12]);
  let x = Ne(b),
    S,
    w;
  if (
    t[13] !== h ||
    t[14] !== x ||
    t[15] !== n.available ||
    t[16] !== p ||
    t[17] !== s ||
    t[18] !== v ||
    t[19] !== a ||
    t[20] !== r
  ) {
    S = [];
    let e;
    if (
      (t[23] !== h || t[24] !== s || t[25] !== a
        ? ((e =
            h == null && a && !s.isLoading && !s.allowed
              ? [
                  {
                    description: (0, Z.jsx)(g, { ...N.restrictedAvailabilityDescription }),
                    icon: (0, Z.jsx)(`img`, {
                      alt: ``,
                      className: `h-full w-full object-contain`,
                      src: Ce,
                    }),
                    id: `chrome-unavailable`,
                    title: (0, Z.jsx)(g, { ...Q.googleChrome }),
                  },
                ]
              : []),
          (t[23] = h),
          (t[24] = s),
          (t[25] = a),
          (t[26] = e))
        : (e = t[26]),
      (w = e),
      n.available && p != null)
    ) {
      let e, n;
      t[27] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((e = (0, Z.jsx)(g, { ...Q.anyApp })),
          (n = (0, Z.jsx)(g, {
            id: `settings.computerUse.anyApp.description`,
            defaultMessage: `Let Codex control apps on your computer`,
            description: `Description for the Computer Use plugin control row`,
          })),
          (t[27] = e),
          (t[28] = n))
        : ((e = t[27]), (n = t[28]));
      let r;
      (t[29] === p
        ? (r = t[30])
        : ((r = { plugin: p, title: e, description: n }), (t[29] = p), (t[30] = r)),
        S.push(r));
    }
    if (h != null) {
      let e;
      t[31] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((e = (0, Z.jsx)(g, { ...Q.googleChrome })), (t[31] = e))
        : (e = t[31]);
      let n;
      t[32] === x ? (n = t[33]) : ((n = (0, Z.jsx)(Se, { status: x })), (t[32] = x), (t[33] = n));
      let i;
      t[34] !== v || t[35] !== r
        ? ((i = v
            ? (0, Z.jsx)(y, {
                color: `secondary`,
                onClick: () => {
                  r(ke);
                },
                size: `toolbar`,
                children: (0, Z.jsx)(g, {
                  id: `settings.computerUse.chrome.manage`,
                  defaultMessage: `Manage`,
                  description: `Button label to manage Google Chrome settings`,
                }),
              })
            : null),
          (t[34] = v),
          (t[35] = r),
          (t[36] = i))
        : (i = t[36]);
      let a;
      (t[37] !== h || t[38] !== n || t[39] !== i
        ? ((a = { plugin: h, title: e, description: n, action: i }),
          (t[37] = h),
          (t[38] = n),
          (t[39] = i),
          (t[40] = a))
        : (a = t[40]),
        S.push(a));
    }
    ((t[13] = h),
      (t[14] = x),
      (t[15] = n.available),
      (t[16] = p),
      (t[17] = s),
      (t[18] = v),
      (t[19] = a),
      (t[20] = r),
      (t[21] = S),
      (t[22] = w));
  } else ((S = t[21]), (w = t[22]));
  let T, E;
  t[41] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((T = (0, Z.jsx)(g, {
        id: `settings.computerUse.install.empty`,
        defaultMessage: `Computer Use plugins unavailable`,
        description: `Empty state shown when computer use plugins cannot be found`,
      })),
      (E = (0, Z.jsx)(g, {
        id: `settings.computerUse.install.button`,
        defaultMessage: `Install`,
        description: `Button label for installing a computer use plugin`,
      })),
      (t[41] = T),
      (t[42] = E))
    : ((T = t[41]), (E = t[42]));
  let O;
  return (
    t[43] !== S || t[44] !== u || t[45] !== i || t[46] !== w
      ? ((O = (0, Z.jsx)(D, {
          emptyStateTitle: T,
          installButtonLabel: E,
          items: S,
          pluginsQuery: u,
          selectedHostId: i,
          unavailableItems: w,
        })),
        (t[43] = S),
        (t[44] = u),
        (t[45] = i),
        (t[46] = w),
        (t[47] = O))
      : (O = t[47]),
    O
  );
}
function Ne(e) {
  let t = (0, X.c)(10),
    { hostId: n, plugin: r, shouldReadBrowserExtension: i } = e,
    a;
  t[0] !== n || t[1] !== r || t[2] !== i
    ? ((a = { enabled: i, hostId: n, plugin: r }), (t[0] = n), (t[1] = r), (t[2] = i), (t[3] = a))
    : (a = t[3]);
  let o = re(a),
    s = o?.[0] ?? null,
    c = i && o === void 0,
    l = s?.id ?? null,
    u;
  t[4] === l ? (u = t[5]) : ((u = { browserExtensionId: l }), (t[4] = l), (t[5] = u));
  let d = te(u),
    f = c || d.isLoading,
    p;
  return (
    t[6] !== s || t[7] !== d.isConnected || t[8] !== f
      ? ((p = {
          browserExtension: s,
          isConnected: d.isConnected,
          isInstalled: d.isConnected,
          isLoading: f,
        }),
        (t[6] = s),
        (t[7] = d.isConnected),
        (t[8] = f),
        (t[9] = p))
      : (p = t[9]),
    p
  );
}
function Pe() {
  let e = (0, X.c)(39),
    t = a(s),
    { platform: n } = P(),
    { selectedHostId: r } = G(),
    i;
  e[0] === Symbol.for(`react.memo_cache_sentinel`) ? ((i = []), (e[0] = i)) : (i = e[0]);
  let o = ee(r, i),
    c = ve(r),
    u;
  e[1] !== c || e[2] !== o.installedPlugins
    ? ((u = Ke(o.installedPlugins, c)), (e[1] = c), (e[2] = o.installedPlugins), (e[3] = u))
    : (u = e[3]);
  let d = u,
    p = d?.plugin.installed === !0,
    m = p && d.plugin.enabled,
    h;
  e[4] !== d || e[5] !== p || e[6] !== r
    ? ((h = { hostId: r, plugin: d, shouldReadBrowserExtension: p }),
      (e[4] = d),
      (e[5] = p),
      (e[6] = r),
      (e[7] = h))
    : (h = e[7]);
  let _ = Ne(h),
    v = _.browserExtension,
    x = v?.id,
    S = _.isInstalled,
    C = _.isLoading;
  if (o.isLoading && d == null) {
    let t, n;
    e[8] === Symbol.for(`react.memo_cache_sentinel`)
      ? ((t = (0, Z.jsx)(Le, {})),
        (n = (0, Z.jsx)(g, {
          id: `settings.computerUse.chrome.title`,
          defaultMessage: `Google Chrome`,
          description: `Title for Google Chrome computer use settings`,
        })),
        (e[8] = t),
        (e[9] = n))
      : ((t = e[8]), (n = e[9]));
    let r;
    return (
      e[10] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((r = (0, Z.jsx)(q, {
            backSlot: t,
            title: n,
            children: (0, Z.jsx)(`div`, {
              className: `flex min-h-[120px] items-center justify-center text-token-text-secondary`,
              children: (0, Z.jsx)(b, { className: `icon-xs` }),
            }),
          })),
          (e[10] = r))
        : (r = e[10]),
      r
    );
  }
  if (!o.isLoading && !m) {
    let t;
    return (
      e[11] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((t = (0, Z.jsx)(A, { to: `/settings/computer-use`, replace: !0 })), (e[11] = t))
        : (t = e[11]),
      t
    );
  }
  let D = v?.url == null,
    j;
  e[12] === v
    ? (j = e[13])
    : ((j = () => {
        v?.url != null &&
          l.dispatchMessage(`open-in-browser`, { url: v.url, useExternalBrowser: !0 });
      }),
      (e[12] = v),
      (e[13] = j));
  let M;
  e[14] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((M = (0, Z.jsx)(g, {
        id: `settings.computerUse.chrome.reinstallExtension`,
        defaultMessage: `Reinstall extension`,
        description: `Button label to reinstall the Google Chrome extension`,
      })),
      (e[14] = M))
    : (M = e[14]);
  let N;
  e[15] !== D || e[16] !== j
    ? ((N = (0, Z.jsx)(y, {
        color: `secondary`,
        disabled: D,
        onClick: j,
        size: `toolbar`,
        children: M,
      })),
      (e[15] = D),
      (e[16] = j),
      (e[17] = N))
    : (N = e[17]);
  let F;
  e[18] !== x || e[19] !== S || e[20] !== C || e[21] !== n || e[22] !== t
    ? ((F =
        n === `macOS` || n === `windows`
          ? (0, Z.jsx)(y, {
              color: `danger`,
              disabled: x == null || C || !S,
              onClick: () => {
                x != null &&
                  f(`chrome-extension-settings-open`, { params: { extensionId: x } }).catch(() => {
                    t.get(k).danger(
                      (0, Z.jsx)(g, {
                        id: `settings.computerUse.chrome.openExtensionSettingsError`,
                        defaultMessage: `Unable to open Chrome extension settings`,
                        description: `Toast shown when the app fails to open Chrome extension settings`,
                      }),
                    );
                  });
              },
              size: `toolbar`,
              children: (0, Z.jsx)(g, {
                id: `settings.computerUse.chrome.removeExtension`,
                defaultMessage: `Remove extension`,
                description: `Button label to remove the Google Chrome extension`,
              }),
            })
          : null),
      (e[18] = x),
      (e[19] = S),
      (e[20] = C),
      (e[21] = n),
      (e[22] = t),
      (e[23] = F))
    : (F = e[23]);
  let I;
  e[24] !== N || e[25] !== F
    ? ((I = (0, Z.jsxs)(`div`, { className: `flex items-center gap-2`, children: [N, F] })),
      (e[24] = N),
      (e[25] = F),
      (e[26] = I))
    : (I = e[26]);
  let L;
  e[27] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((L = (0, Z.jsx)(Le, {})), (e[27] = L))
    : (L = e[27]);
  let R;
  e[28] !== S || e[29] !== C
    ? ((R = C ? null : (0, Z.jsx)(Ie, { installed: S })), (e[28] = S), (e[29] = C), (e[30] = R))
    : (R = e[30]);
  let z;
  e[31] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((z = (0, Z.jsx)(g, {
        id: `settings.computerUse.chrome.title`,
        defaultMessage: `Google Chrome`,
        description: `Title for Google Chrome computer use settings`,
      })),
      (e[31] = z))
    : (z = e[31]);
  let B;
  e[32] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((B = (0, Z.jsx)(Y.Header, {
        title: (0, Z.jsx)(g, {
          id: `settings.computerUse.chrome.permissions.title`,
          defaultMessage: `Permissions`,
          description: `Title for Google Chrome browser permissions settings section`,
        }),
      })),
      (e[32] = B))
    : (B = e[32]);
  let V, H, U;
  e[33] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((V = (0, Z.jsxs)(Y, {
        children: [
          B,
          (0, Z.jsx)(Y.Content, {
            children: (0, Z.jsxs)(_e, {
              children: [
                (0, Z.jsx)(E, {}),
                (0, Z.jsx)(w, {}),
                (0, Z.jsx)(Fe, { kind: `download` }),
                (0, Z.jsx)(Fe, { kind: `upload` }),
              ],
            }),
          }),
        ],
      })),
      (H = (0, Z.jsx)(O, { surface: `googleChrome` })),
      (U = (0, Z.jsx)(T, {})),
      (e[33] = V),
      (e[34] = H),
      (e[35] = U))
    : ((V = e[33]), (H = e[34]), (U = e[35]));
  let W;
  return (
    e[36] !== R || e[37] !== I
      ? ((W = (0, Z.jsxs)(q, {
          action: I,
          backSlot: L,
          subtitle: R,
          subtitleClassName: `flex`,
          title: z,
          children: [V, H, U],
        })),
        (e[36] = R),
        (e[37] = I),
        (e[38] = W))
      : (W = e[38]),
    W
  );
}
function Fe(e) {
  let t = (0, X.c)(39),
    { kind: n } = e,
    r = a(s),
    { data: i, isLoading: c } = o(ge),
    l = he(),
    u = n === `download` ? i?.downloadApprovalMode : i?.uploadApprovalMode,
    d;
  t[0] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((d = (0, Z.jsx)(g, {
        id: `settings.browserUse.approval.alwaysAsk.label`,
        defaultMessage: `Always ask`,
        description: `Label for browser use always ask approval mode`,
      })),
      (t[0] = d))
    : (d = t[0]);
  let f;
  t[1] === n
    ? (f = t[2])
    : ((f = {
        id: `alwaysAsk`,
        label: d,
        description:
          n === `download`
            ? (0, Z.jsx)(g, {
                id: `settings.browserUse.downloadApproval.alwaysAsk.description`,
                defaultMessage: `Ask before downloading files`,
                description: `Description for browser use always ask download approval mode`,
              })
            : (0, Z.jsx)(g, {
                id: `settings.browserUse.uploadApproval.alwaysAsk.description`,
                defaultMessage: `Ask before uploading files`,
                description: `Description for browser use always ask upload approval mode`,
              }),
      }),
      (t[1] = n),
      (t[2] = f));
  let p;
  t[3] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((p = (0, Z.jsx)(g, {
        id: `settings.browserUse.approval.neverAsk.label`,
        defaultMessage: `Always allow`,
        description: `Label for browser use never ask approval mode`,
      })),
      (t[3] = p))
    : (p = t[3]);
  let m;
  t[4] === n
    ? (m = t[5])
    : ((m = {
        id: `neverAsk`,
        label: p,
        description:
          n === `download`
            ? (0, Z.jsx)(g, {
                id: `settings.browserUse.downloadApproval.neverAsk.description`,
                defaultMessage: `Download files without asking`,
                description: `Description for browser use never ask download approval mode`,
              })
            : (0, Z.jsx)(g, {
                id: `settings.browserUse.uploadApproval.neverAsk.description`,
                defaultMessage: `Upload files without asking`,
                description: `Description for browser use never ask upload approval mode`,
              }),
      }),
      (t[4] = n),
      (t[5] = m));
  let h;
  t[6] !== f || t[7] !== m ? ((h = [f, m]), (t[6] = f), (t[7] = m), (t[8] = h)) : (h = t[8]);
  let _ = h,
    v;
  t[9] !== u || t[10] !== _
    ? ((v = _.find((e) => e.id === u) ?? _[0]), (t[9] = u), (t[10] = _), (t[11] = v))
    : (v = t[11]);
  let y = v,
    b = l.isPending,
    S;
  t[12] !== u || t[13] !== b || t[14] !== n || t[15] !== r || t[16] !== l
    ? ((S = async (e) => {
        if (!(e === u || b))
          try {
            await l.mutateAsync({ approvalMode: e, kind: n });
          } catch {
            r.get(k).danger(
              n === `download`
                ? (0, Z.jsx)(g, {
                    id: `settings.browserUse.downloadApproval.saveError`,
                    defaultMessage: `Unable to save download setting`,
                    description: `Toast shown when saving browser use download approval mode fails`,
                  })
                : (0, Z.jsx)(g, {
                    id: `settings.browserUse.uploadApproval.saveError`,
                    defaultMessage: `Unable to save upload setting`,
                    description: `Toast shown when saving browser use upload approval mode fails`,
                  }),
            );
          }
      }),
      (t[12] = u),
      (t[13] = b),
      (t[14] = n),
      (t[15] = r),
      (t[16] = l),
      (t[17] = S))
    : (S = t[17]);
  let C = S,
    w,
    T;
  t[18] === n
    ? ((w = t[19]), (T = t[20]))
    : ((w =
        n === `download`
          ? (0, Z.jsx)(g, {
              id: `settings.browserUse.downloadApproval.label`,
              defaultMessage: `Downloads`,
              description: `Label for browser use download approval mode setting`,
            })
          : (0, Z.jsx)(g, {
              id: `settings.browserUse.uploadApproval.label`,
              defaultMessage: `Uploads`,
              description: `Label for browser use upload approval mode setting`,
            })),
      (T =
        n === `download`
          ? (0, Z.jsx)(g, {
              id: `settings.browserUse.downloadApproval.description`,
              defaultMessage: `Choose if Codex asks before downloading files from websites`,
              description: `Description for browser use download approval mode setting`,
            })
          : (0, Z.jsx)(g, {
              id: `settings.browserUse.uploadApproval.description`,
              defaultMessage: `Choose if Codex asks before uploading files to websites`,
              description: `Description for browser use upload approval mode setting`,
            })),
      (t[18] = n),
      (t[19] = w),
      (t[20] = T));
  let E = c || b,
    D = c || b,
    O = y?.label,
    A;
  t[21] === O
    ? (A = t[22])
    : ((A = (0, Z.jsx)(`span`, { className: `truncate`, children: O })), (t[21] = O), (t[22] = A));
  let j;
  t[23] !== D || t[24] !== A
    ? ((j = (0, Z.jsx)(me, { className: `w-[152px]`, disabled: D, children: A })),
      (t[23] = D),
      (t[24] = A),
      (t[25] = j))
    : (j = t[25]);
  let M;
  t[26] !== u || t[27] !== C || t[28] !== b || t[29] !== _
    ? ((M = (0, Z.jsx)(`div`, {
        className: `flex flex-col`,
        children: _.map((e) => {
          let t = e.id === u;
          return (0, Z.jsx)(
            ie.Item,
            {
              allowWrap: !0,
              disabled: b,
              RightIcon: t ? x : void 0,
              onSelect: () => {
                C(e.id);
              },
              children: (0, Z.jsxs)(`div`, {
                className: `flex min-w-0 flex-col gap-0.5`,
                children: [
                  (0, Z.jsx)(`span`, { className: `truncate`, children: e.label }),
                  (0, Z.jsx)(`span`, {
                    className: `truncate text-sm text-token-text-secondary`,
                    children: e.description,
                  }),
                ],
              }),
            },
            e.id,
          );
        }),
      })),
      (t[26] = u),
      (t[27] = C),
      (t[28] = b),
      (t[29] = _),
      (t[30] = M))
    : (M = t[30]);
  let N;
  t[31] !== E || t[32] !== j || t[33] !== M
    ? ((N = (0, Z.jsx)(ae, {
        contentWidth: `menuWide`,
        align: `end`,
        disabled: E,
        triggerButton: j,
        children: M,
      })),
      (t[31] = E),
      (t[32] = j),
      (t[33] = M),
      (t[34] = N))
    : (N = t[34]);
  let P;
  return (
    t[35] !== N || t[36] !== w || t[37] !== T
      ? ((P = (0, Z.jsx)(J, { label: w, description: T, control: N })),
        (t[35] = N),
        (t[36] = w),
        (t[37] = T),
        (t[38] = P))
      : (P = t[38]),
    P
  );
}
function Ie(e) {
  let t = (0, X.c)(12),
    { installed: n } = e,
    r = n
      ? `bg-[var(--color-background-status-success)] text-[var(--color-text-success)]`
      : `bg-token-charts-red/10 text-token-charts-red`,
    i;
  t[0] === r
    ? (i = t[1])
    : ((i = v(`inline-flex w-max items-center gap-2 rounded-lg px-2 py-1 text-sm font-medium`, r)),
      (t[0] = r),
      (t[1] = i));
  let a = n ? `bg-[var(--color-icon-success)]` : `bg-token-charts-red`,
    o;
  t[2] === a ? (o = t[3]) : ((o = v(`h-2 w-2 rounded-full`, a)), (t[2] = a), (t[3] = o));
  let s;
  t[4] === o ? (s = t[5]) : ((s = (0, Z.jsx)(`span`, { className: o })), (t[4] = o), (t[5] = s));
  let c;
  t[6] === n
    ? (c = t[7])
    : ((c = n
        ? (0, Z.jsx)(g, {
            id: `settings.computerUse.chrome.connected`,
            defaultMessage: `Connected`,
            description: `Status badge for connected Google Chrome extension`,
          })
        : (0, Z.jsx)(g, {
            id: `settings.computerUse.chrome.notConnected`,
            defaultMessage: `Not connected`,
            description: `Status badge for a Google Chrome extension that is not connected`,
          })),
      (t[6] = n),
      (t[7] = c));
  let l;
  return (
    t[8] !== i || t[9] !== s || t[10] !== c
      ? ((l = (0, Z.jsxs)(`span`, { className: i, children: [s, c] })),
        (t[8] = i),
        (t[9] = s),
        (t[10] = c),
        (t[11] = l))
      : (l = t[11]),
    l
  );
}
function Le() {
  let e = (0, X.c)(11),
    t = j(),
    n;
  e[0] === t
    ? (n = e[1])
    : ((n = () => {
        t(`/settings/computer-use`);
      }),
      (e[0] = t),
      (e[1] = n));
  let r, i;
  e[2] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((r = (0, Z.jsx)(U, { className: `icon-xs` })),
      (i = (0, Z.jsx)(g, {
        id: `settings.computerUse.chrome.back`,
        defaultMessage: `Back`,
        description: `Button label to go back to Computer use settings`,
      })),
      (e[2] = r),
      (e[3] = i))
    : ((r = e[2]), (i = e[3]));
  let a;
  e[4] === n
    ? (a = e[5])
    : ((a = (0, Z.jsxs)(y, { color: `ghost`, size: `toolbar`, onClick: n, children: [r, i] })),
      (e[4] = n),
      (e[5] = a));
  let o, s;
  e[6] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((o = (0, Z.jsx)(g, {
        id: `settings.computerUse.breadcrumb.computerUse`,
        defaultMessage: `Computer use`,
        description: `Computer use breadcrumb label`,
      })),
      (s = (0, Z.jsx)(S, { className: `icon-xs text-token-text-secondary` })),
      (e[6] = o),
      (e[7] = s))
    : ((o = e[6]), (s = e[7]));
  let c;
  e[8] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((c = (0, Z.jsxs)(`div`, {
        className: `flex items-center gap-1`,
        children: [
          o,
          s,
          (0, Z.jsx)(`span`, {
            className: `text-token-text-primary`,
            children: (0, Z.jsx)(g, {
              id: `settings.computerUse.chrome.breadcrumb.googleChrome`,
              defaultMessage: `Google Chrome`,
              description: `Google Chrome breadcrumb label`,
            }),
          }),
        ],
      })),
      (e[8] = c))
    : (c = e[8]);
  let l;
  return (
    e[9] === a
      ? (l = e[10])
      : ((l = (0, Z.jsxs)(`nav`, {
          className: `flex items-center gap-2 text-sm font-medium text-token-text-secondary`,
          children: [a, c],
        })),
        (e[9] = a),
        (e[10] = l)),
    l
  );
}
function Re() {
  let e = (0, X.c)(20),
    t = o(le),
    n = d(),
    r;
  e[0] === n
    ? (r = e[1])
    : ((r = {
        mutationFn: fe,
        onSuccess: (e) => {
          n.setQueryData(p(`computer-use-sound-mode-read`), e);
        },
      }),
      (e[0] = n),
      (e[1] = r));
  let i = u(r);
  if (t.data == null) return null;
  let a;
  e[2] === t.data ? (a = e[3]) : ((a = He(t.data) ? t.data : Ae), (e[2] = t.data), (e[3] = a));
  let s = a,
    c;
  e[4] === s
    ? (c = e[5])
    : ((c = $.find((e) => e.value === s) ?? $.find(ze) ?? $[0]), (e[4] = s), (e[5] = c));
  let l = c,
    f = t.isLoading || i.isPending,
    m;
  e[6] === l.label
    ? (m = e[7])
    : ((m = (0, Z.jsx)(g, { ...l.label })), (e[6] = l.label), (e[7] = m));
  let h;
  e[8] !== f || e[9] !== m
    ? ((h = (0, Z.jsx)(me, {
        className: `w-max max-w-full`,
        contentClassName: `flex-none`,
        disabled: f,
        children: m,
      })),
      (e[8] = f),
      (e[9] = m),
      (e[10] = h))
    : (h = e[10]);
  let _ = t.isLoading || i.isPending,
    v;
  e[11] !== s || e[12] !== i
    ? ((v = $.map((e) =>
        (0, Z.jsx)(
          ie.Item,
          {
            onSelect: () => {
              i.mutate(e.value);
            },
            RightIcon: s === e.value ? x : void 0,
            children: (0, Z.jsx)(`span`, {
              className: `text-sm`,
              children: (0, Z.jsx)(g, { ...e.label }),
            }),
          },
          e.value,
        ),
      )),
      (e[11] = s),
      (e[12] = i),
      (e[13] = v))
    : (v = e[13]);
  let y;
  e[14] === v
    ? (y = e[15])
    : ((y = (0, Z.jsx)(`div`, { className: `w-max max-w-[calc(100vw-2rem)]`, children: v })),
      (e[14] = v),
      (e[15] = y));
  let b;
  return (
    e[16] !== h || e[17] !== _ || e[18] !== y
      ? ((b = (0, Z.jsx)(ae, { triggerButton: h, align: `start`, disabled: _, children: y })),
        (e[16] = h),
        (e[17] = _),
        (e[18] = y),
        (e[19] = b))
      : (b = e[19]),
    b
  );
}
function ze(e) {
  return e.value === Ae;
}
function Be() {
  let e = (0, X.c)(25),
    t = a(s),
    n = h(),
    r = d(),
    i = o(de),
    c;
  e[0] !== r || e[1] !== t
    ? ((c = (e) => {
        if (
          (r.setQueryData(p(`computer-use-background-auth-read`), (t) => ({
            computerIconDataURL: t?.computerIconDataURL ?? null,
            enabled: e,
            lockIconDataURL: t?.lockIconDataURL ?? null,
          })),
          e != null)
        ) {
          if (e) {
            t.get(k).success(
              (0, Z.jsx)(g, {
                id: `settings.computerUse.backgroundAuth.enabled`,
                defaultMessage: `Locked use enabled`,
                description: `Toast shown after enabling Locked use`,
              }),
            );
            return;
          }
          t.get(k).success(
            (0, Z.jsx)(g, {
              id: `settings.computerUse.backgroundAuth.disabled`,
              defaultMessage: `Locked use disabled`,
              description: `Toast shown after disabling Locked use`,
            }),
          );
        }
      }),
      (e[0] = r),
      (e[1] = t),
      (e[2] = c))
    : (c = e[2]);
  let l;
  e[3] === t
    ? (l = e[4])
    : ((l = () => {
        t.get(k).danger(
          (0, Z.jsx)(g, {
            id: `settings.computerUse.backgroundAuth.saveError`,
            defaultMessage: `Unable to update Locked use`,
            description: `Toast shown when updating Locked use fails`,
          }),
        );
      }),
      (e[3] = t),
      (e[4] = l));
  let f;
  e[5] !== c || e[6] !== l
    ? ((f = { mutationFn: ue, onSuccess: c, onError: l }), (e[5] = c), (e[6] = l), (e[7] = f))
    : (f = e[7]);
  let m = u(f);
  if (i.data?.enabled == null) return null;
  let _;
  e[8] !== i.data.computerIconDataURL || e[9] !== i.data.lockIconDataURL
    ? ((_ =
        i.data.computerIconDataURL != null && i.data.lockIconDataURL != null
          ? (0, Z.jsx)(`span`, {
              className: `flex h-10 w-10 items-center justify-center`,
              children: (0, Z.jsx)(`span`, {
                className: `origin-center scale-[0.65]`,
                children: (0, Z.jsx)(be, {
                  computerImageDataUrl: i.data.computerIconDataURL,
                  lockImageDataUrl: i.data.lockIconDataURL,
                }),
              }),
            })
          : null),
      (e[8] = i.data.computerIconDataURL),
      (e[9] = i.data.lockIconDataURL),
      (e[10] = _))
    : (_ = e[10]);
  let v;
  e[11] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((v = (0, Z.jsx)(g, {
        id: `settings.computerUse.backgroundAuth.label`,
        defaultMessage: `Locked use`,
        description: `Label for the Locked use toggle`,
      })),
      (e[11] = v))
    : (v = e[11]);
  let y;
  e[12] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((y = (0, Z.jsx)(g, {
        id: `settings.computerUse.backgroundAuth.description`,
        defaultMessage: `Let Codex use your Mac when it's locked. <a>Learn more</a>`,
        description: `Description for the Locked use toggle`,
        values: { a: Ve },
      })),
      (e[12] = y))
    : (y = e[12]);
  let b;
  e[13] === n
    ? (b = e[14])
    : ((b = n.formatMessage({
        id: `settings.computerUse.backgroundAuth.ariaLabel`,
        defaultMessage: `Enable Locked use`,
        description: `Accessible label for the Locked use toggle`,
      })),
      (e[13] = n),
      (e[14] = b));
  let x = i.isLoading || m.isPending,
    S;
  e[15] === m
    ? (S = e[16])
    : ((S = (e) => {
        m.mutate(e);
      }),
      (e[15] = m),
      (e[16] = S));
  let C;
  e[17] !== i.data.enabled || e[18] !== b || e[19] !== x || e[20] !== S
    ? ((C = (0, Z.jsx)(se, { ariaLabel: b, checked: i.data.enabled, disabled: x, onChange: S })),
      (e[17] = i.data.enabled),
      (e[18] = b),
      (e[19] = x),
      (e[20] = S),
      (e[21] = C))
    : (C = e[21]);
  let w;
  return (
    e[22] !== _ || e[23] !== C
      ? ((w = (0, Z.jsx)(_e, {
          children: (0, Z.jsx)(J, { icon: _, label: v, description: y, control: C }),
        })),
        (e[22] = _),
        (e[23] = C),
        (e[24] = w))
      : (w = e[24]),
    w
  );
}
function Ve(e) {
  return (0, Z.jsx)(`a`, {
    className: `inline-flex text-token-text-link-foreground`,
    href: W,
    target: `_blank`,
    rel: `noreferrer`,
    children: e,
  });
}
function He(e) {
  return $.some((t) => t.value === e);
}
function Ue() {
  let e = (0, X.c)(7),
    t = a(s),
    n = o(K),
    r,
    i;
  if (
    (e[0] === t
      ? ((r = e[1]), (i = e[2]))
      : ((r = () => {
          t.get(K).refetch();
        }),
        (i = [t]),
        (e[0] = t),
        (e[1] = r),
        (e[2] = i)),
    (0, ye.useEffect)(r, i),
    n.isLoading)
  ) {
    let t;
    return (
      e[3] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((t = (0, Z.jsxs)(`div`, {
            className: `flex items-center gap-2 p-4 text-sm text-token-text-secondary`,
            children: [
              (0, Z.jsx)(b, { className: `icon-xs` }),
              (0, Z.jsx)(g, {
                id: `settings.computerUse.allowedApps.loading`,
                defaultMessage: `Loading allowed apps`,
                description: `Message shown while loading computer use allowed apps`,
              }),
            ],
          })),
          (e[3] = t))
        : (t = e[3]),
      t
    );
  }
  if (n.isError) {
    let t;
    return (
      e[4] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((t = (0, Z.jsx)(`div`, {
            className: `p-4 text-sm text-token-text-secondary`,
            children: (0, Z.jsx)(g, {
              id: `settings.computerUse.allowedApps.loadError`,
              defaultMessage: `Unable to load allowed apps.`,
              description: `Message shown when computer use allowed apps fail to load`,
            }),
          })),
          (e[4] = t))
        : (t = e[4]),
      t
    );
  }
  let c;
  return (
    e[5] === n.data
      ? (c = e[6])
      : ((c = (0, Z.jsx)(We, { approvals: n.data })), (e[5] = n.data), (e[6] = c)),
    c
  );
}
function We(e) {
  let t = (0, X.c)(41),
    { approvals: n } = e,
    r = a(s),
    i = h(),
    [o, c] = (0, ye.useState)(null),
    l;
  t[0] === r
    ? (l = t[1])
    : ((l = {
        mutationFn: Ge,
        onSuccess: (e) => {
          (r.query.setData(K, e),
            r
              .get(k)
              .success(
                (0, Z.jsx)(g, {
                  id: `settings.computerUse.allowedApps.saved`,
                  defaultMessage: `Allowed app removed`,
                  description: `Toast shown after removing a computer use allowed app`,
                }),
              ));
        },
        onError: () => {
          r.get(k).danger(
            (0, Z.jsx)(g, {
              id: `settings.computerUse.allowedApps.saveError`,
              defaultMessage: `Unable to save allowed apps`,
              description: `Toast shown when saving computer use allowed apps fails`,
            }),
          );
        },
      }),
      (t[0] = r),
      (t[1] = l));
  let d = u(l),
    f;
  t[2] !== o || t[3] !== d
    ? ((f = async () => {
        o != null && (await d.mutateAsync(o.bundleIdentifier), c(null));
      }),
      (t[2] = o),
      (t[3] = d),
      (t[4] = f))
    : (f = t[4]);
  let p = f,
    m;
  t[5] === n?.approvedApps
    ? (m = t[6])
    : ((m = n?.approvedApps ?? []), (t[5] = n?.approvedApps), (t[6] = m));
  let _;
  t[7] !== i || t[8] !== d.isPending || t[9] !== m
    ? ((_ = (0, Z.jsx)(qe, {
        approvedApps: m,
        intl: i,
        isSaving: d.isPending,
        onRequestRemoval: c,
      })),
      (t[7] = i),
      (t[8] = d.isPending),
      (t[9] = m),
      (t[10] = _))
    : (_ = t[10]);
  let v = o != null,
    b;
  t[11] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((b = (e) => {
        e || c(null);
      }),
      (t[11] = b))
    : (b = t[11]);
  let x = o?.displayName ?? ``,
    S;
  t[12] === x
    ? (S = t[13])
    : ((S = (0, Z.jsx)(g, {
        id: `settings.computerUse.allowedApps.removeDialogTitle`,
        defaultMessage: `Remove “{displayName}” from always allowed apps?`,
        description: `Title for dialog confirming removal of a computer use allowed app`,
        values: { displayName: x },
      })),
      (t[12] = x),
      (t[13] = S));
  let C = o?.displayName ?? ``,
    w;
  t[14] === C
    ? (w = t[15])
    : ((w = (0, Z.jsx)(g, {
        id: `settings.computerUse.allowedApps.removeDialogSubtitle`,
        defaultMessage: `Codex will ask to use “{displayName}” in the next computer use session.`,
        description: `Subtitle for dialog confirming removal of a computer use allowed app`,
        values: { displayName: C },
      })),
      (t[14] = C),
      (t[15] = w));
  let T;
  t[16] !== w || t[17] !== S
    ? ((T = (0, Z.jsx)(R, { children: (0, Z.jsx)(z, { title: S, subtitle: w }) })),
      (t[16] = w),
      (t[17] = S),
      (t[18] = T))
    : (T = t[18]);
  let E, D;
  t[19] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((E = () => {
        c(null);
      }),
      (D = (0, Z.jsx)(g, {
        id: `settings.computerUse.allowedApps.removeDialogCancel`,
        defaultMessage: `Cancel`,
        description: `Cancel button label for remove allowed app dialog`,
      })),
      (t[19] = E),
      (t[20] = D))
    : ((E = t[19]), (D = t[20]));
  let O;
  t[21] === d.isPending
    ? (O = t[22])
    : ((O = (0, Z.jsx)(y, { color: `ghost`, disabled: d.isPending, onClick: E, children: D })),
      (t[21] = d.isPending),
      (t[22] = O));
  let A;
  t[23] === p
    ? (A = t[24])
    : ((A = () => {
        p();
      }),
      (t[23] = p),
      (t[24] = A));
  let j;
  t[25] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((j = (0, Z.jsx)(g, {
        id: `settings.computerUse.allowedApps.removeDialogConfirm`,
        defaultMessage: `Remove`,
        description: `Confirm button label for remove allowed app dialog`,
      })),
      (t[25] = j))
    : (j = t[25]);
  let M;
  t[26] !== d.isPending || t[27] !== A
    ? ((M = (0, Z.jsx)(y, { color: `danger`, loading: d.isPending, onClick: A, children: j })),
      (t[26] = d.isPending),
      (t[27] = A),
      (t[28] = M))
    : (M = t[28]);
  let N;
  t[29] !== O || t[30] !== M
    ? ((N = (0, Z.jsx)(R, { children: (0, Z.jsxs)(H, { children: [O, M] }) })),
      (t[29] = O),
      (t[30] = M),
      (t[31] = N))
    : (N = t[31]);
  let P;
  t[32] !== T || t[33] !== N
    ? ((P = (0, Z.jsxs)(V, { children: [T, N] })), (t[32] = T), (t[33] = N), (t[34] = P))
    : (P = t[34]);
  let F;
  t[35] !== P || t[36] !== v
    ? ((F = (0, Z.jsx)(B, { open: v, onOpenChange: b, size: `compact`, children: P })),
      (t[35] = P),
      (t[36] = v),
      (t[37] = F))
    : (F = t[37]);
  let I;
  return (
    t[38] !== F || t[39] !== _
      ? ((I = (0, Z.jsxs)(Z.Fragment, { children: [_, F] })), (t[38] = F), (t[39] = _), (t[40] = I))
      : (I = t[40]),
    I
  );
}
async function Ge(e) {
  let t = await ce(e);
  if (t == null) throw Error(`Computer use app approvals are unavailable`);
  return t;
}
function Ke(e, t) {
  let n = L();
  return (
    (n === m.Dev ? C(e, De, t) : m.isInternal(n) ? (C(e, Oe, t) ?? C(e, De, t)) : null) ??
    C(e, Ee, t)
  );
}
function qe(e) {
  let t = (0, X.c)(12),
    { approvedApps: n, intl: r, isSaving: i, onRequestRemoval: a } = e;
  if (n.length === 0) {
    let e;
    return (
      t[0] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((e = (0, Z.jsx)(J, {
            className: `justify-center`,
            label: (0, Z.jsx)(`span`, {
              className: `text-token-text-secondary`,
              children: (0, Z.jsx)(g, {
                id: `settings.computerUse.allowedApps.emptyTitle`,
                defaultMessage: `None yet`,
                description: `Empty state title for computer use allowed apps`,
              }),
            }),
            control: null,
          })),
          (t[0] = e))
        : (e = t[0]),
      e
    );
  }
  let o;
  if (t[1] !== n || t[2] !== r || t[3] !== i || t[4] !== a) {
    let e;
    (t[6] !== r || t[7] !== i || t[8] !== a
      ? ((e = (e) =>
          (0, Z.jsx)(
            J,
            {
              icon: (0, Z.jsx)(Je, { approvedApp: e }),
              label: (0, Z.jsx)(`span`, { className: `font-medium`, children: e.displayName }),
              description: null,
              control: (0, Z.jsx)(y, {
                "aria-label": r.formatMessage(
                  {
                    id: `settings.computerUse.allowedApps.removeAriaLabel`,
                    defaultMessage: `Remove {displayName}`,
                    description: `Aria label for button that removes a computer use allowed app`,
                  },
                  { displayName: e.displayName },
                ),
                color: `ghost`,
                disabled: i,
                onClick: () => {
                  a(e);
                },
                size: `icon`,
                children: (0, Z.jsx)(oe, { className: `icon-2xs` }),
              }),
            },
            e.bundleIdentifier,
          )),
        (t[6] = r),
        (t[7] = i),
        (t[8] = a),
        (t[9] = e))
      : (e = t[9]),
      (o = n.map(e)),
      (t[1] = n),
      (t[2] = r),
      (t[3] = i),
      (t[4] = a),
      (t[5] = o));
  } else o = t[5];
  let s;
  return (
    t[10] === o
      ? (s = t[11])
      : ((s = (0, Z.jsx)(Z.Fragment, { children: o })), (t[10] = o), (t[11] = s)),
    s
  );
}
function Je(e) {
  let t = (0, X.c)(6),
    { approvedApp: n } = e;
  if (n.iconDataURL != null) {
    let e;
    return (
      t[0] === n.iconDataURL
        ? (e = t[1])
        : ((e = (0, Z.jsx)(`img`, {
            alt: ``,
            className: `h-9 w-9 shrink-0 rounded-md`,
            draggable: !1,
            src: n.iconDataURL,
          })),
          (t[0] = n.iconDataURL),
          (t[1] = e)),
      e
    );
  }
  let r;
  t[2] === n.displayName
    ? (r = t[3])
    : ((r = n.displayName.slice(0, 1).toUpperCase()), (t[2] = n.displayName), (t[3] = r));
  let i;
  return (
    t[4] === r
      ? (i = t[5])
      : ((i = (0, Z.jsx)(`div`, {
          className: `flex h-9 w-9 shrink-0 items-center justify-center rounded-md bg-token-foreground/10 text-sm font-semibold text-token-description-foreground`,
          children: r,
        })),
        (t[4] = r),
        (t[5] = i)),
    i
  );
}
export { je as ComputerUseSettings };
//# sourceMappingURL=computer-use-settings.js.map
