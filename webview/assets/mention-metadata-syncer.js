...
import { Ar as T, Gi as E, Qi as D, Zi as O } from "./src-3.js";
...
import { t as ee } from "./image-preview-dialog-Bb-vTCJ6.js";
...
async function Ee({
  ...
  let { config: g } = await s(`read-config-for-host`, { hostId: i, includeLayers: !1, cwd: r }),
    _ = x(g);
  return (
  ...
  if (
    (e.get(g, i) &&
      (await s(`maybe-resume-conversation`, {
        ...
        collaborationMode: r.collaborationMode ?? _,
      })),
    e.get(v, i)?.at(-1)?.status === `inProgress`)
  )
    return Me({
      scope: e,
      manager: t,
...
async function Me({
  ...
  let y = d(v.imageAttachments),
    b = [
      ...
      ...Te(v, h, { shouldRestrictRemoteHostImageSize: !1 }),
    ],
