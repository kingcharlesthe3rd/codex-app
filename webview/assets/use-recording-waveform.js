...
import { l as i } from "./vscode-api.js";
import {
  Bi as a,
  C as o,
  Ki as s,
...
import { n as h } from "./setting-storage.js";
import { a as g, n as _ } from "./request.js";
var v = e(t()),
  y = n(),
  b = (e) =>
    (0, y.jsx)(`svg`, {
...
var C = [`codex-app`, `dictation`, `streaming`].join(`-`),
  w = d({ code: s(), message: s(), retryable: f() }),
  T = d({
    session_id: s(),
    status: u([`active`, `closed`]),
    config: d({
      provider_mode: u([`buffered`, `streaming_sse`]),
      transcript_delivery_mode: u([`final_only`, `segment`, `delta`]),
    }),
  }),
  E = l(`type`, [
    d({ type: c(`session.started`), sequence_no: a(), session: T }),
    d({ type: c(`session.updated`), sequence_no: a(), session: T }),
    d({ type: c(`speech.started`), sequence_no: a(), utterance_id: s() }),
    d({ type: c(`speech.stopped`), sequence_no: a(), utterance_id: s() }),
    d({
      type: c(`transcript.delta`),
      sequence_no: a(),
      utterance_id: s(),
      revision: a(),
      text: s(),
    ...
    d({
      type: c(`transcript.final`),
      sequence_no: a(),
      utterance_id: s(),
      revision: a(),
      text: s(),
    }),
    d({ type: c(`transcript.failed`), sequence_no: a(), utterance_id: s().nullish(), error: w }),
    d({ type: c(`session.error`), sequence_no: a(), fatal: f(), error: w }),
  ]),
  D = 1e4,
  O = 8e3,
  ...
  A = class {
    ...
    async start(e) {
      ...
      if (t == null) throw new k(`AudioContext is not available for streaming dictation.`);
      let n = new t();
      ((this.audioContext = n),
    ...
    async finish() {
      return (this.stopAudioCapture(), await this.client.finish(), z(this.transcriptState));
    }
    close() {
  ...
  j = class {
    ...
    async connect(e) {
      ...
      return new Promise((t, n) => {
        ...
        let o = !1,
          ...
          d = window.setTimeout(() => {
            let e = new k(`Dictation stream timed out before session.start completed.`);
            ((c = e), a.close(), l(e));
          }, D);
        (a.addEventListener(
          `open`,
          ...
          a.addEventListener(`message`, (e) => {
            let t = B(e.data);
            if (t == null) {
              let e = new k(`Dictation stream returned an invalid event payload.`);
              ((c = e), l(e), a.close());
              return;
            }
    ...
    appendPCM16(e) {
      let t = { type: `audio.append`, audio: g(e) };
        ...
        ? this.finishPromise
          ...
          : ((this.finishPromise = new Promise((e, t) => {
              let n = window.setTimeout(() => {
                let e = new k(`Dictation stream timed out while closing the session.`);
    ...
    close() {
      ((this.pendingStartupAudioAppends = null),
        this.rejectConnectBeforeWebsocket?.(
          new k(`Dictation stream closed before websocket connection started.`),
        ),
        (this.rejectConnectBeforeWebsocket = null),
...
async function M() {
  return (await i.getInstance().post(`/codex/dictation-stream-connect-info`, void 0)).body;
}
function N(e) {
  return {
    type: `session.start`,
...
function L(e, t) {
  switch (t.type) {
    ...
    case `transcript.final`:
      (R(e, t.utterance_id), (e.finalTextByUtteranceId[t.utterance_id] = t.text));
      return;
...
var V = `gpt-5.4-mini`,
  te = d({
    output: p(d({ type: s(), content: p(d({ type: s(), text: s().optional() })).optional() })),
  }),
  ne = d({
    type: s().optional(),
    delta: s().optional(),
    ...
    error: m([s(), d({ message: s().optional() })]).optional(),
  }).passthrough();
async function re({ instructions: e, input: t, model: n = V }) {
  let r = t.trim();
...
function ie(e) {
  return new Promise((t, n) => {
    ...
    function l(e) {
      s || ((s = !0), o != null && i.getInstance().cancelStream(o), n(e));
    }
    o = i.getInstance().stream(`POST`, `/codex/responses`, {
      body: e,
      headers: _(),
      onEvent: (e) => {
        let t = ae(e);
        ...
        (t.delta != null && r.push(t.delta), t.completedText != null && (a = t.completedText));
      },
      onError: (e) => {
        l(Error(e.error));
      },
...
function ae(e) {
  ...
  return n == null
    ? t.response == null
      ...
      : { delta: null, completedText: ce(t.response), error: null }
...
async function le({ transcript: e, surroundingText: t, cleanupEnabled: n }) {
  ...
  try {
    ...
    return (
      (await re({
        instructions: G,
        input: ue({ transcript: r, surroundingText: t ?? null, dictionary: e }),
      })) ?? r
    );
  } catch {
    return r;
...
function ue({ transcript: e, surroundingText: t, dictionary: n }) {
  let r = t?.trim().slice(0, U);
...
async function de() {
  let e = W.safeParse(await h(o.dictationDictionary));
  return e.success
    ? e.data
        .map((e) => e.trim())
        .filter((e) => e.length > 0)
...
async function fe(e, t = {}) {
  let n = t.contentType ?? (e.type && e.type.trim().length > 0 ? e.type : `audio/webm`),
    r = n.split(/[/;]/)[1] ?? `webm`,
    a = me(t.filename ?? `codex.${r}`),
    o = he(),
    s = g(await pe({ blob: e, boundary: o, filename: a, contentType: n, language: t.language })),
    c = { "Content-Type": `multipart/form-data; boundary=${o}`, "X-Codex-Base64": `1`, ..._() };
  return (await i.getInstance().post(`/transcribe`, s, c)).body.text;
}
async function pe({ blob: e, boundary: t, filename: n, contentType: r, language: i }) {
...
function Te(e) {
  ...
  t[4] !== a || t[5] !== o
    ? ((T = (e) => {
        if (e == null) return !1;
        let t = Math.max(1, o === `compact` ? Y : Math.floor(e.clientWidth / 4));
        return (
          (h.current = Array.from({ length: t }, Ee)),
          (C.current = Math.max(1, Math.floor((ee * a) / t))),
          (S.current = new Float32Array()),
          !0
        );
      }),
      (t[4] = a),
  ...
  t[10] !== E || t[11] !== o
    ? ((N = () => {
        ...
        if (r === 0 || n === 0) return;
        let i = o === `compact` ? Y : Math.max(1, Math.floor(r / 4));
        h.current.length !== i && E(e);
        let a = h.current;
  ...
  let P = N,
    F;
  t[13] !== P || t[14] !== E || t[15] !== o
    ? ((F = (e) => {
        if ((A(), M(), E(p.current), P(), typeof AudioContext > `u`)) return;
        ...
        ((d.current = r),
          ...
          (r.onaudioprocess = (e) => {
            ...
            for (let e = 0; e < t.length; e += 1) {
              ...
              ((n += r * r), (t[e] = r < 0.0025 ? x : r));
            }
            h.current.length === 0 && E(p.current);
            let r = h.current.length,
              i = Math.sqrt(n / Math.max(1, t.length));
            if (o === `compact`) {
              let e = Math.max(0, i - J),
                n = Math.min(1, e / (K - J)) ** 0.6 * Q,
                a = _.current,
                o = n > a ? xe : we,
                s = a * (1 - o) + n * o;
              ((_.current = s), (y.current += Ce));
              for (let e = 0; e < r; e += 1) {
                let n = 0.9 + ((Math.sin(y.current - e * 0.8) + 1) / 2) * 0.1,
                  a = De(t, e, r, i),
                  o = Math.min(Q, x + s * n * a),
                  c = h.current[e] ?? 0.0025;
                h.current[e] = c * (1 - X) + o * X;
              }
              P();
            } else if (o === `centered`) {
              let e = Math.max(0, i - J),
              ...
              for (let e = 0; e < r; e += 1) {
                ...
                h.current[e] = Math.min(q, x + s * $(e, r) * n);
              }
              P();
            } else {
              let e = S.current,
  ...
  t[17] === s
    ? (L = t[18])
    : ((L = () => (f.current == null ? s : Math.max(0, performance.now() - f.current))),
      (t[17] = s),
      (t[18] = L));
  let R = L,
    z,
  ...
  let V;
  return (
    t[21] !== R || t[22] !== s || t[23] !== I
      ? ((V = {
          getCurrentRecordingDurationMs: R,
          recordingDurationMs: s,
          waveformCanvasRef: p,
          startWaveformCapture: I,
...
function Oe(e, t, n) {
  let r = Math.min(be, e.length),
    i = ke(t, Math.max(1, e.length - r)),
    a = 0;
...
export { C as a, A as i, fe as n, S as o, le as r, b as s, Te as t };
