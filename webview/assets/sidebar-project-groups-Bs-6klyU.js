...
import { Ks as a, gn as o, io as s, po as c } from "./app-server-manager-signals.js";
...
function M(e, t, n) {
  ...
  return (e?.roots ?? []).map((o) => {
    let s = i(o),
      c = r(o, n),
      l = (e?.labels?.[o] ?? e?.labels?.[s] ?? ``).trim() || (0, E.default)(B(o)) || ``,
      u = a[s] ?? void 0,
...
function I(e, t, n, r, a, o) {
  ...
  return (
    e.forEach((e) => {
      if (e.kind === `local`) W(e, n, l, r, o?.gitOriginsByHostId, a, o?.primaryHostId, o);
...
var W = (e, t, n, i, a, o, s = c, u) => {
  ...
  if (
    _ &&
    ((u?.enabledRemoteHostIds && !u.enabledRemoteHostIds.has(p)) || !v?.some((e) => e.hostId === p))
  )
    return;
  let y = J({ gitOrigins: i, gitOriginsByHostId: a, hostId: p ?? void 0, primaryHostId: s });
  if (r(d, o) || (_ && Y(d, y))) {
  ...
  x &&
    ...
    f !== d && u?.onDiscoverThreadWorkspaceRootHint?.(e.conversationId, x.path));
};
function G(e, t) {
  return e == null
    ? null
...
function K(e, t, n, r) {
  ...
  return Array.from(a.entries())
    .map(([e, t]) => ({ hostId: e, dirs: (0, D.default)(t).sort((e, t) => e.localeCompare(t)) }))
    .filter(({ hostId: e, dirs: n }) => e === t || n.length > 0);
}
function q(e, t) {
  let n = new Map(t.map(({ hostId: e, dirs: t }) => [e, new Set(t.map(i))]));
  return e
    .map(({ hostId: e, dirs: t }) => ({ hostId: e, dirs: t.filter((t) => !n.get(e)?.has(i(t))) }))
    .filter(({ dirs: e }) => e.length > 0);
}
function J({ gitOrigins: e, gitOriginsByHostId: t, hostId: n, primaryHostId: r }) {
  return n && t?.[n] ? t[n] : n && t && n !== r ? [] : (e ?? []);
}
function Y(e, t) {
  let n = R(e, t ?? []);
  return n?.commonDir
    ? i(n.commonDir).replace(/\/+$/, ``) !== `${i(n.root).replace(/\/+$/, ``)}/.git`
    : !1;
}
  ...
  let s = t[i(o)] ?? null,
    c =
      a.hostId === `local`
        ? H(r, o)
        : (n.find(
            (e) => e.projectKind === `remote` && e.hostId === a.hostId && i(e.path) === i(o),
          ) ?? null);
  c &&
    ((s && c.repositoryData?.ownerRepo != null && !L(c.repositoryData.ownerRepo, s)) ||
...
//# sourceMappingURL=sidebar-project-groups-Bs-6klyU.js.map
