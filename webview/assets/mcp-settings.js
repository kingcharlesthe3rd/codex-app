...
function He() {
  let { selectedHostId: e } = E(),
    ...
    ue = le ? (b[e] ?? []) : [],
    F = (0, qe.default)(
      (t == null ? [] : (n?.data ?? [])).filter((e) => !M.has(e.name) && !ue.includes(e.name)),
      (e) => e.name,
    ),
    I = A.length > 0,
    L = F.length > 0,
  ...
  let z = () => {
    ...
    me = async (t) => {
      ...
      try {
        (await i.mutateAsync({ filePath: V(), key: n, value: null }),
          x((r) => ({ ...r, [e]: (0, Je.default)([...(r[e] ?? []), n, t, O[n]?.name ?? n]) })),
          z(),
          B());
      } catch (e) {
        throw (
    ...
    he = async (t) => {
      ...
      let n = p[t];
      if (n) {
        pe({ href: n, initiator: `open_in_browser_bridge`, openTarget: `external-browser` });
        return;
      }
      g((e) => ({ ...e, [t]: null }));
      try {
        let { authorizationUrl: n } = await te(`login-mcp-server`, { hostId: e, name: t });
        n &&
          (g((e) => ({ ...e, [t]: n })),
          pe({ href: n, initiator: `open_in_browser_bridge`, openTarget: `external-browser` }));
      } catch (e) {
