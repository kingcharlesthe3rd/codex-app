import { s as e } from "./chunk.js";
import { n as t, t as n } from "./jsx-runtime.js";
import { Z as r, s as i } from "./app-scope-CWE-zIhQ.js";
import "./vscode-api.js";
import "./isEqual.js";
import "./src-3.js";
import { t as a } from "./react-dom.js";
import { l as o, s } from "./lib-1.js";
import { t as c } from "./clsx.js";
import { t as l } from "./proxy.js";
import "./setting-storage.js";
import "./reduced-motion-preference.js";
import { t as ee } from "./use-reduced-motion-1.js";
import "./window-zoom-context.js";
import { t as te } from "./use-stable-callback.js";
import { t as ne } from "./tooltip-B.js";
import { n as u } from "./tooltip-dismiss.js";
import { t as re } from "./clock-Cid-cc8u.js";
import { n as d, t as f } from "./markdown-to-search-text.js";
import "./marked.esm.js";
import { r as ie, zn as ae } from "./product-logger.js";
import { _ as p, a as oe, g as se, n as m, u as h } from "./parse-directives.js";
import "./mime-types.js";
import { A as ce, O as g, g as le } from "./mention-item.js";
import { t as _ } from "./external-link-source.js";
import { t as ue } from "./rich-link.js";
import { t as v } from "./apps.js";
import { t as y } from "./known-app-icon.js";
import "./branch.js";
import "./github-mark.js";
import { t as b } from "./google-drive.js";
import "./file.js";
import "./terminal.js";
import { n as x } from "./get-file-icon.js";
import "./code.js";
import "./folder.js";
import "./json-2.js";
import "./notebook.js";
import "./settings.cog.js";
import "./skills.js";
import { n as S } from "./mention-icons.js";
import { t as C } from "./inline-mention-content-CTO-ayBF.js";
import { t as de } from "./app-shell-ref.js";
import { t as w } from "./globe.js";
import { t as T } from "./external-link-favicon.js";
import { t as E } from "./commit.js";
import { t as D } from "./pull-request.js";
import { t as fe } from "./images.js";
import { n as pe } from "./thread-scroll-controller-context-value.js";
var me = e(d(), 1),
  he = e(a(), 1),
  O = r(),
  k = e(t()),
  A = n(),
  j = (e) =>
    (0, A.jsxs)(`svg`, {
      width: 16,
      height: 16,
      viewBox: `0 0 16 16`,
      fill: `none`,
      xmlns: `http://www.w3.org/2000/svg`,
      ...e,
      children: [
        (0, A.jsx)(`path`, {
          d: `M9.12725 9.17656C9.39819 9.06319 9.7106 9.19089 9.82412 9.46172C9.93751 9.73275 9.80921 10.0444 9.53818 10.1578C8.54869 10.5718 7.63742 10.5982 6.65771 10.2359L6.46084 10.1578L6.36631 10.1062C6.16212 9.96683 6.07651 9.69872 6.17568 9.46172C6.27497 9.22478 6.52602 9.09763 6.76865 9.14531L6.87178 9.17656L7.01865 9.23438C7.74842 9.50658 8.38411 9.4874 9.12725 9.17656Z`,
          fill: `currentColor`,
        }),
        (0, A.jsx)(`path`, {
          d: `M5.66631 5.66719C6.21851 5.66719 6.66618 6.11502 6.66631 6.66719C6.66631 7.21947 6.21859 7.66719 5.66631 7.66719C5.11416 7.66703 4.66631 7.21937 4.66631 6.66719C4.66644 6.11511 5.11424 5.66735 5.66631 5.66719Z`,
          fill: `currentColor`,
        }),
        (0, A.jsx)(`path`, {
          d: `M10.3335 5.66719C10.8856 5.66735 11.3334 6.11511 11.3335 6.66719C11.3335 7.21937 10.8856 7.66703 10.3335 7.66719C9.78121 7.66719 9.3335 7.21947 9.3335 6.66719C9.33363 6.11502 9.78129 5.66719 10.3335 5.66719Z`,
          fill: `currentColor`,
        }),
        (0, A.jsx)(`path`, {
          fillRule: `evenodd`,
          clipRule: `evenodd`,
          d: `M7.9999 1.46875C8.29372 1.46875 8.53193 1.70697 8.53193 2.00078V2.46875H10.4671C11.0181 2.46875 11.464 2.46824 11.8241 2.49766C12.1902 2.52761 12.5151 2.59109 12.8163 2.74453L12.9905 2.84219C13.3872 3.08547 13.7101 3.43468 13.9226 3.85156L13.9765 3.96563C14.0918 4.23459 14.1432 4.52333 14.1694 4.84375C14.1912 5.11055 14.1952 5.42422 14.1968 5.79297C14.5926 5.98867 14.8654 6.3958 14.8655 6.86719V7.86719C14.8655 8.3407 14.5899 8.74887 14.1913 8.94375C14.1852 9.3005 14.1746 9.61715 14.1515 9.9C14.1094 10.4152 14.0283 10.8547 13.8569 11.2539L13.7772 11.4227C13.4299 12.1041 12.8885 12.6662 12.2233 13.0391L12.0882 13.1117C11.6443 13.3378 11.1548 13.4378 10.5663 13.4859C9.98384 13.5335 9.258 13.5328 8.3335 13.5328H7.66709C6.74267 13.5328 6.01674 13.5335 5.43428 13.4859C4.91911 13.4438 4.47957 13.362 4.08037 13.1906L3.91162 13.1117C3.18452 12.7412 2.59312 12.1497 2.22256 11.4227C1.99632 10.9786 1.89644 10.4887 1.84834 9.9C1.82523 9.61715 1.81366 9.30051 1.80771 8.94375C1.40945 8.74874 1.13428 8.34045 1.13428 7.86719V6.86719C1.13438 6.3958 1.40723 5.98867 1.80303 5.79297C1.80434 5.42421 1.80857 5.11056 1.83037 4.84375C1.8603 4.47753 1.9238 4.15285 2.07725 3.85156C2.32 3.37519 2.70789 2.98726 3.18428 2.74453C3.4855 2.59117 3.81033 2.52758 4.17646 2.49766C4.53661 2.46825 4.98237 2.46875 5.5335 2.46875H7.46787V2.00078C7.46787 1.70697 7.70609 1.46875 7.9999 1.46875ZM5.5335 3.53281C4.96484 3.53281 4.56996 3.53355 4.26318 3.55859C3.9628 3.58314 3.79347 3.62862 3.66709 3.69297C3.39093 3.83369 3.16642 4.05823 3.02568 4.33438C2.96128 4.46078 2.91587 4.62992 2.89131 4.93047C2.86624 5.23728 2.86553 5.63195 2.86553 6.20078V7.66719C2.86553 8.60929 2.86606 9.28415 2.90928 9.81328C2.952 10.3362 3.03394 10.6701 3.171 10.9391C3.43957 11.4661 3.86815 11.8947 4.39521 12.1633L4.49912 12.2125C4.75035 12.3205 5.06357 12.3876 5.521 12.425C6.05008 12.4682 6.72516 12.4688 7.66709 12.4688H8.3335C9.27554 12.4688 9.95049 12.4682 10.4796 12.425C11.0023 12.3823 11.3364 12.3003 11.6054 12.1633L11.7983 12.0555C12.237 11.7863 12.5946 11.4002 12.8296 10.9391L12.878 10.8352C12.9861 10.5839 13.0539 10.2709 13.0913 9.81328C13.1345 9.28416 13.1351 8.60927 13.1351 7.66719V6.20078C13.1351 5.63197 13.1343 5.23728 13.1093 4.93047C13.0908 4.70493 13.0606 4.55329 13.0194 4.43828L12.9749 4.33438C12.8518 4.09269 12.6642 3.89027 12.4343 3.74922L12.3335 3.69297C12.2072 3.6286 12.0376 3.58317 11.7374 3.55859C11.4307 3.53353 11.0356 3.53281 10.4671 3.53281H5.5335Z`,
          fill: `currentColor`,
        }),
      ],
    }),
  M = 2;
function ge(e) {
  let t = (0, O.c)(12),
    { item: n } = e,
    r;
  t[0] === n ? (r = t[1]) : ((r = n.getLabel()), (t[0] = n), (t[1] = r));
  let i = r,
    a;
  if (t[2] !== n || t[3] !== i) {
    let e = n.getPreview(),
      r = f(m(e.response)),
      o;
    t[5] === n.isHeartbeat
      ? (o = t[6])
      : ((o = n.isHeartbeat
          ? (0, A.jsx)(re, { className: `icon-xs shrink-0 text-token-description-foreground` })
          : null),
        (t[5] = n.isHeartbeat),
        (t[6] = o));
    let c;
    t[7] === i
      ? (c = t[8])
      : ((c = (0, A.jsx)(`span`, {
          className: `min-w-0 truncate`,
          children:
            i.length === 0
              ? (0, A.jsx)(s, {
                  id: `thread.userMessageNavigation.noContent`,
                  defaultMessage: `(No content)`,
                  description: `Fallback label for an empty user message in the floating thread navigation tooltip`,
                })
              : (0, A.jsx)(L, { label: i }),
        })),
        (t[7] = i),
        (t[8] = c));
    let l;
    (t[9] !== o || t[10] !== c
      ? ((l = (0, A.jsxs)(`div`, {
          className: `flex min-w-0 items-center gap-1.5 font-medium`,
          children: [o, c],
        })),
        (t[9] = o),
        (t[10] = c),
        (t[11] = l))
      : (l = t[11]),
      (a = (0, A.jsxs)(`div`, {
        "data-thread-user-message-navigation-tooltip-preview": !0,
        className: `w-80 max-w-[calc(100vw-1rem)] overflow-hidden rounded-xl bg-token-dropdown-background/95 p-2 text-sm leading-5 text-token-foreground shadow-xl-spread ring-[0.5px] ring-token-border backdrop-blur-sm`,
        children: [
          l,
          r.length > 0
            ? (0, A.jsx)(`div`, {
                className: `mt-1 line-clamp-3 text-token-description-foreground`,
                children: r,
              })
            : null,
          e.outputs.length > 0
            ? (0, A.jsxs)(`div`, {
                className: `mt-2 flex min-w-0 items-center gap-3 text-token-description-foreground`,
                children: [
                  e.outputs.slice(0, M).map(N),
                  e.outputs.length > M
                    ? (0, A.jsx)(`span`, {
                        className: `shrink-0 tabular-nums`,
                        children: (0, A.jsx)(s, {
                          id: `thread.userMessageNavigation.moreOutputs`,
                          defaultMessage: `+{count}`,
                          description: `Count of additional turn outputs hidden from the prompt rail tooltip preview`,
                          values: { count: e.outputs.length - M },
                        }),
                      })
                    : null,
                ],
              })
            : null,
        ],
      })),
      (t[2] = n),
      (t[3] = i),
      (t[4] = a));
  } else a = t[4];
  return a;
}
function N(e, t) {
  return (0, A.jsx)(P, { output: e }, `${e.type}:${e.label ?? t}`);
}
function P(e) {
  let t = (0, O.c)(9),
    { output: n } = e,
    r;
  t[0] === n ? (r = t[1]) : ((r = F(n)), (t[0] = n), (t[1] = r));
  let i;
  t[2] === n ? (i = t[3]) : ((i = I(n)), (t[2] = n), (t[3] = i));
  let a;
  t[4] === i
    ? (a = t[5])
    : ((a = (0, A.jsx)(`span`, { className: `max-w-36 truncate`, children: i })),
      (t[4] = i),
      (t[5] = a));
  let o;
  return (
    t[6] !== r || t[7] !== a
      ? ((o = (0, A.jsxs)(`span`, {
          className: `flex min-w-0 items-center gap-1.5`,
          children: [r, a],
        })),
        (t[6] = r),
        (t[7] = a),
        (t[8] = o))
      : (o = t[8]),
    o
  );
}
function F(e) {
  switch (e.type) {
    case `app`:
      return (0, A.jsx)(v, { className: `icon-sm shrink-0 text-token-charts-blue` });
    case `commit`:
      return (0, A.jsx)(E, { className: `icon-sm shrink-0` });
    case `file`:
      return (0, k.createElement)(x(e.label ?? void 0), { className: `icon-sm shrink-0` });
    case `google-drive`:
      return (0, A.jsx)(b, { className: `icon-sm shrink-0` });
    case `image`:
      return (0, A.jsx)(fe, { className: `icon-sm shrink-0` });
    case `pull-request`:
      return (0, A.jsx)(D, { className: `icon-sm shrink-0` });
    case `review`:
      return (0, A.jsx)(j, { className: `icon-sm shrink-0` });
    case `website`:
      return (0, A.jsx)(w, { className: `icon-sm shrink-0` });
  }
}
function I(e) {
  if (e.label != null) return e.label;
  switch (e.type) {
    case `app`:
      return (0, A.jsx)(s, {
        id: `thread.userMessageNavigation.appOutput`,
        defaultMessage: `App preview`,
        description: `Fallback label for an app output in the prompt rail tooltip`,
      });
    case `commit`:
      return (0, A.jsx)(s, {
        id: `thread.userMessageNavigation.commitOutput`,
        defaultMessage: `Commit`,
        description: `Label for a commit output in the prompt rail tooltip`,
      });
    case `file`:
      return (0, A.jsx)(s, {
        id: `thread.userMessageNavigation.fileOutput`,
        defaultMessage: `File`,
        description: `Fallback label for a file output in the prompt rail tooltip`,
      });
    case `google-drive`:
      return (0, A.jsx)(s, {
        id: `thread.userMessageNavigation.googleDriveOutput`,
        defaultMessage: `Google Drive`,
        description: `Fallback label for a Google Drive output in the prompt rail tooltip`,
      });
    case `image`:
      return (0, A.jsx)(s, {
        id: `thread.userMessageNavigation.imageOutput`,
        defaultMessage: `Image`,
        description: `Label for a generated image output in the prompt rail tooltip`,
      });
    case `pull-request`:
      return (0, A.jsx)(s, {
        id: `thread.userMessageNavigation.pullRequestOutput`,
        defaultMessage: `Pull request`,
        description: `Fallback label for a pull request output in the prompt rail tooltip`,
      });
    case `review`:
      return (0, A.jsx)(s, {
        id: `thread.userMessageNavigation.reviewOutput`,
        defaultMessage: `Review`,
        description: `Fallback label for a review output in the prompt rail tooltip`,
      });
    case `website`:
      return (0, A.jsx)(s, {
        id: `thread.userMessageNavigation.websiteOutput`,
        defaultMessage: `Web preview`,
        description: `Label for a website output in the prompt rail tooltip`,
      });
  }
}
function L(e) {
  let t = (0, O.c)(2),
    { label: n } = e,
    r;
  if (t[0] !== n) {
    let e = [],
      i = 0,
      a = 0;
    for (; i < n.length; ) {
      let t = h(n, i);
      if (t == null) break;
      i = t.end;
      let r = se(t.label).trim(),
        o = p(t.path),
        s,
        c = r,
        l;
      if (g(o) || ce(o)) ((s = y(r) ?? v), (c = le(r)));
      else if (r.startsWith(`$`))
        ((s = S()), (c = oe(r.startsWith(`$[`) && r.endsWith(`]`) ? r.slice(2, -1) : r.slice(1))));
      else {
        let e = ue(o);
        if (e == null) continue;
        let t = _(e);
        ((s = (t == null ? null : y(t.appId)) ?? (0, A.jsx)(T, { href: e })), (l = e));
      }
      (e.push(n.slice(a, t.start)),
        e.push((0, A.jsx)(C, { className: `first:pl-0`, icon: s, text: c, title: l }, t.start)),
        (a = t.end));
    }
    (e.push(n.slice(a)), (r = (0, A.jsx)(A.Fragment, { children: e })), (t[0] = n), (t[1] = r));
  } else r = t[1];
  return r;
}
var R = {
    marker: `_marker_1okf4_1`,
    row: `_row_1okf4_6`,
    railList: `_railList_1okf4_26`,
    outputDot: `_outputDot_1okf4_32`,
  },
  _e = 12,
  z = 4,
  ve = 36,
  ye = 16,
  be = `[data-turn-key], [data-content-search-turn-key]`,
  B = `color-mix(in srgb, var(--color-token-foreground) 14%, transparent)`,
  V = `color-mix(in srgb, var(--color-token-foreground) 5%, transparent)`;
function H(e) {
  let t = (0, O.c)(3),
    { items: n, onRevealItem: r } = e;
  if (n.length < z) return null;
  let i;
  return (
    t[0] !== n || t[1] !== r
      ? ((i = (0, A.jsx)(U, { items: n, onRevealItem: r })), (t[0] = n), (t[1] = r), (t[2] = i))
      : (i = t[2]),
    i
  );
}
function U(e) {
  let t = (0, O.c)(71),
    { items: n, onRevealItem: r } = e,
    { getScrollElement: a } = pe(),
    s = (0, k.useContext)(de),
    re = i(ie),
    d = n.at(-1)?.id ?? null,
    f;
  t[0] === d ? (f = t[1]) : ((f = () => new Set(d == null ? [] : [d])), (t[0] = d), (t[1] = f));
  let [p, oe] = (0, k.useState)(f),
    [se, m] = (0, k.useState)(!1),
    [h, ce] = (0, k.useState)(null),
    [g, le] = (0, k.useState)(!1),
    [_, ue] = (0, k.useState)(null),
    v = (0, k.useRef)(null),
    y = (0, k.useRef)(null),
    b = (0, k.useRef)(!1),
    x = o(),
    S = ee(),
    C;
  t[2] === n ? (C = t[3]) : ((C = n.map(xe).join(`\0`)), (t[2] = n), (t[3] = C));
  let w = C,
    T;
  t[4] !== p || t[5] !== n || t[6] !== d
    ? ((T = n.find((e) => p.has(e.id))?.id ?? d), (t[4] = p), (t[5] = n), (t[6] = d), (t[7] = T))
    : (T = t[7]);
  let E = T,
    D;
  t[8] === E
    ? (D = t[9])
    : ((D = () => {
        y.current ?? Ce(v.current, E);
      }),
      (t[8] = E),
      (t[9] = D));
  let fe = (0, k.useEffectEvent)(D),
    j;
  t[10] !== a || t[11] !== r || t[12] !== S
    ? ((j = async (e, t) => {
        let n = a();
        if (n == null) return;
        let i = Se(n, e.id);
        if (i == null && r != null) {
          (await r(e), (i = Se(n, e.id)), Te(i, S));
          return;
        }
        (i?.scrollIntoView({ behavior: t, block: `start` }), Te(i, S));
      }),
      (t[10] = a),
      (t[11] = r),
      (t[12] = S),
      (t[13] = j))
    : (j = t[13]);
  let M = te(j),
    N;
  t[14] !== M || t[15] !== re
    ? ((N = (e) => (re.logProductEvent(ae, {}), M(e, `smooth`))),
      (t[14] = M),
      (t[15] = re),
      (t[16] = N))
    : (N = t[16]);
  let P = N,
    F;
  t[17] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((F = (e) => {
        let t = y.current;
        t?.pointerId === e.pointerId &&
          ((y.current = null),
          ce(null),
          t.pointerCaptureTarget.hasPointerCapture?.(e.pointerId) &&
            t.pointerCaptureTarget.releasePointerCapture?.(e.pointerId),
          e.currentTarget.matches(`:hover`) || m(!1),
          window.setTimeout(() => {
            b.current = !1;
          }, 0));
      }),
      (t[17] = F))
    : (F = t[17]);
  let I = te(F),
    L;
  t[18] !== a || t[19] !== fe
    ? ((L = () => {
        let e = a(),
          t = e?.querySelector(`[data-mcp-app-portal-target="true"]`) ?? null;
        if (e == null || t == null) return;
        let n = null,
          r = () => {
            n ??= window.requestAnimationFrame(() => {
              ((n = null), ue(e.parentElement));
              let r = e.getBoundingClientRect(),
                i = t.getBoundingClientRect(),
                a = e.offsetWidth > 0 ? r.width / e.offsetWidth : 1;
              (le((i.left - r.left) / (a > 0 ? a : 1) >= _e + ve), fe());
            });
          },
          i = new ResizeObserver(r);
        (i.observe(e), i.observe(t));
        let o = new MutationObserver(r);
        return (
          o.observe(e.firstElementChild ?? e, { attributes: !0, attributeFilter: [`style`] }),
          e.addEventListener(`scroll`, u, { passive: !0 }),
          window.addEventListener(`resize`, r),
          r(),
          () => {
            (n != null && window.cancelAnimationFrame(n),
              i.disconnect(),
              o.disconnect(),
              e.removeEventListener(`scroll`, u),
              window.removeEventListener(`resize`, r));
          }
        );
      }),
      (t[18] = a),
      (t[19] = fe),
      (t[20] = L))
    : (L = t[20]);
  let z;
  (t[21] === a ? (z = t[22]) : ((z = [a]), (t[21] = a), (t[22] = z)), (0, k.useEffect)(L, z));
  let B, V;
  (t[23] !== a || t[24] !== w
    ? ((B = () => {
        let e = a();
        if (e == null || typeof IntersectionObserver > `u`) return;
        let t = new Set(),
          n = new Map(),
          r = new Set(),
          i = w.length === 0 ? [] : w.split(`\0`),
          o = new Set(i),
          s = () => {
            let e = i.findIndex((e) => t.has(e));
            if (e === -1) return;
            let n = (0, me.default)(i, (e) => t.has(e)),
              r = new Set(i.slice(e, n + 1));
            oe((e) => (e.size === r.size && [...e].every((e) => r.has(e)) ? e : r));
          },
          c = new IntersectionObserver(
            (e) => {
              for (let r of e) {
                if (!(r.target instanceof HTMLElement)) continue;
                let e = n.get(r.target);
                e != null && (r.isIntersecting ? t.add(e) : t.delete(e));
              }
              s();
            },
            { root: e, rootMargin: `-${ye}px 0px 0px 0px` },
          ),
          l = () => {
            let i = new Set();
            for (let t of e.querySelectorAll(`[data-content-search-unit-key]`)) {
              let e = t.dataset.contentSearchUnitKey;
              if (e == null || !o.has(e)) continue;
              let a = t.closest(be),
                s = a == null || i.has(a) ? t : a;
              (i.add(s), n.set(s, e), r.has(s) || (c.observe(s), r.add(s)));
            }
            for (let e of r) {
              if (i.has(e)) continue;
              let a = n.get(e);
              (a != null && t.delete(a), n.delete(e), c.unobserve(e), r.delete(e));
            }
            s();
          },
          ee = new MutationObserver((e) => {
            Ee(e) && l();
          });
        return (
          ee.observe(e, { childList: !0, subtree: !0 }),
          l(),
          () => {
            (ee.disconnect(), c.disconnect());
          }
        );
      }),
      (V = [a, w]),
      (t[23] = a),
      (t[24] = w),
      (t[25] = B),
      (t[26] = V))
    : ((B = t[25]), (V = t[26])),
    (0, k.useEffect)(B, V));
  let H;
  t[27] !== E || t[28] !== h
    ? ((H = () => {
        h ?? Ce(v.current, E);
      }),
      (t[27] = E),
      (t[28] = h),
      (t[29] = H))
    : (H = t[29]);
  let U;
  if (
    (t[30] !== E || t[31] !== g || t[32] !== h
      ? ((U = [E, g, h]), (t[30] = E), (t[31] = g), (t[32] = h), (t[33] = U))
      : (U = t[33]),
    (0, k.useLayoutEffect)(H, U),
    !g || _ == null || typeof document > `u`)
  )
    return null;
  let W;
  t[34] === x
    ? (W = t[35])
    : ((W = x.formatMessage({
        id: `thread.userMessageNavigation.ariaLabel`,
        defaultMessage: `User messages`,
        description: `Aria label for the floating thread navigation rail that jumps between user messages`,
      })),
      (t[34] = x),
      (t[35] = W));
  let G;
  t[36] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((G = { opacity: 1 }), (t[36] = G))
    : (G = t[36]);
  let De;
  t[37] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((De = { opacity: 0 }), (t[37] = De))
    : (De = t[37]);
  let K;
  t[38] === S
    ? (K = t[39])
    : ((K = S ? { duration: 0 } : { duration: 0.15, ease: [0.23, 1, 0.32, 1] }),
      (t[38] = S),
      (t[39] = K));
  let Oe = h == null ? void 0 : !0,
    ke;
  t[40] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((ke = c(
        R.railList,
        `vertical-scroll-fade-mask hide-scrollbar flex max-h-[min(70vh,40rem)] flex-col overflow-y-auto overscroll-contain [--edge-fade-distance:2.5rem]`,
      )),
      (t[40] = ke))
    : (ke = t[40]);
  let q;
  t[41] === n
    ? (q = t[42])
    : ((q = (e) => {
        if (e.button !== 0) return;
        let t = we(n, e.currentTarget, e.target instanceof Element ? e.target : null);
        t != null &&
          ((b.current = !1),
          (y.current = {
            itemId: t.item.id,
            pointerCaptureTarget: t.button,
            pointerId: e.pointerId,
          }),
          ce(t.item.id),
          m(!0),
          u(),
          t.button.setPointerCapture?.(e.pointerId));
      }),
      (t[41] = n),
      (t[42] = q));
  let Ae;
  t[43] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((Ae = () => {
        y.current ?? m(!1);
      }),
      (t[43] = Ae))
    : (Ae = t[43]);
  let J;
  t[44] !== I || t[45] !== M || t[46] !== n
    ? ((J = (e) => {
        let t = y.current;
        if (t?.pointerId !== e.pointerId) return;
        if (e.buttons % 2 == 0) {
          I(e);
          return;
        }
        let r = e.currentTarget.getBoundingClientRect(),
          i = we(
            n,
            e.currentTarget,
            document.elementFromPoint(
              r.left + r.width / 2,
              Math.max(r.top, Math.min(e.clientY, r.bottom - 1)),
            ),
          );
        i == null ||
          i.item.id === t.itemId ||
          ((y.current = { ...t, itemId: i.item.id }),
          ce(i.item.id),
          (b.current = !0),
          M(i.item, `instant`));
      }),
      (t[44] = I),
      (t[45] = M),
      (t[46] = n),
      (t[47] = J))
    : (J = t[47]);
  let Y;
  t[48] !== p ||
  t[49] !== P ||
  t[50] !== x ||
  t[51] !== se ||
  t[52] !== n ||
  t[53] !== s ||
  t[54] !== h
    ? ((Y = n.map((e, t) =>
        (0, A.jsx)(
          ne,
          {
            align: `center`,
            disableHoverOpen: se,
            portalContainer: s,
            side: `right`,
            sideOffset: 0,
            skipDelayKey: `thread-user-message-navigation`,
            tooltipClassName: `!m-0 !z-20`,
            tooltipMaxWidth: `min(20rem, calc(100vw - 16px))`,
            variant: `unstyled`,
            tooltipContent: (0, A.jsx)(ge, { item: e }),
            children: (0, A.jsx)(`button`, {
              "data-thread-user-message-navigation-item-id": e.id,
              "data-scrub-target": h === e.id ? !0 : void 0,
              "aria-current": p.has(e.id) ? `true` : void 0,
              "aria-label": x.formatMessage(
                {
                  id: `thread.userMessageNavigation.jumpAriaLabel`,
                  defaultMessage: `Jump to user message {position}`,
                  description: `Aria label for a row in the floating thread user-message navigation rail`,
                },
                { position: t + 1 },
              ),
              className: c(
                R.row,
                `group/navigation-row flex h-2.5 w-9 shrink-0 cursor-interaction items-center outline-none`,
              ),
              type: `button`,
              onClick: () => {
                if (b.current) {
                  b.current = !1;
                  return;
                }
                (m(!0), u(), P(e));
              },
              children: (0, A.jsxs)(`span`, {
                className: `flex items-center gap-0.5`,
                children: [
                  (0, A.jsx)(`span`, {
                    className: c(
                      R.marker,
                      `h-0.5 bg-token-description-foreground opacity-40 group-focus-visible/navigation-row:bg-token-foreground group-focus-visible/navigation-row:opacity-100`,
                      h == null &&
                        `group-hover/navigation-row:bg-token-foreground group-hover/navigation-row:opacity-100`,
                      p.has(e.id) && h !== e.id && `bg-token-foreground opacity-60`,
                      h === e.id && `bg-token-foreground opacity-100`,
                    ),
                  }),
                  e.hasOutput
                    ? (0, A.jsx)(`span`, {
                        className: c(
                          R.outputDot,
                          `size-0.5 shrink-0 rounded-full bg-token-description-foreground opacity-40 group-focus-visible/navigation-row:bg-token-foreground group-focus-visible/navigation-row:opacity-100`,
                          h == null &&
                            `group-hover/navigation-row:bg-token-foreground group-hover/navigation-row:opacity-100`,
                          p.has(e.id) && h !== e.id && `bg-token-foreground opacity-60`,
                          h === e.id && `bg-token-foreground opacity-100`,
                        ),
                      })
                    : null,
                ],
              }),
            }),
          },
          e.id,
        ),
      )),
      (t[48] = p),
      (t[49] = P),
      (t[50] = x),
      (t[51] = se),
      (t[52] = n),
      (t[53] = s),
      (t[54] = h),
      (t[55] = Y))
    : (Y = t[55]);
  let X;
  t[56] === Y
    ? (X = t[57])
    : ((X = (0, A.jsx)(`div`, { className: `flex flex-col`, children: Y })),
      (t[56] = Y),
      (t[57] = X));
  let Z;
  t[58] !== I || t[59] !== Oe || t[60] !== q || t[61] !== J || t[62] !== X
    ? ((Z = (0, A.jsx)(`div`, {
        ref: v,
        "data-thread-user-message-navigation-rail-list": !0,
        "data-scrubbing": Oe,
        className: ke,
        onLostPointerCapture: I,
        onPointerCancelCapture: I,
        onPointerDownCapture: q,
        onPointerLeave: Ae,
        onPointerMove: J,
        onPointerUpCapture: I,
        onScroll: u,
        children: X,
      })),
      (t[58] = I),
      (t[59] = Oe),
      (t[60] = q),
      (t[61] = J),
      (t[62] = X),
      (t[63] = Z))
    : (Z = t[63]);
  let Q;
  t[64] !== W || t[65] !== K || t[66] !== Z
    ? ((Q = (0, A.jsx)(l.nav, {
        "aria-label": W,
        animate: G,
        className: `absolute top-1/2 left-3 z-20 -translate-y-1/2`,
        initial: De,
        transition: K,
        children: Z,
      })),
      (t[64] = W),
      (t[65] = K),
      (t[66] = Z),
      (t[67] = Q))
    : (Q = t[67]);
  let $;
  return (
    t[68] !== _ || t[69] !== Q
      ? (($ = (0, he.createPortal)(Q, _)), (t[68] = _), (t[69] = Q), (t[70] = $))
      : ($ = t[70]),
    $
  );
}
function xe(e) {
  return e.id;
}
function Se(e, t) {
  return e.querySelector(`[data-content-search-unit-key="${G(t)}"]`);
}
function Ce(e, t) {
  if (e == null) return;
  let n = W(e, t);
  n != null &&
    (n.offsetTop < e.scrollTop
      ? (e.scrollTop = n.offsetTop)
      : n.offsetTop + n.offsetHeight > e.scrollTop + e.clientHeight &&
        (e.scrollTop = n.offsetTop + n.offsetHeight - e.clientHeight + 1));
}
function W(e, t) {
  return t == null || e == null
    ? null
    : e.querySelector(`[data-thread-user-message-navigation-item-id="${G(t)}"]`);
}
function we(e, t, n) {
  let r = n?.closest(`[data-thread-user-message-navigation-item-id]`);
  if (r == null || !t.contains(r)) return null;
  let i = e.find((e) => e.id === r.dataset.threadUserMessageNavigationItemId);
  return i == null ? null : { button: r, item: i };
}
function Te(e, t) {
  (
    e?.querySelector(`[data-user-message-bubble]`) ??
    e?.querySelector(`[data-composer-attachment-pill]`)
  )?.animate?.(
    [{ backgroundColor: B }, { backgroundColor: B, offset: 0.35 }, { backgroundColor: V }],
    { duration: t ? 0 : 1400, easing: `cubic-bezier(0.23, 1, 0.32, 1)` },
  );
}
function Ee(e) {
  return e.some((e) =>
    [...e.addedNodes, ...e.removedNodes].some(
      (e) => e instanceof HTMLElement && (e.matches(be) || e.querySelector(be) != null),
    ),
  );
}
function G(e) {
  return typeof CSS < `u` && CSS.escape != null ? CSS.escape(e) : e.replace(/"/g, `\\"`);
}
export { H as ThreadUserMessageNavigationRail };
//# sourceMappingURL=thread-user-message-navigation-rail-BmS-7Jd8.js.map
