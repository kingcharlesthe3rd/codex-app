...
var i = [`thread`, `navigation`, `panels`, `workspace`, `skills`, `configure`, `app`],
  ...
  o = new Map();
for (let e of n) {
  if (o.has(e.id)) throw Error(`Duplicate Codex command id: ${e.id}`);
  o.set(e.id, e);
}
var s = n.filter((e) => e.kind === `webview` && /^thread[1-9]$/.test(e.id)).map((e) => e.id),
  c = n
...
function v(e, t) {
  return e === t ? !0 : a.some((n) => n.includes(e) && n.includes(t));
}
function y({ commandId: e, isMacOS: t }) {
  return b({ commandId: e, isMacOS: t })[0] ?? null;
}
