import { s as e } from "./chunk.js";
import { n as t, t as n } from "./jsx-runtime.js";
import { o as r, t as i, z as a } from "./app-scope.js";
import {
  Ka as o,
  Ts as s,
  lr as c,
  ot as l,
  qa as u,
} from "./app-server-manager-signals.js";
import { _ as d, h as f, n as p } from "./vscode-api.js";
import "./isEqual.js";
import { T as m } from "./src-2.js";
import "./client.js";
import "./react-dom.js";
import { l as h, s as g } from "./lib.js";
import "./persisted-signal.js";
import "./proxy.js";
import { G as _ } from "./app-shell-state.js";
import "./app-shell.js";
import "./button.js";
import { a as v } from "./setting-storage.js";
import "./reduced-motion-preference.js";
import "./use-reduced-motion-2.js";
import { t as y } from "./spinner.js";
import "./Combination.js";
import "./window-zoom-context.js";
import "./check-md.js";
import "./chevron-right.js";
import "./use-stable-callback.js";
import "./tooltip.js";
import "./context-menu.js";
import { r as b } from "./toast-signal.js";
import "./arrow-up.js";
import "./use-dictation.js";
import "./transcribe-audio.js";
import "./regenerate.js";
import "./x.js";
import "./mime-types.js";
import { hr as x, t as S } from "./product-logger.js";
import "./format-skill-title.js";
import "./rpc-2.js";
import { u as ee } from "./thread-side-panel-tabs-2.js";
import "./statsig.js";
import "./request.js";
import "./platform.js";
import "./marked.esm.js";
import "./invalidate-queries-and-broadcast.js";
import "./thread-context-inputs.js";
import "./config-queries.js";
import "./appshot-availability.js";
import "./dropdown.js";
import "./search.js";
import "./appshot-window.js";
import "./app-intl-signal.js";
import "./appshot-icon-cache.js";
import "./modal-controller-state.js";
import "./dialog-layout-B.js";
import "./with-window.js";
import "./badge-C.js";
import "./file.js";
import "./folder.js";
import "./undo.js";
import "./use-debounced-value-Dw-7BPJJ.js";
import "./toggle.js";
import "./experimental-features-queries.js";
import "./use-in-app-browser-use-availability.js";
import "./image-square.js";
import "./plan.js";
import "./plus.js";
import "./target-Tv-aqp7h.js";
import "./check-plugin-availability.js";
import { i as C } from "./app-server-manager-hooks.js";
import "./selectable-remote-connections-signal.js";
import "./remote-connection-visibility.js";
import "./sidebar-signals.js";
import "./use-is-copilot-api-available.js";
import "./use-global-state.js";
import "./use-auth.js";
import "./use-codex-home.js";
import "./use-os-info.js";
import "./use-platform.js";
import "./browser-sidebar-availability.js";
import "./use-plugins.js";
import "./apps-queries.js";
import "./startCase.js";
import "./plugin-detail-queries.js";
import "./plugins-page-selectors.js";
import "./mention-item.js";
import "./mcp.js";
import "./get-skill-icon.js";
import "./bug.js";
import "./chrome.js";
import "./codex-Dw-qSlib.js";
import "./notion.js";
import "./openai-blossom.js";
import "./pencil.js";
import "./filesystem-media-src.js";
import "./plugin-mention-utils.js";
import "./use-composer-controller.js";
import "./apps-2.js";
import "./use-collaboration-mode.js";
import "./use-model-settings.js";
import "./link-external.js";
import "./open-config-toml-button.js";
import "./open-workspace-file.js";
import "./mcp-capability-signals.js";
import "./send-open-file-request.js";
import "./model-queries.js";
import "./use-webview-execution-target.js";
import "./remote-projects.js";
import "./collaboration-mode-queries.js";
import "./composer-footer.js";
import "./use-recording-waveform.js";
import "./codex-api-error.js";
import "./chevron.js";
import "./clock.js";
import "./warning.js";
import "./shiki-highlight-provider-gate.js";
import "./core.esm.js";
import "./katex-3.js";
import "./pdf.js";
import "./known-app-icon.js";
import "./branch-D.js";
import "./github-mark.js";
import "./google-drive.js";
import "./_baseEach.js";
import "./_baseOrderBy.js";
import "./codex-api.js";
import "./use-is-dark.js";
import "./connector-logo.js";
import { t as te } from "./route-scope.js";
import "./prompt-text.js";
import "./use-environment.js";
import "./composer-view-state.js";
import "./permissions-mode-defaults.js";
import "./chats.js";
import "./info-1.js";
import "./projectless-thread.js";
import "./plugin-install-store.js";
import "./check-circle-filled.js";
import "./diff-view-mode.js";
import { n as ne } from "./app-shell-tab-controller.js";
import "./browser-sidebar-manager.js";
import { Tt as re, Wt as ie, Xt as ae } from "./review-navigation-model.js";
import "./file-tree-search-input.js";
import "./parse-diff-BMY7-MEq.js";
import "./review-file-source-tab.js";
import "./file-diff.js";
import "./sumBy-DDE-KTtd.js";
import "./thread-context-BQm-6r7t.js";
import "./git-current-branch-query.js";
import "./project-context-signal.js";
import "./focus-composer.js";
import "./thread-panel-state.js";
import "./thread-browser-panel-tabs-CU-ZDBn5.js";
import "./terminal.js";
import "./error-boundary.js";
import "./download.js";
import "./use-resolved-theme-variant.js";
import "./use-window-controls-safe-area.js";
import "./appgen-gating.js";
import "./plugin-detail-route-utils.js";
import "./use-native-apps.electron.js";
import "./get-file-icon.js";
import "./code-CBI-Jt6Z.js";
import "./json.js";
import "./notebook.js";
import "./settings.cog.js";
import "./skills.js";
import "./mention-icons.js";
import "./inline-mentions.js";
import "./workspace-file-context-menu.js";
import "./command-menu-state.js";
import "./open-project-setup-dialog.js";
import "./local-projects.js";
import "./select-project.js";
import "./sidebar-project-group-signals.js";
import "./sidebar-project-groups.js";
import "./_defineProperty.js";
import "./run-command.js";
import "./use-register-command.js";
import "./use-skills.js";
import "./inline-mention-content.js";
import "./external-link.js";
import "./initial-route-atom.js";
import "./sites-color.js";
import "./plugin-config-edits.js";
import "./bundled-plugin-auto-install-preference.js";
import "./build-worktree-label-from-input.js";
import { t as w } from "./read-service-tier-for-request.js";
import "./pending-worktree-store.js";
import "./use-composer-top-menu-max-height.js";
import { t as oe } from "./use-navigate-to-local-conversation.js";
import "./electron-menu-shortcuts.js";
import "./pinned-threads-query.js";
import "./sidebar-thread-row-signals.js";
import "./use-hotkey-Ct4Y-I9A.js";
import "./command-keybindings.js";
import "./homepage-logo.js";
import "./loading-page.js";
import "./sortable.esm.js";
import "./use-resize-observer.js";
import "./expand.js";
import "./keyboard-modifier-state.js";
import "./arrow-left.js";
import "./app-shell-bottom-panel-scroll-sync.js";
import "./app-shell-ref.js";
import "./checkbox.js";
import "./minus.js";
import "./chat.js";
import "./folders.js";
import "./globe.js";
import "./three-dots.js";
import "./trash.js";
import "./format-relative-date-time.js";
import "./cloud.js";
import "./macbook.js";
import "./worktree.js";
import "./brain-DK1_-NNs.js";
import "./pointer-outline.js";
import "./unselected-circle.js";
import "./review-header-toolbar.js";
import "./x-circle-filled-Do-SvWo6.js";
import "./markdown-1.js";
import "./external-link-favicon.js";
import "./external-markdown-link.js";
import "./code-snippet.js";
import "./copy.js";
import "./copy-button.js";
import "./image-preview-dialog.js";
import "./use-diff-annotations.js";
import "./pull-request-open.js";
import "./gh-pull-request-status-query.js";
import "./use-is-background-subagents-enabled.js";
import "./sortBy.js";
import "./use-connected-remote-connections.js";
import "./settings-host-context.js";
import { t as se } from "./use-git-current-branch.js";
import "./arrow-up-right-lg-Cev-8THl.js";
import "./composer-controller.js";
/* empty css                    */ import "./thread-side-panel-active-signals.js";
import "./composer-top-menu-chrome.js";
import "./browser-sidebar-state.js";
import "./use-git-default-branch.js";
import "./arrow-top-right.js";
import "./connected-apps.js";
import "./app-connect-oauth.js";
import "./hooks-settings-queries.js";
import { a as ce, t as T } from "./slash-command-item.js";
import "./attachment-remove-button.js";
import "./comment-B.js";
import "./use-dictation-hotkey.js";
import "./diff-stats.js";
import "./send-to-cloud.js";
import "./format-skill-scope.js";
import "./use-is-dictation-supported-CTk-PkHk.js";
import "./list-navigation.js";
/* empty css             */ import "./use-diff-comments.js";
import "./right-panel-composer-overlay-scroll-reserve.js";
import "./use-workspace-file-search.js";
import "./at-mention-list-1.js";
import "./mcp-capability-view-frame.js";
import "./browser-sidebar-comment-mode-coachmark-state.js";
import "./project-dropdown-options.js";
import "./anchored-autocomplete-overlay.js";
import "./mcp-capability-client.js";
import "./prompt-editor.js";
import "./thread-right-panel-state.js";
import "./back-forward-navigation-buttons.js";
import "./use-diff-comment-sources.js";
import "./browser-use-settings-visibility.js";
import "./google-chrome.js";
import "./annotation-mode-button.js";
import "./browser-sidebar-comment-light-dismiss.js";
import "./arrow-rotate-ccw.js";
import "./drag.js";
import "./link.js";
import "./more-menu-trigger-3O9O-Cbz.js";
import "./browser-use-origin-state-queries.js";
import "./browser-sidebar-webview.js";
import "./large-empty-state.js";
import "./action-popover-primitives.js";
import "./pull-request-code-review-comments.js";
import "./codex-analytics-attribution.js";
import "./use-target-apps.js";
import "./diff-unified.js";
import "./rich-preview-primitives.js";
import "./markdown-surface.js";
import "./use-autocomplete-overlay-placement.js";
import "./open-artifact-side-panel-tab.js";
import "./use-app-connect-flow.js";
import "./compound-button.js";
import "./artifact-preview-header.js";
import "./plugins-availability-4A-24D7s.js";
import "./history.js";
import "./alert.js";
import "./mcp-tool-item-content-utils.js";
import "./workspace-directory-tree.js";
import "./git-branch-picker-dropdown-content.js";
import "./use-git-synced-branch.js";
import "./summary-panel-row.js";
import "./middle-truncated-text.js";
import "./use-measured-text-collapse.js";
import "./document-search.js";
import { t as le } from "./use-git-recent-branches.js";
var E = a(),
  ue = e(t(), 1),
  D = `# Review Guidelines

You are acting as a reviewer for a proposed code change made by another engineer.

Review the change and respond in normal Markdown. Do not return JSON, XML, a findings object, or any structured review schema.

When feedback should be attached directly to a changed line, emit one \`::code-comment{...}\` directive for that issue. The directive creates an inline code comment in the review UI; keep the visible response as normal Markdown. Emit no directives when there are no actionable inline comments.

Required \`code-comment\` attributes: \`title\`, \`body\`, and \`file\`. Optional attributes: \`start\`, \`end\`, and \`priority\`. Use the shortest useful line range. \`file\` should be an absolute path or include the workspace folder segment.

Focus on discrete, actionable issues the original author would likely fix if they knew about them. Prefer no issues over speculative or low-signal feedback.

General guidelines for whether to call out an issue:

1. It meaningfully impacts correctness, performance, security, or maintainability.
2. It is discrete and actionable.
3. It was introduced by the change under review.
4. The author would likely fix it once aware.
5. It does not rely on unstated assumptions about intent.
6. It identifies the affected behavior clearly rather than speculating broadly.

When you call out an issue, include the relevant file and line or function in prose, explain the scenario where it matters, and keep the explanation concise. Use priority labels such as \`[P1]\` or \`[P2]\` only when helpful to communicate severity.

If there are no actionable issues, say that directly and briefly.
`,
  O =
    "Review the code changes against the base branch '{baseBranch}'. The merge base commit for this comparison is {mergeBaseSha}. Run `git diff {mergeBaseSha}` to inspect the changes relative to {baseBranch}. Provide concise, actionable feedback in a normal Markdown response.",
  k = `Review the current code changes (staged, unstaged, and untracked files) and provide concise, actionable feedback in a normal Markdown response.`;
function A({ reviewInstructions: e, requestMessage: t }) {
  return [o, D.trim(), e.trim(), u, t].join(`
`);
}
async function j({ context: e, hostId: t, intl: n }) {
  if (e.mode === `uncommitted`)
    return {
      diffFilter: `unstaged`,
      prompt: A({
        reviewInstructions: k,
        requestMessage: n.formatMessage({
          id: `quickAction.request.codeReview.uncommitted`,
          defaultMessage: `Please review my uncommitted changes`,
          description: `User message used when reviewing uncommitted changes`,
        }),
      }),
      baseBranch: null,
    };
  let r = await p(`git-merge-base`, {
    source: `review_model`,
    params: { gitRoot: e.gitRoot, baseBranch: e.baseBranch, hostId: t },
  });
  if (!r.mergeBaseSha)
    throw Error(`Failed to resolve a merge base between HEAD and ${e.baseBranch}.`);
  let i = n.formatMessage(
    {
      id: `quickAction.request.codeReview.branches`,
      defaultMessage: `Please review changes on {from} against {to}`,
      description: `User message used when reviewing against a selected base branch`,
    },
    { from: e.sourceBranch, to: e.baseBranch },
  );
  return {
    diffFilter: `branch`,
    prompt: A({
      reviewInstructions: O.replaceAll(`{baseBranch}`, e.baseBranch).replaceAll(
        `{mergeBaseSha}`,
        r.mergeBaseSha.trim(),
      ),
      requestMessage: i,
    }),
    baseBranch: e.baseBranch,
  };
}
function de(e) {
  let t = (0, E.c)(13),
    { hostId: n, onError: a, onSuccess: o } = e,
    c = r(i),
    u = h(),
    p = C(n),
    m;
  t[0] !== n || t[1] !== p || t[2] !== u || t[3] !== c
    ? ((m = async (e) => {
        let { context: t, conversationId: r, delivery: i } = e,
          { gitRoot: a, cwd: o } = t,
          d = await j({ context: t, hostId: n, intl: u }),
          f = r == null ? null : c.get(l, r);
        if (i === `inline` && r != null && f === n) {
          if (p == null) throw Error(`Code review host is unavailable`);
          return (
            await s(`start-turn-for-host`, {
              hostId: p.getHostId(),
              conversationId: r,
              params: {
                cwd: o,
                input: [{ type: `text`, text: d.prompt, text_elements: [] }],
                approvalsReviewer: `user`,
                collaborationMode: null,
                inheritThreadSettings: !1,
                serviceTier: await w(c, n, null),
              },
            }),
            { delivery: `inline`, diffFilter: d.diffFilter, baseBranch: d.baseBranch }
          );
        }
        let m = await s(`start-conversation`, {
          hostId: n,
          input: [{ type: `text`, text: d.prompt, text_elements: [] }],
          cwd: o,
          workspaceRoots: [a],
          collaborationMode: null,
          serviceTier: await w(c, n, null),
          approvalsReviewer: `user`,
        });
        return {
          baseBranch: d.baseBranch,
          conversationId: m,
          delivery: `detached`,
          diffFilter: d.diffFilter,
        };
      }),
      (t[0] = n),
      (t[1] = p),
      (t[2] = u),
      (t[3] = c),
      (t[4] = m))
    : (m = t[4]);
  let g;
  t[5] === o
    ? (g = t[6])
    : ((g = (e) => {
        o(e);
      }),
      (t[5] = o),
      (t[6] = g));
  let _;
  t[7] === a
    ? (_ = t[8])
    : ((_ = (e) => {
        (f.error(`Failed to start quick review conversation`, {
          safe: {},
          sensitive: { error: e },
        }),
          a(e));
      }),
      (t[7] = a),
      (t[8] = _));
  let v;
  return (
    t[9] !== m || t[10] !== g || t[11] !== _
      ? ((v = { mutationFn: m, onSuccess: g, onError: _ }),
        (t[9] = m),
        (t[10] = g),
        (t[11] = _),
        (t[12] = v))
      : (v = t[12]),
    d(v)
  );
}
function fe(e, t, n, r) {
  let i = (0, E.c)(2),
    a;
  return (
    i[0] === n
      ? (a = i[1])
      : ((a = (e) => {
          let { root: t } = e;
          return { operationSource: n, root: t };
        }),
        (i[0] = n),
        (i[1] = a)),
    c(e, t, `base-branch`, a, n, r)
  );
}
var M = `main`;
function pe({ currentBranch: e, defaultTargetBranch: t, recentBranches: n }) {
  let r = [],
    i = [t ?? M],
    a = new Set();
  return (
    e && a.add(e),
    n != null && i.push(...n),
    i.forEach((e) => {
      !e || a.has(e) || (r.push(e), a.add(e));
    }),
    r
  );
}
var N = n();
function me(e) {
  let t = (0, E.c)(17),
    { onSelectUnstaged: n, onSelectBaseBranch: r, isSubmitting: i, isLoadingBaseBranch: a } = e,
    o = h(),
    s;
  t[0] === o
    ? (s = t[1])
    : ((s = o.formatMessage({
        id: `composer.reviewMode.option.baseBranch.simple`,
        defaultMessage: `Review against a base branch`,
        description: `Button label for reviewing against a base branch`,
      })),
      (t[0] = o),
      (t[1] = s));
  let c = i || a,
    l = a ? y : void 0,
    u;
  t[2] !== r || t[3] !== s || t[4] !== c || t[5] !== l
    ? ((u = (0, N.jsx)(T, {
        value: `base-branch`,
        title: s,
        onSelect: r,
        disabled: c,
        RightIcon: l,
      })),
      (t[2] = r),
      (t[3] = s),
      (t[4] = c),
      (t[5] = l),
      (t[6] = u))
    : (u = t[6]);
  let d;
  t[7] === o
    ? (d = t[8])
    : ((d = o.formatMessage({
        id: `composer.reviewMode.option.unstaged.simple`,
        defaultMessage: `Review uncommitted changes`,
        description: `Button label for reviewing unstaged changes`,
      })),
      (t[7] = o),
      (t[8] = d));
  let f = i ? y : void 0,
    p;
  t[9] !== i || t[10] !== n || t[11] !== d || t[12] !== f
    ? ((p = (0, N.jsx)(T, { value: `unstaged`, title: d, onSelect: n, disabled: i, RightIcon: f })),
      (t[9] = i),
      (t[10] = n),
      (t[11] = d),
      (t[12] = f),
      (t[13] = p))
    : (p = t[13]);
  let m;
  return (
    t[14] !== u || t[15] !== p
      ? ((m = (0, N.jsxs)(N.Fragment, { children: [u, p] })), (t[14] = u), (t[15] = p), (t[16] = m))
      : (m = t[16]),
    m
  );
}
function he(e) {
  let t = (0, E.c)(10),
    {
      onSelect: n,
      branchLines: r,
      isLoading: i,
      isError: a,
      refetchBranchOverview: o,
      submittingBranchName: s,
    } = e,
    c = s != null,
    l;
  t[0] !== r || t[1] !== a || t[2] !== i || t[3] !== c || t[4] !== n || t[5] !== o || t[6] !== s
    ? ((l = i
        ? (0, N.jsx)(`div`, {
            className: `flex items-center justify-center gap-2 py-4 text-xs text-token-foreground/70`,
            children: (0, N.jsx)(y, { className: `size-3` }),
          })
        : a
          ? (0, N.jsxs)(`div`, {
              className: `flex flex-col gap-2 py-2`,
              children: [
                (0, N.jsx)(`span`, {
                  className: `text-center text-xs text-token-foreground/70`,
                  children: (0, N.jsx)(g, {
                    id: `composer.reviewMode.branches.error`,
                    defaultMessage: `Unable to load branches`,
                    description: `Error message when branch list could not be loaded`,
                  }),
                }),
                (0, N.jsx)(`button`, {
                  type: `button`,
                  className: `text-xs font-medium text-token-text-link-foreground`,
                  onClick: o,
                  children: (0, N.jsx)(g, {
                    id: `composer.reviewMode.branches.retry`,
                    defaultMessage: `Retry`,
                    description: `Retry button for branch list error`,
                  }),
                }),
              ],
            })
          : r.map((e) =>
              (0, N.jsx)(
                T,
                {
                  value: e.label,
                  title: e.label,
                  onSelect: n,
                  disabled: c,
                  RightIcon: s === e.key ? y : void 0,
                },
                e.key,
              ),
            )),
      (t[0] = r),
      (t[1] = a),
      (t[2] = i),
      (t[3] = c),
      (t[4] = n),
      (t[5] = o),
      (t[6] = s),
      (t[7] = l))
    : (l = t[7]);
  let u;
  return (
    t[8] === l
      ? (u = t[9])
      : ((u = (0, N.jsx)(N.Fragment, { children: l })), (t[8] = l), (t[9] = u)),
    u
  );
}
function P(e) {
  let t = (0, E.c)(56),
    { conversationId: n, onItemsChanged: i, onClose: a, cwd: o, hostConfig: s } = e,
    c = r(te),
    l = h(),
    u;
  t[0] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((u = { status: `choose-target` }), (t[0] = u))
    : (u = t[0]);
  let [d, p] = (0, ue.useState)(u),
    g = oe(),
    y;
  t[1] === s.id
    ? (y = t[2])
    : ((y = { hostId: s.id, source: `review_mode_content` }), (t[1] = s.id), (t[2] = y));
  let { gitRoot: C } = ce(o, y),
    { data: w, isLoading: T } = se(C, s, `review_mode_content`),
    { data: D, isLoading: O, isError: k, refetch: A } = fe(C, s, `review_mode_content`),
    { data: j, isLoading: M, isError: P, refetch: F } = le(C, s, `review_mode_content`),
    _e = re(D ?? null),
    I = v(m.reviewDelivery),
    ve = pe({ currentBranch: w, defaultTargetBranch: _e, recentBranches: j }),
    L = O || M,
    R = k || P,
    z;
  t[3] !== A || t[4] !== F
    ? ((z = () => {
        Promise.all([A(), F()]);
      }),
      (t[3] = A),
      (t[4] = F),
      (t[5] = z))
    : (z = t[5]);
  let B = z,
    V;
  t[6] !== l || t[7] !== c
    ? ((V = (e) => {
        (f.error(`Failed to start code review`, { safe: {}, sensitive: { error: e } }),
          c
            .get(b)
            .danger(
              l.formatMessage({
                id: `composer.reviewMode.quickReviewError`,
                defaultMessage: `Failed to start code review.`,
                description: `Toast shown when quick review action fails`,
              }),
            ));
      }),
      (t[6] = l),
      (t[7] = c),
      (t[8] = V))
    : (V = t[8]);
  let H;
  t[9] !== n || t[10] !== g || t[11] !== c
    ? ((H = (e) => {
        if ((ae(c, e.diffFilter), e.delivery === `detached`)) {
          (ie(c, e.conversationId, e.baseBranch), g(e.conversationId));
          return;
        }
        ie(c, n, e.baseBranch);
        let t = c.get(ne.activeTab$)?.tabId;
        (ee(c, !t?.startsWith(`sidechat:`)), c.set(_, !1));
      }),
      (t[9] = n),
      (t[10] = g),
      (t[11] = c),
      (t[12] = H))
    : (H = t[12]);
  let U;
  t[13] !== s.id || t[14] !== V || t[15] !== H
    ? ((U = { hostId: s.id, onError: V, onSuccess: H }),
      (t[13] = s.id),
      (t[14] = V),
      (t[15] = H),
      (t[16] = U))
    : (U = t[16]);
  let { mutate: W, isPending: G, variables: ye } = de(U),
    K = G && ye?.context.mode === `base-branch` ? ye.context.baseBranch : null,
    q;
  t[17] !== n ||
  t[18] !== w ||
  t[19] !== o ||
  t[20] !== C ||
  t[21] !== l ||
  t[22] !== G ||
  t[23] !== a ||
  t[24] !== i ||
  t[25] !== I ||
  t[26] !== c ||
  t[27] !== W
    ? ((q = (e) => {
        if (!G) {
          if (!C) {
            c.get(b).danger(
              l.formatMessage({
                id: `composer.reviewMode.gitRoot.error`,
                defaultMessage: `Git root not found`,
                description: `Toast shown when git root not found`,
              }),
            );
            return;
          }
          if (e === `base-branch`) {
            (S(c, x, { target: `base_branch` }), p({ status: `choose-base` }), i());
            return;
          }
          (S(c, x, { target: `unstaged` }),
            W(
              {
                conversationId: n,
                context: { mode: `uncommitted`, sourceBranch: w ?? `HEAD`, gitRoot: C, cwd: o },
                delivery: I ?? `inline`,
              },
              { onSuccess: a },
            ));
        }
      }),
      (t[17] = n),
      (t[18] = w),
      (t[19] = o),
      (t[20] = C),
      (t[21] = l),
      (t[22] = G),
      (t[23] = a),
      (t[24] = i),
      (t[25] = I),
      (t[26] = c),
      (t[27] = W),
      (t[28] = q))
    : (q = t[28]);
  let J = q,
    Y;
  t[29] !== n ||
  t[30] !== w ||
  t[31] !== o ||
  t[32] !== C ||
  t[33] !== l ||
  t[34] !== G ||
  t[35] !== a ||
  t[36] !== I ||
  t[37] !== c ||
  t[38] !== W
    ? ((Y = (e) => {
        if (!G) {
          if (!C) {
            c.get(b).danger(
              l.formatMessage({
                id: `composer.reviewMode.gitRoot.error`,
                defaultMessage: `Git root not found`,
                description: `Toast shown when git root not found`,
              }),
            );
            return;
          }
          W(
            {
              conversationId: n,
              context: {
                mode: `base-branch`,
                sourceBranch: w ?? `HEAD`,
                baseBranch: e,
                gitRoot: C,
                cwd: o,
              },
              delivery: I ?? `inline`,
            },
            { onSuccess: a },
          );
        }
      }),
      (t[29] = n),
      (t[30] = w),
      (t[31] = o),
      (t[32] = C),
      (t[33] = l),
      (t[34] = G),
      (t[35] = a),
      (t[36] = I),
      (t[37] = c),
      (t[38] = W),
      (t[39] = Y))
    : (Y = t[39]);
  let X = Y;
  if (d.status === `choose-target`) {
    let e, n;
    t[40] === J
      ? ((e = t[41]), (n = t[42]))
      : ((n = () => J(`unstaged`)),
        (e = () => J(`base-branch`)),
        (t[40] = J),
        (t[41] = e),
        (t[42] = n));
    let r;
    return (
      t[43] !== T || t[44] !== G || t[45] !== e || t[46] !== n
        ? ((r = (0, N.jsx)(me, {
            onSelectUnstaged: n,
            onSelectBaseBranch: e,
            isSubmitting: G,
            isLoadingBaseBranch: T,
          })),
          (t[43] = T),
          (t[44] = G),
          (t[45] = e),
          (t[46] = n),
          (t[47] = r))
        : (r = t[47]),
      r
    );
  }
  let Z = he,
    Q = ve.map(ge),
    $;
  return (
    t[48] !== Z ||
    t[49] !== X ||
    t[50] !== R ||
    t[51] !== L ||
    t[52] !== B ||
    t[53] !== K ||
    t[54] !== Q
      ? (($ = (0, N.jsx)(Z, {
          onSelect: X,
          branchLines: Q,
          isLoading: L,
          isError: R,
          refetchBranchOverview: B,
          submittingBranchName: K,
        })),
        (t[48] = Z),
        (t[49] = X),
        (t[50] = R),
        (t[51] = L),
        (t[52] = B),
        (t[53] = K),
        (t[54] = Q),
        (t[55] = $))
      : ($ = t[55]),
    $
  );
}
function ge(e) {
  return { key: e, label: e };
}
export { P as ReviewModeContent };
//# sourceMappingURL=review-mode-content.js.map
