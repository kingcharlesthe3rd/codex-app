...
import { Cn as d, E as ee, Sn as te } from "./app-server-manager-signals.js";
import "./jsx-runtime.js";
import "./marked.esm-BR-H6018.js";
import { f, r as p } from "./vscode-api.js";
import "./lib-2.js";
...
import "./button.js";
import { n as m, s as ne } from "./setting-storage.js";
...
import { W as h, a as re, g as ie, o as g, u as _ } from "./app-shell-state.js";
import "./platform.js";
import "./use-platform.js";
import { d as v, g as y, l as b, m as x } from "./diff-view-mode.js";
import { l as S, n as C, s as w } from "./app-shell-tab-controller.js";
import { t as T } from "./browser-sidebar-manager.js";
import { t as ae } from "./local-conversation-title-signals.js";
import { J as E, Ut as D, i as oe } from "./review-navigation-model.js";
...
import { a as se, s as ce } from "./thread-page-bottom-panel-state.js";
...
var ue = U({
    schema: o({ type: i(`app.appearance.get`) }),
    run: async () => {
      let [e, n, r, i, a] = await Promise.all([
        m(t.theme),
        m(t.lightCodeThemeId),
        m(t.darkCodeThemeId),
        m(t.lightChromeTheme),
        m(t.darkChromeTheme),
      ]);
      return {
        schemaVersion: 1,
  ...
  de = U({
    schema: o({ type: i(`app.appearance.get_available_themes`) }),
    run: () => ({
      schemaVersion: 1,
      themes: v().map((e) => ({
        id: e.id,
...
async function G(e, t, n, r) {
  let i = p(`get-settings`),
    a = e.queryClient?.getQueryData(i),
    o = r?.optimistic ?? !0;
  o && e.queryClient?.setQueryData(i, { values: { ...a?.values, [t.key]: n } });
  try {
    (await ne(t, n), o || e.queryClient?.setQueryData(i, { values: { ...a?.values, [t.key]: n } }));
  } catch (t) {
    throw (a != null && e.queryClient?.setQueryData(i, a), t);
  } finally {
    (await e.queryClient?.invalidateQueries({ queryKey: i }),
      f.dispatchMessage(`query-cache-invalidate`, { queryKey: [...i] }));
  }
}
var fe = U({
    schema: o({ type: i(`app.appearance.set_mode`), mode: e([`light`, `dark`, `system`]) }),
    run: async ({ mode: e }, n) => (
      await G(n, t.theme, e, { optimistic: !1 }),
      { schemaVersion: 1, mode: e }
    ),
  }),
  K = c().regex(/^#[0-9a-fA-F]{6}$/),
  pe = o({ code: c().nullable().optional(), ui: c().nullable().optional() }),
  me = o({ diffAdded: K, diffRemoved: K, skill: K }),
  he = o({
    accent: K.optional(),
    contrast: u().int().min(0).max(100).optional(),
    fonts: pe.optional(),
    ink: K.optional(),
    opaqueWindows: r().optional(),
  ...
  ge = U({
    schema: o({
      ...
      theme: n(`kind`, [
        o({ kind: i(`preset`), themeId: c().refine(x, `Invalid code theme id`) }),
        o({ kind: i(`custom`), patch: he }),
      ]),
      variant: e([`light`, `dark`, `both`]).default(`both`),
    }),
...
function ve(e) {
  return e === `light`
    ? { chromeThemeSetting: t.lightChromeTheme, codeThemeSetting: t.lightCodeThemeId }
    : { chromeThemeSetting: t.darkChromeTheme, codeThemeSetting: t.darkCodeThemeId };
}
function J(e) {
...
var ye = U({
  schema: o({ type: i(`app.get_summary`) }),
  run: (e, t) => {
    let n = J(t),
      r = n.get(_),
      i = n.get(re),
    ...
    return {
      ...
      window: {
        ...
        thread: xe(n),
        panels: {
          browser: Ce(Te(n), s, o, u),
          sidebar: { open: r },
          review: { open: l, fullscreen: l && u, fileTreeOpen: l && n.get(ie), view: n.get(D) },
          terminal: { open: d },
          rightPanel: { fullscreen: u, kind: c },
        },
        ...(r ? { sidebar: { viewport: Y(M.sidebarScroll), rows: Ee() } } : {}),
        ...(l ? { review: { viewport: Y(M.reviewScroll), files: Oe(n) } } : {}),
  ...
  switch (e.value.routeKind) {
    case `local-thread`:
      return {
        ...
        title: e.get(ae, e.value.conversationId),
      };
    case `remote-thread`:
...
function Ce(e, t, n, r) {
  let i = w(t, e),
    a = i != null,
    o = e == null || i == null ? null : T.getSnapshot(e, i);
  return {
    canGoBack: o?.canGoBack ?? !1,
    canGoForward: o?.canGoForward ?? !1,
    fullscreen: a && r,
    isLoading: o?.isLoading ?? !1,
    open: a,
    tabs: we(e, n, i),
    title: o?.title ?? null,
    url: o?.url ?? null,
...
function we(e, t, n) {
  if (e == null) return [];
  let r = T.getBrowserUseBrowserTabIds(e),
    i = [
      ...t.flatMap((t) => {
        let n = w(t, e);
        return n == null ? [] : [n];
  ...
  return i.flatMap((t) => {
    if (o.has(t)) return [];
    o.add(t);
    let r = T.getSnapshot(e, t);
    return [
      {
        active: t === n,
        browserTabId: t,
        isBrowserUseActive: T.isBrowserUseActive(e, t),
...
function Ee() {
  return Array.from(document.querySelectorAll(le)).map((e, t) => De(e, t));
}
function De(e, t) {
  let n = Z(e);
...
function Oe(e) {
  let t = ke(),
    n = new Set(e.get(E).map((e) => e.path));
  return [
    ...e
      .get(E)
      .map((e) => ({
...
var Ae = o({ type: i(`app.help`), action: c().optional() });
function je(e) {
  return U({ schema: Ae, run: ({ action: t }) => Me(e(), t) });
}
function Me(e, t, n = Ne) {
  return {
    schemaVersion: 1,
    prompt: n,
    actions: e
      .filter((e) => (t == null ? !0 : e.type === t))
      .map((e) => ({ type: e.type, jsonSchema: JSON.stringify(s(e.schema), null, 2) })),
  };
}
var Ne = `You can inspect or operate the Codex desktop app itself by calling this dynamic tool with exactly one JSON action payload.
...
Prefer the smallest action that directly satisfies the user request.`,
  Pe = U({
    schema: o({ type: i(`windows.nav.back`), windowId: I }),
    run: () => {
      f.dispatchHostMessage({ type: `navigate-back` });
    },
  }),
  Fe = U({
    schema: o({ type: i(`windows.nav.forward`), windowId: I }),
    run: () => {
      f.dispatchHostMessage({ type: `navigate-forward` });
    },
  }),
  Ie = U({
    schema: o({ type: i(`windows.review.collapse_all`), windowId: I }),
    run: () => {
  ...
  Le = U({
    schema: o({ type: i(`windows.review.expand_all`), windowId: I }),
    run: () => {
      window.dispatchEvent(
  ...
  ze = U({
    schema: o({ type: i(`windows.review.scroll`), windowId: I, scroll: F }),
    run: ({ scroll: e }) => {
  ...
  Be = U({
    schema: o({
      ...
      windowId: I,
      path: c(),
      align: e([`top`, `center`, `bottom`]).optional(),
    }),
    run: ({ path: e, align: t }, n) => {
      let r = N(e);
      (oe(J(n), r.dataset.reviewPath ?? e),
        r.scrollIntoView({ block: Ve(t ?? `top`), behavior: `auto` }));
    },
  });
function Ve(e) {
  switch (e) {
...
var He = U({
    schema: o({ type: i(`windows.review.set_fullscreen`), windowId: I, fullscreen: r() }),
    run: ({ fullscreen: e }, t) => {
      J(t).set(h, e);
    },
  }),
  Q = U({
    schema: o({
      ...
      windowId: I,
      view: e([`turn`, `branch`, `unstaged`, `staged`]),
    }),
    run: ({ view: e }, t) => {
      J(t).set(D, e === `turn` ? `last-turn` : e);
    },
  }),
  Ue = U({
    schema: o({ type: i(`windows.review.toggle`), windowId: I }),
    run: () => {
      f.dispatchHostMessage({ type: `toggle-diff-panel` });
    },
  }),
  We = U({
    schema: o({ type: i(`windows.show_home`), windowId: I }),
    run: () => {
      f.dispatchHostMessage({ type: `new-chat` });
    },
  }),
  Ge = U({
    schema: o({ type: i(`windows.show_thread`), windowId: I, threadId: c() }),
    run: ({ threadId: e }) => {
      f.dispatchHostMessage({ type: `navigate-to-route`, path: Ke(e) });
    },
  });
function Ke(e) {
  let t = d(e);
  return t == null ? l(a(e)) : te(t.key);
}
var qe = [
    ye,
    ue,
    ...
    U({
      schema: o({ type: i(`windows.sidebar.scroll`), windowId: I, scroll: F }),
      run: ({ scroll: e }) => {
    ...
    U({
      schema: o({ type: i(`windows.sidebar.select_project`), windowId: I, project: A }),
      run: ({ project: e }) => {
    ...
    U({
      schema: o({ type: i(`windows.sidebar.toggle`), windowId: I }),
      run: () => {
        f.dispatchHostMessage({ type: `toggle-sidebar` });
      },
    }),
    U({
      schema: o({ type: i(`windows.terminal.toggle`), windowId: I }),
      run: (e, t) => {
        ce(J(t));
      },
    }),
    U({
      schema: o({ type: i(`windows.timeline.scroll`), windowId: I, scroll: F }),
      run: ({ scroll: e }) => {
        P(z(M.timelineScroll), e);
      },
    }),
    U({
      schema: o({ type: i(`windows.timeline.scroll_to_turn`), windowId: I, direction: R }),
      run: ({ direction: e }) => {
...
export { Je as appActionRegistry };
