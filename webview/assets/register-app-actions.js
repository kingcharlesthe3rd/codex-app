...
import "./app-scope.js";
import { f as e, r as t } from "./vscode-api.js";
import "./isEqual.js";
import {
  Bi as n,
  Ki as r,
...
import { D as p, En as ee, Tn as te } from "./app-server-manager-signals.js";
import "./lib-1.js";
import "./persisted-signal.js";
import { A as ne, W as m, a as h, g, o as _, r as v, u as y } from "./app-shell-state.js";
import "./button.js";
import { n as b, s as x } from "./setting-storage.js";
...
import { d as re, g as ie, l as ae, m as oe } from "./diff-view-mode.js";
import { n as se, t as ce, u as le } from "./app-shell-tab-controller.js";
import { t as S } from "./browser-sidebar-manager.js";
import { t as ue } from "./local-conversation-title-signals.js";
import { J as C, Wt as w, i as T } from "./review-navigation-model.js";
...
import { n as E, s as D } from "./thread-browser-panel-tabs.js";
import "./terminal.js";
import "./app-intl-signal.js";
import { a as O, s as k } from "./thread-page-bottom-panel-state.js";
...
var me = W({
    schema: u({ type: i(`app.appearance.get`) }),
    run: async () => {
  ...
  he = W({
    schema: u({ type: i(`app.appearance.get_available_themes`) }),
    run: () => ({
      schemaVersion: 1,
      themes: re().map((e) => ({
        id: e.id,
...
async function G(n, r, i, a) {
  let o = t(`get-settings`),
    s = n.queryClient?.getQueryData(o),
    c = a?.optimistic ?? !0;
  c && n.queryClient?.setQueryData(o, { values: { ...s?.values, [r.key]: i } });
  try {
    (await x(r, i), c || n.queryClient?.setQueryData(o, { values: { ...s?.values, [r.key]: i } }));
  } catch (e) {
    throw (s != null && n.queryClient?.setQueryData(o, s), e);
  } finally {
    (await n.queryClient?.invalidateQueries({ queryKey: o }),
      e.dispatchMessage(`query-cache-invalidate`, { queryKey: [...o] }));
  }
}
var ge = W({
    schema: u({ type: i(`app.appearance.set_mode`), mode: l([`light`, `dark`, `system`]) }),
    run: async ({ mode: e }, t) => (
      await G(t, s.theme, e, { optimistic: !1 }),
      { schemaVersion: 1, mode: e }
    ),
  }),
  K = r().regex(/^#[0-9a-fA-F]{6}$/),
  _e = u({ code: r().nullable().optional(), ui: r().nullable().optional() }),
  ve = u({ diffAdded: K, diffRemoved: K, skill: K }),
  ye = u({
    accent: K.optional(),
    contrast: n().int().min(0).max(100).optional(),
    fonts: _e.optional(),
    ink: K.optional(),
    opaqueWindows: d().optional(),
    semanticColors: ve.partial().optional(),
    surface: K.optional(),
  }),
  be = W({
    schema: u({
      type: i(`app.appearance.set_theme`),
      theme: o(`kind`, [
        u({ kind: i(`preset`), themeId: r().refine(oe, `Invalid code theme id`) }),
        u({ kind: i(`custom`), patch: ye }),
      ]),
      variant: l([`light`, `dark`, `both`]).default(`both`),
    }),
...
async function q(e, t, n) {
  let { chromeThemeSetting: r, codeThemeSetting: i } = Se(t),
    a = ie(await b(r), t);
  if (n.kind === `custom`) {
    let t = xe(a, n.patch);
    return (await G(e, r, t), t);
  }
  let o = await ae(n.themeId, t),
    s = {
      ...a,
      ...o,
...
function Se(e) {
  return e === `light`
    ? { chromeThemeSetting: s.lightChromeTheme, codeThemeSetting: s.lightCodeThemeId }
    : { chromeThemeSetting: s.darkChromeTheme, codeThemeSetting: s.darkCodeThemeId };
}
function J(e) {
...
var Ce = W({
  schema: u({ type: i(`app.get_summary`) }),
  run: (e, t) => {
    let n = J(t),
      r = n.get(y),
      i = n.get(h),
      a = n.get(_),
      o = i && a ? n.get(se.activeTab$) : null,
      s = n.get(v) ? n.get(ce.activeTab$) : null,
      c = o?.tabId ?? null,
      l = c === le.DIFF,
      u = n.get(m),
      d = n.get(O);
    return {
      ...
      window: {
        ...
        thread: Te(n),
        panels: {
          browser: De(n, ke(n), s, o, u),
          sidebar: { open: r },
          review: { open: l, fullscreen: l && u, fileTreeOpen: l && n.get(g), view: n.get(w) },
          terminal: { open: d },
          rightPanel: { fullscreen: u, kind: c },
        },
        ...(r ? { sidebar: { viewport: Y(F.sidebarScroll), rows: Ae() } } : {}),
        ...(l ? { review: { viewport: Y(F.reviewScroll), files: Me(n) } } : {}),
        ...(we(n.value) ? { timeline: Y(F.timelineScroll) } : {}),
      },
    };
  },
});
  ...
  switch (e.value.routeKind) {
    case `local-thread`:
      return {
        ...
        kind: `local`,
        hostId: e.get(p, e.value.conversationId) ?? `local`,
...
function De(e, t, n, r, i) {
  let a = t == null ? [] : E(e, t),
    o = t == null ? null : D(t, e.get(ne), { bottom: n, right: r }),
    s = o == null ? null : (a.find((e) => e.browserTabId === o) ?? null),
    c = t == null || o == null ? null : S.getSnapshot(t, o);
  return {
...
function Oe(e, t, n) {
  if (e == null) return [];
  let r = S.getBrowserUseBrowserTabIds(e),
    i = [...t, ...r],
    a = new Set(r),
    o = new Set();
  return i.flatMap((t) => {
    if (o.has(t)) return [];
    o.add(t);
    let r = S.getSnapshot(e, t);
    return [
      {
        active: t === n,
        browserTabId: t,
        isBrowserUseActive: S.isBrowserUseActive(e, t),
...
function Ae() {
  return Array.from(document.querySelectorAll(A)).map((e, t) => je(e, t));
}
function je(e, t) {
  let n = Z(e);
  return e.matches(F.sidebarSection)
    ? {
        type: `section`,
        index: t,
...
function X(e) {
  let t = e?.querySelector(F.reviewFileToggle);
  return {
...
function Ne() {
  let e = Array.from(document.querySelectorAll(F.reviewFile));
  return new Map(
    e.flatMap((e) => {
...
var Pe = u({ type: i(`app.help`), action: r().optional() });
function Fe(e) {
  return W({ schema: Pe, run: ({ action: t }) => Ie(e(), t) });
}
function Ie(e, t, n = Le) {
  return {
    schemaVersion: 1,
    prompt: n,
    actions: e
      .filter((e) => (t == null ? !0 : e.type === t))
      .map((e) => ({ type: e.type, jsonSchema: JSON.stringify(c(e.schema), null, 2) })),
  };
}
var Le = `You can inspect or operate the Codex desktop app itself by calling this dynamic tool with exactly one JSON action payload.
...
Prefer the smallest action that directly satisfies the user request.`,
  Re = W({
    schema: u({ type: i(`windows.nav.back`), windowId: z }),
    run: () => {
      e.dispatchHostMessage({ type: `navigate-back` });
    },
  }),
  ze = W({
    schema: u({ type: i(`windows.nav.forward`), windowId: z }),
    run: () => {
      e.dispatchHostMessage({ type: `navigate-forward` });
    },
  }),
  Be = W({
    schema: u({ type: i(`windows.review.collapse_all`), windowId: z }),
    run: () => {
  ...
  Ve = W({
    schema: u({ type: i(`windows.review.expand_all`), windowId: z }),
    run: () => {
      window.dispatchEvent(
  ...
  He = W({
    schema: u({
      type: i(`windows.review.file_set_expanded`),
      windowId: z,
      path: r(),
      expanded: d(),
    }),
    run: ({ path: e, expanded: t }) => {
      let n = I(e).querySelector(F.reviewFileToggle);
      if (n == null) throw Error(`Missing review file toggle: ${e}`);
      n.dataset.appActionReviewFileExpanded !== String(t) && n.click();
    },
  }),
  Q = W({
    schema: u({ type: i(`windows.review.scroll`), windowId: z, scroll: R }),
    run: ({ scroll: e }) => {
      M(H(F.reviewScroll), e);
    },
  }),
  Ue = W({
    schema: u({
      type: i(`windows.review.scroll_to_file`),
      windowId: z,
      path: r(),
      align: l([`top`, `center`, `bottom`]).optional(),
    }),
    run: ({ path: e, align: t }, n) => {
      let r = I(e);
      (T(J(n), r.dataset.reviewPath ?? e),
        r.scrollIntoView({ block: We(t ?? `top`), behavior: `auto` }));
    },
  });
function We(e) {
  switch (e) {
...
var Ge = W({
    schema: u({ type: i(`windows.review.set_fullscreen`), windowId: z, fullscreen: d() }),
    run: ({ fullscreen: e }, t) => {
      J(t).set(m, e);
    },
  }),
  Ke = W({
    schema: u({
      type: i(`windows.review.set_view`),
      windowId: z,
      view: l([`turn`, `branch`, `unstaged`, `staged`]),
    }),
    run: ({ view: e }, t) => {
  ...
  qe = W({
    schema: u({ type: i(`windows.review.toggle`), windowId: z }),
    run: () => {
      e.dispatchHostMessage({ type: `toggle-diff-panel` });
    },
  }),
  Je = W({
    schema: u({ type: i(`windows.show_home`), windowId: z }),
    run: () => {
      e.dispatchHostMessage({ type: `new-chat` });
    },
  }),
  Ye = W({
    schema: u({ type: i(`windows.show_thread`), windowId: z, threadId: r() }),
    run: ({ threadId: t }) => {
      e.dispatchHostMessage({ type: `navigate-to-route`, path: Xe(t) });
    },
  });
function Xe(e) {
  let t = ee(e);
  return t == null ? f(a(e)) : te(t.key);
}
var Ze = [
    Ce,
    me,
    ...
    W({
      schema: u({
        type: i(`windows.sidebar.project_set_collapsed`),
        windowId: z,
        project: N,
        collapsed: d(),
      }),
    ...
    W({
      schema: u({
        type: i(`windows.sidebar.project_set_show_all`),
        windowId: z,
        project: N,
        showAll: d(),
      }),
      run: ({ project: e, showAll: t }) => {
        ...
        let a = i.querySelector(F.sidebarProjectShowAllToggle);
        if (a == null) throw Error(`Missing sidebar project show more toggle: ${r}`);
        a.click();
      },
    }),
    W({
      schema: u({ type: i(`windows.sidebar.scroll`), windowId: z, scroll: R }),
      run: ({ scroll: e }) => {
        M(H(F.sidebarScroll), e);
      },
    }),
    W({
      schema: u({
        type: i(`windows.sidebar.section_set_collapsed`),
        windowId: z,
        section: P,
        collapsed: d(),
      }),
      run: ({ section: e, collapsed: t }) => {
        ...
        let r = n.querySelector(F.sidebarSectionToggle);
        if (r == null) throw Error(`Sidebar section does not have a collapse toggle`);
        r.click();
      },
    }),
    W({
      schema: u({ type: i(`windows.sidebar.select_project`), windowId: z, project: N }),
      run: ({ project: e }) => {
        let t = j(e).querySelector(F.sidebarProjectSelect);
        if (t == null) throw Error(`Missing sidebar project select action`);
        t.click();
      },
    }),
    W({
      schema: u({ type: i(`windows.sidebar.toggle`), windowId: z }),
      run: () => {
        e.dispatchHostMessage({ type: `toggle-sidebar` });
      },
    }),
    W({
      schema: u({ type: i(`windows.terminal.toggle`), windowId: z }),
      run: (e, t) => {
        k(J(t));
      },
    }),
    W({
      schema: u({ type: i(`windows.timeline.scroll`), windowId: z, scroll: R }),
      run: ({ scroll: e }) => {
        L(H(F.timelineScroll), e);
      },
    }),
    W({
      schema: u({ type: i(`windows.timeline.scroll_to_turn`), windowId: z, direction: V }),
      run: ({ direction: e }) => {
        fe(H(F.timelineScroll), e);
      },
    }),
  ],
  $ = [Fe($e), ...Ze],
