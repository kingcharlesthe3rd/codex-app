...
import { E as ee, hn as d, mn as f } from "./app-server-manager-signals.js";
...
import { l as ae, n as oe, s as se } from "./app-shell-tab-controller.js";
import { t as ce } from "./local-conversation-title-signals.js";
import { L as C, Nt as w, r as le } from "./review-navigation-model.js";
...
async function G(e, t, n, r) {
  ...
  try {
    (await te(t, n), o || e.queryClient?.setQueryData(i, { values: { ...a?.values, [t.key]: n } }));
  } catch (t) {
...
var fe = U({
    schema: r({ type: a(`app.appearance.set_mode`), mode: t([`light`, `dark`, `system`]) }),
    run: async ({ mode: t }, n) => (
  ...
  ge = U({
    schema: r({
      ...
      theme: n(`kind`, [
        r({ kind: a(`preset`), themeId: c().refine(ne, `Invalid code theme id`) }),
...
function ve(t) {
  return t === `light`
    ? { chromeThemeSetting: e.lightChromeTheme, codeThemeSetting: e.lightCodeThemeId }
    : { chromeThemeSetting: e.darkChromeTheme, codeThemeSetting: e.darkCodeThemeId };
}
function J(e) {
    ...
    return {
      ...
      window: {
        ...
        panels: {
          browser: Ce(we(n), o, l),
          sidebar: { open: r },
          review: { open: c, fullscreen: c && l, fileTreeOpen: c && n.get(y), view: n.get(w) },
          terminal: { open: u },
...
var Be = U({
    schema: r({ type: a(`windows.review.set_fullscreen`), windowId: I, fullscreen: i() }),
    run: ({ fullscreen: e }, t) => {
      J(t).set(g, e);
    },
  }),
...
var Ke = [
    ...
    U({
      schema: r({ type: a(`windows.sidebar.select_project`), windowId: I, project: A }),
      run: ({ project: e }) => {
    ...
    U({
      schema: r({ type: a(`windows.timeline.scroll_to_turn`), windowId: I, direction: R }),
      run: ({ direction: e }) => {
