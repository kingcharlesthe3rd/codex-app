...
import { V as e } from "./thread-context-inputs.js";
import { f as t, r as n } from "./vscode-api.js";
import "./isEqual.js";
import {
  Aa as r,
  Ia as i,
...
import "./lib-1.js";
import { G as m, a as ee, g as h, j as g, o as _, r as v, u as y } from "./app-shell-state.js";
import "./button-oF-qgtAS.js";
import { n as b, s as te } from "./setting-storage.js";
...
import "./platform.js";
import { d as k, s as ue } from "./persisted-signal.js";
...
import { t as L } from "./browser-sidebar-manager.js";
import { r as R, t as z } from "./app-shell-tab-controller.js";
import { t as B } from "./local-conversation-title-signals.js";
import { en as V, i as H, it as U } from "./review-navigation-model.js";
...
import { a as de, n as fe, s as pe } from "./thread-browser-panel-tabs.js";
import "./terminal.js";
import "./app-intl-signal.js";
import { a as me, c as he, s as ge } from "./thread-page-bottom-panel-state.js";
import "./error-boundary.js";
import "./app-identity.js";
import "./download.js";
import "./with-window.js";
import "./use-resolved-theme-variant-D.js";
import { c as _e, s as ve } from "./sidebar-thread-keys.js";
import "./windows-tabs-open-schema.js";
var ye = A({
    schema: a({ type: p(`app.appearance.get`) }),
    run: async () => {
      let [e, t, n, r, i] = await Promise.all([
        b(l.theme),
        b(l.lightCodeThemeId),
        b(l.darkCodeThemeId),
        b(l.lightChromeTheme),
        b(l.darkChromeTheme),
      ]);
      return {
        schemaVersion: 1,
  ...
  be = A({
    schema: a({ type: p(`app.appearance.get_available_themes`) }),
    run: () => ({
      schemaVersion: 1,
      themes: N().map((e) => ({
        id: e.id,
...
async function W(e, r, i, a) {
  let o = n(`get-settings`),
    s = e.queryClient?.getQueryData(o),
    c = a?.optimistic ?? !0;
  c && e.queryClient?.setQueryData(o, { values: { ...s?.values, [r.key]: i } });
  try {
    (await te(r, i), c || e.queryClient?.setQueryData(o, { values: { ...s?.values, [r.key]: i } }));
  } catch (t) {
    throw (s != null && e.queryClient?.setQueryData(o, s), t);
  } finally {
    (await e.queryClient?.invalidateQueries({ queryKey: o }),
      t.dispatchMessage(`query-cache-invalidate`, { queryKey: [...o] }));
  }
}
var xe = A({
    schema: a({ type: p(`app.appearance.set_mode`), mode: u([`light`, `dark`, `system`]) }),
    run: async ({ mode: e }, t) => (
      await W(t, l.theme, e, { optimistic: !1 }),
      { schemaVersion: 1, mode: e }
    ),
  }),
  G = r().regex(/^#[0-9a-fA-F]{6}$/),
  Se = a({ code: r().nullable().optional(), ui: r().nullable().optional() }),
  Ce = a({ diffAdded: G, diffRemoved: G, skill: G }),
  we = a({
    accent: G.optional(),
    contrast: f().int().min(0).max(100).optional(),
    fonts: Se.optional(),
    ink: G.optional(),
    opaqueWindows: c().optional(),
    semanticColors: Ce.partial().optional(),
    surface: G.optional(),
  }),
  Te = A({
    schema: a({
      ...
      theme: o(`kind`, [
        a({ kind: p(`preset`), themeId: r().refine(I, `Invalid code theme id`) }),
        a({ kind: p(`custom`), patch: we }),
      ]),
      variant: u([`light`, `dark`, `both`]).default(`both`),
    }),
...
async function K(e, t, n) {
  let { chromeThemeSetting: r, codeThemeSetting: i } = De(t),
    a = P(await b(r), t);
  if (n.kind === `custom`) {
    let t = Ee(a, n.patch);
...
function De(e) {
  return e === `light`
    ? { chromeThemeSetting: l.lightChromeTheme, codeThemeSetting: l.lightCodeThemeId }
    : { chromeThemeSetting: l.darkChromeTheme, codeThemeSetting: l.darkCodeThemeId };
}
function q(e) {
...
var Oe = A({
  schema: a({ type: p(`app.get_summary`) }),
  run: (e, t) => {
    let n = q(t),
      r = n.get(y),
      i = n.get(ee),
      a = n.get(_),
      o = i && a ? n.get(R.activeTab$) : null,
      s = n.get(v) ? n.get(z.activeTab$) : null,
      c = o?.tabId ?? null,
      l = null;
    c === k.DIFF ? (l = `right`) : s?.tabId === k.DIFF && (l = `bottom`);
    let u = l != null,
      d = n.get(m),
      f = de(n.get(g), { bottom: n.get(me), right: n.get(ge) });
    return {
      schemaVersion: 1,
      window: {
        windowId: oe,
        ...
        thread: ke(n),
        panels: {
          browser: je(n, J(n.value) ? ue(n) : null, s, o, d),
          sidebar: { open: r },
          review: {
            open: u,
            placement: l,
            fullscreen: l === `right` && d,
            fileTreeOpen: u && n.get(h),
            view: n.get(V),
          },
        ...
        ...(J(n.value) ? { timeline: Y(S.timelineScroll) } : {}),
      },
    };
  },
});
...
function ke(t) {
  switch (t.value.routeKind) {
    case `local-thread`:
      return {
        id: t.value.conversationId,
        kind: `local`,
        hostId: t.get(e, t.value.conversationId) ?? `local`,
        title: t.get(B, t.value.conversationId),
      };
    case `remote-thread`:
      return { id: t.value.taskId, kind: `remote`, title: null };
    case `chatgpt-thread`:
      return { id: t.value.conversationId, kind: `chatgpt`, title: null };
    case `home`:
    case `new-thread-panel`:
    case `other`:
      return null;
...
function je(e, t, n, r, i) {
  let a = t == null ? [] : fe(e, t),
    o = t == null ? null : pe(t, e.get(g), { bottom: n, right: r }),
    s = o == null ? null : (a.find((e) => e.browserTabId === o) ?? null),
    c = t == null || o == null ? null : L.getSnapshot(t, o);
  return {
...
function Me(e, t, n) {
  if (e == null) return [];
  let r = L.getBrowserUseBrowserTabIds(e),
    i = [...t, ...r],
    a = new Set(r),
    o = new Set();
  return i.flatMap((t) => {
    if (o.has(t)) return [];
    o.add(t);
    let r = L.getSnapshot(e, t);
    return [
      {
        active: t === n,
        browserTabId: t,
        isBrowserUseActive: L.isBrowserUseActive(e, t),
...
function Ne() {
  return Array.from(document.querySelectorAll(ne)).map((e, t) => Pe(e, t));
}
function Pe(e, t) {
  let n = Z(e);
...
function Fe(e) {
  let t = Ie(),
    n = new Set(e.get(U).map((e) => e.path));
  return [
    ...e
      .get(U)
      .map((e) => ({
...
var Le = a({ type: p(`app.help`), action: r().optional() });
function Re(e) {
...
function ze(e, t, n = Be) {
  return {
    ...
    actions: e
      ...
      .map((e) => ({ type: e.type, jsonSchema: JSON.stringify(i(e.schema), null, 2) })),
  };
}
...
Prefer the smallest action that directly satisfies the user request.`,
  Ve = A({
    schema: a({ type: p(`windows.nav.back`), windowId: T }),
    run: () => {
      t.dispatchHostMessage({ type: `navigate-back` });
    },
  }),
  He = A({
    schema: a({ type: p(`windows.nav.forward`), windowId: T }),
    run: () => {
      t.dispatchHostMessage({ type: `navigate-forward` });
    },
  }),
  Q = A({
    schema: a({ type: p(`windows.review.collapse_all`), windowId: T }),
    run: () => {
  ...
  Ue = A({
    schema: a({ type: p(`windows.review.expand_all`), windowId: T }),
    run: () => {
      window.dispatchEvent(
  ...
  We = A({
    ...
    run: ({ path: e, expanded: t }) => {
      let n = O(e).querySelector(S.reviewFileToggle);
      if (n == null) throw Error(`Missing review file toggle: ${e}`);
      n.dataset.appActionReviewFileExpanded !== String(t) && n.click();
    },
  }),
  Ge = A({
    schema: a({ type: p(`windows.review.scroll`), windowId: T, scroll: x }),
    run: ({ scroll: e }) => {
      C(E(S.reviewScroll), e);
    },
  }),
  Ke = A({
    schema: a({
      ...
      windowId: T,
      path: r(),
      align: u([`top`, `center`, `bottom`]).optional(),
    }),
    run: ({ path: e, align: t }, n) => {
      let r = O(e);
      (H(q(n), r.dataset.reviewPath ?? e),
...
var Je = A({
    schema: a({ type: p(`windows.review.set_fullscreen`), windowId: T, fullscreen: c() }),
    run: ({ fullscreen: e }, t) => {
      q(t).set(m, e);
    },
  }),
  Ye = A({
    schema: a({
      ...
      windowId: T,
      view: u([`turn`, `branch`, `unstaged`, `staged`]),
    }),
    run: ({ view: e }, t) => {
      q(t).set(V, e === `turn` ? `last-turn` : e);
    },
  }),
  Xe = A({
    schema: a({ type: p(`windows.review.toggle`), windowId: T }),
    run: () => {
      t.dispatchHostMessage({ type: `toggle-diff-panel` });
    },
  }),
  Ze = A({
    schema: a({ type: p(`windows.show_home`), windowId: T }),
    run: () => {
      t.dispatchHostMessage({ type: `new-chat` });
    },
  }),
  Qe = A({
    schema: a({ type: p(`windows.show_thread`), windowId: T, threadId: r() }),
    run: ({ threadId: e }) => {
      t.dispatchHostMessage({ type: `navigate-to-route`, path: $e(e) });
    },
  });
function $e(e) {
  let t = _e(e);
  return t == null ? d(s(e)) : ve(t.key);
}
var et = [
    Oe,
    ye,
    ...
    A({
      schema: a({ type: p(`windows.sidebar.scroll`), windowId: T, scroll: x }),
      run: ({ scroll: e }) => {
        C(E(S.sidebarScroll), e);
      },
    }),
    A({
      schema: a({
    ...
    A({
      schema: a({ type: p(`windows.sidebar.select_project`), windowId: T, project: w }),
      run: ({ project: e }) => {
        let t = D(e).querySelector(S.sidebarProjectSelect);
        if (t == null) throw Error(`Missing sidebar project select action`);
        t.click();
      },
    }),
    A({
      schema: a({ type: p(`windows.sidebar.toggle`), windowId: T }),
      run: () => {
        t.dispatchHostMessage({ type: `toggle-sidebar` });
      },
    }),
    A({
      schema: a({ type: p(`windows.terminal.toggle`), windowId: T }),
      run: (e, t) => {
        he(q(t));
      },
    }),
    j,
    A({
      schema: a({ type: p(`windows.timeline.scroll`), windowId: T, scroll: x }),
      run: ({ scroll: e }) => {
        se(E(S.timelineScroll), e);
      },
    }),
    A({
      schema: a({ type: p(`windows.timeline.scroll_to_turn`), windowId: T, direction: re }),
      run: ({ direction: e }) => {
        le(E(S.timelineScroll), e);
      },
    }),
  ],
  $ = [Re(nt), ...et],
