...
import "./app-scope.js";
import { D as e, Dn as t, En as n } from "./app-server-manager-signals.js";
...
import { G as g, a as ee, g as _, j as v, o as y, r as b, u as x } from "./app-shell-state.js";
import "./button-DO-oxX3-.js";
import { n as S, s as te } from "./setting-storage.js";
...
import { i as j, n as M, r as N } from "./windows-tabs-open.js";
import { d as P, g as F, l as I, m as L } from "./diff-view-mode.js";
import { n as R, t as z, u as B } from "./app-shell-tab-controller.js";
import { t as V } from "./browser-sidebar-manager.js";
import { t as H } from "./local-conversation-title-signals.js";
import { J as U, Wt as W, i as ue } from "./review-navigation-model.js";
...
var _e = j({
    schema: d({ type: h(`app.appearance.get`) }),
    run: async () => {
      let [e, t, n, r, i] = await Promise.all([
        S(a.theme),
        S(a.lightCodeThemeId),
        S(a.darkCodeThemeId),
        S(a.lightChromeTheme),
        S(a.darkChromeTheme),
      ]);
      return {
        schemaVersion: 1,
  ...
  ve = j({
    schema: d({ type: h(`app.appearance.get_available_themes`) }),
    run: () => ({
      schemaVersion: 1,
      themes: P().map((e) => ({
        id: e.id,
...
async function G(e, t, n, a) {
  ...
  try {
    (await te(t, n), c || e.queryClient?.setQueryData(o, { values: { ...s?.values, [t.key]: n } }));
  } catch (t) {
...
var ye = j({
    schema: d({ type: h(`app.appearance.set_mode`), mode: s([`light`, `dark`, `system`]) }),
    run: async ({ mode: e }, t) => (
      await G(t, a.theme, e, { optimistic: !1 }),
      { schemaVersion: 1, mode: e }
    ),
  }),
  K = m().regex(/^#[0-9a-fA-F]{6}$/),
  be = d({ code: m().nullable().optional(), ui: m().nullable().optional() }),
  xe = d({ diffAdded: K, diffRemoved: K, skill: K }),
  Se = d({
    accent: K.optional(),
    contrast: u().int().min(0).max(100).optional(),
    fonts: be.optional(),
    ink: K.optional(),
    opaqueWindows: l().optional(),
  ...
  Ce = j({
    schema: d({
      ...
      theme: c(`kind`, [
        d({ kind: h(`preset`), themeId: m().refine(L, `Invalid code theme id`) }),
        d({ kind: h(`custom`), patch: Se }),
      ]),
      variant: s([`light`, `dark`, `both`]).default(`both`),
    }),
...
var Ee = j({
  schema: d({ type: h(`app.get_summary`) }),
  run: (e, t) => {
    let n = J(t),
      r = n.get(x),
      i = n.get(ee),
      ...
      s = n.get(b) ? n.get(z.activeTab$) : null,
      c = o?.tabId ?? null,
      l = null;
    c === B.DIFF ? (l = `right`) : s?.tabId === B.DIFF && (l = `bottom`);
    let u = l != null,
      d = n.get(g),
    ...
    return {
      ...
      window: {
        ...
        ...(r ? { sidebar: { viewport: Y(C.sidebarScroll), rows: Ne() } } : {}),
        ...(u ? { review: { viewport: Y(C.reviewScroll), files: Fe(n) } } : {}),
        ...(De(n.value) ? { timeline: Y(C.timelineScroll) } : {}),
      },
    };
  },
});
  ...
  switch (t.value.routeKind) {
    case `local-thread`:
      return {
        ...
        title: t.get(H, t.value.conversationId),
      };
    case `remote-thread`:
...
var Le = d({ type: h(`app.help`), action: m().optional() });
function Re(e) {
  return j({ schema: Le, run: ({ action: t }) => ze(e(), t) });
}
function ze(e, t, n = Be) {
  return {
    ...
    actions: e
      ...
      .map((e) => ({ type: e.type, jsonSchema: JSON.stringify(f(e.schema), null, 2) })),
  };
}
...
Prefer the smallest action that directly satisfies the user request.`,
  Ve = j({
    schema: d({ type: h(`windows.nav.back`), windowId: E }),
    run: () => {
      r.dispatchHostMessage({ type: `navigate-back` });
    },
  }),
  He = j({
    schema: d({ type: h(`windows.nav.forward`), windowId: E }),
    run: () => {
      r.dispatchHostMessage({ type: `navigate-forward` });
    },
  }),
  Q = j({
    schema: d({ type: h(`windows.review.collapse_all`), windowId: E }),
    run: () => {
  ...
  Ue = j({
    schema: d({ type: h(`windows.review.expand_all`), windowId: E }),
    run: () => {
      window.dispatchEvent(
  ...
  We = j({
    ...
    run: ({ path: e, expanded: t }) => {
      let n = A(e).querySelector(C.reviewFileToggle);
      if (n == null) throw Error(`Missing review file toggle: ${e}`);
      n.dataset.appActionReviewFileExpanded !== String(t) && n.click();
    },
  }),
  Ge = j({
    schema: d({ type: h(`windows.review.scroll`), windowId: E, scroll: T }),
    run: ({ scroll: e }) => {
      k(O(C.reviewScroll), e);
    },
  }),
  Ke = j({
    schema: d({
      ...
      windowId: E,
      path: m(),
      align: s([`top`, `center`, `bottom`]).optional(),
    }),
    run: ({ path: e, align: t }, n) => {
      let r = A(e);
      (ue(J(n), r.dataset.reviewPath ?? e),
...
var Je = j({
    schema: d({ type: h(`windows.review.set_fullscreen`), windowId: E, fullscreen: l() }),
    run: ({ fullscreen: e }, t) => {
      J(t).set(g, e);
    },
  }),
  Ye = j({
    schema: d({
      ...
      windowId: E,
      view: s([`turn`, `branch`, `unstaged`, `staged`]),
    }),
    run: ({ view: e }, t) => {
  ...
  Xe = j({
    schema: d({ type: h(`windows.review.toggle`), windowId: E }),
    run: () => {
      r.dispatchHostMessage({ type: `toggle-diff-panel` });
    },
  }),
  Ze = j({
    schema: d({ type: h(`windows.show_home`), windowId: E }),
    run: () => {
      r.dispatchHostMessage({ type: `new-chat` });
    },
  }),
  Qe = j({
    schema: d({ type: h(`windows.show_thread`), windowId: E, threadId: m() }),
    run: ({ threadId: e }) => {
...
var et = [
    ...
    j({
      schema: d({ type: h(`windows.sidebar.scroll`), windowId: E, scroll: T }),
      run: ({ scroll: e }) => {
        k(O(C.sidebarScroll), e);
      },
    }),
    j({
      schema: d({
    ...
    j({
      schema: d({ type: h(`windows.sidebar.select_project`), windowId: E, project: w }),
      run: ({ project: e }) => {
        let t = D(e).querySelector(C.sidebarProjectSelect);
        if (t == null) throw Error(`Missing sidebar project select action`);
        t.click();
      },
    }),
    j({
      schema: d({ type: h(`windows.sidebar.toggle`), windowId: E }),
      run: () => {
        r.dispatchHostMessage({ type: `toggle-sidebar` });
      },
    }),
    j({
      schema: d({ type: h(`windows.terminal.toggle`), windowId: E }),
      run: (e, t) => {
        he(J(t));
      },
    }),
    M,
    j({
      schema: d({ type: h(`windows.timeline.scroll`), windowId: E, scroll: T }),
      run: ({ scroll: e }) => {
        ae(O(C.timelineScroll), e);
      },
    }),
    j({
      schema: d({ type: h(`windows.timeline.scroll_to_turn`), windowId: E, direction: le }),
      run: ({ direction: e }) => {
        ie(O(C.timelineScroll), e);
      },
    }),
  ],
  $ = [Re(nt), ...et],
