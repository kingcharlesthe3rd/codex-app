...
import { Bt as r, o as i, r as a } from "./use-host-config-C.js";
import { f as o, n as s } from "./vscode-api.js";
import { r as c } from "./toast-signal.js";
import { N as l } from "./rpc-1.js";
import { r as u } from "./base64.js";
import { t as d } from "./v4.js";
import { t as f } from "./app-intl-signal.js";
var p = 4e3;
async function m({
  scope: e,
...
async function h({ scope: e, conversationId: t, hostId: n, status: i }) {
  try {
    return (await r(`set-thread-goal-status`, { conversationId: t, hostId: n, status: i }), !0);
  } catch {
    return (
      e
        .get(c)
...
async function g({ scope: e, conversationId: t, hostId: n, intl: i }) {
  try {
    return (await r(`clear-thread-goal`, { conversationId: t, hostId: n }), !0);
  } catch {
    return (
      e
        .get(c)
...
var _ = `Read the Codex goal objective file at `,
  v = ` before continuing.`,
  y = `goal-objective.md`,
  b = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
async function x({ draft: e, hostId: t }) {
  let n = e.objective.trim(),
    i = null;
  try {
    let a = await Promise.all(
        e.pastedTextAttachments.map(async (e, n) => ({
          contentsBase64: await k(e.file.fsPath, e.hostId ?? t),
    ...
    for (let [n, r] of e.imageAttachments.entries()) {
      let e = n + 1;
      if (te(r.src)) s.push({ position: e, url: r.src });
      else {
        let n = await A(r, t);
        o.push({ contentsBase64: n, filename: `image-${e}.${j(r)}`, position: e });
      }
    }
    if (n.length === 0 && a.length === 0 && o.length === 0 && s.length === 0)
      throw Error(`Goal objective must not be empty`);
    let c = async () => {
        if (i != null) return i;
        let e = await r(`create-thread-goal-attachment-directory-for-host`, { hostId: t });
        return ((i = e.path), e.path);
      },
      d = async ({ contentsBase64: e, filename: n }) =>
        r(`write-thread-goal-attachment-for-host`, {
          contentsBase64: e,
      ...
      f = [];
    for (let e of a) {
      let t = await d({ contentsBase64: e.contentsBase64, filename: e.filename });
      f.push(`- pasted text file: ${t.path}. Read this file before continuing.`);
    }
    n = M(n, `Referenced pasted text files:`, f);
    let p = [];
    for (let e of o) {
      let t = await d({ contentsBase64: e.contentsBase64, filename: e.filename });
      p.push(`- [Image #${e.position}]: ${t.path}`);
    }
    if (
      ((n = M(n, `Referenced image files:`, p)),
      (n = M(
        n,
      ...
      return { objective: n, attachmentDirectory: i };
    let m = ee(l(await c(), y));
    return (
      await d({ contentsBase64: u(n), filename: y }), { objective: m, attachmentDirectory: i }
    );
  } catch (e) {
    throw (i != null && (await O(t, i)), e);
  }
}
async function S({ hostId: e, materialized: t }) {
  t.attachmentDirectory != null && (await O(e, t.attachmentDirectory));
}
async function C({ draft: e, fallbackHostId: t }) {
  await Promise.allSettled(
    e.pastedTextAttachments.map((e) =>
      r(`remove-pasted-text-attachment-for-host`, { hostId: e.hostId ?? t, path: e.file.path }),
    ),
  );
}
async function w({ hostId: e, objective: t }) {
  let n = E(t);
  if (n == null) return null;
  let { codexHome: r } = await s(`codex-home`, { params: { hostId: e } });
  return D(n, r) ? n : null;
}
async function T({ hostId: e, objective: t }) {
  let n = await w({ hostId: e, objective: t });
  if (n == null) return t;
  let { contents: r } = await s(`read-file`, { params: { hostId: e, path: n } });
  return r;
}
function E(e) {
  if (!e.startsWith(_) || !e.endsWith(v)) return null;
  let t = e.slice(38, -19);
...
function ee(e) {
  ...
  if (Array.from(t).length > 4e3)
    throw Error(`Goal objective file reference exceeds ${p} characters`);
  return t;
}
function D(e, t) {
  let n = e.split(/[\\/]/u).filter(Boolean).at(-2);
  return n == null || !b.test(n) ? !1 : e === l(t, `attachments`, n, y);
}
async function O(e, t) {
  await r(`remove-thread-goal-attachment-directory-for-host`, { hostId: e, path: t }).catch(
    () => void 0,
  );
}
...
async function A(e, t) {
  if (e.src.startsWith(`data:`)) {
    ...
    return t[1] == null ? u(decodeURIComponent(t[2] ?? ``)) : (t[2] ?? ``);
  }
  let n = e.localPath ?? e.src.replace(/^file:\/\//i, ``);
  return k(e.localPath == null ? decodeURIComponent(n) : n, e.localPath == null ? a : t);
}
function j(e) {
...
var N = e(t(), 1),
  ...
  I = [];
function L() {
  return (0, N.useSyncExternalStore)(B, V);
}
function R() {
  return {
    addPendingWorktreeConversationStart: z,
    beginPendingWorktreeConversationStart: (e) =>
      F.get(e)?.state === `waiting` ? (F.set(e, { state: `starting` }), H(), !0) : !1,
    failPendingWorktreeConversationStart: (e) => {
      F.has(e) && (F.set(e, { state: `failed` }), H());
    },
    succeedPendingWorktreeConversationStart: (e, t) => {
      F.has(e) && (F.set(e, { state: `succeeded`, conversationId: t }), H());
    },
    retryPendingWorktreeConversationStart: (e) => {
      F.has(e) && (F.set(e, { state: `waiting` }), H());
    },
...
function H() {
  ((I = Array.from(F, ([e, t]) => ({ pendingWorktreeId: e, ...t }))),
    P.forEach((e) => {
      e();
    }));
}
...
function K(e) {
  let t = e.lastIndexOf(`:`);
  return t <= 0 || t === e.length - 1 ? a : e.slice(0, t);
}
function q(e) {
  switch (e) {
    case `queued`:
...
function J() {
  let [e] = i(`pending_worktrees`);
  return e ?? W;
}
function Y(e) {
  let t = (0, U.c)(3),
    [n] = i(`pending_worktrees`);
  if (!e) return null;
  if (n === void 0) return;
  let r;
  return (
...
function X() {
  let e = (0, U.c)(32),
    [t, n] = i(`pending_worktrees`),
    { removePendingWorktreeConversationStart: r, retryPendingWorktreeConversationStart: a } = R(),
    s;
  e[0] === n
    ? (s = e[1])
    : ((s = (e) => {
        ...
        return window.electronBridge == null
          ? (r.launchMode !== `create-stable-worktree` && z(t),
            n((e) => [...(e === void 0 ? [] : e), r]),
            t)
          : $(t, e);
      }),
    ...
    : ((l = (e, t) => {
        if (t.length !== 0) {
          if (window.electronBridge == null) {
            n((n) => (n === void 0 ? [] : n).map((n) => (n.id === e ? re(n, t) : n)));
            return;
          }
          t.forEach((t) => {
  ...
  e[8] === u
    ? (h = e[9])
    : ((h = (e, t) => {
        u(e, [{ type: `pinnedBeforeThreadId`, beforeThreadId: t }]);
      }),
      (e[8] = u),
      (e[9] = h));
  let g = h,
  ...
  e[10] === u
    ? (_ = e[11])
    : ((_ = (e) => {
        u(e, [{ type: `needsAttention`, needsAttention: !1 }]);
      }),
      (e[10] = u),
      (e[11] = _));
  let v = _,
  ...
  e[12] !== a || e[13] !== n
    ? ((y = (e) => {
        if ((a(e), window.electronBridge == null)) {
          n((t) =>
            (t === void 0 ? [] : t).map((t) =>
              t.id === e
                ? {
        ...
        o.dispatchMessage(`pending-worktree-retry`, { hostId: K(e), id: e });
      }),
      (e[12] = a),
      (e[13] = n),
      (e[14] = y))
  ...
  e[15] === n
    ? (x = e[16])
    : ((x = (e) => {
        if ((z(e), window.electronBridge == null)) {
          n((t) =>
            (t === void 0 ? [] : t).map((t) =>
              t.id === e &&
              t.phase === `failed` &&
        ...
        o.dispatchMessage(`pending-worktree-continue`, { hostId: K(e), id: e });
      }),
      (e[15] = n),
      (e[16] = x));
  let S = x,
  ...
  if (e[17] !== t || e[18] !== r || e[19] !== n) {
    ((w = (e) => {
      ...
      o.dispatchMessage(`pending-worktree-cancel`, { hostId: K(e), id: e });
    }),
      (T = (e) => {
        if ((r(e), i(e), window.electronBridge == null)) {
          n((t) => (t === void 0 ? [] : t).filter((t) => t.id !== e));
        ...
        o.dispatchMessage(`pending-worktree-dismiss`, { hostId: K(e), id: e });
      }));
    function i(e) {
      let n = t?.find((t) => t.id === e);
      n?.launchMode !== `start-conversation` ||
        n.threadGoalDraft == null ||
        C({ draft: n.threadGoalDraft, fallbackHostId: n.hostId });
    }
    ((e[17] = t), (e[18] = r), (e[19] = n), (e[20] = w), (e[21] = T));
  } else ((w = e[20]), (T = e[21]));
  let E;
  return (
    e[22] !== w ||
    ...
    e[30] !== g
      ? ((E = {
          ...
          cancelPendingWorktree: w,
          dismissPendingWorktree: T,
        }),
        (e[22] = w),
        (e[23] = v),
        (e[24] = S),
...
function $(e, t) {
  ...
  return (
    Q(e, t).launchMode !== `create-stable-worktree` && z(e),
    o.dispatchMessage(`pending-worktree-create`, { hostId: t.hostId, request: ne(e, t) }),
    e
  );
}
function ne(e, t) {
