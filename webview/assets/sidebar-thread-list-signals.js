import { Hr as e, Qr as t, r as n } from "./src-2.js";
import {
  Cn as r,
  bn as i,
  gn as a,
...
import { t as ee } from "./jsx-runtime.js";
import { L as p, R as m, a as h, h as g, u as _, z as v } from "./vscode-api.js";
import { o as y } from "./lib-2.js";
import { t as te } from "./app-intl-signal-B.js";
import { d as ne, g as b, l as x, v as S } from "./sidebar-project-group-signals.js";
...
function j({ chatLabel: e, task: t, projectLabelByThreadKey: n, projectlessThreadIds: r }) {
  ...
  if (
    ...
    return e;
  if (t.kind === `local`) return (t.conversation.cwd ? u(t.conversation.cwd) : ``) || e;
...
var F = [],
  ...
  B = p(g, ({ get: t }) => {
    let n = s(t, e.THREAD_PROJECT_ASSIGNMENTS),
      r = s(t, e.PROJECTLESS_THREAD_IDS);
    return P(
      t(x, { threadKeys: t(b), enabled: !0, threadProjectAssignments: n, projectlessThreadIds: r })
        .groups,
    );
  }),
  V = m(g, (e, { get: t }) => {
  ...
  W = m(g, (t, { get: n }) => {
    ...
    let i = s(n, e.PROJECTLESS_THREAD_IDS);
    return j({
  ...
  G = m(g, (t, { get: r }) => {
    ...
    return i?.kind === `local`
      ? n({
          ...
          assignment: s(r, e.THREAD_PROJECT_ASSIGNMENTS)?.[i.conversation.id],
        })
      : null;
  }),
  K = m(g, (t, { get: n }) =>
    t == null ? null : (s(n, e.THREAD_WORKSPACE_ROOT_HINTS)?.[t] ?? null),
  ),
  q = m(g, (t, { get: n }) =>
    t == null ? null : (s(n, e.SIDEBAR_THREAD_METADATA)?.[t]?.labelColor ?? null),
  );
function J(e) {
  let t = new Set(),
    n = new Map();
...
function $({ get: e, threadKeys: n, pinnedThreadIds: o }) {
  if (n.length === 0) return Z;
  let s = new Set(n),
    c = [];
  for (let t of n) r(t)?.kind === `pending-worktree` && e(V, t) && c.push(t);
  for (let e of o) {
    let n = a(t(e));
    if (s.has(n)) {
      c.push(n);
