...
import { $ as m, Q as h, S as g, U as _, _ as v, et as y, f as ee } from "./setting-storage.js";
import { t as te } from "./app-intl-signal.js";
import { d as b, g as x, l as S, v as C } from "./sidebar-project-group-signals.js";
import { t as w } from "./pinned-threads-query.js";
var T = p();
...
function ne({ projectSidebarEnabled: e, organizeMode: t, showProjectsTitle: n, showRecent: r }) {
  return e
    ? r
      ? `All chats`
      : t === `connection`
...
function j({ chatLabel: e, task: t, projectLabelByThreadKey: n, projectlessThreadIds: i }) {
  let a = n.get(t.key)?.trim();
  if (a) return a;
  if (
    (t.kind === `local` &&
      (t.conversation.workspaceKind === `projectless` || i?.has(t.conversation.id) === !0)) ||
    (t.kind === `remote` && i?.has(t.task.id) === !0)
  )
    return e;
  if (t.kind === `local`) return (t.conversation.cwd ? r(t.conversation.cwd) : ``) || e;
...
var F = [],
  ...
  B = h(g, ({ get: t }) => {
    ...
    return P(
      t(S, { threadKeys: t(x), enabled: !0, threadProjectAssignments: n, projectlessThreadIds: r })
        .groups,
    );
  }),
  V = m(g, (e, { get: t }) => {
  ...
  q = m(g, (t, { get: n }) =>
    t == null ? null : (a(n, e.SIDEBAR_THREAD_METADATA)?.[t]?.labelColor ?? null),
  );
function J(e) {
  let t = new Set(),
    n = new Map();
...
function $({ get: e, threadKeys: t, pinnedThreadIds: r }) {
  ...
  return { allSidebarThreadKeys: [...a, ...f], pinnedThreadKeys: a, unpinnedThreadKeys: f };
}
export {
  ie as S,
  k as _,
