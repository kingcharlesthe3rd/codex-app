import { a as e } from "./skus.js";
var t = 1440,
  n = 7 * t,
  r = 30 * t,
  i = 365 * t,
  a = 60 * t,
  o = 90;
function s(e) {
  return e != null && (e.windowDurationMins ?? 0) > 0;
}
function c(e, t = new Date()) {
  let n = _(e);
  return n ? Math.floor((n.getTime() - t.getTime()) / 1e3) : null;
}
function l(e) {
  let { primary: t, secondary: n } = e ?? { primary: null, secondary: null },
    r = t?.usedPercent ?? 0,
    i = n?.usedPercent ?? 0,
    a = t?.windowDurationMins ?? 0,
    s = n?.windowDurationMins ?? 0,
    c = Math.max(r, i);
  return c < o
    ? null
    : i > r || (r === i && s > a)
      ? {
          severity: c >= 100 ? `blocked` : `warning`,
          usedPercent: i,
          remainingPercent: u(i),
          windowMinutes: s,
          resetsAt: n?.resetsAt ?? null,
        }
      : c === r
        ? {
            severity: c >= 100 ? `blocked` : `warning`,
            usedPercent: r,
            remainingPercent: u(r),
            windowMinutes: a,
            resetsAt: t?.resetsAt ?? null,
          }
        : null;
}
function u(e) {
  return Number.isFinite(e) ? Math.min(Math.max(100 - e, 0), 100) : 100;
}
function d(e) {
  if (!Number.isFinite(e)) return `–`;
  let t = Math.min(Math.max(e, 0), 100);
  return `${Math.round(t)}%`;
}
function f({ intl: e, minutes: a, variant: o = `summary` }) {
  let s = a ?? 0,
    c = p(s, i);
  if (c != null)
    return o === `summary`
      ? e.formatMessage(
          {
            id: `composer.mode.rateLimit.annualDynamicTitle`,
            defaultMessage: `{years, plural, one {Annual} other {{years} Years}}`,
            description: `Dynamic annual rate limit label (title case)`,
          },
          { years: c },
        )
      : e.formatMessage(
          {
            id: `composer.mode.rateLimit.annualDynamicSentence`,
            defaultMessage: `{years, plural, one {annual limit} other {{years} year limit}}`,
            description: `Dynamic annual rate limit label (sentence case)`,
          },
          { years: c },
        );
  let l = p(s, r);
  if (l != null)
    return o === `summary`
      ? e.formatMessage(
          {
            id: `composer.mode.rateLimit.monthlyDynamicTitle`,
            defaultMessage: `{months, plural, one {Monthly} other {{months} Months}}`,
            description: `Dynamic monthly rate limit label (title case)`,
          },
          { months: l },
        )
      : e.formatMessage(
          {
            id: `composer.mode.rateLimit.monthlyDynamicSentence`,
            defaultMessage: `{months, plural, one {monthly limit} other {{months} month limit}}`,
            description: `Dynamic monthly rate limit label (sentence case)`,
          },
          { months: l },
        );
  if (s >= 10079) {
    let t = Math.ceil(s / n);
    return o === `summary`
      ? e.formatMessage(
          {
            id: `composer.mode.rateLimit.weeklyDynamicTitle`,
            defaultMessage: `{weeks, plural, one {Weekly} other {{weeks} Weeks}}`,
            description: `Dynamic weekly rate limit label (title case)`,
          },
          { weeks: t },
        )
      : e.formatMessage(
          {
            id: `composer.mode.rateLimit.weeklyDynamicSentence`,
            defaultMessage: `{weeks, plural, one {weekly limit} other {{weeks} week limit}}`,
            description: `Dynamic weekly rate limit label (sentence case)`,
          },
          { weeks: t },
        );
  }
  if (s >= 1439) {
    let n = Math.ceil(s / t);
    return e.formatMessage(
      {
        id: `composer.mode.rateLimit.day`,
        defaultMessage: `{days}d`,
        description: `Daily rate limit label`,
      },
      { days: n },
    );
  }
  if (s >= 60) {
    let t = Math.ceil(s / 60);
    return e.formatMessage(
      {
        id: `composer.mode.rateLimit.hour`,
        defaultMessage: `{hours}h`,
        description: `Hourly rate limit label`,
      },
      { hours: t },
    );
  }
  if (s > 0) {
    let t = Math.ceil(s);
    return e.formatMessage(
      {
        id: `composer.mode.rateLimit.minute`,
        defaultMessage: `{minutes}m`,
        description: `Minute rate limit label`,
      },
      { minutes: t },
    );
  }
  return e.formatMessage({
    id: `composer.mode.rateLimit.heading`,
    defaultMessage: `Usage remaining`,
    description: `Rate limit summary heading`,
  });
}
function p(e, t) {
  if (!Number.isFinite(e) || e <= 0) return null;
  let n = Math.max(1, Math.round(e / t));
  return m(e, n * t) ? n : null;
}
function m(e, t) {
  return e >= t * 0.95 && e <= t * 1.05;
}
function h(e, t = new Date()) {
  let n = Math.floor((e.getTime() - t.getTime()) / 1e3);
  return n <= 0
    ? new Intl.RelativeTimeFormat(void 0, { numeric: `auto` }).format(0, `second`)
    : n < a
      ? new Intl.DateTimeFormat(void 0, { timeStyle: `short` }).format(e)
      : new Intl.DateTimeFormat(void 0, { month: `short`, day: `numeric` }).format(e);
}
function g(e) {
  return {
    date: new Intl.DateTimeFormat(void 0, { month: `short`, day: `numeric` }).format(e),
    time: new Intl.DateTimeFormat(void 0, { timeStyle: `short` }).format(e),
  };
}
function _(e) {
  if (e == null || !Number.isFinite(e)) return null;
  let t = e * 1e3;
  return Number.isFinite(t) ? new Date(t) : null;
}
function v(e) {
  return e?.rate_limit ?? null;
}
function y(e) {
  return e?.credits ?? null;
}
function b(e) {
  let t = v(e),
    n = [];
  return (
    t?.primary_window && n.push(t.primary_window),
    t?.secondary_window && n.push(t.secondary_window),
    n
  );
}
function x(e) {
  let t = b(e);
  return t.length === 0
    ? null
    : t.reduce((e, t) =>
        !e || t.used_percent > e.used_percent
          ? t
          : t.used_percent < e.used_percent
            ? e
            : t.reset_at > e.reset_at
              ? t
              : (t.reset_at, e.reset_at, e),
      );
}
function S(e) {
  return x(e)?.reset_at ?? null;
}
function C(e) {
  if (F(e)) return null;
  let t = e?.spend_control?.individual_limit?.used_percent;
  return t == null || t < 70 ? null : t >= 85 ? 85 : 70;
}
function w(e) {
  let t = I(e?.effective_monthly_limit?.limit),
    n = I(e?.current_month_usage);
  if (t == null || n == null || t < 0) return null;
  let r = t === 0 ? 100 : Math.min(Math.max((n / t) * 100, 0), 100);
  return { limit: t, used: n, usedPercent: r, remainingPercent: u(r) };
}
function T(e) {
  return e?.effective_monthly_limit?.enforcement_mode === `HARD_CAP`
    ? I(e.effective_monthly_limit.limit)
    : null;
}
function E(e) {
  let t = T(e),
    n = I(e?.current_month_usage);
  return t != null && n != null && n >= t;
}
function D(e, t) {
  return !F(e) || !E(t) ? !1 : k(e, t);
}
function O(e, t) {
  return C(e) != null && k(e, t);
}
function k(e, t) {
  let n = T(t),
    r = I(e?.spend_control?.individual_limit?.limit ?? null);
  return n != null && (r == null || r === n);
}
function A(t) {
  if (t?.rate_limit_reached_type != null) return !0;
  let n = v(t),
    r = e(t?.plan_type) && t?.credits?.unlimited === !1 && t?.credits?.has_credits === !1;
  return !!(n?.limit_reached === !0 || n?.allowed === !1 || r || F(t));
}
function j({ rateLimitStatus: e, isWorkspaceAccount: t }) {
  let n = e?.rate_limit_reached_type?.type,
    r = n != null && n !== `rate_limit_reached`;
  return t && P(e) && !F(e) && !r;
}
function M({ rateLimitStatus: e, isWorkspaceAccount: t }) {
  return t && F(e);
}
function N({ rateLimitStatus: e, isWorkspaceAccount: t, isCreditsEnabled: n }) {
  let r = !t && P(e);
  return (
    n &&
    !M({ rateLimitStatus: e, isWorkspaceAccount: t }) &&
    A(e) &&
    !r &&
    !j({ rateLimitStatus: e, isWorkspaceAccount: t })
  );
}
function P(e) {
  let t = y(e);
  return t?.unlimited === !0 || t?.has_credits === !0;
}
function F(e) {
  return e?.spend_control?.reached === !0;
}
function I(e) {
  if (e == null) return null;
  let t = typeof e == `number` ? e : Number(e);
  return Number.isFinite(t) ? t : null;
}
function L(e) {
  let t = e?.rate_limit;
  return t?.limit_reached === !0 || t?.allowed === !1;
}
function R(e, t) {
  let n = new Date(),
    r = new Date(t * 1e3);
  return r.getFullYear() === n.getFullYear() &&
    r.getMonth() === n.getMonth() &&
    r.getDate() === n.getDate()
    ? r.toLocaleTimeString(e.locale, { timeStyle: `short` })
    : r.toLocaleString(e.locale, { timeStyle: `short`, dateStyle: `medium` });
}
export {
  c as C,
  u as S,
  _ as T,
  d as _,
  S as a,
  h as b,
  L as c,
  N as d,
  j as f,
  i as g,
  n as h,
  C as i,
  A as l,
  r as m,
  O as n,
  w as o,
  t as p,
  R as r,
  P as s,
  D as t,
  M as u,
  f as v,
  s as w,
  l as x,
  g as y,
};
//# sourceMappingURL=rate-limit-status.js.map
