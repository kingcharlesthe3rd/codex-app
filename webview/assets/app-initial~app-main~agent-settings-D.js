...
import { Ka as l, qa as u } from "./app-initial~app-main~onboarding-page.js";
var d,
  f,
  p = e(() => {
...
function h({ diagnostics: e, durationMs: t }) {
  return {
    durationMs: t,
    problemCount: e.problems.length,
    status: e.installed
      ? o.CODEX_PRIMARY_RUNTIME_DEPENDENCIES_DIAGNOSE_STATUS_OK
      : o.CODEX_PRIMARY_RUNTIME_DEPENDENCIES_DIAGNOSE_STATUS_PROBLEM,
    ...x(e.bundleVersion),
  };
}
function g({ durationMs: e }) {
  return { durationMs: e, status: o.CODEX_PRIMARY_RUNTIME_DEPENDENCIES_DIAGNOSE_STATUS_FAILED };
}
...
function v(e) {
  switch (e) {
    case `latest`:
      return s.CODEX_PRIMARY_RUNTIME_RELEASE_LATEST;
    case `latest-alpha`:
      return s.CODEX_PRIMARY_RUNTIME_RELEASE_LATEST_ALPHA;
  }
}
function y(e) {
  switch (e) {
    case `already-current`:
      return c.CODEX_PRIMARY_RUNTIME_INSTALL_RESULT_STATUS_ALREADY_CURRENT;
    case `canceled`:
      return c.CODEX_PRIMARY_RUNTIME_INSTALL_RESULT_STATUS_CANCELED;
    case `failed`:
      return c.CODEX_PRIMARY_RUNTIME_INSTALL_RESULT_STATUS_FAILED;
    case `installed`:
      return c.CODEX_PRIMARY_RUNTIME_INSTALL_RESULT_STATUS_INSTALLED;
  }
}
function b(e) {
  switch (e) {
    case `already-current`:
      return i.CODEX_PRIMARY_RUNTIME_DEPENDENCIES_RESET_STATUS_ALREADY_CURRENT;
    case `canceled`:
      return i.CODEX_PRIMARY_RUNTIME_DEPENDENCIES_RESET_STATUS_CANCELED;
    case `failed`:
      return i.CODEX_PRIMARY_RUNTIME_DEPENDENCIES_RESET_STATUS_FAILED;
    case `installed`:
      return i.CODEX_PRIMARY_RUNTIME_DEPENDENCIES_RESET_STATUS_INSTALLED;
  }
}
function x(e) {
...
async function C({ formatMessage: e, hostId: n, productLogger: r, release: i, toast: o }) {
  let s = Date.now(),
    c = o.info(
      e({
  ...
  try {
    let t = await u({ hostId: n, release: i, request: `install` });
    if (
      (r.logProductEvent(
        a,
        m({
    ...
    if (w(n)) {
      (r.logProductEvent(
        a,
        m({ bundleVersion: null, durationMs: Date.now() - s, release: i, status: `canceled` }),
      ),
        o.info(
          e({
    ...
    (t.error(`Error installing primary runtime`, { safe: { release: i }, sensitive: { error: n } }),
      r.logProductEvent(
        a,
        m({ bundleVersion: null, durationMs: Date.now() - s, release: i, status: `failed` }),
      ),
      o.danger(
        e({
...
export { g as a, f as c, h as i, d as l, w as n, _ as o, C as r, S as s, T as t, p as u };
//# sourceMappingURL=app-initial~app-main~agent-settings-D.js.map
