...
import { t as h } from "./button-oF-qgtAS.js";
import { t as g } from "./spinner.js";
import { t as _ } from "./tooltip-B.js";
import { n as ee, r as te, t as v } from "./speaker.js";
import { r as ne } from "./use-recording-waveform-Cfd-Ecbb.js";
import { t as y } from "./x.js";
import { t as b } from "./waveform.js";
import { t as x } from "./get-resize-observer-entry-size-DrKQ-0pS.js";
import { t as S } from "./permission-request-model.js";
function C({ axis: e, debounce: t, initialSize: n = !0, onChange: r, target: i }) {
...
function k(
  e,
  t,
  { includeMcpElicitationCancelAction: n = !1, planStartCollaborationMode: r } = {},
) {
  switch (e?.type) {
    case `userInput`:
      return e.isOnboardingDynamicInput === !0 ? null : M(e, t);
    case `approval`:
      return e.item.type === `exec` ? N(e.item, t) : P(e.item, t);
...
function A(e, t) {
  switch (e.kind) {
    ...
    case `patch`:
      return [
        t.formatMessage(O.fileCount, { count: e.fileCount }),
...
function M(e, t) {
  ...
  let r = G(n.question) ?? G(n.header);
  return r == null || n.options.length === 0
    ? null
    : {
        kind: `question`,
        requestId: e.item.requestId,
        title: t.formatMessage(O.askQuestion),
        prompt: r,
        options: n.options.map((e) => ({
          label: G(e.label) ?? t.formatMessage(O.option),
...
function N(e, t) {
  ...
  let n = G(e.networkApprovalContext?.host ?? ``);
  return n == null
    ? {
        kind: `exec`,
        requestId: e.approvalRequestId,
        operation: t.formatMessage(O.runCommand),
        summary: G(e.cmd.join(` && `)) ?? t.formatMessage(O.command),
        title: t.formatMessage(O.reviewCommand),
        actions: [
          {
            commandDecision: `accept`,
            intent: `command-approval`,
            label: t.formatMessage(O.runOnce),
            tone: `primary`,
          },
          {
            commandDecision: `decline`,
            intent: `command-approval`,
            label: t.formatMessage(O.deny),
            tone: `danger`,
          },
        ],
      }
    : {
        kind: `network`,
        requestId: e.approvalRequestId,
        operation: t.formatMessage(O.allowNetwork),
        target: n,
        title: t.formatMessage(O.allowNetwork),
        actions: [
          {
            ariaLabel: t.formatMessage(O.allowOnce),
            commandDecision: `accept`,
            intent: `command-approval`,
            label: t.formatMessage(O.allow),
            tone: `primary`,
          },
          {
            commandDecision: `decline`,
...
function P(e, t) {
  ...
  return {
    kind: `patch`,
    requestId: e.approvalRequestId,
    operation: t.formatMessage(O.applyChanges),
    summary: n[0]?.[0] ?? t.formatMessage(O.oneFileChanged),
    title: t.formatMessage(O.applyChanges),
    files: n.map(([e]) => e),
    fileCount: n.length,
    ...
    actions: [
      {
        ...
        label: t.formatMessage(O.apply),
        tone: `primary`,
      },
      { intent: `open`, label: t.formatMessage(O.review), tone: `secondary` },
    ],
  };
}
function F(e, t) {
  ...
  let i, a;
  if (r.kind === `network`) ((a = t.formatMessage(O.networkAccess)), (i = a));
  else {
    a = t.formatMessage(O.fileAccess);
    let e = r.paths[0] ?? a,
    ...
    actions: [
      {
        ...
        ariaLabel: t.formatMessage(O.allowOnce),
        label: t.formatMessage(O.allow),
        permissionResponse: { permissions: R(n), scope: `turn` },
        tone: `primary`,
      },
      {
        intent: `permission-response`,
...
function I(e, t, n) {
  ...
  return i == null
    ...
    : {
        kind: `plan`,
        operation: t.formatMessage(O.plan),
        planContent: r,
        summary: i,
        title: t.formatMessage(O.plan),
        turnId: e.turnId,
        actions: [
          {
            intent: `plan-start`,
            label: t.formatMessage(O.implementPlan),
            ...(n == null ? {} : { planStartCollaborationMode: n }),
            tone: `primary`,
          },
        ],
      };
...
function L(e, t, n) {
  let { elicitation: r } = e,
    i = e.request.params,
    a = G(i.serverName),
    o = U(a) ?? a ?? t.formatMessage(O.toolServer),
    s,
    c = o,
    l = null,
    u = null;
  switch (r.kind) {
    case `formElicitation`:
      ((s = t.formatMessage(O.answerTarget, { target: o })), (l = G(r.message)), (u = []));
      break;
    case `mcpToolCall`: {
      let e = G(r.approval.connector_name) ?? U(r.approval.connector_id) ?? o,
        n = z(r.message);
      ((s =
        n == null
          ? t.formatMessage(O.allowTarget, { target: e })
          : t.formatMessage(O.allowTarget, { target: n })),
        (c = e),
        (l = G(r.subtitle) ?? B(r.approval.tool_params) ?? G(r.message)));
      break;
    }
    case `toolSuggestion`: {
      let e = G(r.suggestion.tool_name) ?? U(r.suggestion.tool_id) ?? t.formatMessage(O.tool);
      ((s =
        r.suggestion.suggest_type === `enable`
          ? t.formatMessage(O.enableTool, { toolName: e })
          : t.formatMessage(O.installTool, { toolName: e })),
        (c =
          r.suggestion.tool_type === `connector`
            ? t.formatMessage(O.connector)
            : t.formatMessage(O.plugin)),
        (l = G(r.suggestion.suggest_reason)));
      break;
    }
    case `connectorAuth`: {
      let e = G(r.connector.connector_name) ?? U(r.connector.connector_id) ?? o,
        n = r.connector.auth_reason;
      ((s =
        n === `missing_link`
          ? t.formatMessage(O.connectTarget, { target: e })
          : n === `oauth_upgrade_required`
            ? t.formatMessage(O.additionalAccessTarget, { target: e })
            : t.formatMessage(O.reconnectTarget, { target: e })),
        (c = e),
        (l = G(r.subtitle) ?? G(r.message) ?? V(r.url)),
        (u = [
          {
            intent: `open`,
            label:
              n === `missing_link`
                ? t.formatMessage(O.connect)
                : n === `oauth_upgrade_required`
                  ? t.formatMessage(O.updateAccess)
                  : t.formatMessage(O.reconnect),
            tone: `primary`,
          },
        ]));
      break;
    ...
    case `urlAction`:
      ((c = U(r.serverName) ?? G(r.serverName) ?? o),
        (s = t.formatMessage(O.openLink)),
        (l = G(r.subtitle) ?? V(r.url) ?? G(r.message)),
        (u = [{ intent: `open`, label: t.formatMessage(O.openLink), tone: `primary` }]));
      break;
    case `generic`:
      ((s =
        re({ message: r.message, metadata: r.metadata }) ??
        t.formatMessage(O.allowTarget, { target: o })),
        (l = G(r.subtitle) ?? B(r.toolParams) ?? G(r.message)));
      break;
  }
  r.riskLevel === `high` &&
    (u = [{ intent: `open`, label: t.formatMessage(O.review), tone: `primary` }]);
  let d = {
    ariaLabel: t.formatMessage(O.allowTarget, { target: c }),
    intent: `mcp-elicitation`,
    label: t.formatMessage(O.allowTarget, { target: c }),
    ...
    actions:
      ...
      (n
        ? [
            {
              intent: `mcp-elicitation`,
              label: t.formatMessage(O.cancel),
...
function z(e) {
  ...
  if (t?.[1] != null) return G(t[1]);
  let n = /\baccess\s+(.+?)\?*$/i.exec(e);
  return n?.[1] == null ? null : G(n[1]);
}
function B(e) {
  ...
  let [n, r] = t,
    i = H(r);
  return G(i == null ? n : `${n}: ${i}`);
}
function V(e) {
  try {
    let t = new URL(e),
      n = t.pathname === `/` ? `` : t.pathname;
    return K(`${t.hostname}${n}`);
  } catch {
    return K(e);
  }
}
...
function re({ message: e, metadata: t }) {
  let n = ae(t),
    r = ie(e, n);
  return n != null && oe(r) ? n : G(r);
}
function ie(e, t) {
  let n = e.trim();
  if (t == null) return n;
...
function ae(e) {
  ...
  let t = e.reason;
  return typeof t == `string` ? G(t) : null;
}
function oe(e) {
...
function le({
  ...
  intl: o,
  realtimeConversationId: s,
}) {
  if (c(e)) return null;
  let u = e.hostId ?? `local`,
    d = u === `local` ? `local` : `remote-host`,
    f = pe(e);
  if ((e.threadSource === `realtime_voice` || e.id === s) && f !== `waiting` && f !== `failed`)
    return null;
  let p = t && f === `waiting` ? i(e) : null;
  return {
    ...
    source: d,
    sortAtMs: a(e)?.turnStartedAtMs ?? e.updatedAt,
    status: f,
    subtitle: de(e, o),
    title: r(e) ?? o.formatMessage(q.newThread),
    turnKey: String(l(e)),
    updatedAtMs: e.updatedAt,
    waitingRequest: k(p, o, {
      includeMcpElicitationCancelAction: n,
      planStartCollaborationMode: {
        mode: `default`,
        settings: { ...e.latestCollaborationMode.settings, developer_instructions: null },
      },
    }),
  };
}
function ue(e, t) {
  ...
  return {
    ...
    controlTarget: r == null ? null : { type: `cloud-task`, taskId: e.id, turnId: r },
    hostId: null,
    key: `cloud:` + e.id,
    ...
    status: me(e),
    subtitle: null,
    title: e.title?.trim() || t.formatMessage(q.newThread),
    turnKey: r,
    updatedAtMs: n,
    waitingRequest: null,
  };
      ...
      let e = u(n.text),
        t =
          e?.decision === `DONT_NOTIFY`
            ? null
            : Y(e?.notificationMessage ?? e?.visibleText ?? n.text);
      if (t != null) return t;
    }
  }
...
function fe(e, t) {
  ...
  if (e.type === `commandExecution`) {
    ...
    if (n == null) return r ? t.formatMessage(q.runningCommand) : t.formatMessage(q.ranCommand);
    switch (n.type) {
      case `read`:
        return r
          ? t.formatMessage(q.readingFile, { fileName: n.name })
          : t.formatMessage(q.readFile, { fileName: n.name });
      case `listFiles`:
        return r ? t.formatMessage(q.listingFiles) : t.formatMessage(q.listedFiles);
      case `search`: {
        let e = Y(n.query ?? ``);
        return e == null
          ? r
            ? t.formatMessage(q.searchingFiles)
            : t.formatMessage(q.searchedFiles)
          : r
            ? t.formatMessage(q.searchingQuery, { query: e })
            : t.formatMessage(q.searchedQuery, { query: e });
      }
      case `unknown`:
        return r ? t.formatMessage(q.runningCommand) : t.formatMessage(q.ranCommand);
    }
  }
  if (e.type === `fileChange`) {
    let n = e.changes.length;
    return e.status === `inProgress`
      ? t.formatMessage(q.editingFiles, { fileCount: n })
      : t.formatMessage(q.editedFiles, { fileCount: n });
  }
  if (e.type === `mcpToolCall`) {
    ...
    return r == null
      ? n
        ? t.formatMessage(q.callingTool)
        : t.formatMessage(q.calledTool)
      : n
        ? t.formatMessage(q.callingToolName, { toolName: r })
        : t.formatMessage(q.calledToolName, { toolName: r });
  }
  if (e.type === `webSearch`) {
    let n = Y(e.query);
    return n == null
      ? t.formatMessage(q.searchedWeb)
      : t.formatMessage(q.searchedQuery, { query: n });
  }
  return null;
}
function Y(e) {
  ...
  let t = e.resumeState === `needs_resume` ? e.threadRuntimeStatus : null,
    n =
      e.resumeState === `needs_resume`
        ? t?.type === `active`
        : e.resumeState === `resuming` || a(e)?.status === `inProgress`,
    r =
      e.resumeState === `needs_resume`
        ? t?.type === `active` && t.activeFlags.includes(`waitingOnUserInput`)
        : e.requests.some((e) => e.method === `item/tool/requestUserInput`),
    i = s(e).some((e) => e.items.some((e) => e.type === `planImplementation` && !e.isCompleted)),
    c = e.resumeState === `needs_resume` ? t?.type === `systemError` : a(e)?.status === `failed`;
  return o(e) || r || i
    ? `waiting`
    : c
      ? `failed`
...
function ge(e) {
  ...
  if (j) {
    ...
    t[0] !== S || t[1] !== M
      ? ((e = m(
          ...
          S ? `pointer-events-auto opacity-100` : [`pointer-events-none opacity-0`, M],
        )),
        (t[0] = S),
        (t[1] = M),
        (t[2] = e))
    ...
    let n;
    t[3] !== D || t[4] !== T
      ? ((n = D.formatMessage(T ? $.unmuteMicrophone : $.muteMicrophone)),
        (t[3] = D),
        (t[4] = T),
        (t[5] = n))
      : (n = t[5]);
    let r = T ? $.unmuteMicrophone : $.muteMicrophone,
      i;
    t[6] === r ? (i = t[7]) : ((i = (0, Z.jsx)(p, { ...r })), (t[6] = r), (t[7] = i));
    let a;
    t[8] === T
      ? (a = t[9])
      : ((a = T
        ...
        (t[9] = a));
    let o;
    t[10] !== T || t[11] !== u || t[12] !== a || t[13] !== n || t[14] !== i
      ? ((o = (0, Z.jsx)(Q, {
          ariaLabel: n,
          isPressed: T,
          tooltipContent: i,
    ...
    let s;
    t[16] !== D || t[17] !== E
      ? ((s = D.formatMessage(E ? $.unmuteOutput : $.muteOutput)),
        (t[16] = D),
        (t[17] = E),
        (t[18] = s))
      : (s = t[18]);
    let c = E ? $.unmuteOutput : $.muteOutput,
      f;
    t[19] === c ? (f = t[20]) : ((f = (0, Z.jsx)(p, { ...c })), (t[19] = c), (t[20] = f));
    let h;
    t[21] === E
      ? (h = t[22])
      : ((h = E
        ...
        (t[22] = h));
    let g;
    t[23] !== E || t[24] !== d || t[25] !== s || t[26] !== f || t[27] !== h
      ? ((g = (0, Z.jsx)(Q, {
          ariaLabel: s,
          isPressed: E,
          tooltipContent: f,
      ...
      : (g = t[28]);
    let _;
    t[29] === D ? (_ = t[30]) : ((_ = D.formatMessage($.stop)), (t[29] = D), (t[30] = _));
    let b;
    t[31] === Symbol.for(`react.memo_cache_sentinel`)
      ? ((b = (0, Z.jsx)(p, { ...$.stop })), (t[31] = b))
      : (b = t[31]);
    let C;
    t[32] === l
      ? (C = t[33])
    ...
    t[36] === Symbol.for(`react.memo_cache_sentinel`)
      ? ((O = (0, Z.jsx)(y, {
          className: `icon-xs hidden group-hover/realtime-voice-stop:block group-focus-visible/realtime-voice-stop:block`,
        })),
        (t[36] = O))
      : (O = t[36]);
    let k;
    t[37] === w
      ? (k = t[38])
      : ((k = (0, Z.jsxs)(Z.Fragment, { children: [w, O] })), (t[37] = w), (t[38] = k));
    let A;
    t[39] !== _ || t[40] !== C || t[41] !== k
      ? ((A = (0, Z.jsx)(Q, {
          ariaLabel: _,
  ...
  let N = k ? $.starting : O ? $.stop : $.start,
    P;
  t[48] === N ? (P = t[49]) : ((P = (0, Z.jsx)(p, { ...N })), (t[48] = N), (t[49] = P));
  let F;
  t[50] !== D || t[51] !== N
    ? ((F = D.formatMessage(N)), (t[50] = D), (t[51] = N), (t[52] = F))
    : (F = t[52]);
  let I = k ? `cursor-default` : `cursor-interaction`,
    L =
      A && !w
    ...
    R;
  t[53] !== S || t[54] !== O || t[55] !== w || t[56] !== M || t[57] !== I || t[58] !== L
    ? ((R = m(
  ...
  t[60] !== O || t[61] !== c || t[62] !== l
    ...
    : (z = t[63]);
  let B;
  t[64] !== A || t[65] !== w || t[66] !== k || t[67] !== x
    ? ((B =
        w && A
          ? (0, Z.jsx)(y, { className: `icon-xs` })
          : k
    ...
    : (B = t[68]);
  let V;
  t[69] !== k || t[70] !== R || t[71] !== z || t[72] !== B || t[73] !== F
    ? ((V = (0, Z.jsx)(`button`, {
        type: `button`,
        "aria-label": F,
        disabled: k,
  ...
  return (
    t[75] !== V || t[76] !== P
      ? ((H = (0, Z.jsx)(_, { tooltipContent: P, children: V })),
        (t[75] = V),
        (t[76] = P),
        (t[77] = H))
      : (H = t[77]),
...
function Q(e) {
  let t = (0, X.c)(14),
    { ariaLabel: n, children: r, className: i, isPressed: a, onClick: o, tooltipContent: s } = e,
  ...
  return (
    t[11] !== d || t[12] !== s
      ? ((f = (0, Z.jsx)(_, { tooltipContent: s, children: d })),
        (t[11] = d),
        (t[12] = s),
        (t[13] = f))
      : (f = t[13]),
...
export { j as a, C as c, J as i, he as n, A as o, ce as r, D as s, ge as t };
//# sourceMappingURL=avatar-overlay-realtime-voice-button.js.map
