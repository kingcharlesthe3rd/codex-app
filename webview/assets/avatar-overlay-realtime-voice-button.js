...
import { t as y } from "./waveform.js";
import { t as b } from "./get-resize-observer-entry-size.js";
import { t as x } from "./permission-request-model.js";
function S({ axis: e, debounce: t, initialSize: n = !0, onChange: r, target: i }) {
...
var C = d({
  ...
  w = {
    ...
    fallbackBodyMessage: C.info,
    iconClassName: `icon-xs shrink-0 text-token-text-secondary`,
    iconType: `clock`,
    labelMessage: C.info,
    mascotState: `idle`,
  },
  T = { ...w, mascotState: `waving` };
function E(e) {
  if (e == null) return w;
  if (e.kind === `first-awake`) return T;
  if (e.isLoading)
    return {
      badgeBackgroundColor: `var(--color-token-activity-bar-badge-background)`,
      badgeForegroundColor: `var(--color-token-activity-bar-badge-foreground)`,
      fallbackBodyMessage: C.runningFallbackBody,
      iconClassName: `icon-xs shrink-0 text-token-text-secondary`,
      iconType: `spinner`,
      labelMessage: C.running,
      mascotState: `running`,
    };
  switch (e.level) {
    case `warning`:
...
function O(e, t, { includeMcpElicitationCancelAction: n = !1 } = {}) {
  switch (e?.type) {
    case `userInput`:
      return e.isOnboardingDynamicInput === !0 ? null : j(e, t);
    case `approval`:
      return e.item.type === `exec` ? M(e.item, t) : ee(e.item, t);
...
function k(e, t) {
  switch (e.kind) {
    ...
    case `patch`:
      return [
        t.formatMessage(D.fileCount, { count: e.fileCount }),
...
function j(e, t) {
  ...
  let r = U(n.question) ?? U(n.header);
  return r == null || n.options.length === 0
    ? null
    : {
        kind: `question`,
        requestId: e.item.requestId,
        title: t.formatMessage(D.askQuestion),
        prompt: r,
        options: n.options.map((e) => ({
          label: U(e.label) ?? t.formatMessage(D.option),
...
function M(e, t) {
  ...
  let n = U(e.networkApprovalContext?.host ?? ``);
  return n == null
    ? {
        kind: `exec`,
        requestId: e.approvalRequestId,
        operation: t.formatMessage(D.runCommand),
        summary: U(e.cmd.join(` && `)) ?? t.formatMessage(D.command),
        title: t.formatMessage(D.reviewCommand),
        actions: [
          {
            commandDecision: `accept`,
            intent: `command-approval`,
            label: t.formatMessage(D.runOnce),
            tone: `primary`,
          },
          {
            commandDecision: `decline`,
            intent: `command-approval`,
            label: t.formatMessage(D.deny),
            tone: `danger`,
          },
        ],
      }
    : {
        kind: `network`,
        requestId: e.approvalRequestId,
        operation: t.formatMessage(D.allowNetwork),
        target: n,
        title: t.formatMessage(D.allowNetwork),
        actions: [
          {
            ariaLabel: t.formatMessage(D.allowOnce),
            commandDecision: `accept`,
            intent: `command-approval`,
            label: t.formatMessage(D.allow),
            tone: `primary`,
          },
          {
            commandDecision: `decline`,
...
function ee(e, t) {
  ...
  return {
    kind: `patch`,
    requestId: e.approvalRequestId,
    operation: t.formatMessage(D.applyChanges),
    summary: n[0]?.[0] ?? t.formatMessage(D.oneFileChanged),
    title: t.formatMessage(D.applyChanges),
    files: n.map(([e]) => e),
    fileCount: n.length,
    ...
    actions: [
      {
        ...
        label: t.formatMessage(D.apply),
        tone: `primary`,
      },
      { intent: `open`, label: t.formatMessage(D.review), tone: `secondary` },
    ],
  };
}
function te(e, t) {
  ...
  let i, a;
  if (r.kind === `network`) ((a = t.formatMessage(D.networkAccess)), (i = a));
  else {
    a = t.formatMessage(D.fileAccess);
    let e = r.paths[0] ?? a,
    ...
    actions: [
      {
        ...
        ariaLabel: t.formatMessage(D.allowOnce),
        label: t.formatMessage(D.allow),
        permissionResponse: { permissions: ie(n), scope: `turn` },
        tone: `primary`,
      },
      {
        intent: `permission-response`,
...
function ne(e, t) {
  ...
  return r == null
    ...
    : {
        kind: `plan`,
        operation: t.formatMessage(D.plan),
        planContent: n,
        summary: r,
        title: t.formatMessage(D.plan),
        turnId: e.turnId,
        actions: [
          { intent: `plan-start`, label: t.formatMessage(D.implementPlan), tone: `primary` },
        ],
      };
}
function re(e, t, n) {
  let { elicitation: r } = e,
    i = e.request.params,
    a = U(i.serverName),
    o = B(a) ?? a ?? t.formatMessage(D.toolServer),
    s,
    c = o,
    l = null,
    u = null;
  switch (r.kind) {
    case `formElicitation`:
      ((s = t.formatMessage(D.answerTarget, { target: o })), (l = U(r.message)), (u = []));
      break;
    case `mcpToolCall`: {
      let e = U(r.approval.connector_name) ?? B(r.approval.connector_id) ?? o,
        n = ae(r.message);
      ((s =
        n == null
          ? t.formatMessage(D.allowTarget, { target: e })
          : t.formatMessage(D.allowTarget, { target: n })),
        (c = e),
        (l = U(r.subtitle) ?? N(r.approval.tool_params) ?? U(r.message)));
      break;
    }
    case `toolSuggestion`: {
      let e = U(r.suggestion.tool_name) ?? B(r.suggestion.tool_id) ?? t.formatMessage(D.tool);
      ((s =
        r.suggestion.suggest_type === `enable`
          ? t.formatMessage(D.enableTool, { toolName: e })
          : t.formatMessage(D.installTool, { toolName: e })),
        (c =
          r.suggestion.tool_type === `connector`
            ? t.formatMessage(D.connector)
            : t.formatMessage(D.plugin)),
        (l = U(r.suggestion.suggest_reason)));
      break;
    }
    case `connectorAuth`: {
      let e = U(r.connector.connector_name) ?? B(r.connector.connector_id) ?? o,
        n = r.connector.auth_reason;
      ((s =
        n === `missing_link`
          ? t.formatMessage(D.connectTarget, { target: e })
          : n === `oauth_upgrade_required`
            ? t.formatMessage(D.additionalAccessTarget, { target: e })
            : t.formatMessage(D.reconnectTarget, { target: e })),
        (c = e),
        (l = U(r.subtitle) ?? U(r.message) ?? P(r.url)),
        (u = [
          {
            intent: `open`,
            label:
              n === `missing_link`
                ? t.formatMessage(D.connect)
                : n === `oauth_upgrade_required`
                  ? t.formatMessage(D.updateAccess)
                  : t.formatMessage(D.reconnect),
            tone: `primary`,
          },
        ]));
      break;
    ...
    case `urlAction`:
      ((c = B(r.serverName) ?? U(r.serverName) ?? o),
        (s = t.formatMessage(D.openLink)),
        (l = U(r.subtitle) ?? P(r.url) ?? U(r.message)),
        (u = [{ intent: `open`, label: t.formatMessage(D.openLink), tone: `primary` }]));
      break;
    case `generic`:
      ((s =
        I({ message: r.message, metadata: r.metadata }) ??
        t.formatMessage(D.allowTarget, { target: o })),
        (l = U(r.subtitle) ?? N(r.toolParams) ?? U(r.message)));
      break;
  }
  r.riskLevel === `high` &&
    (u = [{ intent: `open`, label: t.formatMessage(D.review), tone: `primary` }]);
  let d = {
    ariaLabel: t.formatMessage(D.allowTarget, { target: c }),
    intent: `mcp-elicitation`,
    label: t.formatMessage(D.allowTarget, { target: c }),
    ...
    actions:
      ...
      (n
        ? [
            {
              intent: `mcp-elicitation`,
              label: t.formatMessage(D.cancel),
...
function ae(e) {
  ...
  if (t?.[1] != null) return U(t[1]);
  let n = /\baccess\s+(.+?)\?*$/i.exec(e);
  return n?.[1] == null ? null : U(n[1]);
}
function N(e) {
  ...
  let [n, r] = t,
    i = F(r);
  return U(i == null ? n : `${n}: ${i}`);
}
function P(e) {
  try {
    let t = new URL(e),
      n = t.pathname === `/` ? `` : t.pathname;
    return W(`${t.hostname}${n}`);
  } catch {
    return W(e);
  }
}
...
function I({ message: e, metadata: t }) {
  let n = R(t),
    r = L(e, n);
  return n != null && z(r) ? n : U(r);
}
function L(e, t) {
  let n = e.trim();
  if (t == null) return n;
...
function R(e) {
  ...
  let t = e.reason;
  return typeof t == `string` ? U(t) : null;
}
function z(e) {
...
function K({
  ...
  for (let i of r) {
    ...
    r == null || o.has(r.key) || (o.add(r.key), a.push(r));
  }
  for (let e of i) {
    let t = oe(e, n);
    o.has(t.key) || (o.add(t.key), a.push(t));
  }
  return a;
}
function q({
  ...
  return {
    ...
    status: d,
    subtitle: se(e, r),
    title: i(e) ?? r.formatMessage(G.newThread),
    turnKey: String(u(e)),
    updatedAtMs: e.updatedAt,
    waitingRequest: O(f, r, { includeMcpElicitationCancelAction: n }),
  };
}
function oe(e, t) {
...
function se(e, t) {
  return J(l(e)?.items ?? [], t);
}
function J(e, t) {
  for (let t = e.length - 1; t >= 0; --t) {
    let r = e[t];
    if (r?.type === `reasoning`)
      for (let e = r.summary.length - 1; e >= 0; --e) {
        let t = Y(r.summary[e]);
        if (t != null) return t;
      }
    if (r?.type === `agentMessage`) {
      let e = n(r.text),
        t =
          e?.decision === `DONT_NOTIFY`
            ? null
            : Y(e?.notificationMessage ?? e?.visibleText ?? r.text);
      if (t != null) return t;
    }
  }
...
function ce(e, t) {
  ...
  if (e.type === `commandExecution`) {
    ...
    if (n == null) return r ? t.formatMessage(G.runningCommand) : t.formatMessage(G.ranCommand);
    switch (n.type) {
      case `read`:
        return r
      ...
      case `listFiles`:
        return r ? t.formatMessage(G.listingFiles) : t.formatMessage(G.listedFiles);
      case `search`: {
        let e = Y(n.query ?? ``);
        return e == null
          ? r
      ...
      case `unknown`:
        return r ? t.formatMessage(G.runningCommand) : t.formatMessage(G.ranCommand);
    }
  }
...
function le(e) {
  let t = e.resumeState === `needs_resume` ? e.threadRuntimeStatus : null,
    ...
    a = e.resumeState === `needs_resume` ? t?.type === `systemError` : l(e)?.status === `failed`;
  return c(e) || r || i
    ? `waiting`
    : a
      ? `failed`
...
function de(e) {
  let t = (0, X.c)(27),
    {
      canStart: n,
      isRealtimeVoiceSurfaceVisible: r,
      onStart: i,
      onStop: a,
      phase: o,
      waveformCanvasRef: s,
  ...
  if (!u && !n) return null;
  let h = o === `active`,
    b = d ? $.starting : u ? $.stop : $.start,
    x;
  t[0] === b ? (x = t[1]) : ((x = (0, Z.jsx)(p, { ...b })), (t[0] = b), (t[1] = x));
  let S;
  t[2] !== l || t[3] !== b
    ? ((S = l.formatMessage(b)), (t[2] = l), (t[3] = b), (t[4] = S))
    : (S = t[4]);
  let C = d ? `cursor-default` : `cursor-interaction`,
    w =
      h && !c
    ...
    T = u && !c ? `opacity-100` : `opacity-0 group-hover:opacity-100 focus-visible:opacity-100`,
    E;
  t[5] !== C || t[6] !== w || t[7] !== T
    ? ((E = m(
  ...
  t[13] !== h || t[14] !== c || t[15] !== d || t[16] !== s
    ...
    : (O = t[17]);
  let k;
  t[18] !== d || t[19] !== S || t[20] !== E || t[21] !== D || t[22] !== O
    ? ((k = (0, Z.jsx)(`button`, {
        type: `button`,
        "aria-label": S,
        disabled: d,
  ...
  return (
    t[24] !== k || t[25] !== x
      ? ((A = (0, Z.jsx)(_, { tooltipContent: x, children: k })),
        (t[24] = k),
        (t[25] = x),
        (t[26] = A))
      : (A = t[26]),
...
export { A as a, S as c, J as i, Q as n, k as o, K as r, E as s, de as t };
