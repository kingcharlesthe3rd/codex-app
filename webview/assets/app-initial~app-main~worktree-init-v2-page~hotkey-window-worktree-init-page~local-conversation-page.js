...
import { Ig as f, Ji as p, Lg as m, Xi as h } from "./app-initial~app-main~onboarding-page.js";
import {
  an as g,
  un as _,
...
async function v({ entry: e, threadGoalObjective: t, workspaceRoot: n }) {
  ...
  if (e.launchMode === `fork-conversation`) {
    ...
    return (
      (t =
        e.targetTurnId == null
          ? await c(`fork-conversation-from-latest`, {
              hostId: e.hostId,
          ...
          : await c(`fork-conversation-from-turn`, {
              conversationId: e.sourceConversationId,
      ...
      r != null &&
        (await c(`add-worktree-init-synthetic-turn`, { conversationId: t, worktreeInit: r })),
      t
    );
  }
...
async function b({ entry: e, conversationId: t, hostConfig: i, threadGoalObjective: a }) {
  let o = e.initialThreadTitle == null && e.labelEdited,
    s = (e.initialThreadTitle ?? (o ? e.label : ``)).trim();
  if (
    (s.length > 0 &&
      (await c(`set-thread-title`, {
        conversationId: t,
        title: s,
        ...(o ? { onlyIfUntitled: !0 } : {}),
      })),
    ...
    try {
      await n(`worktree-set-owner-thread`, {
        params: { hostId: i.id, worktree: d(e.worktreeGitRoot), conversationId: t },
      });
    } catch (e) {
      r.warning(
  ...
  if (e.isPinned)
    try {
      await h(t, !0, e.pinnedBeforeThreadId);
    } catch (e) {
  ...
  e.launchMode === `start-conversation` &&
    a != null &&
    (await c(`set-thread-goal`, {
      appendTranscriptItem: !1,
      conversationId: t,
      hostId: e.threadStartHostId ?? i.id,
      objective: a,
    }));
}
function x(e, t) {
...
function S(e, t) {
  ...
  let r = e.browserTransferSourceBrowserTabIds,
    i = r != null && r.length > 0,
    a = o(`${n}:legacy`),
    s = r != null && r.length > 0 ? r : [e.browserTransferSourceBrowserTabId ?? a],
    c =
      e.browserTransferSourceBrowserTabId != null && s.includes(e.browserTransferSourceBrowserTabId)
        ? e.browserTransferSourceBrowserTabId
        : s.at(-1);
  for (let r of s)
    _(t, {
      active: r === c,
      browserTabId: i || e.browserTransferSourceBrowserTabId != null ? r : o(`${t}:legacy`),
