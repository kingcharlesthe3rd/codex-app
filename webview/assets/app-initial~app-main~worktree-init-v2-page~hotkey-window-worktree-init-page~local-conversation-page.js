...
async function v({ entry: e, threadGoalObjective: t, workspaceRoot: n }) {
  ...
  if (e.launchMode === `fork-conversation`) {
    ...
    return (
      ...
      r != null &&
        (await p(`add-worktree-init-synthetic-turn`, { conversationId: t, worktreeInit: r })),
      t
    );
  }
...
async function b({ entry: e, conversationId: n, hostConfig: r, threadGoalObjective: i }) {
  ...
  if (
    ...
    try {
      await o(`worktree-set-owner-thread`, {
        params: { hostId: r.id, worktree: s(e.worktreeGitRoot), conversationId: n },
      });
    } catch (e) {
      t.warning(
  ...
  if (e.isPinned)
    try {
      ...
      t.warning(`Worktree conversation started, but failed to set pinned metadata: {}`, {
        safe: {},
        sensitive: { error: e },
      });
    }
...
function S(e, t) {
  ...
  let r = e.browserTransferSourceBrowserTabIds,
    i = r != null && r.length > 0,
    o = a(`${n}:legacy`),
    s = r != null && r.length > 0 ? r : [e.browserTransferSourceBrowserTabId ?? o],
    c =
  ...
  for (let r of s)
    l(t, {
      active: r === c,
      browserTabId: i || e.browserTransferSourceBrowserTabId != null ? r : a(`${t}:legacy`),
