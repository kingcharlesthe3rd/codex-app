...
import { Z as r, a as i } from "./app-scope-CWE-zIhQ.js";
import { Zt as a } from "./use-host-config-C.js";
import {
  Br as o,
  Ni as s,
  Nr as c,
...
import { f as p, v as m } from "./vscode-api.js";
import {
  Aa as h,
  Ta as g,
  ai as _,
...
import { i as T, l as E, s as D } from "./lib-1.js";
import { t as O } from "./clsx.js";
import { t as k } from "./proxy.js";
import { t as ee } from "./chevron-right.js";
import { L as A } from "./rpc-1.js";
import { T as j } from "./parse-directives.js";
import "./thread-detail-level.js";
import { t as M } from "./settings.cog.js";
import { t as N } from "./codex.js";
import {
  D as te,
  E as ne,
  M as P,
  ...
  u as pe,
  x as me,
} from "./app-server-dynamic-tools-Gzn-GJ0M.js";
import { t as I } from "./thinking-shimmer.js";
import { t as he } from "./animations.js";
import { t as ge } from "./chat.js";
import { t as _e } from "./web-search-favicon-icon.js";
import { t as ve } from "./is-hotkey-window-context.js";
import { t as ye } from "./comment.js";
import { i as be, r as xe, t as Se } from "./thread-resource-card.js";
import { o as Ce } from "./mcp-tool-item-content-utils.js";
import { t as we } from "./thread-handoff-step-row.js";
import { n as Te, t as Ee } from "./patch-diff-stats.js";
var De = new WeakMap();
function L({ apps: e, functionName: t, serverName: n, toolName: r }) {
  let i = R(n),
    a = R(r),
    o = t
      .split(`__`)
...
function Oe(e) {
  ...
  let n = [
      R(e.name),
      R(e.id),
      R(e.id.replace(/^connector[_-]/i, ``)),
      ...(e.pluginDisplayNames ?? []).map(R),
    ],
    r = n.filter((e, t) => (e.length === 0 ? !1 : n.findIndex((t) => ke(t, e)) === t));
  return (De.set(e, r), r);
}
function R(e) {
  return e
...
function z({ summary: e, cwd: t }) {
  return B({ summary: e, cwd: t })?.isSkillDefinitionFile === !0;
}
function je({ summary: e, cwd: t, intl: n, threadDetailLevel: r, formatSearchQuery: i }) {
  let a = B({ summary: e, cwd: t });
  if (!a) return null;
  switch (e.type) {
    case `read`:
      return a.isSkillDefinitionFile
        ? r === `STEPS_PROSE`
          ? Me({ skillName: a.skillName, isFinished: e.isFinished, intl: n })
          : n.formatMessage(
              {
...
function Me({ skillName: e, isFinished: t, intl: n }) {
  return t
    ? n.formatMessage(
        {
...
function V({ path: e, cwd: t }) {
  if (!e) return null;
  let n = d(e);
  return n.length === 0 ? null : t == null ? n : d(A(t, n));
}
function H(e) {
  throw Error(`Unexpected value: ${String(e)}`);
}
function Pe({ item: e, mcpServerStatuses: t }) {
...
function Fe({ item: e, mcpServerStatuses: t }) {
  return Pe({ item: e, mcpServerStatuses: t }) === `mcp-app`;
}
function Ie({ item: e, mcpServerStatuses: t }) {
  return Pe({ item: e, mcpServerStatuses: t }) !== `not-mcp-app`;
}
function Le(e) {
  return e.some((e) =>
    e.kind !== `item` || e.item.type !== `mcp-tool-call`
      ...
      : e.item.result?.type === `success` ||
        (!e.item.completed && e.item.result == null && e.item.mcpAppResourceUri == null),
  );
}
var Re = /(?:^|\s)(?:-X\s*|--request(?:=|\s+))(?:POST|PUT|PATCH|DELETE)\b/i,
  ze = /(?:^|\s)(?:--data(?:-[^\s=]+)?|--json|--form|--upload-file)(?:=|\s|$)/,
  Be = /(?:^|\s)-(?:d|F|T)(?:=|\s|$)/;
function Ve(
  e,
  {
  ...
  if (e.entry.kind === `exploration`) return Ke(e.entry);
  let o = e.entry.item;
  if (o.type === `assistant-message`) return { type: `assistant-message` };
  if (o.type === `patch`) return { ...qe(o, a), ...U(o.automaticApprovalReviews) };
  if (o.type === `exec`) {
    if (
      Ue(o, i) ||
      (o.parsedCmd.type === `read` &&
        !o.parsedCmd.isFinished &&
        z({ summary: o.parsedCmd, cwd: o.cwd }))
    )
      return { type: `other` };
    let e = !o.parsedCmd.isFinished,
      ...
      r = t != null && !1,
      a = t != null && We(t),
      s = U(o.automaticApprovalReviews);
    return r
      ? { type: `exec`, isInProgress: e, createsVisualization: !0, ...s }
      : n
        ? { type: `exec`, isInProgress: e, createsFolder: !0, ...s }
        : a && (e || o.output?.exitCode === 0)
          ? { type: `exec`, isInProgress: e, searchesWeb: !0, ...s }
          : { type: `exec`, isInProgress: e, ...s };
  }
  if (o.type === `mcp-tool-call`)
    return Fe({ item: o, mcpServerStatuses: t }) ||
      (r === !0 && Ie({ item: o, mcpServerStatuses: t }))
      ? { type: `other` }
      : {
          type: `mcp-tool-call`,
          isInProgress: !o.completed,
          source: He(o, n),
          ...U(o.automaticApprovalReviews),
        };
  if (o.type === `automatic-approval-review`)
    switch (o.status) {
      case `denied`:
      case `timedOut`:
        return { type: `automatic-approval-review`, id: o.id, status: o.status };
      case `aborted`:
      case `approved`:
      case `inProgress`:
        return { type: `other` };
    }
  return o.type === `web-search`
...
function We(e) {
  if (!/^\s*curl(?:\s|$)/.test(e) || Re.test(e) || ze.test(e) || Be.test(e)) return !1;
  let t = e.match(/\bhttps?:\/\/[^\s'"<>]+/gi);
  return t == null ? !1 : t.some(Ge);
}
function Ge(e) {
  try {
...
function Ke(e) {
  ...
  for (let u of e.items)
    if (u.type === `exec`) {
      if ((t.push(...(u.automaticApprovalReviews ?? [])), u.parsedCmd.type === `search`)) {
      ...
      if (u.parsedCmd.type === `read`) {
        let e = Je(u.parsedCmd.path ?? u.parsedCmd.name, u.cwd);
        if (z({ summary: u.parsedCmd, cwd: u.cwd })) {
...
function qe(e, t) {
  let n = [],
    ...
    u = [],
    d = [];
  for (let [f, p] of Object.entries(e.changes)) {
    let m = Ee(c(f, p), f);
    if (((o += (m?.added ?? 0) + (m?.deleted ?? 0)), p.type === `add`)) {
      (n.push(f),
        e.success == null && t ? i.push(f) : (e.success ?? (r.push(f), (a += Ye(p.content)))));
      continue;
    }
    if (p.type === `delete`) {
      (u.push(f), e.success ?? d.push(f));
      continue;
    }
    (s.push(f), e.success ?? l.push(f));
  }
  return {
    type: `patch`,
    createdPaths: n,
    ...
    changedLineCount: o,
    editedPaths: s,
    runningEditedPaths: l,
    deletedPaths: u,
    runningDeletedPaths: d,
  };
}
function Je(e, t) {
  let n = d(e);
  return t == null ? n : d(A(t, n));
}
function Ye(e) {
  let t = e.replace(
    /\r\n/g,
...
function Ze(e) {
  let t = (0, W.c)(31),
    ...
    a = !n.completed,
    o;
  t[0] === r ? (o = t[1]) : ((o = () => Qe(r)), (t[0] = r), (t[1] = o));
  let s;
  t[2] !== n.callId || t[3] !== r
  ...
  let p = i === `summary-text` ? `inline` : `flex gap-2`,
    ...
    _;
  t[14] !== p || t[15] !== h || t[16] !== g
    ? ((_ = O(`text-size-chat min-w-0 items-center`, p, h, g)),
      (t[14] = p),
      (t[15] = h),
      (t[16] = g),
      (t[17] = _))
  ...
  let y = !n.completed,
    ...
    x;
  t[21] === b ? (x = t[22]) : ((x = O(b)), (t[21] = b), (t[22] = x));
  let S;
  t[23] !== f || t[24] !== y || t[25] !== x
    ? ((S = (0, G.jsx)(I, { active: y, className: x, children: f })),
      (t[23] = f),
      (t[24] = y),
      (t[25] = x),
      (t[26] = S))
  ...
  return (
    t[27] !== S || t[28] !== _ || t[29] !== v
      ? ((C = (0, G.jsxs)(`span`, { className: _, children: [v, S] })),
        (t[27] = S),
        (t[28] = _),
        (t[29] = v),
        (t[30] = C))
...
async function Qe(e) {
  ...
  try {
    ...
    return { faviconUrl: Xe(n, t.runtime), title: et(n) };
  } catch {
    return null;
  }
}
...
var tt = x({ tabId: w().int().nonnegative() }),
  nt = [{ getCompletedSummaryPartKey: it, namespace: l, render: rt, tool: s }];
function rt(e, t) {
  let n = tt.safeParse(e.arguments);
  return n.success ? (0, G.jsx)(Ze, { item: e, tabId: n.data.tabId, variant: t }) : null;
}
function it(e) {
  return tt.safeParse(e.arguments).success ? e.callId : null;
}
var at = [{ namespace: F, render: ot, tool: P }];
function ot(e, t) {
  let n = e.completed && e.success === !1 ? K.failed : e.completed ? K.completed : K.active;
  return t === `row`
    ? (0, G.jsxs)(`span`, {
        className: `text-size-chat my-1 flex min-w-0 items-start gap-2 text-token-conversation-summary-leading`,
        children: [
          (0, G.jsx)(`span`, {
            className: `mt-0.5 flex size-5 shrink-0 items-center justify-center rounded-full bg-token-bg-secondary text-token-text-secondary`,
            children: (0, G.jsx)(ye, { className: `icon-xs` }),
          }),
          (0, G.jsxs)(`span`, {
            ...
            children: [
              (0, G.jsx)(I, {
                ...
                children: (0, G.jsx)(D, { ...n }),
              }),
              e.completed
                ? null
                : (0, G.jsx)(`span`, {
                    className: `text-size-chat-sm text-token-text-tertiary`,
                    children: (0, G.jsx)(D, { ...K.activeDetail }),
                  }),
            ],
          }),
        ],
    ...
    : (0, G.jsxs)(`span`, {
        ...
        children: [
          t === `summary-text`
            ? null
            : (0, G.jsx)(ye, { className: `icon-xs shrink-0 text-token-text-secondary` }),
          (0, G.jsx)(I, {
            active: !e.completed,
            className: O(t !== `summary-text` && `min-w-0 truncate`),
            children: (0, G.jsx)(D, { ...n }),
          }),
        ],
      });
...
var K = T({
    active: {
      id: `localConversation.realtimeFeedbackToolCall.active`,
      defaultMessage: `Sending realtime voice feedback`,
      description: `In-progress label for submitting feedback about a realtime voice session.`,
    },
    activeDetail: {
      id: `localConversation.realtimeFeedbackToolCall.activeDetail`,
      defaultMessage: `Attaching session context and sending it to the team`,
      description: `Secondary in-progress text for submitting feedback about a realtime voice session.`,
    },
    completed: {
      id: `localConversation.realtimeFeedbackToolCall.completed`,
      defaultMessage: `Sent realtime voice feedback`,
      description: `Completed label for submitting feedback about a realtime voice session.`,
    },
    failed: {
      id: `localConversation.realtimeFeedbackToolCall.failed`,
      defaultMessage: `Could not send realtime voice feedback`,
      description: `Failure label for submitting feedback about a realtime voice session.`,
    },
  }),
  st = [
    { namespace: F, render: ct, tool: re },
    { namespace: F, render: lt, tool: ce },
  ];
function ct(e, t) {
  return ut(e, e.completed ? `readCompleted` : `readActive`, t);
}
function lt(e, t) {
  return ut(e, e.completed ? `writeCompleted` : `writeActive`, t);
}
function ut(e, t, n) {
  return (0, G.jsxs)(`span`, {
    ...
    children: [
      n === `summary-text`
        ? null
        : (0, G.jsx)(M, { className: `icon-xs shrink-0 text-token-text-secondary` }),
      (0, G.jsx)(I, {
        active: !e.completed,
        className: O(n !== `summary-text` && `min-w-0 truncate`),
        children: (0, G.jsx)(D, { ...dt[t] }),
      }),
    ],
  });
}
var dt = T({
  ...
  ft = e(t(), 1),
  pt = g({
    destinationHostDisplayName: h().min(1),
    operationId: h().min(1),
    status: S([`queued`, `running`, `success`, `warning`, `error`]),
    threadTitle: h().min(1),
  });
function mt(e) {
  let t = (0, W.c)(52),
    { item: n, variant: r } = e,
    a = n.completed && n.success === !0 ? gt(n.contentItems ?? null) : null,
    o = i(ne, a?.operationId ?? n.callId),
    s = o?.threadTitle ?? a?.threadTitle ?? null,
    c = o?.destinationHostDisplayName ?? a?.destinationHostDisplayName ?? null,
    l = o == null ? (a == null ? n.completed : te(a.status)) : te(o.status),
    u =
      o == null
        ? n.success
        : o.status === `error`
          ? !1
          : o.status === `success` || o.status === `warning`,
    [d, f] = (0, ft.useState)(!1),
    [p, m] = (0, ft.useState)(!1),
    { elementHeightPx: h, elementRef: g } = Te(),
    _ = o != null && o.steps.length > 0,
    v = _ && (l ? p : !d);
  if (r !== `row`) {
    ...
    t[2] === r
      ...
      : ((i =
          r === `summary-text`
            ? null
            : (0, G.jsx)(N, { className: `icon-xs shrink-0 text-token-text-secondary` })),
        (t[2] = r),
        (t[3] = i));
    let a = !l,
      o = r !== `summary-text` && `min-w-0 truncate`,
      d;
    t[4] === o ? (d = t[5]) : ((d = O(o)), (t[4] = o), (t[5] = d));
    let f;
    t[6] !== l || t[7] !== c || t[8] !== u || t[9] !== s
      ? ((f = (0, G.jsx)(ht, {
          completed: l,
          destinationHostDisplayName: c,
          success: u,
          threadTitle: s,
        })),
        (t[6] = l),
    ...
    t[11] !== a || t[12] !== d || t[13] !== f
      ? ((p = (0, G.jsx)(I, { active: a, className: d, children: f })),
        (t[11] = a),
        (t[12] = d),
        (t[13] = f),
        (t[14] = p))
    ...
    let m;
    return (
      t[15] !== n || t[16] !== i || t[17] !== p
        ? ((m = (0, G.jsxs)(`span`, { className: n, children: [i, p] })),
          (t[15] = n),
          (t[16] = i),
          (t[17] = p),
          (t[18] = m))
  ...
  let y = _ ? v : void 0,
    b = _ && `cursor-interaction`,
    x;
  t[19] === b
    ? (x = t[20])
    : ((x = O(`group flex min-w-0 items-center gap-2 text-left`, b)), (t[19] = b), (t[20] = x));
  let S = !_,
    C;
  t[21] !== d || t[22] !== l || t[23] !== p
    ? ((C = () => {
        l ? m(!p) : f(!d);
      }),
      (t[21] = d),
  ...
  t[25] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((w = (0, G.jsx)(N, { className: `icon-xs shrink-0 text-token-text-secondary` })),
      (t[25] = w))
    : (w = t[25]);
  let T = !l,
    E;
  t[26] !== l || t[27] !== c || t[28] !== u || t[29] !== s
    ? ((E = (0, G.jsx)(ht, {
        completed: l,
        destinationHostDisplayName: c,
        success: u,
  ...
  t[34] !== _ || t[35] !== v
    ...
    : (A = t[36]);
  let j;
  t[37] !== y || t[38] !== A || t[39] !== x || t[40] !== S || t[41] !== C || t[42] !== D
    ? ((j = (0, G.jsxs)(`button`, {
        type: `button`,
        "aria-expanded": y,
        className: x,
    ...
    : (j = t[43]);
  let M;
  t[44] !== h || t[45] !== g || t[46] !== v || t[47] !== o
    ? ((M =
        o == null
          ? null
          : (0, G.jsx)(k.div, {
              initial: !1,
              animate: { height: v ? h : 0, opacity: v ? 1 : 0 },
              "aria-hidden": !v,
              className: v ? `overflow-visible` : `overflow-hidden`,
  ...
  return (
    t[49] !== j || t[50] !== M
      ? ((P = (0, G.jsxs)(`div`, { className: `my-1 min-w-0`, children: [j, M] })),
        (t[49] = j),
        (t[50] = M),
        (t[51] = P))
      : (P = t[51]),
...
function _t(e) {
  ...
  t[3] !== i || t[4] !== n.pendingWorktreeId || t[5] !== n.threadId
    ? ((s = () => {
        p.dispatchHostMessage({
          ...
          path: i ? y(n.pendingWorktreeId) : C(_(n.threadId)),
        });
      }),
      (t[3] = i),
  ...
  t[7] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((l = (0, G.jsx)(`span`, {
        ...
        children: (0, G.jsx)(ge, { className: `icon-sm`, "aria-hidden": !0 }),
      })),
      (t[7] = l))
    : (l = t[7]);
  let u = i ? q.worktreeThreadQueued : q.threadCreated,
    d;
  t[8] === u ? (d = t[9]) : ((d = (0, G.jsx)(D, { ...u })), (t[8] = u), (t[9] = d));
  let f = i ? q.openSetup : q.openThread,
    m;
  t[10] === f
    ? (m = t[11])
    : ((m = (0, G.jsx)(xe, { label: (0, G.jsx)(D, { ...f }) })), (t[10] = f), (t[11] = m));
  let h;
  t[12] !== d || t[13] !== m
    ? ((h = (0, G.jsx)(be, { icon: l, title: d, trailing: m })),
      (t[12] = d),
      (t[13] = m),
      (t[14] = h))
    : (h = t[14]);
...
var q = T({
  ...
  vt = g({ target: g({ type: h(), environment: g({ type: h() }).optional() }) }),
  yt = g({ environment: g({ type: h() }).optional() }),
  bt = g({ destinationHostId: h().min(1).optional(), threadId: h().min(1) }),
  xt = g({ operationId: h().min(1) }),
  St = g({ threadId: h().min(1) }),
  Ct = b([g({ threadId: h().min(1) }), g({ pendingWorktreeId: h().min(1) })]),
  wt = [
    { getCompletedSummaryPartKey: Y, namespace: F, render: J, tool: se },
    { getCompletedSummaryPartKey: Y, namespace: F, render: J, tool: de },
    {
      getCompletedSummaryPartKey: Et,
      namespace: F,
      render: Tt,
    ...
    {
      getCompletedSummaryPartKey: Dt,
      namespace: F,
      render: J,
      summaryOnlyInConversationGroup: !0,
      tool: me,
    },
    { continuesLiveActivityBetweenCalls: !0, namespace: F, render: J, tool: ue },
    { continuesLiveActivityBetweenCalls: !0, namespace: F, render: J, tool: fe },
    { namespace: F, render: J, tool: ae },
    { namespace: F, render: J, tool: pe },
    { namespace: F, render: J, tool: le },
    { namespace: F, render: J, tool: oe },
  ];
function Tt(e, t) {
  return bt.safeParse(e.arguments).success ? (0, G.jsx)(mt, { item: e, variant: t }) : null;
}
function Et(e) {
  let t = bt.safeParse(e.arguments);
  return t.success ? JSON.stringify([t.data.threadId, t.data.destinationHostId ?? null]) : null;
}
function Dt(e) {
  let t = xt.safeParse(e.arguments);
...
function J(e, t) {
  ...
  let r =
    e.tool === `create_thread` && e.completed && e.success === !0 && t === `row`
      ? Ot(e.contentItems ?? null)
      : null;
  if (r != null) return (0, G.jsx)(_t, { result: r });
  let i =
      t === `row` && (e.tool === `read_thread` || e.tool === `send_message_to_thread`)
        ? St.safeParse(e.arguments)
        : null,
    a = i?.success === !0 ? i.data.threadId : null,
    o = O(
      `text-size-chat min-w-0 items-center`,
      t === `summary-text` ? `inline` : `flex gap-2`,
      t === `row` && `my-1`,
        ...
        t === `summary-text`
          ...
          : (0, G.jsx)(N, {
              className: O(
                ...
                a != null && `group-hover:!text-token-foreground`,
              ),
            }),
        (0, G.jsx)(I, {
          active: !e.completed,
          className: O(
            t !== `summary-text` && `min-w-0 truncate`,
            a != null && `group-hover:!text-token-foreground`,
          ),
          children: (0, G.jsx)(D, { ...kt[n] }),
        }),
      ],
    });
  if (a != null) {
    ...
    return (0, G.jsx)(`button`, {
      ...
      className: o,
      onClick: () => {
        p.dispatchHostMessage({ type: `navigate-to-route`, path: ve() ? v(e) : C(e) });
      },
      children: s,
    });
  }
  return (0, G.jsx)(`span`, { className: o, children: s });
}
function Y(e) {
  switch (e.tool) {
    case se: {
      let t = yt.safeParse(e.arguments);
      return t.success
    ...
    case de: {
      let t = vt.safeParse(e.arguments);
      return t.success
    ...
    case me:
      return xt.safeParse(e.arguments).success
        ? e.completed
...
var kt = T({
    ...
    threadsHandoffStatusActive: {
      ...
      description: `In-progress label for checking an asynchronous Codex thread handoff operation.`,
    },
    threadsHandoffStatusCompleted: {
      id: `localConversation.appControlToolCall.threadsHandoffStatus.completed`,
      defaultMessage: `Checked handoff status`,
      description: `Completed label for checking an asynchronous Codex thread handoff operation.`,
    },
  ...
  At = [...nt, ...wt, ...at, ...st];
function X(e) {
  return At.find((t) => t.namespace === e.namespace && t.tool === e.tool);
}
function jt(e) {
  return X(e)?.continuesLiveActivityBetweenCalls === !0;
}
function Mt(e) {
  return X(e)?.summaryOnlyInConversationGroup === !0;
}
function Nt(e) {
  let t = X(e)?.getCompletedSummaryPartKey?.(e);
  return `${e.namespace}:${e.tool}:${t ?? ``}`;
}
function Pt({ items: e, isActive: t }) {
  if (!t) return !1;
  if (e.some((e) => !e.completed)) return !0;
  let n = e.at(-1);
  return n != null && jt(n);
}
function Ft({ items: e, keepLatestLiveActivityInGroup: t }) {
  let n = e.at(-1);
  return e.length > 1 || (t && n != null && jt(n));
}
function It({ units: e, isActivitySliceClosed: t }) {
  let n = [],
    r = Lt(e, t);
  for (let { startIndex: t, endIndex: i } of r) {
    ...
    for (; r < i; ) {
      ...
      for (; r < i; ) {
        let t = e[r];
        if (t == null || !Rt(t)) break;
        (Bt(o, t), (r += 1));
      }
      n.push({ startIndex: a, endIndex: r, summary: Vt(o) });
    }
  }
  return n;
}
...
function Bt(e, t) {
  if (t.type === `exploration`) {
    Z(e, t.automaticApprovalReviewFailures);
  ...
  if (t.type === `exec`) {
    if ((Z(e, t.automaticApprovalReviewFailures), (e.commandCount += 1), t.isInProgress)) {
  ...
  if (t.type === `mcp-tool-call`) {
    if ((Z(e, t.automaticApprovalReviewFailures), (e.mcpToolCallCount += 1), t.source != null)) {
...
function Jt({
  ...
  for (let t = 0; t < e.length; ) {
    ...
    for (; l < e.length; ) {
      let t = on(e[l]);
      if (t == null || an(t, n, r)) break;
      let a = rn({ item: t, resolvedApps: i });
      if (c != null && a !== c) break;
      ((c = a), s.push(t), (l += 1));
    }
    if (s.length > 1 || (a && s.length > 0 && l === e.length)) {
      (o.push({ kind: `pending-mcp-tool-calls`, key: s[0]?.callId ?? `${t}`, items: s }), (t = l));
      continue;
    }
    let u = e[t];
    (u != null && o.push(u), (t += 1));
...
function Yt({ units: e, keepLatestLiveActivityInGroup: t = !1 }) {
  let n = [];
  for (let r = 0; r < e.length; ) {
    ...
    for (; o < e.length; ) {
      let t = sn(e[o]);
      if (t == null) break;
      if (X(t)?.standaloneInConversation === !0) {
        if (i.length > 0) break;
    ...
    if (Ft({ items: i, keepLatestLiveActivityInGroup: t && o === e.length })) {
      (n.push({ kind: `dynamic-tool-call-group`, key: i[0]?.callId ?? `${r}`, items: i }), (r = o));
      continue;
    }
    (a != null && n.push(a), (r = o));
  }
    ...
    switch (n.kind) {
      case `entry`:
        return n.entry.kind === `exploration`
          ...
          : e === `STEPS_PROSE` &&
              (n.entry.item.type === `automatic-approval-review` || n.entry.item.type === `exec`)
            ? t
            : t + 1;
...
function tn({
  ...
  let c = e.map((e) =>
    ...
    l = [...It({ units: c, isActivitySliceClosed: t })];
  if (!t) {
    let e = cn(c),
      t = [];
    ...
    for (let n of It({ units: t, isActivitySliceClosed: !0 }))
  ...
  for (let r = 0; r < e.length; ) {
    ...
    if (i && r === i.startIndex) {
      ...
      if (a.length === 1 && o != null && o.kind === `entry` && o.entry.kind === `item`) {
        let e = o.entry.item;
        if (e.type === `mcp-tool-call` || (n !== `STEPS_PROSE` && e.type === `exec` && !s)) {
          (u.push(o), (r = i.endIndex), (d += 1));
          continue;
        }
      }
      if (o != null) {
        let e = nn({ groupedUnits: a, isCurrentToolActivity: s, summary: i.summary });
        u.push({ kind: `collapsed-tool-activity`, key: qt(o, i.startIndex), units: a, summary: e });
      }
      ((r = i.endIndex), (d += 1));
      continue;
    }
...
function nn({ groupedUnits: e, isCurrentToolActivity: t, summary: n }) {
  return !t || e.at(-1)?.kind !== `web-search-group`
    ? n
    : { ...n, runningWebSearchCount: n.webSearchCount };
}
function rn({ item: e, resolvedApps: t }) {
  let n = o({ item: e });
...
var ln = e(f(), 1);
function un({ agentItems: e, isTurnInProgress: t, isAnyNonAgentItemInProgress: n }) {
  let r = [],
    i = null,
    a = !1,
    o = !1,
...
function dn(e) {
  return e.type !== `exec` ||
    (e.parsedCmd.type === `read` &&
      ...
      z({ summary: e.parsedCmd, cwd: e.cwd }))
    ? !1
...
function mn(e, t) {
  ...
  let S = yn(f),
    ...
    w =
      d.size > 0
        ? S.filter((e) => e.type !== `mcp-tool-call` || e.completed || !d.has(e.invocation.server))
        : S,
    T = w[w.length - 1],
    E = _n(T) ? T : null,
    D = (E?.content?.trim().length ?? 0) > 0 || !!E?.structuredOutput;
  E ? (w.pop(), h.push(...C)) : w.push(...C);
  let O = w[w.length - 1],
    k = t !== `in_progress` && !D && Sn(O) ? O : null;
  k && w.pop();
  let ee =
    E != null && E.completed && p.length > 0
      ? {
          ...E,
          automationCitations: (0, pn.default)(
            [...p].reverse(),
  ...
  return {
    userItems: i,
    agentItems: w,
    automationUpdateItems: E == null ? p : [],
    assistantItem: ee,
    toolOutputItems: m,
...
var hn = (e) => e.type === `patch` && e.approvalRequestId != null && e.success == null,
  gn = (e) => e.type === `exec` && e.approvalRequestId != null && e.output?.exitCode === void 0,
  _n = (e) => e?.type === `assistant-message`,
  vn = (e) => {
    let t = [];
    for (;;) {
      let n = e[e.length - 1];
  ...
  yn = (e) => {
    ...
    return e.flatMap((e) => {
      if (e.type === `automatic-approval-review` && e.targetItemId != null && t.has(e.targetItemId))
        return [];
      let n = bn(e),
