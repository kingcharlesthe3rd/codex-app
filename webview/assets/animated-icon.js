const __vite__mapDeps = (
  i,
  m = __vite__mapDeps,
  d = m.f ||
    (m.f = [
      "./analyze_image_animation.js",
      "./rolldown-runtime.js",
      "./automation.js",
      "./browsing_animation.js",
      "./code-searching-icon.js",
      "./codex-happy-small.js",
      "./codex-looking-around.js",
      "./edit_files_animation.js",
      "./hello.js",
      "./loader-Cr_-5mLI.js",
      "./internal-knowledge-icon.js",
      "./list_files_animation.js",
      "./local_context_animation.js",
      "./run_command_animation.js",
      "./searching_animation.js",
      "./to_do_animation.js",
      "./web-search-icon.js",
      "./browser.js",
      "./app-initial~app-main~worktree-init-v2-page~remote-conversation-page~pull-requests-page~plug~kmtatxxf.js",
      "./app-initial~app-main~worktree-init-v2-page~remote-conversation-page~pull-requests-page~plug~kmtatxxf.css",
    ]),
) => i.map((i) => d[i]);
import { n as e, s as t } from "./rolldown-runtime.js";
import {
  $r as n,
  Qr as r,
  RV as i,
  TP as a,
  ei as o,
  fL as s,
  jV as c,
  pL as l,
  ti as ee,
  vN as u,
  wP as te,
  yN as ne,
  zV as d,
} from "./app-initial~app-main~worktree-init-v2-page~remote-conversation-page~pull-requests-page~plug~kmtatxxf.js";
import {
  An as f,
  Em as re,
  SD as p,
  Tm as m,
  _y as h,
  kn as g,
  vy as _,
  xD as v,
} from "./app-initial~app-main~onboarding-page.js";
import {
  A as y,
  T as b,
  j as x,
  w as ie,
} from "./app-initial~app-main~worktree-init-v2-page~appgen-publication-terms-route~remote-conversati~coa484up.js";
var S,
  ae,
  C = e(() => {
    (t(d()),
      (S = c()),
      (ae = (e) =>
        (0, S.jsxs)(`svg`, {
          width: 20,
          height: 20,
          viewBox: `0 0 20 20`,
          fill: `currentColor`,
          xmlns: `http://www.w3.org/2000/svg`,
          ...e,
          children: [
            (0, S.jsx)(`path`, {
              fillRule: `evenodd`,
              clipRule: `evenodd`,
              d: `M3.74709 9.4286C4.09594 9.54326 4.47122 9.35358 4.58596 9.00478C5.08292 7.49281 6.19702 6.25947 7.62893 5.60243C8.06385 6.47457 8.96218 7.07502 10.003 7.07509C11.4688 7.07509 12.6572 5.88666 12.6572 4.42079C12.6572 2.9549 11.4688 1.76649 10.003 1.76649C8.58666 1.76659 7.43205 2.87614 7.35549 4.27333C5.45427 5.04691 3.9686 6.62635 3.32326 8.58974C3.20868 8.9386 3.39823 9.31394 3.74709 9.4286ZM11.3272 4.42079C11.3272 3.68944 10.7343 3.09657 10.003 3.09657C9.27168 3.09667 8.67873 3.6895 8.67873 4.42079C8.67875 5.15207 9.2717 5.74491 10.003 5.74501C10.7343 5.74501 11.3272 5.15213 11.3272 4.42079Z`,
            }),
            (0, S.jsx)(`path`, {
              fillRule: `evenodd`,
              clipRule: `evenodd`,
              d: `M14.4619 7.23427C15.2394 8.2087 15.7041 9.4424 15.7041 10.786C15.7041 10.9669 15.6951 11.1459 15.6787 11.3222C14.7046 11.2611 13.734 11.7399 13.2129 12.6425C12.4802 13.9118 12.9144 15.5346 14.1836 16.2675C15.453 17.0003 17.0767 16.5651 17.8096 15.2958C18.5164 14.0711 18.1344 12.5197 16.9668 11.7538C17.0104 11.4372 17.0332 11.114 17.0332 10.786C17.0332 9.13002 16.4602 7.6065 15.502 6.40517L15.3057 6.16982L15.2071 6.07704C14.9612 5.8895 14.6094 5.89431 14.3672 6.10536C14.1251 6.31651 14.0723 6.66436 14.2246 6.93349L14.3028 7.04286L14.4619 7.23427ZM16.1729 12.8222C15.5397 12.4566 14.7299 12.6743 14.3643 13.3075C13.999 13.9406 14.2158 14.7495 14.8487 15.1151C15.4819 15.4807 16.2917 15.264 16.6572 14.6308C17.0227 13.9976 16.806 13.1877 16.1729 12.8222Z`,
            }),
            (0, S.jsx)(`path`, {
              fillRule: `evenodd`,
              clipRule: `evenodd`,
              d: `M6.78713 12.6425C6.05428 11.3731 4.43147 10.9379 3.16213 11.6708C1.89304 12.4036 1.45791 14.0266 2.19045 15.2958C2.89893 16.5229 4.438 16.9667 5.68654 16.3329C6.87794 17.2613 8.37572 17.8172 10.003 17.8173C10.4526 17.8173 10.8932 17.7743 11.3203 17.6933C11.6812 17.6248 11.9181 17.2767 11.8496 16.9159C11.781 16.5553 11.433 16.3182 11.0723 16.3866C10.7265 16.4522 10.3689 16.4872 10.003 16.4872C8.77108 16.4872 7.63223 16.0956 6.70022 15.4315C7.23668 14.6191 7.30697 13.5429 6.78713 12.6425ZM5.63576 13.3075C5.27019 12.6743 4.46037 12.4566 3.82717 12.8222C3.19401 13.1877 2.97733 13.9976 3.3428 14.6308C3.70838 15.264 4.51818 15.4807 5.15139 15.1151C5.78423 14.7495 6.00103 13.9406 5.63576 13.3075Z`,
            }),
          ],
        })));
  });
function oe({ animation: e, animationData: t, fallback: n }) {
  let r = { animationJson: t ? Promise.resolve(t) : null, SvgFallback: n || null };
  return (e && ((r.animationJson = w[e]()), (r.SvgFallback = se[e])), r);
}
var w,
  se,
  T = e(() => {
    (p(),
      _(),
      ee(),
      re(),
      b(),
      C(),
      n(),
      f(),
      x(),
      l(),
      (w = {
        "analyze-image": () =>
          s(
            () => import(`./analyze_image_animation.js`).then((e) => e.default),
            __vite__mapDeps([0, 1]),
            import.meta.url,
          ),
        automation: () =>
          s(
            () => import(`./automation.js`).then((e) => e.default),
            __vite__mapDeps([2, 1]),
            import.meta.url,
          ),
        browsing: () =>
          s(
            () => import(`./browsing_animation.js`).then((e) => e.default),
            __vite__mapDeps([3, 1]),
            import.meta.url,
          ),
        "code-searching": () =>
          s(
            () => import(`./code-searching-icon.js`).then((e) => e.default),
            __vite__mapDeps([4, 1]),
            import.meta.url,
          ),
        "codex-happy-small": () =>
          s(
            () => import(`./codex-happy-small.js`).then((e) => e.default),
            __vite__mapDeps([5, 1]),
            import.meta.url,
          ),
        "codex-looking-around": () =>
          s(
            () => import(`./codex-looking-around.js`).then((e) => e.default),
            __vite__mapDeps([6, 1]),
            import.meta.url,
          ),
        "edit-files": () =>
          s(
            () => import(`./edit_files_animation.js`).then((e) => e.default),
            __vite__mapDeps([7, 1]),
            import.meta.url,
          ),
        hello: () =>
          s(
            () => import(`./hello.js`).then((e) => e.default),
            __vite__mapDeps([8, 1]),
            import.meta.url,
          ),
        loader: () =>
          s(
            () => import(`./loader-Cr_-5mLI.js`).then((e) => e.default),
            __vite__mapDeps([9, 1]),
            import.meta.url,
          ),
        "internal-knowledge": () =>
          s(
            () => import(`./internal-knowledge-icon.js`).then((e) => e.default),
            __vite__mapDeps([10, 1]),
            import.meta.url,
          ),
        "list-files": () =>
          s(
            () => import(`./list_files_animation.js`).then((e) => e.default),
            __vite__mapDeps([11, 1]),
            import.meta.url,
          ),
        "local-context": () =>
          s(
            () => import(`./local_context_animation.js`).then((e) => e.default),
            __vite__mapDeps([12, 1]),
            import.meta.url,
          ),
        "run-command": () =>
          s(
            () => import(`./run_command_animation.js`).then((e) => e.default),
            __vite__mapDeps([13, 1]),
            import.meta.url,
          ),
        searching: () =>
          s(
            () => import(`./searching_animation.js`).then((e) => e.default),
            __vite__mapDeps([14, 1]),
            import.meta.url,
          ),
        "to-do": () =>
          s(
            () => import(`./to_do_animation.js`).then((e) => e.default),
            __vite__mapDeps([15, 1]),
            import.meta.url,
          ),
        "web-search": () =>
          s(
            () => import(`./web-search-icon.js`).then((e) => e.default),
            __vite__mapDeps([16, 1]),
            import.meta.url,
          ),
      }),
      (se = {
        "analyze-image": ie,
        automation: v,
        browsing: o,
        "code-searching": y,
        "codex-happy-small": h,
        "codex-looking-around": h,
        "edit-files": y,
        hello: m,
        loader: h,
        "internal-knowledge": ae,
        "list-files": y,
        "local-context": r,
        "run-command": y,
        searching: r,
        "to-do": g,
        "web-search": o,
      }));
  });
function ce(e) {
  let t = getComputedStyle(e).color;
  if (!t) return null;
  if (j === void 0) {
    let e = document.createElement(`canvas`);
    ((e.width = 1), (e.height = 1), (j = e.getContext(`2d`, { willReadFrequently: !0 })));
  }
  return j
    ? (j.clearRect(0, 0, 1, 1),
      (j.fillStyle = t),
      j.fillRect(0, 0, 1, 1),
      Array.from(j.getImageData(0, 0, 1, 1).data, (e) => e / 255))
    : null;
}
function le(e, t) {
  let n = `${e}:${t}`,
    r = M.get(n);
  if (r !== void 0) return r;
  let i = document.createElement(`div`);
  switch (e) {
    case `color`:
      i.style.color = t;
      break;
    case `tokenColor`:
      i.className = t;
      break;
  }
  document.body.appendChild(i);
  let a = ce(i);
  return (i.remove(), M.set(n, a), a);
}
function ue(e) {
  return (
    N.add(e),
    P ||
      ((P = new MutationObserver(() => {
        (M.clear(),
          N.forEach((e) => {
            e();
          }));
      })),
      P.observe(document.documentElement, { attributes: !0, attributeFilter: [`class`, `style`] })),
    e(),
    () => {
      (N.delete(e), N.size === 0 && (P?.disconnect(), (P = null), M.clear()));
    }
  );
}
function de({ animationData: e, color: t }) {
  if (!t) return { size: { width: 100, height: 100 }, data: e };
  let n = structuredClone(e),
    r = n?.w && n?.h ? { width: n.w, height: n.h } : { width: 100, height: 100 },
    i = (e) => {
      ((e.ty === `fl` || e.ty === `st`) && e?.c?.k && (e.c.k = t),
        Array.isArray(e.it) && e.it.forEach(i));
    },
    a = (e) => {
      (e.ty === 4 && Array.isArray(e.shapes) && e.shapes.forEach(i),
        Array.isArray(e.layers) && e.layers.forEach(a));
    };
  return (a(n), Array.isArray(n.assets) && n.assets.forEach(a), { size: r, data: n });
}
function fe(e) {
  let t = (0, me.c)(71),
    {
      animation: n,
      animationData: r,
      fallbackSvg: i,
      animated: a,
      size: o,
      color: s,
      tokenColor: c,
      matchTextColor: l,
      loop: ee,
      mainDotLottieRefCallback: u,
      renderer: d,
      segment: f,
      showFallbackWhileLoading: re,
      className: p,
    } = e,
    m = a === void 0 ? !0 : a,
    h = o === void 0 ? `md` : o,
    g = l === void 0 ? !0 : l,
    _ = ee === void 0 ? !0 : ee,
    v = d === void 0 ? `worker` : d,
    y = re === void 0 ? !0 : re,
    b = (0, E.useRef)(null),
    [x, ie] = (0, E.useState)(null),
    S;
  t[0] !== n || t[1] !== r || t[2] !== i
    ? ((S = oe({ animation: n, animationData: r, fallback: i })),
      (t[0] = n),
      (t[1] = r),
      (t[2] = i),
      (t[3] = S))
    : (S = t[3]);
  let { SvgFallback: ae, animationJson: C } = S,
    [w, se] = (0, E.useState)(null),
    [T, fe] = (0, E.useState)(null),
    O = ne(),
    k = typeof h == `number` ? `` : he[h],
    A;
  t[4] === h
    ? (A = t[5])
    : ((A = typeof h == `number` ? { width: h, height: h } : {}), (t[4] = h), (t[5] = A));
  let j = A,
    M = typeof h == `number` ? `` : ge[h],
    N;
  t[6] !== h || t[7] !== j
    ? ((N = typeof h == `number` ? j : {}), (t[6] = h), (t[7] = j), (t[8] = N))
    : (N = t[8]);
  let P = N,
    F;
  t[9] !== p || t[10] !== M || t[11] !== c
    ? ((F = te(M, p, c, `flex items-center justify-center`)),
      (t[9] = p),
      (t[10] = M),
      (t[11] = c),
      (t[12] = F))
    : (F = t[12]);
  let I = F,
    L,
    R;
  (t[13] === C
    ? ((L = t[14]), (R = t[15]))
    : ((L = () => {
        let e = new AbortController();
        return (
          (async function () {
            let t = await C;
            e.signal.aborted ||
              (0, E.startTransition)(() => {
                fe(t);
              });
          })(),
          () => {
            e.abort();
          }
        );
      }),
      (R = [C]),
      (t[13] = C),
      (t[14] = L),
      (t[15] = R)),
    (0, E.useEffect)(L, R));
  let _e = (0, E.useRef)(null),
    z,
    B;
  (t[16] !== s || t[17] !== g || t[18] !== c
    ? ((z = () => {
        let e = b.current,
          t,
          n = (e) => {
            let n = e?.join(`,`) ?? null;
            t !== n &&
              ((t = n),
              (0, E.startTransition)(() => {
                ie(e);
              }));
          };
        if (s)
          return ue(() => {
            n(le(`color`, s));
          });
        if (c)
          return ue(() => {
            n(le(`tokenColor`, c));
          });
        if (!g || !e) {
          n(null);
          return;
        }
        let r = () => {
            let e = b.current;
            n(e ? ce(e) : null);
          },
          i = ue(r),
          a = new MutationObserver(r);
        return (
          a.observe(e, { attributes: !0, attributeFilter: [`class`, `style`] }),
          () => {
            (i(), a.disconnect());
          }
        );
      }),
      (B = [s, c, g]),
      (t[16] = s),
      (t[17] = g),
      (t[18] = c),
      (t[19] = z),
      (t[20] = B))
    : ((z = t[19]), (B = t[20])),
    (0, E.useEffect)(z, B));
  let V, H;
  (t[21] !== x || t[22] !== T
    ? ((V = () => {
        T &&
          (0, E.startTransition)(() => {
            se(de({ animationData: T, color: x }));
          });
      }),
      (H = [x, T]),
      (t[21] = x),
      (t[22] = T),
      (t[23] = V),
      (t[24] = H))
    : ((V = t[23]), (H = t[24])),
    (0, E.useEffect)(V, H));
  let ve = O || !m || (y && !w),
    U = null;
  if (!m || ve) {
    let e = ae;
    if (!e)
      throw Error(
        `Either provide 'fallbackSvg' prop or use an 'animation' type when animated=false`,
      );
    let n;
    t[25] !== s || t[26] !== j
      ? ((n = { ...j }),
        s &&
          (s === `white`
            ? (n.filter = `brightness(0) invert(1)`)
            : (n.filter = `hue-rotate(${s})`)),
        (t[25] = s),
        (t[26] = j),
        (t[27] = n))
      : (n = t[27]);
    let r = s ? n : void 0,
      i;
    t[28] !== e || t[29] !== r
      ? ((i = (0, D.jsx)(e, { className: `h-full w-full`, style: r })),
        (t[28] = e),
        (t[29] = r),
        (t[30] = i))
      : (i = t[30]);
    let a;
    if (
      (t[31] !== I || t[32] !== P || t[33] !== i
        ? ((a = (0, D.jsx)(`div`, { ref: b, className: I, style: P, children: i })),
          (t[31] = I),
          (t[32] = P),
          (t[33] = i),
          (t[34] = a))
        : (a = t[34]),
      (U = a),
      !m)
    )
      return U;
  }
  if (!O && !w && !y) {
    let e;
    return (
      t[35] !== I || t[36] !== P
        ? ((e = (0, D.jsx)(`div`, { ref: b, className: I, style: P })),
          (t[35] = I),
          (t[36] = P),
          (t[37] = e))
        : (e = t[37]),
      e
    );
  }
  let ye = ve ? `visible` : `hidden`,
    W;
  t[38] !== U || t[39] !== ye
    ? ((W = (0, D.jsx)(E.Activity, { mode: ye, children: U })),
      (t[38] = U),
      (t[39] = ye),
      (t[40] = W))
    : (W = t[40]);
  let be = ve ? `hidden` : `visible`,
    G;
  t[41] === k
    ? (G = t[42])
    : ((G = te(k, `flex items-center justify-center`)), (t[41] = k), (t[42] = G));
  let xe = w?.data ?? void 0,
    K;
  t[43] === w
    ? (K = t[44])
    : ((K = (e) => {
        if (!e || !w || typeof OffscreenCanvas > `u`) return;
        let t = _e.current;
        if (!t) {
          ((t = new OffscreenCanvas(w.size.width, w.size.height)), (_e.current = t));
          let e = t.getContext(`2d`, { alpha: !0, desynchronized: !0, willReadFrequently: !1 });
          e &&
            ((e.imageSmoothingEnabled = !0),
            (e.imageSmoothingQuality = `high`),
            (e.globalCompositeOperation = `source-over`));
        }
        e.setCanvas(t);
      }),
      (t[43] = w),
      (t[44] = K));
  let q;
  t[45] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((q = { autoResize: !0, freezeOnOffscreen: !0, quality: 20 }), (t[45] = q))
    : (q = t[45]);
  let Se = !O,
    J;
  t[46] !== _ ||
  t[47] !== u ||
  t[48] !== v ||
  t[49] !== f ||
  t[50] !== xe ||
  t[51] !== K ||
  t[52] !== Se
    ? ((J = (0, D.jsx)(pe, {
        data: xe,
        mainDotLottieRefCallback: u,
        dotLottieRefCallback: K,
        loop: _,
        renderer: v,
        segment: f,
        renderConfig: q,
        autoplay: Se,
        className: `pointer-events-none h-full w-full contain-[paint_style_layout_inline-size]`,
      })),
      (t[46] = _),
      (t[47] = u),
      (t[48] = v),
      (t[49] = f),
      (t[50] = xe),
      (t[51] = K),
      (t[52] = Se),
      (t[53] = J))
    : (J = t[53]);
  let Y;
  t[54] !== U || t[55] !== J
    ? ((Y = (0, D.jsx)(E.Suspense, { fallback: U, children: J })),
      (t[54] = U),
      (t[55] = J),
      (t[56] = Y))
    : (Y = t[56]);
  let X;
  t[57] !== j || t[58] !== G || t[59] !== Y
    ? ((X = (0, D.jsx)(`div`, { className: G, style: j, children: Y })),
      (t[57] = j),
      (t[58] = G),
      (t[59] = Y),
      (t[60] = X))
    : (X = t[60]);
  let Z;
  t[61] !== I || t[62] !== P || t[63] !== X
    ? ((Z = (0, D.jsx)(`div`, { ref: b, className: I, style: P, children: X })),
      (t[61] = I),
      (t[62] = P),
      (t[63] = X),
      (t[64] = Z))
    : (Z = t[64]);
  let Q;
  t[65] !== be || t[66] !== Z
    ? ((Q = (0, D.jsx)(E.Activity, { mode: be, children: Z })),
      (t[65] = be),
      (t[66] = Z),
      (t[67] = Q))
    : (Q = t[67]);
  let $;
  return (
    t[68] !== W || t[69] !== Q
      ? (($ = (0, D.jsxs)(D.Fragment, { children: [W, Q] })), (t[68] = W), (t[69] = Q), (t[70] = $))
      : ($ = t[70]),
    $
  );
}
function pe(e) {
  let t = (0, me.c)(13),
    n,
    r,
    i,
    a;
  switch (
    (t[0] === e
      ? ((n = t[1]), (r = t[2]), (i = t[3]), (a = t[4]))
      : (({ dotLottieRefCallback: n, mainDotLottieRefCallback: r, renderer: a, ...i } = e),
        (t[0] = e),
        (t[1] = n),
        (t[2] = r),
        (t[3] = i),
        (t[4] = a)),
    a)
  ) {
    case `main`: {
      let e = (0, E.use)(k),
        n;
      return (
        t[5] !== e || t[6] !== r || t[7] !== i
          ? ((n = (0, D.jsx)(e, { ...i, dotLottieRefCallback: r })),
            (t[5] = e),
            (t[6] = r),
            (t[7] = i),
            (t[8] = n))
          : (n = t[8]),
        n
      );
    }
    case `worker`: {
      let e = (0, E.use)(A),
        r;
      return (
        t[9] !== e || t[10] !== n || t[11] !== i
          ? ((r = (0, D.jsx)(e, { ...i, dotLottieRefCallback: n })),
            (t[9] = e),
            (t[10] = n),
            (t[11] = i),
            (t[12] = r))
          : (r = t[12]),
        r
      );
    }
  }
}
var me,
  E,
  D,
  O,
  k,
  A,
  he,
  ge,
  j,
  M,
  N,
  P,
  F = e(() => {
    ((me = i()),
      a(),
      (E = t(d(), 1)),
      u(),
      T(),
      (D = c()),
      l(),
      (O = s(
        () =>
          import(`./browser.js`).then(
            (e) => (
              e.setWasmUrl(`https://cdn.openai.com/common/wasm/dotlottie-player-017-13.wasm`),
              e
            ),
          ),
        __vite__mapDeps([17, 18, 1, 19]),
        import.meta.url,
      )),
      (k = O.then((e) => e.DotLottieReact)),
      (A = O.then((e) => e.DotLottieWorkerReact)),
      (he = { fill: `h-full w-full`, xs: `w-3 h-3`, sm: `w-4 h-4`, md: `w-5 h-5`, lg: `w-6 h-6` }),
      (ge = { fill: `h-full w-full`, xs: `w-3 h-3`, sm: `w-4 h-4`, md: `w-6 h-6`, lg: `w-8 h-8` }),
      (M = new Map()),
      (N = new Set()),
      (P = null));
  });
export { F as n, fe as t };
//# sourceMappingURL=animated-icon.js.map
