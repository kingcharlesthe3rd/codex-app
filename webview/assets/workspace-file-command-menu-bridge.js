...
import { f as fe, m as D, n as O, p as pe, s as me } from "./vscode-api.js";
import {
  Bt as k,
  J as he,
  a as ge,
...
import { c as De, o as Oe, r as ke } from "./lib.js";
import { t as Ae } from "./setting-storage.js";
import { t as je } from "./use-stable-callback.js";
import { r as Me } from "./tooltip.js";
import { D as Ne, O as Pe } from "./thread-side-panel-tabs-1.js";
import { f as j } from "./statsig.js";
import { t as Fe } from "./thread-context-inputs.js";
...
import { r as He } from "./thread-context.js";
import { t as Ue } from "./project-context-signal-N.js";
import { t as M } from "./app-intl-signal-D.js";
import { r as N } from "./use-register-command.js";
import { v as We } from "./interrupted-turn-state.js";
import { v as Ge } from "./sidebar-thread-row-signals-De-atrPX.js";
import { n as Ke, t as qe } from "./sidebar-thread-list-signals.js";
import { a as Je } from "./command-keybindings.js";
import { t as Ye } from "./search.js";
...
import { n as L, t as tt } from "./dist-12.js";
...
function dt({ sections: e, drop: t }) {
  return t == null
    ...
    : t.targetContainerId === `pinned` ||
      ...
      ? C({ sections: e, threadId: t.threadId })
...
var z = o(u, null),
  ft = o(u, null),
  pt = a(u, ({ get: e }) => e(j, `2413345355`)),
  mt = a(u, ({ get: e }) => e(g, A.SIDEBAR_CUSTOM_SECTIONS)),
  ht = a(u, ({ get: e }) => (e(pt) ? ue(e, A.SIDEBAR_CUSTOM_SECTIONS) : [])),
  gt = a(u, ({ get: e }) => e(mt).isFetched),
  B = a(u, ({ get: e }) => dt({ sections: e(ht), drop: e(ft) })),
  _t = a(u, ({ get: e }) => {
    let t = e(B),
      n = ot({
        sectionOrder: e(Ie),
  ...
  vt = d(u, ({ sectionId: e, currentThreadKey: t }) => {
    let n = [];
    return a(u, ({ get: r }) => {
      let i = r(B).find((t) => t.id === e);
      if (i == null) return ((n = []), n);
      let { allSidebarThreadKeys: a } = r(r(Ke, { currentConversationId: p(t) })),
        o = ie({ items: r(r(qe, a)), threadIds: i.threadIds, sortKey: i.sortKey });
      return ((n = yt(n, o)), n);
    });
  });
function yt(e, t) {
...
function wt({ order: e, tasks: t, threadId: n, beforeThreadId: r }) {
  ...
  return x(i, c) ? { threadIds: i } : { threadIds: c };
}
...
function kt({ currentCwd: e, targetGroup: t, worktreeWorkspaceRoot: n }) {
  ...
  return {
    ...
    ...(r == null ? {} : { path: r }),
    pendingCoreUpdate: r != null && _e(e ?? ``) !== _e(r),
  };
}
async function At(e, t, n) {
  ...
  try {
    return (
      await ce(`git`).request({
        method: `resolve-worktree-for-thread`,
        params: {
          cwd: r,
          conversationId: t,
          hostConfig: v(e.get, `host_config`) ?? ne(`local`, null),
    ...
    return (
      D.warning(`Failed to resolve an existing worktree for moved thread.`, {
        safe: {},
        sensitive: { error: k(e) },
      }),
      null
...
function Ft(e, t) {
  return G(e, (e) => {
    ...
    return r != null && x(r.threadIds, i.threadIds) ? n : { ...n, [t.projectId]: i };
  });
}
function It(e, t) {
  return Promise.all([
...
function Vt(e, t) {
  return e != null && x(e.threadIds, t.threadIds) ? e : t;
}
function Ht(e, t) {
...
function W(e, t) {
  let n = async () => {
      let { value: n } = await O(`get-global-state`, {
          params: { key: A.SIDEBAR_CHAT_THREAD_ORDER },
        }),
        r = ve(n) ?? null,
        i = t(r);
      i == null || i === r || (await h(e, A.SIDEBAR_CHAT_THREAD_ORDER, i));
    },
    r = jt.then(n, n);
  return ((jt = r.catch(() => {})), r);
}
function G(e, t) {
  let n = async () => {
      let { value: n } = await O(`get-global-state`, {
          params: { key: A.SIDEBAR_PROJECT_THREAD_ORDERS },
        }),
        r = ye(n),
        i = t(r);
      i !== r && (await h(e, A.SIDEBAR_PROJECT_THREAD_ORDERS, i));
    },
    r = Mt.then(n, n);
  return ((Mt = r.catch(() => {})), r);
}
...
function K(e, t) {
  let n = async () => {
      let { value: n } = await O(`get-global-state`, {
          params: { key: A.THREAD_PROJECT_ASSIGNMENTS },
        }),
        r = we(n);
      Se(r[t.threadId], t.assignment) ||
        (await h(
          e,
          A.THREAD_PROJECT_ASSIGNMENTS,
...
function qt(e, t) {
  let n = async () => {
      let { value: n } = await O(`get-global-state`, { params: { key: A.PROJECTLESS_THREAD_IDS } }),
        r = Wt.safeParse(n).data ?? [];
      r.includes(t.threadId) !== t.projectless &&
        (await h(
          e,
          A.PROJECTLESS_THREAD_IDS,
...
function Jt({
  ...
  let s = t.find((e) => e.threadKeys.includes(o.key)),
    c = Ce({ cwd: o.conversation.cwd ?? null, assignment: a?.[o.conversation.id] });
  if (s == null || !he(c ?? ``, r ?? void 0)) return null;
...
async function on(
  ...
  let u = r.find((e) => de(e.task.key) === o)?.task;
    ...
    if (u.kind === `remote`)
      return Y(
        Promise.all([
          ...
          Ft(e, {
            projectId: i.projectId,
            tasks: [...Ge(i.threadKeys, new Map(r.map((e) => [e.task.key, e]))), u],
            threadId: o,
            beforeThreadId: l,
          }),
          _(e, o),
  ...
  let d = b(e, { sectionId: c.slice(7), threadId: o, beforeThreadId: l });
  return J(s === `pinned` ? P(e, o, d) : d);
}
function J(e) {
  return e.catch((e) => {
    D.error(`Failed to save sidebar custom sections`, { safe: {}, sensitive: { error: k(e) } });
  });
}
function Y(e) {
  return e.catch((e) => {
    D.error(`Failed to save thread project assignment`, { safe: {}, sensitive: { error: k(e) } });
  });
}
var X = ke({
  moveTo: {
...
var Z = n(),
  dn = a(u, ({ get: e }) => {
    let t = e(j, `2553306736`);
    return Ae(e, Te.showSidebarPrIcons) ?? t;
  }),
  fn = s(u, (e, { get: t }) => (!t(dn) || e == null ? null : hn(e, t(M)))),
  pn = s(u, (e, { get: t }) => {
    if (!t(dn)) return null;
    let n = t(E, e),
      r = t(le, e),
    ...
    return a.type === `success` ? _n(a.data, t(M)) : null;
  });
function mn(e) {
...
function Sn(e) {
  ...
  if (
    ...
    let r = p ?? ``,
      i;
    t[12] === r ? (i = t[13]) : ((i = (0, Z.jsx)(Me, { keysLabel: r })), (t[12] = r), (t[13] = i));
    let a;
    t[14] !== n || t[15] !== d
      ? ((a = () => {
          (n(), d.set($, `files`));
  ...
  t[22] === Symbol.for(`react.memo_cache_sentinel`)
    ...
    : (w = t[22]);
  let T;
  t[23] !== r || t[24] !== b.emptyState || t[25] !== b.items || t[26] !== f || t[27] !== o
    ? ((T =
        b.items.length > 0
          ? b.items.map((e, t) =>
              (0, Z.jsx)(
...
function Tn() {
  ...
  (e[6] !== f || e[7] !== t
    ? ((h = () => {
        f &&
          (fe.dispatchHostMessage({ type: `command-menu`, query: `` }),
          t.set($, `files`),
          t.set(N, !0));
      }),
      (e[6] = f),
      ...
      (e[8] = h))
    : (h = e[8]),
    pe(`file-search-command-menu`, je(h)));
  let g;
