...
import { On as n, ds as r, ks as i, rs as a } from "./app-server-manager-signals.js";
import { m as o } from "./vscode-api.js";
import { d as s } from "./isEqual.js";
import { J as c, ar as l } from "./src-1.js";
import { n as u } from "./remote-projects.js";
import { t as d } from "./_baseEach-BPh-9Ri0.js";
...
function O(e, t, n) {
  let r = (0, x.default)(Object.values(e?.canonicalPathByRoot ?? {}), l),
    i = (0, b.default)(t.map((e) => [l(e.dir), e]));
  return (e?.roots ?? []).map((a) => {
    let o = l(a),
      s = e?.canonicalPathByRoot?.[a],
      u = l(s ?? a),
      d = s != null && u !== o && r[u]?.length === 1 ? s : void 0,
      p = c(a, n),
...
function M(e, t, n, r, i, a) {
  let o = (0, b.default)(
    ...
    c = new Map();
  for (let e of n) e.projectKind === `local` && L(e) && c.set(l(e.path), e);
  for (let e of n) {
  ...
  return (
    e.forEach((e) => {
      if (e.kind === `local`) B(e, n, c, r, a?.gitOriginsByHostId, i, a?.primaryHostId, a);
        ...
        Y(e, s, n);
      } else e.kind === `pending-worktree` && J(e, o, n, c);
    }),
    n
  );
}
...
function P(e, t) {
  let n = l(e).replace(/\/+$/, ``);
  return t.find((e) => l(e.dir).replace(/\/+$/, ``) === n) ?? null;
}
function F(e, t) {
  let n = P(e, t ?? []);
  if (!n?.root) return [];
...
function R(e, t) {
  return e.get(l(t)) ?? null;
}
function z(e, t) {
  return e.has(l(t));
}
var B = (e, t, n, i, a, s, l = r, d) => {
  let f = e.cwd;
  if (!f || !I(f).length) {
    o.warning(`No cwd found for local task`, {
  ...
  if (
    ...
    return;
  let y = W({ gitOrigins: i, gitOriginsByHostId: a, hostId: m ?? void 0, primaryHostId: l });
  if (c(f, s) || (_ && G(f, y))) {
...
function H(e, t, n, r, i) {
  ...
  for (let n of e) {
    if (n.kind === `local`) {
      if ((n.summary != null && !c(n.cwd, i)) || n.workspaceKind === `projectless` || n.cwd === `~`)
        continue;
      let e = n.hostId ?? t,
        r = n.cwd;
...
function U(e, t) {
  let n = new Map(t.map(({ hostId: e, dirs: t }) => [e, new Set(t.map(l))]));
  return e
    .map(({ hostId: e, dirs: t }) => ({ hostId: e, dirs: t.filter((t) => !n.get(e)?.has(l(t))) }))
...
function G(e, t) {
  let n = P(e, t ?? []);
  return n?.commonDir
    ? l(n.commonDir).replace(/\/+$/, ``) !== `${l(n.root).replace(/\/+$/, ``)}/.git`
    : !1;
}
function K(e, t, n, r, i, a, o = !1) {
  ...
  let u = P(e, i);
  if (!u) return c?.path ?? null;
  let d = u.originUrl ?? null,
    f = u.commonDir ?? null;
  if (!d && !f) return o ? (c?.path ?? null) : null;
  ...
  if (w) return w.path;
  let T = g[0];
  return T ? T.path : o ? (c?.path ?? null) : null;
}
function q(e, t) {
...
function J(e, t, n, r) {
  ...
  if (!a) {
    o.warning(`No original clone cwd found for pending worktree task`, {
  ...
  let s = t[l(a)] ?? null,
    c =
      i.hostId === `local`
        ? R(r, a)
        : (n.find(
            (e) => e.projectKind === `remote` && e.hostId === i.hostId && l(e.path) === l(a),
          ) ?? null);
  c &&
...
function Y(e, t, n) {
  let r = Z(e, t),
    i = Q(r);
  if (!i) {
    o.warning(`No owner repo found for remote task`, {
