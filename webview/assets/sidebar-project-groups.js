...
import { r as n } from "./use-host-config-C.js";
import { wi as r } from "./thread-context-inputs.js";
import { h as i } from "./vscode-api.js";
import { _ as a } from "./isEqual.js";
import { J as o, jr as s } from "./src-1.js";
import { a as c } from "./markdown-to-search-text.js";
import { n as l } from "./remote-projects.js";
import { t as u } from "./_baseEach.js";
import { n as d } from "./parse-owner-repo.js";
...
function O({ groups: e, hiddenTaskKeys: t }) {
  return t.size === 0
    ? e
    : e.flatMap((e) => {
...
function k(e, t, n) {
  let r = (0, x.default)(Object.values(e?.canonicalPathByRoot ?? {}), s),
    i = (0, b.default)(t.map((e) => [s(e.dir), e]));
  return (e?.roots ?? []).map((a) => {
    let c = s(a),
      l = e?.canonicalPathByRoot?.[a],
      u = s(l ?? a),
      f = l != null && u !== c && r[u]?.length === 1 ? l : void 0,
      p = o(a, n),
      m = (e?.labels?.[a] ?? e?.labels?.[c] ?? ``).trim() || (0, S.default)(L(a)) || ``,
      h = i[c] ?? void 0,
      g = h?.originUrl ? d(h.originUrl) : null;
    if (h && j(h)) {
      let e = {
        ownerRepo: g,
        repoPath: I(a, t).join(`/`),
        rootFolder: (0, S.default)(L(h.root)) ?? ``,
      };
      return {
        projectId: a,
        projectKind: `local`,
...
function N(e, t, n, r, i, a) {
  let o = (0, b.default)(
    ...
    l = new Map();
  for (let e of n) e.projectKind === `local` && R(e) && l.set(s(e.path), e);
  for (let e of n) {
    if (e.projectKind !== `local` || !R(e) || e.pathAlias == null) continue;
    let t = s(e.pathAlias);
    l.has(t) || l.set(t, e);
  }
  return (
    e.forEach((e) => {
      if (e.kind === `local`) V(e, n, l, r, a?.gitOriginsByHostId, i, a?.primaryHostId, a);
      else if (e.kind === `remote`) {
        let t = H(a?.threadProjectAssignments?.[e.task.id], n);
        if (t != null) {
        ...
        X(e, c, n);
      } else e.kind === `pending-worktree` && Y(e, o, n, l);
    }),
    n
  );
}
...
function F(e, t) {
  let n = s(e).replace(/\/+$/, ``);
  return t.find((e) => s(e.dir).replace(/\/+$/, ``) === n) ?? null;
}
function I(e, t) {
  let n = F(e, t ?? []);
  if (!n?.root) return [];
...
function z(e, t) {
  return e.get(s(t)) ?? null;
}
function B(e, t) {
  return e.has(s(t));
}
var V = (e, t, r, a, s, c, u = n, d) => {
  let f = e.cwd;
  if (!f || !L(f).length) {
    i.warning(`No cwd found for local task`, {
  ...
  let p = f,
    ...
    g =
      h != null &&
      (h.projectKind === `local`
        ? e.hostId == null || e.hostId === u
        : h.hostId != null && (e.hostId ?? u) === h.hostId)
        ? H(h, t)
        : null;
  if (g != null) {
  ...
  let y = [];
  if (y.length === 1) {
    (t.find((e) => e.projectId === y[0]?.id) ?? null)?.threadKeys.push(e.key);
    return;
  }
  if (y.length > 1) return;
  let b = G({ gitOrigins: a, gitOriginsByHostId: s, hostId: m ?? void 0, primaryHostId: u });
  if (o(f, c) || (_ && K(f, b))) {
    let n = q(f, e.conversationId, t, r, b, d?.threadWorkspaceRootHints, e.summary != null);
    n && (p = n);
  }
  let x = v == null ? null : l(v, m ?? void 0, p);
  if (x != null) {
    let n = t.find((e) => e.projectId === x.id) ?? null;
    if (n != null) {
  ...
  let S = z(r, p);
  S &&
    (S.threadKeys.push(e.key),
    p !== f && d?.onDiscoverThreadWorkspaceRootHint?.(e.conversationId, S.path));
};
function H(e, t) {
  return e == null
    ? null
...
function U(e, t, n, r, i) {
  ...
  for (let n of e) {
    if (n.kind === `local`) {
      if ((n.summary != null && !o(n.cwd, i)) || n.workspaceKind === `projectless` || n.cwd === `~`)
        continue;
      let e = n.hostId ?? t,
        r = n.cwd;
...
function W(e, t) {
  let n = new Map(t.map(({ hostId: e, dirs: t }) => [e, new Set(t.map(s))]));
  return e
    .map(({ hostId: e, dirs: t }) => ({ hostId: e, dirs: t.filter((t) => !n.get(e)?.has(s(t))) }))
    .filter(({ dirs: e }) => e.length > 0);
}
function G({ gitOrigins: e, gitOriginsByHostId: t, hostId: n, primaryHostId: r }) {
  return n && t?.[n] ? t[n] : n && t && n !== r ? [] : (e ?? []);
}
function K(e, t) {
  let n = F(e, t ?? []);
  return n?.commonDir
    ? s(n.commonDir).replace(/\/+$/, ``) !== `${s(n.root).replace(/\/+$/, ``)}/.git`
    : !1;
}
function q(e, t, n, r, i, a, o = !1) {
  if (B(r, e)) return null;
  let c = a?.[t],
    l = c ? z(r, c) : null;
...
function Y(e, t, n, r) {
  ...
  if (!o) {
    i.warning(`No original clone cwd found for pending worktree task`, {
  ...
  let c = t[s(o)] ?? null,
    l =
      a.hostId === `local`
        ? z(r, o)
        : (n.find(
            (e) => e.projectKind === `remote` && e.hostId === a.hostId && s(e.path) === s(o),
          ) ?? null);
  l &&
    ((c && l.repositoryData?.ownerRepo != null && !P(l.repositoryData.ownerRepo, c)) ||
...
function X(e, t, n) {
  ...
  if (!a) {
    w.has(e.task.id) ||
      (w.add(e.task.id),
      i.warning(`No owner repo found for remote task`, {
  ...
  (
    n.find(
      (e) =>
        P(e.repositoryData?.ownerRepo, a) &&
        e.repositoryData?.repoPath === `` &&
        e.repositoryData?.rootFolder?.toLowerCase() === o,
    ) ??
    null ??
    n.find((e) => P(e.repositoryData?.ownerRepo, a)) ??
    Z(a, r, n)
