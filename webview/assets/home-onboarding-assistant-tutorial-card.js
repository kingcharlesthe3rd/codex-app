...
import { n as Oe, t as ke } from "./setup-codex-wizard-step-view-BS-WCe9O.js";
...
var P,
  ...
  W = e(() => {
    ((P = t(ye(), 1)),
      ...
      (L = h(I, {})),
      (R = T(g, `ambient-suggestions`, ({ projectRoot: e }) => ({ enabled: e != null }))),
      (z = T(
        g,
      ...
      (B = ne(g, (e) => null)),
      (V = D(g, (e, { get: t }) => Fe(Ie(t, e)))),
      (H = D(g, (e, { get: t }) => {
        let n = t(R, e);
        return n.data != null || n.isError;
      })),
      (U = D(g, ({ domain: e, hostId: t, projectRoot: n }, { get: r }) => {
...
function Be({ completed: e, domain: t, hostId: n, onLocalConversationCreated: r, projectRoot: i }) {
  let o = s(g),
    l = le(),
    {
      agentMode: u,
    ...
    } = ae({ conversationId: null, cwdOverride: i, hostId: n }),
    { activeMode: _ } = Ce(null),
    { serviceTierSettings: v } = Te(null),
    y = ee(De),
    b = je(),
    [x, S] = (0, J.useState)(!1),
    C = l.formatMessage({
    ...
    T = f || i == null;
  Oe(m.CODEX_ONBOARDING_WIZARD_PHASE_LAUNCH, !e);
  let E = async () => {
    if (!(T || e || x || i == null)) {
      (O(o, m.CODEX_ONBOARDING_WIZARD_PHASE_LAUNCH, c.CODEX_ONBOARDING_WIZARD_ACTION_COMPLETED),
        S(!0));
      try {
        let e = Ue(y.roles),
          a = Me(b, Z)?.replace(/^name:\s*.*$/m, `name: ${Q}`);
        (await We({
          ...
          e != null &&
            O(
              o,
              m.CODEX_ONBOARDING_WIZARD_PHASE_ROLE_PICKER,
              c.CODEX_ONBOARDING_WIZARD_ACTION_SKIPPED,
            ),
          A(o, { domain: t, hostId: n, projectRoot: i }, X, `accepted`));
      } catch {
        o.get(_e).danger(
          (0, Y.jsx)(he, {
  ...
  return (0, Y.jsxs)(`button`, {
    ...
    children: [
      (0, Y.jsx)(`span`, {
        className: a(`flex h-5 shrink-0 items-center`, e && `opacity-30`),
        children: (0, Y.jsx)(K, { "aria-hidden": !0, className: `size-5 shrink-0` }),
      }),
      (0, Y.jsxs)(`span`, {
        className: a(`flex w-full min-w-0 flex-col gap-1 text-base`, e && `opacity-30`),
        children: [
          (0, Y.jsx)(`span`, {
...
async function Ve({
  ...
  let o = await b(`install-recommended-skill`, {
    params: {
      forceReinstall: a,
      hostId: e,
      installRoot: null,
  ...
  return (
    t(),
    await k(`list-skills-for-host`, { forceReload: !0, hostId: e }),
    q.default.join(o.destination, `SKILL.md`)
  );
}
function He(e) {
  return de({ name: `Setup Codex`, path: e });
}
function Ue(e) {
...
async function We({
  ...
  let u = [o],
    ...
    p = await pe(u, { prompt: s }),
    m = p.cwd ?? o;
  if (d && p.projectlessOutputDirectory == null)
    throw Error(`No projectless output directory found`);
  let { config: h } = await k(`read-config-for-host`, { hostId: r, includeLayers: !1, cwd: m });
  await i(
