import { R as e, Xs as t, j as n, tt as r } from "./app-server-manager-signals.js";
...
import { n as l } from "./local-environment-selection.js";
async function u(a, { sourceConversationId: l, sourceWorkspaceRoot: u }) {
  ...
  try {
    ...
    return (
      i.error(`Error forking local task`, { safe: {}, sensitive: { error: e } }),
...
async function d(
  ...
  if ((m.origins.find((e) => e.dir === d)?.root ?? m.origins[0]?.root ?? null) == null)
    return (t.get(o).danger(t.get(s).formatMessage(c.forkThreadRequiresGitRepo)), null);
  let h = await f({ hostId: p, localEnvironmentSelectionsByWorkspace: l, sourceWorkspaceRoot: d }),
    g = t.get(s),
    _ = t.get(e, u);
  return i({
    hostId: p,
...
async function f({ hostId: e, localEnvironmentSelectionsByWorkspace: t, sourceWorkspaceRoot: n }) {
  let r = e ?? `local`;
  try {
...
//# sourceMappingURL=fork-conversation-actions.js.map
