...
import { Sn as c } from "./src-3.js";
import { l } from "./statsig.js";
...
function S({ accountStructure: e, plan: t, workspaceName: n }) {
  let r = U(n);
  return r != null && e?.toLowerCase() === `workspace` ? r : te(t);
}
function C(e) {
  let t = e.trim();
...
function E({ dailyUsage: e, todayIso: t, view: n }) {
  let r = I({ columnCount: A(t), dailyUsage: e, startDateIso: j(t), todayIso: t });
  switch (n) {
    case `cumulative`:
      return se(r);
    case `daily`:
...
function D({ dailyUsage: e, todayIso: t }) {
  let n = V(L(t), -175);
  return M(I({ columnCount: 26, dailyUsage: e, startDateIso: n, todayIso: t })).map((e, r) =>
    V(n, r) > t ? null : e,
  );
}
function O({ dailyUsage: e, todayIso: t }) {
  return I({ columnCount: A(t), dailyUsage: e, startDateIso: j(t), todayIso: t });
}
function k({ dailyUsage: e, todayIso: t }) {
  let n = j(t),
    r = A(t),
...
function re({ dailyUsage: e, todayIso: t }) {
  return P(k({ dailyUsage: e, todayIso: t }));
}
function ie(e = new Date()) {
...
function A(e) {
  let t = new Date(`${L(e)}T00:00:00.000Z`).getTime(),
    n = new Date(`${m}T00:00:00.000Z`).getTime(),
    r = Math.floor((t - n) / (10080 * 60 * 1e3));
  return Math.min(p, Math.max(1, r + 1));
}
function j(e) {
  return V(L(e), -(A(e) - 1) * 7);
}
function ae(e, t) {
  return V(j(t), e);
}
...
function I({ columnCount: e, dailyUsage: t, startDateIso: n, todayIso: r }) {
  let i = B(t.filter((e) => e.date <= r));
  return Array.from({ length: e * 7 }, (e, t) => i.get(V(n, t)) ?? 0);
}
function L(e) {
...
var W = e(),
  ...
  ce = `profile_usage_warm_fetch_enabled`,
...
function le(e) {
  let t = (0, W.c)(13),
    { accountId: n, enabled: a, userId: c } = e,
    u = l(G),
    d;
  ...
  let f = d,
    p;
  t[2] !== n || t[3] !== c
    ? ((p = J({ accountId: n, userId: c })), (t[2] = n), (t[3] = c), (t[4] = p))
    : (p = t[4]);
  let m = a && !0,
    h;
  t[5] !== n || t[6] !== c
...
function ue() {
  let e = (0, W.c)(2),
    t = l(G),
    n;
  return (e[0] === t ? (n = e[1]) : ((n = t.get(ce, !0)), (e[0] = t), (e[1] = n)), n);
}
function de(e) {
  let r = (0, W.c)(6),
...
async function me() {
  let e = await d.safeGet(`/wham/profiles/me`);
  return {
...
async function he(e) {
  return Q(await Z({ profile_asset_pointer: await Y(e) }));
}
async function Y(e) {
  let t = ye();
  return (
    await a
      .getInstance()
      .post(`/wham/profiles/me/photo`, u(await ve(e, t)), {
...
async function ge(e) {
  ...
  if ((e.displayName != null && (t.display_name = e.displayName), e.photo != null))
    try {
      t.profile_asset_pointer = await Y(e.photo);
    } catch (e) {
      throw new q(e);
    }
  return Z(t);
...
async function Z(e) {
  return d.safePatch(`/wham/profiles/me`, { requestBody: e });
}
function _e(e, t, n) {
