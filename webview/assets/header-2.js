import { n as e, s as t } from "./rolldown-runtime.js";
import {
  At as n,
  Bt as r,
  Ct as i,
  El as a,
  Ft as o,
  Pt as s,
  Tl as c,
  Tt as l,
  Xs as u,
  ec as d,
  el as f,
  kt as p,
  nc as m,
  nl as h,
  uo as g,
  vl as _,
} from "./app-initial~app-main~worktree-init-v2-page~appgen-settings-page~page~appgen-publication-ter~fzo5ij6p.js";
import {
  Eb as v,
  Hd as y,
  Pf as b,
  Ud as x,
  bg as S,
  gh as C,
  hh as w,
  jf as T,
  o_ as E,
  r_ as D,
  s_ as O,
  wb as k,
  xg as A,
} from "./app-initial~app-main~onboarding-page.js";
import {
  $ as ee,
  E as j,
  G as M,
  O as N,
  S as te,
  W as P,
  dt as F,
  ot as ne,
  st as I,
  tt as L,
  ut as R,
  w as z,
} from "./app-initial~app-main~remote-conversation-page~new-thread-panel-page~onboarding-page~project~okyna5mq.js";
import {
  Cd as B,
  Dv as V,
  Ib as H,
  Kn as U,
  Md as W,
  P as G,
  Qs as re,
  Rb as ie,
  Rs as ae,
  Sx as K,
  V as oe,
  Xs as se,
  Yn as ce,
  Zs as le,
  b as ue,
  hy as de,
  jd as fe,
  nx as pe,
  o_ as me,
  py as he,
  q as ge,
  rx as _e,
  u_ as ve,
  vt as ye,
  vx as be,
  wd as xe,
  wx as Se,
  x as Ce,
  yx as we,
} from "./app-initial~app-main~new-thread-panel-page.js";
import {
  Al as Te,
  Cu as Ee,
  Du as De,
  Eu as Oe,
  Ll as ke,
  Qr as Ae,
  Su as je,
  Tn as Me,
  Xl as Ne,
  Yr as Pe,
  Zl as Fe,
  Zr as Ie,
  _a as Le,
  aa as Re,
  d as ze,
  ga as Be,
  h as Ve,
  ia as He,
  il as Ue,
  ql as We,
  qr as Ge,
  ra as Ke,
  rl as qe,
  ua as Je,
  wn as Ye,
} from "./app-initial~app-main~remote-conversation-page~new-thread-panel-page~onboarding-page~appgen-~o4yhvtva.js";
import {
  n as Xe,
  t as Ze,
} from "./app-initial~app-main~remote-conversation-page~hotkey-window-thread-page~keyboard-shortcuts-~n7jwlpf0.js";
import {
  $ as Qe,
  F as $e,
  P as et,
  Q as tt,
  Z as q,
  et as nt,
} from "./app-initial~app-main~worktree-init-v2-page~remote-conversation-page~pull-requests-page~new-~djgpfzje.js";
import { P as rt } from "./app-initial~app-main~worktree-init-v2-page~remote-conversation-page~pull-requests-page~new-~mam8fk3d.js";
import {
  _ as it,
  m as at,
} from "./app-initial~app-main~worktree-init-v2-page~appgen-page~remote-conversation-page~pull-reques~filfqv6y.js";
import {
  i as ot,
  r as st,
} from "./app-initial~app-main~appgen-page~remote-conversation-page~plugin-detail-page~onboarding-pag~f3o4q9dk.js";
import {
  H as ct,
  W as lt,
} from "./app-initial~app-main~remote-conversation-page~plugin-detail-page~new-thread-panel-page~appg~ijdupmx5.js";
import {
  d as ut,
  u as dt,
} from "./app-initial~app-main~projects-index-page~hotkey-window-new-thread-page~hotkey-window-home-p~hswrsggc.js";
import {
  c as ft,
  l as pt,
  n as mt,
  s as ht,
  t as gt,
  u as _t,
} from "./app-initial~app-main~remote-conversation-page~projects-index-page~hotkey-window-thread-page~hc7acb17.js";
import { n as vt, t as yt } from "./dock.js";
function bt(e) {
  let t = (0, Dt.c)(27),
    { conversationId: n } = e,
    r = Se(),
    i,
    a;
  t[0] === r
    ? ((i = t[1]), (a = t[2]))
    : ((i = Et()),
      (a = i == null ? null : r.formatMessage(jt.extensionVersion, { version: i })),
      (t[0] = r),
      (t[1] = i),
      (t[2] = a));
  let o = a,
    s;
  t[3] === r ? (s = t[4]) : ((s = r.formatMessage(jt.codexForChrome)), (t[3] = r), (t[4] = s));
  let c = s,
    l = o == null ? c : `${c} ${o}`,
    u = Ct,
    d = St,
    f = xt,
    p;
  t[5] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((p = (0, J.jsx)(K, { ...jt.moreActionsTrigger })), (t[5] = p))
    : (p = t[5]);
  let m;
  t[6] === r ? (m = t[7]) : ((m = r.formatMessage(jt.moreActionsTrigger)), (t[6] = r), (t[7] = m));
  let h;
  t[8] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((h = (0, J.jsx)(st, { className: `icon-xs` })), (t[8] = h))
    : (h = t[8]);
  let g;
  t[9] === m
    ? (g = t[10])
    : ((g = (0, J.jsx)(ee, {
        tooltipContent: p,
        children: (0, J.jsx)(q, { color: `ghost`, size: `icon`, "aria-label": m, children: h }),
      })),
      (t[9] = m),
      (t[10] = g));
  let _;
  t[11] === n
    ? (_ = t[12])
    : ((_ =
        n == null
          ? null
          : (0, J.jsx)(z.Item, {
              RightIcon: S,
              onSelect: () => {
                wt(`codex://threads/${n}`);
              },
              children: (0, J.jsx)(K, {
                id: `header.openInApp`,
                defaultMessage: `Open in app`,
                description: `Menu item label for opening the active Chrome extension thread in the Codex desktop app`,
              }),
            })),
      (t[11] = n),
      (t[12] = _));
  let v;
  t[13] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((v = (0, J.jsx)(z.Item, {
        RightIcon: S,
        onSelect: d,
        children: (0, J.jsx)(K, { ...jt.appSettings }),
      })),
      (t[13] = v))
    : (v = t[13]);
  let y;
  t[14] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((y = (0, J.jsx)(z.Item, {
        RightIcon: S,
        onSelect: f,
        children: (0, J.jsx)(K, { ...jt.chromeComputerUseSettings }),
      })),
      (t[14] = y))
    : (y = t[14]);
  let b;
  t[15] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((b = (0, J.jsx)(K, { ...jt.codexForChrome })), (t[15] = b))
    : (b = t[15]);
  let x;
  t[16] === i
    ? (x = t[17])
    : ((x =
        i == null
          ? null
          : (0, J.jsx)(`span`, {
              className: `shrink-0`,
              children: (0, J.jsx)(K, { ...jt.extensionVersion, values: { version: i } }),
            })),
      (t[16] = i),
      (t[17] = x));
  let C;
  t[18] === x
    ? (C = t[19])
    : ((C = (0, J.jsxs)(`span`, {
        className: `flex w-full items-center justify-between gap-4`,
        children: [b, x],
      })),
      (t[18] = x),
      (t[19] = C));
  let w;
  t[20] !== l || t[21] !== C
    ? ((w = (0, J.jsx)(z.Item, {
        "aria-label": l,
        className: `text-token-foreground/50 hover:!bg-transparent focus:!bg-transparent`,
        onSelect: u,
        children: C,
      })),
      (t[20] = l),
      (t[21] = C),
      (t[22] = w))
    : (w = t[22]);
  let T;
  return (
    t[23] !== w || t[24] !== g || t[25] !== _
      ? ((T = (0, J.jsxs)(te, {
          align: `end`,
          contentWidth: `menuBounded`,
          triggerButton: g,
          children: [_, v, y, w],
        })),
        (t[23] = w),
        (t[24] = g),
        (t[25] = _),
        (t[26] = T))
      : (T = t[26]),
    T
  );
}
function xt() {
  wt(kt);
}
function St() {
  wt(Ot);
}
function Ct() {
  wt(Tt());
}
function wt(e) {
  it({ href: e, initiator: `open_in_browser_bridge` });
}
function Tt() {
  return globalThis.chrome?.runtime?.getURL(At) ?? At;
}
function Et() {
  return globalThis.chrome?.runtime?.getManifest?.().version ?? null;
}
var Dt,
  J,
  Ot,
  kt,
  At,
  jt,
  Mt = e(() => {
    ((Dt = c()),
      we(),
      tt(),
      N(),
      at(),
      L(),
      A(),
      ot(),
      (J = _()),
      (Ot = `codex://settings`),
      (kt = `codex://settings/computer-use/google-chrome`),
      (At = `popup.html`),
      (jt = be({
        moreActionsTrigger: {
          id: `header.moreActionsTrigger`,
          defaultMessage: `More actions`,
          description: `Button label for opening more actions from the Chrome extension side panel header`,
        },
        appSettings: {
          id: `header.appSettings`,
          defaultMessage: `App settings`,
          description: `Menu item label for opening app settings in the Codex desktop app from the Chrome extension`,
        },
        chromeComputerUseSettings: {
          id: `header.chromeComputerUseSettings`,
          defaultMessage: `Chrome computer use settings`,
          description: `Menu item label for opening Chrome computer use settings in the Codex desktop app from the Chrome extension`,
        },
        codexForChrome: {
          id: `header.codexForChrome`,
          defaultMessage: `Codex for Chrome`,
          description: `Footer label for the Chrome extension in the more actions menu`,
        },
        extensionVersion: {
          id: `header.extensionVersion`,
          defaultMessage: `v{version}`,
          description: `Footer label for the Chrome extension version in the settings menu`,
        },
      })));
  });
function Nt() {
  let e = (0, Pt.c)(12),
    t = lt(),
    n = Se(),
    r = d(ze, `newThread`),
    i;
  e[0] === t
    ? (i = e[1])
    : ((i = (e) => {
        e.defaultPrevented || t();
      }),
      (e[0] = t),
      (e[1] = i));
  let a = i,
    o;
  e[2] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((o = (0, Ft.jsx)(K, { ...It.newChat })), (e[2] = o))
    : (o = e[2]);
  let s;
  e[3] === n ? (s = e[4]) : ((s = n.formatMessage(It.newChat)), (e[3] = n), (e[4] = s));
  let c;
  e[5] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((c = (0, Ft.jsx)(dt, { className: `icon-xs` })), (e[5] = c))
    : (c = e[5]);
  let l;
  e[6] !== a || e[7] !== s
    ? ((l = (0, Ft.jsx)(q, {
        color: `ghost`,
        size: `icon`,
        onClick: a,
        "aria-label": s,
        children: c,
      })),
      (e[6] = a),
      (e[7] = s),
      (e[8] = l))
    : (l = e[8]);
  let u;
  return (
    e[9] !== r || e[10] !== l
      ? ((u = (0, Ft.jsx)(ee, { tooltipContent: o, shortcut: r, children: l })),
        (e[9] = r),
        (e[10] = l),
        (e[11] = u))
      : (u = e[11]),
    u
  );
}
var Pt,
  Ft,
  It,
  Lt = e(() => {
    ((Pt = c()),
      u(),
      we(),
      Ve(),
      tt(),
      L(),
      ct(),
      ut(),
      (Ft = _()),
      (It = be({
        newChat: {
          id: `localConversationPage.newChat`,
          defaultMessage: `New chat`,
          description: `Label for starting a new chat`,
        },
      })));
  });
function Rt(e, t) {
  switch (e.kind) {
    case `remote`:
      return t.remote(e);
    case `local`:
      return t.local(e);
    case `pending-worktree`:
      return t[`pending-worktree`](e);
  }
}
var zt = e(() => {});
function Bt(e) {
  let t = (0, Ut.c)(23),
    { mergedTasks: n, tasksQuery: r } = e,
    a = re(),
    o = se(),
    { cancelPendingWorktree: s } = E();
  if (r.isLoading && n.length === 0) return null;
  let c = r.error instanceof i && r.error.status === 404;
  if (r.isError && n.length === 0 && !c) {
    let e;
    t[0] === Symbol.for(`react.memo_cache_sentinel`)
      ? ((e = (0, Wt.jsx)(K, {
          id: `codex.recentTasksMenu.errorCloud.inline`,
          defaultMessage: `Failed to load cloud tasks.`,
          description: `Inline error indicator for cloud tasks in recent feed`,
        })),
        (t[0] = e))
      : (e = t[0]);
    let n;
    t[1] === r
      ? (n = t[2])
      : ((n = () => {
          r.refetch();
        }),
        (t[1] = r),
        (t[2] = n));
    let i;
    t[3] === Symbol.for(`react.memo_cache_sentinel`)
      ? ((i = (0, Wt.jsx)(K, {
          id: `codex.common.retry`,
          defaultMessage: `Retry`,
          description: `Retry button`,
        })),
        (t[3] = i))
      : (i = t[3]);
    let a;
    return (
      t[4] === n
        ? (a = t[5])
        : ((a = (0, Wt.jsxs)(`div`, {
            className: `mb-2 flex items-center gap-2 text-base text-token-input-placeholder-foreground`,
            children: [
              e,
              (0, Wt.jsx)(q, { size: `default`, color: `outline`, onClick: n, children: i }),
            ],
          })),
          (t[4] = n),
          (t[5] = a)),
      a
    );
  }
  if (n.length === 0) return null;
  let l;
  t[6] === n ? (l = t[7]) : (n.filter(Ht), (l = n), (t[6] = n), (t[7] = l));
  let u = l,
    d;
  if (t[8] !== s || t[9] !== o || t[10] !== a || t[11] !== u) {
    let e;
    (t[13] !== s || t[14] !== o || t[15] !== a
      ? ((e = (e) =>
          Rt(e, {
            remote: (e) =>
              (0, Wt.jsx)(
                pt,
                {
                  useStableTrailingRail: !0,
                  isActive: o.pathname === `/remote/${e.task.id}`,
                  task: e.task,
                  onClose: Vt,
                },
                e.key,
              ),
            local: (e) =>
              (0, Wt.jsx)(
                gt,
                {
                  useStableTrailingRail: !0,
                  isActive: o.pathname === `/local/${e.conversation.id}`,
                  conversationId: e.conversation.id,
                  statusIndicatorReplacesMeta: !0,
                  metaContent:
                    e.conversation.createdAt == null
                      ? void 0
                      : (0, Wt.jsx)(T, {
                          dateString: new Date(e.conversation.createdAt).toISOString(),
                        }),
                  onClick: () => {
                    a(`/local/${e.conversation.id}`);
                  },
                },
                e.key,
              ),
            "pending-worktree": (e) =>
              (0, Wt.jsx)(
                ht,
                {
                  useStableTrailingRail: !0,
                  task: e.pendingWorktree,
                  hasAttention: e.pendingWorktree.needsAttention,
                  statusIndicatorReplacesMeta: !0,
                  onClick: () => {
                    a(`/worktree-init-v2/${e.pendingWorktree.id}`);
                  },
                  onArchive: () => {
                    s(e.pendingWorktree.id);
                  },
                },
                e.key,
              ),
          })),
        (t[13] = s),
        (t[14] = o),
        (t[15] = a),
        (t[16] = e))
      : (e = t[16]),
      (d = u.map(e)),
      (t[8] = s),
      (t[9] = o),
      (t[10] = a),
      (t[11] = u),
      (t[12] = d));
  } else d = t[12];
  let f;
  t[17] !== n.length || t[18] !== !1
    ? ((f = !1), (t[17] = n.length), (t[18] = !1), (t[19] = f))
    : (f = t[19]);
  let p;
  return (
    t[20] !== d || t[21] !== f
      ? ((p = (0, Wt.jsxs)(`div`, {
          className: `group/inline -mx-[var(--padding-row-x)] flex flex-col gap-px rounded-xl pb-1 transition-colors [--task-row-trailing-inset:calc(var(--spacing)*1.5)]`,
          children: [d, f],
        })),
        (t[20] = d),
        (t[21] = f),
        (t[22] = p))
      : (p = t[22]),
    p
  );
}
function Vt() {}
function Ht(e) {
  if (e.kind === `remote`) {
    let t = e.task.task_status_display?.latest_turn_status_display?.turn_status;
    return e.task.has_unread_turn === !0 || t === `in_progress` || t === `pending`;
  }
  return e.kind === `local` && e.conversation.hasUnreadTurn
    ? !0
    : e.kind === `local`
      ? xe(e.conversation)
      : e.kind === `pending-worktree`;
}
var Ut,
  Wt,
  Gt = e(() => {
    ((Ut = c()),
      t(V(), 1),
      we(),
      ae(),
      B(),
      tt(),
      b(),
      l(),
      D(),
      _t(),
      mt(),
      zt(),
      ft(),
      (Wt = _()));
  });
function Kt(e, t) {
  return typeof e == `string` && e.toLowerCase().includes(t);
}
var qt = e(() => {}),
  Jt,
  Yt = e(() => {
    (je(), (Jt = Ee(`cloudTasksEnvironmentFilterId`, null)));
  });
function Xt(e) {
  let t = (0, Zt.c)(15),
    { searchQuery: n, onQueryChange: r, autoFocus: i } = e,
    a = i === void 0 ? !1 : i,
    o = Se(),
    s;
  t[0] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((s = (0, Qt.jsx)(`label`, {
        className: `sr-only`,
        htmlFor: `recent-tasks-search`,
        children: (0, Qt.jsx)(K, {
          id: `codex.recentTasksMenu.search`,
          defaultMessage: `Search recent tasks`,
          description: `Label for the recent tasks menu search input`,
        }),
      })),
      (t[0] = s))
    : (s = t[0]);
  let c;
  t[1] === r ? (c = t[2]) : ((c = (e) => r(e.target.value)), (t[1] = r), (t[2] = c));
  let l;
  t[3] === o
    ? (l = t[4])
    : ((l = o.formatMessage({
        id: `codex.recentTasksMenu.search`,
        defaultMessage: `Search recent tasks`,
        description: `Label for the recent tasks menu search input`,
      })),
      (t[3] = o),
      (t[4] = l));
  let u;
  t[5] !== o || t[6] !== r || t[7] !== n.length
    ? ((u =
        n.length > 0
          ? (0, Qt.jsx)(`button`, {
              type: `button`,
              className: `flex items-center justify-center text-token-input-placeholder-foreground transition-colors hover:text-token-foreground`,
              onClick: () => r(``),
              "aria-label": o.formatMessage({
                id: `codex.recentTasksMenu.clearSearch`,
                defaultMessage: `Clear search`,
                description: `Button label to clear the recent tasks menu search input`,
              }),
              children: (0, Qt.jsx)(et, { className: `icon-2xs` }),
            })
          : null),
      (t[5] = o),
      (t[6] = r),
      (t[7] = n.length),
      (t[8] = u))
    : (u = t[8]);
  let d;
  return (
    t[9] !== a || t[10] !== n || t[11] !== c || t[12] !== l || t[13] !== u
      ? ((d = (0, Qt.jsxs)(`div`, {
          children: [
            s,
            (0, Qt.jsx)(j, {
              id: `recent-tasks-search`,
              autoFocus: a,
              value: n,
              onChange: c,
              placeholder: l,
              trailingContent: u,
            }),
          ],
        })),
        (t[9] = a),
        (t[10] = n),
        (t[11] = c),
        (t[12] = l),
        (t[13] = u),
        (t[14] = d))
      : (d = t[14]),
    d
  );
}
var Zt,
  Qt,
  $t = e(() => {
    ((Zt = c()), we(), N(), $e(), (Qt = _()));
  });
function en() {
  let e = (0, tn.c)(1),
    t;
  return (
    e[0] === Symbol.for(`react.memo_cache_sentinel`)
      ? ((t = (0, nn.jsx)(`div`, {
          className: `flex h-48 items-center justify-center text-sm text-token-input-placeholder-foreground`,
          children: (0, nn.jsx)(K, {
            id: `codex.recentTasksMenu.empty`,
            defaultMessage: `No chats yet`,
            description: `Empty state for recent tasks menu`,
          }),
        })),
        (e[0] = t))
      : (t = e[0]),
    t
  );
}
var tn,
  nn,
  rn = e(() => {
    ((tn = c()), we(), (nn = _()));
  }),
  an,
  on = e(() => {
    an = `w-full justify-between !px-[var(--padding-row-x)] !py-[var(--padding-row-y)] text-left`;
  });
function sn(e) {
  let t = (0, ln.c)(64),
    { className: n } = e,
    r = Se(),
    [i, a] = (0, un.useState)(!1),
    [o, s] = (0, un.useState)(``),
    [c, l] = h(Jt),
    u = Ue(),
    { data: d, isLoading: f, isError: p, refetch: m } = Fe(),
    g;
  t[0] === o ? (g = t[1]) : ((g = o.trim()), (t[0] = o), (t[1] = g));
  let _ = g.length > 0,
    v;
  t[2] === _ ? (v = t[3]) : ((v = { enabled: _ }), (t[2] = _), (t[3] = v));
  let { data: y, isLoading: b, isError: x, refetch: S } = ke(o, v),
    C;
  if (t[4] !== o || t[5] !== d || t[6] !== y || t[7] !== u) {
    bb0: {
      let e = o.trim(),
        n = e.length > 0 ? (y ?? []) : (d ?? []);
      if (e.length === 0 && u) {
        let e;
        t[9] === u ? (e = t[10]) : ((e = (e) => e.id === u.id), (t[9] = u), (t[10] = e));
        let r = n.find(e);
        if (!r) {
          C = n;
          break bb0;
        }
        let i;
        (t[11] === u ? (i = t[12]) : ((i = (e) => e.id !== u.id), (t[11] = u), (t[12] = i)),
          (C = [r, ...n.filter(i)]));
        break bb0;
      }
      C = n;
    }
    ((t[4] = o), (t[5] = d), (t[6] = y), (t[7] = u), (t[8] = C));
  } else C = t[8];
  let w = C,
    T;
  t[13] !== c || t[14] !== d
    ? ((T = d?.find((e) => e.id === c) ?? null), (t[13] = c), (t[14] = d), (t[15] = T))
    : (T = t[15]);
  let E = T,
    D;
  t[16] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((D = (0, Y.jsx)(K, {
        id: `codex.recentTasksMenu.filterTooltip`,
        defaultMessage: `Filter tasks by environment`,
        description: `Tooltip explaining the environment filter button`,
      })),
      (t[16] = D))
    : (D = t[16]);
  let O = c ? `default` : `icon`,
    k;
  t[17] === n ? (k = t[18]) : ((k = pe(`mr-1`, n)), (t[17] = n), (t[18] = k));
  let A;
  t[19] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((A = (0, Y.jsx)(yt, { className: `icon-2xs` })), (t[19] = A))
    : (A = t[19]);
  let j;
  t[20] !== c || t[21] !== E
    ? ((j = !!c && E?.label && (0, Y.jsx)(`span`, { className: `text-sm`, children: E.label })),
      (t[20] = c),
      (t[21] = E),
      (t[22] = j))
    : (j = t[22]);
  let M;
  t[23] === j
    ? (M = t[24])
    : ((M = (0, Y.jsxs)(`span`, { className: `flex items-center gap-1.5`, children: [A, j] })),
      (t[23] = j),
      (t[24] = M));
  let N;
  t[25] !== M || t[26] !== O || t[27] !== k
    ? ((N = (0, Y.jsx)(Ie, {
        asChild: !0,
        children: (0, Y.jsx)(ee, {
          tooltipContent: D,
          children: (0, Y.jsx)(q, { color: `ghost`, size: O, className: k, children: M }),
        }),
      })),
      (t[25] = M),
      (t[26] = O),
      (t[27] = k),
      (t[28] = N))
    : (N = t[28]);
  let te;
  t[29] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((te = (0, Y.jsx)(`div`, {
        className: `pb-1 text-xs tracking-wide text-token-input-placeholder-foreground uppercase`,
        children: (0, Y.jsx)(K, {
          id: `codex.recentTasksMenu.filterCloudTasks`,
          defaultMessage: `Filter cloud tasks`,
          description: `Title for environment filter menu in recent tasks`,
        }),
      })),
      (t[29] = te))
    : (te = t[29]);
  let P = !c,
    F;
  t[30] === l
    ? (F = t[31])
    : ((F = () => {
        (l(null), a(!1));
      }),
      (t[30] = l),
      (t[31] = F));
  let ne;
  t[32] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((ne = (0, Y.jsx)(K, {
        id: `codex.recentTasksMenu.filterAll`,
        defaultMessage: `All`,
        description: `All environments filter option`,
      })),
      (t[32] = ne))
    : (ne = t[32]);
  let I;
  t[33] !== P || t[34] !== F
    ? ((I = (0, Y.jsx)(cn, { isSelected: P, onClick: F, children: ne })),
      (t[33] = P),
      (t[34] = F),
      (t[35] = I))
    : (I = t[35]);
  let L;
  t[36] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((L = (0, Y.jsx)(z.Separator, {})), (t[36] = L))
    : (L = t[36]);
  let R;
  t[37] === r
    ? (R = t[38])
    : ((R = r.formatMessage({
        id: `composer.searchEnvironments`,
        defaultMessage: `Search environments`,
        description: `Search environments placeholder`,
      })),
      (t[37] = r),
      (t[38] = R));
  let B;
  t[39] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((B = (e) => s(e.currentTarget.value)), (t[39] = B))
    : (B = t[39]);
  let V;
  t[40] !== o || t[41] !== R
    ? ((V = (0, Y.jsx)(z.Section, {
        className: `my-1`,
        children: (0, Y.jsx)(z.SearchInput, { placeholder: R, value: o, onChange: B }),
      })),
      (t[40] = o),
      (t[41] = R),
      (t[42] = V))
    : (V = t[42]);
  let H;
  t[43] !== c ||
  t[44] !== o ||
  t[45] !== p ||
  t[46] !== f ||
  t[47] !== x ||
  t[48] !== b ||
  t[49] !== w ||
  t[50] !== m ||
  t[51] !== S ||
  t[52] !== l
    ? ((H =
        o.trim().length > 0 && x
          ? (0, Y.jsxs)(z.Message, {
              className: `flex w-full items-center justify-center gap-2`,
              children: [
                (0, Y.jsx)(`span`, {
                  children: (0, Y.jsx)(K, {
                    id: `codex.environments.searchError`,
                    defaultMessage: `Failed to search environments.`,
                    description: `Error shown when environment search fails`,
                  }),
                }),
                (0, Y.jsx)(q, {
                  size: `default`,
                  color: `outline`,
                  onClick: () => {
                    S();
                  },
                  children: (0, Y.jsx)(K, {
                    id: `codex.common.retry`,
                    defaultMessage: `Retry`,
                    description: `Retry button`,
                  }),
                }),
              ],
            })
          : o.trim().length === 0 && p
            ? (0, Y.jsxs)(z.Message, {
                className: `flex w-full items-center justify-center gap-2`,
                children: [
                  (0, Y.jsx)(`span`, {
                    children: (0, Y.jsx)(K, {
                      id: `codex.environments.listError`,
                      defaultMessage: `Failed to load environments.`,
                      description: `Error shown when listing environments fails`,
                    }),
                  }),
                  (0, Y.jsx)(q, {
                    size: `default`,
                    color: `outline`,
                    onClick: () => {
                      m();
                    },
                    children: (0, Y.jsx)(K, {
                      id: `codex.common.retry`,
                      defaultMessage: `Retry`,
                      description: `Retry button`,
                    }),
                  }),
                ],
              })
            : w?.length > 0
              ? w.map((e) =>
                  (0, Y.jsx)(
                    cn,
                    {
                      isSelected: e.id === c,
                      onClick: () => {
                        (l(e.id), a(!1));
                      },
                      children: e.label,
                    },
                    e.id,
                  ),
                )
              : b || f
                ? (0, Y.jsx)(Qe, {
                    className: `icon-xxs my-2 self-center text-token-description-foreground`,
                  })
                : (0, Y.jsx)(z.Message, {
                    centered: !0,
                    children: (0, Y.jsx)(K, {
                      id: `codex.environments.noEnvironmentsFound`,
                      defaultMessage: `No environments found`,
                      description: `Message shown when no Codex environments were found`,
                    }),
                  })),
      (t[43] = c),
      (t[44] = o),
      (t[45] = p),
      (t[46] = f),
      (t[47] = x),
      (t[48] = b),
      (t[49] = w),
      (t[50] = m),
      (t[51] = S),
      (t[52] = l),
      (t[53] = H))
    : (H = t[53]);
  let U;
  t[54] === H
    ? (U = t[55])
    : ((U = (0, Y.jsx)(z.Section, {
        className: `flex max-h-[140px] flex-col overflow-y-auto pb-1`,
        children: H,
      })),
      (t[54] = H),
      (t[55] = U));
  let W;
  t[56] !== I || t[57] !== V || t[58] !== U
    ? ((W = (0, Y.jsx)(Pe, {
        className: `w-[240px]`,
        side: `top`,
        children: (0, Y.jsxs)(`div`, {
          className: `flex max-w-full flex-col py-1`,
          children: [te, I, L, V, U],
        }),
      })),
      (t[56] = I),
      (t[57] = V),
      (t[58] = U),
      (t[59] = W))
    : (W = t[59]);
  let G;
  return (
    t[60] !== i || t[61] !== N || t[62] !== W
      ? ((G = (0, Y.jsxs)(Ge, { open: i, onOpenChange: a, children: [N, W] })),
        (t[60] = i),
        (t[61] = N),
        (t[62] = W),
        (t[63] = G))
      : (G = t[63]),
    G
  );
}
function cn(e) {
  let t = (0, ln.c)(11),
    { children: n, isSelected: r, onClick: i } = e,
    a = r && `font-medium`,
    o;
  t[0] === a ? (o = t[1]) : ((o = pe(an, a)), (t[0] = a), (t[1] = o));
  let s;
  t[2] === n
    ? (s = t[3])
    : ((s = (0, Y.jsx)(`span`, { className: `min-w-0 flex-1 truncate`, children: n })),
      (t[2] = n),
      (t[3] = s));
  let c;
  t[4] === r
    ? (c = t[5])
    : ((c = r ? (0, Y.jsx)(R, { className: `icon-2xs shrink-0 opacity-75` }) : null),
      (t[4] = r),
      (t[5] = c));
  let l;
  return (
    t[6] !== i || t[7] !== o || t[8] !== s || t[9] !== c
      ? ((l = (0, Y.jsxs)(q, {
          color: `ghostActive`,
          size: `medium`,
          className: o,
          onClick: i,
          children: [s, c],
        })),
        (t[6] = i),
        (t[7] = o),
        (t[8] = s),
        (t[9] = c),
        (t[10] = l))
      : (l = t[10]),
    l
  );
}
var ln,
  un,
  Y,
  dn = e(() => {
    ((ln = c()),
      _e(),
      f(),
      (un = t(a(), 1)),
      we(),
      Te(),
      tt(),
      N(),
      Ae(),
      nt(),
      L(),
      F(),
      vt(),
      qe(),
      Yt(),
      on(),
      (Y = _()));
  });
function fn(e) {
  let t = (0, hn.c)(31),
    { filter: n, onSelect: r } = e,
    i = _n[n],
    [a, o] = (0, gn.useState)(!1),
    s;
  t[0] === i
    ? (s = t[1])
    : ((s = (0, X.jsx)(`span`, { children: (0, X.jsx)(K, { ...i }) })), (t[0] = i), (t[1] = s));
  let c;
  t[2] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((c = (0, X.jsx)(Oe, { className: `icon-2xs` })), (t[2] = c))
    : (c = t[2]);
  let l;
  t[3] === s
    ? (l = t[4])
    : ((l = (0, X.jsx)(Ie, {
        asChild: !0,
        children: (0, X.jsxs)(`button`, {
          type: `button`,
          className: `flex items-center gap-1 px-2 pt-2 text-sm font-medium text-token-input-placeholder-foreground hover:text-token-foreground`,
          children: [s, c],
        }),
      })),
      (t[3] = s),
      (t[4] = l));
  let u = n === `recent`,
    d;
  t[5] === r
    ? (d = t[6])
    : ((d = () => {
        (r(`recent`), o(!1));
      }),
      (t[5] = r),
      (t[6] = d));
  let f;
  t[7] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((f = (0, X.jsx)(K, {
        id: `codex.recentTasksMenu.recent`,
        defaultMessage: `All tasks`,
        description: `Menu title for recent Codex tasks`,
      })),
      (t[7] = f))
    : (f = t[7]);
  let p;
  t[8] !== u || t[9] !== d
    ? ((p = (0, X.jsx)(mn, { isSelected: u, onClick: d, children: f })),
      (t[8] = u),
      (t[9] = d),
      (t[10] = p))
    : (p = t[10]);
  let m = n === `cloud`,
    h;
  t[11] === r
    ? (h = t[12])
    : ((h = () => {
        (r(`cloud`), o(!1));
      }),
      (t[11] = r),
      (t[12] = h));
  let g;
  t[13] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((g = (0, X.jsx)(K, {
        id: `codex.recentTasksMenu.cloud`,
        defaultMessage: `Cloud tasks`,
        description: `Menu title for cloud Codex tasks`,
      })),
      (t[13] = g))
    : (g = t[13]);
  let _;
  t[14] !== m || t[15] !== h
    ? ((_ = (0, X.jsx)(mn, { isSelected: m, onClick: h, children: g })),
      (t[14] = m),
      (t[15] = h),
      (t[16] = _))
    : (_ = t[16]);
  let v = n === `local`,
    y;
  t[17] === r
    ? (y = t[18])
    : ((y = () => {
        (r(`local`), o(!1));
      }),
      (t[17] = r),
      (t[18] = y));
  let b;
  t[19] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((b = (0, X.jsx)(K, {
        id: `codex.recentTasksMenu.local`,
        defaultMessage: `Local tasks`,
        description: `Menu title for local Codex tasks`,
      })),
      (t[19] = b))
    : (b = t[19]);
  let x;
  t[20] !== v || t[21] !== y
    ? ((x = (0, X.jsx)(mn, { isSelected: v, onClick: y, children: b })),
      (t[20] = v),
      (t[21] = y),
      (t[22] = x))
    : (x = t[22]);
  let S;
  t[23] !== _ || t[24] !== x || t[25] !== p
    ? ((S = (0, X.jsx)(Pe, {
        className: `w-[220px]`,
        onOpenAutoFocus: pn,
        children: (0, X.jsxs)(`div`, { className: `flex flex-col`, children: [p, _, x] }),
      })),
      (t[23] = _),
      (t[24] = x),
      (t[25] = p),
      (t[26] = S))
    : (S = t[26]);
  let C;
  return (
    t[27] !== a || t[28] !== S || t[29] !== l
      ? ((C = (0, X.jsxs)(Ge, { open: a, onOpenChange: o, children: [l, S] })),
        (t[27] = a),
        (t[28] = S),
        (t[29] = l),
        (t[30] = C))
      : (C = t[30]),
    C
  );
}
function pn(e) {
  e.preventDefault();
}
function mn(e) {
  let t = (0, hn.c)(11),
    { children: n, isSelected: r, onClick: i } = e,
    a = r && `font-medium`,
    o;
  t[0] === a ? (o = t[1]) : ((o = pe(an, a)), (t[0] = a), (t[1] = o));
  let s;
  t[2] === n
    ? (s = t[3])
    : ((s = (0, X.jsx)(`span`, { className: `min-w-0 flex-1 truncate`, children: n })),
      (t[2] = n),
      (t[3] = s));
  let c;
  t[4] === r
    ? (c = t[5])
    : ((c = r ? (0, X.jsx)(R, { className: `icon-2xs shrink-0 opacity-75` }) : null),
      (t[4] = r),
      (t[5] = c));
  let l;
  return (
    t[6] !== i || t[7] !== o || t[8] !== s || t[9] !== c
      ? ((l = (0, X.jsxs)(q, {
          color: `ghostActive`,
          size: `medium`,
          className: o,
          onClick: i,
          children: [s, c],
        })),
        (t[6] = i),
        (t[7] = o),
        (t[8] = s),
        (t[9] = c),
        (t[10] = l))
      : (l = t[10]),
    l
  );
}
var hn,
  gn,
  X,
  _n,
  vn = e(() => {
    ((hn = c()),
      _e(),
      (gn = t(a(), 1)),
      we(),
      tt(),
      Ae(),
      F(),
      De(),
      on(),
      (X = _()),
      (_n = be({
        recent: {
          id: `codex.recentTasksMenu.recent`,
          defaultMessage: `All tasks`,
          description: `Menu title for recent Codex tasks`,
        },
        cloud: {
          id: `codex.recentTasksMenu.cloud`,
          defaultMessage: `Cloud tasks`,
          description: `Menu title for cloud Codex tasks`,
        },
        local: {
          id: `codex.recentTasksMenu.local`,
          defaultMessage: `Local tasks`,
          description: `Menu title for local Codex tasks`,
        },
      })));
  });
function yn() {
  let e = (0, bn.c)(1),
    t;
  return (
    e[0] === Symbol.for(`react.memo_cache_sentinel`)
      ? ((t = (0, xn.jsx)(`div`, {
          className: `flex h-48 items-center justify-center text-sm text-token-input-placeholder-foreground`,
          children: (0, xn.jsx)(K, {
            id: `codex.recentTasksMenu.searchEmpty`,
            defaultMessage: `No result`,
            description: `Empty state for recent tasks menu search results`,
          }),
        })),
        (e[0] = t))
      : (t = e[0]),
    t
  );
}
var bn,
  xn,
  Sn = e(() => {
    ((bn = c()), we(), (xn = _()));
  });
function Cn({ conversation: e, isBackgroundSubagentsEnabled: t }) {
  return !de(e, t);
}
var wn = e(() => {
  he();
});
function Tn(e, t, n) {
  let r = O(),
    i = ie(`12346831`),
    a = m(ye),
    o = i ? fe : a,
    s = C(),
    c = (0, kn.useRef)(new Map());
  return (0, kn.useMemo)(() => {
    let i = Dn(
      En({
        tasks: e,
        localConversations: t,
        pendingWorktrees: r,
        envForFilter: n,
        threadSortKey: o,
        isBackgroundSubagentsEnabled: s,
      }),
      c.current,
    );
    return ((c.current = new Map(i.map((e) => [e.key, e]))), i);
  }, [e, n, s, t, r, o]);
}
function En({
  tasks: e,
  localConversations: t,
  pendingWorktrees: n,
  envForFilter: r,
  threadSortKey: i,
  isBackgroundSubagentsEnabled: a,
}) {
  let s = e ?? [],
    c = t.filter((e) => Cn({ conversation: e, isBackgroundSubagentsEnabled: a })),
    l = (0, On.default)(
      r ? s.filter((e) => e.task_status_display?.environment_label === r.label) : s,
      `id`,
    ).map((e) => {
      let t =
        i === `updated_at`
          ? (e.updated_at ?? e.created_at ?? 0)
          : (e.created_at ?? e.updated_at ?? 0);
      return { kind: `remote`, key: Re(e.id), at: t * 1e3, task: e };
    }),
    u = c.map((e) => {
      let t = i === `updated_at` ? (e.recencyAt ?? e.updatedAt) : e.createdAt;
      Number.isFinite(t) || o.error(`local conversation has invalid createdAt or updatedAt`);
      let n = Number.isFinite(t) ? t : Number.isFinite(e.createdAt) ? e.createdAt : 0;
      return { kind: `local`, key: Ke(e.id), at: n, conversation: e };
    }),
    d = n.map((e) => ({
      kind: `pending-worktree`,
      key: He(e.id),
      at: e.createdAt,
      pendingWorktree: e,
    }));
  return [...l, ...u, ...d].sort((e, t) => t.at - e.at);
}
function Dn(e, t) {
  return e.map((e) => {
    let n = t.get(e.key);
    if (n == null || n.at !== e.at) return e;
    switch (e.kind) {
      case `remote`:
        return n.kind === `remote` && n.task === e.task ? n : e;
      case `local`:
        return n.kind === `local` && n.conversation === e.conversation ? n : e;
      case `pending-worktree`:
        return n.kind === `pending-worktree` && n.pendingWorktree === e.pendingWorktree ? n : e;
    }
  });
}
var On,
  kn,
  An = e(() => {
    ((On = t(V(), 1)), u(), (kn = t(a(), 1)), w(), ge(), Je(), H(), W(), s(), D(), wn());
  });
function jn(e) {
  let t = (0, In.c)(33),
    {
      cloudtasksQuery: n,
      localConversations: r,
      onClose: i,
      autoFocusSearch: a,
      showFilters: o,
    } = e,
    s = a === void 0 ? !1 : a,
    c = o === void 0 ? !0 : o,
    l = Se(),
    { authMethod: u } = Ce(),
    [d, f] = h(Rn),
    [p] = h(Jt),
    m = c ? d : `recent`,
    g = le(`/local/:conversationId`)?.params?.conversationId ?? null,
    { data: _ } = Fe(),
    v = C(),
    y;
  t[0] !== p || t[1] !== c || t[2] !== _
    ? ((y = c ? (_?.find((e) => e.id === p) ?? null) : null),
      (t[0] = p),
      (t[1] = c),
      (t[2] = _),
      (t[3] = y))
    : (y = t[3]);
  let b = y,
    x;
  t[4] === v
    ? (x = t[5])
    : ((x = (e) => Cn({ conversation: e, isBackgroundSubagentsEnabled: v })),
      (t[4] = v),
      (t[5] = x));
  let S = r.filter(x),
    w = Tn(n.data, r, b),
    [T, E] = (0, Ln.useState)(``),
    D = (0, Ln.useDeferredValue)(T).trim().toLowerCase(),
    O = D.length > 0,
    k = w.filter(Mn),
    A = O
      ? k.filter((e) => {
          let { task: t } = e;
          return Kt(t.title, D);
        })
      : k,
    ee = O ? S.filter((e) => Kt(me(e), D)) : S,
    j = O
      ? w.filter((e) =>
          e.kind === `remote`
            ? Kt(e.task.title, D)
            : e.kind === `local`
              ? Kt(me(e.conversation), D)
              : Kt(Nn(e.pendingWorktree, l), D),
        )
      : w,
    M;
  t[6] !== s || t[7] !== T
    ? ((M = (0, Z.jsx)(z.Section, {
        children: (0, Z.jsx)(Xt, { searchQuery: T, onQueryChange: E, autoFocus: s }),
      })),
      (t[6] = s),
      (t[7] = T),
      (t[8] = M))
    : (M = t[8]);
  let N;
  t[9] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((N = (0, Z.jsx)(`div`, {
        className: `mx-2 border-t-[0.5px] border-token-border`,
        "aria-hidden": !0,
      })),
      (t[9] = N))
    : (N = t[9]);
  let te;
  t[10] !== u || t[11] !== d || t[12] !== f || t[13] !== c
    ? ((te =
        c &&
        u === `chatgpt` &&
        (0, Z.jsxs)(z.Section, {
          className: `flex items-center justify-between px-[var(--padding-row-x)]`,
          children: [
            (0, Z.jsx)(fn, { filter: d, onSelect: f }),
            d !== `local` && (0, Z.jsx)(sn, {}),
          ],
        })),
      (t[10] = u),
      (t[11] = d),
      (t[12] = f),
      (t[13] = c),
      (t[14] = te))
    : (te = t[14]);
  let P = z,
    F =
      m === `cloud` &&
      (n.isError
        ? (0, Z.jsx)(Fn, {
            onRetry: () => {
              n.refetch();
            },
          })
        : n.isLoading
          ? (0, Z.jsx)(Pn, {})
          : A.length === 0
            ? O
              ? (0, Z.jsx)(yn, {})
              : (0, Z.jsx)(en, {})
            : A.map((e) => (0, Z.jsx)(pt, { task: e.task, onClose: i }, e.key))),
    ne =
      m === `local` &&
      (ee.length
        ? ee.map((e) =>
            (0, Z.jsx)(
              zn,
              {
                conversationId: e.id,
                updatedAt: e.recencyAt ?? e.updatedAt,
                isActive: g === e.id,
                onClose: i,
              },
              e.id,
            ),
          )
        : O
          ? (0, Z.jsx)(yn, {})
          : (0, Z.jsx)(en, {})),
    I;
  t[15] !== g ||
  t[16] !== n ||
  t[17] !== j ||
  t[18] !== O ||
  t[19] !== w.length ||
  t[20] !== i ||
  t[21] !== m
    ? ((I =
        m === `recent` &&
        (n.isError && w.length === 0
          ? (0, Z.jsx)(Fn, {
              onRetry: () => {
                n.refetch();
              },
            })
          : n.isLoading && w.length === 0
            ? (0, Z.jsx)(Pn, {})
            : j.length === 0
              ? O
                ? (0, Z.jsx)(yn, {})
                : (0, Z.jsx)(en, {})
              : (0, Z.jsxs)(Z.Fragment, {
                  children: [
                    n.isError &&
                      (0, Z.jsxs)(`div`, {
                        className: `mx-1 my-1 flex items-center justify-between gap-2 px-1 text-base text-token-foreground`,
                        children: [
                          (0, Z.jsx)(K, {
                            id: `codex.recentTasksMenu.errorCloud.inline`,
                            defaultMessage: `Failed to load cloud tasks.`,
                            description: `Inline error indicator for cloud tasks in recent feed`,
                          }),
                          (0, Z.jsx)(q, {
                            size: `default`,
                            color: `outline`,
                            onClick: () => {
                              n.refetch();
                            },
                            children: (0, Z.jsx)(K, {
                              id: `codex.common.retry`,
                              defaultMessage: `Retry`,
                              description: `Retry button`,
                            }),
                          }),
                        ],
                      }),
                    n.isPending &&
                      !w.length &&
                      (0, Z.jsx)(`div`, {
                        className: `mx-1 my-1 flex items-center gap-2 text-sm text-token-input-placeholder-foreground`,
                        children: (0, Z.jsx)(Qe, { className: `icon-xs` }),
                      }),
                    j.map((e) =>
                      (0, Z.jsx)(
                        Bn,
                        {
                          item: e,
                          isActive: e.kind === `local` && g === e.conversation.id,
                          onClose: i,
                        },
                        e.key,
                      ),
                    ),
                  ],
                }))),
      (t[15] = g),
      (t[16] = n),
      (t[17] = j),
      (t[18] = O),
      (t[19] = w.length),
      (t[20] = i),
      (t[21] = m),
      (t[22] = I))
    : (I = t[22]);
  let L;
  t[23] !== P.Section || t[24] !== F || t[25] !== ne || t[26] !== I
    ? ((L = (0, Z.jsxs)(P.Section, {
        className: `vertical-scroll-fade-mask flex max-h-[60vh] flex-col gap-0 overflow-y-auto pb-1`,
        children: [F, ne, I],
      })),
      (t[23] = P.Section),
      (t[24] = F),
      (t[25] = ne),
      (t[26] = I),
      (t[27] = L))
    : (L = t[27]);
  let R;
  return (
    t[28] !== L || t[29] !== M || t[30] !== N || t[31] !== te
      ? ((R = (0, Z.jsxs)(`div`, {
          className: `flex max-h-[300px] w-[calc(var(--radix-popper-available-width)_-_var(--padding-panel))] flex-col gap-1`,
          children: [M, N, te, L],
        })),
        (t[28] = L),
        (t[29] = M),
        (t[30] = N),
        (t[31] = te),
        (t[32] = R))
      : (R = t[32]),
    R
  );
}
function Mn(e) {
  return e.kind === `remote`;
}
function Nn(e, t) {
  let n = e.label?.trim();
  return n && n.length > 0
    ? n
    : e.phase === `failed`
      ? t.formatMessage({
          id: `recentTasks.worktreeInitFailedTitle`,
          defaultMessage: `Worktree init failed`,
          description: `Worktree row title when the init script fails`,
        })
      : t.formatMessage({
          id: `recentTasks.worktreeSettingUpTitle`,
          defaultMessage: `Setting up worktree`,
          description: `Worktree row title while init is pending`,
        });
}
function Pn() {
  let e = (0, In.c)(1),
    t;
  return (
    e[0] === Symbol.for(`react.memo_cache_sentinel`)
      ? ((t = (0, Z.jsx)(`div`, {
          className: `flex h-48 items-center justify-center gap-2 text-sm text-token-input-placeholder-foreground`,
          children: (0, Z.jsx)(Qe, { className: `icon-xs` }),
        })),
        (e[0] = t))
      : (t = e[0]),
    t
  );
}
function Fn(e) {
  let t = (0, In.c)(4),
    { onRetry: n } = e,
    r;
  t[0] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((r = (0, Z.jsx)(K, {
        id: `codex.recentTasksMenu.errorCloud`,
        defaultMessage: `Failed to load tasks.`,
        description: `Error state for cloud tasks in recent tasks menu`,
      })),
      (t[0] = r))
    : (r = t[0]);
  let i;
  t[1] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((i = (0, Z.jsx)(K, {
        id: `codex.common.retry`,
        defaultMessage: `Retry`,
        description: `Retry button`,
      })),
      (t[1] = i))
    : (i = t[1]);
  let a;
  return (
    t[2] === n
      ? (a = t[3])
      : ((a = (0, Z.jsxs)(`div`, {
          className: `flex h-48 items-center justify-center gap-3 text-sm text-token-input-placeholder-foreground`,
          children: [
            r,
            (0, Z.jsx)(q, { size: `default`, color: `outline`, onClick: n, children: i }),
          ],
        })),
        (t[2] = n),
        (t[3] = a)),
    a
  );
}
var In,
  Ln,
  Z,
  Rn,
  zn,
  Bn,
  Vn = e(() => {
    ((In = c()),
      f(),
      (Ln = t(a(), 1)),
      we(),
      ae(),
      ue(),
      Te(),
      tt(),
      N(),
      b(),
      nt(),
      w(),
      ve(),
      p(),
      qt(),
      je(),
      D(),
      _t(),
      mt(),
      Yt(),
      $t(),
      rn(),
      dn(),
      vn(),
      Sn(),
      wn(),
      An(),
      ft(),
      (Z = _()),
      (Rn = Ee(`recent-tasks-filter`, `recent`)),
      (zn = (0, Ln.memo)(function (e) {
        let t = (0, In.c)(7),
          { conversationId: n, updatedAt: r, isActive: i, onClose: a } = e,
          o;
        t[0] === r
          ? (o = t[1])
          : ((o = r == null ? void 0 : (0, Z.jsx)(T, { dateString: new Date(r).toISOString() })),
            (t[0] = r),
            (t[1] = o));
        let s;
        return (
          t[2] !== n || t[3] !== i || t[4] !== a || t[5] !== o
            ? ((s = (0, Z.jsx)(gt, { conversationId: n, isActive: i, metaContent: o, onClick: a })),
              (t[2] = n),
              (t[3] = i),
              (t[4] = a),
              (t[5] = o),
              (t[6] = s))
            : (s = t[6]),
          s
        );
      })),
      (Bn = (0, Ln.memo)(function (e) {
        let t = (0, In.c)(21),
          { item: r, isActive: i, onClose: a } = e,
          { cancelPendingWorktree: o } = E();
        switch (r.kind) {
          case `remote`: {
            let e;
            return (
              t[0] !== r.task || t[1] !== a
                ? ((e = (0, Z.jsx)(pt, { task: r.task, onClose: a })),
                  (t[0] = r.task),
                  (t[1] = a),
                  (t[2] = e))
                : (e = t[2]),
              e
            );
          }
          case `local`: {
            let e;
            t[3] !== r.conversation.recencyAt || t[4] !== r.conversation.updatedAt
              ? ((e =
                  (r.conversation.recencyAt ?? r.conversation.updatedAt) == null
                    ? void 0
                    : (0, Z.jsx)(T, {
                        dateString: new Date(
                          r.conversation.recencyAt ?? r.conversation.updatedAt,
                        ).toISOString(),
                      })),
                (t[3] = r.conversation.recencyAt),
                (t[4] = r.conversation.updatedAt),
                (t[5] = e))
              : (e = t[5]);
            let n;
            return (
              t[6] !== i || t[7] !== r.conversation.id || t[8] !== a || t[9] !== e
                ? ((n = (0, Z.jsx)(gt, {
                    conversationId: r.conversation.id,
                    isActive: i,
                    metaContent: e,
                    onClick: a,
                  })),
                  (t[6] = i),
                  (t[7] = r.conversation.id),
                  (t[8] = a),
                  (t[9] = e),
                  (t[10] = n))
                : (n = t[10]),
              n
            );
          }
          case `pending-worktree`: {
            let e;
            t[11] !== r.pendingWorktree.id || t[12] !== a
              ? ((e = () => {
                  (n.dispatchHostMessage({
                    type: `navigate-to-route`,
                    path: `/worktree-init-v2/${r.pendingWorktree.id}`,
                  }),
                    a());
                }),
                (t[11] = r.pendingWorktree.id),
                (t[12] = a),
                (t[13] = e))
              : (e = t[13]);
            let i;
            t[14] !== o || t[15] !== r.pendingWorktree.id
              ? ((i = () => {
                  o(r.pendingWorktree.id);
                }),
                (t[14] = o),
                (t[15] = r.pendingWorktree.id),
                (t[16] = i))
              : (i = t[16]);
            let s;
            return (
              t[17] !== r.pendingWorktree || t[18] !== e || t[19] !== i
                ? ((s = (0, Z.jsx)(ht, {
                    task: r.pendingWorktree,
                    hasAttention: r.pendingWorktree.needsAttention,
                    onClick: e,
                    onArchive: i,
                  })),
                  (t[17] = r.pendingWorktree),
                  (t[18] = e),
                  (t[19] = i),
                  (t[20] = s))
                : (s = t[20]),
              s
            );
          }
        }
      })));
  });
function Hn(e) {
  let t = (0, Gn.c)(67),
    n;
  t[0] === e ? (n = t[1]) : ((n = e === void 0 ? {} : e), (t[0] = e), (t[1] = n));
  let { label: r, showFilters: i } = n,
    a = i === void 0 ? !0 : i,
    o = Se(),
    { authMethod: s } = Ce(),
    c = se(),
    { data: l } = oe(),
    u = C(),
    [d, f] = (0, Kn.useState)(!1),
    p;
  t[2] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((p = () => {
        f(!1);
      }),
      (t[2] = p))
    : (p = t[2]);
  let m = I(p),
    h = d && s === `chatgpt`,
    g;
  t[3] === h
    ? (g = t[4])
    : ((g = { taskFilter: `current`, limit: 20, enabled: h }), (t[3] = h), (t[4] = g));
  let _ = Ne(g),
    v;
  t[5] === _.data ? (v = t[6]) : ((v = _.data ?? []), (t[5] = _.data), (t[6] = v));
  let b;
  t[7] === v ? (b = t[8]) : ((b = v.filter(Wn)), (t[7] = v), (t[8] = b));
  let x = b,
    S,
    w,
    T,
    E;
  if (t[9] !== l || t[10] !== u || t[11] !== r || t[12] !== c.pathname || t[13] !== x.length) {
    let e;
    t[18] === u
      ? (e = t[19])
      : ((e = (e) => Cn({ conversation: e, isBackgroundSubagentsEnabled: u })),
        (t[18] = u),
        (t[19] = e));
    let n = (l ?? []).filter(e).filter(Un);
    ((S = x.length + n.length),
      (T = c.pathname !== `/` && S > 0),
      (w = r != null),
      (E = !1),
      (t[9] = l),
      (t[10] = u),
      (t[11] = r),
      (t[12] = c.pathname),
      (t[13] = x.length),
      (t[14] = S),
      (t[15] = w),
      (t[16] = T),
      (t[17] = E));
  } else ((S = t[14]), (w = t[15]), (T = t[16]), (E = t[17]));
  let D = E,
    O;
  t[20] !== D || t[21] !== S || t[22] !== o
    ? ((O = D
        ? o.formatMessage(
            {
              id: `codex.recentTasksMenu.triggerWithUnread`,
              defaultMessage: `Recent tasks. {count, plural, one {# in progress} other {# in progress}}. Unread task updates`,
              description: `Accessible label for opening the recent tasks menu when unread task updates are present`,
            },
            { count: S },
          )
        : o.formatMessage(
            {
              id: `codex.recentTasksMenu.trigger`,
              defaultMessage: `Recent tasks. {count, plural, one {# in progress} other {# in progress}}`,
              description: `Accessible label for opening the recent tasks menu`,
            },
            { count: S },
          )),
      (t[20] = D),
      (t[21] = S),
      (t[22] = o),
      (t[23] = O))
    : (O = t[23]);
  let k = O,
    A;
  t[24] !== s || t[25] !== d || t[26] !== _
    ? ((A = () => {
        d && s === `chatgpt` && _.refetch();
      }),
      (t[24] = s),
      (t[25] = d),
      (t[26] = _),
      (t[27] = A))
    : (A = t[27]);
  let j;
  (t[28] === d ? (j = t[29]) : ((j = [d]), (t[28] = d), (t[29] = j)), (0, Kn.useEffect)(A, j));
  let M, N;
  (t[30] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((M = () => {
        let e = (e) => f(!0);
        return (
          window.addEventListener(`open-recent-tasks-menu`, e),
          () => window.removeEventListener(`open-recent-tasks-menu`, e)
        );
      }),
      (N = []),
      (t[30] = M),
      (t[31] = N))
    : ((M = t[30]), (N = t[31])),
    (0, Kn.useEffect)(M, N));
  let P;
  t[32] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((P = (0, Q.jsx)(K, {
        id: `codex.recentTasksMenu.tooltip`,
        defaultMessage: `Task history`,
        description: `Tooltip text for recent tasks menu`,
      })),
      (t[32] = P))
    : (P = t[32]);
  let F = w ? `ghostActive` : `ghost`,
    ne = w ? `default` : `icon`,
    L = w && `max-w-full min-w-0 !px-0 !py-0 hover:!bg-transparent hover:opacity-80`,
    R;
  t[33] === L ? (R = t[34]) : ((R = pe(`relative`, L)), (t[33] = L), (t[34] = R));
  let z = w ? void 0 : k,
    B;
  t[35] === D
    ? (B = t[36])
    : ((B = D
        ? (0, Q.jsx)(`span`, {
            "aria-hidden": `true`,
            className: `absolute top-0.5 right-0.5 size-1.5 rounded-full`,
            style: { backgroundColor: `var(--vscode-textLink-foreground)` },
          })
        : null),
      (t[35] = D),
      (t[36] = B));
  let V;
  t[37] !== r || t[38] !== w || t[39] !== T
    ? ((V = w
        ? (0, Q.jsxs)(Q.Fragment, {
            children: [
              (0, Q.jsx)(`span`, { className: pe(`min-w-0 truncate`, !1), children: r }),
              (0, Q.jsx)(Oe, { className: `icon-2xs shrink-0 text-token-text-tertiary` }),
            ],
          })
        : T
          ? (0, Q.jsx)(`span`, {
              className: `relative inline-flex size-4 justify-center`,
              children: (0, Q.jsx)(Qe, { className: `icon-xs` }),
            })
          : (0, Q.jsx)(y, { className: `icon-xs hover:opacity-80` })),
      (t[37] = r),
      (t[38] = w),
      (t[39] = T),
      (t[40] = V))
    : (V = t[40]);
  let H;
  t[41] !== S || t[42] !== o
    ? ((H = o.formatMessage(
        {
          id: `codex.recentTasksMenu.count`,
          defaultMessage: `{count, plural, =0 {No tasks in progress} one {# task in progress} other {# tasks in progress}}`,
          description: `Live region text announcing in-progress task count for recent tasks trigger`,
        },
        { count: S },
      )),
      (t[41] = S),
      (t[42] = o),
      (t[43] = H))
    : (H = t[43]);
  let U;
  t[44] === H
    ? (U = t[45])
    : ((U = (0, Q.jsx)(`span`, {
        className: `sr-only`,
        "aria-live": `polite`,
        "aria-atomic": `true`,
        children: H,
      })),
      (t[44] = H),
      (t[45] = U));
  let W;
  t[46] !== w ||
  t[47] !== F ||
  t[48] !== ne ||
  t[49] !== R ||
  t[50] !== z ||
  t[51] !== B ||
  t[52] !== V ||
  t[53] !== U
    ? ((W = (0, Q.jsx)(ee, {
        tooltipContent: P,
        children: (0, Q.jsxs)(q, {
          color: F,
          size: ne,
          className: R,
          allowShrink: w,
          "aria-label": z,
          children: [B, V, U],
        }),
      })),
      (t[46] = w),
      (t[47] = F),
      (t[48] = ne),
      (t[49] = R),
      (t[50] = z),
      (t[51] = B),
      (t[52] = V),
      (t[53] = U),
      (t[54] = W))
    : (W = t[54]);
  let G;
  t[55] === l ? (G = t[56]) : ((G = l ?? []), (t[55] = l), (t[56] = G));
  let re;
  t[57] !== m || t[58] !== d || t[59] !== _ || t[60] !== a || t[61] !== G
    ? ((re = (0, Q.jsx)(jn, {
        cloudtasksQuery: _,
        localConversations: G,
        onClose: m,
        autoFocusSearch: d,
        showFilters: a,
      })),
      (t[57] = m),
      (t[58] = d),
      (t[59] = _),
      (t[60] = a),
      (t[61] = G),
      (t[62] = re))
    : (re = t[62]);
  let ie;
  return (
    t[63] !== d || t[64] !== W || t[65] !== re
      ? ((ie = (0, Q.jsx)(te, {
          contentClassName: `!pb-0 mt-[9px]`,
          triggerButton: W,
          open: d,
          onOpenChange: f,
          children: re,
        })),
        (t[63] = d),
        (t[64] = W),
        (t[65] = re),
        (t[66] = ie))
      : (ie = t[66]),
    ie
  );
}
function Un(e) {
  return xe(e);
}
function Wn(e) {
  let t = e.task_status_display?.latest_turn_status_display?.turn_status;
  return t === `in_progress` || t === `pending`;
}
var Gn,
  Kn,
  Q,
  qn = e(() => {
    ((Gn = c()),
      _e(),
      (Kn = t(a(), 1)),
      we(),
      ae(),
      G(),
      B(),
      ue(),
      Te(),
      tt(),
      N(),
      nt(),
      L(),
      w(),
      De(),
      x(),
      ne(),
      Vn(),
      wn(),
      (Q = _()));
  });
function Jn(e) {
  let t = (0, $n.c)(66),
    { className: n, desktopDeepLinkConversationId: r, title: i, onBack: a, trailing: o } = e,
    s = se(),
    c = a ?? Xn,
    l = s.pathname === `/`,
    u = Yn,
    { data: d } = oe(),
    f = l,
    p;
  t[0] === f
    ? (p = t[1])
    : ((p = { taskFilter: `current`, limit: 20, enabled: f }), (t[0] = f), (t[1] = p));
  let m = Ne(p),
    h;
  t[2] === d ? (h = t[3]) : ((h = d ?? []), (t[2] = d), (t[3] = h));
  let g = Tn(m.data, h, null),
    _;
  t[28] === n
    ? (_ = t[29])
    : ((_ = pe(`draggable extension:px-panel`, n)), (t[28] = n), (t[29] = _));
  let v = `justify-between`,
    y;
  t[30] === v
    ? (y = t[31])
    : ((y = pe(`flex items-center electron:h-toolbar extension:py-row-y`, v)),
      (t[30] = v),
      (t[31] = y));
  let b;
  t[32] !== a || t[33] !== !1 ? ((b = null), (t[32] = a), (t[33] = !1), (t[34] = b)) : (b = t[34]);
  let x;
  t[35] !== c || t[36] !== g || t[37] !== i
    ? ((x = (0, $.jsx)(`div`, {
        className: `mr-3 line-clamp-1 flex min-w-0 flex-1 items-center gap-1 truncate`,
        style: { viewTransitionName: `header-title` },
        children: i
          ? (0, $.jsxs)(`div`, {
              className: `flex min-w-0 flex-1 items-center gap-1`,
              children: [
                (0, $.jsx)(Qn, { onClick: c }),
                (0, $.jsx)(q, {
                  color: `ghostActive`,
                  type: `button`,
                  onClick: u,
                  className: `min-w-0 flex-1 truncate !px-0 !py-0 text-left text-sm text-token-foreground hover:!bg-transparent hover:opacity-80 electron:font-medium`,
                  children: (0, $.jsx)(`span`, { className: `truncate`, children: i }),
                }),
              ],
            })
          : (0, $.jsx)(`span`, {
              className: `text-token-description-foreground`,
              children: (0, $.jsx)(Zn, { mergedTasks: g, onBack: c, showBackButton: !0 }),
            }),
      })),
      (t[35] = c),
      (t[36] = g),
      (t[37] = i),
      (t[38] = x))
    : (x = t[38]);
  let S;
  t[39] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((S = (0, $.jsx)(Hn, {})), (t[39] = S))
    : (S = t[39]);
  let C;
  t[40] === r
    ? (C = t[41])
    : ((C = (0, $.jsx)(P, {
        chromeExtension: !0,
        children: (0, $.jsx)(bt, { conversationId: r }),
      })),
      (t[40] = r),
      (t[41] = C));
  let w;
  t[42] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((w = (0, $.jsx)(P, { extension: !0, children: (0, $.jsx)(Ze, {}) })), (t[42] = w))
    : (w = t[42]);
  let T;
  t[43] === !0 ? (T = t[44]) : ((T = (0, $.jsx)(Nt, {})), (t[43] = !0), (t[44] = T));
  let E;
  t[45] !== C || t[46] !== T
    ? ((E = (0, $.jsx)(P, {
        chromeExtension: !0,
        extension: !0,
        children: (0, $.jsx)(`div`, {
          className: `flex flex-shrink-0 items-center`,
          children: (0, $.jsxs)(`div`, {
            className: `flex items-center gap-1`,
            children: [S, C, w, T],
          }),
        }),
      })),
      (t[45] = C),
      (t[46] = T),
      (t[47] = E))
    : (E = t[47]);
  let D;
  t[48] !== E || t[49] !== o
    ? ((D = (0, $.jsxs)(`div`, {
        className: `flex flex-shrink-0 items-center gap-1`,
        children: [o, E],
      })),
      (t[48] = E),
      (t[49] = o),
      (t[50] = D))
    : (D = t[50]);
  let O;
  t[51] !== D || t[52] !== y || t[53] !== b || t[54] !== x
    ? ((O = (0, $.jsxs)(`div`, { className: y, children: [b, x, D] })),
      (t[51] = D),
      (t[52] = y),
      (t[53] = b),
      (t[54] = x),
      (t[55] = O))
    : (O = t[55]);
  let k;
  t[56] !== l || t[57] !== g || t[58] !== m
    ? ((k =
        l && (0, $.jsx)(`div`, { children: (0, $.jsx)(Bt, { tasksQuery: m, mergedTasks: g }) })),
      (t[56] = l),
      (t[57] = g),
      (t[58] = m),
      (t[59] = k))
    : (k = t[59]);
  let A;
  t[60] === k
    ? (A = t[61])
    : ((A = (0, $.jsx)(P, { extension: !0, children: k })), (t[60] = k), (t[61] = A));
  let ee;
  return (
    t[62] !== O || t[63] !== A || t[64] !== _
      ? ((ee = (0, $.jsxs)(`div`, { className: _, children: [O, A] })),
        (t[62] = O),
        (t[63] = A),
        (t[64] = _),
        (t[65] = ee))
      : (ee = t[65]),
    ee
  );
}
function Yn() {
  window.dispatchEvent(new CustomEvent(`open-recent-tasks-menu`));
}
function Xn() {
  Me(`newThread`, `header_new_thread`);
}
function Zn(e) {
  let t = (0, $n.c)(21),
    { mergedTasks: n, onBack: r, showBackButton: i } = e,
    a = se().pathname === `/`,
    o = le(`/local/:conversationId`)?.params?.conversationId ?? null,
    s;
  t[0] === o ? (s = t[1]) : ((s = o == null ? null : g(o)), (t[0] = o), (t[1] = s));
  let c = s,
    l = le(`/remote/:taskId`)?.params?.taskId ?? null,
    { data: u } = We(l),
    f = d(U, c),
    p = d(v, c);
  if (l && u?.task?.title) {
    let e;
    t[2] !== r || t[3] !== i
      ? ((e = i ? (0, $.jsx)(Qn, { onClick: r }) : null), (t[2] = r), (t[3] = i), (t[4] = e))
      : (e = t[4]);
    let n;
    t[5] === u.task.title
      ? (n = t[6])
      : ((n = (0, $.jsx)(`span`, {
          className: `min-w-0 flex-1 text-base text-token-foreground`,
          children: u.task.title,
        })),
        (t[5] = u.task.title),
        (t[6] = n));
    let a;
    return (
      t[7] !== e || t[8] !== n
        ? ((a = (0, $.jsxs)(`div`, {
            className: `flex min-w-0 items-center gap-1`,
            children: [e, n],
          })),
          (t[7] = e),
          (t[8] = n),
          (t[9] = a))
        : (a = t[9]),
      a
    );
  }
  if (c && f) {
    let e;
    t[10] !== r || t[11] !== i
      ? ((e = i ? (0, $.jsx)(Qn, { onClick: r }) : null), (t[10] = r), (t[11] = i), (t[12] = e))
      : (e = t[12]);
    let n;
    t[13] === p
      ? (n = t[14])
      : ((n =
          p ||
          (0, $.jsx)(K, {
            id: `codex.taskRow.title`,
            defaultMessage: `New chat`,
            description: `Default title for a Codex thread that doesn't have a title`,
          })),
        (t[13] = p),
        (t[14] = n));
    let a;
    t[15] === n
      ? (a = t[16])
      : ((a = (0, $.jsx)(`span`, {
          className: `min-w-0 flex-1 text-base text-token-foreground`,
          children: n,
        })),
        (t[15] = n),
        (t[16] = a));
    let o;
    return (
      t[17] !== e || t[18] !== a
        ? ((o = (0, $.jsxs)(`div`, {
            className: `flex min-w-0 items-center gap-1`,
            children: [e, a],
          })),
          (t[17] = e),
          (t[18] = a),
          (t[19] = o))
        : (o = t[19]),
      o
    );
  }
  if (a) {
    if (n.length === 0) return null;
    let e;
    return (
      t[20] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((e = (0, $.jsx)(K, {
            id: `header.recentChats`,
            defaultMessage: `Tasks`,
            description: `Header label for recent tasks`,
          })),
          (t[20] = e))
        : (e = t[20]),
      e
    );
  }
  return null;
}
function Qn(e) {
  let t = (0, $n.c)(7),
    { onClick: n } = e,
    r = Se(),
    i;
  t[0] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((i = (0, $.jsx)(K, { ...er.backButton })), (t[0] = i))
    : (i = t[0]);
  let a;
  t[1] === r ? (a = t[2]) : ((a = r.formatMessage(er.backButton)), (t[1] = r), (t[2] = a));
  let o;
  t[3] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((o = (0, $.jsx)(Be, { className: `size-3` })), (t[3] = o))
    : (o = t[3]);
  let s;
  return (
    t[4] !== n || t[5] !== a
      ? ((s = (0, $.jsx)(P, {
          chromeExtension: !0,
          extension: !0,
          children: (0, $.jsx)(ee, {
            tooltipContent: i,
            children: (0, $.jsx)(q, {
              color: `ghost`,
              size: `icon`,
              onClick: n,
              className: `opacity-70 hover:bg-transparent hover:opacity-100 focus:bg-transparent active:bg-transparent`,
              "aria-label": a,
              children: o,
            }),
          }),
        })),
        (t[4] = n),
        (t[5] = a),
        (t[6] = s))
      : (s = t[6]),
    s
  );
}
var $n,
  $,
  er,
  tr = e(() => {
    (($n = c()),
      _e(),
      u(),
      r(),
      we(),
      ae(),
      G(),
      ce(),
      Te(),
      Ye(),
      tt(),
      L(),
      M(),
      Le(),
      rt(),
      k(),
      Xe(),
      Mt(),
      Lt(),
      Gt(),
      qn(),
      An(),
      ($ = _()),
      (er = be({
        backButton: {
          id: `header.back`,
          defaultMessage: `Back`,
          description: `Back button label for returning to the previous screen`,
        },
        newChatTitle: {
          id: `header.newChatTitle`,
          defaultMessage: `New chat`,
          description: `Fallback title shown in the Chrome extension side panel header before a chat exists`,
        },
      })));
  });
export { tr as n, Jn as t };
//# sourceMappingURL=header-2.js.map
