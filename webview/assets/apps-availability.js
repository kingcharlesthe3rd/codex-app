...
function D(e) {
  let n = (0, k.c)(28),
    ...
    m = C(),
    _;
  n[0] === i ? (_ = n[1]) : ((_ = p(i)), (n[0] = i), (n[1] = _));
  let y = _,
    b;
  n[2] !== y || n[3] !== f
    ? ((b = async () => {
        await Promise.all([f(y), f(w), f([`user-saved-config`])]);
      }),
      (n[2] = y),
      (n[3] = f),
      (n[4] = b))
  ...
  let x = b,
    S;
  n[5] !== i ||
  n[6] !== u?.configWriteTarget?.expectedVersion ||
  n[7] !== u?.configWriteTarget?.filePath
    ? ((S = async (e) => {
        let { appId: t, enabled: n } = e;
        await h(`batch-write-config-value`, {
          hostId: i,
          edits: O({ appId: t, enabled: n }),
          filePath: u?.configWriteTarget?.filePath ?? null,
          expectedVersion: u?.configWriteTarget?.expectedVersion ?? null,
          reloadUserConfig: !0,
        });
      }),
      (n[5] = i),
  ...
  n[9] !== y || n[10] !== d
    ? ((E = async (e) => {
        let { appId: t, enabled: n } = e;
        await d.cancelQueries({ queryKey: y });
        let r = d.getQueryData(y);
        return (
          r &&
            d.setQueryData(
              y,
    ...
    : (E = n[11]);
  let D, A;
  n[12] !== y || n[13] !== m || n[14] !== d || n[15] !== s
    ? ((D = (e, t) => {
        let { appId: n, appName: r, enabled: i } = t,
          a = r ?? d.getQueryData(y)?.find((e) => e.id === n)?.name ?? n;
        s.get(T).success(m.formatMessage(i ? j.enableSuccess : j.disableSuccess, { appName: a }));
      }),
      (A = (e, t, n) => {
        (r.error(`Failed to update app enablement`, { safe: {}, sensitive: { error: e } }),
          s.get(T).danger(m.formatMessage(j.updateError)),
          n?.previousApps && d.setQueryData(y, n.previousApps));
      }),
      (n[12] = y),
      (n[13] = m),
      (n[14] = d),
    ...
    : ((D = n[16]), (A = n[17]));
  let M;
  n[18] !== x || n[19] !== S || n[20] !== E || n[21] !== D || n[22] !== A
    ? ((M = { mutationFn: S, onMutate: E, onSuccess: D, onError: A, onSettled: x }),
      (n[18] = x),
      (n[19] = S),
      (n[20] = E),
      (n[21] = D),
