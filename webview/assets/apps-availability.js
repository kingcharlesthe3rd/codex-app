...
function D(e) {
  let a = (0, k.c)(28),
    ...
    h = C(),
    _;
  a[0] === o ? (_ = a[1]) : ((_ = r(o)), (a[0] = o), (a[1] = _));
  let y = _,
    x;
  a[2] !== y || a[3] !== p
    ? ((x = async () => {
        await Promise.all([p(y), p(b), p([`user-saved-config`])]);
      }),
      (a[2] = y),
      (a[3] = p),
      (a[4] = x))
  ...
  let S = x,
    w;
  a[5] !== o ||
  a[6] !== l?.configWriteTarget?.expectedVersion ||
  a[7] !== l?.configWriteTarget?.filePath
    ? ((w = async (e) => {
  ...
  a[9] !== y || a[10] !== u
    ...
    : (E = a[11]);
  let D, A;
  a[12] !== y || a[13] !== h || a[14] !== u || a[15] !== c
    ? ((D = (e, t) => {
        let { appId: n, appName: r, enabled: i } = t,
          a = r ?? u.getQueryData(y)?.find((e) => e.id === n)?.name ?? n;
        c.get(v).success(h.formatMessage(i ? j.enableSuccess : j.disableSuccess, { appName: a }));
      }),
      (A = (e, n, r) => {
        (t.error(`Failed to update app enablement`, { safe: {}, sensitive: { error: e } }),
          c.get(v).danger(h.formatMessage(j.updateError)),
          r?.previousApps && u.setQueryData(y, r.previousApps));
      }),
      (a[12] = y),
      (a[13] = h),
      (a[14] = u),
    ...
    : ((D = a[16]), (A = a[17]));
  let M;
  a[18] !== S || a[19] !== w || a[20] !== E || a[21] !== D || a[22] !== A
    ? ((M = { mutationFn: w, onMutate: E, onSuccess: D, onError: A, onSettled: S }),
      (a[18] = S),
      (a[19] = w),
      (a[20] = E),
      (a[21] = D),
  ...
  let N = i(M),
    ...
    F;
  return (
    a[24] !== N.isPending || a[25] !== N.mutateAsync || a[26] !== P
