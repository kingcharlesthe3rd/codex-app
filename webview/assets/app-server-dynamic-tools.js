...
import { _ as t, t as n } from "./app-scope-CWE-zIhQ.js";
import {
  Bt as r,
  F as i,
  I as a,
...
import { f as ae, h as A, n as j } from "./vscode-api.js";
import {
  Aa as M,
  Ar as N,
  Br as oe,
...
import { F as he } from "./rpc-1.js";
import { d as ge } from "./statsig.js";
import { t as _e } from "./v4.js";
import { t as ve } from "./model-list-filter.js";
import { t as ye } from "./remote-projects.js";
import { t as V } from "./app-intl-signal.js";
import { t as be } from "./windows-tabs-open-schema.js";
import { f as xe } from "./sidebar-project-group-signals.js";
import { t as Se } from "./read-service-tier-for-request.js";
import { S as Ce, n as we } from "./realtime-controller.js";
import { t as Te } from "./pending-worktree-store.js";
import { t as Ee } from "./threads-create.js";
import { r as De } from "./thread-actions-B.js";
import { n as Oe } from "./web-search-favicon-icon.js";
import { t as ke } from "./score-query-match.js";
import { r as Ae } from "./thread-handoff-composer-block-state-C7-ef4-4.js";
import { t as je } from "./automation-host-support-C.js";
import { t as Me } from "./get-default-branch-name.js";
import { t as Ne } from "./worktree-query-keys.js";
import { n as Pe, r as Fe, t as Ie } from "./run-app-action-in-primary-window.js";
import { t as Le } from "./get-move-to-local-targets.js";
async function Re({ scope: e, threadId: t }) {
  let n = (
      await Promise.all(
        ze(e).map(async (e) => {
...
function ze(e) {
  return e.get(ie).filter((t) => {
    let n = t.getHostId();
    return n === `local` || e.get(ee, n) === `connected`;
  });
}
async function Be({ pinned: e, threadId: t }) {
  return (
    await j(`set-thread-pinned`, { params: { threadId: t, pinned: e } }), { threadId: t, pinned: e }
  );
}
async function Ve({ archived: e, scope: t, threadId: n }) {
  let { hostId: i } = await Re({ scope: t, threadId: n }),
    a = L(n);
  return (
    e
      ? (await r(`hydrate-background-threads`, { hostId: i, threadIds: [a] }),
        await r(`archive-conversation`, { hostId: i, conversationId: a, source: `dynamic_tool` }))
      : await r(`unarchive-conversation`, { hostId: i, conversationId: a }),
    { threadId: n, archived: e }
  );
}
async function He({ scope: e, threadId: t, title: n }) {
  let { hostId: i } = await Re({ scope: e, threadId: t });
  return (
    await r(`set-thread-title`, { hostId: i, conversationId: L(t), title: n }),
    { threadId: t, title: n }
  );
}
async function Ue({
  ...
  let { hostId: o, manager: s } = await Re({ scope: e, threadId: i }),
...
function Ge(e, t, n) {
  return {
    ...
    items: e.items.map((e) => Ke(e, t, n)),
  };
}
function Ke(e, t, n) {
  switch (e.type) {
    case `userMessage`:
      return { type: e.type, id: e.id, content: e.content.map(qe) };
    case `agentMessage`:
    ...
    case `reasoning`:
      return {
        ...
        id: e.id,
        summary: e.summary,
        ...(t ? { content: e.content.map((e) => Je(e, n)) } : {}),
      };
    case `commandExecution`:
      return {
        ...
        exitCode: e.exitCode,
        durationMs: e.durationMs,
        ...(t && e.aggregatedOutput != null ? { output: Je(e.aggregatedOutput, n) } : {}),
      };
    case `fileChange`:
      return {
        type: e.type,
...
function Ye({ sourceConversationId: e, targetConversationId: t, diffComments: n }) {
...
async function Xe({
  ...
  let s = null,
    c = null;
  try {
    let l = await j(`extension-info`),
      u = { app_version: l.version };
    if (
      (n != null && Object.assign(u, n),
      l.buildNumber != null && (u.app_build = l.buildNumber),
      l.buildFlavor != null && (u.buildFlavor = l.buildFlavor),
      a)
    ) {
      let e = await j(`feedback-desktop-log-archive`);
      ((s = e.archiveId), (c = e.archivePath));
    }
    let d = await r(`submit-feedback`, {
      hostId: i,
      classification: e,
      reason: t,
      threadId: o,
      includeLogs: a,
      extraLogFiles: c == null ? void 0 : [c],
      tags: u,
    });
    return { appVersion: l.version, buildNumber: l.buildNumber, feedbackId: d.threadId };
  } finally {
    if (s != null)
      try {
        await j(`delete-feedback-desktop-log-archive`, { params: { archiveId: s } });
      } catch (e) {
        A.warning(`Failed to delete feedback desktop log archive`, {
          safe: {},
          sensitive: { error: e },
        });
      }
...
async function Ze({ conversationId: e, sourceThreadRunning: t }) {
  t && (await r(`interrupt-conversation`, { conversationId: e, initiatedBy: `system` }));
}
var Qe = [
...
async function $e({ sourceConversationId: e, targetConversationId: t }) {
  if (e === t) return;
  let { threadIds: n } = await j(`list-pinned-threads`, {});
  n.indexOf(e) !== -1 &&
    (await j(`set-pinned-threads-order`, {
      params: {
        threadIds: n.map((n) => (n === e ? t : n)).filter((e, t, n) => n.indexOf(e) === t),
      },
    }));
}
async function et({ sourceConversationId: e, targetConversationId: t, getTitle: n, setTitle: r }) {
  if (e === t) return;
  let i = n(e);
  i != null && (await r(t, i));
}
async function tt({
  ...
  if (b?.forkedFromId != null)
    try {
      ...
      ((S = e.worktreeGitRoot), (w = e.worktreeWorkspaceRoot), (T = e.hasUncommittedChanges));
    } catch {
      A.warning(
  ...
  let D = await _(`git`).request({
    ...
    params: {
      ...
      destinationWorktreeWorkspaceRoot: w,
      stashDestinationWorktree: T,
    },
  });
  if (D.status === `error`)
    return {
      ...
      execOutput: D.execOutput ?? null,
    };
  (d.invalidateQueries({ queryKey: Ne(C(c)) }), p?.());
  let O;
  try {
    O = await r(`fork-conversation-from-rollout-path`, {
      hostId: c.id,
      conversationId: e,
      rolloutPath: D.rolloutPath,
      cwd: D.worktreeWorkspaceRoot,
      workspaceRoots: [D.worktreeWorkspaceRoot],
      addForkedSyntheticItem: !1,
    });
  } catch (t) {
    try {
      await j(`worktree-set-owner-thread`, {
        params: { hostId: c.id, worktree: D.worktreeGitRoot, conversationId: e },
      });
    } catch {}
    return (
      A.warning(`Cross-host handoff moved git state, but conversation fork failed: {}`, {
        sensitive: { error: N(t) },
        safe: {},
      }),
      h?.(),
    ...
    await H(`Cross-host handoff copied rollout cleanup failed: {}`, () =>
      _(`git`).request({
  ...
  return (
    v?.(O),
    g?.(D.worktreeWorkspaceRoot),
    await E(O, t),
    await H(
      `Cross-host handoff succeeded, but worktree ownership metadata could not be updated: {}`,
      () =>
        j(`worktree-set-owner-thread`, {
          params: { hostId: c.id, worktree: D.worktreeGitRoot, conversationId: O },
        }),
    ),
    await H(`Cross-host handoff succeeded, but browser state could not be transferred: {}`, () =>
      l(e, O),
    ),
    await H(`Cross-host handoff succeeded, but thread title could not be transferred: {}`, () =>
      et({
        sourceConversationId: e,
        targetConversationId: O,
        getTitle: (e) => o.getConversation(e)?.title ?? null,
        setTitle: (e, t) => r(`set-thread-title`, { conversationId: e, title: t }),
      }),
    ),
    await H(
      `Cross-host handoff succeeded, but saved diff comments could not be transferred: {}`,
      () => {
        u((t) => Ye({ sourceConversationId: e, targetConversationId: O, diffComments: t }));
      },
    ),
    await H(
      `Cross-host handoff succeeded, but pinned-thread state could not be transferred: {}`,
      () => $e({ sourceConversationId: e, targetConversationId: O }),
    ),
    m?.(),
    { status: `success`, conversationId: O }
  );
}
async function H(e, t) {
  try {
    ...
    A.warning(e, { sensitive: { error: N(t) }, safe: {} });
  }
}
async function nt({
  conversationId: e,
  ...
  if (x.status === `error`) {
    ...
    return {
      status: `error`,
      message: `${rt({ branch: t, error: x.error, intl: c })}${e}`,
      execOutput: x.execOutput,
    };
  }
  (x.warnings.includes(`drop-source-stash-failed`) &&
    A.warning(`Move to local succeeded, but source stash cleanup failed`),
    m?.());
  let S;
  try {
    S = await r(`fork-conversation-from-latest`, {
      hostId: l.getHostId(),
      conversationId: e,
      cwd: b,
      workspaceRoots: [b],
    ...
    return (
      A.warning(`Move to local git operations succeeded, but conversation fork failed: {}`, {
        sensitive: { error: N(t) },
        safe: {},
      }),
      g?.(),
  ...
  try {
    await j(`worktree-set-owner-thread`, {
      params: { hostId: f.id, worktree: o, conversationId: S },
    });
  } catch (e) {
    A.warning(`Move to local succeeded, but worktree ownership metadata could not be updated: {}`, {
      sensitive: { error: N(e) },
      safe: {},
    });
  }
  try {
    ...
    A.warning(`Move to local succeeded, but browser state could not be transferred: {}`, {
      sensitive: { error: N(e) },
      safe: {},
    });
  }
  try {
    ...
    A.warning(`Move to local succeeded, but thread title could not be transferred: {}`, {
      sensitive: { error: N(e) },
      safe: {},
    });
  }
  try {
    d((t) => Ye({ sourceConversationId: e, targetConversationId: S, diffComments: t }));
  } catch (e) {
    A.warning(`Move to local succeeded, but saved diff comments could not be transferred: {}`, {
      sensitive: { error: N(e) },
      safe: {},
    });
  }
  try {
    await $e({ sourceConversationId: e, targetConversationId: S });
  } catch (e) {
    A.warning(`Move to local succeeded, but pinned-thread state could not be transferred: {}`, {
      sensitive: { error: N(e) },
      safe: {},
    });
  }
  return (h?.(), { status: `success`, conversationId: S });
}
function rt({ branch: e, error: t, intl: n }) {
  switch (t) {
    case `invalid-params`:
...
async function it({
  ...
  try {
    ...
    ((y = t.worktreeGitRoot), (b = t.worktreeWorkspaceRoot), (S = t.hasUncommittedChanges));
  } catch {
    A.warning(
      `Failed to resolve an existing worktree for thread. Falling back to creating a new worktree.`,
    );
  }
  if (y != null && b != null) ((w = `reuse-existing-worktree`), m?.(w, `running`), m?.(w, `done`));
  else {
    m?.(w, `running`);
    try {
      let e = await j(`worktree-create-managed`, {
        params: {
          hostId: d.id,
          cwd: n,
      ...
      ((y = e.worktreeGitRoot),
        (b = e.worktreeWorkspaceRoot),
        (x = !0),
        (S = !1),
        m?.(w, `done`),
        f.invalidateQueries({ queryKey: Ne(C(d)) }));
    } catch (e) {
      return (
        m?.(w, `failed`),
        {
  ...
  let T = await j(`git-create-branch`, {
    source: `move_to_worktree_dialog`,
    params: { cwd: b, branch: v, hostId: d.id, mode: `worktree` },
  });
  if (T.status === `error`) {
    if (x)
      try {
        await j(`worktree-delete`, {
          params: { hostId: d.id, worktree: y, reason: `new-branch-cleanup` },
        });
      } catch (e) {
        A.warning(`Failed to clean up the newly created worktree: {}`, {
          sensitive: { error: N(e) },
          safe: {},
        });
      } finally {
  ...
  let E = await _(`git`).request({
    ...
    params: {
      ...
      worktreeCheckoutBranch: v,
      worktreeGitRoot: y,
      worktreeWorkspaceRoot: b,
      stashTargetWorktree: S,
      createdWorktree: x,
    },
  });
  if (E.status === `error`) {
    let e =
      E.rollbackErrors.length > 0
        ? o.formatMessage(
            ...
            { issues: E.rollbackErrors.join(`, `) },
          )
        : ``;
    return {
      status: `error`,
      message:
        (E.execOutput?.output.length ?? 0) > 0
  ...
  let D;
  try {
    D = await r(`fork-conversation-from-latest`, {
      hostId: s.getHostId(),
      conversationId: e,
      cwd: b,
      workspaceRoots: [b],
    ...
    try {
      await j(`worktree-set-owner-thread`, {
        params: { hostId: d.id, worktree: y, conversationId: e },
      });
    } catch {}
    return (
      A.warning(`Move to worktree git operations succeeded, but conversation fork failed: {}`, {
        sensitive: { error: N(t) },
        safe: {},
      }),
  ...
  try {
    await j(`worktree-set-owner-thread`, {
      params: { hostId: d.id, worktree: y, conversationId: D },
    });
  } catch (e) {
    A.warning(
      `Move to worktree succeeded, but worktree ownership metadata could not be updated: {}`,
      { sensitive: { error: N(e) }, safe: {} },
    );
  }
  try {
    l(e, D);
  } catch (e) {
    A.warning(`Move to worktree succeeded, but browser state could not be transferred: {}`, {
      sensitive: { error: N(e) },
      safe: {},
    });
  }
  try {
    ...
    A.warning(`Move to worktree succeeded, but thread title could not be transferred: {}`, {
      sensitive: { error: N(e) },
      safe: {},
    });
  }
  try {
    u((t) => Ye({ sourceConversationId: e, targetConversationId: D, diffComments: t }));
  } catch (e) {
    A.warning(`Move to worktree succeeded, but saved diff comments could not be transferred: {}`, {
      sensitive: { error: N(e) },
      safe: {},
    });
  }
  (E.warnings.includes(`stashed-target-worktree-changes`) &&
    A.warning(`Move to worktree stashed uncommitted changes in the reused worktree`),
    E.warnings.includes(`drop-source-stash-failed`) &&
      A.warning(`Move to worktree succeeded, but source stash cleanup failed`),
    E.warnings.includes(`drop-target-stash-failed`) &&
      A.warning(`Move to worktree succeeded, but target stash cleanup failed`));
  try {
    await $e({ sourceConversationId: e, targetConversationId: D });
  } catch (e) {
    A.warning(`Move to worktree succeeded, but pinned-thread state could not be transferred: {}`, {
      sensitive: { error: N(e) },
      safe: {},
    });
  }
  return (m?.(`switching-thread`, `done`), { status: `success`, conversationId: D });
}
var at = `open_in_codex`,
  ot = {
    name: at,
    ...
    inputSchema: ue().parse(ce(be)),
  };
async function st({ argumentsValue: e, hostId: t, threadId: n }) {
  let r = be.safeParse(e);
  if (!r.success) return k(`${at} received invalid arguments.`);
  try {
    let e = await Ie(
    ...
    return k(e instanceof Error ? e.message : `Failed to open Codex tab.`);
  }
}
var ct = `send_realtime_voice_feedback`,
  lt = `[Realtime Voice]`,
  ut = R({
  ...
  dt = {
    ...
    inputSchema: ue().parse(ce(ut)),
  };
async function ft({ argumentsValue: e, hostId: t, threadId: n }) {
  let r = ut.safeParse(e);
  if (!r.success) return k(`${ct} received invalid arguments.`);
  try {
    let e = await Xe({
      classification: `other`,
      description: `${lt} ${r.data.feedback}`,
    ...
    return (
      A.error(`Failed to submit realtime voice feedback`, {
        safe: { threadId: n },
        sensitive: { error: e },
      }),
      k(`Failed to submit realtime voice feedback.`)
    );
  }
}
var U = `speak_to_user`,
  pt = R({ message: M().min(1).describe(`The exact message to speak to the user.`) });
function mt(e = Ce) {
  return { name: U, description: e, inputSchema: ue().parse(ce(pt)) };
}
async function ht({ argumentsValue: e, hostId: t, threadId: n }) {
  let i = pt.safeParse(e);
  if (!i.success) return k(`${U} received invalid arguments.`);
  try {
    return (
      await r(`send-cli-request-for-host`, {
        hostId: t,
        method: `thread/realtime/appendSpeech`,
        params: { threadId: n, text: i.data.message },
      }),
      { contentItems: [{ type: `inputText`, text: JSON.stringify({ spoken: !0 }) }], success: !0 }
    );
  } catch (e) {
    return (
      A.error(`Failed to speak realtime voice message`, {
        safe: { threadId: n },
        sensitive: { error: e },
      }),
      k(`Failed to speak realtime voice message.`)
    );
  }
}
var gt = `read_settings`,
  _t = `write_settings`,
  vt = {
    name: gt,
...
async function bt({ scope: e }) {
  return { schemaVersion: 1, projects: xt(e) };
}
function xt(e) {
  return e.get(xe).map((e) => {
...
var St = `The fork contains completed history only. If the source thread was running, the active turn and unfinished response are not in the child. Send a follow-up message to threadId only if the task requires work to continue there.`,
  Ct = `The worktree is being created asynchronously, so no child thread id exists yet. Wait for pendingWorktreeId to finish, then send a follow-up message to the child only if the task requires work to continue there.`;
async function wt({
  ...
  switch (e.type) {
    case `same-directory`:
      return {
        environment: e,
        sourceThreadId: a,
        threadId: await r(`fork-conversation-from-latest`, {
          hostId: o,
          conversationId: a,
          cwd: s ?? void 0,
    ...
    case `worktree`: {
      ...
      return {
        ...
        pendingWorktreeId: Te({
          hostId: o,
          label: n.formatMessage(De.forkPendingWorktreeTitle),
          sourceWorkspaceRoot: s,
          startingState: e.startingState ?? { type: `working-tree` },
          localEnvironmentConfigPath: null,
          launchMode: `fork-conversation`,
          prompt: n.formatMessage(De.forkPendingWorktreePrompt),
...
function Dt(e) {
  ...
  return { threadId: e.id, title: Tt(e), cwd: e.cwd, updatedAt: Number.isFinite(t) ? t : 0 };
}
function Ot(e, t, n) {
  if (n <= 0) return [];
  let r = t.trim();
  ...
  return (i.sort(jt), i.map(({ candidate: e }) => e));
}
function kt(e, t) {
  return Mt(e.title, t, 0) ?? Mt(e.branch ?? ``, t, 1) ?? Mt(e.cwd, t, 2);
}
function At(e) {
  let t = 0;
  for (let n = 1; n < e.length; n += 1) jt(e[t], e[n]) < 0 && (t = n);
  return t;
}
function jt(e, t) {
...
async function Nt({ scope: e, limit: t, query: n }) {
  let r = n?.trim() ?? ``,
    i = (
      await Promise.all(
        e
  ...
  return {
    ...
    threads: Pt(i, r, t).map(({ hostId: e, thread: t }) => Ft(t, e)),
  };
}
function Pt(e, t, n) {
  if (t.length === 0)
...
async function Lt({
  ...
  let c = e ?? (await Re({ scope: i, threadId: o })).hostId,
    l = L(o),
    u = a == null ? n : d({ sourceThreadId: a, input: n });
  await r(`send-follow-up-message`, {
    hostId: c,
    conversationId: l,
    prompt: u,
    ...(t == null ? {} : { model: t }),
    ...(s == null ? {} : { reasoningEffort: s }),
    serviceTier: await Se(i, c, t ?? null),
  });
  let f = i.get(we);
  if (a != null && f != null) {
    let e = L(a),
      t = f === l ? e : f === e ? l : null,
      n = f === l ? `from-thread` : f === e ? `to-thread` : null;
    if (t == null || n == null) return { threadId: o };
    b({
      direction: n,
      prompt: g(u)?.input ?? u,
      realtimeThreadId: f,
...
function K(e, t, n) {
  ...
  (e.set(zt, t, (e) => {
    ...
    G(e, t)?.revision !== r?.revision && Ht(t));
}
function Bt({ scope: e, operationId: t, waitMs: n, afterRevision: r }) {
  let i = G(e, t);
  return i == null || n === 0 || (r != null && i.revision > r) || Vt(i.status)
    ? Promise.resolve(i)
    : new Promise((r) => {
...
var Ut = e(u(), 1),
  ...
  Wt = 6e4,
  Gt = { id: m, display_name: `Local`, kind: `local` },
  Kt = R({
    threadId: M().min(1),
  ...
  qt = R({
    operationId: M().min(1),
    afterRevision: z().int().min(0).optional(),
    waitMs: z().int().min(0).max(Wt).optional(),
  }),
  Jt = `handoff_thread`,
  Yt = `get_handoff_status`;
function Xt(e) {
  return (0, Ut.default)([h(e.get, `host_config`) ?? Gt, ...e.get(te).map(pe)], ({ id: e }) => e);
}
function Zt(e = [Gt], t = !1) {
  return {
    name: Jt,
    description:
  ...
  return {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        waitMs: {
          type: `number`,
          description: `Optional maximum milliseconds to wait for a status change, from 0 to ${Wt}.`,
        },
      },
...
async function $t({
  ...
  callId: r,
  callingThreadId: i,
  crossHostHandoffEnabled: a = ge(e, `2256010998`),
  queryClient: o,
}) {
  let s = Kt.safeParse(n);
  if (!s.success) return k(`${Jt} received invalid arguments.`);
  let c = L(s.data.threadId);
  if (c === i) return k(`A thread cannot hand itself off. Choose another thread.`);
  try {
    let n = r,
      l = G(e, n);
    if (l != null) return mn(hn(l));
    let u = Xt(e),
      d = await tn({
        scope: e,
        appServerRegistry: t,
    ...
    await Ze({ conversationId: c, sourceThreadRunning: v(d.conversation) });
    let p = f.id === d.hostConfig.id ? await nn(d) : await rn(d, f);
    return (
      K(e, n, p.progress),
      an({
        callingThreadId: i,
      ...
      mn(hn(G(e, n) ?? p.progress))
    );
  } catch (e) {
    return (
      A.warning(`Thread handoff dynamic tool failed: {}`, {
        sensitive: { error: N(e) },
        safe: { threadId: c },
      }),
      k(e instanceof Error ? e.message : `Thread handoff failed.`)
    );
  }
}
async function en({ scope: e, argumentsValue: t }) {
  let n = qt.safeParse(t);
  if (!n.success) return k(`${Yt} received invalid arguments.`);
  let r = await Bt({
  ...
  return r == null
    ? k(`No thread handoff operation found for operationId ${n.data.operationId}.`)
    : mn(hn(r));
}
async function tn({
  scope: e,
  ...
  let s = t.getMaybeForConversationId(o) ?? t.getDefault();
  await r(`maybe-resume-conversation`, {
    hostId: s.getHostId(),
    conversationId: o,
    model: null,
    serviceTier: null,
  ...
  let c = s.getConversation(o);
  if (c == null) throw Error(`Thread ${o} could not be loaded for handoff.`);
  if (e.get(Ae, o) > 0 || (w(e.get, oe.QUEUED_FOLLOW_UPS)?.[o]?.length ?? 0) > 0)
    throw Error(
      `Thread ${o} has pending composer state. Send or remove it before handing the thread off.`,
    );
  let l = c.cwd?.trim() ?? ``;
  if (l.length === 0) throw Error(`Thread ${o} does not have a workspace to hand off.`);
  let u = i.find(({ id: e }) => e === s.getHostId());
  if (u == null) throw Error(`The source host for thread ${o} is not available.`);
  let d = I(l),
    f = await _(`git`).request({
      method: `stable-metadata`,
      params: { cwd: d, hostConfig: u, operationSource: q },
    });
  if (f == null) throw Error(`The source thread workspace is not a git repository.`);
  let { branch: p } = await _(`git`).request({
      method: `current-branch`,
      params: { root: f.root, hostConfig: u, operationSource: q },
    }),
    m = c.title?.trim() ?? ``;
  return {
    operationId: n,
...
async function nn(e) {
  let { codexHome: t } = await j(`codex-home`, { params: { hostId: e.hostConfig.id } });
  if (le(e.cwd, t)) {
    let n = await cn(e, t),
      r = un(e, e.currentBranch);
    return {
      destinationHostId: e.hostConfig.id,
      progress: fn(e, {
        destinationHostDisplayName: e.hostConfig.display_name,
        destinationHostId: e.hostConfig.id,
        destinationCwd: I(n.workspaceRoot),
        direction: `to-local`,
        localBranch: r,
        sourceBranch: r,
        steps: pn([
      ...
      run: () =>
        nt({
          ...
          transferBrowserState: dn,
          setDiffComments: (t) => f(e.scope, `diff_comments`, t),
...
async function rn(e, t) {
  ...
  let n = un(e, e.currentBranch),
    [{ roots: r }, { origins: i }] = await Promise.all([
      j(`workspace-root-options`, { params: { hostId: t.id } }),
      j(`git-origins`, { params: { hostId: e.hostConfig.id, dirs: [e.cwd] }, source: q }),
    ]),
    { origins: a } = await j(`git-origins`, { params: { hostId: t.id, dirs: r }, source: q }),
    o = ye({
...
function an({ callingThreadId: e, context: t, destinationHostId: n, followUpPrompt: r, run: i }) {
  on({ callingThreadId: e, context: t, destinationHostId: n, followUpPrompt: r, run: i });
}
async function on({
  callingThreadId: e,
  context: t,
  ...
  let a = _(`git`).subscribe(`thread-handoff-progress`, (e) => {
  ...
  try {
    ...
    if (a.conversationId === t.threadId) {
      yn(t, `Git handoff completed, but the destination thread could not be created.`);
      return;
    }
    ...
    (A.warning(`Thread handoff failed unexpectedly: {}`, {
      sensitive: { error: N(e) },
      safe: { operationId: t.operationId },
    }),
      vn(t, e instanceof Error ? e.message : `Thread handoff failed.`));
  } finally {
    a();
  }
}
function sn(e) {
  r(`archive-conversation`, {
    ...
    A.warning(`Thread handoff succeeded, but source archive failed: {}`, {
      sensitive: { error: N(t) },
...
async function cn(e, t) {
  let [{ worktrees: n }, { worktrees: r }] = await Promise.all([
    ...
    i = new Set(r.map(({ dir: e }) => he(e))),
    a = Le({
      cwd: e.cwd,
      sourceWorktreeRoot: I(e.gitRoot),
      repoWorktreeEntries: n.filter(({ root: e }) => !i.has(he(e)) && !le(e, t)),
    })[0];
  if (a == null) throw Error(`No checkout was found for the source worktree.`);
  return a;
}
async function ln(e, t, n) {
  let { branches: r } = await _(`git`).request({
...
function un(e, t) {
  ...
  let r = me(e.scope.get, se.branchPrefix),
    i = Me({ branchPrefix: r, conversationTitle: e.conversation.title });
  return i.length > 0 && !i.endsWith(`/`)
    ? i
    : Me({ branchPrefix: r, conversationTitle: e.threadId });
}
function dn(e, t) {
...
function J(e, t, n) {
  K(e.scope, e.operationId, (e) => {
    ...
    let a = i == null ? [...e.steps, r].sort(Sn) : e.steps.map((e) => (e.id === t ? r : e));
    return { ...e, steps: a };
  });
}
function gn(e, t) {
...
function vn(e, t) {
  K(e.scope, e.operationId, (e) => bn(e, t));
}
function yn(e, t) {
  K(e.scope, e.operationId, (e) =>
...
function Sn(e, t) {
  return Qe.indexOf(e.id) - Qe.indexOf(t.id);
}
var X = `received invalid arguments.`,
  Cn = 10,
  wn = 1,
  Tn = 2e3,
  En = `Do not specify a model unless the user explicitly requests a specific model. Otherwise omit this field so the new thread uses the user's configured default model.`,
  Dn = fe([`low`, `medium`, `high`, `xhigh`, `max`]),
  On = F(`type`, [
    P({ type: B(`working-tree`) }),
    P({ type: B(`branch`), branchName: M().min(1) }),
  ]),
  kn = F(`type`, [
    P({ type: B(`local`) }),
    P({ type: B(`worktree`), startingState: On.optional() }),
  ]),
  An = F(`type`, [
    P({ type: B(`project`), projectId: M().min(1), environment: kn }),
    P({ type: B(`projectless`), directoryName: M().min(1).optional() }),
  ]),
  jn = F(`type`, [
    P({ type: B(`same-directory`) }),
    P({ type: B(`worktree`), startingState: On.optional() }),
  ]),
  Z = `fork_thread`,
  Mn = `list_projects`,
  ...
  Pn = `list_threads`,
  Fn = `read_thread`,
  In = `send_message_to_thread`,
  Ln = `set_thread_pinned`,
  Q = `set_thread_archived`,
  Rn = `set_thread_title`,
  zn = R({ prompt: M().min(1), target: An, model: M().min(1).optional(), thinking: Dn.optional() }),
  Bn = R({ threadId: M().min(1).optional(), environment: jn.optional() }),
  Vn = R({}),
  Hn = R({ query: M().optional(), limit: z().int().min(1).max(50).optional() }),
  Un = R({
    threadId: M().min(1),
    cursor: M().min(1).optional(),
    turnLimit: z().int().min(1).max(10).optional(),
    includeOutputs: de().optional(),
    maxOutputCharsPerItem: z().int().min(0).max(2e4).optional(),
  }),
  Wn = R({
    threadId: M().min(1),
    prompt: M().min(1),
    model: M().min(1).optional(),
    thinking: Dn.optional(),
  }),
  Gn = R({ threadId: M().min(1), pinned: de() }),
  Kn = R({ threadId: M().min(1).optional(), archived: de() }),
  qn = R({ threadId: M().min(1), title: M().min(1) });
function Jn({
  ...
  return [cr, Zt(e, n), Qt(), or, hr(sr, t), lr, ur, hr(dr, t), fr, pr, mr].map((e) =>
    r ? { ...e, deferLoading: !0 } : e,
  );
}
async function Yn({ scope: e, argumentsValue: t, sourceThreadId: n }) {
  if (n == null) return k(`${Z} missing calling thread id.`);
  let r = Bn.safeParse(t);
  if (!r.success) return k(`${Z} ${X}`);
  try {
    return $(
      await wt({
        scope: e,
    ...
    return k(e instanceof Error ? e.message : String(e));
  }
}
async function Xn({ argumentsValue: e, scope: t, sourceThreadId: n }) {
  let r = zn.safeParse(e);
  if (!r.success) return k(`${Nn} ${X}`);
  try {
    return $(
      await Ee({
        model: r.data.model,
...
async function Zn({ scope: e, argumentsValue: t }) {
  if (!Vn.safeParse(t).success) return k(`${Mn} ${X}`);
  try {
    return $(await bt({ scope: e }));
  } catch (e) {
    return k(e instanceof Error ? e.message : String(e));
  }
}
async function Qn({ scope: e, argumentsValue: t }) {
  ...
  if (!n.success) return k(`${Pn} ${X}`);
  try {
    return $(await Nt({ scope: e, limit: n.data.limit ?? Cn, query: n.data.query }));
  } catch (e) {
    return k(e instanceof Error ? e.message : String(e));
  }
}
async function $n({ scope: e, argumentsValue: t }) {
  let n = Un.safeParse(t);
  if (!n.success) return k(`${Fn} ${X}`);
  try {
    return $(
      await Ue({
        scope: e,
        ...
        maxOutputCharsPerItem: n.data.maxOutputCharsPerItem ?? Tn,
        threadId: n.data.threadId,
        turnLimit: n.data.turnLimit ?? wn,
      }),
    );
  } catch (e) {
    return k(e instanceof Error ? e.message : String(e));
...
async function er({ argumentsValue: e, scope: t, sourceThreadId: n }) {
  let r = Wn.safeParse(e);
  if (!r.success) return k(`${In} ${X}`);
  try {
    return $(
      await Lt({
        model: r.data.model,
...
async function tr({ argumentsValue: e }) {
  let t = Gn.safeParse(e);
  if (!t.success) return k(`${Ln} ${X}`);
  try {
...
async function nr({ argumentsValue: e, sourceThreadId: t, scope: n }) {
  let r = Kn.safeParse(e);
  if (!r.success) return k(`${Q} ${X}`);
  let i = r.data.threadId ?? t;
  if (i == null) return k(`${Q} missing calling thread id.`);
  try {
    return $(await Ve({ ...r.data, threadId: i, scope: n }));
  } catch (e) {
    return k(e instanceof Error ? e.message : String(e));
  }
}
async function rr({ argumentsValue: e, scope: t }) {
  let n = qn.safeParse(e);
  if (!n.success) return k(`${Rn} ${X}`);
  try {
    return $(await He({ ...n.data, scope: t }));
  } catch (e) {
    return k(e instanceof Error ? e.message : String(e));
  }
}
function ir(e, t) {
  ...
  let n = xt(e).find((e) => e.projectId === t.projectId);
  if (n == null)
    throw Error(
  ...
  sr = {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        model: { type: `string`, description: En },
        thinking: {
          type: `string`,
...
function hr(e, t) {
  ...
  let n = t
    .map((e) =>
      e.description.trim().length === 0 ? e.model : `${e.model} (${e.description.trim()})`,
    )
    .join(`, `);
  return {
    ...e,
    inputSchema: {
      ...
      properties: {
        ...
        model: {
          type: `string`,
          description: `${e.inputSchema.properties.model.description} Available models: ${n}. You may supply a newer model id when explicitly requested.`,
        },
      },
    },
  };
...
var gr = 100,
  ...
  vr = 1e3,
  yr = `codex_app`,
  br = new Set([T.name, y.name, dt.name, U, c, l, i, o, Fe]);
async function xr({
  availableHandoffHosts: e,
  authMethod: t,
  crossHostHandoffEnabled: n = !1,
  ...
  hostId: i,
  isAuthLoading: o,
  listExperimentalFeatures: c,
  listModels: l,
  modelAvailabilityConfig: u,
  speakToUserEnabled: d = !1,
  speakToUserToolDescription: f = Ce,
  threadStartKind: m = `default`,
}) {
  let h = je(i),
    g = r?.[S] === !0,
    _ = m === `realtime_voice` && r?.realtime_conversation === !0,
    v = r?.[re] === !0,
    b = r?.[s] === !0,
    [C, w] = await Promise.all([
      h ? Tr(c) : !1,
      g ? Sr({ authMethod: t, isAuthLoading: o, listModels: l, modelAvailabilityConfig: u }) : [],
    ]);
  return [
    {
      type: `namespace`,
      ...
      tools: [
        ...(h ? [x()] : []),
        ...(r?.open_in_codex === !0 ? [ot] : []),
        T,
        ...(h && C ? [y] : []),
        ...(_ ? [dt, ...(d ? [mt(f)] : [])] : []),
        ...(g
          ? Jn({ availableHandoffHosts: e, availableModels: w, crossHostHandoffEnabled: n })
          : []),
        ...(h && v ? [vt, yt] : []),
        ...(m === `conversational_onboarding` ? [Pe] : []),
        ...(b && m !== `conversational_onboarding` ? [...p, a] : []),
      ].map((e) => ({ type: `function`, ...e, ...(br.has(e.name) ? {} : { deferLoading: !0 }) })),
    },
  ];
}
async function Sr({ authMethod: e, isAuthLoading: t, listModels: n, modelAvailabilityConfig: r }) {
  if (t) return [];
  try {
    let t = await Cr(n),
      { models: i } = ve({
...
async function wr(e) {
  let t = [],
    n = null;
  do {
    let r = await e({ cursor: n, includeHidden: !0, limit: _r });
    (t.push(...r.data), (n = r.nextCursor));
  } while (n != null);
  return t;
}
async function Tr(e, t = null) {
  try {
    let n = await e({ cursor: t, limit: gr });
    return Oe(n.data) ? !0 : n.nextCursor == null ? !1 : await Tr(e, n.nextCursor);
  } catch {
    return !1;
  }
}
