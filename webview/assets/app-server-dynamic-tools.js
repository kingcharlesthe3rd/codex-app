...
import { t as pe } from "./workspace-dependencies-feature.js";
import { t as me } from "./score-query-match.js";
async function C({ config: e, model: t, prompt: n, sourceThreadId: r, target: i, thinking: a }) {
  if (e != null && r != null)
  ...
  return i.type === `project` && i.environment.type === `worktree`
    ...
    : he({ config: e, model: t, prompt: n, sourceThreadId: r, target: i, thinking: a });
}
async function he({ config: e, model: t, prompt: n, sourceThreadId: r, target: i, thinking: a }) {
  let o, s, c, l, u;
  switch (i.type) {
    case `project`: {
      let e = await de({ projectId: i.projectId, prompt: n, validateProjectId: !0 });
      (e == null
...
async function ge({
  ...
  let { roots: o } = await S(`workspace-root-options`, { params: { hostId: b } });
  if (!o.includes(t))
    throw Error(
      `Unknown projectId: ${t}\nSaved projectIds:\n${o.join(`
`)}`,
...
async function _e(t, n) {
  let { config: r } = await h(`read-config-for-host`, { hostId: b, includeLayers: !1, cwd: t });
  return e(x(`agent-mode-by-host-id`, {}).local ?? `auto`, n, r);
}
async function ve({ input: e, model: t, projectId: n, threadSource: r, thinking: i }) {
  let { config: a } = await h(`read-config-for-host`, { hostId: b, includeLayers: !1, cwd: n });
  return {
    input: e,
    workspaceRoots: [n],
    cwd: n,
...
async function ye({ pinned: e, threadId: t }) {
  return (
    await S(`set-thread-pinned`, { params: { threadId: t, pinned: e } }), { threadId: t, pinned: e }
  );
}
async function T({ scope: e, archived: t, hostId: n, threadId: r }) {
  ...
  return (
    t
      ...
      : await h(`unarchive-conversation`, { hostId: n ?? D(e, i), conversationId: i }),
    { threadId: r, archived: t }
  );
}
async function E({ threadId: e, title: t }) {
  return (
    await h(`set-thread-title`, { conversationId: l(e), title: t }), { threadId: e, title: t }
  );
}
function D(e, t) {
  if (!e.get(ce, t)) throw Error(`set_thread_archived requires hostId for unloaded thread ${t}`);
  ...
  switch (e.type) {
    ...
    case `commandExecution`:
      return {
        ...
        exitCode: e.exitCode,
        durationMs: e.durationMs,
        ...(t && e.aggregatedOutput != null ? { output: N(e.aggregatedOutput, n) } : {}),
      };
    case `fileChange`:
      return {
        type: e.type,
...
function Se(e) {
  ...
  return { threadId: e.id, title: be(e), cwd: e.cwd, updatedAt: Number.isFinite(t) ? t : 0 };
}
function F(e, t, n) {
  if (n <= 0) return [];
  let r = t.trim();
...
async function De({ scope: e, limit: t, query: n }) {
  ...
  return { schemaVersion: 1, query: r.length > 0 ? r : null, threads: Oe(i, r, t).map(ke) };
}
function Oe(e, t, n) {
...
async function je({ model: e, prompt: t, sourceThreadId: n, threadId: r, thinking: i }) {
  return (
...
var B = `codex_app`,
  ...
  Y = `set_thread_title`,
  Re = i({ prompt: c().min(1), target: Le, model: c().min(1).optional(), thinking: H.optional() }),
  ...
  Ue = i({ threadId: c().min(1), hostId: c().min(1).optional(), archived: a() }),
  We = i({ threadId: c().min(1), title: c().min(1) });
function Ge({ deferLoading: e = !1 } = {}) {
  return [Qe, $e, et, tt, nt, rt, it].map((t) => (e ? { ...t, deferLoading: !0 } : t));
}
...
async function qe({ scope: e, argumentsValue: t }) {
  ...
  try {
    return Z(await De({ scope: e, limit: n.data.limit ?? Me, query: n.data.query }));
  } catch (e) {
    return u(e instanceof Error ? e.message : String(e));
  }
}
...
async function st({ featureOverrides: e, hostId: t, listExperimentalFeatures: n }) {
  let r = P(t),
    i = r ? await $(n) : !1,
    a = e?.[se] === !0,
    o = e?.[v] === !0,
  ...
  return [
    ...
    ...(r && o ? [Te, Ee] : []),
    ...(s ? p : []),
  ].map((e) => ({ ...e, namespace: Q, ...(ot.has(e.name) ? {} : { deferLoading: !0 }) }));
}
async function $(e, t = null) {
  try {
    let n = await e({ cursor: t, limit: at });
    return pe(n.data) ? !0 : n.nextCursor == null ? !1 : await $(e, n.nextCursor);
  } catch {
    return !1;
  }
}
