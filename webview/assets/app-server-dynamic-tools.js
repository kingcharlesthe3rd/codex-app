...
import { g as t, t as n } from "./app-scope.js";
import { f as r, m as i, n as a } from "./vscode-api.js";
import {
  Bi as o,
  G as s,
  Gi as c,
...
import { t as be } from "./setting-storage.js";
import { t as xe } from "./v4.js";
import { d as Se } from "./statsig-DoZ-0xit.js";
import { o as Ce } from "./thread-context-inputs.js";
import { a as we } from "./models-and-reasoning-efforts.js";
import { t as Te } from "./remote-projects.js";
import { t as P } from "./app-intl-signal.js";
import { t as Ee } from "./read-service-tier-for-request.js";
...
import { r as Me } from "./thread-handoff-composer-block-state.js";
import { t as Ne } from "./get-default-branch-name.js";
import { t as F } from "./worktree-query-keys-B7D-aYWQ.js";
import { t as Pe } from "./get-move-to-local-targets-B5nd-bax.js";
async function Fe({ pinned: e, threadId: t }) {
  return (
    await a(`set-thread-pinned`, { params: { threadId: t, pinned: e } }), { threadId: t, pinned: e }
  );
}
async function Ie({ scope: e, archived: t, hostId: n, threadId: r }) {
  let i = d(r);
  return (
    t
      ? await x(`archive-conversation`, { conversationId: i, source: `dynamic_tool` })
      : await x(`unarchive-conversation`, { hostId: n ?? Re(e, i), conversationId: i }),
    { threadId: r, archived: t }
  );
}
async function Le({ threadId: e, title: t }) {
  return (
    await x(`set-thread-title`, { conversationId: d(e), title: t }), { threadId: e, title: t }
  );
}
function Re(e, t) {
  if (!e.get(re, t)) throw Error(`set_thread_archived requires hostId for unloaded thread ${t}`);
  return e.get(w, t) ?? `local`;
}
async function ze({
  scope: e,
  cursor: t,
  ...
  let { thread: o } = await e.get(le).readThread(i, { includeTurns: !0 }),
...
function Ve(e, t, n) {
  return {
    ...
    items: e.items.map((e) => He(e, t, n)),
  };
}
function He(e, t, n) {
  switch (e.type) {
    case `userMessage`:
      return { type: e.type, id: e.id, content: e.content.map(Ue) };
    case `agentMessage`:
    ...
    case `reasoning`:
      return {
        ...
        id: e.id,
        summary: e.summary,
        ...(t ? { content: e.content.map((e) => I(e, n)) } : {}),
      };
    case `commandExecution`:
      return {
        ...
        exitCode: e.exitCode,
        durationMs: e.durationMs,
        ...(t && e.aggregatedOutput != null ? { output: I(e.aggregatedOutput, n) } : {}),
      };
    case `fileChange`:
      return {
        type: e.type,
...
function L({ sourceConversationId: e, targetConversationId: t, diffComments: n }) {
...
function Je(e, t, n) {
  ...
  return (i.sort(R), i.map(({ candidate: e }) => e));
}
function Ye(e, t) {
  return z(e.title, t, 0) ?? z(e.branch ?? ``, t, 1) ?? z(e.cwd, t, 2);
}
function Xe(e) {
  let t = 0;
  for (let n = 1; n < e.length; n += 1) R(e[t], e[n]) < 0 && (t = n);
  return t;
}
function R(e, t) {
...
async function Ze({ conversationId: e, sourceThreadRunning: t }) {
  t && (await x(`interrupt-conversation`, { conversationId: e }));
}
var B = [
...
async function V({ scope: e, sourceConversationId: t, targetConversationId: n }) {
  t !== n && (await _e(e, { sourceThreadId: t, targetThreadId: n }));
}
async function Qe({ sourceConversationId: e, targetConversationId: t }) {
  if (e === t) return;
...
async function $e({ sourceConversationId: e, targetConversationId: t, getTitle: n, setTitle: r }) {
  if (e === t) return;
  let i = n(e);
  i != null && (await r(t, i));
}
async function et({
  ...
  let S = s.getConversation(e),
    C = S?.rolloutPath ?? null;
  ...
  let w = null,
    T = null,
    E = !1;
  if (S?.forkedFromId != null)
    try {
      let e = await A(`git`).request({
        method: `resolve-worktree-for-thread`,
        params: {
          cwd: r,
          conversationId: S.forkedFromId,
          hostConfig: l,
          operationSource: `move_to_host_worktree`,
        },
      });
      ((w = e.worktreeGitRoot), (T = e.worktreeWorkspaceRoot), (E = e.hasUncommittedChanges));
    } catch {
      i.warning(
  ...
  w != null &&
    T != null &&
    (b?.(`reuse-existing-worktree`, `running`), b?.(`reuse-existing-worktree`, `done`));
  let D = await A(`git`).request({
    ...
    params: {
      ...
      destinationHostConfig: l,
      destinationWorkspaceRoot: r,
      destinationWorktreeGitRoot: w,
      destinationWorktreeWorkspaceRoot: T,
      stashDestinationWorktree: E,
    },
  });
  if (D.status === `error`)
    return {
  ...
  (p.invalidateQueries({ queryKey: F(j(l)) }), h?.());
  let O;
  try {
    O = await x(`fork-conversation-from-rollout-path`, {
      hostId: l.id,
      conversationId: e,
    ...
    try {
      await a(`worktree-set-owner-thread`, {
        params: { hostId: l.id, worktree: D.worktreeGitRoot, conversationId: e },
      });
    } catch {}
    return (
      i.warning(`Cross-host handoff moved git state, but conversation fork failed: {}`, {
        sensitive: { error: g(t) },
        safe: {},
      }),
      v?.(),
    ...
    await H(
      ...
      () =>
        a(`worktree-set-owner-thread`, {
          params: { hostId: l.id, worktree: D.worktreeGitRoot, conversationId: O },
        }),
    ),
...
async function H(e, t) {
  try {
    ...
    i.warning(e, { sensitive: { error: g(t) }, safe: {} });
  }
}
async function tt({
  scope: e,
  ...
  cwd: r,
  localGitRoot: o,
  localWorkspaceRoot: s,
  worktreeRoot: c,
  branchCheckedOutElsewhere: l,
  intl: u,
  appServerManager: d,
  transferBrowserState: f,
  setDiffComments: p,
  hostConfig: m,
  operationId: h,
  onSwitchingThreadStart: _,
  onSwitchingThreadDone: y,
  onSwitchingThreadFailed: b,
  onTargetConversationId: S,
}) {
  if (o == null || s == null || c == null || l)
    return {
      status: `error`,
  ...
  if (w.status === `error`) {
    ...
    return {
      status: `error`,
      message: `${nt({ branch: n, error: w.error, intl: u })}${e}`,
      execOutput: w.execOutput,
    };
  }
  (w.warnings.includes(`drop-source-stash-failed`) &&
    i.warning(`Move to local succeeded, but source stash cleanup failed`),
    _?.());
  let T;
  try {
    T = await x(`fork-conversation-from-latest`, {
      hostId: d.getHostId(),
      conversationId: t,
      cwd: C,
      workspaceRoots: [C],
    ...
    return (
      i.warning(`Move to local git operations succeeded, but conversation fork failed: {}`, {
        sensitive: { error: g(e) },
        safe: {},
      }),
      b?.(),
  ...
  try {
    await a(`worktree-set-owner-thread`, {
      params: { hostId: m.id, worktree: c, conversationId: T },
    });
  } catch (e) {
    i.warning(`Move to local succeeded, but worktree ownership metadata could not be updated: {}`, {
      sensitive: { error: g(e) },
      safe: {},
    });
  }
  try {
    ...
    i.warning(`Move to local succeeded, but browser state could not be transferred: {}`, {
      sensitive: { error: g(e) },
      safe: {},
    });
  }
  try {
    await $e({
      ...
      getTitle: (e) => d.getConversation(e)?.title ?? null,
      setTitle: (e, t) => x(`set-thread-title`, { conversationId: e, title: t }),
    });
  } catch (e) {
    i.warning(`Move to local succeeded, but thread title could not be transferred: {}`, {
      sensitive: { error: g(e) },
      safe: {},
    });
  }
  try {
    p((e) => L({ sourceConversationId: t, targetConversationId: T, diffComments: e }));
  } catch (e) {
    i.warning(`Move to local succeeded, but saved diff comments could not be transferred: {}`, {
      sensitive: { error: g(e) },
      safe: {},
    });
  }
  try {
    await Qe({ sourceConversationId: t, targetConversationId: T });
  } catch (e) {
    i.warning(`Move to local succeeded, but pinned-thread state could not be transferred: {}`, {
      sensitive: { error: g(e) },
      safe: {},
    });
  }
  try {
    await V({ scope: e, sourceConversationId: t, targetConversationId: T });
  } catch (e) {
    i.warning(`Move to local succeeded, but custom sidebar sections could not be transferred: {}`, {
      sensitive: { error: g(e) },
      safe: {},
    });
  }
  return (y?.(), { status: `success`, conversationId: T });
}
function nt({ branch: e, error: t, intl: n }) {
  switch (t) {
    case `invalid-params`:
...
async function rt({
  ...
  currentBranch: n,
  cwd: r,
  worktreeCheckoutBranch: o,
  selectedLocalCheckoutBranch: s,
  intl: c,
  appServerManager: l,
  defaultBranch: u,
  ...
  try {
    ...
    ((b = e.worktreeGitRoot), (S = e.worktreeWorkspaceRoot), (w = e.hasUncommittedChanges));
  } catch {
    i.warning(
      `Failed to resolve an existing worktree for thread. Falling back to creating a new worktree.`,
    );
  }
  if (b != null && S != null) ((T = `reuse-existing-worktree`), _?.(T, `running`), _?.(T, `done`));
  else {
    _?.(T, `running`);
    try {
      ...
      ((b = e.worktreeGitRoot),
        (S = e.worktreeWorkspaceRoot),
        (C = !0),
        (w = !1),
        _?.(T, `done`),
        m.invalidateQueries({ queryKey: F(j(p)) }));
    } catch (e) {
      return (
        _?.(T, `failed`),
        {
  ...
  let E = await a(`git-create-branch`, {
    source: `move_to_worktree_dialog`,
    params: { cwd: S, branch: y, hostId: p.id, mode: `worktree` },
  });
  if (E.status === `error`) {
    if (C)
      try {
        await a(`worktree-delete`, {
          params: { hostId: p.id, worktree: b, reason: `new-branch-cleanup` },
        });
      } catch (e) {
        i.warning(`Failed to clean up the newly created worktree: {}`, {
          sensitive: { error: g(e) },
          safe: {},
        });
      } finally {
  ...
  let D = await A(`git`).request({
    ...
    params: {
      ...
      sourceBranch: n,
      defaultBranch: u,
      localCheckoutBranch: s,
      worktreeCheckoutBranch: y,
      worktreeGitRoot: b,
      worktreeWorkspaceRoot: S,
      stashTargetWorktree: w,
      createdWorktree: C,
    },
  });
  if (D.status === `error`) {
    let e =
      D.rollbackErrors.length > 0
        ? c.formatMessage(
            {
            ...
            { issues: D.rollbackErrors.join(`, `) },
          )
        : ``;
    return {
      status: `error`,
      message:
        (D.execOutput?.output.length ?? 0) > 0
          ? `${c.formatMessage({ id: `localConversation.moveToWorktree.error.moveWithTerminalOutput`, defaultMessage: `Failed to move to worktree`, description: `Error shown when Move to worktree git operations fail with command output shown separately` })}${e}`
          : c.formatMessage(
              {
  ...
  _?.(`switching-thread`, `running`);
  let O;
  try {
    O = await x(`fork-conversation-from-latest`, {
      hostId: l.getHostId(),
      conversationId: t,
      cwd: S,
      workspaceRoots: [S],
    ...
    try {
      await a(`worktree-set-owner-thread`, {
        params: { hostId: p.id, worktree: b, conversationId: t },
      });
    } catch {}
    return (
      i.warning(`Move to worktree git operations succeeded, but conversation fork failed: {}`, {
        sensitive: { error: g(e) },
        safe: {},
      }),
      _?.(`switching-thread`, `failed`),
      { status: `success`, conversationId: t }
    );
  }
  v?.(O);
  try {
    await a(`worktree-set-owner-thread`, {
      params: { hostId: p.id, worktree: b, conversationId: O },
    });
  } catch (e) {
    i.warning(
  ...
  try {
    ...
    i.warning(`Move to worktree succeeded, but browser state could not be transferred: {}`, {
      sensitive: { error: g(e) },
      safe: {},
    });
  }
  try {
    ...
    i.warning(`Move to worktree succeeded, but thread title could not be transferred: {}`, {
      sensitive: { error: g(e) },
      safe: {},
    });
  }
  try {
    f((e) => L({ sourceConversationId: t, targetConversationId: O, diffComments: e }));
  } catch (e) {
    i.warning(`Move to worktree succeeded, but saved diff comments could not be transferred: {}`, {
      sensitive: { error: g(e) },
      safe: {},
    });
  }
  (D.warnings.includes(`stashed-target-worktree-changes`) &&
    i.warning(`Move to worktree stashed uncommitted changes in the reused worktree`),
    D.warnings.includes(`drop-source-stash-failed`) &&
      i.warning(`Move to worktree succeeded, but source stash cleanup failed`),
    D.warnings.includes(`drop-target-stash-failed`) &&
      i.warning(`Move to worktree succeeded, but target stash cleanup failed`));
  try {
    await Qe({ sourceConversationId: t, targetConversationId: O });
  } catch (e) {
    i.warning(`Move to worktree succeeded, but pinned-thread state could not be transferred: {}`, {
      sensitive: { error: g(e) },
      safe: {},
    });
  }
  try {
    await V({ scope: e, sourceConversationId: t, targetConversationId: O });
  } catch (e) {
    i.warning(
  ...
  return (_?.(`switching-thread`, `done`), { status: `success`, conversationId: O });
}
var it = `read_settings`,
  at = `write_settings`,
  ot = {
    name: it,
  ...
  ct = `The fork contains completed history only. If the source thread was running, the active turn and unfinished response are not in the child. Send a follow-up message to threadId only if the task requires work to continue there.`,
  lt = `The worktree is being created asynchronously, so no child thread id exists yet. Wait for pendingWorktreeId to finish, then send a follow-up message to the child only if the task requires work to continue there.`;
async function ut({
  ...
  switch (e.type) {
    case `same-directory`:
      return {
        environment: e,
        sourceThreadId: i,
        threadId: await x(`fork-conversation-from-latest`, {
          hostId: a,
          conversationId: i,
          cwd: o ?? void 0,
...
async function dt({ scope: e, limit: t, query: n }) {
  let r = n?.trim() ?? ``,
    i = await e.get(le).listAllThreads({ modelProviders: null });
  return { schemaVersion: 1, query: r.length > 0 ? r : null, threads: ft(i, r, t).map(pt) };
}
function ft(e, t, n) {
  if (t.length === 0) return e.slice(0, n);
  let r = new Map(e.map((e) => [e.id, e])),
    i = [];
  for (let a of Ke(e, t, n)) {
...
async function ht({ model: e, prompt: t, scope: n, sourceThreadId: r, threadId: i, thinking: a }) {
  return (
    await x(`send-follow-up-message`, {
      conversationId: d(i),
      prompt: r == null ? t : ue({ sourceThreadId: r, input: t }),
      ...(e == null ? {} : { model: e }),
      ...(a == null ? {} : { reasoningEffort: a }),
      serviceTier: await Ee(n, M, e ?? null),
    }),
    { threadId: i }
  );
}
var U = t(n, (e) => null),
  gt = e(de(), 1),
  _t = `codex_app`,
  W = `local_remote_dropdown`,
  vt = { id: M, display_name: `Local`, kind: `local` },
  yt = c({ threadId: l().min(1), destinationHostId: l().min(1).optional() }),
  G = `handoff_thread`;
function bt(e) {
  return (0, gt.default)([ve(e.get, `host_config`) ?? vt, ...e.get(Ce).map(y)], ({ id: e }) => e);
}
function xt(e = [vt], t = !1) {
  return {
    namespace: _t,
    name: G,
    description:
...
async function St({
  ...
  callId: r,
  callingThreadId: a,
  crossHostHandoffEnabled: o = Se(e, `2256010998`),
  queryClient: s,
}) {
  let c = yt.safeParse(n);
  if (!c.success) return N(`${G} received invalid arguments.`);
  let l = d(c.data.threadId);
  if (l === a) return N(`A thread cannot hand itself off. Choose another thread.`);
  try {
    let n = bt(e),
      a = await Ct({
        scope: e,
      ...
      u =
        c.data.destinationHostId == null
          ? a.hostConfig
          : n.find(({ id: e }) => e === c.data.destinationHostId);
    if (u == null)
      throw Error(`Host ${c.data.destinationHostId} is not available for thread handoff.`);
    if (u.id !== a.hostConfig.id && !o) throw Error(`Cross-host thread handoff is not enabled.`);
    await Ze({ conversationId: l, sourceThreadRunning: T(a.conversation) });
    let d = A(`git`).subscribe(`thread-handoff-progress`, (e) => {
      ...
      f = await (u.id === a.hostConfig.id ? wt(a) : Tt(a, u)).finally(d);
    return f.status === `error`
      ? N(f.message)
      : f.conversationId === l
        ? N(`Git handoff completed, but the destination thread could not be created.`)
        : (x(`archive-conversation`, {
            conversationId: l,
            cleanupWorktree: !1,
            source: `thread_handoff`,
          }).catch((e) => {
            i.warning(`Thread handoff succeeded, but source archive failed: {}`, {
              sensitive: { error: g(e) },
              safe: { threadId: l },
            });
          }),
            ...
            contentItems: [
              {
                type: `inputText`,
                text: JSON.stringify({
                  destinationHostDisplayName: u.display_name,
    ...
    return (
      i.warning(`Thread handoff dynamic tool failed: {}`, {
        sensitive: { error: g(e) },
        safe: { threadId: l },
      }),
      N(e instanceof Error ? e.message : `Thread handoff failed.`)
    );
  }
}
async function Ct({
  ...
  await x(`maybe-resume-conversation`, {
    hostId: o.getHostId(),
    conversationId: a,
    model: null,
    serviceTier: null,
  ...
  if (e.get(Me, a) > 0 || (fe(e.get, b.QUEUED_FOLLOW_UPS)?.[a]?.length ?? 0) > 0)
    throw Error(
  ...
  let u = v(c),
    d = await A(`git`).request({
      method: `stable-metadata`,
      params: { cwd: u, hostConfig: l, operationSource: W },
    });
  if (d == null) throw Error(`The source thread workspace is not a git repository.`);
  let { branch: f } = await A(`git`).request({
      method: `current-branch`,
      params: { root: d.root, hostConfig: l, operationSource: W },
    }),
    p = s.title?.trim() ?? ``;
  return {
    callId: n,
...
async function wt(e) {
  ...
  if (s(e.cwd, t)) {
    let n = await Et(e, t),
      r = K(e, e.currentBranch);
    return (
      e.scope.set(U, e.callId, {
      ...
      tt({
        ...
        transferBrowserState: q,
        setDiffComments: (t) => k(e.scope, `diff_comments`, t),
        hostConfig: e.hostConfig,
        operationId: e.callId,
        onSwitchingThreadStart: () => Y(e, `switching-thread`, `running`),
        onSwitchingThreadDone: () => Y(e, `switching-thread`, `done`),
        onSwitchingThreadFailed: () => Y(e, `switching-thread`, `failed`),
      })
    );
  }
  ...
  let { branch: r } = await A(`git`).request({
      method: `default-branch`,
      params: { root: e.gitRoot, hostConfig: e.hostConfig, operationSource: W },
    }),
    i = n === r ? K(e, null) : n,
    o = n === r ? null : await Dt(e, n, r);
  return (
    e.scope.set(U, e.callId, {
      destinationHostDisplayName: e.hostConfig.display_name,
    ...
    rt({
      ...
      operationId: e.callId,
      onStepStatus: (t, n) => Y(e, t, n),
    })
  );
}
async function Tt(e, t) {
  let n = K(e, e.currentBranch);
  e.scope.set(U, e.callId, {
  ...
  let { roots: r } = await a(`workspace-root-options`, { params: { hostId: t.id } }),
    { origins: i } = await a(`git-origins`, { params: { hostId: t.id, dirs: r }, source: W }),
    o = Te({
...
async function Et(e, t) {
  let [{ worktrees: n }, { worktrees: r }] = await Promise.all([
      A(`git`).request({
        ...
        params: { cwd: e.cwd, hostConfig: e.hostConfig, operationSource: W },
      }),
      A(`git`).request({
        method: `codex-worktrees`,
        params: { hostConfig: e.hostConfig, operationSource: W },
      }),
    ]),
    i = new Set(r.map(({ dir: e }) => ee(e))),
    a = Pe({
      cwd: e.cwd,
      sourceWorktreeRoot: v(e.gitRoot),
      repoWorktreeEntries: n.filter(({ root: e }) => !i.has(ee(e)) && !s(e, t)),
    })[0];
  if (a == null) throw Error(`No checkout was found for the source worktree.`);
  return a;
}
async function Dt(e, t, n) {
  let { branches: r } = await A(`git`).request({
...
function K(e, t) {
  ...
  let r = be(e.scope.get, p.branchPrefix),
    i = Ne({ branchPrefix: r, conversationTitle: e.conversation.title });
  return i.length > 0 && !i.endsWith(`/`)
    ? i
    : Ne({ branchPrefix: r, conversationTitle: e.threadId });
}
function q(e, t) {
...
function Ot(e, t) {
  return B.indexOf(e.id) - B.indexOf(t.id);
}
var X = `codex_app`,
  ...
  At = 1,
  jt = 2e3,
  Mt = m([`low`, `medium`, `high`, `xhigh`, `max`]),
  Nt = f(`type`, [
    h({ type: u(`working-tree`) }),
    h({ type: u(`branch`), branchName: l().min(1) }),
  ]),
  Pt = f(`type`, [
    h({ type: u(`local`) }),
    h({ type: u(`worktree`), startingState: Nt.optional() }),
  ]),
  Ft = f(`type`, [
    h({ type: u(`project`), projectId: l().min(1), environment: Pt }),
    h({ type: u(`projectless`), directoryName: l().min(1).optional() }),
  ]),
  It = f(`type`, [
    h({ type: u(`same-directory`) }),
    h({ type: u(`worktree`), startingState: Nt.optional() }),
  ]),
  Q = `fork_thread`,
  Lt = `create_thread`,
  Rt = `list_threads`,
  zt = `read_thread`,
  Bt = `send_message_to_thread`,
  Vt = `set_thread_pinned`,
  Ht = `set_thread_archived`,
  Ut = `set_thread_title`,
  Wt = c({ prompt: l().min(1), target: Ft, model: l().min(1).optional(), thinking: Mt.optional() }),
  Gt = c({ threadId: l().min(1).optional(), environment: It.optional() }),
  Kt = c({ query: l().optional(), limit: o().int().min(1).max(50).optional() }),
  qt = c({
    threadId: l().min(1),
    cursor: l().min(1).optional(),
    turnLimit: o().int().min(1).max(10).optional(),
    includeOutputs: _().optional(),
    maxOutputCharsPerItem: o().int().min(0).max(2e4).optional(),
  }),
  Jt = c({
    threadId: l().min(1),
    prompt: l().min(1),
    model: l().min(1).optional(),
    thinking: Mt.optional(),
  }),
  Yt = c({ threadId: l().min(1), pinned: _() }),
  Xt = c({ threadId: l().min(1), archived: _() }),
  Zt = c({ threadId: l().min(1), title: l().min(1) });
function Qt({
  ...
  let i = _n(t);
  return [ln, xt(e, n), gn(cn, i), un, dn, gn(fn, i), pn, mn, hn].map((e) =>
    r ? { ...e, deferLoading: !0 } : e,
  );
}
async function $t({ scope: e, argumentsValue: t, sourceThreadId: n }) {
  if (n == null) return N(`${Q} missing calling thread id.`);
  let r = Gt.safeParse(t);
  if (!r.success) return N(`${Q} ${Z}`);
  try {
    return $(
      await ut({
        scope: e,
    ...
    return N(e instanceof Error ? e.message : String(e));
  }
}
async function en({ argumentsValue: e, scope: t, sourceThreadId: n }) {
  let r = Wt.safeParse(e);
  if (!r.success) return N(`${Lt} ${Z}`);
  try {
    return $(
      await Oe({
        model: r.data.model,
...
async function tn({ scope: e, argumentsValue: t }) {
  let n = Kt.safeParse(t);
  if (!n.success) return N(`${Rt} ${Z}`);
  try {
    return $(await dt({ scope: e, limit: n.data.limit ?? kt, query: n.data.query }));
  } catch (e) {
    return N(e instanceof Error ? e.message : String(e));
  }
}
async function nn({ scope: e, argumentsValue: t }) {
  let n = qt.safeParse(t);
  if (!n.success) return N(`${zt} ${Z}`);
  try {
    return $(
      await ze({
        scope: e,
        ...
        maxOutputCharsPerItem: n.data.maxOutputCharsPerItem ?? jt,
        threadId: n.data.threadId,
        turnLimit: n.data.turnLimit ?? At,
      }),
    );
  } catch (e) {
    return N(e instanceof Error ? e.message : String(e));
...
async function rn({ argumentsValue: e, scope: t, sourceThreadId: n }) {
  let r = Jt.safeParse(e);
  if (!r.success) return N(`${Bt} ${Z}`);
  try {
    return $(
      await ht({
        model: r.data.model,
...
async function an({ argumentsValue: e }) {
  let t = Yt.safeParse(e);
  if (!t.success) return N(`${Vt} ${Z}`);
  try {
...
async function on({ scope: e, argumentsValue: t }) {
  let n = Xt.safeParse(t);
  if (!n.success) return N(`${Ht} ${Z}`);
  try {
    return $(await Ie({ scope: e, ...n.data }));
  } catch (e) {
    return N(e instanceof Error ? e.message : String(e));
  }
}
async function sn({ argumentsValue: e }) {
  let t = Zt.safeParse(e);
  if (!t.success) return N(`${Ut} ${Z}`);
  try {
    return $(await Le(t.data));
  } catch (e) {
    return N(e instanceof Error ? e.message : String(e));
  }
}
...
var vn = 100,
  ...
  bn = 1e3,
  xn = `codex_app`,
  Sn = new Set([ne.name, se.name, D, ie, oe, O]);
async function Cn({
  availableHandoffHosts: e,
  authMethod: t,
  crossHostHandoffEnabled: n = !1,
  ...
  let l = We(i),
    ...
    [p, m] = await Promise.all([
      l ? Dn(o) : !1,
      l && u
        ? wn({ authMethod: t, isAuthLoading: a, listModels: s, modelAvailabilityConfig: c })
        : [],
    ]);
  return [
    ...(l ? [he()] : []),
    ...
    ...(l && p ? [se] : []),
    ...(l && u
      ? Qt({ availableHandoffHosts: e, availableModelSlugs: m, crossHostHandoffEnabled: n })
      : []),
    ...(l && d ? [ot, st] : []),
    ...(f ? [...C, E] : []),
  ].map((e) => ({ ...e, namespace: xn, ...(Sn.has(e.name) ? {} : { deferLoading: !0 }) }));
}
async function wn({ authMethod: e, isAuthLoading: t, listModels: n, modelAvailabilityConfig: r }) {
  if (t) return [];
  try {
    let t = await Tn(n),
      { models: i } = we({
...
async function En(e) {
  let t = [],
    n = null;
  do {
    let r = await e({ cursor: n, includeHidden: !0, limit: yn });
    (t.push(...r.data), (n = r.nextCursor));
  } while (n != null);
  return t;
}
async function Dn(e, t = null) {
  try {
    let n = await e({ cursor: t, limit: vn });
    return Ae(n.data) ? !0 : n.nextCursor == null ? !1 : await Dn(e, n.nextCursor);
  } catch {
    return !1;
  }
}
