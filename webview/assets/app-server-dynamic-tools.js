...
import { f as se, m as O, n as k } from "./vscode-api.js";
import {
  Ar as A,
  Bt as j,
  Fi as ce,
...
import { t as _e } from "./setting-storage.js";
import { t as ve } from "./v4.js";
import { P as ye } from "./rpc-1.js";
import { d as be } from "./statsig.js";
import { o as xe } from "./thread-context-inputs.js";
import { a as Se } from "./models-and-reasoning-efforts.js";
import { t as Ce } from "./remote-projects.js";
import { t as R } from "./app-intl-signal-D.js";
import { t as we } from "./windows-tabs-open-schema.js";
import { t as Te } from "./read-service-tier-for-request.js";
import { t as Ee } from "./pending-worktree-store.js";
import { t as De } from "./threads-create-aB-XmjWR.js";
import { r as Oe } from "./thread-actions.js";
import { n as ke } from "./web-search-favicon-icon.js";
import { t as Ae } from "./score-query-match-CZO-nWKm.js";
import { r as je } from "./thread-handoff-composer-block-state.js";
import { t as Me } from "./get-default-branch-name.js";
import { t as Ne } from "./worktree-query-keys.js";
import { t as Pe } from "./run-app-action-in-primary-window.js";
import { t as Fe } from "./get-move-to-local-targets.js";
async function Ie({ pinned: e, threadId: t }) {
  return (
    await k(`set-thread-pinned`, { params: { threadId: t, pinned: e } }), { threadId: t, pinned: e }
  );
}
async function Le({ scope: e, archived: t, hostId: n, threadId: r }) {
  let i = A(r);
  return (
    t
      ? await w(`archive-conversation`, { conversationId: i, source: `dynamic_tool` })
      : await w(`unarchive-conversation`, { hostId: n ?? ze(e, i), conversationId: i }),
    { threadId: r, archived: t }
  );
}
async function Re({ threadId: e, title: t }) {
  return (
    await w(`set-thread-title`, { conversationId: A(e), title: t }), { threadId: e, title: t }
  );
}
function ze(e, t) {
  if (!e.get(p, t)) throw Error(`set_thread_archived requires hostId for unloaded thread ${t}`);
  return e.get(i, t) ?? `local`;
}
async function Be({
  scope: e,
  cursor: t,
  ...
  let { thread: o } = await e.get(y).readThread(i, { includeTurns: !0 }),
  ...
  switch (e.type) {
    ...
    case `reasoning`:
      return {
        ...
        id: e.id,
        summary: e.summary,
        ...(t ? { content: e.content.map((e) => Ge(e, n)) } : {}),
      };
    case `commandExecution`:
      return {
        ...
        exitCode: e.exitCode,
        durationMs: e.durationMs,
        ...(t && e.aggregatedOutput != null ? { output: Ge(e.aggregatedOutput, n) } : {}),
      };
    case `fileChange`:
      return {
        type: e.type,
...
function z({ sourceConversationId: e, targetConversationId: t, diffComments: n }) {
...
function Ye(e) {
  ...
  return { threadId: e.id, title: qe(e), cwd: e.cwd, updatedAt: Number.isFinite(t) ? t : 0 };
}
function Xe(e, t, n) {
  if (n <= 0) return [];
  let r = t.trim();
  ...
  return (i.sort(B), i.map(({ candidate: e }) => e));
}
function Ze(e, t) {
  return V(e.title, t, 0) ?? V(e.branch ?? ``, t, 1) ?? V(e.cwd, t, 2);
}
function Qe(e) {
  let t = 0;
  for (let n = 1; n < e.length; n += 1) B(e[t], e[n]) < 0 && (t = n);
  return t;
}
function B(e, t) {
...
async function $e({ conversationId: e, sourceThreadRunning: t }) {
  t && (await w(`interrupt-conversation`, { conversationId: e, initiatedBy: `system` }));
}
var H = [
...
async function U({ scope: e, sourceConversationId: t, targetConversationId: n }) {
  t !== n && (await E(e, { sourceThreadId: t, targetThreadId: n }));
}
async function W({ sourceConversationId: e, targetConversationId: t }) {
  if (e === t) return;
...
async function G({ sourceConversationId: e, targetConversationId: t, getTitle: n, setTitle: r }) {
  if (e === t) return;
  let i = n(e);
  i != null && (await r(t, i));
}
async function et({
  ...
  (d.invalidateQueries({ queryKey: Ne(ee(s)) }), m?.());
  let E;
  try {
    E = await w(`fork-conversation-from-rollout-path`, {
      hostId: s.id,
      conversationId: e,
    ...
    await K(`Cross-host handoff copied rollout cleanup failed: {}`, () =>
      D(`git`).request({
  ...
  return (
    ...
    await K(`Cross-host handoff succeeded, but browser state could not be transferred: {}`, () =>
      c(e, E),
    ),
    await K(`Cross-host handoff succeeded, but thread title could not be transferred: {}`, () =>
      G({
        sourceConversationId: e,
        targetConversationId: E,
        getTitle: (e) => a.getConversation(e)?.title ?? null,
        setTitle: (e, t) => w(`set-thread-title`, { conversationId: e, title: t }),
      }),
    ),
    await K(
      `Cross-host handoff succeeded, but saved diff comments could not be transferred: {}`,
      () => {
        l((t) => z({ sourceConversationId: e, targetConversationId: E, diffComments: t }));
      },
    ),
    await K(
      `Cross-host handoff succeeded, but pinned-thread state could not be transferred: {}`,
      () => W({ sourceConversationId: e, targetConversationId: E }),
    ),
    await K(
      `Cross-host handoff succeeded, but custom sidebar sections could not be transferred: {}`,
      () => U({ scope: u, sourceConversationId: e, targetConversationId: E }),
    ),
    h?.(),
    { status: `success`, conversationId: E }
  );
...
async function tt({
  ...
  if (y.status === `error`) {
    ...
    return {
      status: `error`,
      message: `${nt({ branch: n, error: y.error, intl: c })}${e}`,
  ...
  let b;
  try {
    b = await w(`fork-conversation-from-latest`, {
      hostId: l.getHostId(),
      conversationId: t,
      cwd: v,
      workspaceRoots: [v],
  ...
  try {
    await k(`worktree-set-owner-thread`, {
      params: { hostId: f.id, worktree: o, conversationId: b },
    });
  } catch (e) {
  ...
  try {
    d((e) => z({ sourceConversationId: t, targetConversationId: b, diffComments: e }));
  } catch (e) {
  ...
  try {
    await W({ sourceConversationId: t, targetConversationId: b });
  } catch (e) {
  ...
  try {
    await U({ scope: e, sourceConversationId: t, targetConversationId: b });
  } catch (e) {
    O.warning(`Move to local succeeded, but custom sidebar sections could not be transferred: {}`, {
      sensitive: { error: j(e) },
      safe: {},
    });
  }
  return (h?.(), { status: `success`, conversationId: b });
}
...
async function rt({
  ...
  try {
    ...
    ((_ = e.worktreeGitRoot), (v = e.worktreeWorkspaceRoot), (b = e.hasUncommittedChanges));
  } catch {
    O.warning(
      `Failed to resolve an existing worktree for thread. Falling back to creating a new worktree.`,
    );
  }
  if (_ != null && v != null) ((x = `reuse-existing-worktree`), m?.(x, `running`), m?.(x, `done`));
  else {
    m?.(x, `running`);
    try {
      ...
      ((_ = e.worktreeGitRoot),
        ...
        (b = !1),
        m?.(x, `done`),
        f.invalidateQueries({ queryKey: Ne(ee(d)) }));
    } catch (e) {
      return (
        m?.(x, `failed`),
        {
  ...
  let S = await k(`git-create-branch`, {
    ...
    params: { cwd: v, branch: g, hostId: d.id, mode: `worktree` },
  });
  if (S.status === `error`) {
    if (y)
      try {
        await k(`worktree-delete`, {
          params: { hostId: d.id, worktree: _, reason: `new-branch-cleanup` },
        });
      } catch (e) {
  ...
  let T;
  try {
    T = await w(`fork-conversation-from-latest`, {
      hostId: s.getHostId(),
      conversationId: t,
      cwd: v,
      workspaceRoots: [v],
    ...
    try {
      await k(`worktree-set-owner-thread`, {
        params: { hostId: d.id, worktree: _, conversationId: t },
      });
    } catch {}
    return (
  ...
  try {
    u((e) => z({ sourceConversationId: t, targetConversationId: T, diffComments: e }));
  } catch (e) {
  ...
  try {
    await W({ sourceConversationId: t, targetConversationId: T });
  } catch (e) {
  ...
  try {
    await U({ scope: e, sourceConversationId: t, targetConversationId: T });
  } catch (e) {
    O.warning(
...
var it = `open_in_codex`,
  at = {
    ...
    inputSchema: ue().parse(pe(we)),
  };
async function ot({ argumentsValue: e, hostId: t, threadId: n }) {
  let r = we.safeParse(e);
  if (!r.success) return l(`${it} received invalid arguments.`);
  try {
    let e = await Pe(
    ...
    return l(e instanceof Error ? e.message : `Failed to open Codex tab.`);
  }
}
var st = `read_settings`,
  ct = `write_settings`,
  lt = {
    name: st,
  ...
  dt = `The fork contains completed history only. If the source thread was running, the active turn and unfinished response are not in the child. Send a follow-up message to threadId only if the task requires work to continue there.`,
  ft = `The worktree is being created asynchronously, so no child thread id exists yet. Wait for pendingWorktreeId to finish, then send a follow-up message to the child only if the task requires work to continue there.`;
async function pt({
  ...
  switch (e.type) {
    case `same-directory`:
      return {
        environment: e,
        sourceThreadId: i,
        threadId: await w(`fork-conversation-from-latest`, {
          hostId: a,
          conversationId: i,
          cwd: o ?? void 0,
    ...
    case `worktree`: {
      ...
      return {
        ...
        pendingWorktreeId: Ee({
          hostId: a,
          label: n.formatMessage(Oe.forkPendingWorktreeTitle),
          sourceWorkspaceRoot: o,
          startingState: e.startingState ?? { type: `working-tree` },
          localEnvironmentConfigPath: null,
          launchMode: `fork-conversation`,
          prompt: n.formatMessage(Oe.forkPendingWorktreePrompt),
...
async function mt({ scope: e, limit: t, query: n }) {
  let r = n?.trim() ?? ``,
    i = await e.get(y).listAllThreads({ modelProviders: null });
  return { schemaVersion: 1, query: r.length > 0 ? r : null, threads: ht(i, r, t).map(gt) };
}
function ht(e, t, n) {
  if (t.length === 0) return e.slice(0, n);
  let r = new Map(e.map((e) => [e.id, e])),
    i = [];
  for (let a of Je(e, t, n)) {
...
async function vt({ model: e, prompt: t, scope: n, sourceThreadId: r, threadId: i, thinking: a }) {
  return (
    await w(`send-follow-up-message`, {
      conversationId: A(i),
      prompt: r == null ? t : b({ sourceThreadId: r, input: t }),
      ...(e == null ? {} : { model: e }),
      ...(a == null ? {} : { reasoningEffort: a }),
      serviceTier: await Te(n, S, e ?? null),
    }),
    { threadId: i }
  );
}
var q = t(n, (e) => null),
  yt = e(r(), 1),
  bt = `codex_app`,
  J = `local_remote_dropdown`,
  xt = { id: S, display_name: `Local`, kind: `local` },
  St = L({ threadId: F().min(1), destinationHostId: F().min(1).optional() }),
  Ct = `handoff_thread`;
function wt(e) {
  return (0, yt.default)([m(e.get, `host_config`) ?? xt, ...e.get(xe).map(me)], ({ id: e }) => e);
}
function Tt(e = [xt], t = !1) {
  return {
    namespace: bt,
    name: Ct,
    description:
...
async function Et({
  ...
  callId: r,
  callingThreadId: i,
  crossHostHandoffEnabled: a = be(e, `2256010998`),
  queryClient: o,
}) {
  let s = St.safeParse(n);
  if (!s.success) return l(`${Ct} received invalid arguments.`);
  let c = A(s.data.threadId);
  if (c === i) return l(`A thread cannot hand itself off. Choose another thread.`);
  try {
    let n = wt(e),
      i = await Dt({
        scope: e,
    ...
    if (u.id !== i.hostConfig.id && !a) throw Error(`Cross-host thread handoff is not enabled.`);
    await $e({ conversationId: c, sourceThreadRunning: d(i.conversation) });
    let f = D(`git`).subscribe(`thread-handoff-progress`, (e) => {
      ...
      p = await (u.id === i.hostConfig.id ? Ot(i) : kt(i, u)).finally(f);
    return p.status === `error`
      ? l(p.message)
      : p.conversationId === c
        ? l(`Git handoff completed, but the destination thread could not be created.`)
    ...
    return (
      ...
      l(e instanceof Error ? e.message : `Thread handoff failed.`)
    );
  }
}
async function Dt({
  ...
  await w(`maybe-resume-conversation`, {
    hostId: o.getHostId(),
    conversationId: a,
    model: null,
    serviceTier: null,
  ...
  if (e.get(je, a) > 0 || (re(e.get, he.QUEUED_FOLLOW_UPS)?.[a]?.length ?? 0) > 0)
    throw Error(
  ...
  let u = P(c),
    d = await D(`git`).request({
      method: `stable-metadata`,
      params: { cwd: u, hostConfig: l, operationSource: J },
    });
  if (d == null) throw Error(`The source thread workspace is not a git repository.`);
  let { branch: f } = await D(`git`).request({
      method: `current-branch`,
      params: { root: d.root, hostConfig: l, operationSource: J },
    }),
    p = s.title?.trim() ?? ``;
  return {
    callId: n,
...
async function Ot(e) {
  ...
  if (le(e.cwd, t)) {
    let n = await At(e, t),
      r = Mt(e, e.currentBranch);
    return (
      e.scope.set(q, e.callId, {
      ...
      tt({
        ...
        transferBrowserState: Nt,
        setDiffComments: (t) => o(e.scope, `diff_comments`, t),
  ...
  let { branch: r } = await D(`git`).request({
      ...
      params: { root: e.gitRoot, hostConfig: e.hostConfig, operationSource: J },
    }),
    i = n === r ? Mt(e, null) : n,
    a = n === r ? null : await jt(e, n, r);
  return (
    e.scope.set(q, e.callId, {
      destinationHostDisplayName: e.hostConfig.display_name,
...
async function kt(e, t) {
  ...
  let { roots: r } = await k(`workspace-root-options`, { params: { hostId: t.id } }),
    { origins: i } = await k(`git-origins`, { params: { hostId: t.id, dirs: r }, source: J }),
    a = Ce({
  ...
  if (a == null) throw Error(`No matching saved project was found on ${t.display_name}.`);
  return et({
    conversationId: e.threadId,
    sourceBranch: n,
    cwd: e.cwd,
    destinationWorkspaceRoot: P(a),
    intl: e.scope.get(R),
    appServerManager: e.manager,
...
async function At(e, t) {
  let [{ worktrees: n }, { worktrees: r }] = await Promise.all([
      D(`git`).request({
        ...
        params: { cwd: e.cwd, hostConfig: e.hostConfig, operationSource: J },
      }),
      D(`git`).request({
        method: `codex-worktrees`,
        params: { hostConfig: e.hostConfig, operationSource: J },
      }),
    ]),
    i = new Set(r.map(({ dir: e }) => ye(e))),
    a = Fe({
      cwd: e.cwd,
      sourceWorktreeRoot: P(e.gitRoot),
      repoWorktreeEntries: n.filter(({ root: e }) => !i.has(ye(e)) && !le(e, t)),
    })[0];
...
function Mt(e, t) {
  ...
  let r = _e(e.scope.get, ge.branchPrefix),
...
function Ft(e, t) {
  return H.indexOf(e.id) - H.indexOf(t.id);
}
var X = `codex_app`,
  ...
  Lt = 1,
  Rt = 2e3,
  zt = `Do not specify a model unless the user explicitly requests a specific model. Otherwise omit this field so the new thread uses the user's configured default model.`,
  Bt = ce([`low`, `medium`, `high`, `xhigh`, `max`]),
  Vt = M(`type`, [
    N({ type: I(`working-tree`) }),
    N({ type: I(`branch`), branchName: F().min(1) }),
  ]),
  Ht = M(`type`, [
    N({ type: I(`local`) }),
    N({ type: I(`worktree`), startingState: Vt.optional() }),
  ]),
  Ut = M(`type`, [
    N({ type: I(`project`), projectId: F().min(1), environment: Ht }),
    N({ type: I(`projectless`), directoryName: F().min(1).optional() }),
  ]),
  Wt = M(`type`, [
    N({ type: I(`same-directory`) }),
    N({ type: I(`worktree`), startingState: Vt.optional() }),
  ]),
  Q = `fork_thread`,
  Gt = `create_thread`,
  Kt = `list_threads`,
  qt = `read_thread`,
  Jt = `send_message_to_thread`,
  Yt = `set_thread_pinned`,
  Xt = `set_thread_archived`,
  Zt = `set_thread_title`,
  Qt = L({ prompt: F().min(1), target: Ut, model: F().min(1).optional(), thinking: Bt.optional() }),
  $t = L({ threadId: F().min(1).optional(), environment: Wt.optional() }),
  en = L({ query: F().optional(), limit: fe().int().min(1).max(50).optional() }),
  tn = L({
    threadId: F().min(1),
    cursor: F().min(1).optional(),
  ...
  nn = L({
    threadId: F().min(1),
    prompt: F().min(1),
    model: F().min(1).optional(),
    thinking: Bt.optional(),
  }),
  rn = L({ threadId: F().min(1), pinned: de() }),
  an = L({ threadId: F().min(1), archived: de() }),
  on = L({ threadId: F().min(1), title: F().min(1) });
function sn({
  ...
  return [_n, Tt(e, n), wn(gn, t), vn, yn, wn(bn, t), xn, Sn, Cn].map((e) =>
    r ? { ...e, deferLoading: !0 } : e,
  );
}
async function cn({ scope: e, argumentsValue: t, sourceThreadId: n }) {
  if (n == null) return l(`${Q} missing calling thread id.`);
  let r = $t.safeParse(t);
  if (!r.success) return l(`${Q} ${Z}`);
  try {
    return $(
      await pt({
        scope: e,
    ...
    return l(e instanceof Error ? e.message : String(e));
  }
}
async function ln({ argumentsValue: e, scope: t, sourceThreadId: n }) {
  let r = Qt.safeParse(e);
  if (!r.success) return l(`${Gt} ${Z}`);
  try {
    return $(
      await De({
        model: r.data.model,
...
async function un({ scope: e, argumentsValue: t }) {
  let n = en.safeParse(t);
  if (!n.success) return l(`${Kt} ${Z}`);
  try {
    return $(await mt({ scope: e, limit: n.data.limit ?? It, query: n.data.query }));
  } catch (e) {
    return l(e instanceof Error ? e.message : String(e));
  }
}
async function dn({ scope: e, argumentsValue: t }) {
  let n = tn.safeParse(t);
  if (!n.success) return l(`${qt} ${Z}`);
  try {
    return $(
      await Be({
        scope: e,
        ...
        maxOutputCharsPerItem: n.data.maxOutputCharsPerItem ?? Rt,
        threadId: n.data.threadId,
        turnLimit: n.data.turnLimit ?? Lt,
      }),
    );
  } catch (e) {
    return l(e instanceof Error ? e.message : String(e));
...
async function pn({ argumentsValue: e }) {
  let t = rn.safeParse(e);
  if (!t.success) return l(`${Yt} ${Z}`);
  try {
    return $(await Ie(t.data));
  } catch (e) {
    return l(e instanceof Error ? e.message : String(e));
  }
}
async function mn({ scope: e, argumentsValue: t }) {
  ...
  if (!n.success) return l(`${Xt} ${Z}`);
  try {
    return $(await Le({ scope: e, ...n.data }));
  } catch (e) {
    return l(e instanceof Error ? e.message : String(e));
  }
}
async function hn({ argumentsValue: e }) {
  let t = on.safeParse(e);
  if (!t.success) return l(`${Zt} ${Z}`);
  try {
    return $(await Re(t.data));
  } catch (e) {
    return l(e instanceof Error ? e.message : String(e));
  }
}
...
var gn = {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        model: { type: `string`, description: zt },
        thinking: {
          type: `string`,
...
var Tn = 100,
  ...
  Dn = 1e3,
  On = `codex_app`,
  kn = new Set([oe.name, ie.name, h, v, s, te]);
async function An({
  availableHandoffHosts: e,
  authMethod: t,
  crossHostHandoffEnabled: n = !1,
  ...
  listModels: l,
  modelAvailabilityConfig: d,
}) {
  let f = Ke(i),
    p = r?.[ne] === !0,
    m = r?.[T] === !0,
    h = r?.[c] === !0,
    [g, v] = await Promise.all([
      f ? Pn(s) : !1,
      f && p
        ? jn({ authMethod: t, isAuthLoading: o, listModels: l, modelAvailabilityConfig: d })
        : [],
    ]);
  return [
    ...(f ? [a()] : []),
    ...(r?.open_in_codex === !0 ? [at] : []),
    oe,
    ...(f && g ? [ie] : []),
    ...(f && p
      ? sn({ availableHandoffHosts: e, availableModelSlugs: v, crossHostHandoffEnabled: n })
      : []),
    ...(f && m ? [lt, ut] : []),
    ...(h ? [..._, u] : []),
  ].map((e) => ({ ...e, namespace: On, ...(kn.has(e.name) ? {} : { deferLoading: !0 }) }));
}
async function jn({ authMethod: e, isAuthLoading: t, listModels: n, modelAvailabilityConfig: r }) {
  if (t) return [];
  try {
    let t = await Mn(n),
      { models: i } = Se({
...
async function Nn(e) {
  let t = [],
    n = null;
  do {
    let r = await e({ cursor: n, includeHidden: !0, limit: En });
...
async function Pn(e, t = null) {
  try {
    let n = await e({ cursor: t, limit: Tn });
    return ke(n.data) ? !0 : n.nextCursor == null ? !1 : await Pn(e, n.nextCursor);
  } catch {
    return !1;
  }
}
