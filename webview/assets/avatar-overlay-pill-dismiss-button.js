...
import { n as de, t as _ } from "./permission-request-model.js";
function v({ axis: e, debounce: t, initialSize: n = !0, onChange: r, target: i }) {
...
function Se({
  ...
  return {
    ...
    sortAtMs: t(e)?.turnStartedAtMs ?? e.updatedAt,
    status: u,
    subtitle: f,
    title: i(e) ?? o.formatMessage(U.newThread),
    turnKey: String(te(e)),
    updatedAtMs: e.updatedAt,
    waitingRequest: he(p, o, {
      includeMcpElicitationCancelAction: a,
  ...
  let n = e.resumeState === `needs_resume` ? e.threadRuntimeStatus : null,
    r =
      e.resumeState === `needs_resume`
        ? n?.type === `active`
        : e.resumeState === `resuming` || t(e)?.status === `inProgress`,
    i =
      e.resumeState === `needs_resume`
        ? n?.type === `active` && n.activeFlags.includes(`waitingOnUserInput`)
        : e.requests.some((e) => e.method === `item/tool/requestUserInput`),
    a = se(e).some((e) => e.items.some((e) => e.type === `planImplementation` && !e.isCompleted)),
    o = e.resumeState === `needs_resume` ? n?.type === `systemError` : t(e)?.status === `failed`;
  return p(e) || i || a
    ? `waiting`
    : o
      ? `failed`
...
var G,
  ...
  Z = e(() => {
    ((G = m()),
      ...
      (J = 80),
      (Y = 224),
      (X = u(`avatar-overlay-mascot-width-px`, null)));
  });
function Pe(e) {
  let t = (0, Q.c)(4),
