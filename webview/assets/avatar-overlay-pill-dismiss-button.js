...
function U({ conversation: e, includeCompactWaitingRequests: t, intl: n }) {
  ...
  return {
    ...
    status: s,
    subtitle: G(e, n),
    title: c(e) ?? n.formatMessage(V.newThread),
    turnKey: String(l(e)),
  ...
  let t = e.resumeState === `needs_resume` ? e.threadRuntimeStatus : null,
    r =
      e.resumeState === `needs_resume`
        ? t?.type === `active`
        : e.resumeState === `resuming` || s(e)?.status === `inProgress`,
    i =
      e.resumeState === `needs_resume`
        ? t?.type === `active` && t.activeFlags.includes(`waitingOnUserInput`)
        : e.requests.some((e) => e.method === `item/tool/requestUserInput`),
    a = o(e).some((e) => e.items.some((e) => e.type === `planImplementation` && !e.isCompleted)),
    c = e.resumeState === `needs_resume` ? t?.type === `systemError` : s(e)?.status === `failed`;
  return n(e) || i || a
    ? `waiting`
    : c
      ? `failed`
