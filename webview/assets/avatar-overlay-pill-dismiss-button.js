...
import { _D as de, vD as fe } from "./app-initial~app-main~onboarding-page.js";
import { n as h, t as g } from "./permission-request-model.js";
function pe({ axis: e, debounce: t, initialSize: n = !0, onChange: r, target: i }) {
...
function he(e) {
  if (e == null) return v;
  if (e.kind === `first-awake`) return y;
  if (e.isLoading)
    return {
      ...
      fallbackBodyMessage: _.runningFallbackBody,
      iconClassName: `icon-xs shrink-0 text-token-text-secondary`,
      iconType: `spinner`,
      labelMessage: _.running,
      mascotState: `running`,
    };
  switch (e.level) {
    case `warning`:
  ...
  ge = e(() => {
    (n(),
      ...
      (v = {
        badgeBackgroundColor: `var(--color-token-activity-bar-badge-background)`,
        badgeForegroundColor: `var(--color-token-activity-bar-badge-foreground)`,
        fallbackBodyMessage: _.info,
        iconClassName: `icon-xs shrink-0 text-token-text-secondary`,
        iconType: `clock`,
        labelMessage: _.info,
        mascotState: `idle`,
      }),
      (y = { ...v, mascotState: `waving` }));
  });
function _e(
  e,
  t,
  ...
  switch (e?.type) {
    case `userInput`:
      return e.isOnboardingDynamicInput === !0 ? null : b(e, t);
    case `approval`:
...
function T(e, t, n) {
  ...
  switch (r.kind) {
    ...
    case `openaiForm`:
      ((s = t.formatMessage(z.answerTarget, { target: o })), (l = L(r.message)), (u = []));
      break;
    case `mcpToolCall`: {
    ...
    case `toolSuggestion`: {
      let e = L(r.suggestion.tool_name) ?? F(r.suggestion.tool_id) ?? t.formatMessage(z.tool);
      ((s =
    ...
    case `urlAction`:
      ((c = F(r.serverName) ?? L(r.serverName) ?? o),
        ...
        (u = [{ intent: `open`, label: t.formatMessage(z.openLink), tone: `primary` }]));
      break;
    case `generic`:
      ((s =
  ...
  r.riskLevel === `high` &&
    (u = [{ intent: `open`, label: t.formatMessage(z.review), tone: `primary` }]);
  let d = {
...
function Se({
  ...
  return {
    ...
    source: o,
    showInNotificationTray: !d || u === `waiting` || u === `failed` || f != null,
    sortAtMs: m(e)?.turnStartedAtMs ?? e.updatedAt,
    status: u,
    subtitle: f,
    title: s(e) ?? r.formatMessage(U.newThread),
    turnKey: String(re(e)),
...
function Ce(e, t) {
  ...
  return {
    ...
    status: Oe(e),
    subtitle: null,
    title: e.title?.trim() || t.formatMessage(U.newThread),
    turnKey: r,
    updatedAtMs: n,
    waitingRequest: null,
  };
      ...
      let e = f(n.text),
        t =
          e?.decision === `DONT_NOTIFY`
            ? null
            : H(e?.notificationMessage ?? e?.visibleText ?? n.text);
      if (t != null) return t;
    }
  }
...
function Te(e) {
  ...
  for (let e = t.length - 1; e >= 0; --e) {
    ...
    let r = f(n.text);
    if (r != null)
      return r.decision === `DONT_NOTIFY` ? null : H(r.notificationMessage ?? r.visibleText);
...
function Ee(e, t) {
  ...
  if (e.type === `commandExecution`) {
    ...
    if (n == null) return r ? t.formatMessage(U.runningCommand) : t.formatMessage(U.ranCommand);
    switch (n.type) {
      case `read`:
        return r
          ? t.formatMessage(U.readingFile, { fileName: n.name })
          : t.formatMessage(U.readFile, { fileName: n.name });
      case `listFiles`:
        return r ? t.formatMessage(U.listingFiles) : t.formatMessage(U.listedFiles);
      case `search`: {
        let e = H(n.query ?? ``);
        return e == null
          ? r
            ? t.formatMessage(U.searchingFiles)
            : t.formatMessage(U.searchedFiles)
          : r
            ? t.formatMessage(U.searchingQuery, { query: e })
            : t.formatMessage(U.searchedQuery, { query: e });
      }
      case `unknown`:
        return r ? t.formatMessage(U.runningCommand) : t.formatMessage(U.ranCommand);
    }
  }
  if (e.type === `fileChange`) {
    let n = e.changes.length;
    return e.status === `inProgress`
      ? t.formatMessage(U.editingFiles, { fileCount: n })
      : t.formatMessage(U.editedFiles, { fileCount: n });
  }
  if (e.type === `mcpToolCall`) {
    let n = e.status === `inProgress`,
      r = H(e.tool.replace(/[_-]+/g, ` `));
    return r == null
      ? n
        ? t.formatMessage(U.callingTool)
        : t.formatMessage(U.calledTool)
      : n
        ? t.formatMessage(U.callingToolName, { toolName: r })
        : t.formatMessage(U.calledToolName, { toolName: r });
  }
  if (e.type === `webSearch`) {
    let n = H(e.query);
    return n == null
      ? t.formatMessage(U.searchedWeb)
      : t.formatMessage(U.searchedQuery, { query: n });
  }
  return null;
}
function H(e) {
  ...
  let t = e.resumeState === `needs_resume` ? e.threadRuntimeStatus : null,
    n =
      e.resumeState === `needs_resume`
        ? t?.type === `active`
        : e.resumeState === `resuming` || m(e)?.status === `inProgress`,
    r =
      e.resumeState === `needs_resume`
        ? t?.type === `active` && t.activeFlags.includes(`waitingOnUserInput`)
        : e.requests.some((e) => e.method === `item/tool/requestUserInput`),
    a = ce(e).some((e) => e.items.some((e) => e.type === `planImplementation` && !e.isCompleted)),
    o = e.resumeState === `needs_resume` ? t?.type === `systemError` : m(e)?.status === `failed`;
  return i(e) || r || a
    ? `waiting`
    : o
      ? `failed`
...
function Ne(e) {
  if (e != null) return { [q]: `${W(e)}px` };
}
function W(e) {
  return Number.isFinite(e) ? Math.round(Math.min(Y, Math.max(J, e))) : 112;
}
var G,
  K,
  q,
  ...
  Z = e(() => {
    ((G = a()),
      ...
      ie(),
      (K = 192 / 208),
      (q = `--codex-avatar-width`),
      (J = 80),
      (Y = 224),
      (X = le(`avatar-overlay-mascot-width-px`, null)));
  });
function Pe(e) {
  let t = (0, Q.c)(4),
