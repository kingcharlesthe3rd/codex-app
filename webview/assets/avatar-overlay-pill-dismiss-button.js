...
import { r as u } from "./lib.js";
import { t as d } from "./button-DO-oxX3-.js";
import { t as f } from "./permission-request-model.js";
var p = u({
    running: {
  ...
  m = {
    ...
    fallbackBodyMessage: p.info,
    iconClassName: `icon-xs shrink-0 text-token-text-secondary`,
    iconType: `clock`,
    labelMessage: p.info,
    mascotState: `idle`,
  },
  h = { ...m, mascotState: `waving` };
function g(e) {
  if (e == null) return m;
  if (e.kind === `first-awake`) return h;
  if (e.isLoading)
    return {
      badgeBackgroundColor: `var(--color-token-activity-bar-badge-background)`,
      badgeForegroundColor: `var(--color-token-activity-bar-badge-foreground)`,
      fallbackBodyMessage: p.runningFallbackBody,
      iconClassName: `icon-xs shrink-0 text-token-text-secondary`,
      iconType: `spinner`,
      labelMessage: p.running,
      mascotState: `running`,
    };
  switch (e.level) {
    case `warning`:
...
function v(e, t) {
  switch (e?.type) {
    case `userInput`:
      return e.isOnboardingDynamicInput === !0 ? null : x(e, t);
    case `approval`:
      return e.item.type === `exec` ? S(e.item, t) : C(e.item, t);
...
function y(e, t) {
  switch (e.kind) {
    ...
    case `patch`:
      return [
        t.formatMessage(_.fileCount, { count: e.fileCount }),
...
function x(e, t) {
  ...
  return r == null || n.options.length === 0
    ...
    : {
        ...
        title: t.formatMessage(_.askQuestion),
        prompt: r,
        options: n.options.map((e) => ({
          label: z(e.label) ?? t.formatMessage(_.option),
...
function S(e, t) {
  ...
  return n == null
    ? {
        ...
        operation: t.formatMessage(_.runCommand),
        summary: z(e.cmd.join(` && `)) ?? t.formatMessage(_.command),
        title: t.formatMessage(_.reviewCommand),
        actions: [
          {
            commandDecision: `accept`,
            intent: `command-approval`,
            label: t.formatMessage(_.runOnce),
            tone: `primary`,
          },
          {
            commandDecision: `decline`,
            intent: `command-approval`,
            label: t.formatMessage(_.deny),
            tone: `danger`,
          },
        ],
      }
    : {
        kind: `network`,
        requestId: e.approvalRequestId,
        operation: t.formatMessage(_.allowNetwork),
        target: n,
        title: t.formatMessage(_.allowNetwork),
        actions: [
          {
            ariaLabel: t.formatMessage(_.allowOnce),
            commandDecision: `accept`,
            intent: `command-approval`,
            label: t.formatMessage(_.allow),
            tone: `primary`,
          },
          {
            commandDecision: `decline`,
...
function C(e, t) {
  ...
  return {
    kind: `patch`,
    requestId: e.approvalRequestId,
    operation: t.formatMessage(_.applyChanges),
    summary: n[0]?.[0] ?? t.formatMessage(_.oneFileChanged),
    title: t.formatMessage(_.applyChanges),
    files: n.map(([e]) => e),
    fileCount: n.length,
    ...
    actions: [
      {
        ...
        label: t.formatMessage(_.apply),
        tone: `primary`,
      },
      { intent: `open`, label: t.formatMessage(_.review), tone: `secondary` },
    ],
  };
}
function w(e, t) {
  ...
  let i, a;
  if (r.kind === `network`) ((a = t.formatMessage(_.networkAccess)), (i = a));
  else {
    a = t.formatMessage(_.fileAccess);
    let e = r.paths[0] ?? a,
    ...
    actions: [
      {
        ...
        ariaLabel: t.formatMessage(_.allowOnce),
        label: t.formatMessage(_.allow),
        permissionResponse: { permissions: D(n), scope: `turn` },
        tone: `primary`,
      },
      {
        intent: `permission-response`,
...
function T(e, t) {
  ...
  return r == null
    ...
    : {
        kind: `plan`,
        operation: t.formatMessage(_.plan),
        planContent: n,
        summary: r,
        title: t.formatMessage(_.plan),
        turnId: e.turnId,
        actions: [
          { intent: `plan-start`, label: t.formatMessage(_.implementPlan), tone: `primary` },
        ],
      };
}
function E(e, t) {
  let { elicitation: n } = e,
    r = e.request.params,
    i = z(r.serverName),
    a = I(i) ?? i ?? t.formatMessage(_.toolServer),
    o,
    s = a,
    c = null,
    l = null;
  switch (n.kind) {
    case `formElicitation`:
      ((o = t.formatMessage(_.answerTarget, { target: a })), (c = z(n.message)), (l = []));
      break;
    case `mcpToolCall`: {
      let e = z(n.approval.connector_name) ?? I(n.approval.connector_id) ?? a,
        r = O(n.message);
      ((o =
        r == null
          ? t.formatMessage(_.allowTarget, { target: e })
          : t.formatMessage(_.allowTarget, { target: r })),
        (s = e),
        (c = z(n.subtitle) ?? k(n.approval.tool_params) ?? z(n.message)));
      break;
    }
    case `toolSuggestion`: {
      let e = z(n.suggestion.tool_name) ?? I(n.suggestion.tool_id) ?? t.formatMessage(_.tool);
      ((o =
        n.suggestion.suggest_type === `enable`
          ? t.formatMessage(_.enableTool, { toolName: e })
          : t.formatMessage(_.installTool, { toolName: e })),
        (s =
          n.suggestion.tool_type === `connector`
            ? t.formatMessage(_.connector)
            : t.formatMessage(_.plugin)),
        (c = z(n.suggestion.suggest_reason)));
      break;
    }
    case `connectorAuth`: {
      let e = z(n.connector.connector_name) ?? I(n.connector.connector_id) ?? a,
        r = n.connector.auth_reason;
      ((o =
        r === `missing_link`
          ? t.formatMessage(_.connectTarget, { target: e })
          : r === `oauth_upgrade_required`
            ? t.formatMessage(_.additionalAccessTarget, { target: e })
            : t.formatMessage(_.reconnectTarget, { target: e })),
        (s = e),
        (c = z(n.subtitle) ?? z(n.message) ?? A(n.url)),
        (l = [
          {
            intent: `open`,
            label:
              r === `missing_link`
                ? t.formatMessage(_.connect)
                : r === `oauth_upgrade_required`
                  ? t.formatMessage(_.updateAccess)
                  : t.formatMessage(_.reconnect),
            tone: `primary`,
          },
        ]));
      break;
    ...
    case `urlAction`:
      ((s = I(n.serverName) ?? z(n.serverName) ?? a),
        (o = t.formatMessage(_.openLink)),
        (c = z(n.subtitle) ?? A(n.url) ?? z(n.message)),
        (l = [{ intent: `open`, label: t.formatMessage(_.openLink), tone: `primary` }]));
      break;
    case `generic`:
      ((o =
        M({ message: n.message, metadata: n.metadata }) ??
        t.formatMessage(_.allowTarget, { target: a })),
        (c = z(n.subtitle) ?? k(n.toolParams) ?? z(n.message)));
      break;
  }
  return (
    n.riskLevel === `high` &&
      (l = [{ intent: `open`, label: t.formatMessage(_.review), tone: `primary` }]),
    {
      kind: `tool`,
      requestId: e.requestId,
      operation: o,
      ...
      actions: l ?? [
        {
          ariaLabel: t.formatMessage(_.allowTarget, { target: s }),
          intent: `mcp-elicitation`,
          label: t.formatMessage(_.allowTarget, { target: s }),
...
function M({ message: e, metadata: t }) {
  let n = P(t),
    r = N(e, n);
  return n != null && F(r) ? n : z(r);
}
function N(e, t) {
  let n = e.trim();
  if (t == null) return n;
...
function H({ includeCompactWaitingRequests: e, intl: t, localConversations: n, remoteTasks: r }) {
  let i = [],
    a = new Set();
  for (let r of n) {
    let n = U({ conversation: r, includeCompactWaitingRequests: e, intl: t });
...
function U({ conversation: e, includeCompactWaitingRequests: t, intl: n }) {
  if (c(e)) return null;
  let i = e.hostId ?? `local`,
    o = i === `local` ? `local` : `remote-host`,
    l = Y(e),
    u = t && l === `waiting` ? a(e) : null;
  return {
    ...
    status: l,
    subtitle: G(e, n),
    title: s(e) ?? n.formatMessage(V.newThread),
    turnKey: String(r(e)),
    updatedAtMs: e.updatedAt,
    waitingRequest: v(u, n),
  };
}
function W(e, t) {
  ...
  return {
    ...
    status: X(e),
    subtitle: null,
    title: e.title?.trim() || t.formatMessage(V.newThread),
...
function q(e, t) {
  ...
  if (e.type === `commandExecution`) {
    ...
    if (n == null) return r ? t.formatMessage(V.runningCommand) : t.formatMessage(V.ranCommand);
    switch (n.type) {
      case `read`:
        return r
          ? t.formatMessage(V.readingFile, { fileName: n.name })
          : t.formatMessage(V.readFile, { fileName: n.name });
      case `listFiles`:
        return r ? t.formatMessage(V.listingFiles) : t.formatMessage(V.listedFiles);
      case `search`: {
        let e = J(n.query ?? ``);
        return e == null
          ? r
            ? t.formatMessage(V.searchingFiles)
            : t.formatMessage(V.searchedFiles)
          : r
            ? t.formatMessage(V.searchingQuery, { query: e })
            : t.formatMessage(V.searchedQuery, { query: e });
      }
      case `unknown`:
        return r ? t.formatMessage(V.runningCommand) : t.formatMessage(V.ranCommand);
    }
  }
  if (e.type === `fileChange`) {
    let n = e.changes.length;
    return e.status === `inProgress`
      ? t.formatMessage(V.editingFiles, { fileCount: n })
      : t.formatMessage(V.editedFiles, { fileCount: n });
  }
  if (e.type === `mcpToolCall`) {
    let n = e.status === `inProgress`,
      r = J(e.tool.replace(/[_-]+/g, ` `));
    return r == null
      ? n
        ? t.formatMessage(V.callingTool)
        : t.formatMessage(V.calledTool)
      : n
        ? t.formatMessage(V.callingToolName, { toolName: r })
        : t.formatMessage(V.calledToolName, { toolName: r });
  }
  if (e.type === `webSearch`) {
    let n = J(e.query);
    return n == null
      ? t.formatMessage(V.searchedWeb)
      : t.formatMessage(V.searchedQuery, { query: n });
  }
  return null;
}
function J(e) {
...
function Y(e) {
  let t = e.resumeState === `needs_resume` ? e.threadRuntimeStatus : null,
    ...
    o = i(e).some((e) => e.items.some((e) => e.type === `planImplementation` && !e.isCompleted)),
    s = e.resumeState === `needs_resume` ? t?.type === `systemError` : n(e)?.status === `failed`;
  return l(e) || a || o
    ? `waiting`
    : s
      ? `failed`
...
export { y as a, b as i, H as n, g as o, K as r, $ as t };
//# sourceMappingURL=avatar-overlay-pill-dismiss-button.js.map
