import { s as e } from "./chunk.js";
import { n as t } from "./jsx-runtime.js";
import { F as n, I as r, O as i, P as a, R as o, k as s } from "./app-scope.js";
import {
  A as c,
  C as l,
  D as u,
  E as d,
  O as f,
  S as p,
  T as m,
  b as h,
  w as g,
  x as _,
} from "./vscode-api.js";
function v(e, t) {
  let n = new Set(t);
  return e.filter((e) => !n.has(e));
}
function y(e, t, n) {
  let r = e.slice(0);
  return ((r[t] = n), r);
}
var b = class extends o {
    #e;
    #t;
    #n;
    #r;
    #i;
    #a;
    #o;
    #s;
    #c = [];
    constructor(e, t, n) {
      (super(),
        (this.#e = e),
        (this.#r = n),
        (this.#n = []),
        (this.#i = []),
        (this.#t = []),
        this.setQueries(t));
    }
    onSubscribe() {
      this.listeners.size === 1 &&
        this.#i.forEach((e) => {
          e.subscribe((t) => {
            this.#f(e, t);
          });
        });
    }
    onUnsubscribe() {
      this.listeners.size || this.destroy();
    }
    destroy() {
      ((this.listeners = new Set()),
        this.#i.forEach((e) => {
          e.destroy();
        }));
    }
    setQueries(e, t) {
      ((this.#n = e),
        (this.#r = t),
        s.batch(() => {
          let e = this.#i,
            t = this.#d(this.#n);
          ((this.#c = t), t.forEach((e) => e.observer.setOptions(e.defaultedQueryOptions)));
          let n = t.map((e) => e.observer),
            i = n.map((e) => e.getCurrentResult()),
            a = e.length !== n.length,
            o = n.some((t, n) => t !== e[n]),
            s = a || o,
            c = s
              ? !0
              : i.some((e, t) => {
                  let n = this.#t[t];
                  return !n || !r(e, n);
                });
          (!s && !c) ||
            (s && (this.#i = n),
            (this.#t = i),
            this.hasListeners() &&
              (s &&
                (v(e, n).forEach((e) => {
                  e.destroy();
                }),
                v(n, e).forEach((e) => {
                  e.subscribe((t) => {
                    this.#f(e, t);
                  });
                })),
              this.#p()));
        }));
    }
    getCurrentResult() {
      return this.#t;
    }
    getQueries() {
      return this.#i.map((e) => e.getCurrentQuery());
    }
    getObservers() {
      return this.#i;
    }
    getOptimisticResult(e, t) {
      let n = this.#d(e),
        r = n.map((e) => e.observer.getOptimisticResult(e.defaultedQueryOptions));
      return [r, (e) => this.#u(e ?? r, t), () => this.#l(r, n)];
    }
    #l(e, t) {
      return t.map((n, r) => {
        let i = e[r];
        return n.defaultedQueryOptions.notifyOnChangeProps
          ? i
          : n.observer.trackResult(i, (e) => {
              t.forEach((t) => {
                t.observer.trackProp(e);
              });
            });
      });
    }
    #u(e, t) {
      return t
        ? ((!this.#a || this.#t !== this.#s || t !== this.#o) &&
            ((this.#o = t), (this.#s = this.#t), (this.#a = n(this.#a, t(e)))),
          this.#a)
        : e;
    }
    #d(e) {
      let t = new Map(this.#i.map((e) => [e.options.queryHash, e])),
        n = [];
      return (
        e.forEach((e) => {
          let r = this.#e.defaultQueryOptions(e),
            a = t.get(r.queryHash);
          a
            ? n.push({ defaultedQueryOptions: r, observer: a })
            : n.push({ defaultedQueryOptions: r, observer: new i(this.#e, r) });
        }),
        n
      );
    }
    #f(e, t) {
      let n = this.#i.indexOf(e);
      n !== -1 && ((this.#t = y(this.#t, n, t)), this.#p());
    }
    #p() {
      if (this.hasListeners()) {
        let e = this.#a,
          t = this.#l(this.#t, this.#c);
        e !== this.#u(t, this.#r?.combine) &&
          s.batch(() => {
            this.listeners.forEach((e) => {
              e(this.#t);
            });
          });
      }
    }
  },
  x = e(t(), 1);
function S({ queries: e, ...t }, n) {
  let r = c(n),
    o = f(),
    v = u(),
    y = x.useMemo(
      () =>
        e.map((e) => {
          let t = r.defaultQueryOptions(e);
          return ((t._optimisticResults = o ? `isRestoring` : `optimistic`), t);
        }),
      [e, r, o],
    );
  (y.forEach((e) => {
    (h(e), g(e, v));
  }),
    d(v));
  let [S] = x.useState(() => new b(r, y, t)),
    [C, w, T] = S.getOptimisticResult(y, t.combine),
    E = !o && t.subscribed !== !1;
  (x.useSyncExternalStore(
    x.useCallback((e) => (E ? S.subscribe(s.batchCalls(e)) : a), [S, E]),
    () => S.getCurrentResult(),
    () => S.getCurrentResult(),
  ),
    x.useEffect(() => {
      S.setQueries(y, t);
    }, [y, t, S]));
  let D = C.some((e, t) => p(y[t], e))
    ? C.flatMap((e, t) => {
        let n = y[t];
        if (n) {
          let t = new i(r, n);
          if (p(n, e)) return _(n, t, v);
          l(e, o) && _(n, t, v);
        }
        return [];
      })
    : [];
  if (D.length > 0) throw Promise.all(D);
  let O = C.find((e, t) => {
    let n = y[t];
    return (
      n &&
      m({
        result: e,
        errorResetBoundary: v,
        throwOnError: n.throwOnError,
        query: r.getQueryCache().get(n.queryHash),
        suspense: n.suspense,
      })
    );
  });
  if (O?.error) throw O.error;
  return w(T());
}
export { S as t };
//# sourceMappingURL=useQueries.js.map
