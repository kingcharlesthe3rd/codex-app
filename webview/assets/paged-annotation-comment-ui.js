...
`,
  l = `#2563eb`,
  u = o(c.replace(`currentColor`, l)),
  d = 8,
  f = `top 0.2s cubic-bezier(0.165, 0.88, 0.44, 1), left 0.2s cubic-bezier(0.165, 0.88, 0.44, 1), width 0.2s cubic-bezier(0.165, 0.88, 0.44, 1), height 0.2s cubic-bezier(0.165, 0.88, 0.44, 1)`,
  p = `var(--color-text-accent, var(--color-accent-blue, ${l}))`,
  m = `color-mix(in srgb, var(--color-text-accent, var(--color-accent-blue, ${l})) 20%, transparent)`,
  h = l,
  g = `url("${u}") 13 12, crosshair`,
  _ = i;
function v(e) {
...
function b(e) {
  return { placement: `above`, point: x(e) };
}
function x(e) {
...
function w({ editorScale: e = 1, layer: t, markerPoint: n, pageSize: r }) {
  ...
  let a = (n.x / r.width) * i.width,
    ...
    d = a + 27,
    f = a - 27 - 294 * s;
  for (let e of [d, f]) if (e >= 16 && e <= c) return { x: e, y: O(u, s) };
  let p = k(a - (294 * s) / 2, 16, c),
    m = o + 27;
  return m <= l ? { x: p, y: O(m, s) } : { x: p, y: O(k(o - 27 - 120 * s, 16, l), s) };
}
function T({ editorScale: e = 1, layer: t, pageSize: n, rect: r }) {
  ...
  let a = (r.y / n.height) * i.height,
    ...
    p = a - d - 120 * c,
    m = f <= u ? f : k(p, 16, u);
  return { x: k(o - 294 * c, 16, l), y: O(m, c) };
}
function E(e, t) {
...
var A = e(t()),
  ...
  F = 294,
  I = 30,
  L = `var(--color-token-dropdown-background, var(--color-token-main-surface-primary, white))`,
  R = `var(--color-token-foreground, var(--color-token-text-primary, rgb(38, 38, 38)))`,
  z = `var(--color-token-border-default, var(--color-token-border-light, rgba(13, 13, 13, 0.08)))`,
  B = { color: `white`, fontFamily: `var(--font-sans)` },
  V = `-apple-system, BlinkMacSystemFont, "SF Pro Text", "SF Pro", "Segoe UI", sans-serif`,
  H = `pointer-events-none absolute z-50 cursor-auto paged-annotation-editor-enter`;
function U(e) {
  let t = (0, N.c)(20),
    { anchor: n, label: r, onClick: i, pageSize: a, rect: o, testId: s, zoomScale: c } = e,
    l = c === void 0 ? 1 : c,
    u;
  t[0] !== n || t[1] !== o ? ((u = n ?? b(o)), (t[0] = n), (t[1] = o), (t[2] = u)) : (u = t[2]);
  let d = u,
  ...
  let m = `center`,
    h = `-50%`;
  p === `start` ? ((m = `left`), (h = `0`)) : p === `end` && ((m = `right`), (h = `-100%`));
  let g = 1 / Math.max(l, 2 ** -52),
    _ = 6 * g,
    v = g === 1 ? `` : ` scale(${g})`,
    y = `${f}%`,
    x = `${(d.point.y / a.height) * 100}%`,
    S =
      d.placement === `above`
        ? `translate(${h}, calc(-100% - ${_}px))${v}`
        : `translate(${h}, ${_}px)${v}`,
    C = `${m} ${d.placement === `above` ? `bottom` : `top`}`,
    w;
  t[3] !== y || t[4] !== x || t[5] !== S || t[6] !== C
    ? ((w = { fontFamily: V, left: y, top: x, transform: S, transformOrigin: C }),
      (t[3] = y),
      (t[4] = x),
      (t[5] = S),
      (t[6] = C),
  ...
  let t = (0, N.c)(9),
    ...
    _ =
      g === 0
        ...
        : `calc(${(c.height / s.height) * 100}% + ${g * 2}px)`,
    v = h === 0 ? `${(c.x / s.width) * 100}%` : `calc(${(c.x / s.width) * 100}% - ${h}px)`,
    y = g === 0 ? `${(c.y / s.height) * 100}%` : `calc(${(c.y / s.height) * 100}% - ${g}px)`,
    b =
      h === 0
        ? `${(c.width / s.width) * 100}%`
        : `calc(${(c.width / s.width) * 100}% + ${h * 2}px)`,
    x;
  t[0] !== u || t[1] !== _ || t[2] !== v || t[3] !== y || t[4] !== b
    ? ((x = {
        backgroundColor: m,
        borderColor: p,
        borderStyle: `dashed`,
...
function q(e) {
  let t = (0, N.c)(22),
    ...
    _ = o === void 0 ? !1 : o,
    v = h === void 0 ? 1 : h,
    y = (_ ? 1.05 : 1) / Math.max(v, 2 ** -52),
    b;
  t[0] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((b = (0, j.jsx)(M, { className: `absolute inset-0 size-full` })), (t[0] = b))
    : (b = t[0]);
  let x;
  t[1] === s
    ? (x = t[2])
...
function Y(e) {
  ...
  t[1] !== r || t[2] !== i || t[3] !== a || t[4] !== s
    ? ((d = X({ layer: r, pageSize: i, point: a, previewWidth: s })),
      (t[1] = r),
      (t[2] = i),
      (t[3] = a),
      (t[4] = s),
...
function X({ layer: e, pageSize: t, point: n, previewWidth: r }) {
  if (e == null || t.width <= 0 || t.height <= 0) return null;
  let { height: i, width: a } = S(e),
    o = (n.x / t.width) * a,
    s = (n.y / t.height) * i,
    c = Math.min(F, Math.max(1, a - 32)),
    l = I / 2 + 12,
    u = (r === 0 ? c : Math.min(r, c)) / 2,
    d = Q(o, 16 + u, Math.max(16 + u, a - 16 - u)),
    f = s - l - P;
