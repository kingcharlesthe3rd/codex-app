...
import { li as t } from "./src-2.js";
import { n, t as r } from "./jsx-runtime.js";
import { t as i } from "./clsx.js";
import { lt as a } from "./vscode-api.js";
import { t as o } from "./data-url-from-svg-CpQT-mD6.js";
...
function f(e) {
        ...
        let a = new Map(),
          c = new IntersectionObserver(
            (n) => {
              ...
              let r = h({ container: e, pageElements: t, pageVisibilityByElement: a });
              r != null &&
                ((s.current != null && r !== s.current) ||
                  (r === s.current && ((s.current = null), f.current?.(), (f.current = null)),
                  o(r)));
            },
            { root: e, threshold: u },
          );
        for (let e of t) (a.set(e, 0), c.observe(e));
        let l = h({ container: e, pageElements: t, pageVisibilityByElement: a });
        return (
          l != null && o(l),
          () => {
            (c.disconnect(), (s.current = null), f.current?.(), (f.current = null));
          }
        );
      }),
      (v = [n, r, i]),
  ...
  t[5] !== n || t[6] !== r
    ? ((y = (e) => {
        ...
        let a = null,
          c = () => {
            (a != null && (clearTimeout(a), (a = null)), t.removeEventListener(`scroll`, u));
          },
          l = () => {
            if (s.current !== e) return;
            ((s.current = null), c());
            let n = h({ container: t, pageElements: p(t, r), pageVisibilityByElement: new Map() });
            n != null && o(n);
          },
          u = () => {
...
`,
  _ = `#2563eb`,
  v = o(g.replace(`currentColor`, _)),
  y = 8,
  b = `top 0.2s cubic-bezier(0.165, 0.88, 0.44, 1), left 0.2s cubic-bezier(0.165, 0.88, 0.44, 1), width 0.2s cubic-bezier(0.165, 0.88, 0.44, 1), height 0.2s cubic-bezier(0.165, 0.88, 0.44, 1)`,
  x = `var(--color-text-accent, var(--color-accent-blue, ${_}))`,
  S = `color-mix(in srgb, var(--color-text-accent, var(--color-accent-blue, ${_})) 20%, transparent)`,
  C = _,
  w = `url("${v}") 13 12, crosshair`,
  T = t;
function E(e) {
  return e.key.toLowerCase() === `i` && (e.metaKey || e.ctrlKey) && !e.altKey && !e.shiftKey;
}
function D(e) {
  return { x: e.x + e.width, y: e.y };
}
function ee(e) {
  return { placement: `above`, point: O(e) };
}
function O(e) {
...
function k(e) {
  ...
  return { height: e.offsetHeight || t.height, width: e.offsetWidth || t.width };
}
function A(e, t) {
  e != null && ((e.style.left = `${t.x}px`), (e.style.top = `${t.y}px`));
}
function j({ editorScale: e = 1, layer: t, markerPoint: n, pageSize: r }) {
  if (t == null || r.width <= 0 || r.height <= 0) return null;
  let i = k(t);
  if (i.width <= 0 || i.height <= 0) return null;
  let a = (n.x / r.width) * i.width,
    o = (n.y / r.height) * i.height,
    s = Math.max(e, 2 ** -52),
    c = N(i.width, s),
    l = P(i.height, s),
    u = I(o - (44 * s) / 2, 16, l),
    d = a + 27,
    f = a - 27 - 294 * s;
  for (let e of [d, f]) if (e >= 16 && e <= c) return { x: e, y: F(u, s) };
  let p = I(a - (294 * s) / 2, 16, c),
    m = o + 27;
  return m <= l ? { x: p, y: F(m, s) } : { x: p, y: F(I(o - 27 - 120 * s, 16, l), s) };
}
function M({ editorScale: e = 1, layer: t, pageSize: n, rect: r }) {
  ...
  let a = (r.y / n.height) * i.height,
    ...
    f = a - y - 120 * c,
    p = d <= u ? d : I(f, 16, u);
  return { x: I(o - 294 * c, 16, l), y: F(p, c) };
}
function N(e, t) {
  return Math.max(16, e - 294 * t - 16);
}
function P(e, t) {
  return Math.max(16, e - 120 * t - 16);
}
function F(e, t) {
  let n = (T - 120) * t;
  return e - Math.min(n, Math.max(e - 16, 0));
}
function I(e, t, n) {
...
var L = r(),
  ...
  B = 294,
  V = 30,
  H = `var(--color-token-dropdown-background, var(--color-token-main-surface-primary, white))`,
  U = `var(--color-token-foreground, var(--color-token-text-primary, rgb(38, 38, 38)))`,
  W = `var(--color-token-border-default, var(--color-token-border-light, rgba(13, 13, 13, 0.08)))`,
  G = { color: `white`, fontFamily: `var(--font-sans)` },
  K = `-apple-system, BlinkMacSystemFont, "SF Pro Text", "SF Pro", "Segoe UI", sans-serif`,
  q = `pointer-events-none absolute z-50 cursor-auto paged-annotation-editor-enter`;
function J(e) {
  let t = (0, c.c)(20),
    { anchor: n, label: r, onClick: i, pageSize: a, rect: o, testId: s, zoomScale: l } = e,
    u = l === void 0 ? 1 : l,
    d;
  t[0] !== n || t[1] !== o ? ((d = n ?? ee(o)), (t[0] = n), (t[1] = o), (t[2] = d)) : (d = t[2]);
  let f = d,
    p = (f.point.x / a.width) * 100,
    m = f.alignment;
  m ??= p < 20 ? `start` : p > 80 ? `end` : `center`;
  let h = `center`,
    g = `-50%`;
  m === `start` ? ((h = `left`), (g = `0`)) : m === `end` && ((h = `right`), (g = `-100%`));
  let _ = 1 / Math.max(u, 2 ** -52),
    v = 6 * _,
    y = _ === 1 ? `` : ` scale(${_})`,
    b = `${p}%`,
    x = `${(f.point.y / a.height) * 100}%`,
    S =
      f.placement === `above`
        ? `translate(${g}, calc(-100% - ${v}px))${y}`
        : `translate(${g}, ${v}px)${y}`,
    C = `${h} ${f.placement === `above` ? `bottom` : `top`}`,
    w;
  t[3] !== b || t[4] !== x || t[5] !== S || t[6] !== C
    ? ((w = { fontFamily: K, left: b, top: x, transform: S, transformOrigin: C }),
      (t[3] = b),
      (t[4] = x),
      (t[5] = S),
      (t[6] = C),
  ...
  let k;
  return (
    t[14] !== r || t[15] !== D || t[16] !== w || t[17] !== T || t[18] !== s
      ? ((k = (0, L.jsxs)(`button`, {
          type: `button`,
          "aria-label": r,
...
function X(e) {
  let t = (0, c.c)(9),
    ...
    d = r === void 0 ? (n !== void 0 && n ? 1 : 0) : r,
    f = i === void 0 ? 0 : i,
    p = a === void 0 ? f : a,
    m = o === void 0 ? f : o,
    h =
      m === 0
        ? `${(l.height / s.height) * 100}%`
        : `calc(${(l.height / s.height) * 100}% + ${m * 2}px)`,
    g = p === 0 ? `${(l.x / s.width) * 100}%` : `calc(${(l.x / s.width) * 100}% - ${p}px)`,
    _ = m === 0 ? `${(l.y / s.height) * 100}%` : `calc(${(l.y / s.height) * 100}% - ${m}px)`,
    v =
      p === 0
        ? `${(l.width / s.width) * 100}%`
        : `calc(${(l.width / s.width) * 100}% + ${p * 2}px)`,
    y;
  t[0] !== d || t[1] !== h || t[2] !== g || t[3] !== _ || t[4] !== v
    ? ((y = {
        backgroundColor: S,
        borderColor: x,
        borderStyle: `dashed`,
...
function Q(e) {
  ...
  t[0] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((_ = (0, L.jsx)(R, { className: `absolute inset-0 size-full` })), (t[0] = _))
    : (_ = t[0]);
  let v;
  t[1] === s
    ? (v = t[2])
  ...
  let y = v,
    ...
    C;
  t[3] !== b || t[4] !== S
    ? ((C = { color: x, height: 30, left: b, top: S, width: 30 }),
      (t[3] = b),
      (t[4] = S),
      (t[5] = C))
    : (C = t[5]);
...
function ne(e) {
  ...
  t[1] !== r || t[2] !== i || t[3] !== a || t[4] !== s
    ? ((p = re({ layer: r, pageSize: i, point: a, previewWidth: s })),
      (t[1] = r),
      (t[2] = i),
      (t[3] = a),
      (t[4] = s),
  ...
  t[6] === m
    ...
    : ((h = {
      ...
      (t[7] = h));
  let g;
  t[8] === n ? (g = t[9]) : ((g = ie(n)), (t[8] = n), (t[9] = g));
  let _;
  t[10] === g
    ? (_ = t[11])
    : ((_ = (0, L.jsx)(`div`, {
...
function re({ layer: e, pageSize: t, point: n, previewWidth: r }) {
  if (e == null || t.width <= 0 || t.height <= 0) return null;
  let { height: i, width: a } = k(e),
    o = (n.x / t.width) * a,
    s = (n.y / t.height) * i,
    c = Math.min(B, Math.max(1, a - 32)),
    l = V / 2 + 12,
    u = (r === 0 ? c : Math.min(r, c)) / 2,
    d = $(o, 16 + u, Math.max(16 + u, a - 16 - u)),
    f = s - l - z;
  if (f >= 16) return { left: d, maxWidth: c, top: f, transform: `translateX(-50%)` };
  let p = s + l;
  return p + z <= i - 16
