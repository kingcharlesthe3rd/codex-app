...
function _(e, t) {
  ...
  let O = [],
    ...
    A = ie({
      units: o({
        units: ne({ units: s(k), isActivitySliceClosed: a != null, conversationDetailLevel: u }),
      }),
  ...
  (b != null && F(O, v(b, r)), S != null && F(O, v(S, r)), C != null && F(O, v(C, r)));
...
function C(e) {
  let n = Object.entries(e.changes).flatMap(([e, n]) => {
    let r = t(e, n);
    if (r == null || r.trim().length === 0) return [];
    let i = Ae(r);
    return [I(`${ke(n.type)} ${J(e)} +${i.additions} -${i.deletions}`, Y(`diff`, r))];
  });
  return n.length === 0
    ? null
    : n.join(`
...
function le(e, n) {
  let r = e.grantRoot == null ? n : W(e.grantRoot);
  return Object.entries(e.changes).flatMap(([e, n]) => {
    let i = t(e, n);
    if (i == null || i.trim().length === 0) return [];
    let a = Ae(i);
    return [`${ue(n.type)} ${be(e, r)} (+${a.additions} -${a.deletions})`];
  });
}
function ue(e) {
  switch (e) {
...
function H(e) {
  let t = h.exec(e);
  return t?.[1] == null || t[2] == null || !i(t[1])
    ? { lineSuffix: ``, path: e }
...
function U(e, t, r) {
  ...
  let i = n(e),
    a = n(t);
  return i === a ? r : i.startsWith(`${a}/`) ? `${r}/${e.slice(t.length + 1)}` : null;
}
function W(e) {
  if (e == null) return d;
  let t = Se(e);
