import "./jsx-runtime.js";
import "./app-scope-CWE-zIhQ.js";
import { g as e } from "./use-host-config-C.js";
import { Ir as t, Nr as n, ur as r } from "./thread-context-inputs.js";
import "./vscode-api.js";
import "./isEqual.js";
import { _n as i } from "./src-1.js";
import "./lib-1.js";
import "./proxy.js";
import "./setting-storage.js";
import "./reduced-motion-preference.js";
import "./use-reduced-motion-1.js";
import "./spinner.js";
import "./chevron-right.js";
import "./use-stable-callback.js";
import "./toast-signal.js";
import { F as a, I as o, M as s } from "./rpc-1.js";
import "./statsig.js";
import "./microphone-input.js";
import "./request.js";
import "./chevron.js";
import "./markdown-to-search-text.js";
import "./marked.esm.js";
import "./product-logger.js";
import "./parse-directives.js";
import "./platform.js";
import "./persisted-signal.js";
import "./app-server-manager-hooks.js";
import "./invalidate-queries-and-broadcast.js";
import "./config-queries.js";
import "./selectable-remote-connections-signal.js";
import "./remote-connection-visibility.js";
import "./sidebar-signals.js";
import "./use-auth.js";
import "./use-global-state.js";
import "./use-codex-home.js";
import "./use-os-info.js";
import "./remote-projects.js";
import "./_baseEach.js";
import "./thread-detail-level.js";
import "./permissions-mode-defaults.js";
import "./projectless-thread.js";
import "./use-platform.js";
import "./experimental-features-queries-D.js";
import "./use-is-plugins-enabled-B.js";
import "./browser-sidebar-availability.js";
import "./use-plugins.js";
import "./apps-queries.js";
import "./uniq.js";
import "./parse-diff.js";
import "./sumBy.js";
import "./git-availability-query.js";
import "./app-intl-signal.js";
import "./sidebar-thread-keys.js";
import "./windows-tabs-open-schema.js";
import "./settings.cog.js";
import "./codex.js";
import "./local-projects.js";
import "./sidebar-project-group-signals.js";
import "./sidebar-project-groups.js";
import "./_defineProperty.js";
import "./build-worktree-label-from-input.js";
import "./read-service-tier-for-request.js";
import "./realtime-controller.js";
import "./realtime-conversation-config.js";
import "./check-plugin-availability-C.js";
import "./plugin-detail-queries.js";
import "./plugins-page-selectors.js";
import "./pending-worktree-store.js";
import "./threads-create.js";
import "./app-server-dynamic-tools-Gzn-GJ0M.js";
import "./pinned-threads-query.js";
import "./use-resize-observer.js";
import "./check-circle.js";
import "./x-circle.js";
import "./thinking-shimmer.js";
import "./unselected-circle.js";
import "./chat.js";
import "./globe.js";
import "./set-pinned-thread.js";
import "./thread-actions.js";
import "./web-search-favicon-icon.js";
import "./is-hotkey-window-context.js";
import "./comment.js";
import "./thread-resource-card.js";
import "./thread-handoff-composer-block-state-C7-ef4-4.js";
import "./automation-host-support.js";
import "./mcp-tool-item-content-utils.js";
import "./run-app-action-in-primary-window.js";
import "./get-move-to-local-targets.js";
import "./progress-step-row.js";
import "./thread-handoff-step-row.js";
import "./patch-diff-stats.js";
import {
  c as ee,
  d as te,
  i as c,
  l as ne,
  n as re,
  r as ie,
  t as ae,
  u as oe,
} from "./split-items-into-render-groups.js";
var l = /::git-[a-z-]+\{[^}\n]*\}/g,
  u = `STEPS_PROSE`,
  d = { cwd: null, homeDir: null },
  f = /\]\((<[^>\n]+>|[^)\s\n]+)([ \t]+(?:"[^"\n]*"|'[^'\n]*'|\([^)\n]*\)))?\)/g,
  p = /^\/(?:Users|home)\/[^/]+(?=\/|$)/,
  se = /^[A-Za-z]:\/Users\/[^/]+(?=\/|$)/,
  ce = /^(.*?)(:\d+(?:-\d+)?)$/;
function le({ cwd: e = null, projectlessOutputDirectory: t = null, title: n, turns: r }) {
  let i = [],
    a = W(e);
  i.push(`# ${ve(n ?? `Codex conversation`)}`);
  for (let n of r) {
    let r = m(n, e, a, t);
    r != null && i.push(r);
  }
  return `${i
    .join(`

`)
    .trimEnd()}\n`;
}
function m(e, n, a, o) {
  let s = [],
    c = e.cwd == null ? a : W(e.cwd),
    l =
      o == null
        ? void 0
        : (t) =>
            r({
              cwd: e.cwd ?? (n == null ? null : i(n)),
              projectlessOutputDirectory: o,
              resourcePath: t,
            }),
    {
      userItems: d,
      assistantItem: f,
      agentItems: p,
      automationUpdateItems: se,
      toolOutputItems: ce,
      postAssistantItems: le,
      systemEventItem: m,
      remoteTaskCreatedItems: g,
      personalityChangedItems: _,
      forkedFromConversationItems: y,
      modelChangedItems: b,
      modelReroutedItems: x,
      todoListItem: S,
      proposedPlanItem: C,
      planImplementationItem: w,
      mcpServerElicitationItems: T,
      permissionRequestItems: E,
      approvalItem: D,
      userInputItem: O,
    } = ae(
      l == null
        ? e.items
        : e.items.map((e) =>
            e.type === `assistant-message` ? { ...e, content: t(e.content, l) } : e,
          ),
      e.status,
    );
  for (let e of b) F(s, h(e, c));
  for (let e of d) F(s, ue(e, c));
  for (let e of x) F(s, h(e, c));
  let k = [],
    { renderableAgentItems: A } = re({
      agentItems: p,
      isTurnInProgress: e.status === `in_progress`,
      isAnyNonAgentItemInProgress: ie(f) || ie(C),
    }),
    j = oe({
      units: ne({
        units: ee({ units: te(A), isActivitySliceClosed: f != null, conversationDetailLevel: u }),
      }),
      isActivitySliceClosed: f != null,
    });
  for (let e of j) F(k, de(e, c));
  for (let e of se) F(k, h(e, c));
  m != null && F(k, h(m, c));
  for (let e of ce) F(k, h(e, c));
  for (let e of le) F(k, h(e, c));
  (S != null && F(k, h(S, c)), C != null && F(k, h(C, c)), w != null && F(k, h(w, c)));
  for (let e of T) F(k, h(e, c));
  for (let e of E) F(k, h(e, c));
  (D != null && F(k, h(D, c)),
    O != null && F(k, h(O, c)),
    k.length > 0 &&
      s.push(
        ye(
          Oe(k.length),
          k.join(`

`),
        ),
      ),
    f != null && F(s, v(f, { pathContext: c })));
  for (let e of g) F(s, h(e, c));
  for (let e of _) F(s, h(e, c));
  for (let e of y) F(s, h(e, c));
  return s.length === 0
    ? null
    : s.join(`

`);
}
function h(e, t) {
  switch (e.type) {
    case `user-message`:
      return g(e, { escapeDetailsTags: !0, pathContext: t });
    case `assistant-message`:
      return v(e, { escapeDetailsTags: !0, pathContext: t });
    case `reasoning`:
      return null;
    case `proposed-plan`:
      return N(`Plan`, e.content);
    case `todo-list`:
      return S(e);
    case `exec`:
      return y(e);
    case `patch`:
      return b(e);
    case `turn-diff`:
      return x(e);
    case `web-search`:
      return `Searched the web for ${q(e.query)}`;
    case `generated-image`:
      return D(e.src, e.status);
    case `userInput`:
      return P(
        e.completed ? `User input request` : `User input requested`,
        e.questions.map((e) => `- ${e.question}`),
      );
    case `user-input-response`:
      return P(
        `User input response`,
        e.questionsAndAnswers.flatMap((e) => [
          `- ${e.question}`,
          ...e.answers.map((e) => `  - ${e}`),
        ]),
      );
    case `mcp-server-elicitation`:
      return P(`MCP server elicitation`, [
        e.completed ? `Status: completed` : `Status: pending`,
        `Action: ${e.action ?? `none`}`,
      ]);
    case `permission-request`:
      return P(`Permission request`, [
        e.completed ? `Status: completed` : `Status: pending`,
        `Reason: ${e.reason ?? `Not provided`}`,
        `Response: ${e.response == null ? `none` : `granted`}`,
      ]);
    case `mcp-tool-call`:
      return C(e);
    case `automation-update`:
      return P(`Automation update`, [
        `Mode: ${e.result?.mode ?? `pending`}`,
        `Automation ID: ${e.result?.automationId ?? `pending`}`,
      ]);
    case `dynamic-tool-call`:
      return P(`Tool call`, [
        `Tool: ${e.tool}`,
        e.completed ? `Status: completed` : `Status: running`,
      ]);
    case `automatic-approval-review`:
      return T(e);
    case `multi-agent-action`:
      return P(`Subagent action`, [
        `Action: ${e.action}`,
        `Status: ${e.status}`,
        `Receiver threads: ${e.receiverThreads.length}`,
        e.prompt == null ? null : `Prompt: ${e.prompt}`,
      ]);
    case `subagent-activity`:
      return null;
    case `plan-implementation`:
      return P(`Plan implementation`, [
        e.isCompleted ? `Status: completed` : `Status: running`,
        e.planContent,
      ]);
    case `remote-task-created`:
      return P(`Remote task created`, [`Task ID: ${e.taskId}`]);
    case `context-compaction`:
      return P(`Context compaction`, [
        `Source: ${e.source}`,
        e.completed ? `Status: completed` : `Status: running`,
      ]);
    case `personality-changed`:
      return P(`Personality changed`, [`Personality: ${e.personality}`]);
    case `forked-from-conversation`:
      return P(`Forked conversation`, [`Source conversation: ${e.sourceConversationId}`]);
    case `model-changed`:
      return P(`Model changed`, [`${e.fromModel} -> ${e.toModel}`]);
    case `model-rerouted`:
      return P(`Model rerouted`, [`${e.fromModel} -> ${e.toModel}`, `Reason: ${e.reason}`]);
    case `auto-review-interruption-warning`:
      return null;
    case `system-error`:
      return N(`System error`, e.content);
    case `stream-error`:
      return P(`Stream error`, [e.content, e.additionalDetails]);
    case `steered`:
    case `worked-for`:
      return null;
  }
}
function ue(e, t) {
  let n = g(e, { pathContext: t });
  return n == null ? null : L(n);
}
function g(e, { escapeDetailsTags: t = !1, pathContext: n = d } = {}) {
  let r = [],
    i = z(e.message, n).trim();
  i.length > 0 && r.push(i);
  let a = _(e, n);
  if ((a.length > 0 && r.push(P(`User context`, a)), r.length === 0)) return null;
  let o = r.join(`

`);
  return t ? K(o) : o;
}
function _(e, t) {
  let n = [];
  if (e.attachments.length > 0) {
    n.push(`Attachments:`);
    for (let r of e.attachments) n.push(`- ${r.label}: ${B(r.path, t)}`);
  }
  if (e.images.length > 0) {
    n.push(`Images:`);
    for (let r of e.images) n.push(`- ${B(r, t)}`);
  }
  if (e.comments != null && e.comments.length > 0) {
    n.push(`Comments:`);
    for (let r of e.comments) {
      let e = r.lineRange == null ? `` : ` ${q(r.lineRange)}`;
      n.push(
        `- ${V(r.path, t)}${e}: ${R(r.body).replaceAll(
          `
`,
          ` `,
        )}`,
      );
    }
  }
  return (
    e.referencesPriorConversation && n.push(`Referenced prior conversation`),
    e.reviewMode && n.push(`Mode: code review`),
    e.pullRequestFixMode && n.push(`Mode: pull request fix`),
    e.autoResolveSync && n.push(`Mode: auto resolve merge`),
    e.pullRequestCheckNames != null &&
      e.pullRequestCheckNames.length > 0 &&
      n.push(`Pull request checks: ${e.pullRequestCheckNames.join(`, `)}`),
    e.pullRequestMergeConflictNumber !== void 0 &&
      n.push(
        e.pullRequestMergeConflictNumber == null
          ? `Pull request merge conflict`
          : `Pull request merge conflict: #${e.pullRequestMergeConflictNumber}`,
      ),
    n
  );
}
function v(e, { escapeDetailsTags: t = !1, pathContext: n = d } = {}) {
  let r = z(e.content, n).trim();
  return r.length === 0 ? null : t ? K(r) : r;
}
function y(t) {
  let n = [],
    r = e(t.cmd);
  n.push(Y(`bash`, `$ ${r}`));
  let i = t.output?.aggregatedOutput;
  if (i != null) {
    let e = R(i);
    e.trim().length > 0 && n.push(Y(`text`, e));
  }
  return (
    n.push(Fe(t)),
    F(n, E(t.automaticApprovalReviews)),
    I(
      `Ran ${J(r)}`,
      n.join(`

`),
    )
  );
}
function b(e) {
  let t = Object.entries(e.changes).flatMap(([e, t]) => {
    let r = n(e, t);
    if (r == null || r.trim().length === 0) return [];
    let i = Pe(r);
    return [I(`${Ne(t.type)} ${J(e)} +${i.additions} -${i.deletions}`, Y(`diff`, r))];
  });
  return (
    F(t, E(e.automaticApprovalReviews)),
    t.length === 0
      ? null
      : t.join(`
`)
  );
}
function x(e) {
  return I(`Diff`, Y(`diff`, e.unifiedDiff));
}
function S(e) {
  let t = [];
  return (
    e.explanation != null && e.explanation.trim().length > 0 && t.push(G(e.explanation).trim()),
    t.push(
      e.plan.map((e) => `- [${_e(e.status)}] ${G(e.step)}`).join(`
`),
    ),
    I(
      `Plan`,
      t.join(`

`),
    )
  );
}
function C(e) {
  let t = [];
  if (
    (t.push(`MCP tool call\n\n${G(`${e.invocation.server}.${e.invocation.tool}`)}`),
    t.push(Y(`json`, Z(e.invocation.arguments))),
    F(t, E(e.automaticApprovalReviews)),
    e.result == null)
  )
    return (
      t.push(e.completed ? `Result: none` : `Status: running`),
      t.join(`

`)
    );
  if (e.result.type === `error`)
    return (
      t.push(G(`Error: ${e.result.error}`)),
      t.join(`

`)
    );
  let n = e.result.content.map(w).filter((e) => e.length > 0).join(`

`);
  return (
    n.length > 0 && t.push(n),
    e.result.structuredContent != null && t.push(Y(`json`, Z(e.result.structuredContent))),
    t.join(`

`)
  );
}
function w(e) {
  switch (e.type) {
    case `text`:
      return Y(`text`, e.text);
    case `image`:
      return G(`Image output: ${e.mimeType}`);
    case `audio`:
      return G(`Audio output: ${e.mimeType}`);
    case `resource_link`:
      return G(`Resource: ${e.title ?? e.name ?? e.uri} (${e.uri})`);
    case `embedded_resource`: {
      let t = e.resource.title ?? e.resource.name ?? e.resource.uri;
      return e.resource.text != null && e.resource.text.trim().length > 0
        ? `${G(`Resource: ${t}`)}\n\n${Y(`text`, e.resource.text)}`
        : G(`Resource: ${t}`);
    }
    case `unknown`:
      return Y(`json`, Z(e.raw));
  }
}
function T(e) {
  return P(`Auto-review`, [
    `Status: ${e.status}`,
    e.riskLevel == null ? null : `Risk: ${e.riskLevel}`,
    e.rationale == null ? null : `Rationale: ${e.rationale}`,
  ]);
}
function E(e) {
  return e == null
    ? null
    : e.map(T).join(`

`);
}
function D(e, t) {
  return e == null
    ? P(`Generated image`, [`Status: ${t}`])
    : `Generated image\n\n![Generated image](${e})`;
}
function de(e, t) {
  if (e.kind === `collapsed-tool-activity`) {
    let n = Ae(e.summary);
    if (!c(u, e.units)) {
      let r = e.units.flatMap((e) => A(e, t)).map((e) => `- ${e}`);
      return r.length === 0
        ? n
        : I(
            n,
            r.join(`
`),
          );
    }
    if (e.units.length === 1) {
      let n = e.units[0];
      if (n?.kind === `entry` && n.entry.kind === `exploration`) return M(n, t);
    }
    return I(
      n,
      e.units.map((e) => O(e, t)).filter((e) => e != null).join(`

`),
    );
  }
  return e.kind === `pending-mcp-tool-calls`
    ? I(
        Me(e.items.length),
        e.items.map((e) => C(e)).join(`

`),
      )
    : e.kind === `dynamic-tool-call-group`
      ? I(
          Me(e.items.length),
          e.items.map((e) => h(e, t)).join(`

`),
        )
      : M(e, t);
}
function O(e, t) {
  return e.kind === `entry` &&
    e.entry.kind === `item` &&
    (e.entry.item.type === `exec` || e.entry.item.type === `patch`)
    ? k(e, t)
    : M(e, t);
}
function k(e, t) {
  let n = A(e, t);
  return n.length === 0
    ? null
    : n.map((e) => `- ${e}`).join(`
`);
}
function A(e, t) {
  return e.kind !== `entry` || e.entry.kind !== `item`
    ? []
    : e.entry.item.type === `exec`
      ? [`Ran ${q(fe(e.entry.item))}`, ...j(e.entry.item.automaticApprovalReviews)]
      : e.entry.item.type === `patch`
        ? [...pe(e.entry.item, t), ...j(e.entry.item.automaticApprovalReviews)]
        : [];
}
function j(e) {
  return e == null
    ? []
    : e.flatMap((e) => [
        `Auto-review: ${e.status}`,
        ...(e.riskLevel == null ? [] : [`Auto-review risk: ${e.riskLevel}`]),
        ...(e.rationale == null ? [] : [`Auto-review rationale: ${G(e.rationale)}`]),
      ]);
}
function fe(t) {
  let n = t.parsedCmd.cmd.trim();
  return n.length === 0 ? e(t.cmd) : n;
}
function pe(e, t) {
  let r = e.grantRoot == null ? t : W(e.grantRoot);
  return Object.entries(e.changes).flatMap(([e, t]) => {
    let i = n(e, t);
    if (i == null || i.trim().length === 0) return [];
    let a = Pe(i);
    return [`${me(t.type)} ${we(e, r)} (+${a.additions} -${a.deletions})`];
  });
}
function me(e) {
  switch (e) {
    case `add`:
    case `update`:
      return `Wrote`;
    case `delete`:
      return `Deleted`;
  }
}
function M(e, t) {
  if (e.kind === `web-search-group`)
    return I(
      `Searched the web`,
      e.items.map((e) => `- Searched the web for ${q(e.query)}`).join(`
`),
    );
  if (e.kind === `multi-agent-group`) {
    let t = e.items[0];
    return P(`Subagent action`, [
      t == null ? null : `Action: ${t.action}`,
      t == null ? null : `Status: ${t.status}`,
      `Receiver threads: ${e.items.length}`,
    ]);
  }
  return e.entry.kind === `exploration` ? he(e.entry, t) : h(e.entry.item, t);
}
function he(e, t) {
  let n = e.items
    .map((e) => ge(e, t))
    .filter((e) => e != null)
    .map((e) => `- ${e}`);
  return n.length === 0
    ? null
    : I(
        ke(e.items),
        n.join(`
`),
      );
}
function ge(e, t) {
  if (e.type !== `exec`) return null;
  let n = e.parsedCmd,
    r = e.cwd == null ? t : W(e.cwd);
  switch (n.type) {
    case `read`:
      return `Read ${B(n.path ?? n.name, r)}`;
    case `search`:
      return n.query != null && n.path != null
        ? `Searched for ${q(n.query)} in ${B(n.path, r)}`
        : n.query == null
          ? `Searched for files`
          : `Searched for ${q(n.query)}`;
    case `list_files`:
      return n.path == null ? `Listed files` : `Listed files in ${B(n.path, r)}`;
    case `format`:
    case `test`:
    case `lint`:
    case `noop`:
    case `unknown`:
      return q(n.cmd);
  }
}
function N(e, t) {
  let n = G(t).trim();
  return n.length === 0 ? null : `${e}\n\n${n}`;
}
function P(e, t) {
  let n = t.flatMap((e) => {
    if (e == null) return [];
    let t = G(e);
    return t.trim().length === 0 ? [] : [t];
  });
  return n.length === 0
    ? e
    : `${e}\n\n${n.join(`
`)}`;
}
function F(e, t) {
  t != null && t.trim().length > 0 && e.push(t);
}
function _e(e) {
  switch (e) {
    case `completed`:
      return `x`;
    case `in_progress`:
    case `pending`:
      return ` `;
  }
}
function ve(e) {
  let t = e.replaceAll(/\s+/g, ` `).trim();
  return t.length === 0 ? `Codex conversation` : t.replaceAll(`#`, `\\#`);
}
function I(e, t) {
  return `<details><summary>${e}</summary>\n\n${R(t).trim()}\n\n</details>`;
}
function ye(e, t) {
  return `<details><summary>${e}</summary>\n\n${L(t)}\n</details>`;
}
function L(e) {
  return R(e)
    .trim()
    .split(`
`)
    .map((e) => (e.length === 0 ? `>` : `> ${e}`)).join(`
`);
}
function R(e) {
  return e.replaceAll(
    /\r\n?/g,
    `
`,
  );
}
function z(e, n) {
  return xe(be(R(t(e))), n);
}
function be(e) {
  return e
    .split(`
`)
    .map((e) => {
      let t = e.replaceAll(l, ``);
      return t.trim().length === 0 ? `` : t.trimEnd();
    })
    .join(`
`)
    .replaceAll(
      /\n{3,}/g,
      `

`,
    );
}
function xe(e, t) {
  return e.replaceAll(f, (e, n, r) => {
    let i = n.startsWith(`<`) && n.endsWith(`>`),
      a = i ? n.slice(1, -1) : n,
      o = Se(a, t);
    return o === a ? e : `](${Ce(o, i)}${r ?? ``})`;
  });
}
function Se(e, t) {
  let { path: n } = H(e);
  return s(n) ? V(e, t) : e;
}
function Ce(e, t) {
  return t || /[\s()]/.test(e) ? `<${e}>` : e;
}
function B(e, t) {
  return q(V(e, t));
}
function we(e, t) {
  return q(Te(e, t));
}
function Te(e, t) {
  let n = V(e, t);
  return n === `.` ||
    n === `~` ||
    n.startsWith(`./`) ||
    n.startsWith(`../`) ||
    n.startsWith(`~/`) ||
    s(n)
    ? n
    : `./${n}`;
}
function V(e, t) {
  let { path: n, lineSuffix: r } = H(e),
    i = o(n),
    a = U(i, t.cwd, `.`);
  if (a != null) return `${a}${r}`;
  let s = U(i, t.homeDir, `~`);
  return s == null ? `${i}${r}` : `${s}${r}`;
}
function H(e) {
  let t = ce.exec(e);
  return t?.[1] == null || t[2] == null || !s(t[1])
    ? { lineSuffix: ``, path: e }
    : { lineSuffix: t[2], path: t[1] };
}
function U(e, t, n) {
  if (t == null || t === `/`) return null;
  let r = a(e),
    i = a(t);
  return r === i ? n : r.startsWith(`${i}/`) ? `${n}/${e.slice(t.length + 1)}` : null;
}
function W(e) {
  if (e == null) return d;
  let t = Ee(e);
  return t == null ? d : { cwd: t, homeDir: De(t) };
}
function Ee(e) {
  let t = o(e).replace(/\/+$/, ``);
  return s(t) ? t : null;
}
function De(e) {
  let t = p.exec(e);
  if (t != null) return t[0];
  let n = se.exec(e);
  return n == null ? null : n[0];
}
function G(e) {
  return K(R(e));
}
function K(e) {
  return e.replaceAll(/<\/?details(?=[\s>])[^>]*>/gi, (e) => Ie(e));
}
function q(e) {
  let t = "`".repeat(X(e) + 1);
  return `${t}${e}${t}`;
}
function J(e) {
  return `<code>${Ie(e)}</code>`;
}
function Y(e, t) {
  let n = R(t).trimEnd(),
    r = "`".repeat(Math.max(3, X(n) + 1));
  return `${r}${e}\n${n}\n${r}`;
}
function X(e) {
  let t = 0;
  for (let n of e.matchAll(/`+/g)) t = Math.max(t, n[0].length);
  return t;
}
function Z(e) {
  return JSON.stringify(e, null, 2) ?? `null`;
}
function Oe(e) {
  return `${e} previous ${e === 1 ? `message` : `messages`}`;
}
function ke(e) {
  let t = 0,
    n = 0,
    r = 0;
  for (let i of e)
    if (i.type === `exec`)
      switch (i.parsedCmd.type) {
        case `read`:
          t += 1;
          break;
        case `search`:
          n += 1;
          break;
        case `list_files`:
          r += 1;
          break;
        case `format`:
        case `test`:
        case `lint`:
        case `noop`:
        case `unknown`:
          break;
      }
  let i = [$(t, `file`, `files`), $(n, `search`, `searches`), $(r, `list`, `lists`)].filter(
    (e) => e != null,
  );
  return i.length === 0 ? `Explored` : `Explored ${i.join(`, `)}`;
}
function Ae(e) {
  let t = [];
  (Q(t, e.createdFileCount, `Created`, `created`, `file`, `files`),
    Q(t, e.editedFileCount, `Edited`, `edited`, `file`, `files`),
    Q(t, e.deletedFileCount, `Deleted`, `deleted`, `file`, `files`));
  let n = [
    $(e.exploredFileCount, `file`, `files`),
    $(e.searchCount, `search`, `searches`),
    $(e.listCount, `list`, `lists`),
  ].filter((e) => e != null);
  return (
    n.length > 0 && t.push(`${t.length === 0 ? `Explored` : `explored`} ${n.join(`, `)}`),
    Q(t, e.deniedRequestCount, `Denied`, `denied`, `request`, `requests`),
    je(t, e.timedOutRequestCount),
    Q(t, e.commandCount, `Ran`, `ran`, `command`, `commands`),
    Q(t, e.mcpToolCallCount, `Called`, `called`, `tool`, `tools`),
    e.webSearchCount > 0 && t.push(t.length === 0 ? `Searched the web` : `searched the web`),
    t.length === 0 ? `Tool activity` : t.join(`, `)
  );
}
function Q(e, t, n, r, i, a) {
  let o = $(t, i, a);
  o != null && e.push(`${e.length === 0 ? n : r} ${o}`);
}
function je(e, t) {
  let n = $(t, `request`, `requests`);
  n != null && e.push(`${n} timed out`);
}
function $(e, t, n) {
  return e === 0 ? null : e === 1 ? `a ${t}` : `${e} ${n}`;
}
function Me(e) {
  return e === 1 ? `Called a tool` : `Called ${e} tools`;
}
function Ne(e) {
  switch (e) {
    case `add`:
      return `Created`;
    case `delete`:
      return `Deleted`;
    case `update`:
      return `Edited`;
  }
}
function Pe(e) {
  let t = 0,
    n = 0;
  for (let r of R(e).split(`
`)) {
    if (r.startsWith(`+`) && !r.startsWith(`+++`)) {
      t += 1;
      continue;
    }
    r.startsWith(`-`) && !r.startsWith(`---`) && (n += 1);
  }
  return { additions: t, deletions: n };
}
function Fe(e) {
  return e.executionStatus === `interrupted`
    ? `Stopped`
    : e.output?.exitCode == null
      ? e.executionStatus === `completed`
        ? `Success`
        : `Running`
      : e.output.exitCode === 0
        ? `Success`
        : `Failed with exit code ${e.output.exitCode}`;
}
function Ie(e) {
  return e
    .replaceAll(`&`, `&amp;`)
    .replaceAll(`<`, `&lt;`)
    .replaceAll(`>`, `&gt;`)
    .replaceAll(`"`, `&quot;`);
}
export { le as renderConversationMarkdown };
//# sourceMappingURL=conversation-markdown.js.map
