...
function v({ cwd: e = null, projectlessOutputDirectory: t = null, title: n, turns: r }) {
  let i = [],
    a = U(e);
  ...
  let a = [],
            ...
            }),
    {
      ...
      c == null
        ? e.items
        : e.items.map((e) =>
            e.type === `assistant-message` ? { ...e, content: _(e.content, c) } : e,
          ),
      e.status,
    );
  for (let e of v) P(a, b(e, o));
  for (let e of l) P(a, le(e, o));
  for (let e of y) P(a, b(e, o));
  let D = [],
    { renderableAgentItems: O } = ne({
      agentItems: d,
      isTurnInProgress: e.status === `in_progress`,
      isAnyNonAgentItemInProgress: ee(u) || ee(S),
    }),
    k = ie({
      units: s({
        units: re({ units: oe(O), isActivitySliceClosed: u != null, conversationDetailLevel: Q }),
      }),
      isActivitySliceClosed: u != null,
    });
  for (let e of k) P(D, pe(e, o));
  for (let e of ae) P(D, b(e, o));
  p != null && P(D, b(p, o));
  for (let e of f) P(D, b(e, o));
  for (let e of se) P(D, b(e, o));
  (x != null && P(D, b(x, o)), S != null && P(D, b(S, o)), C != null && P(D, b(C, o)));
  for (let e of de) P(D, b(e, o));
  for (let e of w) P(D, b(e, o));
  (T != null && P(D, b(T, o)),
    E != null && P(D, b(E, o)),
    D.length > 0 &&
      a.push(
...
`),
    ...
    u != null && P(a, ue(u, { pathContext: o })));
  for (let e of m) P(a, b(e, o));
...
function b(e, t) {
  switch (e.type) {
    ...
    case `assistant-message`:
      return ue(e, { escapeDetailsTags: !0, pathContext: t });
    case `reasoning`:
      return null;
    case `proposed-plan`:
    ...
    case `forked-from-conversation`:
      return N(`Forked conversation`, [`Source conversation: ${e.sourceConversationId}`]);
    case `model-changed`:
      return N(`Model changed`, [`${e.fromModel} -> ${e.toModel}`]);
    case `model-rerouted`:
      return N(`Model rerouted`, [`${e.fromModel} -> ${e.toModel}`, `Reason: ${e.reason}`]);
...
function x(e, { escapeDetailsTags: t = !1, pathContext: n = $ } = {}) {
  ...
  i.length > 0 && r.push(i);
  let a = S(e, n);
  if ((a.length > 0 && r.push(N(`User context`, a)), r.length === 0)) return null;
  let o = r.join(`
...
function ue(e, { escapeDetailsTags: t = !1, pathContext: n = $ } = {}) {
...
function C(e) {
  ...
  return (
    t.push(Ve(e)),
    P(t, k(e.automaticApprovalReviews)),
    F(
      `Ran ${q(n)}`,
      t.join(`
...
function de(e) {
  let t = Object.entries(e.changes).flatMap(([e, t]) => {
    ...
    let r = Be(n);
    return [F(`${ze(t.type)} ${q(e)} +${r.additions} -${r.deletions}`, J(`diff`, n))];
  });
  return (
    P(t, k(e.automaticApprovalReviews)),
    t.length === 0
...
function T(e) {
  let t = [];
  return (
    e.explanation != null && e.explanation.trim().length > 0 && t.push(W(e.explanation).trim()),
    t.push(
...
`);
  return (
    n.length > 0 && t.push(n),
    e.result.structuredContent != null && t.push(J(`json`, Y(e.result.structuredContent))),
    t.join(`
...
function A(e, t) {
  return e.kind !== `entry` || e.entry.kind !== `item`
    ...
    : e.entry.item.type === `exec`
      ? [`Ran ${K(ge(e.entry.item))}`, ...j(e.entry.item.automaticApprovalReviews)]
...
function j(e) {
  return e == null
    ...
    : e.flatMap((e) => [
        ...
        ...(e.rationale == null ? [] : [`Auto-review rationale: ${W(e.rationale)}`]),
      ]);
}
function ge(e) {
...
function be(e, t) {
  ...
  switch (n.type) {
    ...
    case `list_files`:
      return n.path == null ? `Listed files` : `Listed files in ${z(n.path, r)}`;
    case `format`:
    case `test`:
    case `lint`:
    case `noop`:
...
function H(e, t, n) {
  ...
  let r = i(e),
    a = i(t);
  return r === a ? n : r.startsWith(`${a}/`) ? `${n}/${e.slice(t.length + 1)}` : null;
}
function U(e) {
  if (e == null) return $;
  let t = je(e);
