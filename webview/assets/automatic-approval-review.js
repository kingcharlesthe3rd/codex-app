...
import { i as r } from "./lib-1.js";
var i = n(),
  a = e(t(), 1);
function o(e, t) {
  let n = (0, i.c)(5),
    r = (0, a.useEffectEvent)(e),
    o;
  n[0] !== t || n[1] !== r
    ? ((o = () => {
        if (t == null) return;
    ...
    : (o = n[2]);
  let s;
  (n[3] === t ? (s = n[4]) : ((s = [t]), (n[3] = t), (n[4] = s)), (0, a.useEffect)(o, s));
}
var s = r({
...
function c(e, t) {
  switch (t.status) {
    case `inProgress`:
      return e.formatMessage(s.titleInProgress);
    case `approved`:
      return e.formatMessage(s.titleApproved);
    case `denied`:
      return t.riskLevel === `high`
        ? e.formatMessage(s.titleDeniedHighRisk)
        : e.formatMessage(s.titleDenied);
    case `timedOut`:
      return e.formatMessage(s.titleTimedOut);
    case `aborted`:
      return e.formatMessage(s.titleAborted);
  }
}
function l(e, t) {
  return t.rationale != null && t.rationale.trim().length > 0
    ? t.rationale.trim()
    : t.status === `inProgress`
      ? e.formatMessage(s.summaryInProgress)
      : t.status === `aborted`
        ? e.formatMessage(s.summaryAborted)
        : t.status === `timedOut`
          ? e.formatMessage(s.summaryTimedOut)
          : e.formatMessage(s.summaryCompleted);
}
function u(e, t) {
  if (t == null) return e.formatMessage(s.actionSummaryRequest);
  switch (t.type) {
    case `command`:
      return t.command;
    case `execve`:
    ...
    case `applyPatch`:
      return t.files.length === 1
        ? e.formatMessage(s.actionSummaryEditingFile, { file: t.files[0] })
        : e.formatMessage(s.actionSummaryEditingFiles, { fileCount: t.files.length });
    case `networkAccess`:
      return e.formatMessage(s.actionSummaryNetworkAccess, { target: t.target });
    case `mcpToolCall`:
      return e.formatMessage(s.actionSummaryMcpToolCall, {
        toolName: t.toolName,
        connector: t.connectorName ?? t.server,
      });
    case `requestPermissions`:
      return t.reason == null
        ? e.formatMessage(s.actionSummaryPermissionRequest)
        : e.formatMessage(s.actionSummaryPermissionRequestWithReason, { reason: t.reason });
  }
}
export { o as i, l as n, c as r, u as t };
