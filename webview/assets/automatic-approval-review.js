...
function n(e, n) {
  switch (n.status) {
    case `inProgress`:
      return e.formatMessage(t.titleInProgress);
    case `approved`:
      return e.formatMessage(t.titleApproved);
    case `denied`:
      return n.riskLevel === `high`
        ? e.formatMessage(t.titleDeniedHighRisk)
        : e.formatMessage(t.titleDenied);
    case `timedOut`:
      return e.formatMessage(t.titleTimedOut);
    case `aborted`:
      return e.formatMessage(t.titleAborted);
  }
}
function r(e, n) {
  return n.rationale != null && n.rationale.trim().length > 0
    ? n.rationale.trim()
    : n.status === `inProgress`
      ? e.formatMessage(t.summaryInProgress)
      : n.status === `aborted`
        ? e.formatMessage(t.summaryAborted)
        : n.status === `timedOut`
          ? e.formatMessage(t.summaryTimedOut)
          : e.formatMessage(t.summaryCompleted);
}
function i(e, n) {
  if (n == null) return e.formatMessage(t.actionSummaryRequest);
  switch (n.type) {
    case `command`:
      return n.command;
    case `execve`:
      return [n.program, ...n.argv].join(` `);
    case `applyPatch`:
      return n.files.length === 1
        ? e.formatMessage(t.actionSummaryEditingFile, { file: n.files[0] })
        : e.formatMessage(t.actionSummaryEditingFiles, { fileCount: n.files.length });
    case `networkAccess`:
      return e.formatMessage(t.actionSummaryNetworkAccess, { target: n.target });
    case `mcpToolCall`:
      return e.formatMessage(t.actionSummaryMcpToolCall, {
        toolName: n.toolName,
        connector: n.connectorName ?? n.server,
      });
    case `requestPermissions`:
      return n.reason == null
        ? e.formatMessage(t.actionSummaryPermissionRequest)
        : e.formatMessage(t.actionSummaryPermissionRequestWithReason, { reason: n.reason });
  }
}
export { r as n, n as r, i as t };
//# sourceMappingURL=automatic-approval-review.js.map
