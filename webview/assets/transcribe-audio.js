...
import { l as r } from "./vscode-api.js";
import {
  Ji as i,
  Qi as a,
  T as o,
...
import { n as m } from "./setting-storage.js";
import { n as h } from "./base64.js";
import { n as g } from "./request.js";
e(t());
var _ = n(),
  v = (e) =>
    (0, _.jsx)(`svg`, {
  ...
  y = [`codex-app`, `dictation`, `streaming`].join(`-`),
  b = p({ code: l(), message: l(), retryable: s() }),
  x = p({
    session_id: l(),
    status: i([`active`, `closed`]),
    config: p({
      provider_mode: i([`buffered`, `streaming_sse`]),
      transcript_delivery_mode: i([`final_only`, `segment`, `delta`]),
    }),
  }),
  S = d(`type`, [
    p({ type: f(`session.started`), sequence_no: u(), session: x }),
    p({ type: f(`session.updated`), sequence_no: u(), session: x }),
    p({ type: f(`speech.started`), sequence_no: u(), utterance_id: l() }),
    p({ type: f(`speech.stopped`), sequence_no: u(), utterance_id: l() }),
    p({
      type: f(`transcript.delta`),
      sequence_no: u(),
      utterance_id: l(),
      revision: u(),
      text: l(),
    ...
    p({
      type: f(`transcript.final`),
      sequence_no: u(),
      utterance_id: l(),
      revision: u(),
      text: l(),
    }),
    p({ type: f(`transcript.failed`), sequence_no: u(), utterance_id: l().nullish(), error: b }),
    p({ type: f(`session.error`), sequence_no: u(), fatal: s(), error: b }),
  ]),
  C = 1e4,
  w = 8e3,
  ...
  E = class {
    ...
    async start(e) {
      ...
      if (t == null) throw new T(`AudioContext is not available for streaming dictation.`);
      let n = new t();
      ((this.audioContext = n),
    ...
    async finish() {
      return (this.stopAudioCapture(), await this.client.finish(), F(this.transcriptState));
    }
    close() {
  ...
  D = class {
    ...
    async connect(e) {
      ...
      return new Promise((t, n) => {
        ...
        let o = !1,
          ...
          d = window.setTimeout(() => {
            let e = new T(`Dictation stream timed out before session.start completed.`);
            ((c = e), a.close(), l(e));
          }, C);
        (a.addEventListener(
          `open`,
          ...
          a.addEventListener(`message`, (e) => {
            let t = I(e.data);
            if (t == null) {
              let e = new T(`Dictation stream returned an invalid event payload.`);
              ((c = e), l(e), a.close());
              return;
            }
          ...
          a.addEventListener(
            `error`,
            () => {
              c ??= new T(`Dictation stream websocket failed.`);
            },
            { once: !0 },
          ),
          a.addEventListener(
    ...
    appendPCM16(e) {
      let t = { type: `audio.append`, audio: h(e) };
        ...
        ? this.finishPromise
          ...
          : ((this.finishPromise = new Promise((e, t) => {
              let n = window.setTimeout(() => {
                let e = new T(`Dictation stream timed out while closing the session.`);
    ...
    close() {
      ((this.pendingStartupAudioAppends = null),
        this.rejectConnectBeforeWebsocket?.(
          new T(`Dictation stream closed before websocket connection started.`),
        ),
        (this.rejectConnectBeforeWebsocket = null),
...
async function O() {
  return (await r.getInstance().post(`/codex/dictation-stream-connect-info`, void 0)).body;
}
function k(e) {
  return {
    type: `session.start`,
...
function N(e, t) {
  switch (t.type) {
    ...
    case `transcript.final`:
      (P(e, t.utterance_id), (e.finalTextByUtteranceId[t.utterance_id] = t.text));
      return;
...
var L = `gpt-5.4-mini`,
  ee = p({
    output: a(p({ type: l(), content: a(p({ type: l(), text: l().optional() })).optional() })),
  }),
  R = p({
    type: l().optional(),
    delta: l().optional(),
    text: l().optional(),
    response: ee.optional(),
    error: c([l(), p({ message: l().optional() })]).optional(),
  }).passthrough();
async function z({ instructions: e, input: t, model: n = L }) {
  let r = t.trim();
...
function B(e) {
  return new Promise((t, n) => {
    ...
    function l(e) {
      s || ((s = !0), o != null && r.getInstance().cancelStream(o), n(e));
    }
    o = r.getInstance().stream(`POST`, `/codex/responses`, {
      body: e,
      headers: g(),
      onEvent: (e) => {
        let t = V(e);
        ...
        (t.delta != null && i.push(t.delta), t.completedText != null && (a = t.completedText));
      },
      onError: (e) => {
        l(Error(e.error));
      },
...
function V(e) {
  ...
  return n == null
    ? t.response == null
      ...
      : { delta: null, completedText: W(t.response), error: null }
...
async function Y({ transcript: e, surroundingText: t, cleanupEnabled: n }) {
  ...
  try {
    ...
    return (
      (await z({
        instructions: J,
        input: X({ transcript: r, surroundingText: t ?? null, dictionary: e }),
      })) ?? r
    );
  } catch {
    return r;
...
function X({ transcript: e, surroundingText: t, dictionary: n }) {
  let r = t?.trim().slice(0, K);
...
async function Z() {
  let e = q.safeParse(await m(o.dictationDictionary));
  return e.success
    ? e.data
        .map((e) => e.trim())
        .filter((e) => e.length > 0)
...
async function Q(e, t = {}) {
  let n = t.contentType ?? (e.type && e.type.trim().length > 0 ? e.type : `audio/webm`),
    i = n.split(/[/;]/)[1] ?? `webm`,
    a = te(t.filename ?? `codex.${i}`),
    o = ne(),
    s = h(await $({ blob: e, boundary: o, filename: a, contentType: n, language: t.language })),
    c = { "Content-Type": `multipart/form-data; boundary=${o}`, "X-Codex-Base64": `1`, ...g() };
  return (await r.getInstance().post(`/transcribe`, s, c)).body.text;
}
async function $({ blob: e, boundary: t, filename: n, contentType: r, language: i }) {
...
export { v as a, y as i, Y as n, E as r, Q as t };
