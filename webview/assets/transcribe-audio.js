...
import { n as f } from "./setting-storage.js";
import { n as p } from "./base64.js";
import { n as m } from "./request.js";
var h = [`codex-app`, `dictation`, `streaming`].join(`-`),
  g = r({ code: t(), message: t(), retryable: a() }),
  _ = r({
    session_id: t(),
  ...
  v = i(`type`, [
    r({ type: d(`session.started`), sequence_no: u(), session: _ }),
    r({ type: d(`session.updated`), sequence_no: u(), session: _ }),
    r({ type: d(`speech.started`), sequence_no: u(), utterance_id: t() }),
    r({ type: d(`speech.stopped`), sequence_no: u(), utterance_id: t() }),
    r({
      type: d(`transcript.delta`),
      sequence_no: u(),
      utterance_id: t(),
      revision: u(),
      text: t(),
    ...
    r({
      type: d(`transcript.final`),
      sequence_no: u(),
      utterance_id: t(),
      revision: u(),
      text: t(),
    }),
    r({ type: d(`transcript.failed`), sequence_no: u(), utterance_id: t().nullish(), error: g }),
    r({ type: d(`session.error`), sequence_no: u(), fatal: a(), error: g }),
  ]),
  y = 1e4,
  b = 8e3,
  ...
  S = class {
    ...
    async start(e) {
      ...
      if (t == null) throw new x(`AudioContext is not available for streaming dictation.`);
      let n = new t();
      ((this.audioContext = n),
    ...
    async finish() {
      return (this.stopAudioCapture(), await this.client.finish(), j(this.transcriptState));
    }
    close() {
  ...
  C = class {
    ...
    async connect(e) {
      ...
      return new Promise((t, n) => {
        ...
        let o = !1,
          ...
          d = window.setTimeout(() => {
            let e = new x(`Dictation stream timed out before session.start completed.`);
            ((c = e), a.close(), l(e));
          }, y);
        (a.addEventListener(
          `open`,
          ...
          a.addEventListener(`message`, (e) => {
            let t = M(e.data);
            if (t == null) {
              let e = new x(`Dictation stream returned an invalid event payload.`);
              ((c = e), l(e), a.close());
              return;
            }
          ...
          a.addEventListener(
            `error`,
            () => {
              c ??= new x(`Dictation stream websocket failed.`);
            },
            { once: !0 },
          ),
          a.addEventListener(
    ...
    appendPCM16(e) {
      let t = { type: `audio.append`, audio: p(e) };
        ...
        ? this.finishPromise
          ...
          : ((this.finishPromise = new Promise((e, t) => {
              let n = window.setTimeout(() => {
                let e = new x(`Dictation stream timed out while closing the session.`);
    ...
    close() {
      ((this.pendingStartupAudioAppends = null),
        this.rejectConnectBeforeWebsocket?.(
          new x(`Dictation stream closed before websocket connection started.`),
        ),
        (this.rejectConnectBeforeWebsocket = null),
...
async function w() {
  return (await e.getInstance().post(`/codex/dictation-stream-connect-info`, void 0)).body;
}
function T(e) {
  return {
    type: `session.start`,
...
function k(e, t) {
  switch (t.type) {
    ...
    case `transcript.final`:
      (A(e, t.utterance_id), (e.finalTextByUtteranceId[t.utterance_id] = t.text));
      return;
...
var N = `gpt-5.4-mini`,
  P = r({
    output: c(r({ type: t(), content: c(r({ type: t(), text: t().optional() })).optional() })),
  }),
  F = r({
    type: t().optional(),
    delta: t().optional(),
    text: t().optional(),
    response: P.optional(),
    error: o([t(), r({ message: t().optional() })]).optional(),
  }).passthrough();
async function I({
  instructions: e,
  input: t,
  ...
  return ee(JSON.stringify(o), r, i);
}
function ee(t, n, r) {
  return (
    r?.throwIfAborted(),
    new Promise((i, a) => {
      let o = [],
        ...
        l = !1;
      function u(e) {
        l || ((l = !0), r?.removeEventListener(`abort`, f), i(e));
      }
      function d(t) {
        l ||
          ((l = !0),
          r?.removeEventListener(`abort`, f),
          c != null && e.getInstance().cancelStream(c),
          a(t));
      }
      function f() {
        d(new DOMException(`The operation was aborted`, `AbortError`));
      }
      (r?.addEventListener(`abort`, f, { once: !0 }),
        (c = e.getInstance().stream(`POST`, `/codex/responses`, {
          body: t,
          headers: m(),
          onEvent: (e) => {
            let t = L(e, n);
            ...
            (t.delta != null && o.push(t.delta), t.completedText != null && (s = t.completedText));
          },
          onError: (e) => {
            d(Error(e.error));
          },
...
function L(e, t) {
  ...
  return r == null
    ? n.response == null
      ...
      : { delta: null, completedText: t ? H(n.response) : B(n.response), error: null }
...
async function q({ transcript: e, surroundingText: t, cleanupEnabled: n }) {
  ...
  try {
    ...
    return (
      (await I({
        instructions: K,
        input: J({ transcript: r, surroundingText: t ?? null, dictionary: e }),
      })) ?? r
    );
  } catch {
    return r;
...
function J({ transcript: e, surroundingText: t, dictionary: n }) {
...
async function Y() {
  let e = G.safeParse(await f(l.dictationDictionary));
  return e.success
    ? e.data
        .map((e) => e.trim())
        .filter((e) => e.length > 0)
...
async function X(t, r = {}) {
  let i = r.contentType ?? (t.type && t.type.trim().length > 0 ? t.type : `audio/webm`),
    a = i.split(/[/;]/)[1] ?? `webm`,
    o = Q(r.filename ?? `codex.${a}`),
    s = $(),
    c = p(await Z({ blob: t, boundary: s, filename: o, contentType: i, language: r.language })),
    l = { "Content-Type": `multipart/form-data; boundary=${s}`, [n]: `1`, ...m() };
  return (await e.getInstance().post(`/transcribe`, c, l)).body.text;
}
async function Z({ blob: e, boundary: t, filename: n, contentType: r, language: i }) {
...
export { h as i, q as n, S as r, X as t };
