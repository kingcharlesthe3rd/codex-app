...
import { a as h, n as g } from "./request.js";
e(t());
var _ = n(),
  v = (e) =>
    (0, _.jsx)(`svg`, {
  ...
  b = l({ code: u(), message: u(), retryable: s() }),
  x = l({
    session_id: u(),
  ...
  S = o(`type`, [
    l({ type: d(`session.started`), sequence_no: c(), session: x }),
    l({ type: d(`session.updated`), sequence_no: c(), session: x }),
    l({ type: d(`speech.started`), sequence_no: c(), utterance_id: u() }),
    l({ type: d(`speech.stopped`), sequence_no: c(), utterance_id: u() }),
    l({
    ...
    l({ type: d(`transcript.failed`), sequence_no: c(), utterance_id: u().nullish(), error: b }),
    l({ type: d(`session.error`), sequence_no: c(), fatal: s(), error: b }),
  ]),
  C = 1e4,
  w = 8e3,
...
var ee = `gpt-5.4-mini`,
  L = l({
    output: p(l({ type: u(), content: p(l({ type: u(), text: u().optional() })).optional() })),
  }),
  R = l({
    type: u().optional(),
    delta: u().optional(),
    text: u().optional(),
    response: L.optional(),
    error: f([u(), l({ message: u().optional() })]).optional(),
  }).passthrough();
async function z({ instructions: e, input: t, model: n = ee }) {
  let r = t.trim();
...
async function Z() {
  let e = q.safeParse(await m(a.dictationDictionary));
  return e.success
    ? e.data
        .map((e) => e.trim())
        .filter((e) => e.length > 0)
