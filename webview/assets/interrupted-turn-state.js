import { s as e } from "./chunk.js";
import { n as t, t as n } from "./jsx-runtime.js";
import {
  R as r,
  a as i,
  c as a,
  g as o,
  h as s,
  l as c,
  o as l,
  s as u,
  t as d,
  v as f,
} from "./app-scope.js";
import {
  D as p,
  Q as m,
  Ro as h,
  St as g,
  T as _,
  X as v,
  Y as y,
  g as b,
  js as x,
  lt as S,
  nt as C,
  ot as w,
  tt as T,
} from "./app-server-manager-signals.js";
import { f as E } from "./vscode-api.js";
import { i as D } from "./react.js";
import { A as O, ft as k } from "./src-1.js";
import { o as A, r as ee } from "./lib.js";
import { Ar as j, kr as te, o as ne } from "./persisted-signal.js";
import { t as M } from "./button-DO-oxX3-.js";
import { a as N } from "./setting-storage.js";
import { o as re } from "./statsig.js";
import { t as P } from "./route-scope.js";
import { g as ie, h as ae } from "./diff-view-mode.js";
import { n as oe } from "./app-shell-tab-controller.js";
import { n as se } from "./local-conversation-title-signals.js";
import { N as ce } from "./review-navigation-model.js";
import { i as le } from "./thread-context.js";
import { t as ue } from "./terminal-C.js";
import {
  d as F,
  f as I,
  g as de,
  h as fe,
  l as pe,
  m as me,
  p as he,
  u as ge,
} from "./thread-page-bottom-panel-state.js";
import "./font-settings.js";
import { n as _e, r as ve } from "./use-resolved-theme-variant.js";
import { a as L, d as ye, i as be, l as xe, n as Se, r as Ce } from "./dialog-layout.js";
import { N as we } from "./app-server-dynamic-tools.js";
import { t as Te } from "./sortBy.js";
var Ee = new (class {
  pendingThreadSwitch = null;
  constructor(e = () => performance.now()) {
    this.now = e;
  }
  start(...[e, t]) {
    switch (e) {
      case `thread_switch_completed`:
        this.pendingThreadSwitch = { defaults: t, startedAtMs: this.now() };
        return;
    }
  }
  complete(e, ...[t, n]) {
    switch (t) {
      case `thread_switch_completed`: {
        let t = this.pendingThreadSwitch;
        if (t == null || t.defaults.conversationId !== n.conversationId) return;
        ((this.pendingThreadSwitch = null),
          ne(e, te, {
            durationMs: Math.max(0, Math.round(this.now() - t.startedAtMs)),
            switchKind: De(t.defaults.needsResume ? `cold` : `hot`),
            turnCount: n.turnCount,
          }));
        return;
      }
    }
  }
})();
function De(e) {
  switch (e) {
    case `hot`:
      return j.CODEX_THREAD_SWITCH_KIND_HOT;
    case `cold`:
      return j.CODEX_THREAD_SWITCH_KIND_COLD;
  }
}
var R = e(t()),
  z = n(),
  Oe = (e) =>
    (0, z.jsx)(`svg`, {
      fill: `none`,
      height: 16,
      viewBox: `0 0 16 16`,
      width: 16,
      xmlns: `http://www.w3.org/2000/svg`,
      ...e,
      children: (0, z.jsx)(`path`, {
        clipRule: `evenodd`,
        d: `m8.69891 2.27345c.61298-.91267 1.85279-1.0459 2.67379-.40625l.1582.13867.003.00196 2.4433 2.41504.002.00195c.4266.42674.5579.99703.499 1.50586-.0582.50163-.3073 1.00104-.7187 1.31836l-.0069.00586-.0078.00586-2.415 1.7246c-.1659.11864-.2814.29628-.3223.4961l-.5293 2.59084-.0029.0166-.0039.0156c-.1932.7147-.70508 1.2981-1.36526 1.5254-.68313.2349-1.44732.0609-2.04883-.585l-1.69336-1.6679-2.99316 2.9941c-.20505.2047-.53727.2049-.74219 0-.2047-.2049-.20464-.5372 0-.7422l2.98731-2.9883-1.59571-1.57125c-.58829-.57126-.78753-1.34848-.59179-2.04199.19649-.69522.77365-1.25029 1.60644-1.41309l2.48047-.57226c.19499-.04518.367-.16098.48145-.3252zm2.02639.4209c-.371-.28905-.90826-.20853-1.15822.16894l-.00684.01075-1.70215 2.44238c-.26325.37786-.65778.64527-1.10644.74902l-2.48047.57227-.01074.00195-.01074.00293c-.46996.08744-.72571.36997-.81055.66992-.07571.26809-.02825.59512.20312.88379l.11133.1211.00293.0039 4.03809 3.9756.00976.0098.00977.0107c.34353.3766.68604.4105.95117.3193.29254-.101.57406-.3851.68848-.7959l.52539-2.57028c.09401-.45959.35861-.86689.74021-1.13965l2.4024-1.71679c.1553-.12183.2834-.34601.3134-.6045.0292-.25251-.0393-.48226-.1962-.64062l-2.4415-2.41211z`,
        fill: `currentColor`,
        fillRule: `evenodd`,
      }),
    }),
  ke = (e) =>
    (0, z.jsx)(`svg`, {
      fill: `none`,
      height: 16,
      viewBox: `0 0 16 16`,
      width: 16,
      xmlns: `http://www.w3.org/2000/svg`,
      ...e,
      children: (0, z.jsx)(`path`, {
        d: `m8.69891 2.27336c.61298-.91267 1.85279-1.04591 2.67379-.40625l.1582.13867.003.00195 2.4433 2.41504.002.00195c.4266.42675.5579.99704.499 1.50586-.0582.50163-.3073 1.00104-.7187 1.31836l-.0069.00586-.0078.00586-2.415 1.72461c-.1659.11864-.2814.29628-.3223.49609l-.5293 2.59084-.0029.0166-.0039.0156c-.1932.7147-.70508 1.2981-1.36526 1.5254-.68313.2349-1.44732.0609-2.04883-.585l-1.69336-1.6679-2.99316 2.9941c-.20505.2047-.53727.2049-.74219 0-.2047-.2049-.20464-.5372 0-.7422l2.98731-2.9883-1.59571-1.57125c-.58829-.57126-.78754-1.34847-.59179-2.04199.19649-.69522.77364-1.25029 1.60644-1.41308l2.48047-.57227c.19499-.04517.367-.16097.48145-.3252z`,
        fill: `currentColor`,
      }),
    }),
  B = r();
function V(e) {
  let t = (0, B.c)(10),
    { isPinned: n, ariaLabel: r, onPin: i, onUnpin: a } = e,
    o = n ? ke : Oe,
    s;
  t[0] !== n || t[1] !== i || t[2] !== a
    ? ((s = (e) => {
        (e.stopPropagation(), n ? a() : i());
      }),
      (t[0] = n),
      (t[1] = i),
      (t[2] = a),
      (t[3] = s))
    : (s = t[3]);
  let c;
  t[4] === o
    ? (c = t[5])
    : ((c = (0, z.jsx)(o, { className: `icon-2xs block shrink-0` })), (t[4] = o), (t[5] = c));
  let l;
  return (
    t[6] !== r || t[7] !== s || t[8] !== c
      ? ((l = (0, z.jsx)(`button`, {
          type: `button`,
          "aria-label": r,
          className: `flex h-5 w-5 items-center justify-center leading-none text-token-foreground/50 hover:text-token-foreground`,
          onClick: s,
          children: c,
        })),
        (t[6] = r),
        (t[7] = s),
        (t[8] = c),
        (t[9] = l))
      : (l = t[9]),
    l
  );
}
var { pin: H, unpin: U } = ee({
  pin: {
    id: `sidebarElectron.pinThread`,
    defaultMessage: `Pin chat`,
    description: `Action label to pin a thread in the sidebar`,
  },
  unpin: {
    id: `sidebarElectron.unpinThread`,
    defaultMessage: `Unpin chat`,
    description: `Action label to unpin a thread from the sidebar`,
  },
});
function Ae({ isPinned: e, onPinnedChange: t }) {
  return {
    id: e ? `unpin-thread` : `pin-thread`,
    message: e ? U : H,
    onSelect: () => {
      t(!e);
    },
  };
}
function je({ isPinned: e, hasUnreadTurn: t, ariaLabel: n, onPin: r, onUnpin: i }) {
  if (t)
    return {
      rest: null,
      hover: (0, z.jsx)(`span`, { "aria-hidden": !0, className: `block h-5 w-5` }),
    };
  let a = (0, z.jsx)(V, { isPinned: e, ariaLabel: n, onPin: r, onUnpin: i });
  return { rest: e ? a : null, hover: a };
}
var Me = (e) =>
  (0, z.jsx)(`svg`, {
    width: 24,
    height: 24,
    viewBox: `0 0 24 24`,
    fill: `none`,
    xmlns: `http://www.w3.org/2000/svg`,
    ...e,
    children: (0, z.jsx)(`path`, {
      d: `M12.8636 3.26029C13.9444 1.74708 16.1254 1.56658 17.4403 2.88151L21.1185 6.55974C22.4335 7.87467 22.2529 10.0556 20.7397 11.1364L16.4786 14.1801C16.1638 14.405 16 14.7306 16 15V17.5C16 18.907 15.0409 19.9513 13.976 20.4105C12.9046 20.8724 11.4792 20.8468 10.4568 19.8244L8.02332 17.3909L3.70711 21.7071C3.31658 22.0977 2.68342 22.0977 2.29289 21.7071C1.90237 21.3166 1.90237 20.6835 2.29289 20.2929L6.60911 15.9767L4.17567 13.5433C3.1532 12.5208 3.12762 11.0955 3.58957 10.024C4.04871 8.95911 5.09306 8.00003 6.5 8.00003H9C9.26948 8.00003 9.59505 7.83624 9.81994 7.52139L12.8636 3.26029Z`,
      fill: `currentColor`,
    }),
  });
function Ne(e) {
  let t = (0, B.c)(37),
    { heartbeatAutomationName: n, open: r, onOpenChange: i, onConfirm: a } = e,
    o = n === void 0 ? null : n,
    s = o != null,
    c;
  t[0] === o ? (c = t[1]) : ((c = o != null && o.trim().length > 0), (t[0] = o), (t[1] = c));
  let l = c,
    u;
  t[2] === s
    ? (u = t[3])
    : ((u = s
        ? (0, z.jsx)(A, {
            id: `threadHeader.archiveConfirmHeartbeatTitle`,
            defaultMessage: `Archive chat and remove automation?`,
            description: `Title for archive chat confirmation dialog when the chat has an active heartbeat automation`,
          })
        : (0, z.jsx)(A, {
            id: `threadHeader.archiveConfirmTitle`,
            defaultMessage: `Archive chat?`,
            description: `Title for archive chat confirmation dialog`,
          })),
      (t[2] = s),
      (t[3] = u));
  let d = u,
    f;
  t[4] !== s || t[5] !== l || t[6] !== o
    ? ((f = s
        ? l
          ? (0, z.jsx)(A, {
              id: `threadHeader.archiveConfirmHeartbeatSubtitleNamed`,
              defaultMessage: `This chat has an active heartbeat automation, {name}. Archiving the chat will also remove it and stop future runs.`,
              description: `Subtitle for archive chat confirmation dialog when the chat has a named active heartbeat automation`,
              values: {
                name: (0, z.jsx)(
                  `strong`,
                  { className: `font-semibold text-token-text-primary`, children: o },
                  `automation-name`,
                ),
              },
            })
          : (0, z.jsx)(A, {
              id: `threadHeader.archiveConfirmHeartbeatSubtitleUnnamed`,
              defaultMessage: `This chat has an active heartbeat automation. Archiving the chat will also remove it and stop future runs.`,
              description: `Subtitle for archive chat confirmation dialog when the chat has an unnamed active heartbeat automation`,
            })
        : (0, z.jsx)(A, {
            id: `threadHeader.archiveConfirmSubtitle`,
            defaultMessage: `You can find it later in your archived chats.`,
            description: `Subtitle for archive chat confirmation dialog`,
          })),
      (t[4] = s),
      (t[5] = l),
      (t[6] = o),
      (t[7] = f))
    : (f = t[7]);
  let p = f,
    m;
  t[8] === i
    ? (m = t[9])
    : ((m = {
        "aria-describedby": void 0,
        onOpenAutoFocus: Pe,
        onEscapeKeyDown: () => {
          i(!1);
        },
      }),
      (t[8] = i),
      (t[9] = m));
  let h;
  t[10] === a
    ? (h = t[11])
    : ((h = (e) => {
        (e.preventDefault(), a());
      }),
      (t[10] = a),
      (t[11] = h));
  let g;
  t[12] === d
    ? (g = t[13])
    : ((g = (0, z.jsx)(ye, { className: `sr-only`, children: d })), (t[12] = d), (t[13] = g));
  let _;
  t[14] !== p || t[15] !== d
    ? ((_ = (0, z.jsx)(L, { children: (0, z.jsx)(be, { title: d, subtitle: p }) })),
      (t[14] = p),
      (t[15] = d),
      (t[16] = _))
    : (_ = t[16]);
  let v;
  t[17] === i ? (v = t[18]) : ((v = () => i(!1)), (t[17] = i), (t[18] = v));
  let y;
  t[19] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((y = (0, z.jsx)(A, {
        id: `threadHeader.archiveConfirmCancel`,
        defaultMessage: `Cancel`,
        description: `Cancel button label for archive thread confirmation dialog`,
      })),
      (t[19] = y))
    : (y = t[19]);
  let b;
  t[20] === v
    ? (b = t[21])
    : ((b = (0, z.jsx)(M, { color: `ghost`, type: `button`, onClick: v, children: y })),
      (t[20] = v),
      (t[21] = b));
  let x;
  t[22] === s
    ? (x = t[23])
    : ((x = (0, z.jsx)(M, {
        "data-archive-confirm-button": !0,
        color: `danger`,
        type: `submit`,
        children: s
          ? (0, z.jsx)(A, {
              id: `threadHeader.archiveConfirmHeartbeatConfirm`,
              defaultMessage: `Archive and remove`,
              description: `Confirm button label for archive chat confirmation dialog when the chat has an active heartbeat automation`,
            })
          : (0, z.jsx)(A, {
              id: `threadHeader.archiveConfirmConfirm`,
              defaultMessage: `Archive`,
              description: `Confirm button label for archive chat confirmation dialog`,
            }),
      })),
      (t[22] = s),
      (t[23] = x));
  let S;
  t[24] !== b || t[25] !== x
    ? ((S = (0, z.jsx)(L, { children: (0, z.jsxs)(Ce, { children: [b, x] }) })),
      (t[24] = b),
      (t[25] = x),
      (t[26] = S))
    : (S = t[26]);
  let C;
  t[27] !== S || t[28] !== h || t[29] !== g || t[30] !== _
    ? ((C = (0, z.jsxs)(Se, { as: `form`, onSubmit: h, children: [g, _, S] })),
      (t[27] = S),
      (t[28] = h),
      (t[29] = g),
      (t[30] = _),
      (t[31] = C))
    : (C = t[31]);
  let w;
  return (
    t[32] !== i || t[33] !== r || t[34] !== C || t[35] !== m
      ? ((w = (0, z.jsx)(xe, {
          open: r,
          onOpenChange: i,
          size: `compact`,
          contentProps: m,
          children: C,
        })),
        (t[32] = i),
        (t[33] = r),
        (t[34] = C),
        (t[35] = m),
        (t[36] = w))
      : (w = t[36]),
    w
  );
}
function Pe(e) {
  (e.preventDefault(), e.currentTarget?.querySelector(`[data-archive-confirm-button]`)?.focus());
}
function Fe(e) {
  let t = (0, B.c)(6),
    { conversationId: n } = e,
    r = re(`459748632`) && n != null,
    i;
  t[0] !== r || t[1] !== n
    ? ((i = () => {
        !r || n == null || E.dispatchMessage(`open-in-new-window`, { path: k(n) });
      }),
      (t[0] = r),
      (t[1] = n),
      (t[2] = i))
    : (i = t[2]);
  let a = i,
    o;
  return (
    t[3] !== r || t[4] !== a
      ? ((o = { canOpenThreadInNewWindow: r, openThreadInNewWindow: a }),
        (t[3] = r),
        (t[4] = a),
        (t[5] = o))
      : (o = t[5]),
    o
  );
}
function W(e) {
  for (let t = e.commandActions.length - 1; t >= 0; --t) {
    let n = e.commandActions[t]?.command.trim() ?? ``;
    if (n.length > 0 && !G(n)) return n;
  }
  let t = e.command.trim();
  return G(t) ? `` : t;
}
function G(e) {
  return /^(?:.*[/\\])?(?:bash|cmd(?:\.exe)?|fish|powershell(?:\.exe)?|pwsh(?:\.exe)?|sh|zsh)(?:\s|$)/iu.test(
    e,
  );
}
var Ie = 60,
  Le = a(d, ({ get: e }) => {
    let t = [];
    for (let n of e(C))
      for (let r of e(T, n))
        t.push({ cwd: e(b, r), hostId: n, id: r, title: e(se, r), turns: e(g, r) ?? [] });
    return t;
  });
function Re(e) {
  let t = [];
  for (let n of e) {
    let e = n.turns.length - 1;
    for (let r = e; r >= 0; --r) {
      let i = n.turns[r];
      if (i == null) continue;
      let a = r === e && i.status === `inProgress`;
      for (let e of i.items) {
        if (e?.type !== `commandExecution`) continue;
        let o = He(e, i, a);
        if (o == null) continue;
        let s = W(e);
        if (s.length === 0) continue;
        let c = i.commandExecutionStartedAtMsById?.[e.id];
        t.push({
          chatTitle: n.title,
          command: s,
          ...(c == null ? {} : { commandExecutionStartedAtMs: c }),
          conversationId: n.id,
          cwd: e.cwd ?? n.cwd,
          hostId: n.hostId,
          id: `${n.id}:${i.turnId ?? r}:${e.id}`,
          itemId: e.id,
          osPid: null,
          processId: e.processId,
          source: o.source,
          startedAtMs: c ?? i.firstTurnWorkItemStartedAtMs ?? i.turnStartedAtMs ?? null,
          stopAction: o.stopAction,
          turnId: i.turnId,
        });
      }
    }
  }
  return t;
}
function ze(e, t) {
  let n = new Map(t.map((e) => [e.id, e]));
  return e.map((e) => {
    let t = n.get(e.conversationId);
    return {
      chatTitle: t?.title ?? e.chatTitle,
      command: e.command,
      commandExecutionStartedAtMs: e.startedAtMs,
      conversationId: e.conversationId,
      cwd: e.cwd ?? t?.cwd ?? null,
      hostId: t?.hostId ?? `local`,
      id: e.id,
      itemId: e.itemId,
      osPid: e.osPid,
      processId: e.processId,
      source: `restored-process`,
      startedAtMs: e.startedAtMs,
      stopAction: `kill-child-process`,
      turnId: e.turnId,
    };
  });
}
function Be(e, t) {
  let n = new Map(t.map((e) => [e.id, e]));
  return [
    ...e.map((e) => {
      let t = n.get(e.id);
      return t == null
        ? e
        : (n.delete(e.id),
          e.source === `restored-process`
            ? t
            : {
                ...e,
                chatTitle: e.chatTitle ?? t.chatTitle,
                ...(t.commandExecutionStartedAtMs == null
                  ? {}
                  : { commandExecutionStartedAtMs: t.commandExecutionStartedAtMs }),
                cwd: e.cwd ?? t.cwd,
                osPid: e.osPid ?? t.osPid,
                processId: e.processId ?? t.processId,
                startedAtMs: t.commandExecutionStartedAtMs ?? e.startedAtMs ?? t.startedAtMs,
              });
    }),
    ...n.values(),
  ];
}
function K(e, t) {
  return e.id === t.id
    ? !0
    : e.command === t.command &&
        e.conversationId === t.conversationId &&
        e.cwd === t.cwd &&
        e.hostId === t.hostId &&
        e.stopAction === t.stopAction &&
        e.turnId === t.turnId;
}
function q(e, t, n = Date.now(), { matchCommandStartTime: r = !1 } = {}) {
  if (t == null) return null;
  let i = r && e.commandExecutionStartedAtMs != null ? Ie : null;
  if (e.osPid != null) {
    let n = J(e.osPid, t);
    if (n != null && Ke(X(e.command), n.command)) return n;
  }
  let a = Ue(e.processId);
  if (a != null) {
    if (e.source !== `restored-process` && i == null) {
      let e = J(a, t);
      if (e != null) return e;
    }
    let r = Ge(e, Ve(a, t), n, i);
    if (r != null) return J(r.pid, t);
  }
  let o = Ge(e, t, n, i);
  return o == null ? null : J(o.pid, t);
}
function Ve(e, t) {
  let n = new Map(t.map((e) => [e.pid, e]));
  return t.filter((t) => We(t.pid, e, n));
}
function He(e, t, n) {
  return t.interruptedCommandExecutionItemIds?.includes(e.id)
    ? null
    : e.status === `inProgress`
      ? n
        ? { source: `active-turn`, stopAction: `interrupt-conversation` }
        : { source: `background-terminal`, stopAction: `kill-child-process` }
      : e.status === `completed`
        ? { source: `restored-process`, stopAction: `kill-child-process` }
        : null;
}
function Ue(e) {
  if (e == null || !/^\d+$/u.test(e)) return null;
  let t = Number(e);
  return Number.isSafeInteger(t) ? t : null;
}
function J(e, t) {
  let n = new Map(t.map((e) => [e.pid, e])),
    r = n.get(e);
  if (r == null) return null;
  let i = 0,
    a = !1,
    o = 0,
    s = !1;
  for (let r of t)
    We(r.pid, e, n) &&
      (r.cpuPercent != null && Number.isFinite(r.cpuPercent) && ((i += r.cpuPercent), (a = !0)),
      r.rssKb != null && Number.isFinite(r.rssKb) && ((o += r.rssKb), (s = !0)));
  return {
    ageSeconds: r.ageSeconds,
    command: r.command,
    cpuPercent: a ? i : null,
    pid: e,
    rssKb: s ? o : null,
  };
}
function We(e, t, n) {
  let r = e,
    i = new Set();
  for (; !i.has(r); ) {
    if (r === t) return !0;
    i.add(r);
    let e = n.get(r);
    if (e == null) return !1;
    r = e.parentPid;
  }
  return !1;
}
function Ge(e, t, n, r) {
  let i = X(e.command);
  if (i.length === 0) return null;
  let a = e.startedAtMs == null ? null : Math.max(0, Math.floor((n - e.startedAtMs) / 1e3));
  return (
    t
      .filter(
        (e) =>
          Ke(i, e.command) &&
          (a == null || r == null || e.ageSeconds == null || Y(e.ageSeconds, a) <= r),
      )
      .sort((e, t) => Je(e, t, a))[0] ?? null
  );
}
function Ke(e, t) {
  let n = X(t);
  return (
    n === e ||
    n.startsWith(`${e} `) ||
    (n.includes(` `) && e.startsWith(`${n} `)) ||
    n.endsWith(` ${e}`) ||
    n.includes(` ${e} `) ||
    qe(e, n)
  );
}
function qe(e, t) {
  let n = e.split(/\s+/u),
    r = t.split(/\s+/u);
  return n.length === r.length
    ? n.every((e, t) => {
        let n = r[t];
        return n == null ? !1 : e === n ? !0 : t === 0 ? !1 : Z(e) === Z(n);
      })
    : !1;
}
function Je(e, t, n) {
  if (n != null) {
    let r = Y(e.ageSeconds, n),
      i = Y(t.ageSeconds, n);
    if (r !== i) return r - i;
  }
  return e.depth === t.depth ? e.pid - t.pid : e.depth - t.depth;
}
function Y(e, t) {
  return e == null ? 2 ** 53 - 1 : Math.abs(e - t);
}
function X(e) {
  let t = e.trim();
  if (t.length === 0) return ``;
  let n = /^"([^"]+)"(.*)$/u.exec(t);
  if (n != null) return `${Z(n[1])}${n[2]}`.trim().toLowerCase();
  let r = /^(\S+)(.*)$/u.exec(t);
  return r == null ? t.toLowerCase() : `${Z(r[1])}${r[2]}`.trim().toLowerCase();
}
function Z(e) {
  let t = e.replaceAll(`\\`, `/`),
    n = t.lastIndexOf(`/`);
  return (n < 0 ? t : t.slice(n + 1)).replace(/\.(?:bat|cmd|com|exe)$/iu, ``);
}
var Q = s(d, () => new Map());
function Ye(e, t, n) {
  e.set(Q, Qe(e.get(Q), t, n));
}
function Xe(e, t) {
  e.set(Q, $e(e.get(Q), t));
}
function Ze(e) {
  e.set(Q, et(e.get(Q)));
}
function Qe(e, t, n) {
  return new Map(e).set(t, n);
}
function $e(e, t) {
  let n = new Map(e);
  return (n.delete(t), n);
}
function et(e) {
  return new Map(Array.from(e).filter(([, e]) => e.status !== `stopped`));
}
var tt = e(Te(), 1),
  nt = f([]);
function rt(e) {
  let t = (0, B.c)(16),
    n = D(nt),
    r;
  t[0] === e.dependencies
    ? (r = t[1])
    : ((r = e.dependencies ? e.dependencies.map(st).join(`|`) : ``),
      (t[0] = e.dependencies),
      (t[1] = r));
  let i = r,
    a;
  t[2] !== e || t[3] !== n
    ? ((a = () => {
        n((t) => {
          let n = !1,
            r = [...t].map((t) => (t.id === e.id ? ((n = !0), e) : t));
          return (n || r.push(e), (0, tt.default)(r.filter(ot), [at, it]));
        });
      }),
      (t[2] = e),
      (t[3] = n),
      (t[4] = a))
    : (a = t[4]);
  let o = (0, R.useEffectEvent)(a),
    s;
  t[5] === o
    ? (s = t[6])
    : ((s = () => {
        o();
      }),
      (t[5] = o),
      (t[6] = s));
  let c;
  (t[7] !== i || t[8] !== e.enabled || t[9] !== e.id || t[10] !== e.order
    ? ((c = [e.id, e.enabled, e.order, i]),
      (t[7] = i),
      (t[8] = e.enabled),
      (t[9] = e.id),
      (t[10] = e.order),
      (t[11] = c))
    : (c = t[11]),
    (0, R.useEffect)(s, c));
  let l, u;
  (t[12] !== e.id || t[13] !== n
    ? ((l = () => () => {
        n((t) => t.filter((t) => t.id !== e.id));
      }),
      (u = [e.id, n]),
      (t[12] = e.id),
      (t[13] = n),
      (t[14] = l),
      (t[15] = u))
    : ((l = t[14]), (u = t[15])),
    (0, R.useEffect)(l, u));
}
function it(e) {
  return e.id;
}
function at(e) {
  return e.order ?? 0;
}
function ot(e) {
  return e.enabled !== !1;
}
function st(e) {
  return ct(e);
}
function ct(e) {
  return e == null ? `` : String(e);
}
function lt(e) {
  let t = (0, B.c)(7),
    { conversationSource: n, diffSource: r, orchestrationId: i, isDefault: a } = e,
    o = n === void 0 ? null : n,
    s = r === void 0 ? null : r,
    c = l(P),
    u,
    d;
  return (
    t[0] !== o || t[1] !== s || t[2] !== a || t[3] !== i || t[4] !== c
      ? ((u = () =>
          ce(c, { conversationSource: o, diffSource: s }, { orchestrationId: i, isDefault: a })),
        (d = [o, s, a, i, c]),
        (t[0] = o),
        (t[1] = s),
        (t[2] = a),
        (t[3] = i),
        (t[4] = c),
        (t[5] = u),
        (t[6] = d))
      : ((u = t[5]), (d = t[6])),
    (0, R.useEffect)(u, d),
    null
  );
}
function ut(e, t) {
  return (0, R.useRef)(null);
}
var dt = a(P, ({ get: e, scope: t }) => (t.value.routeKind === `local-thread` ? e(le) : null));
function ft(e) {
  let t = (0, B.c)(7),
    n;
  t[0] === e ? (n = t[1]) : ((n = e === void 0 ? {} : e), (t[0] = e), (t[1] = n));
  let { conversationId: r, hostId: i } = n,
    a = l(P),
    o = u(dt),
    s = a.value.routeKind === `local-thread` ? a.value.conversationId : null,
    c = r ?? s,
    d = r == null ? o : i,
    f,
    p;
  return (
    t[2] !== c || t[3] !== d || t[4] !== a
      ? ((f = () => {
          if (!(c == null || d == null))
            return (
              S(a, c, d),
              x(`set-active-conversation`, { hostId: d, conversationId: c, active: !0 }),
              () => {
                (S(a, c, null),
                  x(`set-active-conversation`, { hostId: d, conversationId: c, active: !1 }));
              }
            );
        }),
        (p = [c, d, a]),
        (t[2] = c),
        (t[3] = d),
        (t[4] = a),
        (t[5] = f),
        (t[6] = p))
      : ((f = t[5]), (p = t[6])),
    (0, R.useLayoutEffect)(f, p),
    null
  );
}
function pt({
  hasConversation: e,
  hostId: t,
  latestTurnId: n,
  latestTurnStatus: r,
  pendingRequestType: i,
  resumeState: a,
}) {
  return e
    ? t == null || !we(t)
      ? { isEligible: !1, reason: `unsupported_host` }
      : a === `resuming` || (a !== `resumed` && n == null)
        ? { isEligible: !1, reason: `resuming` }
        : i === `userInput`
          ? { isEligible: !1, reason: `waiting_on_user_input` }
          : i === `approval` || i === `mcpServerElicitation`
            ? { isEligible: !1, reason: `waiting_on_approval` }
            : i == null
              ? r == null
                ? { isEligible: !1, reason: `missing_turn` }
                : r === `inProgress`
                  ? { isEligible: !1, reason: `turn_in_progress` }
                  : { isEligible: !0, reason: null }
              : { isEligible: !1, reason: `pending_request` }
    : { isEligible: !1, reason: `missing_conversation` };
}
var mt = c(d, (e, { get: t }) => t(m, e)?.type ?? null),
  ht = c(d, (e, { get: t }) =>
    pt({
      hasConversation: t(_, e),
      hostId: t(p, e),
      latestTurnId: t(y, e),
      latestTurnStatus: t(v, e),
      pendingRequestType: t(mt, e),
      resumeState: t(w, e),
    }),
  ),
  gt = de(),
  _t = fe(),
  vt = me();
function yt(e) {
  let t = (0, B.c)(30),
    { output: n } = e,
    r = ve(_e()),
    i = N(O.lightChromeTheme),
    a = N(O.darkChromeTheme),
    o = N(O.codeFontSize),
    s,
    c,
    l,
    u;
  if (t[0] !== o || t[1] !== a || t[2] !== i || t[3] !== r) {
    s = r === `light` ? ie(i, `light`) : ie(a, `dark`);
    let e = s.fonts.code?.trim() || `ui-monospace, "SFMono-Regular", Menlo, Consolas, monospace`;
    ((l = o ?? 12),
      (c = pe(e)),
      (u = e.split(`,`)[0]?.trim() ?? ``),
      (t[0] = o),
      (t[1] = a),
      (t[2] = i),
      (t[3] = r),
      (t[4] = s),
      (t[5] = c),
      (t[6] = l),
      (t[7] = u));
  } else ((s = t[4]), (c = t[5]), (l = t[6]), (u = t[7]));
  let d = u,
    f = (0, R.useRef)(null),
    p = (0, R.useRef)(null),
    m = (0, R.useRef)(null),
    h = (0, R.useRef)(null),
    g = (0, R.useRef)(``),
    _,
    v;
  (t[8] !== s || t[9] !== r
    ? ((_ = () => {
        let e = f.current;
        e != null && ae(e, s, r);
      }),
      (v = [s, r]),
      (t[8] = s),
      (t[9] = r),
      (t[10] = _),
      (t[11] = v))
    : ((_ = t[10]), (v = t[11])),
    (0, R.useLayoutEffect)(_, v));
  let y;
  t[12] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((y = () => {
        let e = m.current,
          t = f.current;
        e == null ||
          t == null ||
          ((e.options.theme = F(t)), e.rows > 0 && e.refresh(0, e.rows - 1));
      }),
      (t[12] = y))
    : (y = t[12]);
  let b;
  (t[13] !== s || t[14] !== r ? ((b = [s, r]), (t[13] = s), (t[14] = r), (t[15] = b)) : (b = t[15]),
    (0, R.useEffect)(y, b));
  let x, S;
  (t[16] !== c || t[17] !== l || t[18] !== d
    ? ((x = () => {
        let e = m.current;
        if (e == null) return;
        let t = !1,
          n = null;
        return (
          (async () => {
            (await ge(d, l),
              !t &&
                ((e.options.fontFamily = c),
                (e.options.fontSize = l),
                (n = requestAnimationFrame(() => {
                  h.current?.fit();
                }))));
          })(),
          () => {
            ((t = !0), n != null && cancelAnimationFrame(n));
          }
        );
      }),
      (S = [c, l, d]),
      (t[16] = c),
      (t[17] = l),
      (t[18] = d),
      (t[19] = x),
      (t[20] = S))
    : ((x = t[19]), (S = t[20])),
    (0, R.useEffect)(x, S));
  let C, w;
  (t[21] !== c || t[22] !== l
    ? ((C = () => {
        let e = p.current,
          t = f.current;
        if (e == null || t == null) return;
        let n = !1,
          r = null,
          i = new vt.Terminal({
            allowTransparency: !0,
            cursorStyle: `bar`,
            disableStdin: !0,
            fontSize: l,
            fontFamily: c,
            letterSpacing: 0,
            lineHeight: 1.2,
            theme: F(t),
          }),
          a = new gt.FitAddon(),
          o = new _t.WebLinksAddon(bt);
        ((m.current = i),
          (h.current = a),
          i.loadAddon(a),
          i.loadAddon(o),
          i.open(e),
          a.fit(),
          g.current.length > 0 &&
            (i.write(I(g.current)),
            (r ??= requestAnimationFrame(() => {
              ((r = null), i.scrollToBottom());
            }))));
        let s = new ResizeObserver(() => {
          n || a.fit();
        });
        return (
          s.observe(e),
          () => {
            ((n = !0),
              r != null && (cancelAnimationFrame(r), (r = null)),
              s.disconnect(),
              (h.current = null),
              (m.current = null),
              i.dispose());
          }
        );
      }),
      (w = [c, l]),
      (t[21] = c),
      (t[22] = l),
      (t[23] = C),
      (t[24] = w))
    : ((C = t[23]), (w = t[24])),
    (0, R.useEffect)(C, w));
  let T, E;
  (t[25] === n
    ? ((T = t[26]), (E = t[27]))
    : ((T = () => {
        let e = m.current;
        if (e == null) {
          g.current = n;
          return;
        }
        let t = g.current,
          r = he(e);
        if (n.startsWith(t)) {
          let r = n.slice(t.length);
          r.length > 0 && e.write(I(r));
        } else (e.reset(), n.length > 0 && e.write(I(n)));
        if (((g.current = n), !r)) return;
        let i = requestAnimationFrame(() => {
          e.scrollToBottom();
        });
        return () => {
          cancelAnimationFrame(i);
        };
      }),
      (E = [n]),
      (t[25] = n),
      (t[26] = T),
      (t[27] = E)),
    (0, R.useEffect)(T, E));
  let D;
  t[28] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((D = {
        backgroundColor: `var(--vscode-terminal-background)`,
        color: `var(--vscode-terminal-foreground)`,
      }),
      (t[28] = D))
    : (D = t[28]);
  let k;
  return (
    t[29] === Symbol.for(`react.memo_cache_sentinel`)
      ? ((k = (0, z.jsx)(`div`, {
          "data-codex-xterm": !0,
          ref: f,
          className: `relative flex h-full w-full flex-col app-theme`,
          style: D,
          children: (0, z.jsx)(`div`, {
            className: `flex-1 overflow-hidden py-3 pl-3 tracking-normal`,
            children: (0, z.jsx)(`div`, { className: `h-full w-full overflow-hidden`, ref: p }),
          }),
        })),
        (t[29] = k))
      : (k = t[29]),
    k
  );
}
function bt(e, t) {
  t && (e.preventDefault?.(), E.dispatchMessage(`open-in-browser`, { url: t }));
}
function xt(e) {
  let t = (0, B.c)(5),
    { conversationId: n, terminalId: r } = e,
    a = i(g, n),
    o;
  t[0] !== r || t[1] !== a ? ((o = wt(a, r)), (t[0] = r), (t[1] = a), (t[2] = o)) : (o = t[2]);
  let s = o,
    c = St(r),
    l = s?.aggregatedOutput ?? c?.buffer ?? ``,
    u;
  return (
    t[3] === l
      ? (u = t[4])
      : ((u = (0, z.jsx)(`div`, {
          className: `h-full min-h-0 bg-token-main-surface-primary`,
          children:
            l.length > 0
              ? (0, z.jsx)(yt, { output: l })
              : (0, z.jsx)(`div`, {
                  className: `font-vscode-editor text-size-code-sm p-4 text-token-description-foreground`,
                  children: (0, z.jsx)(A, {
                    id: `codex.localConversation.backgroundTerminalTab.noOutput`,
                    defaultMessage: `No output yet`,
                    description: `Placeholder shown in a background terminal output tab before any terminal output is available`,
                  }),
                }),
        })),
        (t[3] = l),
        (t[4] = u)),
    u
  );
}
function St(e) {
  let t = (0, B.c)(3),
    n,
    r;
  return (
    t[0] === e
      ? ((n = t[1]), (r = t[2]))
      : ((n = (t) => h.subscribeToSessionSnapshot(e, t)),
        (r = () => h.getSnapshot(e)),
        (t[0] = e),
        (t[1] = n),
        (t[2] = r)),
    (0, R.useSyncExternalStore)(n, r, Ct)
  );
}
function Ct() {
  return null;
}
function wt(e, t) {
  if (e == null) return null;
  for (let n = e.length - 1; n >= 0; --n) {
    let r = e[n];
    if (r != null) {
      for (let e of r.items) if (e.type === `commandExecution` && e.id === t) return e;
    }
  }
  return null;
}
function Tt({ scope: e, backgroundTerminal: t, conversationId: n, fallbackTitle: r }) {
  oe.openTab(e, xt, {
    icon: (0, z.jsx)(ue, { className: `icon-xs shrink-0` }),
    props: { conversationId: n, terminalId: t.id },
    id: `background-terminal:${n}:${t.id}`,
    title: t.command.length > 0 ? t.command : r,
  });
}
var Et = 3,
  Dt = 1e3;
function Ot(e, t, n) {
  let r = [],
    i = new Set();
  for (let a of e) {
    let e = q(a, t, n);
    if (e == null) continue;
    let o = Nt({ metrics: e, process: a }, n);
    (o != null && o < Et) || i.has(e.pid) || (i.add(e.pid), r.push({ metrics: e, process: a }));
  }
  return r;
}
function kt(e, t) {
  if (t.size === 0) return e;
  let n = e.slice(),
    r = [];
  for (let i of t.values())
    Lt(e, i.row) || r.push({ row: i.row, rowIndex: i.rowIndex ?? n.length });
  r.sort((e, t) => e.rowIndex - t.rowIndex);
  for (let e of r) n.splice(Math.min(e.rowIndex, n.length), 0, e.row);
  return n;
}
function At(e, t, n) {
  if (t.size === 0) return t;
  let r = new Map(e.map((e) => [e.process.id, e])),
    i = new Map();
  for (let [a, o] of t) Pt(o, r.get(a) ?? It(e, o.row), n) || i.set(a, o);
  return i;
}
function jt(e, t) {
  return Mt(e, t)?.sortRow ?? e;
}
function Mt(e, t) {
  let n = t.get(e.process.id);
  if (n != null) return n;
  for (let n of t.values()) if (K(e.process, n.row.process)) return n;
}
function Nt(e, t) {
  return e.metrics?.ageSeconds == null
    ? e.process.startedAtMs == null
      ? null
      : Math.max(0, Math.floor((t - e.process.startedAtMs) / 1e3))
    : e.metrics.ageSeconds;
}
function Pt(e, t, n) {
  return e.status === `starting`
    ? e.expiresAtMs != null && e.expiresAtMs <= n
      ? !0
      : t == null
        ? !1
        : Ft(e, t, n)
    : !1;
}
function Ft(e, t, n) {
  let r = e.row.process.startedAtMs,
    i = t.metrics?.ageSeconds;
  return r == null || i == null ? !0 : n - i * 1e3 >= r - Dt;
}
function It(e, t) {
  return e.find((e) => K(e.process, t.process)) ?? null;
}
function Lt(e, t) {
  return It(e, t) != null;
}
var Rt = s(d, () => null),
  zt = 0;
function Bt(e, t) {
  ((zt += 1), e.set(Rt, { processId: t, requestId: zt }));
}
var Vt = new Set(),
  $ = o(d, (e) => Vt);
function Ht(e, t, n) {
  e.set($, t, (e) => new Set(e).add(n));
}
export {
  Fe as A,
  Le as C,
  Be as D,
  ze as E,
  H as F,
  U as I,
  V as L,
  Me as M,
  je as N,
  K as O,
  Ae as P,
  Ee as R,
  Ye as S,
  Re as T,
  nt as _,
  Mt as a,
  Ze as b,
  Nt as c,
  Pt as d,
  Tt as f,
  lt as g,
  ut as h,
  Rt as i,
  Ne as j,
  W as k,
  kt as l,
  ft as m,
  Ht as n,
  Ot as o,
  ht as p,
  Bt as r,
  jt as s,
  $ as t,
  At as u,
  rt as v,
  q as w,
  Q as x,
  Xe as y,
};
//# sourceMappingURL=interrupted-turn-state.js.map
