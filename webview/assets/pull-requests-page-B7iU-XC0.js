import { s as e } from "./chunk-Bj-mKKzh.js";
import { S as t, zn as n } from "./src-C.js";
import { $s as r, Ps as i } from "./app-server-manager-signals.js";
import { n as a, t as o } from "./jsx-runtime.js";
import "./shiki-highlight-provider-gate.js";
import { t as s } from "./clsx.js";
import "./react-dom.js";
import "./Combination.js";
import "./core.esm.js";
import "./katex-1.js";
import "./marked.esm-BR-H6018.js";
import "./pdf-2.js";
import {
  G as c,
  L as l,
  S as u,
  U as d,
  V as f,
  X as p,
  Y as m,
  Z as h,
  _ as g,
  a as _,
  p as v,
  u as y,
  wt as b,
  y as x,
} from "./setting-storage.js";
import { _ as S, m as C } from "./chunk-8.js";
import "./known-app-icon.js";
import "./branch.js";
import { t as w } from "./github-mark.js";
import "./google-drive.js";
import "./notion.js";
import "./product-logger.js";
import "./rpc.js";
import "./statsig.js";
import "./request.js";
import { c as T } from "./app-server-manager-hooks-DfDI-9lO.js";
import "./use-global-state.js";
import "./invalidate-queries-and-broadcast.js";
import { u as E } from "./thread-context-inputs.js";
import "./config-queries.js";
import "./remote-connection-visibility.js";
import "./sidebar-signals.js";
import "./use-auth.js";
import "./codex-api-B.js";
import "./_baseEach.js";
import "./_baseOrderBy.js";
import "./use-debounced-value.js";
import "./apps-queries.js";
import "./startCase-Bo0-xpFk.js";
import "./experimental-features-queries.js";
import "./use-is-dark.js";
import "./connector-logo.js";
import { r as D } from "./toast-signal.js";
import "./window-zoom-context.js";
import "./tooltip.js";
import { t as O } from "./route-scope.js";
import "./prompt-text.js";
import "./mention-item.js";
import "./use-collaboration-mode.js";
import "./use-model-settings-DhE-8qP1.js";
import { t as k } from "./button.js";
import "./reduced-motion-preference.js";
import { t as A } from "./spinner.js";
import "./use-codex-home.js";
import "./os-info.js";
import "./use-os-info.js";
import "./link-external.js";
import "./open-config-toml-button.js";
import "./open-workspace-file.js";
import "./send-open-file-request.js";
import "./mime-types.js";
import "./model-queries.js";
import "./use-webview-execution-target.js";
import "./remote-projects.js";
import "./collaboration-mode-queries.js";
import "./thread-detail-level.js";
import "./permissions-mode-defaults.js";
import "./use-permissions-mode.js";
import "./proxy.js";
import { _ as j } from "./app-shell-state.js";
import { t as M } from "./app-shell.js";
import "./use-service-tier-settings.js";
import "./apps.js";
import "./x.js";
import "./projectless-thread.js";
import "./plugin-install-store-CS-dd4DH.js";
import "./use-platform.js";
import "./use-is-plugins-enabled.js";
import "./browser-sidebar-availability.js";
import "./use-plugins.js";
import "./check-circle-filled.js";
import "./diff-view-mode.js";
import "./browser-sidebar-manager.js";
import "./app-shell-tab-controller.js";
import "./review-navigation-model.js";
import "./focus-composer.js";
import "./file-diff.js";
import "./sumBy.js";
import "./thread-context.js";
import "./git-current-branch-query.js";
import "./terminal.js";
import "./info.js";
import "./error-boundary.js";
import "./download.js";
import { t as N } from "./with-window.js";
import "./use-resolved-theme-variant.js";
import "./window-app-action-helpers.js";
import "./dialog-layout.js";
import "./build-worktree-label-from-input.js";
import "./local-projects.js";
import "./pending-worktree-store.js";
import "./run-command.js";
import "./modal-controller-state.js";
import "./open-project-setup-dialog.js";
import "./folder.js";
import "./use-stable-callback.js";
import "./electron-menu-shortcuts.js";
import "./local-task-row-signals.js";
import { u as P } from "./sidebar-project-group-signals.js";
import { p as F } from "./sidebar-project-groups-Bs-6klyU.js";
import "./_defineProperty.js";
import "./use-register-command.js";
import "./homepage-logo.js";
import "./loading-page.js";
import "./sortable.esm.js";
import "./use-resize-observer.js";
import "./check-md.js";
import "./chevron-right.js";
import { t as ee } from "./context-menu.js";
import "./command-keybindings.js";
import "./expand.js";
import "./keyboard-modifier-state.js";
import "./arrow-left.js";
import "./use-window-controls-safe-area.js";
import "./app-shell-bottom-panel-scroll-sync.js";
import "./checkbox.js";
import "./minus.js";
import "./badge.js";
import "./bug-B5-GWt65.js";
import "./chat.js";
import "./clock.js";
import "./search.js";
import "./skills.js";
import "./mcp-capability-signals.js";
import { r as I, t as L } from "./dropdown.js";
import "./globe.js";
import "./three-dots.js";
import { t as R } from "./chevron.js";
import "./use-relative-now.js";
import "./format-relative-date-time.js";
import { t as te } from "./copy-to-clipboard.js";
import "./warning.js";
import { r as z } from "./pull-request-status-Ceb-oAPn.js";
import "./markdown.js";
import "./external-markdown-link.js";
import "./use-native-apps.electron.js";
import "./get-file-icon.js";
import "./code.js";
import "./json.js";
import "./image-square.js";
import "./notebook.js";
import "./settings.cog.js";
import "./mcp.js";
import "./chrome-ZI-wJE1V.js";
import "./codex.js";
import "./openai-blossom.js";
import "./pencil.js";
import "./filesystem-media-src.js";
import "./inline-mentions.js";
import "./workspace-file-context-menu.js";
import "./use-skills.js";
import "./code-snippet.js";
import "./copy.js";
import "./copy-button.js";
import "./plus.js";
import "./image-preview-dialog.js";
import "./pull-request-readonly-comment.js";
import "./pull-request-open.js";
import "./automation-queries.js";
import "./get-attached-heartbeat-automation-for-thread.js";
import "./use-is-background-subagents-enabled-B.js";
import "./local-environments.js";
import "./local-environment-selection.js";
import "./arrow-up.js";
import { t as B } from "./bullet-separator.js";
import "./check-lg.js";
import "./sortBy.js";
import "./prompt-editor.js";
/* empty css                    */ import "./composer-top-menu-chrome.js";
import "./toggle.js";
import { s as ne } from "./review-header-toolbar.js";
import "./at-mention-list-1.js";
import { n as V } from "./diff-stats.js";
import "./connected-apps.js";
import "./use-composer-controller.js";
import "./undo.js";
import "./regenerate.js";
import "./format-skill-scope.js";
import "./list-navigation.js";
import "./right-panel-composer-overlay-scroll-reserve.js";
import "./mcp-capability-client.js";
import "./use-workspace-file-search.js";
import { t as H } from "./large-empty-state-C2F-IvSK.js";
import "./pull-request-code-review-comments.js";
import "./file-tree-search-input.js";
import "./codex-analytics-attribution.js";
import "./use-target-apps.js";
import "./diff-unified.js";
import "./use-diff-annotations.js";
import "./rich-preview-primitives.js";
import "./markdown-surface.js";
import "./anchored-autocomplete-overlay.js";
import "./use-autocomplete-overlay-placement.js";
import "./folders-DmG-RYpY.js";
import "./summary-panel-row.js";
import "./middle-truncated-text.js";
import "./use-measured-text-collapse.js";
import "./document-search.js";
import "./automation-schedule.js";
import { t as re } from "./use-is-pull-request-merge-helper-enabled.js";
import { t as ie } from "./selectable-list-row.js";
import { n as U, t as ae } from "./sectioned-page.js";
import { t as W } from "./segmented-toggle.js";
import {
  a as oe,
  c as se,
  d as G,
  f as K,
  i as ce,
  l as le,
  m as q,
  n as ue,
  o as J,
  p as Y,
  r as X,
  s as de,
  t as fe,
  u as pe,
} from "./pull-request-board-model.js";
var Z = b(),
  Q = e(a(), 1),
  $ = o(),
  me = f({
    draft: {
      id: `pullRequestsPage.column.draft`,
      defaultMessage: `Draft`,
      description: `Board column label for draft pull requests`,
    },
    merged: {
      id: `pullRequestsPage.column.merged`,
      defaultMessage: `Merged`,
      description: `Board column label for merged pull requests`,
    },
    ready: {
      id: `pullRequestsPage.column.ready`,
      defaultMessage: `Ready`,
      description: `Board column label for ready pull requests`,
    },
    unmetChecks: {
      id: `pullRequestsPage.column.unmetChecks`,
      defaultMessage: `Unmet checks`,
      description: `Board section label for pull requests with unmet checks`,
    },
  }),
  he = f({
    allRepos: {
      id: `pullRequestsPage.repo.allRepos`,
      defaultMessage: `All repos`,
      description: `Dropdown option label that shows pull requests from all available repositories`,
    },
  }),
  ge = f({
    copyUrl: {
      id: `pullRequestsPage.card.contextMenu.copyUrl`,
      defaultMessage: `Copy URL`,
      description: `Context menu item that copies a pull request URL`,
    },
    copyUrlSuccess: {
      id: `pullRequestsPage.card.contextMenu.copyUrlSuccess`,
      defaultMessage: `Copied pull request URL`,
      description: `Toast shown after copying a pull request URL`,
    },
    copyUrlError: {
      id: `pullRequestsPage.card.contextMenu.copyUrlError`,
      defaultMessage: `Failed to copy pull request URL`,
      description: `Toast shown when copying a pull request URL fails`,
    },
    openInBrowser: {
      id: `pullRequestsPage.card.contextMenu.openInBrowser`,
      defaultMessage: `Open in browser`,
      description: `Context menu item that opens a pull request in the browser`,
    },
  }),
  _e = [];
function ve(e) {
  let r = (0, Z.c)(38),
    {
      selectedFilterView: i,
      repoOption: a,
      repoOptions: o,
      selectedPullRequestNumber: s,
      selectedPullRequestUrl: l,
      selectedRepoKey: u,
      onSelectPullRequest: f,
      onSelectFilterView: p,
      onSelectRepo: m,
    } = e,
    h = c(),
    { data: y } = T(),
    b;
  r[0] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((b = { merged: !0 }), (r[0] = b))
    : (b = r[0]);
  let [x, S] = (0, Q.useState)(b),
    [C, w] = (0, Q.useState)(null),
    E;
  r[1] === i
    ? (E = r[2])
    : ((E = oe({ searchQuery: ce(i), selectedFilterView: i })), (r[1] = i), (r[2] = E));
  let D = E,
    O = u === `all`,
    k;
  r[3] !== D ||
  r[4] !== O ||
  r[5] !== a?.cwd ||
  r[6] !== a?.hostId ||
  r[7] !== a?.repo ||
  r[8] !== o
    ? ((k = O
        ? { repos: o.map(be), searchQuery: D }
        : { cwd: n(a?.cwd ?? `/`), hostId: a?.hostId, repo: a?.repo ?? null, searchQuery: D }),
      (r[3] = D),
      (r[4] = O),
      (r[5] = a?.cwd),
      (r[6] = a?.hostId),
      (r[7] = a?.repo),
      (r[8] = o),
      (r[9] = k))
    : (k = r[9]);
  let j = O ? o.length > 0 : a != null,
    N;
  r[10] === j
    ? (N = r[11])
    : ((N = { enabled: j, intervalMs: g.THIRTY_SECONDS, staleTime: g.THIRTY_SECONDS }),
      (r[10] = j),
      (r[11] = N));
  let P;
  r[12] !== k || r[13] !== N
    ? ((P = { source: `pull_request_board`, params: k, queryConfig: N }),
      (r[12] = k),
      (r[13] = N),
      (r[14] = P))
    : (P = r[14]);
  let { data: F, isLoading: ee } = v(`gh-pr-board`, P),
    I = _(t.pullRequestMergeMethod),
    L = F?.status === `success` ? F.items : [],
    R = fe(L),
    te = R.find(ye)?.items[0] ?? null,
    z = de({ items: L, selectedPullRequestNumber: s, selectedPullRequestUrl: l }),
    B = `${u ?? ``}:${i}:${D ?? ``}`,
    ne = s != null || l != null;
  (0, Q.useEffect)(() => {
    te == null || ne || C === B || (w(B), f(te, { replace: !0 }));
  }, [B, C, te, ne, f]);
  let V;
  r[15] === h
    ? (V = r[16])
    : ((V = (e) => ({ id: je(e.key), title: h.formatMessage(Me(e.key)) })),
      (r[15] = h),
      (r[16] = V));
  let H = R.map(V),
    re = ue(y ?? []),
    ie = z == null ? _e : (re.get(z.headBranch.trim()) ?? _e),
    U;
  r[17] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((U = (0, $.jsx)(`div`, {
        className: `min-w-0 text-base font-medium text-token-foreground`,
        children: (0, $.jsx)(d, {
          id: `pullRequestsPage.title`,
          defaultMessage: `Pull requests`,
          description: `Header title for the pull requests page`,
        }),
      })),
      (r[17] = U))
    : (U = r[17]);
  let W;
  r[18] !== m || r[19] !== o || r[20] !== u
    ? ((W = (0, $.jsxs)(`div`, {
        className: `draggable flex w-full min-w-0 items-center justify-between gap-3 electron:h-toolbar extension:py-row-y`,
        children: [
          U,
          (0, $.jsx)(`div`, {
            className: `flex items-center gap-2`,
            children: (0, $.jsx)(De, { repoOptions: o, selectedRepoKey: u, onSelect: m }),
          }),
        ],
      })),
      (r[18] = m),
      (r[19] = o),
      (r[20] = u),
      (r[21] = W))
    : (W = r[21]);
  let se = W,
    G;
  r[22] !== B || r[23] !== f
    ? ((G = () => {
        (w(B), f(null));
      }),
      (r[22] = B),
      (r[23] = f),
      (r[24] = G))
    : (G = r[24]);
  let K;
  r[25] === se
    ? (K = r[26])
    : ((K = (0, $.jsx)(M.Header, { children: se })), (r[25] = se), (r[26] = K));
  let le = ae,
    q;
  r[27] === h
    ? (q = r[28])
    : ((q = h.formatMessage({
        id: `pullRequestsPage.sectionsNav`,
        defaultMessage: `Pull request sections`,
        description: `Accessible label for the pull request sections on the pull requests page`,
      })),
      (r[27] = h),
      (r[28] = q));
  let J;
  r[29] !== p || r[30] !== i
    ? ((J = (0, $.jsx)(Te, { selectedFilterView: i, onSelectFilterView: p })),
      (r[29] = p),
      (r[30] = i),
      (r[31] = J))
    : (J = r[31]);
  let Y =
      F?.status === `error`
        ? (0, $.jsx)(Se, {})
        : ee
          ? (0, $.jsx)(`div`, {
              className: `flex min-h-[240px] items-center justify-center`,
              children: (0, $.jsx)(A, {}),
            })
          : L.length === 0
            ? (0, $.jsx)(Ce, { isAllReposSelected: O, selectedFilterView: i })
            : R.map((e) =>
                (0, $.jsx)(
                  Ee,
                  {
                    collapsed: x[e.key] === !0,
                    column: e,
                    mergeMethod: I ?? `merge`,
                    selectedPullRequestNumber: z?.number ?? s,
                    selectedPullRequestUrl: z?.url ?? l,
                    repo: a?.repo ?? null,
                    showRepo: O,
                    onSelectPullRequest: f,
                    onToggleSection: () => {
                      S((t) => ({ ...t, [e.key]: t[e.key] !== !0 }));
                    },
                  },
                  e.key,
                ),
              ),
    X;
  return (
    r[32] !== le || r[33] !== H || r[34] !== q || r[35] !== J || r[36] !== Y
      ? ((X = (0, $.jsx)(le, {
          ariaLabel: q,
          className: `[--sectioned-page-leading-inset:0.75rem]`,
          contentInnerClassName: `flex flex-col gap-8 px-panel pb-panel`,
          header: J,
          sections: H,
          showNav: !1,
          children: Y,
        })),
        (r[32] = le),
        (r[33] = H),
        (r[34] = q),
        (r[35] = J),
        (r[36] = Y),
        (r[37] = X))
      : (X = r[37]),
    (0, $.jsxs)(`div`, {
      className: `flex h-full min-h-0 flex-col`,
      children: [
        (0, $.jsx)(xe, {
          hostId: z?.hostId ?? a?.hostId,
          item: z,
          relatedThreads: ie,
          repo: z?.repo ?? a?.repo ?? null,
          onClose: G,
        }),
        K,
        X,
      ],
    })
  );
}
function ye(e) {
  return e.items.length > 0;
}
function be(e) {
  return { cwd: n(e.cwd), hostId: e.hostId, repo: e.repo };
}
function xe(e) {
  let t = (0, Z.c)(10),
    { hostId: n, item: r, relatedThreads: i, repo: a, onClose: o } = e,
    s = p(O),
    c = r != null,
    l,
    u;
  if (
    (t[0] !== c || t[1] !== s
      ? ((l = () => (
          j(s, c),
          () => {
            j(s, !1);
          }
        )),
        (u = [c, s]),
        (t[0] = c),
        (t[1] = s),
        (t[2] = l),
        (t[3] = u))
      : ((l = t[2]), (u = t[3])),
    (0, Q.useLayoutEffect)(l, u),
    r == null)
  )
    return null;
  let d;
  return (
    t[4] !== n || t[5] !== r || t[6] !== o || t[7] !== i || t[8] !== a
      ? ((d = (0, $.jsx)(M.RightPanelOutlet, {
          children: (0, $.jsx)(pe, { hostId: n, item: r, relatedThreads: i, repo: a, onClose: o }),
        })),
        (t[4] = n),
        (t[5] = r),
        (t[6] = o),
        (t[7] = i),
        (t[8] = a),
        (t[9] = d))
      : (d = t[9]),
    d
  );
}
function Se() {
  let e = (0, Z.c)(7),
    t = c(),
    n;
  e[0] === t
    ? (n = e[1])
    : ((n = t.formatMessage({
        id: `pullRequestsPage.error.description`,
        defaultMessage: `Check your GitHub CLI auth and try again in a moment`,
        description: `Description for the pull requests page error state`,
      })),
      (e[0] = t),
      (e[1] = n));
  let r;
  e[2] === t
    ? (r = e[3])
    : ((r = t.formatMessage({
        id: `pullRequestsPage.error.title`,
        defaultMessage: `Couldn’t load pull requests`,
        description: `Title for the pull requests page error state`,
      })),
      (e[2] = t),
      (e[3] = r));
  let i;
  return (
    e[4] !== n || e[5] !== r
      ? ((i = (0, $.jsx)(`div`, {
          className: `flex h-full min-h-full items-center justify-center`,
          children: (0, $.jsx)(H, { description: n, title: r }),
        })),
        (e[4] = n),
        (e[5] = r),
        (e[6] = i))
      : (i = e[6]),
    i
  );
}
function Ce(e) {
  let t = (0, Z.c)(9),
    { isAllReposSelected: n, selectedFilterView: r } = e,
    i = c(),
    a;
  t[0] !== i || t[1] !== n || t[2] !== r
    ? ((a = we({ intl: i, isAllReposSelected: n, selectedFilterView: r })),
      (t[0] = i),
      (t[1] = n),
      (t[2] = r),
      (t[3] = a))
    : (a = t[3]);
  let o;
  t[4] === i
    ? (o = t[5])
    : ((o = i.formatMessage({
        id: `pullRequestsPage.empty.noPullRequests.title`,
        defaultMessage: `No pull requests yet`,
        description: `Title for the pull requests page empty state when the selected repo has no pull requests`,
      })),
      (t[4] = i),
      (t[5] = o));
  let s;
  return (
    t[6] !== a || t[7] !== o
      ? ((s = (0, $.jsx)(`div`, {
          className: `flex h-full min-h-full items-center justify-center`,
          children: (0, $.jsx)(H, { description: a, title: o }),
        })),
        (t[6] = a),
        (t[7] = o),
        (t[8] = s))
      : (s = t[8]),
    s
  );
}
function we({ intl: e, isAllReposSelected: t, selectedFilterView: n }) {
  switch (t ? `all` : n) {
    case `all`:
      return e.formatMessage({
        id: `pullRequestsPage.empty.noPullRequests.allReposDescription`,
        defaultMessage: `Pull requests across your GitHub repos will appear here`,
        description: `Description for the pull requests page empty state when all repos are selected and there are no pull requests`,
      });
    case `authored`:
      return e.formatMessage({
        id: `pullRequestsPage.empty.noPullRequests.description`,
        defaultMessage: `Your recent authored pull requests for this repo will appear here`,
        description: `Description for the pull requests page empty state when the selected repo has no pull requests`,
      });
    case `review`:
      return e.formatMessage({
        id: `pullRequestsPage.empty.noPullRequests.reviewDescription`,
        defaultMessage: `Pull requests requesting your review for this repo will appear here`,
        description: `Description for the pull requests page empty state when the review filter has no pull requests`,
      });
  }
}
function Te(e) {
  let t = (0, Z.c)(8),
    { selectedFilterView: n, onSelectFilterView: r } = e,
    i = c(),
    a;
  t[0] === i
    ? (a = t[1])
    : ((a = i.formatMessage({
        id: `pullRequestsPage.filter.ariaLabel`,
        defaultMessage: `Pull request views`,
        description: `Accessible label for the pull request board saved view toggle`,
      })),
      (t[0] = i),
      (t[1] = a));
  let o;
  t[2] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((o = {
        id: `authored`,
        label: (0, $.jsx)(d, {
          id: `pullRequestsPage.filter.authored`,
          defaultMessage: `Authored`,
          description: `Preset filter for pull requests authored by the user`,
        }),
      }),
      (t[2] = o))
    : (o = t[2]);
  let s;
  t[3] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((s = [
        o,
        {
          id: `review`,
          label: (0, $.jsx)(d, {
            id: `pullRequestsPage.filter.review`,
            defaultMessage: `Review`,
            description: `Preset filter for pull requests requesting a review from the user`,
          }),
        },
      ]),
      (t[3] = s))
    : (s = t[3]);
  let l;
  return (
    t[4] !== r || t[5] !== n || t[6] !== a
      ? ((l = (0, $.jsx)(`div`, {
          className: `flex items-center`,
          children: (0, $.jsx)(W, {
            ariaLabel: a,
            selectedId: n,
            size: `toolbar`,
            options: s,
            onSelect: r,
          }),
        })),
        (t[4] = r),
        (t[5] = n),
        (t[6] = a),
        (t[7] = l))
      : (l = t[7]),
    l
  );
}
function Ee(e) {
  let t = (0, Z.c)(31),
    {
      collapsed: n,
      column: r,
      mergeMethod: i,
      selectedPullRequestNumber: a,
      selectedPullRequestUrl: o,
      showRepo: c,
      onSelectPullRequest: l,
      onToggleSection: u,
      repo: d,
    } = e,
    f;
  t[0] === r.key ? (f = t[1]) : ((f = je(r.key)), (t[0] = r.key), (t[1] = f));
  let p = f,
    m = !n,
    h = `${p}-items`,
    g = n && `-rotate-90`,
    _;
  t[2] === g
    ? (_ = t[3])
    : ((_ = s(`icon-2xs shrink-0 transition-transform`, g)), (t[2] = g), (t[3] = _));
  let v;
  t[4] === _ ? (v = t[5]) : ((v = (0, $.jsx)(R, { className: _ })), (t[4] = _), (t[5] = v));
  let y;
  t[6] === r.key ? (y = t[7]) : ((y = Ae(r.key)), (t[6] = r.key), (t[7] = y));
  let b;
  t[8] === y ? (b = t[9]) : ((b = (0, $.jsx)(`span`, { children: y })), (t[8] = y), (t[9] = b));
  let x;
  t[10] !== u || t[11] !== m || t[12] !== h || t[13] !== v || t[14] !== b
    ? ((x = (0, $.jsxs)(`button`, {
        "aria-expanded": m,
        "aria-controls": h,
        className: `flex cursor-interaction items-center gap-2 text-left`,
        onClick: u,
        type: `button`,
        children: [v, b],
      })),
      (t[10] = u),
      (t[11] = m),
      (t[12] = h),
      (t[13] = v),
      (t[14] = b),
      (t[15] = x))
    : (x = t[15]);
  let S;
  t[16] !== n ||
  t[17] !== r.items ||
  t[18] !== r.key ||
  t[19] !== i ||
  t[20] !== l ||
  t[21] !== d ||
  t[22] !== p ||
  t[23] !== a ||
  t[24] !== o ||
  t[25] !== c
    ? ((S = n
        ? null
        : (0, $.jsx)(`div`, {
            className: `flex flex-col gap-1`,
            id: `${p}-items`,
            role: `list`,
            children: r.items.map((e) =>
              (0, $.jsx)(
                `div`,
                {
                  role: `listitem`,
                  children: (0, $.jsx)(Oe, {
                    item: e,
                    selected: e.url === o || (o == null && e.number === a),
                    mergeMethod: i,
                    onSelectPullRequest: l,
                    repo: e.repo ?? d,
                    showRepo: c,
                  }),
                },
                `${r.key}-${e.url}`,
              ),
            ),
          })),
      (t[16] = n),
      (t[17] = r.items),
      (t[18] = r.key),
      (t[19] = i),
      (t[20] = l),
      (t[21] = d),
      (t[22] = p),
      (t[23] = a),
      (t[24] = o),
      (t[25] = c),
      (t[26] = S))
    : (S = t[26]);
  let C;
  return (
    t[27] !== p || t[28] !== S || t[29] !== x
      ? ((C = (0, $.jsx)(U, { id: p, title: x, children: S })),
        (t[27] = p),
        (t[28] = S),
        (t[29] = x),
        (t[30] = C))
      : (C = t[30]),
    C
  );
}
function De(e) {
  let t = (0, Z.c)(37),
    { repoOptions: n, selectedRepoKey: r, onSelect: i } = e,
    a = c(),
    o = r === `all`,
    l;
  t[0] !== n || t[1] !== r
    ? ((l = n.find((e) => e.key === r) ?? n[0] ?? null), (t[0] = n), (t[1] = r), (t[2] = l))
    : (l = t[2]);
  let u = l,
    f;
  t[3] !== a || t[4] !== o || t[5] !== u?.label
    ? ((f = o ? a.formatMessage(he.allRepos) : u?.label),
      (t[3] = a),
      (t[4] = o),
      (t[5] = u?.label),
      (t[6] = f))
    : (f = t[6]);
  let p = f,
    [m, h] = (0, Q.useState)(!1),
    g;
  t[7] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((g = (0, $.jsx)(w, { className: `icon-sm shrink-0` })), (t[7] = g))
    : (g = t[7]);
  let _;
  t[8] === p
    ? (_ = t[9])
    : ((_ = (0, $.jsx)(`span`, { className: `max-w-[220px] truncate`, children: p })),
      (t[8] = p),
      (t[9] = _));
  let v = m && `rotate-180`,
    y;
  t[10] === v
    ? (y = t[11])
    : ((y = s(`icon-2xs text-token-input-placeholder-foreground transition-transform`, v)),
      (t[10] = v),
      (t[11] = y));
  let b;
  t[12] === y ? (b = t[13]) : ((b = (0, $.jsx)(R, { className: y })), (t[12] = y), (t[13] = b));
  let x;
  t[14] !== m || t[15] !== _ || t[16] !== b
    ? ((x = (0, $.jsxs)(k, {
        color: `outline`,
        size: `toolbar`,
        "aria-expanded": m,
        "aria-haspopup": `menu`,
        children: [g, _, b],
      })),
      (t[14] = m),
      (t[15] = _),
      (t[16] = b),
      (t[17] = x))
    : (x = t[17]);
  let S = o ? `font-medium` : void 0,
    C;
  t[18] === i
    ? (C = t[19])
    : ((C = () => {
        (i(`all`), h(!1));
      }),
      (t[18] = i),
      (t[19] = C));
  let T;
  t[20] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((T = (0, $.jsx)(d, { ...he.allRepos })), (t[20] = T))
    : (T = t[20]);
  let E;
  t[21] !== C || t[22] !== S
    ? ((E = (0, $.jsx)(I.Item, { className: S, onClick: C, children: T })),
      (t[21] = C),
      (t[22] = S),
      (t[23] = E))
    : (E = t[23]);
  let D;
  t[24] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((D = (0, $.jsx)(I.Separator, {})), (t[24] = D))
    : (D = t[24]);
  let O;
  if (t[25] !== i || t[26] !== n || t[27] !== r) {
    let e;
    (t[29] !== i || t[30] !== r
      ? ((e = (e) =>
          (0, $.jsx)(
            I.Item,
            {
              className: e.key === r ? `font-medium` : void 0,
              onClick: () => {
                (i(e.key), h(!1));
              },
              children: e.label,
            },
            e.key,
          )),
        (t[29] = i),
        (t[30] = r),
        (t[31] = e))
      : (e = t[31]),
      (O = n.map(e)),
      (t[25] = i),
      (t[26] = n),
      (t[27] = r),
      (t[28] = O));
  } else O = t[28];
  let A;
  return (
    t[32] !== m || t[33] !== E || t[34] !== O || t[35] !== x
      ? ((A = (0, $.jsxs)(L, { onOpenChange: h, open: m, triggerButton: x, children: [E, D, O] })),
        (t[32] = m),
        (t[33] = E),
        (t[34] = O),
        (t[35] = x),
        (t[36] = A))
      : (A = t[36]),
    A
  );
}
function Oe(e) {
  let t = (0, Z.c)(63),
    { item: n, mergeMethod: r, onSelectPullRequest: i, repo: a, selected: o, showRepo: s } = e,
    f = p(u),
    m = c(),
    h = l(),
    g = re(),
    _;
  t[0] !== m || t[1] !== h || t[2] !== f
    ? ((_ = (e) => {
        e.status === `success` &&
          (h.invalidateQueries({ queryKey: y(`gh-pr-board`) }),
          f
            .get(D)
            .success(
              m.formatMessage({
                id: `pullRequestsPage.card.merge.success`,
                defaultMessage: `Merged pull request`,
                description: `Toast shown when a board merge succeeds`,
              }),
            ));
      }),
      (t[0] = m),
      (t[1] = h),
      (t[2] = f),
      (t[3] = _))
    : (_ = t[3]);
  let v;
  t[4] !== n.cwd || t[5] !== n.headBranch || t[6] !== n.hostId || t[7] !== _
    ? ((v = {
        cwd: n.cwd,
        headBranch: n.headBranch,
        hostId: n.hostId,
        operationSource: `pull_request_board`,
        onSuccess: _,
      }),
      (t[4] = n.cwd),
      (t[5] = n.headBranch),
      (t[6] = n.hostId),
      (t[7] = _),
      (t[8] = v))
    : (v = t[8]);
  let b = ne(v),
    S;
  t[9] !== n || t[10] !== r || t[11] !== b || t[12] !== a || t[13] !== f
    ? ((S = async (e) => {
        (e.stopPropagation(), Y(f, { action: `merge`, item: n, surface: `pull_request_page` }));
        let t = await b.mutateAsync({ cwd: n.cwd, mergeMethod: r, number: n.number, repo: a });
        t.status !== `success` && f.get(D).danger(t.error);
      }),
      (t[9] = n),
      (t[10] = r),
      (t[11] = b),
      (t[12] = a),
      (t[13] = f),
      (t[14] = S))
    : (S = t[14]);
  let C = S,
    w;
  t[15] !== C || t[16] !== n || t[17] !== b.isPending || t[18] !== g || t[19] !== a
    ? ((w = ke({
        isMergePending: b.isPending,
        pullRequestMergeHelperEnabled: g,
        item: n,
        repo: a,
        onMergePullRequest: C,
      })),
      (t[15] = C),
      (t[16] = n),
      (t[17] = b.isPending),
      (t[18] = g),
      (t[19] = a),
      (t[20] = w))
    : (w = t[20]);
  let T = w,
    E;
  t[21] !== m || t[22] !== n || t[23] !== f
    ? ((E = {
        id: `copy-pull-request-url`,
        message: ge.copyUrl,
        onSelect: () => {
          (Y(f, { action: `copy_url`, item: n, surface: `pull_request_page` }),
            te(n.url).then(
              () => {
                f.get(D).success(m.formatMessage(ge.copyUrlSuccess));
              },
              () => {
                f.get(D).danger(m.formatMessage(ge.copyUrlError));
              },
            ));
        },
      }),
      (t[21] = m),
      (t[22] = n),
      (t[23] = f),
      (t[24] = E))
    : (E = t[24]);
  let O;
  t[25] !== n || t[26] !== f
    ? ((O = {
        id: `open-pull-request-in-browser`,
        message: ge.openInBrowser,
        onSelect: () => {
          (Y(f, { action: `open_in_browser`, item: n, surface: `pull_request_page` }),
            x.dispatchMessage(`open-in-browser`, { url: n.url }));
        },
      }),
      (t[25] = n),
      (t[26] = f),
      (t[27] = O))
    : (O = t[27]);
  let k;
  t[28] !== E || t[29] !== O ? ((k = [E, O]), (t[28] = E), (t[29] = O), (t[30] = k)) : (k = t[30]);
  let A = k,
    j;
  t[31] === n.state
    ? (j = t[32])
    : ((j = (0, $.jsx)(z, { className: `icon-sm shrink-0 translate-y-[2px]`, state: n.state })),
      (t[31] = n.state),
      (t[32] = j));
  let M;
  t[33] !== n || t[34] !== i || t[35] !== f
    ? ((M = () => {
        (q(f, { item: n, surface: `pull_request_page` }), i(n));
      }),
      (t[33] = n),
      (t[34] = i),
      (t[35] = f),
      (t[36] = M))
    : (M = t[36]);
  let N;
  t[37] !== n.additions || t[38] !== n.deletions
    ? ((N = (0, $.jsx)(V, { linesAdded: n.additions, linesRemoved: n.deletions })),
      (t[37] = n.additions),
      (t[38] = n.deletions),
      (t[39] = N))
    : (N = t[39]);
  let P;
  t[40] === n.number
    ? (P = t[41])
    : ((P = (0, $.jsx)(d, {
        id: `pullRequestsPage.card.pullRequestNumber`,
        defaultMessage: `#{number}`,
        description: `Pull request number shown on a pull request board card`,
        values: { number: n.number },
      })),
      (t[40] = n.number),
      (t[41] = P));
  let F;
  t[42] !== n.repo || t[43] !== s
    ? ((F =
        s && n.repo != null
          ? (0, $.jsxs)($.Fragment, {
              children: [
                (0, $.jsx)(`span`, { className: `truncate`, children: n.repo }),
                (0, $.jsx)(B, { className: `shrink-0` }),
              ],
            })
          : null),
      (t[42] = n.repo),
      (t[43] = s),
      (t[44] = F))
    : (F = t[44]);
  let I;
  t[45] !== n.baseBranch || t[46] !== n.headBranch
    ? ((I = (0, $.jsx)(`span`, {
        className: `truncate`,
        children: (0, $.jsx)(d, {
          id: `pullRequestsPage.card.branchPair`,
          defaultMessage: `{headBranch} to {baseBranch}`,
          description: `Branch pair shown on a pull request board card`,
          values: { baseBranch: n.baseBranch, headBranch: n.headBranch },
        }),
      })),
      (t[45] = n.baseBranch),
      (t[46] = n.headBranch),
      (t[47] = I))
    : (I = t[47]);
  let L;
  t[48] !== F || t[49] !== I
    ? ((L = (0, $.jsxs)(`span`, {
        className: `flex min-w-0 items-center gap-1 truncate`,
        children: [F, I],
      })),
      (t[48] = F),
      (t[49] = I),
      (t[50] = L))
    : (L = t[50]);
  let R;
  t[51] !== n.title ||
  t[52] !== T ||
  t[53] !== o ||
  t[54] !== N ||
  t[55] !== P ||
  t[56] !== L ||
  t[57] !== j ||
  t[58] !== M
    ? ((R = (0, $.jsx)(ie, {
        ariaLabel: n.title,
        icon: j,
        isSelected: o,
        onSelect: M,
        rightText: N,
        secondaryTitle: P,
        secondLine: L,
        secondLineRightText: T,
        title: n.title,
      })),
      (t[51] = n.title),
      (t[52] = T),
      (t[53] = o),
      (t[54] = N),
      (t[55] = P),
      (t[56] = L),
      (t[57] = j),
      (t[58] = M),
      (t[59] = R))
    : (R = t[59]);
  let H;
  return (
    t[60] !== A || t[61] !== R
      ? ((H = (0, $.jsx)(ee, { items: A, children: R })), (t[60] = A), (t[61] = R), (t[62] = H))
      : (H = t[62]),
    H
  );
}
function ke({
  isMergePending: e,
  pullRequestMergeHelperEnabled: t,
  item: n,
  repo: r,
  onMergePullRequest: i,
}) {
  return n.isAuthor && n.state === `ready`
    ? (0, $.jsx)(k, {
        className: `px-3 text-sm leading-[22px]`,
        color: `secondary`,
        loading: e,
        size: `default`,
        onClick: i,
        children: (0, $.jsx)(d, {
          id: `pullRequestsPage.card.merge`,
          defaultMessage: `Merge`,
          description: `Button label for a ready pull request card`,
        }),
      })
    : t && K(n)
      ? (0, $.jsx)(N, {
          electron: !0,
          children: (0, $.jsx)(G, {
            className: `px-3 text-sm leading-[22px]`,
            item: n,
            repo: r,
            stopPropagation: !0,
          }),
        })
      : null;
}
function Ae(e) {
  return (0, $.jsx)(d, { ...Me(e) });
}
function je(e) {
  return `pull-request-board-section-${e}`;
}
function Me(e) {
  switch (e) {
    case `draft`:
      return me.draft;
    case `merged`:
      return me.merged;
    case `ready`:
      return me.ready;
    case `unmet_checks`:
      return me.unmetChecks;
  }
}
var Ne = i(`pull-request-board:last-selected-repo-key`, null);
function Pe() {
  let e = (0, Z.c)(39),
    t = c(),
    n = C(),
    [i] = S(),
    [a, o] = r(Ne),
    s = i.get(`repoKey`),
    l;
  e[0] === i ? (l = e[1]) : ((l = J(i.get(`view`))), (e[0] = i), (e[1] = l));
  let u = l,
    d;
  e[2] === i ? (d = e[3]) : ((d = i.get(`pullRequestUrl`)), (e[2] = i), (e[3] = d));
  let f = d,
    p;
  e[4] === i ? (p = e[5]) : ((p = se(i.get(`pullRequestNumber`))), (e[4] = i), (e[5] = p));
  let g = p,
    _ = h(P),
    { isLoading: v } = h(E),
    y;
  e[6] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((y = { params: void 0, source: `pull_requests_page` }), (e[6] = y))
    : (y = e[6]);
  let { data: b, isLoading: x } = m(F, y),
    w = X({ gitOrigins: b?.origins ?? [], workspaceGroups: _ }),
    T = s ?? a,
    D = le({ repoOptions: w, requestedRepoKey: T }),
    O = T === `all` ? `all` : (D?.key ?? null);
  if (v || x) {
    let t;
    return (
      e[7] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((t = (0, $.jsx)(`div`, {
            className: `flex h-full items-center justify-center`,
            children: (0, $.jsx)(A, {}),
          })),
          (e[7] = t))
        : (t = e[7]),
      t
    );
  }
  if (w.length === 0) {
    let n;
    e[8] === t
      ? (n = e[9])
      : ((n = t.formatMessage({
          id: `pullRequestsPage.empty.noRepos.description`,
          defaultMessage: `Add a workspace with a GitHub remote to populate this board`,
          description: `Description for the pull requests page empty state when no repos are available`,
        })),
        (e[8] = t),
        (e[9] = n));
    let r;
    e[10] === t
      ? (r = e[11])
      : ((r = t.formatMessage({
          id: `pullRequestsPage.empty.noRepos.title`,
          defaultMessage: `No GitHub repos available`,
          description: `Title for the pull requests page empty state when no repos are available`,
        })),
        (e[10] = t),
        (e[11] = r));
    let i;
    return (
      e[12] !== n || e[13] !== r
        ? ((i = (0, $.jsx)(`div`, {
            className: `flex h-full items-center justify-center`,
            children: (0, $.jsx)(H, { description: n, title: r }),
          })),
          (e[12] = n),
          (e[13] = r),
          (e[14] = i))
        : (i = e[14]),
      i
    );
  }
  let k;
  e[15] !== n || e[16] !== i || e[17] !== O
    ? ((k = (e) => {
        let t = new URLSearchParams(i);
        (O != null && t.set(`repoKey`, O),
          t.set(`view`, e),
          t.delete(`pullRequestNumber`),
          t.delete(`pullRequestUrl`),
          n({ pathname: `/pull-requests`, search: `?${t.toString()}` }));
      }),
      (e[15] = n),
      (e[16] = i),
      (e[17] = O),
      (e[18] = k))
    : (k = e[18]);
  let j;
  e[19] !== n || e[20] !== i || e[21] !== u || e[22] !== O
    ? ((j = (e, t) => {
        let r = new URLSearchParams(i);
        (O != null && r.set(`repoKey`, O),
          r.set(`view`, u),
          e == null
            ? (r.delete(`pullRequestNumber`), r.delete(`pullRequestUrl`))
            : (r.set(`pullRequestNumber`, String(e.number)), r.set(`pullRequestUrl`, e.url)),
          n(
            { pathname: `/pull-requests`, search: `?${r.toString()}` },
            { replace: t?.replace === !0 },
          ));
      }),
      (e[19] = n),
      (e[20] = i),
      (e[21] = u),
      (e[22] = O),
      (e[23] = j))
    : (j = e[23]);
  let M;
  e[24] !== n || e[25] !== i || e[26] !== u || e[27] !== o
    ? ((M = (e) => {
        o(e);
        let t = new URLSearchParams(i);
        (t.set(`repoKey`, e),
          t.set(`view`, u),
          t.delete(`pullRequestNumber`),
          t.delete(`pullRequestUrl`),
          n({ pathname: `/pull-requests`, search: `?${t.toString()}` }));
      }),
      (e[24] = n),
      (e[25] = i),
      (e[26] = u),
      (e[27] = o),
      (e[28] = M))
    : (M = e[28]);
  let N;
  return (
    e[29] !== w ||
    e[30] !== u ||
    e[31] !== g ||
    e[32] !== f ||
    e[33] !== D ||
    e[34] !== O ||
    e[35] !== k ||
    e[36] !== j ||
    e[37] !== M
      ? ((N = (0, $.jsx)(ve, {
          selectedFilterView: u,
          repoOption: D,
          repoOptions: w,
          selectedPullRequestNumber: g,
          selectedPullRequestUrl: f,
          selectedRepoKey: O,
          onSelectFilterView: k,
          onSelectPullRequest: j,
          onSelectRepo: M,
        })),
        (e[29] = w),
        (e[30] = u),
        (e[31] = g),
        (e[32] = f),
        (e[33] = D),
        (e[34] = O),
        (e[35] = k),
        (e[36] = j),
        (e[37] = M),
        (e[38] = N))
      : (N = e[38]),
    N
  );
}
export { Pe as PullRequestsPage };
//# sourceMappingURL=pull-requests-page-B7iU-XC0.js.map
