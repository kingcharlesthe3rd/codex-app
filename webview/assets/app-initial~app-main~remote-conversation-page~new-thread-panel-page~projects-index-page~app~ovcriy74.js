...
import {
  r as Ve,
  t as He,
} from "./app-initial~app-main~remote-conversation-page~pull-requests-page~new-thread-panel-page~proj~jolrh1c9.js";
var z,
  Ue = e(() => {
...
function qe({ order: e, tasks: t, visibleThreadIds: n, nextVisibleThreadIds: r }) {
  return {
    threadIds: O({
      threadIds: (e == null
        ? We(t)
...
function Xe(e, t) {
  return {
    threadIds: B(
      e
        .flatMap((e, n) => (V(e) == null ? [] : [{ task: e, timestamp: Ze(e, t), index: n }]))
...
function tt(e) {
  return [...e].sort(
    (e, t) => H[e.attentionState] - H[t.attentionState] || t.recencyAt - e.recencyAt,
  );
}
var H,
  nt = e(() => {
...
function at(e, t) {
  let n = e.get(N),
    r = e.get(he);
  (!e.get(X).initialized && n === `recent` && !r && e.set(Y, (e) => e ?? `updated_at`),
...
function ut(e, t) {
  ...
  (e.set(Y, (e) => e ?? n), e.set(J, (e) => ({ ...K, ...e, projectSortMode: t })));
}
function dt({
  aeonThreadKeys: e,
...
var Tt,
  ...
  Pt = e(() => {
    (s(),
      ...
      (Mt = _(
        R,
        ({ get: e }) => L(e, h.ACTIVE_REMOTE_PROJECT_ID) ?? e(ie).data?.roots?.[0] ?? null,
      )),
      (Nt = _(R, ({ get: e, scope: t }) => {
        let { allSidebarThreadKeys: n, pinnedThreadKeys: r } = e(ze),
          ...
          u = o.flatMap((e) => (e.task.kind === `local` ? [e.task.conversation.id] : [])),
          ...
          C = vt({ items: o, projectGroups: _, projectlessThreadIds: new Set(f ?? []) }),
...
function Z(e) {
  return (
    e.find((e) => Lt(e.configPath) === `environment.toml` && e.type === `success`) ||
...
function Ht({
  ...
  let a = Bt(i, n),
    { defaultEnvironment: o, defaultEnvironmentNormalized: s, availableEnvironments: c } = zt(t),
    l = o?.configPath ?? null,
...
function Wt(e) {
  let t = (0, Kt.c)(37),
    ...
    [i, a] = l(qt),
    o;
  t[0] !== n || t[1] !== r ? ((o = Bt(r, n)), (t[0] = n), (t[1] = r), (t[2] = o)) : (o = t[2]);
  let s = o,
    c = r ?? ``,
    u;
  t[3] !== n || t[4] !== c
    ? ((u = { hostId: n, workspaceRoot: c }), (t[3] = n), (t[4] = c), (t[5] = u))
    : (u = t[5]);
  let d = s != null,
    f;
  ...
  let p;
  t[8] !== u || t[9] !== f
    ? ((p = { params: u, queryConfig: f, select: Gt }), (t[8] = u), (t[9] = f), (t[10] = p))
    : (p = t[10]);
  let { data: h, isLoading: g, isFetching: _, error: v } = m(`local-environments`, p),
    y,
    b;
  t[11] !== v ||
  t[12] !== n ||
...
async function Yt(e, { sourceConversationId: t, sourceWorkspaceRoot: n }) {
  ...
  try {
    ...
    return (
      a.error(`Error forking local task`, { safe: {}, sensitive: { error: t } }),
...
async function Xt(
  ...
  if ((a.origins.find((e) => e.dir === r)?.root ?? a.origins[0]?.root ?? null) == null)
    return (e.get(pe).danger(e.get(S).formatMessage(T.forkThreadRequiresGitRepo)), null);
  let o = await Qt({ hostId: i, localEnvironmentSelectionsByWorkspace: t, sourceWorkspaceRoot: r }),
    s = e.get(S),
    c = Zt(e, n),
    l = x({
      hostId: i,
  ...
  return (v(e, { pendingWorktreeId: l, sourceConversationId: n, sourceWorkspaceRoot: r }), l);
}
function Zt(e, t) {
  return e.get(ge, t) == null ? e.get(ne, t) : null;
}
async function Qt({ hostId: e, localEnvironmentSelectionsByWorkspace: t, sourceWorkspaceRoot: n }) {
  let r = e ?? `local`;
  try {
