import { ci as e, fi as t, qr as n } from "./src-2.js";
import { f as r, g as i } from "./vscode-api.js";
import { o as a } from "./app-shell-tab-controller.js";
...
function ee({ drag: e, fitHeight: t, fitWidth: n, pointerX: r, pointerY: i, scale: a }) {
  let o = e.edge === `bottom` || e.edge === `bottom-left` || e.edge === `bottom-right`,
    s = e.edge !== `bottom`,
    c = e.edge === `left` || e.edge === `bottom-left` ? -1 : 1;
  return {
    width: s
      ? w(S({ fitSize: n, rawSize: e.startWidth + ((r - e.startPointerX) * c * 2) / a, scale: a }))
      : e.startWidth,
    height: o
      ? T(S({ fitSize: t, rawSize: e.startHeight + (i - e.startPointerY) / a, scale: a }))
      : e.startHeight,
  };
}
...
function ne(e) {
  ...
  return { ...e, ...te(e), presetId: n ? e.presetId : s };
}
  ...
  ae = class {
    ...
    sync(e, t, n, r) {
      ...
      if (
        ...
        ((this.pendingHandoff?.incoming.host === e || this.pendingHandoff?.outgoing.host === e) &&
          this.cancel(`incoming-hidden`, !0),
          e.sync(t, n),
          this.paintedWebview?.host === e && !e.isPainted() && (this.paintedWebview = null));
        return;
      }
      let a = this.pendingHandoff;
      if (a != null) {
        if (a.incoming.host === e) {
          ((a.incoming = i),
            a.releaseFrameId == null ? e.stage(t, n) : (e.sync(t, n), (this.paintedWebview = i)));
          return;
        }
        if (a.releaseFrameId != null)
          (window.cancelAnimationFrame(a.releaseFrameId),
            this.releaseOutgoing(a, `incoming-replaced`));
        else {
          let { outgoing: r } = a;
          if ((this.cancel(`incoming-replaced`, !1), r.host === e)) {
            (e.sync(t, n), (this.paintedWebview = i));
            return;
          }
          if (D(r, i)) {
            this.stage(r, i, t);
      ...
      let o = this.paintedWebview;
      if (o != null && D(o, i)) {
        this.stage(o, i, t);
        return;
      }
      (e.sync(t, n), (this.paintedWebview = e.isPainted() ? i : null));
    }
    detach(e, t, n) {
      if (this.pendingHandoff?.outgoing.host === e) {
        e.releaseRef(t, n);
        return;
      }
      if (this.pendingHandoff?.incoming.host === e) {
        this.cancel(`incoming-detached`, !0);
        return;
      }
      (this.paintedWebview?.host === e && (this.paintedWebview = null), e.detach(t, n));
    }
    acknowledgeAttachment(e, t, n, r) {
      ...
      r === n
        ? (i.info(`IAB_TAB_SWITCH renderer acknowledged Owl webview handoff`, {
            safe: { ...O(a), acknowledgedMountGeneration: n, currentMountGeneration: r },
            sensitive: {},
          }),
          this.scheduleCommitAfterPrewarm(a))
        : a != null &&
          i.info(`IAB_TAB_SWITCH renderer ignored stale Owl webview handoff acknowledgement`, {
            safe: { ...O(a), acknowledgedMountGeneration: n, currentMountGeneration: r },
            sensitive: {},
          });
    }
    removeTab(e) {
      let t = this.pendingHandoff;
      if (t?.outgoing.host === e) {
        (this.commit(t),
          t.releaseFrameId != null &&
            (window.cancelAnimationFrame(t.releaseFrameId), (t.releaseFrameId = null)),
          this.releaseOutgoing(t, `tab-removed`));
        return;
      }
      if (t?.incoming.host === e) {
        this.cancel(`tab-removed`, !0);
        return;
      }
      let n = this.paintedWebview;
      n?.host === e && ((this.paintedWebview = null), e.detach(n.webviewRef, n.mountGeneration));
    }
    transferRoute(e) {
      (this.pendingHandoff?.incoming.host === e || this.pendingHandoff?.outgoing.host === e) &&
        this.cancel(`route-transferred`, !0);
    }
    removeConversation(e) {
      ((this.pendingHandoff?.incoming.host.getConversationId() === e ||
        this.pendingHandoff?.outgoing.host.getConversationId() === e) &&
        this.cancel(`conversation-removed`, !0),
        this.paintedWebview?.host.getConversationId() === e && (this.paintedWebview = null));
    }
    disposeHost(e) {
      ((this.pendingHandoff?.incoming.host === e || this.pendingHandoff?.outgoing.host === e) &&
        this.cancel(`host-disposed`, !0),
        this.paintedWebview?.host === e && (this.paintedWebview = null));
    }
    disposeAll() {
      (this.cancel(`manager-disposed`, !0), (this.paintedWebview = null));
    }
    stage(e, t, n) {
      e.host.blockInteraction();
      let r = {
        incoming: t,
        outgoing: e,
        prewarmFrameId: null,
      ...
      ((this.pendingHandoff = r),
        (r.removeIncomingDidAttachListener = t.host.listenForDidAttach(() => {
          this.pendingHandoff === r &&
            (i.info(`IAB_TAB_SWITCH renderer observed Owl webview did-attach during handoff`, {
              safe: O(r),
              sensitive: {},
            }),
            this.scheduleCommitAfterPrewarm(r));
        })),
        (r.timeoutId = window.setTimeout(() => {
          this.pendingHandoff === r &&
            (r.prewarmFrameId != null &&
              (window.cancelAnimationFrame(r.prewarmFrameId), (r.prewarmFrameId = null)),
            i.info(`IAB_TAB_SWITCH renderer timed out Owl webview handoff`, {
              safe: O(r),
              sensitive: {},
            }),
            this.commit(r));
        }, re)),
        i.info(`IAB_TAB_SWITCH renderer staged Owl webview handoff`, { safe: O(r), sensitive: {} }),
        t.host.stage(n, t.webviewRef));
    }
    scheduleCommitAfterPrewarm(e, t = ie) {
      if (!(this.pendingHandoff !== e || e.prewarmFrameId != null || e.releaseFrameId != null)) {
        if (t === 0) {
          (i.info(`IAB_TAB_SWITCH renderer prewarmed Owl webview handoff before reveal`, {
            safe: O(e),
            sensitive: {},
          }),
            this.commit(e));
        ...
        (i.info(`IAB_TAB_SWITCH renderer prewarming Owl webview handoff before reveal`, {
          safe: { ...O(e), remainingFrames: t },
          sensitive: {},
        }),
          (e.prewarmFrameId = window.requestAnimationFrame(() => {
            this.pendingHandoff === e &&
              ((e.prewarmFrameId = null), this.scheduleCommitAfterPrewarm(e, t - 1));
          })));
      }
    }
    commit(e) {
      this.pendingHandoff !== e ||
        e.releaseFrameId != null ||
        (e.timeoutId != null && (window.clearTimeout(e.timeoutId), (e.timeoutId = null)),
        e.prewarmFrameId != null &&
          (window.cancelAnimationFrame(e.prewarmFrameId), (e.prewarmFrameId = null)),
        e.removeIncomingDidAttachListener?.(),
        (e.removeIncomingDidAttachListener = null),
        e.incoming.host.reveal(e.incoming.webviewRef),
        (this.paintedWebview = e.incoming),
        i.info(`IAB_TAB_SWITCH renderer committed Owl webview handoff`, {
          safe: O(e),
          sensitive: {},
        }),
        (e.releaseFrameId = window.requestAnimationFrame(() => {
          this.releaseOutgoing(e, `next-animation-frame`);
        })));
    }
    releaseOutgoing(e, t) {
      this.pendingHandoff === e &&
        ((this.pendingHandoff = null),
        e.outgoing.host.detach(e.outgoing.webviewRef, e.outgoing.mountGeneration),
        i.info(`IAB_TAB_SWITCH renderer released outgoing Owl webview handoff`, {
          safe: { ...O(e), timing: t },
          sensitive: {},
        }));
    }
    cancel(e, t) {
      let n = this.pendingHandoff;
      n != null &&
        ((this.pendingHandoff = null),
        n.timeoutId != null && window.clearTimeout(n.timeoutId),
        n.prewarmFrameId != null && window.cancelAnimationFrame(n.prewarmFrameId),
        n.releaseFrameId != null && window.cancelAnimationFrame(n.releaseFrameId),
        n.removeIncomingDidAttachListener?.(),
        n.incoming.host.detach(n.incoming.webviewRef, n.incoming.mountGeneration),
        t
          ? (n.outgoing.host.detach(n.outgoing.webviewRef, n.outgoing.mountGeneration),
            (this.paintedWebview = null))
          : n.outgoing.host.isConnected()
            ? (n.outgoing.host.restoreInteraction(), (this.paintedWebview = n.outgoing))
            : (this.paintedWebview = null),
        i.info(`IAB_TAB_SWITCH renderer cancelled Owl webview handoff`, {
          safe: { ...O(n), detachOutgoing: t, reason: e },
          sensitive: {},
        }));
    }
  };
...
var oe = `about:blank`,
  ...
  A = `data-browser-sidebar-browser-tab-id`,
  se = `data-browser-sidebar-webview-host-root`,
  ce = `data-browser-sidebar-cursor-overlay-host`,
  le = `owl-webcontents-adoption-lease`,
  ue = `owl-webcontents-adopted-web-contents-id`,
  de = `data-browser-sidebar-adopted-initial-url`,
  fe = `persist:codex-browser-app-route:`,
  ...
  N = class {
    ...
    releaseRef(e, t) {
      let n = this.webview;
      n == null || (t != null && t !== this.state.mountGeneration) || z(e, null, n);
    }
    sync(e, t) {
      let n = this.webview;
      if (this.isDisposed || n == null) {
    ...
    bringToFront() {
      let e = this.container;
      e?.isConnected === !0 && e.nextElementSibling != null && R().append(e);
    }
    ...
    setAdoptionAttributes(e, t, n) {
      if (this.webview != null) {
        if (e == null || t == null) {
          (this.webview.removeAttribute(le),
            this.webview.removeAttribute(ue),
            this.webview.removeAttribute(de));
          return;
        }
        (this.webview.setAttribute(le, e),
          this.webview.setAttribute(ue, t.toString()),
          this.webview.setAttribute(de, n));
      }
    }
    resync() {
      let e = this.container;
    ...
    transfer({ browserTabId: e, conversationId: t, elementKey: n, partition: r }) {
      ...
      i == null ||
        ...
        this.cursorOverlayHost?.setAttribute(ce, n),
        a.setAttribute(k, t),
        a.setAttribute(A, e),
    ...
    dispose() {
      ...
      let e = this.webview;
      (this.detachFromDom(),
        this.webviewRef != null && z(this.webviewRef, null, e ?? void 0),
        _e(e),
      ...
      return i != null && this.browserUseCaptureSurfaceSize == null
        ? ((this.lastVisibleBounds = i),
          ...
          r || this.state.shouldPaint === !1
            ? (I(e, n, t, i, this.state.scale, this.state.windowZoom ?? 1), `bootstrap`)
            : this.isStaged
              ? (he(e, n, t, i, this.state.scale, this.state.windowZoom ?? 1), `staged`)
    ...
    attachToDom() {
      let e = this.container;
      e != null && !e.isConnected && R().append(e);
    }
    parkInDom() {
      let e = this.container,
        t = this.cursorOverlayHost,
      ...
      (this.attachToDom(), I(e, n, t, r, this.state.scale, this.state.windowZoom ?? 1));
    }
    detachFromDom() {
...
function P(e) {
  return `${fe}${encodeURIComponent(e)}`;
}
...
function he(e, t, n, r, i, a) {
  (F(e, t, n, r, i, a), Object.assign(e.style, { opacity: pe, pointerEvents: `none` }));
}
function ge(e, t, n = 1) {
  Object.assign(e.style, {
    height: `${t.height}px`,
...
var ye = `about:blank`,
  B = `data-browser-sidebar-conversation-id`,
  V = `data-browser-sidebar-browser-tab-id`,
  be = `data-browser-sidebar-retained-webview`,
  H = `data-browser-sidebar-cursor-overlay-host`,
  xe = {
    contain: ``,
    height: `1px`,
    left: `-10000px`,
  ...
  G = class {
    ...
    constructor({
      ...
      ((this.browserTabId = e),
        ...
        this.cursorOverlayHost.setAttribute(H, i),
        K(
          this.container,
          this.webview,
        ...
        this.webview.setAttribute(B, r),
        this.webview.setAttribute(V, e),
        this.webview.setAttribute(be, ``),
        this.webview.setAttribute(`partition`, s),
        this.webview.setAttribute(`src`, a.length === 0 ? ye : a),
    ...
    transfer({ browserTabId: e, conversationId: t, elementKey: n, partition: r }) {
      ((this.container.dataset.browserSidebarWebview = n),
        this.cursorOverlayHost.setAttribute(H, n),
        this.webview.setAttribute(B, t),
        this.webview.setAttribute(V, e),
    ...
    syncContainerStyle() {
      ...
      if (this.browserUseCaptureSurfaceSize != null) {
        q(this.container, this.webview, this.cursorOverlayHost, e);
        return;
      }
...
function K(e, t, n, r, i, a) {
  if (a) {
    q(e, t, n, r ?? (i == null ? W : X(i)));
    return;
  }
  (Object.assign(e.style, xe), J(t), Y(n, { width: 1, height: 1 }));
}
function q(e, t, n, r) {
  (Object.assign(e.style, {
...
var De = { responsiveViewportSize: null, toolbarState: m },
  Oe = new Set([`new-conversation`, `panel-new-conversation`]),
  ke = new (class {
    listeners = new Set();
    ...
    getBrowserUseBrowserTabSummarySyncKey(e) {
      ...
      return n == null ? `` : je(n, this.getSnapshot(e, n));
    }
    getDeviceToolbarTabState(e, t = $(e, void 0)) {
      return this.deviceToolbarTabStates.get(Z(e, t)) ?? De;
    }
    ...
    syncElectronWebview(e, t, n, r = `panel`) {
      this.electronPageHandoff.sync(e, t, n, r);
    }
    detachElectronWebview(e, t, n) {
      this.electronPageHandoff.detach(e, t, n);
    }
    ...
    removeTab(e, t) {
      ...
      (r instanceof N && this.electronPageHandoff.removeTab(r),
    ...
    setBrowserUseViewportSize(e, ...t) {
      let n = t.length === 1 ? $(e, void 0) : t[0],
        ...
        i = Z(e, n),
        a = r == null ? null : { width: w(r.width), height: T(r.height) };
    ...
    reassociateTabState(e, ...n) {
      let a = n.length === 1 ? $(e, void 0) : n[0],
        o = n.length === 1 ? n[0] : n[1],
        s = n.length === 1 ? $(o, void 0) : n[2],
        c = t(Z(e, a), Z(o, s)),
        l = Z(e, a),
        u = Z(o, s);
      if (l === u || this.transferredWebviewKeys.has(c)) return;
      let d = this.webviews.get(l) ?? null;
      d instanceof N && this.electronPageHandoff.transferRoute(d);
      ...
      let v = !1;
      (h && (this.browserUseTabKeys.add(u), this.syncBrowserUseTabKeys(e), (v = !0)),
        _ && this.browserUseActiveTabKeys.add(u),
      ...
      let y = this.localServersStates.get(e),
        b = this.deviceToolbarTabStates.get(l);
      (p != null && (this.snapshots.delete(l), this.snapshots.set(u, p)),
        v && this.syncBrowserUseTabKeys(o),
        y != null && this.localServersStates.set(o, y),
        b != null && (this.deviceToolbarTabStates.delete(l), this.deviceToolbarTabStates.set(u, b)),
    ...
    removeElectronWebviewFromHandoff(e) {
      e instanceof N && this.electronPageHandoff.disposeHost(e);
    }
    ...
    getFirstConversationBrowserTabId(t, n) {
      let r = `${t}\0`;
      for (let t of n) if (t.startsWith(r)) return e(t.slice(r.length));
      return null;
    }
    getConversationBrowserTabIdsFromKeys(t, n) {
      let r = `${t}\0`,
        i = [];
      for (let t of n) t.startsWith(r) && i.push(e(t.slice(r.length)));
      return i;
    }
    ...
    syncBrowserUseTabKeys(e) {
      ...
      (this.browserUseTabIdsKeysByConversation.set(e, t.join(`\0`)),
        this.browserUseTabSummarySyncKeysByConversation.set(
          e,
          t.map((t) => je(t, this.getSnapshot(e, t))).join(`\0`),
        ),
        this.refreshBrowserUseTabs());
    }
    refreshBrowserUseTabs() {
      this.browserUseTabs = Array.from(this.browserUseTabKeys, Ae);
    }
...
function Ae(t) {
  let n = t.indexOf(`\0`);
  return { browserTabId: e(t.slice(n + 1)), conversationId: t.slice(0, n) };
}
function je(e, t) {
