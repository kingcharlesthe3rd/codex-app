...
import { c } from "./config-queries-BHYT-TjG.js";
...
async function u(e, t) {
  return t === `local` &&
    ...
    ? `copilot`
    : d((await e.get(o, t)?.getAccount())?.account ?? null);
}
function d(e) {
...
async function f(a, o, c) {
  try {
    ...
    return d.service_tier == null
      ? n(await m(o, c ?? d.model), d.service_tier, e)
      : n(null, d.service_tier, e);
  } catch (t) {
    return (
      e.error(`Failed to read service tier for request`, { safe: {}, sensitive: { error: t } }),
      null
    );
  }
}
...
async function m(n, r) {
  try {
    let { data: e } = await t(`list-models-for-host`, {
      hostId: n,
      includeHidden: !0,
      cursor: null,
      limit: 100,
    ...
    return r == null
      ? (e.find((e) => e.isDefault) ?? null)
      : (e.find((e) => e.model === r || e.id === r) ?? null);
  } catch (t) {
    return (
      e.error(`Failed to read service tier model`, { safe: {}, sensitive: { error: t } }), null
    );
  }
}
