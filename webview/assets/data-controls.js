import { s as e } from "./chunk.js";
import { n as t, t as n } from "./jsx-runtime.js";
import { R as r, o as i, t as a } from "./app-scope.js";
import { n as o, t as s } from "./fuse.js";
import { _ as c, c as l, g as u, k as d, u as f } from "./vscode-api.js";
import "./isEqual.js";
import { Lr as p } from "./src-4.js";
import { As as m, Ni as h, T as g, us as _ } from "./app-server-manager-signals.js";
import "./react-dom.js";
import { c as v, o as y, r as b } from "./lib-1.js";
import "./persisted-signal.js";
import "./proxy.js";
import { t as x } from "./button.js";
import "./setting-storage.js";
import "./reduced-motion-preference.js";
import "./use-reduced-motion-1.js";
import "./spinner.js";
import "./Combination.js";
import "./window-zoom-context.js";
import { t as S } from "./check-md.js";
import "./chevron-right.js";
import "./use-stable-callback.js";
import { t as C } from "./tooltip.js";
import "./rpc-2.js";
import "./statsig-DoZ-0xit.js";
import { t as w } from "./request.js";
import "./platform.js";
import "./marked.esm.js";
import "./selectable-remote-connections-signal-DJZ-ZbLr.js";
import { r as T } from "./toast-signal.js";
import { m as E } from "./chunk-8.js";
import "./_baseEach.js";
import "./_baseOrderBy.js";
import "./x.js";
import "./with-window.js";
import { t as D } from "./folder.js";
import { a as O, i as k, l as A, n as j, r as M } from "./dialog-layout.js";
import "./_defineProperty.js";
import "./initial-route-atom.js";
import { t as N } from "./use-navigate-to-local-conversation.js";
import "./links.js";
import { t as P } from "./folders.js";
import { t as F } from "./search.js";
import { r as I, t as L } from "./dropdown.js";
import "./three-dots.js";
import { t as R } from "./trash.js";
import "./chevron.js";
import { t as z } from "./cloud.js";
import "./sortBy.js";
import "./use-connected-remote-connections.js";
import { t as B } from "./settings-host-context.js";
import { t as V } from "./get-project-name.js";
import { i as H, t as U } from "./settings-shared.js";
import { t as ee } from "./settings-content-layout.js";
import { t as W } from "./more-menu-trigger.js";
import { n as te } from "./settings-row.js";
import { n as ne, t as re } from "./settings-group.js";
import { t as ie } from "./infinite-scroll-pagination-spinner.js";
import { t as ae } from "./filter.js";
import { t as oe } from "./control-group.js";
var G = r(),
  se = e(t(), 1),
  K = b({
    delete: {
      id: `settings.dataControls.archivedChats.delete`,
      defaultMessage: `Delete`,
      description: `Button label to permanently delete an archived chat`,
    },
    deleteAll: {
      id: `settings.dataControls.archivedChats.deleteAll`,
      defaultMessage: `Delete all`,
      description: `Button label to permanently delete all archived local chats`,
    },
    deleteAllError: {
      id: `settings.dataControls.archivedChats.deleteAllError`,
      defaultMessage: `Failed to delete archived chats`,
      description: `Error message when deleting all archived chats fails`,
    },
    deleteError: {
      id: `settings.dataControls.archivedChats.deleteError`,
      defaultMessage: `Failed to delete archived chat`,
      description: `Error message when deleting an archived chat fails`,
    },
    deleteSuccessPlain: {
      id: `settings.dataControls.archivedChats.deleteSuccessPlain`,
      defaultMessage: `Deleted archived chat`,
      description: `Success toast after deleting an archived chat`,
    },
    deleting: {
      id: `settings.dataControls.archivedChats.deleting`,
      defaultMessage: `Deleting archived chat…`,
      description: `Toast shown while deleting an archived chat`,
    },
    deletingAll: {
      id: `settings.dataControls.archivedChats.deletingAll`,
      defaultMessage: `Deleting archived chats…`,
      description: `Toast shown while deleting all archived chats`,
    },
    unarchiveError: {
      id: `settings.dataControls.archivedChats.unarchiveError`,
      defaultMessage: `Failed to unarchive chat`,
      description: `Error message when unarchiving a chat`,
    },
    unarchiveSuccessPlain: {
      id: `settings.dataControls.archivedChats.unarchiveSuccessPlain`,
      defaultMessage: `Unarchived chat`,
      description: `Success toast after unarchiving a chat`,
    },
    viewNow: {
      id: `settings.dataControls.archivedChats.viewNow`,
      defaultMessage: `View now`,
      description: `Action label to open an unarchived chat from the success toast`,
    },
    unarchive: {
      id: `settings.dataControls.archivedChats.unarchive`,
      defaultMessage: `Unarchive`,
      description: `Button label to unarchive a chat`,
    },
  });
function ce({ cloudTasks: e, localThreads: t }) {
  return [
    ...(e ?? []).map((e) => {
      let t = h(e.title ?? ``).trim(),
        n = e.task_status_display?.environment_label?.trim() ?? ``,
        r = V(n);
      return {
        kind: `cloud`,
        createdAt: q(e.created_at, e.updated_at),
        projectId: r == null ? null : n,
        projectLabel: r,
        searchValues: [t, r ?? ``, n],
        task: e,
        title: t,
        updatedAt: q(e.updated_at, e.created_at),
      };
    }),
    ...(t ?? []).map((e) => {
      let t = h(e.name?.trim() || e.preview).trim(),
        n = V(e.cwd),
        r = V(e.path),
        i = n ?? r;
      return {
        kind: `local`,
        createdAt: q(e.createdAt, e.updatedAt),
        projectId: n == null ? (r == null ? null : e.path) : e.cwd,
        projectLabel: i,
        searchValues: [t, i ?? ``],
        thread: e,
        title: t,
        updatedAt: q(e.updatedAt, e.createdAt),
      };
    }),
  ].sort((e, t) => t.updatedAt - e.updatedAt);
}
function le(e) {
  let t = new s(e, { ignoreLocation: !0, keys: [`searchValues`], threshold: 0.4 });
  return (n) => {
    let r = n.trim();
    return r.length === 0 ? e : t.search(r).map(({ item: e }) => e);
  };
}
function ue({ entries: e, groupKey: t, kindFilter: n, searchQuery: r, sortKey: i }) {
  let a = le(e.filter((e) => n === `all` || e.kind === n))(r);
  if (t === `none`)
    return a.length === 0
      ? []
      : [{ id: `all`, projectLabel: null, entries: [...a].sort((e, t) => Y(e, t, i)) }];
  let o = new Map();
  for (let e of a) {
    let t = e.projectId ?? ``,
      n = o.get(t);
    if (n != null) {
      n.entries.push(e);
      continue;
    }
    o.set(t, { id: t, projectLabel: e.projectLabel, entries: [e] });
  }
  let s = [...o.values()];
  for (let e of s) e.entries.sort((e, t) => Y(e, t, i));
  return s.sort((e, t) => {
    if (i === `alphabetical`) {
      let n = (e.projectLabel ?? ``).localeCompare(t.projectLabel ?? ``);
      return n === 0 ? e.id.localeCompare(t.id) : n;
    }
    let n = fe(t, i) - fe(e, i);
    if (n !== 0) return n;
    let r = (e.projectLabel ?? ``).localeCompare(t.projectLabel ?? ``);
    return r === 0 ? e.id.localeCompare(t.id) : r;
  });
}
function de(e, t) {
  return e.flatMap((e) => (e.kind === `local` && (e.projectId ?? ``) === t ? [e.thread] : []));
}
function q(e, t) {
  return e != null && Number.isFinite(e) ? e : t != null && Number.isFinite(t) ? t : 0;
}
function J(e, t) {
  switch (t) {
    case `alphabetical`:
      return 0;
    case `created`:
      return e.createdAt;
    case `updated`:
      return e.updatedAt;
  }
}
function Y(e, t, n) {
  if (n === `alphabetical`) {
    let n = e.title.localeCompare(t.title);
    return n === 0 ? t.updatedAt - e.updatedAt : n;
  }
  let r = J(t, n) - J(e, n);
  return r === 0 ? e.title.localeCompare(t.title) : r;
}
function fe(e, t) {
  let n = 0;
  for (let r of e.entries) n = Math.max(n, J(r, t));
  return n;
}
var X = n();
function Z(e) {
  let t = (0, G.c)(55),
    { archivedTask: n, showProjectName: r } = e,
    o = i(a),
    s = d(),
    c = v(),
    l = E(),
    f,
    p;
  t[0] === n.id
    ? ((f = t[1]), (p = t[2]))
    : ((f = [`recover-cloud-task`, n.id]),
      (p = () =>
        w.safePost(`/wham/tasks/{task_id}/recover`, { parameters: { path: { task_id: n.id } } })),
      (t[0] = n.id),
      (t[1] = f),
      (t[2] = p));
  let m;
  t[3] !== c || t[4] !== o
    ? ((m = () => {
        o.get(T).danger(c.formatMessage(K.unarchiveError));
      }),
      (t[3] = c),
      (t[4] = o),
      (t[5] = m))
    : (m = t[5]);
  let g;
  t[6] !== n.id || t[7] !== l || t[8] !== s || t[9] !== o
    ? ((g = () => {
        (s.setQueryData([`tasks`, `archived`], (e) =>
          e == null
            ? e
            : {
                ...e,
                pages: e.pages.map((e) => ({ ...e, items: e.items.filter((e) => e.id !== n.id) })),
              },
        ),
          o.get(T).info(
            (0, X.jsxs)(`span`, {
              children: [
                (0, X.jsx)(y, { ...K.unarchiveSuccessPlain }),
                (0, X.jsx)(`button`, {
                  className: `pointer-events-auto ml-1 cursor-interaction text-token-link underline-offset-2 hover:underline`,
                  type: `button`,
                  onClick: () => {
                    l(`/remote/${n.id}`);
                  },
                  children: (0, X.jsx)(y, { ...K.viewNow }),
                }),
              ],
            }),
            { id: `recover-cloud-task-${n.id}` },
          ));
      }),
      (t[6] = n.id),
      (t[7] = l),
      (t[8] = s),
      (t[9] = o),
      (t[10] = g))
    : (g = t[10]);
  let _;
  t[11] === s
    ? (_ = t[12])
    : ((_ = () => {
        s.invalidateQueries({ queryKey: [`tasks`] });
      }),
      (t[11] = s),
      (t[12] = _));
  let b;
  t[13] !== f || t[14] !== p || t[15] !== m || t[16] !== g || t[17] !== _
    ? ((b = { mutationKey: f, mutationFn: p, onError: m, onSuccess: g, onSettled: _ }),
      (t[13] = f),
      (t[14] = p),
      (t[15] = m),
      (t[16] = g),
      (t[17] = _),
      (t[18] = b))
    : (b = t[18]);
  let S = u(b),
    C;
  t[19] === n.title ? (C = t[20]) : ((C = h(n.title ?? ``).trim()), (t[19] = n.title), (t[20] = C));
  let D = C,
    O,
    k,
    A,
    j;
  if (t[21] !== n || t[22] !== c || t[23] !== r) {
    let e = pe(n),
      i;
    (t[28] !== n.task_status_display?.environment_label || t[29] !== r
      ? ((i = r ? n.task_status_display?.environment_label?.trim() : null),
        (t[28] = n.task_status_display?.environment_label),
        (t[29] = r),
        (t[30] = i))
      : (i = t[30]),
      (k = i),
      (A = e != null),
      (O = A ? c.formatDate(e, { year: `numeric`, month: `short`, day: `numeric` }) : ``),
      (j = A ? c.formatTime(e, { hour: `numeric`, minute: `2-digit` }) : ``),
      (t[21] = n),
      (t[22] = c),
      (t[23] = r),
      (t[24] = O),
      (t[25] = k),
      (t[26] = A),
      (t[27] = j));
  } else ((O = t[24]), (k = t[25]), (A = t[26]), (j = t[27]));
  let M = j,
    N;
  t[31] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((N = (0, X.jsx)(z, { className: `icon-sm shrink-0` })), (t[31] = N))
    : (N = t[31]);
  let P;
  t[32] === D
    ? (P = t[33])
    : ((P =
        D.length > 0
          ? D
          : (0, X.jsx)(y, {
              id: `settings.dataControls.archivedChats.untitled`,
              defaultMessage: `Untitled chat`,
              description: `Fallback title for an archived chat`,
            })),
      (t[32] = D),
      (t[33] = P));
  let F;
  t[34] === P
    ? (F = t[35])
    : ((F = (0, X.jsxs)(`div`, {
        className: `flex min-w-0 items-center gap-2 text-base font-medium`,
        children: [N, (0, X.jsx)(`span`, { className: `truncate`, children: P })],
      })),
      (t[34] = P),
      (t[35] = F));
  let I;
  t[36] !== O || t[37] !== k || t[38] !== A || t[39] !== M
    ? ((I = A
        ? k
          ? (0, X.jsx)(y, {
              id: `settings.dataControls.archivedChats.dateTimeWithEnvironment`,
              defaultMessage: `{date}, {time} • {environment}`,
              description: `Date, time, and environment name for an archived cloud chat`,
              values: { date: O, time: M, environment: k },
            })
          : (0, X.jsx)(y, {
              id: `settings.dataControls.archivedChats.dateTime`,
              defaultMessage: `{date}, {time}`,
              description: `Date and time for an archived chat`,
              values: { date: O, time: M },
            })
        : null),
      (t[36] = O),
      (t[37] = k),
      (t[38] = A),
      (t[39] = M),
      (t[40] = I))
    : (I = t[40]);
  let L;
  t[41] === I
    ? (L = t[42])
    : ((L = (0, X.jsx)(`div`, {
        className: `mt-1 flex min-w-0 flex-col gap-0.5 text-sm`,
        children: (0, X.jsx)(`div`, {
          className: `truncate text-token-text-secondary`,
          children: I,
        }),
      })),
      (t[41] = I),
      (t[42] = L));
  let R;
  t[43] !== F || t[44] !== L
    ? ((R = (0, X.jsxs)(`div`, {
        className: `min-w-0 flex-1 text-left text-token-text-primary`,
        children: [F, L],
      })),
      (t[43] = F),
      (t[44] = L),
      (t[45] = R))
    : (R = t[45]);
  let B;
  t[46] === S
    ? (B = t[47])
    : ((B = () => {
        S.isPending || S.mutate();
      }),
      (t[46] = S),
      (t[47] = B));
  let V;
  t[48] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((V = (0, X.jsx)(y, { ...K.unarchive })), (t[48] = V))
    : (V = t[48]);
  let H;
  t[49] !== B || t[50] !== S.isPending
    ? ((H = (0, X.jsx)(x, {
        className: `shrink-0`,
        color: `secondary`,
        size: `toolbar`,
        disabled: S.isPending,
        loading: S.isPending,
        onClick: B,
        children: V,
      })),
      (t[49] = B),
      (t[50] = S.isPending),
      (t[51] = H))
    : (H = t[51]);
  let U;
  return (
    t[52] !== R || t[53] !== H
      ? ((U = (0, X.jsxs)(`div`, {
          className: `flex w-full items-center justify-between gap-3 border-b border-token-border bg-[var(--color-background-panel,var(--color-token-bg-fog))] px-4 py-3 last:border-b-0 hover:bg-token-list-hover-background`,
          children: [R, H],
        })),
        (t[52] = R),
        (t[53] = H),
        (t[54] = U))
      : (U = t[54]),
    U
  );
}
function pe(e) {
  let t = e.updated_at ?? e.created_at;
  if (t == null) return null;
  let n = new Date(t * 1e3);
  return Number.isFinite(n.getTime()) ? n : null;
}
function me() {
  let e = (0, G.c)(2),
    t;
  e[0] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((t = [`tasks`, `archived`]), (e[0] = t))
    : (t = e[0]);
  let n;
  return (
    e[1] === Symbol.for(`react.memo_cache_sentinel`)
      ? ((n = {
          queryKey: t,
          initialPageParam: null,
          queryFn: ve,
          getNextPageParam: _e,
          select: he,
          staleTime: f.FIVE_SECONDS,
        }),
        (e[1] = n))
      : (n = e[1]),
    o(n)
  );
}
function he(e) {
  return e.pages.flatMap(ge);
}
function ge(e) {
  return e.items;
}
function _e(e) {
  return e.cursor ?? null;
}
function ve(e) {
  let { pageParam: t } = e;
  return ye(t);
}
async function ye(e) {
  try {
    return await w.safeGet(`/wham/tasks/list`, {
      parameters: { query: { limit: 20, cursor: e, task_filter: `archived` } },
    });
  } catch (e) {
    if (e instanceof l && (e.status === 401 || e.status === 403 || e.status === 404))
      return { items: [], cursor: null };
    throw e;
  }
}
var Q = b({
  searchArchivedChats: {
    id: `settings.dataControls.archivedChats.search.placeholder`,
    defaultMessage: `Search archived chats`,
    description: `Placeholder for the archived chats search input`,
  },
  filterArchivedChats: {
    id: `settings.dataControls.archivedChats.filter.ariaLabel`,
    defaultMessage: `Filter archived chats`,
    description: `Accessible label for the archived chats filter dropdown`,
  },
  groupArchivedChats: {
    id: `settings.dataControls.archivedChats.group.ariaLabel`,
    defaultMessage: `Group archived chats`,
    description: `Accessible label for the archived chats grouping dropdown`,
  },
  type: {
    id: `settings.dataControls.archivedChats.filter.section.type`,
    defaultMessage: `Type`,
    description: `Section label for archived chat type filter options`,
  },
  sortBy: {
    id: `settings.dataControls.archivedChats.filter.section.sort`,
    defaultMessage: `Sort by`,
    description: `Section label for archived chat sort options`,
  },
  allChats: {
    id: `settings.dataControls.archivedChats.filter.all`,
    defaultMessage: `All chats`,
    description: `Filter option showing every archived chat`,
  },
  local: {
    id: `settings.dataControls.archivedChats.filter.local`,
    defaultMessage: `Local`,
    description: `Filter option showing local archived chats`,
  },
  cloud: {
    id: `settings.dataControls.archivedChats.filter.cloud`,
    defaultMessage: `Cloud`,
    description: `Filter option showing cloud archived chats`,
  },
  project: {
    id: `settings.dataControls.archivedChats.group.project`,
    defaultMessage: `Project`,
    description: `Grouping option that groups archived chats by project`,
  },
  none: {
    id: `settings.dataControls.archivedChats.group.none`,
    defaultMessage: `None`,
    description: `Grouping option that shows archived chats in one list`,
  },
  updated: {
    id: `settings.dataControls.archivedChats.sort.updated`,
    defaultMessage: `Updated`,
    description: `Sort option for archived chats by last updated time`,
  },
  created: {
    id: `settings.dataControls.archivedChats.sort.created`,
    defaultMessage: `Created`,
    description: `Sort option for archived chats by creation time`,
  },
  alphabetical: {
    id: `settings.dataControls.archivedChats.sort.alphabetical`,
    defaultMessage: `Alphabetical`,
    description: `Sort option for archived chats by title`,
  },
  deleteProjectChats: {
    id: `settings.dataControls.archivedChats.deleteProjectChats`,
    defaultMessage: `Delete all in project`,
    description: `Menu item label for deleting local archived chats in a project`,
  },
});
function $() {
  let e = (0, G.c)(16),
    { selectedHostId: t } = B(),
    n;
  e[0] === t
    ? (n = e[1])
    : ((n = {
        queryKey: [`archived-threads`, t],
        queryFn: () => m(`list-archived-threads`, { hostId: t }),
        enabled: !0,
        staleTime: f.FIVE_SECONDS,
      }),
      (e[0] = t),
      (e[1] = n));
  let { data: r, isLoading: i, isError: a } = c(n),
    {
      data: o,
      fetchNextPage: s,
      hasNextPage: l,
      isLoading: u,
      isFetchNextPageError: d,
      isFetchingNextPage: p,
      isError: h,
    } = me(),
    g;
  e[2] !== o || e[3] !== r || e[4] !== t
    ? ((g = ce({ cloudTasks: o, localThreads: t === `local` ? r : [] })),
      (e[2] = o),
      (e[3] = r),
      (e[4] = t),
      (e[5] = g))
    : (g = e[5]);
  let _ = g,
    v = l && !d,
    y = i || u,
    b = a || (o == null && h),
    x;
  e[6] === s
    ? (x = e[7])
    : ((x = () => {
        s();
      }),
      (e[6] = s),
      (e[7] = x));
  let S;
  return (
    e[8] !== _ ||
    e[9] !== p ||
    e[10] !== t ||
    e[11] !== v ||
    e[12] !== y ||
    e[13] !== b ||
    e[14] !== x
      ? ((S = (0, X.jsx)(be, {
          archivedChats: _,
          hostId: t,
          hasNextPage: v,
          isLoading: y,
          isFetchingNextPage: p,
          isError: b,
          onLoadNextPage: x,
        })),
        (e[8] = _),
        (e[9] = p),
        (e[10] = t),
        (e[11] = v),
        (e[12] = y),
        (e[13] = b),
        (e[14] = x),
        (e[15] = S))
      : (S = e[15]),
    S
  );
}
function be(e) {
  let t = (0, G.c)(118),
    {
      archivedChats: n,
      hostId: r,
      hasNextPage: o,
      isLoading: s,
      isFetchingNextPage: c,
      isError: l,
      onLoadNextPage: f,
    } = e,
    h = i(a),
    _ = d(),
    b = v(),
    S = E(),
    C = N(),
    [w, D] = (0, se.useState)(null),
    [O, k] = (0, se.useState)(``),
    [A, j] = (0, se.useState)(`project`),
    [M, P] = (0, se.useState)(`all`),
    [F, I] = (0, se.useState)(`updated`),
    L;
  t[0] !== n || t[1] !== A || t[2] !== M || t[3] !== O || t[4] !== F
    ? ((L = ue({ entries: n, groupKey: A, kindFilter: M, searchQuery: O, sortKey: F })),
      (t[0] = n),
      (t[1] = A),
      (t[2] = M),
      (t[3] = O),
      (t[4] = F),
      (t[5] = L))
    : (L = t[5]);
  let z = L,
    B = M !== `local` && o,
    V;
  t[6] !== n || t[7] !== r
    ? ((V = r === `local` && n.some(we)), (t[6] = n), (t[7] = r), (t[8] = V))
    : (V = t[8]);
  let U = V,
    W;
  t[9] !== r || t[10] !== S || t[11] !== C || t[12] !== h
    ? ((W = (e) => {
        (p(e.id), r === `local` && C(e.id));
      }),
      (t[9] = r),
      (t[10] = S),
      (t[11] = C),
      (t[12] = h),
      (t[13] = W))
    : (W = t[13]);
  let ae = W,
    oe,
    ce;
  t[14] === r
    ? ((oe = t[15]), (ce = t[16]))
    : ((oe = [`unarchive-thread`, r]),
      (ce = async (e) => (
        await m(`unarchive-conversation`, { hostId: r, conversationId: p(e.id) }),
        e
      )),
      (t[14] = r),
      (t[15] = oe),
      (t[16] = ce));
  let le;
  t[17] !== r || t[18] !== _ || t[19] !== h
    ? ((le = async (e) => {
        (h
          .get(T)
          .info(
            (0, X.jsx)(y, {
              id: `settings.dataControls.archivedChats.unarchiving`,
              defaultMessage: `Unarchiving chat…`,
              description: `Toast shown while unarchiving a chat`,
            }),
            { id: `unarchive-thread-${e.id}`, duration: 0, hasCloseButton: !1 },
          ),
          await _.cancelQueries({ queryKey: [`archived-threads`, r] }));
        let t = _.getQueryData([`archived-threads`, r]) ?? [];
        return (
          _.setQueryData(
            [`archived-threads`, r],
            t.filter((t) => t.id !== e.id),
          ),
          { previousThreads: t }
        );
      }),
      (t[17] = r),
      (t[18] = _),
      (t[19] = h),
      (t[20] = le))
    : (le = t[20]);
  let q;
  t[21] !== r || t[22] !== b || t[23] !== _ || t[24] !== h
    ? ((q = (e, t, n) => {
        (n?.previousThreads && _.setQueryData([`archived-threads`, r], n.previousThreads),
          h.get(T).danger(b.formatMessage(K.unarchiveError), { id: `unarchive-thread-${t.id}` }));
      }),
      (t[21] = r),
      (t[22] = b),
      (t[23] = _),
      (t[24] = h),
      (t[25] = q))
    : (q = t[25]);
  let J;
  t[26] !== r || t[27] !== ae || t[28] !== h
    ? ((J = async (e) => {
        let t = p(e.id),
          n = await m(`hydrate-pinned-threads`, { hostId: r, threadIds: [e.id] }).then(
            () => h.get(g, t),
            Ce,
          );
        h.get(T).info(
          n
            ? (0, X.jsxs)(`span`, {
                children: [
                  (0, X.jsx)(y, { ...K.unarchiveSuccessPlain }),
                  (0, X.jsx)(`button`, {
                    className: `pointer-events-auto ml-1 cursor-interaction text-token-link underline-offset-2 hover:underline`,
                    type: `button`,
                    onClick: () => {
                      ae(e);
                    },
                    children: (0, X.jsx)(y, { ...K.viewNow }),
                  }),
                ],
              })
            : (0, X.jsx)(y, { ...K.unarchiveSuccessPlain }),
          { id: `unarchive-thread-${e.id}` },
        );
      }),
      (t[26] = r),
      (t[27] = ae),
      (t[28] = h),
      (t[29] = J))
    : (J = t[29]);
  let Y;
  t[30] !== r || t[31] !== _
    ? ((Y = () => {
        _.invalidateQueries({ queryKey: [`archived-threads`, r] });
      }),
      (t[30] = r),
      (t[31] = _),
      (t[32] = Y))
    : (Y = t[32]);
  let fe;
  t[33] !== oe || t[34] !== ce || t[35] !== le || t[36] !== q || t[37] !== J || t[38] !== Y
    ? ((fe = {
        mutationKey: oe,
        mutationFn: ce,
        onMutate: le,
        onError: q,
        onSuccess: J,
        onSettled: Y,
      }),
      (t[33] = oe),
      (t[34] = ce),
      (t[35] = le),
      (t[36] = q),
      (t[37] = J),
      (t[38] = Y),
      (t[39] = fe))
    : (fe = t[39]);
  let Z = u(fe),
    pe,
    me;
  t[40] === r
    ? ((pe = t[41]), (me = t[42]))
    : ((pe = [`delete-archived-conversations`, r]),
      (me = async (e) =>
        e.kind === `all`
          ? m(`delete-all-archived-conversations`, { hostId: r })
          : e.kind === `project`
            ? (
                await Promise.all(
                  e.threadIds.map((e) =>
                    m(`delete-archived-conversation`, { hostId: r, conversationId: p(e) }),
                  ),
                )
              ).flat()
            : m(`delete-archived-conversation`, { hostId: r, conversationId: p(e.thread.id) })),
      (t[40] = r),
      (t[41] = pe),
      (t[42] = me));
  let he;
  t[43] !== r || t[44] !== _ || t[45] !== h
    ? ((he = async (e) => {
        let t = e.kind === `single` ? [e.thread.id] : e.threadIds;
        (h
          .get(T)
          .info(
            e.kind === `single`
              ? (0, X.jsx)(y, { ...K.deleting })
              : (0, X.jsx)(y, { ...K.deletingAll }),
            { id: Pe(e), duration: 0, hasCloseButton: !1 },
          ),
          await _.cancelQueries({ queryKey: [`archived-threads`, r] }));
        let n = _.getQueryData([`archived-threads`, r]) ?? [];
        return (
          _.setQueryData(
            [`archived-threads`, r],
            n.filter((e) => !t.includes(e.id)),
          ),
          { deletingThreadIds: t, previousThreads: n }
        );
      }),
      (t[43] = r),
      (t[44] = _),
      (t[45] = h),
      (t[46] = he))
    : (he = t[46]);
  let ge;
  t[47] !== r || t[48] !== b || t[49] !== _ || t[50] !== h
    ? ((ge = (e, t, n) => {
        (n?.previousThreads && _.setQueryData([`archived-threads`, r], n.previousThreads),
          h
            .get(T)
            .danger(b.formatMessage(t.kind === `single` ? K.deleteError : K.deleteAllError), {
              id: Pe(t),
            }));
      }),
      (t[47] = r),
      (t[48] = b),
      (t[49] = _),
      (t[50] = h),
      (t[51] = ge))
    : (ge = t[51]);
  let _e;
  t[52] !== b || t[53] !== h
    ? ((_e = (e, t) => {
        if (t.kind !== `single`) {
          h.get(T).success(
            b.formatMessage(
              {
                id: `settings.dataControls.archivedChats.deleteAllSuccess`,
                defaultMessage: `Deleted {count, plural, one {# archived chat} other {# archived chats}}`,
                description: `Success toast after deleting archived chats`,
              },
              { count: e.length || t.threadIds.length },
            ),
            { id: Pe(t) },
          );
          return;
        }
        h.get(T).success(b.formatMessage(K.deleteSuccessPlain), { id: Pe(t) });
      }),
      (t[52] = b),
      (t[53] = h),
      (t[54] = _e))
    : (_e = t[54]);
  let ve;
  t[55] !== r || t[56] !== _
    ? ((ve = () => {
        _.invalidateQueries({ queryKey: [`archived-threads`, r] });
      }),
      (t[55] = r),
      (t[56] = _),
      (t[57] = ve))
    : (ve = t[57]);
  let ye;
  t[58] !== pe || t[59] !== me || t[60] !== he || t[61] !== ge || t[62] !== _e || t[63] !== ve
    ? ((ye = {
        mutationKey: pe,
        mutationFn: me,
        onMutate: he,
        onError: ge,
        onSuccess: _e,
        onSettled: ve,
      }),
      (t[58] = pe),
      (t[59] = me),
      (t[60] = he),
      (t[61] = ge),
      (t[62] = _e),
      (t[63] = ve),
      (t[64] = ye))
    : (ye = t[64]);
  let Q = u(ye),
    $ = Z.isPending || Q.isPending,
    be;
  if (s) {
    let e;
    (t[65] === Symbol.for(`react.memo_cache_sentinel`)
      ? ((e = (0, X.jsx)(ne, {
          children: (0, X.jsx)(te, {
            label: (0, X.jsx)(y, {
              id: `settings.dataControls.archivedChats.loading`,
              defaultMessage: `Loading archived chats…`,
              description: `Loading state label for archived chats list`,
            }),
            control: null,
          }),
        })),
        (t[65] = e))
      : (e = t[65]),
      (be = e));
  } else if (l) {
    let e;
    (t[66] === Symbol.for(`react.memo_cache_sentinel`)
      ? ((e = (0, X.jsx)(ne, {
          children: (0, X.jsx)(te, {
            label: (0, X.jsx)(y, {
              id: `settings.dataControls.archivedChats.error`,
              defaultMessage: `Could not load archived chats.`,
              description: `Error state label for archived chats list`,
            }),
            control: null,
          }),
        })),
        (t[66] = e))
      : (e = t[66]),
      (be = e));
  } else if (n.length === 0) {
    let e;
    (t[67] === Symbol.for(`react.memo_cache_sentinel`)
      ? ((e = (0, X.jsx)(ne, {
          children: (0, X.jsx)(te, {
            label: (0, X.jsx)(y, {
              id: `settings.dataControls.archivedChats.empty`,
              defaultMessage: `No archived chats.`,
              description: `Empty state label for archived chats list`,
            }),
            control: null,
          }),
        })),
        (t[67] = e))
      : (e = t[67]),
      (be = e));
  } else {
    let e;
    t[68] !== A || t[69] !== M || t[70] !== F
      ? ((e = (0, X.jsx)(`div`, {
          className: `rounded-t-[7px] bg-token-main-surface-primary/95 p-3 backdrop-blur-md`,
          children: (0, X.jsx)(Te, {
            groupKey: A,
            kindFilter: M,
            sortKey: F,
            onGroupKeyChange: j,
            onKindFilterChange: P,
            onSearchQueryChange: k,
            onSortKeyChange: I,
          }),
        })),
        (t[68] = A),
        (t[69] = M),
        (t[70] = F),
        (t[71] = e))
      : (e = t[71]);
    let n;
    t[72] !== B || t[73] !== c || t[74] !== z.length
      ? ((n =
          z.length === 0 && !B && !c
            ? (0, X.jsx)(`div`, {
                className: `px-4 py-3 text-base text-token-text-secondary`,
                children: (0, X.jsx)(`span`, {
                  children: (0, X.jsx)(y, {
                    id: `settings.dataControls.archivedChats.noMatches`,
                    defaultMessage: `No matching archived chats`,
                    description: `Empty state shown when archived chat search has no matches`,
                  }),
                }),
              })
            : null),
        (t[72] = B),
        (t[73] = c),
        (t[74] = z.length),
        (t[75] = n))
      : (n = t[75]);
    let i;
    if (t[76] !== $ || t[77] !== A || t[78] !== r || t[79] !== Z || t[80] !== z) {
      let e;
      (t[82] !== $ || t[83] !== A || t[84] !== r || t[85] !== Z
        ? ((e = (e) =>
            (0, X.jsx)(
              Oe,
              {
                group: e,
                hostId: r,
                isDeleteDisabled: $,
                projectArchivedThreads: de(e.entries, e.id),
                unarchivingThreadId: Z.variables?.id ?? null,
                isUnarchiving: Z.isPending,
                showHeader: A === `project`,
                onDelete: (e) => {
                  D({ kind: `single`, thread: e });
                },
                onUnarchive: (e) => {
                  Z.mutate(e);
                },
                onDeleteProject: (e, t) => {
                  D({ kind: `project`, projectLabel: e, threads: t });
                },
              },
              e.id,
            )),
          (t[82] = $),
          (t[83] = A),
          (t[84] = r),
          (t[85] = Z),
          (t[86] = e))
        : (e = t[86]),
        (i = z.map(e)),
        (t[76] = $),
        (t[77] = A),
        (t[78] = r),
        (t[79] = Z),
        (t[80] = z),
        (t[81] = i));
    } else i = t[81];
    let a;
    t[87] !== B || t[88] !== c || t[89] !== f
      ? ((a = (0, X.jsx)(ie, { hasNextPage: B, isFetchingNextPage: c, onLoadNextPage: f })),
        (t[87] = B),
        (t[88] = c),
        (t[89] = f),
        (t[90] = a))
      : (a = t[90]);
    let o;
    t[91] !== n || t[92] !== i || t[93] !== a
      ? ((o = (0, X.jsx)(`div`, {
          className: `min-h-0 flex-1 overflow-y-auto`,
          children: (0, X.jsxs)(`div`, { className: `flex flex-col`, children: [n, i, a] }),
        })),
        (t[91] = n),
        (t[92] = i),
        (t[93] = a),
        (t[94] = o))
      : (o = t[94]);
    let s;
    (t[95] !== e || t[96] !== o
      ? ((s = (0, X.jsxs)(ne, { className: `isolate max-h-[min(80vh)]`, children: [e, o] })),
        (t[95] = e),
        (t[96] = o),
        (t[97] = s))
      : (s = t[97]),
      (be = s));
  }
  let Ee;
  t[98] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((Ee = (0, X.jsx)(H, { slug: `data-controls` })), (t[98] = Ee))
    : (Ee = t[98]);
  let De;
  t[99] !== $ || t[100] !== U || t[101] !== l || t[102] !== s
    ? ((De =
        U && !s && !l
          ? (0, X.jsxs)(x, {
              color: `danger`,
              size: `toolbar`,
              disabled: $,
              onClick: () => {
                D({ kind: `all` });
              },
              children: [
                (0, X.jsx)(R, { className: `icon-xs shrink-0` }),
                (0, X.jsx)(y, { ...K.deleteAll }),
              ],
            })
          : null),
      (t[99] = $),
      (t[100] = U),
      (t[101] = l),
      (t[102] = s),
      (t[103] = De))
    : (De = t[103]);
  let ke;
  t[104] === be
    ? (ke = t[105])
    : ((ke = (0, X.jsx)(re, {
        className: `gap-2`,
        children: (0, X.jsx)(re.Content, { children: be }),
      })),
      (t[104] = be),
      (t[105] = ke));
  let je;
  t[106] !== n || t[107] !== Q || t[108] !== w
    ? ((je = () => {
        if (w != null) {
          if ((D(null), w.kind === `all`)) {
            Q.mutate({ kind: `all`, threadIds: n.flatMap(Se) });
            return;
          }
          if (w.kind === `project`) {
            Q.mutate({
              kind: `project`,
              projectLabel: w.projectLabel,
              threadIds: w.threads.map(xe),
            });
            return;
          }
          Q.mutate({ kind: `single`, thread: w.thread });
        }
      }),
      (t[106] = n),
      (t[107] = Q),
      (t[108] = w),
      (t[109] = je))
    : (je = t[109]);
  let Me;
  t[110] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((Me = (e) => {
        e || D(null);
      }),
      (t[110] = Me))
    : (Me = t[110]);
  let Ne;
  t[111] !== w || t[112] !== je
    ? ((Ne = (0, X.jsx)(Ae, { state: w, onConfirm: je, onOpenChange: Me })),
      (t[111] = w),
      (t[112] = je),
      (t[113] = Ne))
    : (Ne = t[113]);
  let Fe;
  return (
    t[114] !== De || t[115] !== ke || t[116] !== Ne
      ? ((Fe = (0, X.jsxs)(ee, {
          title: Ee,
          titleStackClassName: `gap-1.5`,
          action: De,
          children: [ke, Ne],
        })),
        (t[114] = De),
        (t[115] = ke),
        (t[116] = Ne),
        (t[117] = Fe))
      : (Fe = t[117]),
    Fe
  );
}
function xe(e) {
  return e.id;
}
function Se(e) {
  return e.kind === `local` ? [e.thread.id] : [];
}
function Ce() {
  return !1;
}
function we(e) {
  return e.kind === `local`;
}
function Te(e) {
  let t = (0, G.c)(26),
    {
      groupKey: n,
      kindFilter: r,
      sortKey: i,
      onGroupKeyChange: a,
      onKindFilterChange: o,
      onSearchQueryChange: s,
      onSortKeyChange: c,
    } = e,
    l = v(),
    [u, d] = (0, se.useState)(``),
    f;
  t[0] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((f = (0, X.jsx)(F, {
        className: `icon-xs shrink-0 text-token-input-placeholder-foreground`,
      })),
      (t[0] = f))
    : (f = t[0]);
  let p;
  t[1] === l ? (p = t[2]) : ((p = l.formatMessage(Q.searchArchivedChats)), (t[1] = l), (t[2] = p));
  let m;
  t[3] === l ? (m = t[4]) : ((m = l.formatMessage(Q.searchArchivedChats)), (t[3] = l), (t[4] = m));
  let h;
  t[5] === s
    ? (h = t[6])
    : ((h = (e) => {
        let t = e.currentTarget.value;
        (d(t),
          (0, se.startTransition)(() => {
            s(t);
          }));
      }),
      (t[5] = s),
      (t[6] = h));
  let g;
  t[7] !== u || t[8] !== p || t[9] !== m || t[10] !== h
    ? ((g = (0, X.jsxs)(`div`, {
        className: `flex h-[28px] min-h-[28px] min-w-0 flex-1 items-center gap-2 rounded-lg border border-transparent bg-token-foreground/5 px-2 text-base leading-[18px] text-token-foreground`,
        children: [
          f,
          (0, X.jsx)(`input`, {
            className: `min-w-0 flex-1 appearance-none border-0 bg-transparent p-0 text-token-foreground outline-none placeholder:text-token-input-placeholder-foreground`,
            "aria-label": p,
            placeholder: m,
            value: u,
            onChange: h,
          }),
        ],
      })),
      (t[7] = u),
      (t[8] = p),
      (t[9] = m),
      (t[10] = h),
      (t[11] = g))
    : (g = t[11]);
  let _;
  t[12] !== r || t[13] !== o || t[14] !== c || t[15] !== i
    ? ((_ = (0, X.jsx)(Ee, {
        kindFilter: r,
        sortKey: i,
        onKindFilterChange: o,
        onSortKeyChange: c,
      })),
      (t[12] = r),
      (t[13] = o),
      (t[14] = c),
      (t[15] = i),
      (t[16] = _))
    : (_ = t[16]);
  let y;
  t[17] !== n || t[18] !== a
    ? ((y = (0, X.jsx)(De, { groupKey: n, onGroupKeyChange: a })),
      (t[17] = n),
      (t[18] = a),
      (t[19] = y))
    : (y = t[19]);
  let b;
  t[20] !== _ || t[21] !== y
    ? ((b = (0, X.jsxs)(oe, { className: `shrink-0`, children: [_, y] })),
      (t[20] = _),
      (t[21] = y),
      (t[22] = b))
    : (b = t[22]);
  let x;
  return (
    t[23] !== g || t[24] !== b
      ? ((x = (0, X.jsxs)(`div`, {
          className: `flex flex-col gap-2 md:flex-row md:items-center`,
          children: [g, b],
        })),
        (t[23] = g),
        (t[24] = b),
        (t[25] = x))
      : (x = t[25]),
    x
  );
}
function Ee(e) {
  let t = (0, G.c)(57),
    { kindFilter: n, sortKey: r, onKindFilterChange: i, onSortKeyChange: a } = e,
    o = v(),
    s;
  t[0] === o ? (s = t[1]) : ((s = o.formatMessage(Q.filterArchivedChats)), (t[0] = o), (t[1] = s));
  let c;
  t[2] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((c = (0, X.jsx)(ae, { className: `icon-xs shrink-0` })), (t[2] = c))
    : (c = t[2]);
  let l;
  t[3] === n ? (l = t[4]) : ((l = je(n)), (t[3] = n), (t[4] = l));
  let u;
  t[5] === l
    ? (u = t[6])
    : ((u = (0, X.jsx)(`span`, { className: `truncate`, children: (0, X.jsx)(y, { ...l }) })),
      (t[5] = l),
      (t[6] = u));
  let d;
  t[7] !== s || t[8] !== u
    ? ((d = (0, X.jsxs)(U, { className: `w-[150px] md:w-36`, "aria-label": s, children: [c, u] })),
      (t[7] = s),
      (t[8] = u),
      (t[9] = d))
    : (d = t[9]);
  let f;
  t[10] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((f = (0, X.jsx)(I.SectionLabel, { children: (0, X.jsx)(y, { ...Q.type }) })), (t[10] = f))
    : (f = t[10]);
  let p = n === `all` ? S : void 0,
    m;
  t[11] === i
    ? (m = t[12])
    : ((m = () => {
        i(`all`);
      }),
      (t[11] = i),
      (t[12] = m));
  let h;
  t[13] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((h = (0, X.jsx)(y, { ...Q.allChats })), (t[13] = h))
    : (h = t[13]);
  let g;
  t[14] !== p || t[15] !== m
    ? ((g = (0, X.jsx)(I.Item, { RightIcon: p, onSelect: m, children: h })),
      (t[14] = p),
      (t[15] = m),
      (t[16] = g))
    : (g = t[16]);
  let _ = n === `local` ? S : void 0,
    b;
  t[17] === i
    ? (b = t[18])
    : ((b = () => {
        i(`local`);
      }),
      (t[17] = i),
      (t[18] = b));
  let x;
  t[19] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((x = (0, X.jsx)(y, { ...Q.local })), (t[19] = x))
    : (x = t[19]);
  let C;
  t[20] !== _ || t[21] !== b
    ? ((C = (0, X.jsx)(I.Item, { RightIcon: _, onSelect: b, children: x })),
      (t[20] = _),
      (t[21] = b),
      (t[22] = C))
    : (C = t[22]);
  let w = n === `cloud` ? S : void 0,
    T;
  t[23] === i
    ? (T = t[24])
    : ((T = () => {
        i(`cloud`);
      }),
      (t[23] = i),
      (t[24] = T));
  let E;
  t[25] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((E = (0, X.jsx)(y, { ...Q.cloud })), (t[25] = E))
    : (E = t[25]);
  let D;
  t[26] !== w || t[27] !== T
    ? ((D = (0, X.jsx)(I.Item, { RightIcon: w, onSelect: T, children: E })),
      (t[26] = w),
      (t[27] = T),
      (t[28] = D))
    : (D = t[28]);
  let O;
  t[29] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((O = (0, X.jsx)(I.Separator, {})), (t[29] = O))
    : (O = t[29]);
  let k;
  t[30] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((k = (0, X.jsx)(I.SectionLabel, { children: (0, X.jsx)(y, { ...Q.sortBy }) })), (t[30] = k))
    : (k = t[30]);
  let A = r === `updated` ? S : void 0,
    j;
  t[31] === a
    ? (j = t[32])
    : ((j = () => {
        a(`updated`);
      }),
      (t[31] = a),
      (t[32] = j));
  let M;
  t[33] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((M = (0, X.jsx)(y, { ...Q.updated })), (t[33] = M))
    : (M = t[33]);
  let N;
  t[34] !== A || t[35] !== j
    ? ((N = (0, X.jsx)(I.Item, { RightIcon: A, onSelect: j, children: M })),
      (t[34] = A),
      (t[35] = j),
      (t[36] = N))
    : (N = t[36]);
  let P = r === `created` ? S : void 0,
    F;
  t[37] === a
    ? (F = t[38])
    : ((F = () => {
        a(`created`);
      }),
      (t[37] = a),
      (t[38] = F));
  let R;
  t[39] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((R = (0, X.jsx)(y, { ...Q.created })), (t[39] = R))
    : (R = t[39]);
  let z;
  t[40] !== P || t[41] !== F
    ? ((z = (0, X.jsx)(I.Item, { RightIcon: P, onSelect: F, children: R })),
      (t[40] = P),
      (t[41] = F),
      (t[42] = z))
    : (z = t[42]);
  let B = r === `alphabetical` ? S : void 0,
    V;
  t[43] === a
    ? (V = t[44])
    : ((V = () => {
        a(`alphabetical`);
      }),
      (t[43] = a),
      (t[44] = V));
  let H;
  t[45] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((H = (0, X.jsx)(y, { ...Q.alphabetical })), (t[45] = H))
    : (H = t[45]);
  let ee;
  t[46] !== B || t[47] !== V
    ? ((ee = (0, X.jsx)(I.Item, { RightIcon: B, onSelect: V, children: H })),
      (t[46] = B),
      (t[47] = V),
      (t[48] = ee))
    : (ee = t[48]);
  let W;
  return (
    t[49] !== g ||
    t[50] !== C ||
    t[51] !== D ||
    t[52] !== N ||
    t[53] !== z ||
    t[54] !== ee ||
    t[55] !== d
      ? ((W = (0, X.jsxs)(L, {
          side: `bottom`,
          align: `end`,
          contentWidth: `menuNarrow`,
          triggerButton: d,
          children: [f, g, C, D, O, k, N, z, ee],
        })),
        (t[49] = g),
        (t[50] = C),
        (t[51] = D),
        (t[52] = N),
        (t[53] = z),
        (t[54] = ee),
        (t[55] = d),
        (t[56] = W))
      : (W = t[56]),
    W
  );
}
function De(e) {
  let t = (0, G.c)(26),
    { groupKey: n, onGroupKeyChange: r } = e,
    i = v(),
    a;
  t[0] === i ? (a = t[1]) : ((a = i.formatMessage(Q.groupArchivedChats)), (t[0] = i), (t[1] = a));
  let o;
  t[2] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((o = (0, X.jsx)(P, { className: `icon-xs shrink-0` })), (t[2] = o))
    : (o = t[2]);
  let s;
  t[3] === n ? (s = t[4]) : ((s = Me(n)), (t[3] = n), (t[4] = s));
  let c;
  t[5] === s
    ? (c = t[6])
    : ((c = (0, X.jsx)(`span`, { className: `truncate`, children: (0, X.jsx)(y, { ...s }) })),
      (t[5] = s),
      (t[6] = c));
  let l;
  t[7] !== a || t[8] !== c
    ? ((l = (0, X.jsxs)(U, { className: `w-[150px] md:w-36`, "aria-label": a, children: [o, c] })),
      (t[7] = a),
      (t[8] = c),
      (t[9] = l))
    : (l = t[9]);
  let u = n === `project` ? S : void 0,
    d;
  t[10] === r
    ? (d = t[11])
    : ((d = () => {
        r(`project`);
      }),
      (t[10] = r),
      (t[11] = d));
  let f;
  t[12] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((f = (0, X.jsx)(y, { ...Q.project })), (t[12] = f))
    : (f = t[12]);
  let p;
  t[13] !== u || t[14] !== d
    ? ((p = (0, X.jsx)(I.Item, { RightIcon: u, onSelect: d, children: f })),
      (t[13] = u),
      (t[14] = d),
      (t[15] = p))
    : (p = t[15]);
  let m = n === `none` ? S : void 0,
    h;
  t[16] === r
    ? (h = t[17])
    : ((h = () => {
        r(`none`);
      }),
      (t[16] = r),
      (t[17] = h));
  let g;
  t[18] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((g = (0, X.jsx)(y, { ...Q.none })), (t[18] = g))
    : (g = t[18]);
  let _;
  t[19] !== m || t[20] !== h
    ? ((_ = (0, X.jsx)(I.Item, { RightIcon: m, onSelect: h, children: g })),
      (t[19] = m),
      (t[20] = h),
      (t[21] = _))
    : (_ = t[21]);
  let b;
  return (
    t[22] !== _ || t[23] !== l || t[24] !== p
      ? ((b = (0, X.jsxs)(L, {
          side: `bottom`,
          align: `end`,
          contentWidth: `menuNarrow`,
          triggerButton: l,
          children: [p, _],
        })),
        (t[22] = _),
        (t[23] = l),
        (t[24] = p),
        (t[25] = b))
      : (b = t[25]),
    b
  );
}
function Oe(e) {
  let t = (0, G.c)(29),
    {
      group: n,
      hostId: r,
      isDeleteDisabled: i,
      projectArchivedThreads: a,
      unarchivingThreadId: o,
      isUnarchiving: s,
      showHeader: c,
      onDelete: l,
      onDeleteProject: u,
      onUnarchive: d,
    } = e,
    f = v(),
    p = r === `local` && a.length > 0,
    m;
  t[0] !== p ||
  t[1] !== n.entries.length ||
  t[2] !== n.projectLabel ||
  t[3] !== f ||
  t[4] !== i ||
  t[5] !== u ||
  t[6] !== a ||
  t[7] !== c
    ? ((m = c
        ? (0, X.jsxs)(`div`, {
            className: `sticky top-0 z-10 flex items-center justify-between gap-3 bg-token-main-surface-primary/80 px-4 py-1.5 shadow-sm backdrop-blur-md`,
            children: [
              (0, X.jsxs)(`span`, {
                className: `flex min-w-0 items-center gap-2 text-base text-token-description-foreground`,
                children: [
                  (0, X.jsx)(D, { className: `icon-xs shrink-0` }),
                  (0, X.jsx)(`span`, {
                    className: `truncate`,
                    children:
                      n.projectLabel == null
                        ? (0, X.jsx)(y, {
                            id: `settings.dataControls.archivedChats.noProject`,
                            defaultMessage: `No project`,
                            description: `Fallback project header for archived chats without a project`,
                          })
                        : Ne(n.projectLabel),
                  }),
                ],
              }),
              (0, X.jsxs)(`div`, {
                className: `ml-auto flex shrink-0 items-center gap-1`,
                children: [
                  (0, X.jsx)(`span`, {
                    className: `text-base text-token-description-foreground`,
                    children: (0, X.jsx)(y, {
                      id: `settings.dataControls.archivedChats.groupCount`,
                      defaultMessage: `{count, plural, one {# chat} other {# chats}}`,
                      description: `Archived chat count shown in a project group header`,
                      values: { count: n.entries.length },
                    }),
                  }),
                  p
                    ? (0, X.jsx)(L, {
                        align: `end`,
                        contentWidth: `menu`,
                        triggerButton: (0, X.jsx)(W, {
                          className: `text-token-description-foreground hover:bg-token-list-hover-background hover:text-token-foreground`,
                          label: f.formatMessage({
                            id: `settings.dataControls.archivedChats.projectActions`,
                            defaultMessage: `Project actions`,
                            description: `Accessible label for the project archived chats actions menu`,
                          }),
                          size: `toolbar`,
                        }),
                        children: (0, X.jsx)(I.Item, {
                          LeftIcon: R,
                          className: `!text-token-error-foreground`,
                          leftIconClassName: `icon-xs text-token-error-foreground`,
                          disabled: i,
                          onSelect: () => {
                            u(n.projectLabel, a);
                          },
                          children: (0, X.jsx)(y, { ...Q.deleteProjectChats }),
                        }),
                      })
                    : null,
                ],
              }),
            ],
          })
        : null),
      (t[0] = p),
      (t[1] = n.entries.length),
      (t[2] = n.projectLabel),
      (t[3] = f),
      (t[4] = i),
      (t[5] = u),
      (t[6] = a),
      (t[7] = c),
      (t[8] = m))
    : (m = t[8]);
  let h;
  if (
    t[9] !== n.entries ||
    t[10] !== r ||
    t[11] !== i ||
    t[12] !== s ||
    t[13] !== l ||
    t[14] !== d ||
    t[15] !== c ||
    t[16] !== o
  ) {
    let e;
    (t[18] !== r ||
    t[19] !== i ||
    t[20] !== s ||
    t[21] !== l ||
    t[22] !== d ||
    t[23] !== c ||
    t[24] !== o
      ? ((e = (e) => {
          switch (e.kind) {
            case `cloud`:
              return (0, X.jsx)(Z, { archivedTask: e.task, showProjectName: !c }, e.task.id);
            case `local`:
              return (0, X.jsx)(
                ke,
                {
                  archivedThread: e.thread,
                  isDeleteDisabled: i,
                  isUnarchiving: s && o === e.thread.id,
                  showDeleteButton: r === _,
                  showProjectName: !c,
                  onDelete: () => {
                    l(e.thread);
                  },
                  onUnarchive: () => {
                    d(e.thread);
                  },
                },
                e.thread.id,
              );
          }
        }),
        (t[18] = r),
        (t[19] = i),
        (t[20] = s),
        (t[21] = l),
        (t[22] = d),
        (t[23] = c),
        (t[24] = o),
        (t[25] = e))
      : (e = t[25]),
      (h = n.entries.map(e)),
      (t[9] = n.entries),
      (t[10] = r),
      (t[11] = i),
      (t[12] = s),
      (t[13] = l),
      (t[14] = d),
      (t[15] = c),
      (t[16] = o),
      (t[17] = h));
  } else h = t[17];
  let g;
  return (
    t[26] !== m || t[27] !== h
      ? ((g = (0, X.jsxs)(`div`, { className: `flex flex-col`, children: [m, h] })),
        (t[26] = m),
        (t[27] = h),
        (t[28] = g))
      : (g = t[28]),
    g
  );
}
function ke(e) {
  let t = (0, G.c)(52),
    {
      archivedThread: n,
      isDeleteDisabled: r,
      isUnarchiving: i,
      showDeleteButton: a,
      showProjectName: o,
      onDelete: s,
      onUnarchive: c,
    } = e,
    l = v(),
    u;
  t[0] !== n.name || t[1] !== n.preview
    ? ((u = h(n.name?.trim() || n.preview).trim()), (t[0] = n.name), (t[1] = n.preview), (t[2] = u))
    : (u = t[2]);
  let d = u,
    f,
    p,
    m,
    g;
  if (
    t[3] !== n.createdAt ||
    t[4] !== n.cwd ||
    t[5] !== n.path ||
    t[6] !== n.updatedAt ||
    t[7] !== l ||
    t[8] !== o
  ) {
    let e = new Date(Number(n.updatedAt) * 1e3),
      r = new Date(Number(n.createdAt) * 1e3),
      i = Number.isFinite(e.getTime()) ? e : Number.isFinite(r.getTime()) ? r : null,
      a;
    (t[13] !== n.cwd || t[14] !== n.path || t[15] !== o
      ? ((a = o ? (V(n.cwd) ?? V(n.path)) : null),
        (t[13] = n.cwd),
        (t[14] = n.path),
        (t[15] = o),
        (t[16] = a))
      : (a = t[16]),
      (m = a),
      (p = i != null),
      (f = p ? l.formatDate(i, { year: `numeric`, month: `short`, day: `numeric` }) : ``),
      (g = p ? l.formatTime(i, { hour: `numeric`, minute: `2-digit` }) : ``),
      (t[3] = n.createdAt),
      (t[4] = n.cwd),
      (t[5] = n.path),
      (t[6] = n.updatedAt),
      (t[7] = l),
      (t[8] = o),
      (t[9] = f),
      (t[10] = p),
      (t[11] = m),
      (t[12] = g));
  } else ((f = t[9]), (p = t[10]), (m = t[11]), (g = t[12]));
  let _ = g,
    b;
  t[17] === l
    ? (b = t[18])
    : ((b = l.formatMessage({
        id: `settings.dataControls.archivedChats.deleteAriaLabel`,
        defaultMessage: `Delete archived chat`,
        description: `Aria label for deleting an archived chat`,
      })),
      (t[17] = l),
      (t[18] = b));
  let S = b,
    w;
  t[19] === d
    ? (w = t[20])
    : ((w =
        d.length > 0
          ? d
          : (0, X.jsx)(y, {
              id: `settings.dataControls.archivedChats.untitled`,
              defaultMessage: `Untitled chat`,
              description: `Fallback title for an archived chat`,
            })),
      (t[19] = d),
      (t[20] = w));
  let T;
  t[21] === w
    ? (T = t[22])
    : ((T = (0, X.jsx)(`div`, { className: `truncate text-base font-medium`, children: w })),
      (t[21] = w),
      (t[22] = T));
  let E;
  t[23] !== f || t[24] !== p || t[25] !== m || t[26] !== _
    ? ((E = p
        ? m
          ? (0, X.jsx)(y, {
              id: `settings.dataControls.archivedChats.dateTimeWithRepo`,
              defaultMessage: `{date}, {time} • {repo}`,
              description: `Date, time, and repo name for an archived chat`,
              values: { date: f, time: _, repo: m },
            })
          : (0, X.jsx)(y, {
              id: `settings.dataControls.archivedChats.dateTime`,
              defaultMessage: `{date}, {time}`,
              description: `Date and time for an archived chat`,
              values: { date: f, time: _ },
            })
        : null),
      (t[23] = f),
      (t[24] = p),
      (t[25] = m),
      (t[26] = _),
      (t[27] = E))
    : (E = t[27]);
  let D;
  t[28] === E
    ? (D = t[29])
    : ((D = (0, X.jsx)(`div`, {
        className: `mt-1 flex min-w-0 flex-col gap-0.5 text-sm`,
        children: (0, X.jsx)(`div`, {
          className: `truncate text-token-text-secondary`,
          children: E,
        }),
      })),
      (t[28] = E),
      (t[29] = D));
  let O;
  t[30] !== T || t[31] !== D
    ? ((O = (0, X.jsxs)(`div`, {
        className: `min-w-0 flex-1 text-left text-token-text-primary`,
        children: [T, D],
      })),
      (t[30] = T),
      (t[31] = D),
      (t[32] = O))
    : (O = t[32]);
  let k;
  t[33] !== S || t[34] !== r || t[35] !== s || t[36] !== a
    ? ((k = a
        ? (0, X.jsx)(C, {
            tooltipContent: S,
            children: (0, X.jsx)(x, {
              "aria-label": S,
              className: `text-token-charts-red enabled:hover:bg-token-charts-red/10`,
              color: `ghost`,
              size: `toolbar`,
              uniform: !0,
              disabled: r,
              onClick: s,
              children: (0, X.jsx)(R, { className: `icon-xs` }),
            }),
          })
        : null),
      (t[33] = S),
      (t[34] = r),
      (t[35] = s),
      (t[36] = a),
      (t[37] = k))
    : (k = t[37]);
  let A;
  t[38] !== r || t[39] !== c
    ? ((A = () => {
        r || c();
      }),
      (t[38] = r),
      (t[39] = c),
      (t[40] = A))
    : (A = t[40]);
  let j;
  t[41] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((j = (0, X.jsx)(y, { ...K.unarchive })), (t[41] = j))
    : (j = t[41]);
  let M;
  t[42] !== r || t[43] !== i || t[44] !== A
    ? ((M = (0, X.jsx)(x, {
        className: `shrink-0`,
        color: `secondary`,
        size: `toolbar`,
        disabled: r,
        loading: i,
        onClick: A,
        children: j,
      })),
      (t[42] = r),
      (t[43] = i),
      (t[44] = A),
      (t[45] = M))
    : (M = t[45]);
  let N;
  t[46] !== M || t[47] !== k
    ? ((N = (0, X.jsxs)(`div`, {
        className: `flex shrink-0 items-center gap-2 opacity-0 group-focus-within:opacity-100 group-hover:opacity-100`,
        children: [k, M],
      })),
      (t[46] = M),
      (t[47] = k),
      (t[48] = N))
    : (N = t[48]);
  let P;
  return (
    t[49] !== N || t[50] !== O
      ? ((P = (0, X.jsxs)(`div`, {
          className: `group flex w-full items-center justify-between gap-3 border-b border-token-border bg-[var(--color-background-panel,var(--color-token-bg-fog))] px-4 py-3 last:border-b-0 hover:bg-token-list-hover-background`,
          children: [O, N],
        })),
        (t[49] = N),
        (t[50] = O),
        (t[51] = P))
      : (P = t[51]),
    P
  );
}
function Ae(e) {
  let t = (0, G.c)(23),
    { state: n, onConfirm: r, onOpenChange: i } = e;
  if (n == null) return null;
  let a;
  t[0] === n.kind
    ? (a = t[1])
    : ((a = (0, X.jsx)(O, {
        children: (0, X.jsx)(k, {
          title:
            n.kind === `all`
              ? (0, X.jsx)(y, {
                  id: `settings.dataControls.archivedChats.deleteAllConfirm.title`,
                  defaultMessage: `Delete all archived local chats?`,
                  description: `Title for confirming deletion of all archived local chats`,
                })
              : n.kind === `project`
                ? (0, X.jsx)(y, {
                    id: `settings.dataControls.archivedChats.deleteProjectConfirm.title`,
                    defaultMessage: `Delete all in project?`,
                    description: `Title for confirming deletion of archived local chats in one project`,
                  })
                : (0, X.jsx)(y, {
                    id: `settings.dataControls.archivedChats.deleteConfirm.title`,
                    defaultMessage: `Delete archived chat?`,
                    description: `Title for confirming deletion of an archived chat`,
                  }),
        }),
      })),
      (t[0] = n.kind),
      (t[1] = a));
  let o;
  t[2] !== n.kind || t[3] !== n.threads
    ? ((o = (0, X.jsx)(O, {
        className: `text-token-description-foreground`,
        children: (0, X.jsx)(`p`, {
          children:
            n.kind === `all`
              ? (0, X.jsx)(y, {
                  id: `settings.dataControls.archivedChats.deleteAllConfirm.body`,
                  defaultMessage: `This permanently deletes all local archived chats`,
                  description: `Body copy in the delete all archived local chats confirmation dialog`,
                })
              : n.kind === `project`
                ? (0, X.jsx)(y, {
                    id: `settings.dataControls.archivedChats.deleteProjectConfirm.body`,
                    defaultMessage: `This permanently deletes {count, plural, one {# local archived chat} other {# local archived chats}} in this project`,
                    description: `Body copy in the delete archived local chats in project confirmation dialog`,
                    values: { count: n.threads.length },
                  })
                : (0, X.jsx)(y, {
                    id: `settings.dataControls.archivedChats.deleteConfirm.body`,
                    defaultMessage: `This permanently deletes the archived chat`,
                    description: `Body copy in the delete archived chat confirmation dialog`,
                  }),
        }),
      })),
      (t[2] = n.kind),
      (t[3] = n.threads),
      (t[4] = o))
    : (o = t[4]);
  let s;
  t[5] === i
    ? (s = t[6])
    : ((s = () => {
        i(!1);
      }),
      (t[5] = i),
      (t[6] = s));
  let c;
  t[7] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((c = (0, X.jsx)(y, {
        id: `settings.dataControls.archivedChats.deleteConfirm.cancel`,
        defaultMessage: `Cancel`,
        description: `Cancel button label for deleting archived chats`,
      })),
      (t[7] = c))
    : (c = t[7]);
  let l;
  t[8] === s
    ? (l = t[9])
    : ((l = (0, X.jsx)(x, { color: `ghost`, onClick: s, children: c })), (t[8] = s), (t[9] = l));
  let u;
  t[10] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((u = (0, X.jsx)(y, { ...K.delete })), (t[10] = u))
    : (u = t[10]);
  let d;
  t[11] === r
    ? (d = t[12])
    : ((d = (0, X.jsx)(x, { color: `danger`, onClick: r, children: u })), (t[11] = r), (t[12] = d));
  let f;
  t[13] !== l || t[14] !== d
    ? ((f = (0, X.jsx)(O, { children: (0, X.jsxs)(M, { children: [l, d] }) })),
      (t[13] = l),
      (t[14] = d),
      (t[15] = f))
    : (f = t[15]);
  let p;
  t[16] !== a || t[17] !== o || t[18] !== f
    ? ((p = (0, X.jsxs)(j, { children: [a, o, f] })),
      (t[16] = a),
      (t[17] = o),
      (t[18] = f),
      (t[19] = p))
    : (p = t[19]);
  let m;
  return (
    t[20] !== i || t[21] !== p
      ? ((m = (0, X.jsx)(A, { open: !0, showDialogClose: !1, onOpenChange: i, children: p })),
        (t[20] = i),
        (t[21] = p),
        (t[22] = m))
      : (m = t[22]),
    m
  );
}
function je(e) {
  switch (e) {
    case `all`:
      return Q.allChats;
    case `cloud`:
      return Q.cloud;
    case `local`:
      return Q.local;
  }
}
function Me(e) {
  switch (e) {
    case `none`:
      return Q.none;
    case `project`:
      return Q.project;
  }
}
function Ne(e) {
  return e
    .replaceAll(`-`, ` `)
    .replaceAll(`_`, ` `)
    .split(` `)
    .filter((e) => e.length > 0)
    .map((e) => `${e.charAt(0).toLocaleUpperCase()}${e.slice(1)}`)
    .join(` `);
}
function Pe(e) {
  return e.kind === `all`
    ? `delete-archived-chats`
    : e.kind === `project`
      ? `delete-archived-project-${e.projectLabel ?? `no-project`}`
      : `delete-archived-thread-${e.thread.id}`;
}
export { $ as DataControlsSettings };
//# sourceMappingURL=data-controls.js.map
