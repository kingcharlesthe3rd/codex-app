import { s as e } from "./chunk-Bj-mKKzh.js";
import { Qr as t } from "./src-2.js";
import { Oi as n, Wo as r, fs as i, w as a } from "./app-server-manager-signals.js";
import { n as o, t as s } from "./jsx-runtime.js";
import "./react-dom.js";
import "./Combination.js";
import "./marked.esm-BR-H6018.js";
import {
  A as c,
  F as l,
  _ as u,
  c as d,
  h as f,
  lt as p,
  u as m,
  v as h,
} from "./vscode-api.js";
import { n as g, t as _ } from "./fuse.js";
import { c as v, o as y, r as b } from "./lib-2.js";
import { m as x } from "./chunk-12.js";
import "./persisted-signal-CweW-bgN.js";
import "./rpc-1.js";
import "./statsig.js";
import { t as S } from "./request.js";
import "./selectable-remote-connections-signal.js";
import "./_baseEach.js";
import "./_baseOrderBy.js";
import { r as C } from "./toast-signal.js";
import "./window-zoom-context.js";
import { t as w } from "./tooltip.js";
import { t as ee } from "./button.js";
import "./setting-storage.js";
import "./reduced-motion-preference.js";
import "./spinner.js";
import "./x.js";
import "./with-window.js";
import "./use-stable-callback.js";
import { a as T, i as E, l as D, n as O, r as k } from "./dialog-layout.js";
import "./initial-route-atom.js";
import { t as A } from "./use-navigate-to-local-conversation.js";
import "./_defineProperty.js";
import "./links.js";
import { t as j } from "./trash.js";
import "./chevron.js";
import { t as M } from "./cloud.js";
import { t as N } from "./get-project-name.js";
import "./sortBy.js";
import "./use-connected-remote-connections.js";
import { i as P } from "./settings-shared.js";
import { t as F } from "./settings-host-context.js";
import { t as I } from "./settings-content-layout.js";
import { n as L } from "./settings-row.js";
import { n as R, t as z } from "./settings-group.js";
import { t as B } from "./infinite-scroll-pagination-spinner.js";
var V = p(),
  H = e(o(), 1),
  U = b({
    delete: {
      id: `settings.dataControls.archivedChats.delete`,
      defaultMessage: `Delete`,
      description: `Button label to permanently delete an archived chat`,
    },
    deleteAll: {
      id: `settings.dataControls.archivedChats.deleteAll`,
      defaultMessage: `Delete all`,
      description: `Button label to permanently delete all archived local chats`,
    },
    deleteAllError: {
      id: `settings.dataControls.archivedChats.deleteAllError`,
      defaultMessage: `Failed to delete archived chats`,
      description: `Error message when deleting all archived chats fails`,
    },
    deleteError: {
      id: `settings.dataControls.archivedChats.deleteError`,
      defaultMessage: `Failed to delete archived chat`,
      description: `Error message when deleting an archived chat fails`,
    },
    deleteSuccessPlain: {
      id: `settings.dataControls.archivedChats.deleteSuccessPlain`,
      defaultMessage: `Deleted archived chat`,
      description: `Success toast after deleting an archived chat`,
    },
    deleting: {
      id: `settings.dataControls.archivedChats.deleting`,
      defaultMessage: `Deleting archived chat…`,
      description: `Toast shown while deleting an archived chat`,
    },
    deletingAll: {
      id: `settings.dataControls.archivedChats.deletingAll`,
      defaultMessage: `Deleting archived chats…`,
      description: `Toast shown while deleting all archived chats`,
    },
    unarchiveError: {
      id: `settings.dataControls.archivedChats.unarchiveError`,
      defaultMessage: `Failed to unarchive chat`,
      description: `Error message when unarchiving a chat`,
    },
    unarchiveSuccessPlain: {
      id: `settings.dataControls.archivedChats.unarchiveSuccessPlain`,
      defaultMessage: `Unarchived chat`,
      description: `Success toast after unarchiving a chat`,
    },
    viewNow: {
      id: `settings.dataControls.archivedChats.viewNow`,
      defaultMessage: `View now`,
      description: `Action label to open an unarchived chat from the success toast`,
    },
    unarchive: {
      id: `settings.dataControls.archivedChats.unarchive`,
      defaultMessage: `Unarchive`,
      description: `Button label to unarchive a chat`,
    },
  });
function W({ cloudTasks: e, localThreads: t }) {
  return [
    ...(e ?? []).map((e) => ({
      kind: `cloud`,
      at: G(e.updated_at, e.created_at),
      searchValues: [
        n(e.title ?? ``).trim(),
        e.task_status_display?.environment_label?.trim() ?? ``,
      ],
      task: e,
    })),
    ...(t ?? []).map((e) => ({
      kind: `local`,
      at: G(e.updatedAt, e.createdAt),
      searchValues: [n(e.name?.trim() || e.preview).trim(), N(e.cwd) ?? N(e.path) ?? ``],
      thread: e,
    })),
  ].sort((e, t) => t.at - e.at);
}
function te(e) {
  let t = new _(e, { ignoreLocation: !0, keys: [`searchValues`], threshold: 0.4 });
  return (n) => {
    let r = n.trim();
    return r.length === 0 ? e : t.search(r).map(({ item: e }) => e);
  };
}
function G(e, t) {
  return e != null && Number.isFinite(e) ? e : t != null && Number.isFinite(t) ? t : 0;
}
var K = s();
function ne(e) {
  let t = (0, V.c)(53),
    { archivedTask: r } = e,
    i = l(f),
    a = c(),
    o = v(),
    s = x(),
    d,
    p;
  t[0] === r.id
    ? ((d = t[1]), (p = t[2]))
    : ((d = [`recover-cloud-task`, r.id]),
      (p = () =>
        S.safePost(`/wham/tasks/{task_id}/recover`, { parameters: { path: { task_id: r.id } } })),
      (t[0] = r.id),
      (t[1] = d),
      (t[2] = p));
  let m;
  t[3] !== o || t[4] !== i
    ? ((m = () => {
        i.get(C).danger(o.formatMessage(U.unarchiveError));
      }),
      (t[3] = o),
      (t[4] = i),
      (t[5] = m))
    : (m = t[5]);
  let h;
  t[6] !== r.id || t[7] !== s || t[8] !== a || t[9] !== i
    ? ((h = () => {
        (a.setQueryData([`tasks`, `archived`], (e) =>
          e == null
            ? e
            : {
                ...e,
                pages: e.pages.map((e) => ({ ...e, items: e.items.filter((e) => e.id !== r.id) })),
              },
        ),
          i.get(C).info(
            (0, K.jsxs)(`span`, {
              children: [
                (0, K.jsx)(y, { ...U.unarchiveSuccessPlain }),
                (0, K.jsx)(`button`, {
                  className: `pointer-events-auto ml-1 cursor-interaction text-token-link underline-offset-2 hover:underline`,
                  type: `button`,
                  onClick: () => {
                    s(`/remote/${r.id}`);
                  },
                  children: (0, K.jsx)(y, { ...U.viewNow }),
                }),
              ],
            }),
            { id: `recover-cloud-task-${r.id}` },
          ));
      }),
      (t[6] = r.id),
      (t[7] = s),
      (t[8] = a),
      (t[9] = i),
      (t[10] = h))
    : (h = t[10]);
  let g;
  t[11] === a
    ? (g = t[12])
    : ((g = () => {
        a.invalidateQueries({ queryKey: [`tasks`] });
      }),
      (t[11] = a),
      (t[12] = g));
  let _;
  t[13] !== d || t[14] !== p || t[15] !== m || t[16] !== h || t[17] !== g
    ? ((_ = { mutationKey: d, mutationFn: p, onError: m, onSuccess: h, onSettled: g }),
      (t[13] = d),
      (t[14] = p),
      (t[15] = m),
      (t[16] = h),
      (t[17] = g),
      (t[18] = _))
    : (_ = t[18]);
  let b = u(_),
    w;
  t[19] === r.title ? (w = t[20]) : ((w = n(r.title ?? ``).trim()), (t[19] = r.title), (t[20] = w));
  let T = w,
    E,
    D,
    O,
    k;
  if (t[21] !== r || t[22] !== o) {
    let e = q(r),
      n;
    (t[27] === r.task_status_display?.environment_label
      ? (n = t[28])
      : ((n = r.task_status_display?.environment_label?.trim()),
        (t[27] = r.task_status_display?.environment_label),
        (t[28] = n)),
      (D = n),
      (O = e != null),
      (E = O ? o.formatDate(e, { year: `numeric`, month: `short`, day: `numeric` }) : ``),
      (k = O ? o.formatTime(e, { hour: `numeric`, minute: `2-digit` }) : ``),
      (t[21] = r),
      (t[22] = o),
      (t[23] = E),
      (t[24] = D),
      (t[25] = O),
      (t[26] = k));
  } else ((E = t[23]), (D = t[24]), (O = t[25]), (k = t[26]));
  let A = k,
    j;
  t[29] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((j = (0, K.jsx)(M, { className: `icon-sm shrink-0` })), (t[29] = j))
    : (j = t[29]);
  let N;
  t[30] === T
    ? (N = t[31])
    : ((N =
        T.length > 0
          ? T
          : (0, K.jsx)(y, {
              id: `settings.dataControls.archivedChats.untitled`,
              defaultMessage: `Untitled chat`,
              description: `Fallback title for an archived chat`,
            })),
      (t[30] = T),
      (t[31] = N));
  let P;
  t[32] === N
    ? (P = t[33])
    : ((P = (0, K.jsxs)(`div`, {
        className: `flex min-w-0 items-center gap-2 text-base font-medium`,
        children: [j, (0, K.jsx)(`span`, { className: `truncate`, children: N })],
      })),
      (t[32] = N),
      (t[33] = P));
  let F;
  t[34] !== E || t[35] !== D || t[36] !== O || t[37] !== A
    ? ((F = O
        ? D
          ? (0, K.jsx)(y, {
              id: `settings.dataControls.archivedChats.dateTimeWithEnvironment`,
              defaultMessage: `{date}, {time} • {environment}`,
              description: `Date, time, and environment name for an archived cloud chat`,
              values: { date: E, time: A, environment: D },
            })
          : (0, K.jsx)(y, {
              id: `settings.dataControls.archivedChats.dateTime`,
              defaultMessage: `{date}, {time}`,
              description: `Date and time for an archived chat`,
              values: { date: E, time: A },
            })
        : null),
      (t[34] = E),
      (t[35] = D),
      (t[36] = O),
      (t[37] = A),
      (t[38] = F))
    : (F = t[38]);
  let I;
  t[39] === F
    ? (I = t[40])
    : ((I = (0, K.jsx)(`div`, {
        className: `mt-1 flex min-w-0 flex-col gap-0.5 text-sm`,
        children: (0, K.jsx)(`div`, {
          className: `truncate text-token-text-secondary`,
          children: F,
        }),
      })),
      (t[39] = F),
      (t[40] = I));
  let L;
  t[41] !== P || t[42] !== I
    ? ((L = (0, K.jsxs)(`div`, {
        className: `min-w-0 flex-1 text-left text-token-text-primary`,
        children: [P, I],
      })),
      (t[41] = P),
      (t[42] = I),
      (t[43] = L))
    : (L = t[43]);
  let R;
  t[44] === b
    ? (R = t[45])
    : ((R = () => {
        b.isPending || b.mutate();
      }),
      (t[44] = b),
      (t[45] = R));
  let z;
  t[46] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((z = (0, K.jsx)(y, { ...U.unarchive })), (t[46] = z))
    : (z = t[46]);
  let B;
  t[47] !== R || t[48] !== b.isPending
    ? ((B = (0, K.jsx)(ee, {
        className: `shrink-0`,
        color: `secondary`,
        size: `toolbar`,
        disabled: b.isPending,
        loading: b.isPending,
        onClick: R,
        children: z,
      })),
      (t[47] = R),
      (t[48] = b.isPending),
      (t[49] = B))
    : (B = t[49]);
  let H;
  return (
    t[50] !== L || t[51] !== B
      ? ((H = (0, K.jsxs)(`div`, {
          className: `flex w-full items-center justify-between gap-3 px-4 py-3 hover:bg-token-list-hover-background`,
          children: [L, B],
        })),
        (t[50] = L),
        (t[51] = B),
        (t[52] = H))
      : (H = t[52]),
    H
  );
}
function q(e) {
  let t = e.updated_at ?? e.created_at;
  if (t == null) return null;
  let n = new Date(t * 1e3);
  return Number.isFinite(n.getTime()) ? n : null;
}
function J() {
  let e = (0, V.c)(2),
    t;
  e[0] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((t = [`tasks`, `archived`]), (e[0] = t))
    : (t = e[0]);
  let n;
  return (
    e[1] === Symbol.for(`react.memo_cache_sentinel`)
      ? ((n = {
          queryKey: t,
          initialPageParam: null,
          queryFn: Z,
          getNextPageParam: X,
          select: Y,
          staleTime: m.FIVE_SECONDS,
        }),
        (e[1] = n))
      : (n = e[1]),
    g(n)
  );
}
function Y(e) {
  return e.pages.flatMap(re);
}
function re(e) {
  return e.items;
}
function X(e) {
  return e.cursor ?? null;
}
function Z(e) {
  let { pageParam: t } = e;
  return Q(t);
}
async function Q(e) {
  try {
    return await S.safeGet(`/wham/tasks/list`, {
      parameters: { query: { limit: 20, cursor: e, task_filter: `archived` } },
    });
  } catch (e) {
    if (e instanceof d && (e.status === 401 || e.status === 403 || e.status === 404))
      return { items: [], cursor: null };
    throw e;
  }
}
function $() {
  let e = (0, V.c)(16),
    { selectedHostId: t } = F(),
    n;
  e[0] === t
    ? (n = e[1])
    : ((n = {
        queryKey: [`archived-threads`, t],
        queryFn: () => i(`list-archived-threads`, { hostId: t }),
        enabled: !0,
        staleTime: m.FIVE_SECONDS,
      }),
      (e[0] = t),
      (e[1] = n));
  let { data: r, isLoading: a, isError: o } = h(n),
    {
      data: s,
      fetchNextPage: c,
      hasNextPage: l,
      isLoading: u,
      isFetchNextPageError: d,
      isFetchingNextPage: f,
      isError: p,
    } = J(),
    g;
  e[2] !== s || e[3] !== r
    ? ((g = W({ cloudTasks: s, localThreads: r })), (e[2] = s), (e[3] = r), (e[4] = g))
    : (g = e[4]);
  let _ = g,
    v;
  e[5] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((v = (0, K.jsx)(P, { slug: `data-controls` })), (e[5] = v))
    : (v = e[5]);
  let y = l && !d,
    b = a || u,
    x = o || (s == null && p),
    S;
  e[6] === c
    ? (S = e[7])
    : ((S = () => {
        c();
      }),
      (e[6] = c),
      (e[7] = S));
  let C;
  return (
    e[8] !== _ ||
    e[9] !== f ||
    e[10] !== t ||
    e[11] !== y ||
    e[12] !== b ||
    e[13] !== x ||
    e[14] !== S
      ? ((C = (0, K.jsx)(I, {
          title: v,
          children: (0, K.jsx)(z, {
            className: `gap-2`,
            children: (0, K.jsx)(z.Content, {
              children: (0, K.jsx)(ie, {
                archivedChats: _,
                hostId: t,
                hasNextPage: y,
                isLoading: b,
                isFetchingNextPage: f,
                isError: x,
                onLoadNextPage: S,
              }),
            }),
          }),
        })),
        (e[8] = _),
        (e[9] = f),
        (e[10] = t),
        (e[11] = y),
        (e[12] = b),
        (e[13] = x),
        (e[14] = S),
        (e[15] = C))
      : (C = e[15]),
    C
  );
}
function ie(e) {
  let n = (0, V.c)(93),
    {
      archivedChats: o,
      hostId: s,
      hasNextPage: d,
      isLoading: p,
      isFetchingNextPage: m,
      isError: h,
      onLoadNextPage: g,
    } = e,
    _ = l(f),
    b = c(),
    S = v(),
    w = x(),
    T = A(),
    [E, D] = (0, H.useState)(null),
    [O, k] = (0, H.useState)(``),
    j;
  n[0] === o ? (j = n[1]) : ((j = te(o)), (n[0] = o), (n[1] = j));
  let M = j(O),
    N;
  n[2] !== o || n[3] !== s
    ? ((N = s === `local` && o.some(se)), (n[2] = o), (n[3] = s), (n[4] = N))
    : (N = n[4]);
  let P = N,
    F;
  n[5] !== s || n[6] !== w || n[7] !== T
    ? ((F = (e) => {
        if (s !== `local`) {
          w(`/remote/${e}`);
          return;
        }
        T(e);
      }),
      (n[5] = s),
      (n[6] = w),
      (n[7] = T),
      (n[8] = F))
    : (F = n[8]);
  let I = F,
    z,
    W;
  n[9] === s
    ? ((z = n[10]), (W = n[11]))
    : ((z = [`unarchive-thread`, s]),
      (W = async (e) => (
        await i(`unarchive-conversation`, { hostId: s, conversationId: t(e.id) }),
        e
      )),
      (n[9] = s),
      (n[10] = z),
      (n[11] = W));
  let G;
  n[12] !== s || n[13] !== b || n[14] !== _
    ? ((G = async (e) => {
        (_.get(C).info(
          (0, K.jsx)(y, {
            id: `settings.dataControls.archivedChats.unarchiving`,
            defaultMessage: `Unarchiving chat…`,
            description: `Toast shown while unarchiving a chat`,
          }),
          { id: `unarchive-thread-${e.id}`, duration: 0, hasCloseButton: !1 },
        ),
          await b.cancelQueries({ queryKey: [`archived-threads`, s] }));
        let t = b.getQueryData([`archived-threads`, s]) ?? [];
        return (
          b.setQueryData(
            [`archived-threads`, s],
            t.filter((t) => t.id !== e.id),
          ),
          { previousThreads: t }
        );
      }),
      (n[12] = s),
      (n[13] = b),
      (n[14] = _),
      (n[15] = G))
    : (G = n[15]);
  let q;
  n[16] !== s || n[17] !== S || n[18] !== b || n[19] !== _
    ? ((q = (e, t, n) => {
        (n?.previousThreads && b.setQueryData([`archived-threads`, s], n.previousThreads),
          _.get(C).danger(S.formatMessage(U.unarchiveError), { id: `unarchive-thread-${t.id}` }));
      }),
      (n[16] = s),
      (n[17] = S),
      (n[18] = b),
      (n[19] = _),
      (n[20] = q))
    : (q = n[20]);
  let J;
  n[21] !== s || n[22] !== I || n[23] !== _
    ? ((J = async (e) => {
        let n = t(e.id),
          r = await i(`hydrate-pinned-threads`, { hostId: s, threadIds: [e.id] }).then(
            () => _.get(a, n),
            oe,
          );
        _.get(C).info(
          r
            ? (0, K.jsxs)(`span`, {
                children: [
                  (0, K.jsx)(y, { ...U.unarchiveSuccessPlain }),
                  (0, K.jsx)(`button`, {
                    className: `pointer-events-auto ml-1 cursor-interaction text-token-link underline-offset-2 hover:underline`,
                    type: `button`,
                    onClick: () => {
                      I(e.id);
                    },
                    children: (0, K.jsx)(y, { ...U.viewNow }),
                  }),
                ],
              })
            : (0, K.jsx)(y, { ...U.unarchiveSuccessPlain }),
          { id: `unarchive-thread-${e.id}` },
        );
      }),
      (n[21] = s),
      (n[22] = I),
      (n[23] = _),
      (n[24] = J))
    : (J = n[24]);
  let Y;
  n[25] !== s || n[26] !== b
    ? ((Y = () => {
        b.invalidateQueries({ queryKey: [`archived-threads`, s] });
      }),
      (n[25] = s),
      (n[26] = b),
      (n[27] = Y))
    : (Y = n[27]);
  let re;
  n[28] !== z || n[29] !== W || n[30] !== G || n[31] !== q || n[32] !== J || n[33] !== Y
    ? ((re = {
        mutationKey: z,
        mutationFn: W,
        onMutate: G,
        onError: q,
        onSuccess: J,
        onSettled: Y,
      }),
      (n[28] = z),
      (n[29] = W),
      (n[30] = G),
      (n[31] = q),
      (n[32] = J),
      (n[33] = Y),
      (n[34] = re))
    : (re = n[34]);
  let X = u(re),
    Z,
    Q;
  n[35] === s
    ? ((Z = n[36]), (Q = n[37]))
    : ((Z = [`delete-archived-conversations`, s]),
      (Q = async (e) =>
        e.kind === `all`
          ? i(`delete-all-archived-conversations`, { hostId: s })
          : i(`delete-archived-conversation`, { hostId: s, conversationId: t(e.thread.id) })),
      (n[35] = s),
      (n[36] = Z),
      (n[37] = Q));
  let $;
  n[38] !== s || n[39] !== b || n[40] !== _
    ? (($ = async (e) => {
        let t = e.kind === `all` ? e.threadIds : [e.thread.id];
        (_.get(C).info(
          e.kind === `all` ? (0, K.jsx)(y, { ...U.deletingAll }) : (0, K.jsx)(y, { ...U.deleting }),
          { id: de(e), duration: 0, hasCloseButton: !1 },
        ),
          await b.cancelQueries({ queryKey: [`archived-threads`, s] }));
        let n = b.getQueryData([`archived-threads`, s]) ?? [];
        return (
          b.setQueryData(
            [`archived-threads`, s],
            n.filter((e) => !t.includes(e.id)),
          ),
          { deletingThreadIds: t, previousThreads: n }
        );
      }),
      (n[38] = s),
      (n[39] = b),
      (n[40] = _),
      (n[41] = $))
    : ($ = n[41]);
  let ie;
  n[42] !== s || n[43] !== S || n[44] !== b || n[45] !== _
    ? ((ie = (e, t, n) => {
        (n?.previousThreads && b.setQueryData([`archived-threads`, s], n.previousThreads),
          _.get(C).danger(S.formatMessage(t.kind === `all` ? U.deleteAllError : U.deleteError), {
            id: de(t),
          }));
      }),
      (n[42] = s),
      (n[43] = S),
      (n[44] = b),
      (n[45] = _),
      (n[46] = ie))
    : (ie = n[46]);
  let fe;
  n[47] !== S || n[48] !== _
    ? ((fe = (e, t) => {
        if (t.kind === `all`) {
          _.get(C).success(
            S.formatMessage(
              {
                id: `settings.dataControls.archivedChats.deleteAllSuccess`,
                defaultMessage: `Deleted {count, plural, one {# archived chat} other {# archived chats}}`,
                description: `Success toast after deleting archived chats`,
              },
              { count: e.length || t.threadIds.length },
            ),
            { id: de(t) },
          );
          return;
        }
        _.get(C).success(S.formatMessage(U.deleteSuccessPlain), { id: de(t) });
      }),
      (n[47] = S),
      (n[48] = _),
      (n[49] = fe))
    : (fe = n[49]);
  let pe;
  n[50] !== s || n[51] !== b
    ? ((pe = () => {
        b.invalidateQueries({ queryKey: [`archived-threads`, s] });
      }),
      (n[50] = s),
      (n[51] = b),
      (n[52] = pe))
    : (pe = n[52]);
  let me;
  n[53] !== Z || n[54] !== Q || n[55] !== $ || n[56] !== ie || n[57] !== fe || n[58] !== pe
    ? ((me = {
        mutationKey: Z,
        mutationFn: Q,
        onMutate: $,
        onError: ie,
        onSuccess: fe,
        onSettled: pe,
      }),
      (n[53] = Z),
      (n[54] = Q),
      (n[55] = $),
      (n[56] = ie),
      (n[57] = fe),
      (n[58] = pe),
      (n[59] = me))
    : (me = n[59]);
  let he = u(me),
    ge = X.isPending || he.isPending;
  if (p) {
    let e;
    return (
      n[60] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((e = (0, K.jsx)(R, {
            children: (0, K.jsx)(L, {
              label: (0, K.jsx)(y, {
                id: `settings.dataControls.archivedChats.loading`,
                defaultMessage: `Loading archived chats…`,
                description: `Loading state label for archived chats list`,
              }),
              control: null,
            }),
          })),
          (n[60] = e))
        : (e = n[60]),
      e
    );
  }
  if (h) {
    let e;
    return (
      n[61] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((e = (0, K.jsx)(R, {
            children: (0, K.jsx)(L, {
              label: (0, K.jsx)(y, {
                id: `settings.dataControls.archivedChats.error`,
                defaultMessage: `Could not load archived chats.`,
                description: `Error state label for archived chats list`,
              }),
              control: null,
            }),
          })),
          (n[61] = e))
        : (e = n[61]),
      e
    );
  }
  if (o.length === 0) {
    let e;
    return (
      n[62] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((e = (0, K.jsx)(R, {
            children: (0, K.jsx)(L, {
              label: (0, K.jsx)(y, {
                id: `settings.dataControls.archivedChats.empty`,
                defaultMessage: `No archived chats.`,
                description: `Empty state label for archived chats list`,
              }),
              control: null,
            }),
          })),
          (n[62] = e))
        : (e = n[62]),
      e
    );
  }
  let _e;
  n[63] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((_e = (0, K.jsx)(ce, { onSearchQueryChange: k })), (n[63] = _e))
    : (_e = n[63]);
  let ve = R,
    ye =
      M.length === 0 && !d && !m
        ? (0, K.jsx)(L, {
            label: (0, K.jsx)(y, {
              id: `settings.dataControls.archivedChats.noMatches`,
              defaultMessage: `No matching archived chats`,
              description: `Empty state shown when archived chat search has no matches`,
            }),
            control: null,
          })
        : null,
    be;
  n[64] !== ge || n[65] !== s || n[66] !== X
    ? ((be = (e) => {
        switch (e.kind) {
          case `cloud`:
            return (0, K.jsx)(ne, { archivedTask: e.task }, `cloud:${e.task.id}`);
          case `local`:
            return (0, K.jsx)(
              le,
              {
                archivedThread: e.thread,
                isDeleteDisabled: ge,
                isUnarchiving: X.isPending && X.variables?.id === e.thread.id,
                showDeleteButton: s === r,
                onDelete: () => {
                  D({ kind: `single`, thread: e.thread });
                },
                onUnarchive: () => {
                  X.mutate(e.thread);
                },
              },
              `local:${e.thread.id}`,
            );
        }
      }),
      (n[64] = ge),
      (n[65] = s),
      (n[66] = X),
      (n[67] = be))
    : (be = n[67]);
  let xe = M.map(be),
    Se;
  n[68] !== d || n[69] !== m || n[70] !== g
    ? ((Se = (0, K.jsx)(B, { hasNextPage: d, isFetchingNextPage: m, onLoadNextPage: g })),
      (n[68] = d),
      (n[69] = m),
      (n[70] = g),
      (n[71] = Se))
    : (Se = n[71]);
  let Ce;
  n[72] !== ve || n[73] !== ye || n[74] !== xe || n[75] !== Se
    ? ((Ce = (0, K.jsxs)(ve, {
        className: `max-h-[min(80vh)] overflow-y-auto`,
        children: [ye, xe, Se],
      })),
      (n[72] = ve),
      (n[73] = ye),
      (n[74] = xe),
      (n[75] = Se),
      (n[76] = Ce))
    : (Ce = n[76]);
  let we;
  n[77] !== ge || n[78] !== P
    ? ((we = P
        ? (0, K.jsx)(`div`, {
            className: `flex items-center justify-end gap-2`,
            children: (0, K.jsx)(ee, {
              color: `danger`,
              size: `toolbar`,
              disabled: ge,
              onClick: () => {
                D({ kind: `all` });
              },
              children: (0, K.jsx)(y, { ...U.deleteAll }),
            }),
          })
        : null),
      (n[77] = ge),
      (n[78] = P),
      (n[79] = we))
    : (we = n[79]);
  let Te;
  n[80] !== o || n[81] !== he || n[82] !== E
    ? ((Te = () => {
        if (E != null) {
          if ((D(null), E.kind === `all`)) {
            he.mutate({ kind: `all`, threadIds: o.flatMap(ae) });
            return;
          }
          he.mutate({ kind: `single`, thread: E.thread });
        }
      }),
      (n[80] = o),
      (n[81] = he),
      (n[82] = E),
      (n[83] = Te))
    : (Te = n[83]);
  let Ee;
  n[84] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((Ee = (e) => {
        e || D(null);
      }),
      (n[84] = Ee))
    : (Ee = n[84]);
  let De;
  n[85] !== E || n[86] !== Te
    ? ((De = (0, K.jsx)(ue, { state: E, onConfirm: Te, onOpenChange: Ee })),
      (n[85] = E),
      (n[86] = Te),
      (n[87] = De))
    : (De = n[87]);
  let Oe;
  return (
    n[88] !== _e || n[89] !== Ce || n[90] !== we || n[91] !== De
      ? ((Oe = (0, K.jsxs)(K.Fragment, { children: [_e, Ce, we, De] })),
        (n[88] = _e),
        (n[89] = Ce),
        (n[90] = we),
        (n[91] = De),
        (n[92] = Oe))
      : (Oe = n[92]),
    Oe
  );
}
function ae(e) {
  return e.kind === `local` ? [e.thread.id] : [];
}
function oe() {
  return !1;
}
function se(e) {
  return e.kind === `local`;
}
function ce(e) {
  let t = (0, V.c)(11),
    { onSearchQueryChange: n } = e,
    r = v(),
    [i, a] = (0, H.useState)(``),
    o;
  t[0] === r
    ? (o = t[1])
    : ((o = r.formatMessage({
        id: `settings.dataControls.archivedChats.search.ariaLabel`,
        defaultMessage: `Search archived chats`,
        description: `Accessible label for the archived chats search input`,
      })),
      (t[0] = r),
      (t[1] = o));
  let s;
  t[2] === r
    ? (s = t[3])
    : ((s = r.formatMessage({
        id: `settings.dataControls.archivedChats.search.placeholder`,
        defaultMessage: `Search archived chats`,
        description: `Placeholder for the archived chats search input`,
      })),
      (t[2] = r),
      (t[3] = s));
  let c;
  t[4] === n
    ? (c = t[5])
    : ((c = (e) => {
        let t = e.currentTarget.value;
        (a(t),
          (0, H.startTransition)(() => {
            n(t);
          }));
      }),
      (t[4] = n),
      (t[5] = c));
  let l;
  return (
    t[6] !== i || t[7] !== o || t[8] !== s || t[9] !== c
      ? ((l = (0, K.jsx)(`input`, {
          className: `w-full rounded-md border border-token-border bg-transparent px-3 py-2 text-sm text-token-text-primary outline-none placeholder:text-token-text-tertiary`,
          "aria-label": o,
          placeholder: s,
          value: i,
          onChange: c,
        })),
        (t[6] = i),
        (t[7] = o),
        (t[8] = s),
        (t[9] = c),
        (t[10] = l))
      : (l = t[10]),
    l
  );
}
function le(e) {
  let t = (0, V.c)(50),
    {
      archivedThread: r,
      isDeleteDisabled: i,
      isUnarchiving: a,
      showDeleteButton: o,
      onDelete: s,
      onUnarchive: c,
    } = e,
    l = v(),
    u;
  t[0] !== r.name || t[1] !== r.preview
    ? ((u = n(r.name?.trim() || r.preview).trim()), (t[0] = r.name), (t[1] = r.preview), (t[2] = u))
    : (u = t[2]);
  let d = u,
    f,
    p,
    m,
    h;
  if (
    t[3] !== r.createdAt ||
    t[4] !== r.cwd ||
    t[5] !== r.path ||
    t[6] !== r.updatedAt ||
    t[7] !== l
  ) {
    let e = new Date(Number(r.updatedAt) * 1e3),
      n = new Date(Number(r.createdAt) * 1e3),
      i = Number.isFinite(e.getTime()) ? e : Number.isFinite(n.getTime()) ? n : null,
      a;
    (t[12] !== r.cwd || t[13] !== r.path
      ? ((a = N(r.cwd) ?? N(r.path)), (t[12] = r.cwd), (t[13] = r.path), (t[14] = a))
      : (a = t[14]),
      (m = a),
      (p = i != null),
      (f = p ? l.formatDate(i, { year: `numeric`, month: `short`, day: `numeric` }) : ``),
      (h = p ? l.formatTime(i, { hour: `numeric`, minute: `2-digit` }) : ``),
      (t[3] = r.createdAt),
      (t[4] = r.cwd),
      (t[5] = r.path),
      (t[6] = r.updatedAt),
      (t[7] = l),
      (t[8] = f),
      (t[9] = p),
      (t[10] = m),
      (t[11] = h));
  } else ((f = t[8]), (p = t[9]), (m = t[10]), (h = t[11]));
  let g = h,
    _;
  t[15] === l
    ? (_ = t[16])
    : ((_ = l.formatMessage({
        id: `settings.dataControls.archivedChats.deleteAriaLabel`,
        defaultMessage: `Delete archived chat`,
        description: `Aria label for deleting an archived chat`,
      })),
      (t[15] = l),
      (t[16] = _));
  let b = _,
    x;
  t[17] === d
    ? (x = t[18])
    : ((x =
        d.length > 0
          ? d
          : (0, K.jsx)(y, {
              id: `settings.dataControls.archivedChats.untitled`,
              defaultMessage: `Untitled chat`,
              description: `Fallback title for an archived chat`,
            })),
      (t[17] = d),
      (t[18] = x));
  let S;
  t[19] === x
    ? (S = t[20])
    : ((S = (0, K.jsx)(`div`, { className: `truncate text-base font-medium`, children: x })),
      (t[19] = x),
      (t[20] = S));
  let C;
  t[21] !== f || t[22] !== p || t[23] !== m || t[24] !== g
    ? ((C = p
        ? m
          ? (0, K.jsx)(y, {
              id: `settings.dataControls.archivedChats.dateTimeWithRepo`,
              defaultMessage: `{date}, {time} • {repo}`,
              description: `Date, time, and repo name for an archived chat`,
              values: { date: f, time: g, repo: m },
            })
          : (0, K.jsx)(y, {
              id: `settings.dataControls.archivedChats.dateTime`,
              defaultMessage: `{date}, {time}`,
              description: `Date and time for an archived chat`,
              values: { date: f, time: g },
            })
        : null),
      (t[21] = f),
      (t[22] = p),
      (t[23] = m),
      (t[24] = g),
      (t[25] = C))
    : (C = t[25]);
  let T;
  t[26] === C
    ? (T = t[27])
    : ((T = (0, K.jsx)(`div`, {
        className: `mt-1 flex min-w-0 flex-col gap-0.5 text-sm`,
        children: (0, K.jsx)(`div`, {
          className: `truncate text-token-text-secondary`,
          children: C,
        }),
      })),
      (t[26] = C),
      (t[27] = T));
  let E;
  t[28] !== S || t[29] !== T
    ? ((E = (0, K.jsxs)(`div`, {
        className: `min-w-0 flex-1 text-left text-token-text-primary`,
        children: [S, T],
      })),
      (t[28] = S),
      (t[29] = T),
      (t[30] = E))
    : (E = t[30]);
  let D;
  t[31] !== b || t[32] !== i || t[33] !== s || t[34] !== o
    ? ((D = o
        ? (0, K.jsx)(w, {
            tooltipContent: b,
            children: (0, K.jsx)(ee, {
              "aria-label": b,
              className: `text-token-charts-red enabled:hover:bg-token-charts-red/10`,
              color: `ghost`,
              size: `toolbar`,
              uniform: !0,
              disabled: i,
              onClick: s,
              children: (0, K.jsx)(j, { className: `icon-xs` }),
            }),
          })
        : null),
      (t[31] = b),
      (t[32] = i),
      (t[33] = s),
      (t[34] = o),
      (t[35] = D))
    : (D = t[35]);
  let O;
  t[36] !== i || t[37] !== c
    ? ((O = () => {
        i || c();
      }),
      (t[36] = i),
      (t[37] = c),
      (t[38] = O))
    : (O = t[38]);
  let k;
  t[39] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((k = (0, K.jsx)(y, { ...U.unarchive })), (t[39] = k))
    : (k = t[39]);
  let A;
  t[40] !== i || t[41] !== a || t[42] !== O
    ? ((A = (0, K.jsx)(ee, {
        className: `shrink-0`,
        color: `secondary`,
        size: `toolbar`,
        disabled: i,
        loading: a,
        onClick: O,
        children: k,
      })),
      (t[40] = i),
      (t[41] = a),
      (t[42] = O),
      (t[43] = A))
    : (A = t[43]);
  let M;
  t[44] !== A || t[45] !== D
    ? ((M = (0, K.jsxs)(`div`, {
        className: `flex shrink-0 items-center gap-2`,
        children: [D, A],
      })),
      (t[44] = A),
      (t[45] = D),
      (t[46] = M))
    : (M = t[46]);
  let P;
  return (
    t[47] !== M || t[48] !== E
      ? ((P = (0, K.jsxs)(`div`, {
          className: `flex w-full items-center justify-between gap-3 px-4 py-3 hover:bg-token-list-hover-background`,
          children: [E, M],
        })),
        (t[47] = M),
        (t[48] = E),
        (t[49] = P))
      : (P = t[49]),
    P
  );
}
function ue(e) {
  let t = (0, V.c)(22),
    { state: n, onConfirm: r, onOpenChange: i } = e;
  if (n == null) return null;
  let a;
  t[0] === n.kind
    ? (a = t[1])
    : ((a = (0, K.jsx)(T, {
        children: (0, K.jsx)(E, {
          title:
            n.kind === `all`
              ? (0, K.jsx)(y, {
                  id: `settings.dataControls.archivedChats.deleteAllConfirm.title`,
                  defaultMessage: `Delete all archived local chats?`,
                  description: `Title for confirming deletion of all archived local chats`,
                })
              : (0, K.jsx)(y, {
                  id: `settings.dataControls.archivedChats.deleteConfirm.title`,
                  defaultMessage: `Delete archived chat?`,
                  description: `Title for confirming deletion of an archived chat`,
                }),
        }),
      })),
      (t[0] = n.kind),
      (t[1] = a));
  let o;
  t[2] === n.kind
    ? (o = t[3])
    : ((o = (0, K.jsx)(T, {
        className: `text-token-description-foreground`,
        children: (0, K.jsx)(`p`, {
          children:
            n.kind === `all`
              ? (0, K.jsx)(y, {
                  id: `settings.dataControls.archivedChats.deleteAllConfirm.body`,
                  defaultMessage: `This permanently deletes all archived chats`,
                  description: `Body copy in the delete all archived local chats confirmation dialog`,
                })
              : (0, K.jsx)(y, {
                  id: `settings.dataControls.archivedChats.deleteConfirm.body`,
                  defaultMessage: `This permanently deletes the archived chat`,
                  description: `Body copy in the delete archived chat confirmation dialog`,
                }),
        }),
      })),
      (t[2] = n.kind),
      (t[3] = o));
  let s;
  t[4] === i
    ? (s = t[5])
    : ((s = () => {
        i(!1);
      }),
      (t[4] = i),
      (t[5] = s));
  let c;
  t[6] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((c = (0, K.jsx)(y, {
        id: `settings.dataControls.archivedChats.deleteConfirm.cancel`,
        defaultMessage: `Cancel`,
        description: `Cancel button label for deleting archived chats`,
      })),
      (t[6] = c))
    : (c = t[6]);
  let l;
  t[7] === s
    ? (l = t[8])
    : ((l = (0, K.jsx)(ee, { color: `ghost`, onClick: s, children: c })), (t[7] = s), (t[8] = l));
  let u;
  t[9] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((u = (0, K.jsx)(y, { ...U.delete })), (t[9] = u))
    : (u = t[9]);
  let d;
  t[10] === r
    ? (d = t[11])
    : ((d = (0, K.jsx)(ee, { color: `danger`, onClick: r, children: u })),
      (t[10] = r),
      (t[11] = d));
  let f;
  t[12] !== l || t[13] !== d
    ? ((f = (0, K.jsx)(T, { children: (0, K.jsxs)(k, { children: [l, d] }) })),
      (t[12] = l),
      (t[13] = d),
      (t[14] = f))
    : (f = t[14]);
  let p;
  t[15] !== a || t[16] !== o || t[17] !== f
    ? ((p = (0, K.jsxs)(O, { children: [a, o, f] })),
      (t[15] = a),
      (t[16] = o),
      (t[17] = f),
      (t[18] = p))
    : (p = t[18]);
  let m;
  return (
    t[19] !== i || t[20] !== p
      ? ((m = (0, K.jsx)(D, { open: !0, showDialogClose: !1, onOpenChange: i, children: p })),
        (t[19] = i),
        (t[20] = p),
        (t[21] = m))
      : (m = t[21]),
    m
  );
}
function de(e) {
  return e.kind === `all` ? `delete-archived-chats` : `delete-archived-thread-${e.thread.id}`;
}
export { $ as DataControlsSettings };
//# sourceMappingURL=data-controls.js.map
