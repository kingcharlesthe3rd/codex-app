import { s as e } from "./chunk.js";
import { n as t, t as n } from "./jsx-runtime.js";
import { Z as r, a as i, g as a, o, s as ee, t as s } from "./app-scope-CWE-zIhQ.js";
import { Bt as c, r as l } from "./use-host-config.js";
import { A as u, ln as d, ta as f } from "./thread-context-inputs.js";
import { f as p, o as m } from "./vscode-api.js";
import { ai as h } from "./src.js";
import { l as g, s as _ } from "./lib-1.js";
import { t as v } from "./clsx.js";
import { t as y } from "./button-oF-qgtAS.js";
import { t as te } from "./tooltip-B.js";
import { r as ne } from "./toast-signal.js";
import { l as re, o as b } from "./statsig.js";
import { Pn as ie, t as ae } from "./product-logger.js";
import { m as oe, p as se } from "./chunk-13.js";
import { t as ce } from "./use-auth.js";
import { t as le } from "./link-external.js";
import { b as x, c as ue, p as de, t as fe } from "./codex-api.js";
import { t as pe } from "./use-collaboration-mode.js";
import { n as me, t as S } from "./skus.js";
import { n as he } from "./app-identity.js";
import { t as C } from "./with-window.js";
import { t as ge } from "./settings.cog.js";
import { t as _e } from "./codex.js";
import { t as ve } from "./openai-blossom.js";
import { t as ye } from "./copy-to-clipboard.js";
import { r as be } from "./modal-controller-state.js";
import { d as w, i as T, l as xe, n as Se, r as Ce } from "./dialog-layout.js";
import { t as we } from "./settings-sections.js";
import { n as Te, t as Ee } from "./upgrade-plan-dialog-launcher.js";
import { a as De } from "./command-keybindings.js";
import { p as Oe } from "./usage-queries.js";
import { a as ke } from "./links.js";
import { t as Ae } from "./use-codex-pricing-url.js";
import { m as E, o as D } from "./rate-limit-status.js";
import { c as je } from "./plan-management-state.js";
import { c as Me, d as Ne, o as Pe, t as Fe } from "./use-rate-limit.js";
import { i as O, r as Ie, t as Le } from "./dropdown.js";
import { t as Re } from "./use-active-conversation-id.js";
import { t as ze } from "./avatar.js";
import { t as Be } from "./building.js";
import { t as Ve } from "./keyboard.js";
import { t as He } from "./log-out.js";
import { t as Ue } from "./rate-limit-reset-modal.js";
import { t as We } from "./rate-limit-summary.js";
import { t as Ge } from "./referral-invite-modal-CL-_nI4E.js";
import { n as Ke, r as qe } from "./profile-queries.js";
import { t as Je } from "./profile-visibility.js";
import { t as Ye } from "./use-usage-settings-access.js";
var Xe = e(t()),
  k = n(),
  A = (e) =>
    (0, k.jsx)(`svg`, {
      width: 16,
      height: 16,
      viewBox: `0 0 16 16`,
      fill: `none`,
      xmlns: `http://www.w3.org/2000/svg`,
      ...e,
      children: (0, k.jsx)(`path`, {
        fillRule: `evenodd`,
        clipRule: `evenodd`,
        d: `M12.0368 1.69459C13.4379 1.22769 14.7711 2.56142 14.304 3.96256L11.2337 13.1735C10.7213 14.7106 8.59383 14.8331 7.90871 13.3649L6.3423 10.0079C6.28804 9.89173 6.20464 9.79237 6.1009 9.71959L5.99074 9.65631L2.63449 8.08913C1.16665 7.40386 1.28853 5.27746 2.82512 4.76491L12.0368 1.69459ZM13.3884 2.9985C12.9661 3.64157 12.2113 4.53879 11.997 4.75319L7.2673 9.48288C7.28051 9.50799 7.2951 9.53284 7.30715 9.55866L8.87199 12.9149C9.15031 13.5114 10.0151 13.4618 10.2236 12.8376L13.0814 4.26334L13.3501 3.40084C13.4001 3.25067 13.41 3.11459 13.3884 2.9985ZM12.9978 2.60944C12.8823 2.58865 12.7466 2.5989 12.5978 2.6485L11.7353 2.91725L3.16183 5.77506C2.53754 5.98334 2.48745 6.84731 3.08371 7.12584L6.44074 8.69225L6.51496 8.73053L11.2446 4.00084C11.4589 3.78664 12.3548 3.0319 12.9978 2.60944Z`,
        fill: `currentColor`,
      }),
    }),
  Ze = (e) =>
    (0, k.jsx)(`svg`, {
      width: 20,
      height: 20,
      viewBox: `0 0 20 20`,
      fill: `none`,
      xmlns: `http://www.w3.org/2000/svg`,
      ...e,
      children: (0, k.jsx)(`path`, {
        d: `M17.665 10C17.665 10.6877 17.1785 11.2454 16.5488 11.3945L16.4219 11.4189C14.7098 11.6665 13.6129 12.1305 12.877 12.8623C12.1414 13.5938 11.6742 14.6843 11.4238 16.3887C11.3197 17.0973 10.7182 17.665 9.96484 17.665C9.27085 17.665 8.68836 17.1772 8.53613 16.5215C8.12392 14.7459 7.6623 13.619 6.95703 12.8652C6.31314 12.1772 5.39414 11.7268 3.88672 11.4688L3.57715 11.4199C2.88869 11.319 2.33496 10.734 2.33496 10C2.33496 9.26603 2.88869 8.681 3.57715 8.58008L3.88672 8.53125C5.39414 8.27321 6.31314 7.82277 6.95703 7.13477C7.6623 6.38104 8.12392 5.25413 8.53613 3.47852L8.56934 3.35742C8.76133 2.76356 9.31424 2.33496 9.96484 2.33496C10.7182 2.33497 11.3197 2.9027 11.4238 3.61133L11.5283 4.22266C11.7954 5.58295 12.2334 6.49773 12.877 7.1377C13.6129 7.86952 14.7098 8.33351 16.4219 8.58105C17.1119 8.68101 17.665 9.26667 17.665 10Z`,
        fill: `currentColor`,
      }),
    }),
  Qe = a(s, !1);
function $e(e) {
  return e?.structure?.toLowerCase() === `personal`;
}
function j({ currentAccount: e, accounts: t }) {
  return $e(e) ? (t == null ? !0 : t.some((e) => !$e(e))) : !1;
}
function et({ authMethod: e, plan: t, currentAccount: n, accounts: r }) {
  return e !== `chatgpt` || (t !== S.FREE && t !== S.GO) || n == null || r == null
    ? !1
    : !j({ currentAccount: n, accounts: r });
}
var tt = r(),
  nt = `1038162578`,
  rt = `3648137593`;
function it(e) {
  let t = (0, tt.c)(203),
    { triggerButton: n } = e,
    r = o(s),
    a = ee(Qe),
    re;
  t[0] === r
    ? (re = t[1])
    : ((re = (e) => {
        r.set(Qe, e);
      }),
      (t[0] = r),
      (t[1] = re));
  let b = re,
    x = oe(),
    {
      accountId: fe,
      email: S,
      userId: w,
      authMethod: T,
      planAtLogin: xe,
      requiresAuth: Se,
      isCopilotApiAvailable: Ce,
      openAIAuth: E,
      setAuthMethod: D,
    } = ce(),
    { data: Ge } = ue(),
    { data: A, isError: $e } = de(),
    j = T === `chatgpt`,
    it = T === `copilot`,
    ut = T === `apikey`,
    dt = T === `amazonBedrock`,
    ft = j || ut,
    pt;
  t[2] === j ? (pt = t[3]) : ((pt = { queryConfig: { enabled: j } }), (t[2] = j), (t[3] = pt));
  let { data: mt } = m(`account-info`, pt),
    M = A?.plan_type ?? mt?.plan ?? xe,
    ht;
  t[4] !== j || t[5] !== M ? ((ht = j && me(M)), (t[4] = j), (t[5] = M), (t[6] = ht)) : (ht = t[6]);
  let gt = ht,
    _t = Ge?.accounts,
    vt;
  t[7] !== A || t[8] !== T || t[9] !== M || t[10] !== _t
    ? ((vt = et({ authMethod: T, plan: M, currentAccount: A, accounts: _t })),
      (t[7] = A),
      (t[8] = T),
      (t[9] = M),
      (t[10] = _t),
      (t[11] = vt))
    : (vt = t[11]);
  let yt = vt,
    bt;
  t[12] === M ? (bt = t[13]) : ((bt = je(M)?.pricePlan ?? null), (t[12] = M), (t[13] = bt));
  let xt = bt,
    St = !ft && E != null,
    Ct = !it && Ce,
    wt = !ft && E == null && Se,
    Tt = Se || !1,
    Et = Je(),
    Dt;
  t[14] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((Dt = { logExposure: !1 }), (t[14] = Dt))
    : (Dt = t[14]);
  let Ot = Ae(Dt),
    N = Re(),
    kt = se(`/remote/:taskId`),
    At;
  t[15] !== N || t[16] !== kt
    ? ((At = N ?? (kt?.params.taskId == null ? null : h(kt.params.taskId))),
      (t[15] = N),
      (t[16] = kt),
      (t[17] = At))
    : (At = t[17]);
  let jt = At,
    Mt = i(u, N),
    Nt;
  t[18] !== Mt || t[19] !== N
    ? ((Nt = N == null ? void 0 : { workspaceRoot: Mt }), (t[18] = Mt), (t[19] = N), (t[20] = Nt))
    : (Nt = t[20]);
  let P = Nt,
    { activeMode: Pt } = pe(jt),
    Ft = Pt?.settings.model ?? null,
    It = i(De, `settings`),
    Lt = g(),
    { data: F } = ee(d),
    Rt = Pe(F),
    zt = Me(F),
    Bt;
  t[21] === F?.plan_type
    ? (Bt = t[22])
    : ((Bt = je(F?.plan_type)?.pricePlan ?? null), (t[21] = F?.plan_type), (t[22] = Bt));
  let Vt = Bt,
    Ht = Fe(Rt, { activeLimitName: zt, selectedModel: Ft }),
    Ut = Ne(F, { activeLimitName: zt, selectedModel: Ft }),
    Wt = F?.rate_limit_reset_credits?.available_count ?? 0,
    { isUsageSettingsVisible: Gt, isUsageSettingsAccessLoading: Kt } = Ye(),
    qt = a && gt && Gt && !Kt,
    Jt = A?.id ?? fe ?? mt?.accountId ?? null,
    Yt;
  t[23] !== qt || t[24] !== Jt
    ? ((Yt = { accountId: Jt, enabled: qt }), (t[23] = qt), (t[24] = Jt), (t[25] = Yt))
    : (Yt = t[25]);
  let { data: Xt, isFetching: Zt } = Oe(Yt),
    Qt = ct(Xt === void 0 ? null : Xt),
    I = Qt != null,
    $t = gt && Zt && !I,
    L = Ht;
  gt && ((L = []), Qt != null && (L = [Qt]));
  let en = gt ? null : zt,
    R = gt ? 0 : Wt,
    z = $t || L.some(ot) || R > 0,
    tn = Ee(),
    nn = Te(),
    rn = Ke(),
    an = fe ?? mt?.accountId ?? null,
    on = a && Et && rn,
    sn = w ?? mt?.userId ?? null,
    cn;
  (t[26] !== an || t[27] !== on || t[28] !== sn
    ? ((cn = { accountId: an, enabled: on, userId: sn }),
      (t[26] = an),
      (t[27] = on),
      (t[28] = sn),
      (t[29] = cn))
    : (cn = t[29]),
    qe(cn));
  let ln;
  t[30] === A?.structure
    ? (ln = t[31])
    : ((ln = function () {
        return A?.structure?.toLowerCase() === `personal`;
      }),
      (t[30] = A?.structure),
      (t[31] = ln));
  let B = ln,
    un;
  t[32] === A?.structure
    ? (un = t[33])
    : ((un = function () {
        return A?.structure?.toLowerCase() === `workspace`;
      }),
      (t[32] = A?.structure),
      (t[33] = un));
  let dn = un,
    V = null;
  B() ? (V = nt) : dn() && (V = rt);
  let fn;
  t[34] !== A?.name || t[35] !== B
    ? ((fn = B()
        ? (0, k.jsx)(_, {
            id: `codex.profileDropdown.personalAccountTitle`,
            defaultMessage: `Personal account`,
            description: `Label for a personal account`,
          })
        : (A?.name ??
          (0, k.jsx)(_, {
            id: `codex.profileDropdown.defaultAccountTitle`,
            defaultMessage: `Default account`,
            description: `Fallback label when a non-personal account has no display name`,
          }))),
      (t[34] = A?.name),
      (t[35] = B),
      (t[36] = fn))
    : (fn = t[36]);
  let pn = fn,
    H = A?.profile_picture_url ?? null,
    [mn, hn] = (0, Xe.useState)(null),
    gn;
  t[37] !== Lt || t[38] !== r || t[39] !== w
    ? ((gn = async () => {
        if (w != null)
          try {
            (await ye(w),
              r
                .get(ne)
                .success(
                  Lt.formatMessage({
                    id: `codex.profileDropdown.copyUserIdSuccess`,
                    defaultMessage: `Copied user ID`,
                    description: `Toast shown after copying the ChatGPT user ID from the profile dropdown`,
                  }),
                ));
          } catch {
            r.get(ne).danger(
              Lt.formatMessage({
                id: `codex.profileDropdown.copyUserIdError`,
                defaultMessage: `Failed to copy user ID`,
                description: `Toast shown when copying the ChatGPT user ID from the profile dropdown fails`,
              }),
            );
          }
      }),
      (t[37] = Lt),
      (t[38] = r),
      (t[39] = w),
      (t[40] = gn))
    : (gn = t[40]);
  let _n = gn,
    vn;
  t[41] === b
    ? (vn = t[42])
    : ((vn = () => {
        (b(!1), p.dispatchMessage(`open-in-browser`, { url: ke }));
      }),
      (t[41] = b),
      (t[42] = vn));
  let yn = vn,
    bn;
  t[43] === b
    ? (bn = t[44])
    : ((bn = () => {
        (b(!1), p.dispatchHostMessage({ type: `navigate-to-route`, path: `/settings/usage` }));
      }),
      (t[43] = b),
      (t[44] = bn));
  let xn = bn,
    Sn;
  t[45] !== r || t[46] !== b || t[47] !== R
    ? ((Sn = () => {
        (b(!1),
          be(r, Ue, { initialAvailableCount: R, isRateLimitReached: !1, onResetComplete: at }));
      }),
      (t[45] = r),
      (t[46] = b),
      (t[47] = R),
      (t[48] = Sn))
    : (Sn = t[48]);
  let Cn = Sn,
    wn;
  t[49] !== x || t[50] !== r
    ? ((wn = async () => {
        (await f(r, `use-copilot-auth-if-available`, !1),
          await c(`logout`, { hostId: l }),
          x(`/login`));
      }),
      (t[49] = x),
      (t[50] = r),
      (t[51] = wn))
    : (wn = t[51]);
  let Tn = wn,
    En;
  t[52] !== H || t[53] !== mn
    ? ((En = function (e) {
        return H && H !== mn
          ? (0, k.jsx)(`img`, {
              src: H,
              alt: ``,
              className: v(`rounded-full`, e.className),
              onError: () => {
                hn(H);
              },
            })
          : (0, k.jsx)(ge, { className: e.className });
      }),
      (t[52] = H),
      (t[53] = mn),
      (t[54] = En))
    : (En = t[54]);
  let Dn = En,
    U;
  if (
    t[55] !== Dn ||
    t[56] !== pn ||
    t[57] !== $e ||
    t[58] !== xt ||
    t[59] !== S ||
    t[60] !== Ot ||
    t[61] !== _n ||
    t[62] !== dt ||
    t[63] !== ut ||
    t[64] !== j ||
    t[65] !== it ||
    t[66] !== x ||
    t[67] !== E ||
    t[68] !== tn ||
    t[69] !== r ||
    t[70] !== D ||
    t[71] !== b ||
    t[72] !== wt ||
    t[73] !== Ct ||
    t[74] !== St ||
    t[75] !== yt
  ) {
    if (((U = []), j)) {
      if (S) {
        let e;
        t[77] === _n
          ? (e = t[78])
          : ((e = () => {
              _n();
            }),
            (t[77] = _n),
            (t[78] = e));
        let n;
        t[79] === S
          ? (n = t[80])
          : ((n = (0, k.jsx)(O, { LeftIcon: ze, disabled: !0, children: S })),
            (t[79] = S),
            (t[80] = n));
        let r;
        (t[81] !== e || t[82] !== n
          ? ((r = (0, k.jsx)(`div`, { onClick: e, children: n }, `email`)),
            (t[81] = e),
            (t[82] = n),
            (t[83] = r))
          : (r = t[83]),
          U.push(r));
      }
      if (!$e) {
        let e;
        (t[84] !== Dn || t[85] !== pn
          ? ((e = (0, k.jsx)(O, { LeftIcon: Dn, disabled: !0, children: pn }, `account`)),
            (t[84] = Dn),
            (t[85] = pn),
            (t[86] = e))
          : (e = t[86]),
          U.push(e));
      }
      if (yt) {
        let e;
        t[87] !== xt || t[88] !== Ot || t[89] !== tn || t[90] !== r || t[91] !== b
          ? ((e = () => {
              (b(!1),
                tn({
                  scope: r,
                  currentPlan: xt,
                  getPricingUrl: Ot,
                  source: `profile_dropdown_upgrade_cta`,
                }));
            }),
            (t[87] = xt),
            (t[88] = Ot),
            (t[89] = tn),
            (t[90] = r),
            (t[91] = b),
            (t[92] = e))
          : (e = t[92]);
        let n;
        t[93] === Symbol.for(`react.memo_cache_sentinel`)
          ? ((n = (0, k.jsx)(_, {
              id: `codex.profileDropdown.getPlus`,
              defaultMessage: `Upgrade for higher limits`,
              description: `Menu item in the profile dropdown to upgrade a free account for higher limits`,
            })),
            (t[93] = n))
          : (n = t[93]);
        let i;
        (t[94] === e
          ? (i = t[95])
          : ((i = (0, k.jsx)(
              O,
              { LeftIcon: Ze, RightIcon: le, onClick: e, children: n },
              `upgrade-account`,
            )),
            (t[94] = e),
            (t[95] = i)),
          U.push(i));
      }
    } else if (ut) {
      let e;
      (t[96] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((e = (0, k.jsx)(
            O,
            {
              LeftIcon: ze,
              disabled: !0,
              children: (0, k.jsx)(_, {
                id: `codex.profileDropdown.apiKeyAuth`,
                defaultMessage: `Logged in with API key`,
                description: `Label indicating the user is authenticated with an API key`,
              }),
            },
            `api-key-auth`,
          )),
          (t[96] = e))
        : (e = t[96]),
        U.push(e));
    } else if (dt) {
      let e;
      (t[97] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((e = (0, k.jsx)(
            O,
            {
              LeftIcon: ze,
              disabled: !0,
              children: (0, k.jsx)(_, {
                id: `codex.profileDropdown.amazonBedrockAuth`,
                defaultMessage: `Logged in with Amazon Bedrock`,
                description: `Label indicating the user is authenticated with Amazon Bedrock`,
              }),
            },
            `amazon-bedrock-auth`,
          )),
          (t[97] = e))
        : (e = t[97]),
        U.push(e));
    }
    if (it) {
      let e;
      (t[98] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((e = (0, k.jsx)(
            O,
            {
              LeftIcon: ze,
              disabled: !0,
              children: (0, k.jsx)(_, {
                id: `codex.profileDropdown.copilotAuth`,
                defaultMessage: `Logged in with Copilot`,
                description: `Label indicating the user is authenticated with Copilot`,
              }),
            },
            `copilot-auth`,
          )),
          (t[98] = e))
        : (e = t[98]),
        U.push(e));
    }
    if (St) {
      let e;
      t[99] !== E || t[100] !== r || t[101] !== D || t[102] !== b
        ? ((e = () => {
            (b(!1), f(r, `use-copilot-auth-if-available`, !1), D(E));
          }),
          (t[99] = E),
          (t[100] = r),
          (t[101] = D),
          (t[102] = b),
          (t[103] = e))
        : (e = t[103]);
      let n;
      t[104] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((n = (0, k.jsx)(_, {
            id: `codex.profileDropdown.switchToOpenAIAccount`,
            defaultMessage: `Use OpenAI account`,
            description: `Label showing the option to switch to OpenAI authentication`,
          })),
          (t[104] = n))
        : (n = t[104]);
      let i;
      (t[105] === e
        ? (i = t[106])
        : ((i = (0, k.jsx)(O, { onClick: e, LeftIcon: He, children: n }, `switch-to-openai`)),
          (t[105] = e),
          (t[106] = i)),
        U.push(i));
    } else if (Ct) {
      let e;
      t[107] !== r || t[108] !== D || t[109] !== b
        ? ((e = () => {
            (b(!1), f(r, `use-copilot-auth-if-available`, !0), D(`copilot`));
          }),
          (t[107] = r),
          (t[108] = D),
          (t[109] = b),
          (t[110] = e))
        : (e = t[110]);
      let n;
      t[111] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((n = (0, k.jsx)(_, {
            id: `codex.profileDropdown.switchToCopilotAccount`,
            defaultMessage: `Use Copilot account`,
            description: `Label showing the option to switch to Copilot authentication`,
          })),
          (t[111] = n))
        : (n = t[111]);
      let i;
      (t[112] === e
        ? (i = t[113])
        : ((i = (0, k.jsx)(O, { onClick: e, LeftIcon: He, children: n }, `switch-to-copilot`)),
          (t[112] = e),
          (t[113] = i)),
        U.push(i));
    }
    if (wt) {
      let e;
      t[114] !== x || t[115] !== b
        ? ((e = () => {
            (b(!1), x(`/login`));
          }),
          (t[114] = x),
          (t[115] = b),
          (t[116] = e))
        : (e = t[116]);
      let n;
      t[117] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((n = (0, k.jsx)(_, {
            id: `codex.profileDropdown.signInWithOpenAI`,
            defaultMessage: `Sign in with ChatGPT`,
            description: `Profile menu item to sign in with ChatGPT`,
          })),
          (t[117] = n))
        : (n = t[117]);
      let r;
      (t[118] === e
        ? (r = t[119])
        : ((r = (0, k.jsx)(O, { onClick: e, LeftIcon: ve, children: n }, `sign-in-openai`)),
          (t[118] = e),
          (t[119] = r)),
        U.push(r));
    }
    ((t[55] = Dn),
      (t[56] = pn),
      (t[57] = $e),
      (t[58] = xt),
      (t[59] = S),
      (t[60] = Ot),
      (t[61] = _n),
      (t[62] = dt),
      (t[63] = ut),
      (t[64] = j),
      (t[65] = it),
      (t[66] = x),
      (t[67] = E),
      (t[68] = tn),
      (t[69] = r),
      (t[70] = D),
      (t[71] = b),
      (t[72] = wt),
      (t[73] = Ct),
      (t[74] = St),
      (t[75] = yt),
      (t[76] = U));
  } else U = t[76];
  let On;
  t[120] === n
    ? (On = t[121])
    : ((On =
        n ??
        (0, k.jsx)(te, {
          tooltipContent: (0, k.jsx)(_, {
            id: `codex.header.settingsTooltip`,
            defaultMessage: `Settings`,
            description: `Tooltip text for opening settings`,
          }),
          children: (0, k.jsx)(y, {
            color: `ghost`,
            size: `icon`,
            children: (0, k.jsx)(ge, { className: `icon-xs` }),
          }),
        })),
      (t[120] = n),
      (t[121] = On));
  let W;
  t[122] === U.length
    ? (W = t[123])
    : ((W = U.length > 0 && (0, k.jsx)(Ie.Separator, {})), (t[122] = U.length), (t[123] = W));
  let G;
  t[124] !== Et || t[125] !== x || t[126] !== r || t[127] !== b || t[128] !== P
    ? ((G =
        Et &&
        (0, k.jsx)(O, {
          LeftIcon: ze,
          onClick: () => {
            (ae(r, ie, { source: `profile_dropdown` }),
              b(!1),
              x(`/settings/profile`, { state: P }));
          },
          children: (0, k.jsx)(_, {
            id: `codex.profileDropdown.profile`,
            defaultMessage: `Profile`,
            description: `Menu item to open the Codex profile page`,
          }),
        })),
      (t[124] = Et),
      (t[125] = x),
      (t[126] = r),
      (t[127] = b),
      (t[128] = P),
      (t[129] = G))
    : (G = t[129]);
  let K;
  t[130] !== x || t[131] !== b || t[132] !== P
    ? ((K = () => {
        (b(!1), x(`/settings/${we}`, { state: P }));
      }),
      (t[130] = x),
      (t[131] = b),
      (t[132] = P),
      (t[133] = K))
    : (K = t[133]);
  let kn;
  t[134] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((kn = (0, k.jsx)(_, {
        id: `codex.profileDropdown.settingsPage`,
        defaultMessage: `Settings`,
        description: `Menu item to open Codex settings page`,
      })),
      (t[134] = kn))
    : (kn = t[134]);
  let q;
  t[135] !== It || t[136] !== K
    ? ((q = (0, k.jsx)(O, { LeftIcon: ge, keyboardShortcut: It, onClick: K, children: kn })),
      (t[135] = It),
      (t[136] = K),
      (t[137] = q))
    : (q = t[137]);
  let J;
  t[138] !== G || t[139] !== q
    ? ((J = (0, k.jsxs)(C, { browser: !0, electron: !0, children: [G, q] })),
      (t[138] = G),
      (t[139] = q),
      (t[140] = J))
    : (J = t[140]);
  let Y;
  t[141] === b
    ? (Y = t[142])
    : ((Y = () => {
        (b(!1), p.dispatchMessage(`show-settings`, { section: we }));
      }),
      (t[141] = b),
      (t[142] = Y));
  let An;
  t[143] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((An = (0, k.jsx)(_, {
        id: `codex.profileDropdown.codexSettings`,
        defaultMessage: `{appName} settings`,
        description: `Menu item to view Codex settings`,
        values: { appName: he },
      })),
      (t[143] = An))
    : (An = t[143]);
  let X;
  t[144] === Y
    ? (X = t[145])
    : ((X = (0, k.jsx)(C, {
        extension: !0,
        children: (0, k.jsx)(O, { LeftIcon: _e, onClick: Y, children: An }),
      })),
      (t[144] = Y),
      (t[145] = X));
  let Z;
  t[146] !== j || t[147] !== dn || t[148] !== yn
    ? ((Z =
        j && dn()
          ? (0, k.jsx)(O, {
              LeftIcon: Be,
              RightIcon: le,
              onClick: yn,
              children: (0, k.jsx)(_, {
                id: `codex.profileDropdown.workspaceSettings`,
                defaultMessage: `Workspace settings`,
                description: `Menu item to open ChatGPT workspace settings`,
              }),
            })
          : null),
      (t[146] = j),
      (t[147] = dn),
      (t[148] = yn),
      (t[149] = Z))
    : (Z = t[149]);
  let jn;
  t[150] === b
    ? (jn = t[151])
    : ((jn = () => {
        (b(!1), p.dispatchMessage(`open-keyboard-shortcuts`, {}));
      }),
      (t[150] = b),
      (t[151] = jn));
  let Mn;
  t[152] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((Mn = (0, k.jsx)(_, {
        id: `codex.profileDropdown.keyboardShortcuts`,
        defaultMessage: `Keyboard shortcuts`,
        description: `Menu item to open keyboard shortcuts filtered to this extension`,
      })),
      (t[152] = Mn))
    : (Mn = t[152]);
  let Nn;
  t[153] === jn
    ? (Nn = t[154])
    : ((Nn = (0, k.jsx)(C, {
        extension: !0,
        children: (0, k.jsx)(O, { LeftIcon: Ve, onClick: jn, children: Mn }),
      })),
      (t[153] = jn),
      (t[154] = Nn));
  let Q;
  t[155] !== B || t[156] !== j || t[157] !== V
    ? ((Q = j && V != null ? (0, k.jsx)(st, { isPersonalAccount: B(), layer: V }) : null),
      (t[155] = B),
      (t[156] = j),
      (t[157] = V),
      (t[158] = Q))
    : (Q = t[158]);
  let Pn;
  t[159] === Q
    ? (Pn = t[160])
    : ((Pn = (0, k.jsx)(C, { electron: !0, children: Q })), (t[159] = Q), (t[160] = Pn));
  let Fn;
  t[161] === z
    ? (Fn = t[162])
    : ((Fn = z ? (0, k.jsx)(Ie.Separator, {}) : null), (t[161] = z), (t[162] = Fn));
  let In;
  t[163] === Fn
    ? (In = t[164])
    : ((In = (0, k.jsx)(C, { electron: !0, children: Fn })), (t[163] = Fn), (t[164] = In));
  let Ln;
  t[165] !== I ||
  t[166] !== Cn ||
  t[167] !== xn ||
  t[168] !== F?.plan_type ||
  t[169] !== Vt ||
  t[170] !== r ||
  t[171] !== Ft ||
  t[172] !== z ||
  t[173] !== $t ||
  t[174] !== Ut ||
  t[175] !== nn ||
  t[176] !== en ||
  t[177] !== R ||
  t[178] !== L
    ? ((Ln = z
        ? (0, k.jsx)(We, {
            rateLimits: L,
            activeLimitName: en,
            planType: F?.plan_type,
            suppressUpsell: I ? !1 : Ut,
            selectedModel: Ft,
            availableRateLimitResetCount: R,
            onRateLimitResetClick: Cn,
            isLoading: $t,
            layout: `compact`,
            showLearnMore: !I,
            onRequestLimitIncreaseClick: I ? xn : void 0,
            onPlanUpgradeClick:
              Vt == null
                ? void 0
                : (e) => {
                    nn({
                      scope: r,
                      currentPlan: Vt,
                      defaultTab: `personal`,
                      source: `profile_dropdown_rate_limit_summary`,
                    }) && e.preventDefault();
                  },
          })
        : null),
      (t[165] = I),
      (t[166] = Cn),
      (t[167] = xn),
      (t[168] = F?.plan_type),
      (t[169] = Vt),
      (t[170] = r),
      (t[171] = Ft),
      (t[172] = z),
      (t[173] = $t),
      (t[174] = Ut),
      (t[175] = nn),
      (t[176] = en),
      (t[177] = R),
      (t[178] = L),
      (t[179] = Ln))
    : (Ln = t[179]);
  let Rn;
  t[180] === Ln
    ? (Rn = t[181])
    : ((Rn = (0, k.jsx)(C, { electron: !0, children: Ln })), (t[180] = Ln), (t[181] = Rn));
  let zn;
  t[182] !== Tn || t[183] !== r || t[184] !== b || t[185] !== Tt
    ? ((zn =
        Tt &&
        (0, k.jsx)(O, {
          onClick: () => {
            (b(!1), be(r, lt, { onConfirm: Tn }));
          },
          LeftIcon: He,
          children: (0, k.jsx)(_, {
            id: `codex.profileDropdown.logOut`,
            defaultMessage: `Log out`,
            description: `Menu item to log out of ChatGPT`,
          }),
        })),
      (t[182] = Tn),
      (t[183] = r),
      (t[184] = b),
      (t[185] = Tt),
      (t[186] = zn))
    : (zn = t[186]);
  let $;
  t[187] !== U ||
  t[188] !== W ||
  t[189] !== J ||
  t[190] !== X ||
  t[191] !== Z ||
  t[192] !== Nn ||
  t[193] !== Pn ||
  t[194] !== In ||
  t[195] !== Rn ||
  t[196] !== zn
    ? (($ = (0, k.jsxs)(`div`, {
        className: `flex w-full min-w-0 flex-col gap-0`,
        children: [U, W, J, X, Z, Nn, Pn, In, Rn, zn],
      })),
      (t[187] = U),
      (t[188] = W),
      (t[189] = J),
      (t[190] = X),
      (t[191] = Z),
      (t[192] = Nn),
      (t[193] = Pn),
      (t[194] = In),
      (t[195] = Rn),
      (t[196] = zn),
      (t[197] = $))
    : ($ = t[197]);
  let Bn;
  return (
    t[198] !== a || t[199] !== b || t[200] !== On || t[201] !== $
      ? ((Bn = (0, k.jsx)(Le, {
          open: a,
          onOpenChange: b,
          contentWidth: `panel`,
          triggerButton: On,
          children: $,
        })),
        (t[198] = a),
        (t[199] = b),
        (t[200] = On),
        (t[201] = $),
        (t[202] = Bn))
      : (Bn = t[202]),
    Bn
  );
}
function at() {}
function ot(e) {
  return !!e.snapshot?.primary?.windowDurationMins || !!e.snapshot?.secondary?.windowDurationMins;
}
function st(e) {
  let t = (0, tt.c)(17),
    { isPersonalAccount: n, layer: r } = e,
    i = o(s),
    a = b(`1823918333`),
    ee = b(`3502353992`),
    c = re(r),
    l;
  t[0] === c ? (l = t[1]) : ((l = c.get(`enabled`, !1)), (t[0] = c), (t[1] = l));
  let u = l,
    d = a && u,
    f;
  t[2] === d ? (f = t[3]) : ((f = { enabled: d }), (t[2] = d), (t[3] = f));
  let { data: p } = x(f);
  if (!a || !u || p == null) return null;
  let m =
      ee &&
      p.remaining_referrals === 0 &&
      (p.ineligible_reason_code === `user_limit_reached` ||
        p.ineligible_reason_code === `workspace_limit_reached`),
    h = m && p.ineligible_reason_code === `workspace_limit_reached`;
  if (!p.should_show && !m) return null;
  let g;
  t[4] !== p.grant_action ||
  t[5] !== p.grant_amount ||
  t[6] !== p.remaining_referrals ||
  t[7] !== m ||
  t[8] !== h ||
  t[9] !== n ||
  t[10] !== i
    ? ((g = () => {
        (i.set(Qe, !1),
          be(i, Ge, {
            grantAmount: p.grant_amount,
            hasReachedReferralLimit: m,
            hasReachedWorkspaceReferralLimit: h,
            isPersonalAccount: n,
            referralBeaconType: `rate_limit_reached`,
            referralGrantType: p.grant_action,
            referralKey: fe,
            remainingReferrals: p.remaining_referrals,
          }));
      }),
      (t[4] = p.grant_action),
      (t[5] = p.grant_amount),
      (t[6] = p.remaining_referrals),
      (t[7] = m),
      (t[8] = h),
      (t[9] = n),
      (t[10] = i),
      (t[11] = g))
    : (g = t[11]);
  let v;
  t[12] === n
    ? (v = t[13])
    : ((v = n
        ? (0, k.jsx)(_, {
            id: `codex.profileDropdown.inviteFriend`,
            defaultMessage: `Invite a friend`,
            description: `Menu item to invite a friend through a persistent referral`,
          })
        : (0, k.jsx)(_, {
            id: `codex.profileDropdown.inviteCoworker`,
            defaultMessage: `Invite a coworker`,
            description: `Menu item to invite a coworker through a persistent referral`,
          })),
      (t[12] = n),
      (t[13] = v));
  let y;
  return (
    t[14] !== g || t[15] !== v
      ? ((y = (0, k.jsx)(O, {
          LeftIcon: A,
          leftIconClassName: `icon-xs`,
          onClick: g,
          children: v,
        })),
        (t[14] = g),
        (t[15] = v),
        (t[16] = y))
      : (y = t[16]),
    y
  );
}
function ct(e) {
  let t = D(e);
  return t == null
    ? null
    : {
        limitName: null,
        snapshot: {
          limitId: null,
          limitName: null,
          primary: { usedPercent: t.usedPercent, windowDurationMins: E, resetsAt: null },
          secondary: null,
          credits: null,
          individualLimit: null,
          planType: null,
          rateLimitReachedType: null,
        },
      };
}
function lt(e) {
  let t = (0, tt.c)(20),
    { onClose: n, onConfirm: r } = e,
    i;
  t[0] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((i = (0, k.jsx)(_, {
        id: `codex.profileDropdown.logOutConfirmation.title`,
        defaultMessage: `Log out?`,
        description: `Title for the confirmation dialog shown before logging out from the profile dropdown`,
      })),
      (t[0] = i))
    : (i = t[0]);
  let a = i,
    o;
  t[1] !== n || t[2] !== r
    ? ((o = () => {
        (n(), r());
      }),
      (t[1] = n),
      (t[2] = r),
      (t[3] = o))
    : (o = t[3]);
  let ee = o,
    s;
  t[4] === ee
    ? (s = t[5])
    : ((s = (e) => {
        (e.preventDefault(), ee());
      }),
      (t[4] = ee),
      (t[5] = s));
  let c;
  t[6] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((c = (0, k.jsx)(w, { asChild: !0, children: (0, k.jsx)(`span`, { children: a }) })),
      (t[6] = c))
    : (c = t[6]);
  let l;
  t[7] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((l = (0, k.jsx)(T, {
        className: `pr-8`,
        title: c,
        subtitle: (0, k.jsx)(_, {
          id: `codex.profileDropdown.logOutConfirmation.subtitle`,
          defaultMessage: `You’ll need to sign in again to keep using {appName}`,
          description: `Subtitle for the confirmation dialog shown before logging out from the profile dropdown`,
          values: { appName: he },
        }),
      })),
      (t[7] = l))
    : (l = t[7]);
  let u;
  t[8] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((u = (0, k.jsx)(_, {
        id: `codex.profileDropdown.logOutConfirmation.cancel`,
        defaultMessage: `Cancel`,
        description: `Cancel button label for the log out confirmation dialog`,
      })),
      (t[8] = u))
    : (u = t[8]);
  let d;
  t[9] === n
    ? (d = t[10])
    : ((d = (0, k.jsx)(y, { color: `ghost`, type: `button`, onClick: n, children: u })),
      (t[9] = n),
      (t[10] = d));
  let f;
  t[11] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((f = (0, k.jsx)(y, {
        color: `danger`,
        type: `submit`,
        children: (0, k.jsx)(_, {
          id: `codex.profileDropdown.logOutConfirmation.confirm`,
          defaultMessage: `Log out`,
          description: `Confirm button label for logging out from the profile dropdown`,
        }),
      })),
      (t[11] = f))
    : (f = t[11]);
  let p;
  t[12] === d
    ? (p = t[13])
    : ((p = (0, k.jsxs)(Ce, { className: `gap-2`, children: [d, f] })), (t[12] = d), (t[13] = p));
  let m;
  t[14] !== s || t[15] !== p
    ? ((m = (0, k.jsxs)(Se, {
        as: `form`,
        className: `gap-5 px-6 py-6`,
        onSubmit: s,
        children: [l, p],
      })),
      (t[14] = s),
      (t[15] = p),
      (t[16] = m))
    : (m = t[16]);
  let h;
  return (
    t[17] !== n || t[18] !== m
      ? ((h = (0, k.jsx)(xe, { open: !0, onOpenChange: n, size: `narrow`, children: m })),
        (t[17] = n),
        (t[18] = m),
        (t[19] = h))
      : (h = t[19]),
    h
  );
}
export { et as n, Ze as r, it as t };
//# sourceMappingURL=profile-dropdown.js.map
