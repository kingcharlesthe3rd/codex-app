import { n as e } from "./rolldown-runtime.js";
import {
  Tl as t,
  Xs as n,
  ec as r,
} from "./app-initial~app-main~worktree-init-v2-page~appgen-settings-page~page~appgen-publication-ter~fzo5ij6p.js";
import { pa as i, xa as a } from "./app-initial~app-main~new-thread-panel-page.js";
import {
  Ht as o,
  Vt as s,
} from "./app-initial~app-main~remote-conversation-page~new-thread-panel-page~appgen-library-page~hot~djo67r4n.js";
function c(e, t, n, a, o, c) {
  let u = (0, l.c)(9),
    d = e ?? null,
    f;
  u[0] === c ? (f = u[1]) : ((f = i(c, null)), (u[0] = c), (u[1] = f));
  let p;
  return (
    u[2] !== t || u[3] !== n || u[4] !== o || u[5] !== a || u[6] !== d || u[7] !== f
      ? ((p = { cwd: d, hostConfig: t, key: n, operationSource: o, scope: a, ...f }),
        (u[2] = t),
        (u[3] = n),
        (u[4] = o),
        (u[5] = a),
        (u[6] = d),
        (u[7] = f),
        (u[8] = p))
      : (p = u[8]),
    r(s, p)
  );
}
var l,
  u = e(() => {
    ((l = t()), n(), o(), a());
  });
export { c as n, u as t };
//# sourceMappingURL=use-git-config-value.js.map
