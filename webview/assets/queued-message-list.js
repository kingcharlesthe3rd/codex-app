import { n as e, s as t } from "./rolldown-runtime.js";
import {
  El as n,
  Tl as r,
  vl as i,
} from "./app-initial~app-main~worktree-init-v2-page~appgen-settings-page~page~appgen-publication-ter~fzo5ij6p.js";
import {
  $C as a,
  Cd as o,
  Fh as s,
  Ph as c,
  Sd as l,
  Td as u,
  Ti as d,
  Tw as f,
  XC as p,
  _i as m,
  cw as ee,
  ew as h,
  gi as g,
  lw as _,
  nw as v,
  ow as y,
  sw as b,
  vm as x,
  wd as S,
  wi as C,
  ww as w,
  xd as te,
  ym as T,
} from "./app-initial~app-main~onboarding-page.js";
import {
  $ as ne,
  In as re,
  O as E,
  On as D,
  Rn as ie,
  S as ae,
  tt as oe,
  w as se,
} from "./app-initial~app-main~remote-conversation-page~new-thread-panel-page~onboarding-page~project~okyna5mq.js";
import {
  Sx as O,
  co as ce,
  lv as le,
  nx as ue,
  oo as de,
  rx as fe,
  uv as k,
  vx as A,
  wx as pe,
  yx as me,
} from "./app-initial~app-main~new-thread-panel-page.js";
import { lr as j, or as M, sr as N } from "./app-initial~app-main~automations-page.js";
import {
  Q as P,
  Z as he,
} from "./app-initial~app-main~worktree-init-v2-page~remote-conversation-page~pull-requests-page~new-~djgpfzje.js";
import {
  Rt as ge,
  zt as F,
} from "./app-initial~app-main~worktree-init-v2-page~remote-conversation-page~pull-requests-page~new-~mam8fk3d.js";
import {
  $ as _e,
  dt as I,
  et as L,
  ft as ve,
  lt as R,
  ut as z,
} from "./app-initial~app-main~remote-conversation-page~new-thread-panel-page~appgen-library-page~hot~djo67r4n.js";
import {
  i as B,
  r as ye,
} from "./app-initial~app-main~appgen-page~remote-conversation-page~plugin-detail-page~onboarding-pag~f3o4q9dk.js";
var V,
  be,
  H = e(() => {
    (t(n()),
      (V = i()),
      (be = (e) =>
        (0, V.jsx)(`svg`, {
          width: 20,
          height: 20,
          viewBox: `0 0 20 20`,
          fill: `none`,
          xmlns: `http://www.w3.org/2000/svg`,
          ...e,
          children: (0, V.jsx)(`path`, {
            d: `M2.66797 11V3.33301C2.66797 2.96574 2.96574 2.66797 3.33301 2.66797C3.70028 2.66797 3.99805 2.96574 3.99805 3.33301V11C3.99805 11.7109 3.99894 12.2044 4.03027 12.5879C4.06098 12.9634 4.11776 13.175 4.19824 13.333L4.26856 13.459C4.44487 13.7465 4.69781 13.9808 5 14.1348L5.12988 14.1904C5.27366 14.2419 5.46311 14.2797 5.74512 14.3027C6.12864 14.3341 6.62197 14.335 7.33301 14.335H15L15.0674 14.3418L14.1123 13.3867L14.0273 13.2822C13.8571 13.0242 13.8854 12.6735 14.1123 12.4463C14.3397 12.2189 14.6911 12.1906 14.9492 12.3613L15.0537 12.4463L17.1367 14.5293C17.3964 14.7889 17.3963 15.21 17.1367 15.4697L15.0537 17.5537C14.794 17.8134 14.372 17.8134 14.1123 17.5537C13.8526 17.294 13.8526 16.872 14.1123 16.6123L15.0664 15.6582L15 15.665H7.33301C6.64392 15.665 6.08696 15.6647 5.63672 15.6279C5.23614 15.5952 4.87531 15.5309 4.53906 15.3867L4.39649 15.3193C3.87528 15.0538 3.43887 14.6502 3.13477 14.1543L3.0127 13.9365C2.82084 13.5599 2.74153 13.1541 2.7041 12.6963C2.66732 12.2461 2.66797 11.6889 2.66797 11ZM15.665 15C15.665 15.0226 15.6594 15.0444 15.6572 15.0664L15.7256 14.999L15.6572 14.9316C15.6595 14.9541 15.665 14.9769 15.665 15ZM11.666 8.91797L11.8008 8.93164C12.1036 8.99381 12.3311 9.2618 12.3311 9.58301C12.3311 9.90422 12.1036 10.1722 11.8008 10.2344L11.666 10.248H7.5C7.13273 10.248 6.83496 9.95028 6.83496 9.58301C6.83496 9.21574 7.13273 8.91797 7.5 8.91797H11.666ZM14.166 4.33496L14.3008 4.34863C14.6036 4.41083 14.8311 4.67881 14.8311 5C14.8309 5.32109 14.6035 5.58924 14.3008 5.65137L14.166 5.66504H7.5C7.13284 5.66504 6.83514 5.36712 6.83496 5C6.83496 4.63273 7.13273 4.33496 7.5 4.33496H14.166Z`,
            fill: `currentColor`,
          }),
        })));
  }),
  U,
  xe,
  W = e(() => {
    (t(n()),
      (U = i()),
      (xe = (e) =>
        (0, U.jsx)(`svg`, {
          width: 21,
          height: 21,
          viewBox: `0 0 21 21`,
          fill: `none`,
          xmlns: `http://www.w3.org/2000/svg`,
          ...e,
          children: (0, U.jsx)(`path`, {
            d: `M13.1293 7.34753C13.3565 7.12027 13.7081 7.09207 13.9662 7.26257L14.0707 7.34753L18.0707 11.3475C18.3304 11.6072 18.3304 12.0292 18.0707 12.2889L14.0707 16.2889C13.811 16.5486 13.389 16.5486 13.1293 16.2889C12.8696 16.0292 12.8696 15.6072 13.1293 15.3475L15.9935 12.4833H6.59998C4.57585 12.4833 2.93494 10.8424 2.93494 8.81824V5.31824C2.93494 4.95097 3.23271 4.6532 3.59998 4.6532C3.96724 4.6532 4.26501 4.95097 4.26501 5.31824V8.81824C4.26501 10.1078 5.31039 11.1532 6.59998 11.1532H15.9935L13.1293 8.28894L13.0443 8.18445C12.8738 7.92632 12.902 7.5748 13.1293 7.34753Z`,
            fill: `currentColor`,
          }),
        })));
  });
function G(e) {
  let t = (0, we.c)(40),
    {
      messages: n,
      isMessagePaused: r,
      isInterrupted: i,
      editingMessageId: a,
      hostId: o,
      isQueueingEnabled: s,
      onEditMessage: c,
      onDeleteMessage: l,
      onSendNowMessage: d,
      onReorderMessages: f,
      onQueueingChange: m,
      onResumeInterruptedQueue: ee,
    } = e,
    h = pe(),
    g;
  t[0] === n ? (g = t[1]) : ((g = n.map(Se)), (t[0] = n), (t[1] = g));
  let _ = g,
    v;
  t[2] !== _ || t[3] !== f
    ? ((v = { messageIds: _, onReorderMessages: f }), (t[2] = _), (t[3] = f), (t[4] = v))
    : (v = t[4]);
  let y = q(v);
  if (n.length === 0) return null;
  let b;
  t[5] !== i || t[6] !== ee
    ? ((b = i
        ? (0, J.jsx)(_e, {
            title: (0, J.jsx)(O, {
              id: `composer.queuedMessage.interruptedQueue`,
              defaultMessage: `Queue paused because you interrupted`,
              description: `Header shown above queued messages paused because the user interrupted the running turn`,
            }),
            actions: (0, J.jsx)(he, {
              color: `ghost`,
              onClick: ee,
              children: (0, J.jsx)(O, {
                id: `composer.queuedMessage.resumeInterruptedQueue`,
                defaultMessage: `Resume`,
                description: `Button label to resume queued messages paused by an interruption`,
              }),
            }),
          })
        : null),
      (t[5] = i),
      (t[6] = ee),
      (t[7] = b))
    : (b = t[7]);
  let x;
  if (
    t[8] !== a ||
    t[9] !== o ||
    t[10] !== h ||
    t[11] !== r ||
    t[12] !== s ||
    t[13] !== n ||
    t[14] !== l ||
    t[15] !== c ||
    t[16] !== m ||
    t[17] !== d
  ) {
    let e;
    (t[19] !== a ||
    t[20] !== o ||
    t[21] !== h ||
    t[22] !== r ||
    t[23] !== s ||
    t[24] !== l ||
    t[25] !== c ||
    t[26] !== m ||
    t[27] !== d
      ? ((e = (e) => {
          let {
              annotationCommentAttachments: t,
              designTweakCommentAttachments: n,
              diffCommentAttachments: i,
            } = ve(e.context.commentAttachments ?? []),
            u = le(e.context.fileAttachments, e.context.generatedPastedTextAttachmentPaths);
          return (0, J.jsx)(
            K,
            {
              messageId: e.id,
              messageText: Ce(h, {
                text: ce(e.text),
                browserCommentCount: t.length,
                designTweakCount: n.length,
                diffCommentCount: i.length,
                imageCommentCount: e.context.imageCommentDraft?.comments.length ?? 0,
                pastedTextAttachmentPreview:
                  e.context.pastedTextAttachments?.[0]?.preview ?? (u == null ? void 0 : ``),
                pastedTextAttachmentCount: e.context.pastedTextAttachments?.length ?? u?.length,
                selectedTextAttachmentCount: e.context.selectedTextAttachments?.length,
              }),
              isPaused: r(e.pausedReason),
              hostId: o,
              isEditing: e.id === a,
              isQueueingEnabled: s,
              onEditMessage: c,
              onDeleteMessage: l,
              onSendNowMessage: d,
              onQueueingChange: m,
            },
            e.id,
          );
        }),
        (t[19] = a),
        (t[20] = o),
        (t[21] = h),
        (t[22] = r),
        (t[23] = s),
        (t[24] = l),
        (t[25] = c),
        (t[26] = m),
        (t[27] = d),
        (t[28] = e))
      : (e = t[28]),
      (x = n.map(e)),
      (t[8] = a),
      (t[9] = o),
      (t[10] = h),
      (t[11] = r),
      (t[12] = s),
      (t[13] = n),
      (t[14] = l),
      (t[15] = c),
      (t[16] = m),
      (t[17] = d),
      (t[18] = x));
  } else x = t[18];
  let S;
  t[29] === x
    ? (S = t[30])
    : ((S = (0, J.jsx)(ie, { initial: !1, children: x })), (t[29] = x), (t[30] = S));
  let C;
  t[31] !== _ || t[32] !== S
    ? ((C = (0, J.jsx)(te, { items: _, strategy: u, children: S })),
      (t[31] = _),
      (t[32] = S),
      (t[33] = C))
    : (C = t[33]);
  let w;
  t[34] !== y || t[35] !== C
    ? ((w = (0, J.jsx)(p, { ...y, children: C })), (t[34] = y), (t[35] = C), (t[36] = w))
    : (w = t[36]);
  let T;
  return (
    t[37] !== b || t[38] !== w
      ? ((T = (0, J.jsx)(R, {
          children: (0, J.jsxs)(`div`, {
            className: `vertical-scroll-fade-mask hide-scrollbar flex max-h-[30dvh] flex-col gap-px overflow-x-hidden overflow-y-auto px-3 py-row-y`,
            children: [b, w],
          }),
        })),
        (t[37] = b),
        (t[38] = w),
        (t[39] = T))
      : (T = t[39]),
    T
  );
}
function Se(e) {
  return e.id;
}
function K(e) {
  let t = (0, we.c)(92),
    {
      messageId: n,
      messageText: r,
      isPaused: i,
      hostId: a,
      isEditing: o,
      isQueueingEnabled: s,
      onEditMessage: l,
      onDeleteMessage: u,
      onSendNowMessage: d,
      onQueueingChange: f,
    } = e,
    p = pe(),
    m;
  t[0] === n ? (m = t[1]) : ((m = { id: n }), (t[0] = n), (t[1] = m));
  let {
      attributes: h,
      listeners: g,
      setNodeRef: _,
      setActivatorNodeRef: v,
      transform: y,
      transition: b,
      isDragging: te,
    } = S(m),
    T = i ? Te.retryQueuedMessage : Te.sendQueuedMessageNow,
    E;
  t[2] === y ? (E = t[3]) : ((E = ee.Translate.toString(y)), (t[2] = y), (t[3] = E));
  let D;
  t[4] !== E || t[5] !== b
    ? ((D = { transform: E, transition: b }), (t[4] = E), (t[5] = b), (t[6] = D))
    : (D = t[6]);
  let ie, oe, ce, le;
  t[7] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((ie = { height: 0, opacity: 0 }),
      (oe = { height: `auto`, opacity: 1 }),
      (ce = { height: 0, opacity: 0 }),
      (le = { duration: 0.18 }),
      (t[7] = ie),
      (t[8] = oe),
      (t[9] = ce),
      (t[10] = le))
    : ((ie = t[7]), (oe = t[8]), (ce = t[9]), (le = t[10]));
  let de = o || te ? `opacity-60` : void 0,
    fe = te ? `opacity-100` : `opacity-0 group-hover:opacity-100`,
    k;
  t[11] === fe
    ? (k = t[12])
    : ((k = ue(
        `icon-2xs text-token-input-placeholder-foreground/70 pointer-events-none absolute left-0 top-1/2 -translate-y-1/2 transition-opacity`,
        fe,
      )),
      (t[11] = fe),
      (t[12] = k));
  let A;
  t[13] === k
    ? (A = t[14])
    : ((A = (0, J.jsx)(c, { className: k, "aria-hidden": !0 })), (t[13] = k), (t[14] = A));
  let me;
  t[15] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((me = (0, J.jsx)(be, { className: `icon-2xs text-token-input-placeholder-foreground/70` })),
      (t[15] = me))
    : (me = t[15]);
  let j;
  t[16] !== h || t[17] !== g || t[18] !== v || t[19] !== A
    ? ((j = (0, J.jsxs)(`span`, {
        ref: v,
        className: `relative -ml-3 flex h-4 cursor-grab items-center justify-center pl-3 active:cursor-grabbing`,
        ...h,
        ...g,
        children: [A, me],
      })),
      (t[16] = h),
      (t[17] = g),
      (t[18] = v),
      (t[19] = A),
      (t[20] = j))
    : (j = t[20]);
  let M;
  t[21] === i
    ? (M = t[22])
    : ((M = i
        ? (0, J.jsx)(ne, {
            side: `top`,
            tooltipClassName: `max-w-80`,
            tooltipBodyClassName: `text-center whitespace-normal leading-snug`,
            tooltipContent: (0, J.jsxs)(`div`, {
              className: `space-y-1 text-center`,
              children: [
                (0, J.jsx)(`p`, {
                  children: (0, J.jsx)(O, {
                    id: `composer.queuedMessage.pausedTooltip`,
                    defaultMessage: `This queued message could not be sent`,
                    description: `Primary tooltip text for a queued message that failed to send`,
                  }),
                }),
                (0, J.jsx)(`p`, {
                  className: `text-token-description-foreground`,
                  children: (0, J.jsx)(O, {
                    id: `composer.queuedMessage.pausedTooltipRemedy`,
                    defaultMessage: `Retry, edit, or delete it to continue the queue`,
                    description: `Secondary tooltip text explaining how to resolve a queued message that failed to send`,
                  }),
                }),
              ],
            }),
            children: (0, J.jsx)(`span`, {
              className: `mt-0.5 inline-flex shrink-0`,
              children: (0, J.jsx)(w, {
                className: `icon-2xs text-token-editor-warning-foreground`,
              }),
            }),
          })
        : null),
      (t[21] = i),
      (t[22] = M));
  let N;
  t[23] !== a || t[24] !== r
    ? ((N = (0, J.jsx)(C, {
        className: `line-clamp-2 min-w-0 leading-4 text-token-text-secondary`,
        text: r,
        cwd: null,
        hostId: a,
      })),
      (t[23] = a),
      (t[24] = r),
      (t[25] = N))
    : (N = t[25]);
  let P;
  t[26] !== M || t[27] !== N
    ? ((P = (0, J.jsxs)(`div`, {
        className: `flex min-w-0 items-start gap-1.5`,
        children: [M, N],
      })),
      (t[26] = M),
      (t[27] = N),
      (t[28] = P))
    : (P = t[28]);
  let F;
  t[29] === i
    ? (F = t[30])
    : ((F = i
        ? (0, J.jsxs)(`div`, {
            className: `space-y-1 text-center`,
            children: [
              (0, J.jsx)(`p`, {
                children: (0, J.jsx)(O, {
                  id: `composer.queuedMessage.retryTooltip`,
                  defaultMessage: `Try sending this queued message again`,
                  description: `Primary tooltip text for retrying a queued follow-up that failed to send`,
                }),
              }),
              (0, J.jsx)(`p`, {
                className: `text-token-description-foreground`,
                children: (0, J.jsx)(O, {
                  id: `composer.queuedMessage.retryTooltipRemedy`,
                  defaultMessage: `Edit or delete it if retry keeps failing`,
                  description: `Secondary tooltip text explaining alternatives when retrying a queued follow-up keeps failing`,
                }),
              }),
            ],
          })
        : (0, J.jsx)(O, {
            id: `composer.queuedMessage.sendNowTooltip`,
            defaultMessage: `Submit without interrupting the model`,
            description: `Primary tooltip text for steering with a queued follow-up without interrupting the current model run`,
          })),
      (t[29] = i),
      (t[30] = F));
  let I;
  t[31] !== p || t[32] !== T
    ? ((I = p.formatMessage(T)), (t[31] = p), (t[32] = T), (t[33] = I))
    : (I = t[33]);
  let L;
  t[34] !== n || t[35] !== d
    ? ((L = (e) => {
        (e.stopPropagation(), d(n));
      }),
      (t[34] = n),
      (t[35] = d),
      (t[36] = L))
    : (L = t[36]);
  let ve;
  t[37] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((ve = (0, J.jsx)(xe, { className: `icon-2xs shrink-0` })), (t[37] = ve))
    : (ve = t[37]);
  let R;
  t[38] === T ? (R = t[39]) : ((R = (0, J.jsx)(O, { ...T })), (t[38] = T), (t[39] = R));
  let z;
  t[40] !== I || t[41] !== L || t[42] !== R
    ? ((z = (0, J.jsxs)(he, {
        className: `cursor-interaction gap-1 px-2`,
        "aria-label": I,
        color: `ghost`,
        size: `default`,
        onClick: L,
        children: [ve, R],
      })),
      (t[40] = I),
      (t[41] = L),
      (t[42] = R),
      (t[43] = z))
    : (z = t[43]);
  let B;
  t[44] !== F || t[45] !== z
    ? ((B = (0, J.jsx)(ne, {
        side: `top`,
        tooltipClassName: `max-w-80`,
        tooltipBodyClassName: `text-center whitespace-normal leading-snug`,
        tooltipContent: F,
        children: z,
      })),
      (t[44] = F),
      (t[45] = z),
      (t[46] = B))
    : (B = t[46]);
  let V;
  t[47] === p
    ? (V = t[48])
    : ((V = p.formatMessage({
        id: `composer.queuedMessage.delete`,
        defaultMessage: `Delete queued message`,
        description: `Aria label for deleting a queued message`,
      })),
      (t[47] = p),
      (t[48] = V));
  let H;
  t[49] !== n || t[50] !== u
    ? ((H = (e) => {
        (e.stopPropagation(), u(n));
      }),
      (t[49] = n),
      (t[50] = u),
      (t[51] = H))
    : (H = t[51]);
  let U;
  t[52] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((U = (0, J.jsx)(x, { className: `icon-2xs` })), (t[52] = U))
    : (U = t[52]);
  let W;
  t[53] !== V || t[54] !== H
    ? ((W = (0, J.jsx)(he, {
        className: `[&>svg]:icon-2xs`,
        "aria-label": V,
        color: `ghost`,
        size: `icon`,
        onClick: H,
        children: U,
      })),
      (t[53] = V),
      (t[54] = H),
      (t[55] = W))
    : (W = t[55]);
  let G;
  t[56] === p
    ? (G = t[57])
    : ((G = p.formatMessage({
        id: `composer.queuedMessage.more`,
        defaultMessage: `Queued message actions`,
        description: `Aria label for the queued message row actions menu`,
      })),
      (t[56] = p),
      (t[57] = G));
  let Se;
  t[58] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((Se = (0, J.jsx)(ye, { className: `icon-2xs` })), (t[58] = Se))
    : (Se = t[58]);
  let K;
  t[59] === G
    ? (K = t[60])
    : ((K = (0, J.jsx)(he, {
        className: `[&>svg]:icon-2xs`,
        "aria-label": G,
        color: `ghost`,
        size: `icon`,
        children: Se,
      })),
      (t[59] = G),
      (t[60] = K));
  let q;
  t[61] !== n || t[62] !== l
    ? ((q = (e) => {
        (e.stopPropagation(), l(n));
      }),
      (t[61] = n),
      (t[62] = l),
      (t[63] = q))
    : (q = t[63]);
  let Ce;
  t[64] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((Ce = (0, J.jsx)(O, {
        id: `composer.queuedMessage.edit`,
        defaultMessage: `Edit message`,
        description: `Menu item to edit a queued message`,
      })),
      (t[64] = Ce))
    : (Ce = t[64]);
  let Y;
  t[65] === q
    ? (Y = t[66])
    : ((Y = (0, J.jsx)(se.Item, {
        LeftIcon: ge,
        className: `text-token-description-foreground`,
        onClick: q,
        children: Ce,
      })),
      (t[65] = q),
      (t[66] = Y));
  let X;
  t[67] !== s || t[68] !== f
    ? ((X = (e) => {
        (e.stopPropagation(), f(!s));
      }),
      (t[67] = s),
      (t[68] = f),
      (t[69] = X))
    : (X = t[69]);
  let Z;
  t[70] === s
    ? (Z = t[71])
    : ((Z = s
        ? (0, J.jsx)(O, {
            id: `composer.queuedMessage.turnOff`,
            defaultMessage: `Turn off queueing`,
            description: `Menu item to switch the default follow up behavior to steer`,
          })
        : (0, J.jsx)(O, {
            id: `composer.queuedMessage.turnOn`,
            defaultMessage: `Turn on queueing`,
            description: `Menu item to switch the default follow up behavior to queue`,
          })),
      (t[70] = s),
      (t[71] = Z));
  let Q;
  t[72] !== X || t[73] !== Z
    ? ((Q = (0, J.jsx)(se.Item, {
        LeftIcon: be,
        className: `text-token-description-foreground`,
        onClick: X,
        children: Z,
      })),
      (t[72] = X),
      (t[73] = Z),
      (t[74] = Q))
    : (Q = t[74]);
  let Ee;
  t[75] !== K || t[76] !== Y || t[77] !== Q
    ? ((Ee = (0, J.jsxs)(ae, { align: `end`, triggerButton: K, children: [Y, Q] })),
      (t[75] = K),
      (t[76] = Y),
      (t[77] = Q),
      (t[78] = Ee))
    : (Ee = t[78]);
  let De;
  t[79] !== B || t[80] !== W || t[81] !== Ee
    ? ((De = (0, J.jsxs)(J.Fragment, { children: [B, W, Ee] })),
      (t[79] = B),
      (t[80] = W),
      (t[81] = Ee),
      (t[82] = De))
    : (De = t[82]);
  let $;
  t[83] !== j || t[84] !== P || t[85] !== De || t[86] !== de
    ? (($ = (0, J.jsx)(_e, { className: de, icon: j, title: P, actions: De })),
      (t[83] = j),
      (t[84] = P),
      (t[85] = De),
      (t[86] = de),
      (t[87] = $))
    : ($ = t[87]);
  let Oe;
  return (
    t[88] !== _ || t[89] !== D || t[90] !== $
      ? ((Oe = (0, J.jsx)(re.div, {
          ref: _,
          style: D,
          initial: ie,
          animate: oe,
          exit: ce,
          transition: le,
          className: `overflow-visible`,
          children: $,
        })),
        (t[88] = _),
        (t[89] = D),
        (t[90] = $),
        (t[91] = Oe))
      : (Oe = t[91]),
    Oe
  );
}
function q(e) {
  let t = (0, we.c)(8),
    { messageIds: n, onReorderMessages: r } = e,
    i;
  t[0] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((i = { activationConstraint: { distance: 6 } }), (t[0] = i))
    : (i = t[0]);
  let o = b(y(a, i)),
    s;
  t[1] === Symbol.for(`react.memo_cache_sentinel`) ? ((s = [j, N]), (t[1] = s)) : (s = t[1]);
  let c;
  t[2] !== n || t[3] !== r
    ? ((c = (e) => {
        let { active: t, over: i } = e;
        if (!i) return;
        let a = String(t.id),
          o = String(i.id);
        if (a === o) return;
        let s = n.indexOf(a),
          c = n.indexOf(o);
        s === -1 || c === -1 || r(l(n, s, c));
      }),
      (t[2] = n),
      (t[3] = r),
      (t[4] = c))
    : (c = t[4]);
  let u;
  return (
    t[5] !== o || t[6] !== c
      ? ((u = { sensors: o, collisionDetection: h, modifiers: s, onDragEnd: c }),
        (t[5] = o),
        (t[6] = c),
        (t[7] = u))
      : (u = t[7]),
    u
  );
}
function Ce(
  e,
  {
    text: t,
    browserCommentCount: n,
    designTweakCount: r,
    diffCommentCount: i,
    imageCommentCount: a,
    pastedTextAttachmentCount: o = 0,
    pastedTextAttachmentPreview: s,
    selectedTextAttachmentCount: c = 0,
  },
) {
  if (t.trim().length > 0) return t;
  if (s != null) {
    let t =
      s ||
      e.formatMessage({
        id: `composer.queuedMessage.pastedTextAttachment`,
        defaultMessage: `Pasted text`,
        description: `Summary shown for an attached large text paste without displayable text in a queued message`,
      });
    return o <= 1
      ? t
      : e.formatMessage(
          {
            id: `composer.queuedMessage.additionalPastedTextAttachments`,
            defaultMessage: `{preview} (+{remainingCount, plural, one {# more pasted text attachment} other {# more pasted text attachments}})`,
            description: `Summary shown for a queued message with multiple attached large text pastes`,
          },
          { preview: t, remainingCount: o - 1 },
        );
  }
  return n > 0 || r > 0 || i > 0 || a > 0
    ? g(e, { annotationCount: n, designTweakCount: r, commentCount: i + a })
    : c > 0
      ? e.formatMessage(
          {
            id: `selectedTextAttachments.numSelections`,
            defaultMessage: `{count, plural, one {# selection} other {# selections}}`,
            description: `Number of selected text snippets attached to the message`,
          },
          { count: c },
        )
      : ``;
}
var we, J, Te;
e(() => {
  ((we = r()),
    v(),
    M(),
    o(),
    _(),
    fe(),
    D(),
    me(),
    P(),
    E(),
    oe(),
    d(),
    m(),
    I(),
    s(),
    H(),
    F(),
    W(),
    B(),
    T(),
    f(),
    L(),
    z(),
    k(),
    de(),
    (J = i()),
    (Te = A({
      retryQueuedMessage: {
        id: `composer.queuedMessage.retry`,
        defaultMessage: `Retry`,
        description: `Button label to retry a queued follow-up that failed to send`,
      },
      sendQueuedMessageNow: {
        id: `composer.queuedMessage.sendNow`,
        defaultMessage: `Steer`,
        description: `Button label for sending a queued follow-up as a steer`,
      },
    })));
})();
export { G as QueuedMessageList };
//# sourceMappingURL=queued-message-list.js.map
