...
function S({ durationMs: e }) {
  return { durationMs: e, status: u.CODEX_PRIMARY_RUNTIME_DEPENDENCIES_DIAGNOSE_STATUS_FAILED };
}
...
async function k({ formatMessage: e, hostId: n, productLogger: r, release: i, toast: o }) {
  let s = Date.now(),
    c = o.info(
      e({
  ...
  try {
    ...
    if (A(n)) {
      (r.logProductEvent(
        a,
        b({ bundleVersion: null, durationMs: Date.now() - s, release: i, status: `canceled` }),
      ),
        o.info(
          e({
    ...
    (t.error(`Error installing primary runtime`, { safe: { release: i }, sensitive: { error: n } }),
      r.logProductEvent(
        a,
        b({ bundleVersion: null, durationMs: Date.now() - s, release: i, status: `failed` }),
      ),
      o.danger(
        e({
