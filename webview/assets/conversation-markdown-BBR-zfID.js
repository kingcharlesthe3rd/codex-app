...
function se({ cwd: e = null, projectlessOutputDirectory: t = null, title: n, turns: r }) {
  let i = [],
    a = U(e);
...
function ce(e, n, r, a) {
  let s = [],
            ...
            }),
    {
      ...
      l == null
        ? e.items
        : e.items.map((e) =>
            e.type === `assistant-message` ? { ...e, content: d(e.content, l) } : e,
          ),
      e.status,
    );
  ...
  let D = [],
    ...
    k = ie({
      units: te({
        units: ee({ units: ne(O), isActivitySliceClosed: f != null, conversationDetailLevel: Q }),
      }),
  ...
  (x != null && F(D, b(x, c)), S != null && F(D, b(S, c)), w != null && F(D, b(w, c)));
...
function ue(e) {
  let t = Object.entries(e.changes).flatMap(([e, t]) => {
    ...
    let r = Be(n);
    return [I(`${ze(t.type)} ${q(e)} +${r.additions} -${r.deletions}`, J(`diff`, n))];
  });
  return (
    F(t, k(e.automaticApprovalReviews)),
    t.length === 0
