import { r as e } from "./lib-1.js";
import { r as t } from "./onboarding-mail-provider.js";
var n = 3,
  r = e({
...
function a(e) {
  return e === `default` ? `engineering` : e in i ? e : `something_else`;
}
function o(e) {
...
function s({ roles: e }) {
  return {
    connectAppsRowMessage: r,
    role: a(e[0] ?? `something_else`),
    suggestionPrompts: c({ roles: e, promptsByRole: i }),
  };
}
function c({ roles: e, promptsByRole: t, limit: r = n }) {
  let i = o(e).map((e) => t[e]),
    a = i.map(() => 0),
    s = [],
    c = new Set();
  for (let [e, t] of i.entries()) {
    let n = t[a[e]];
    if (((a[e] += 1), !(!n || c.has(n)) && (c.add(n), s.push(n), s.length >= r))) return s;
  }
  for (; s.length < r; ) {
    let e = !1;
    for (let t = 0; t < i.length; t += 1) {
      let n = i[t][a[t]];
      if (((a[t] += 1), !(!n || c.has(n)) && (c.add(n), s.push(n), (e = !0), s.length >= r))) break;
    }
    if (!e) break;
  }
  return s;
...
function E({ intl: e, mailProvider: n, plan: r = null, plugins: i = [], roles: a }) {
  let o = a[0] ?? `something_else`,
    s = t(n, r);
  return T[o].flatMap((t) => {
...
export { r as n, s as r, E as t };
//# sourceMappingURL=onboarding-plugin-suggestions.js.map
