import { n as e, s as t } from "./rolldown-runtime.js";
import {
  At as n,
  Bt as r,
  Dt as i,
  El as a,
  Et as o,
  H as s,
  K as c,
  Rt as l,
  Tl as u,
  V as d,
  W as f,
  Xs as p,
  bt as m,
  ec as h,
  el as g,
  kt as _,
  nc as v,
  nl as y,
  qc as b,
  tc as x,
  uo as S,
  vl as C,
  xt as w,
  zt as T,
} from "./app-initial~app-main~worktree-init-v2-page~appgen-settings-page~page~appgen-publication-ter~fzo5ij6p.js";
import {
  An as E,
  Cc as D,
  Qn as ee,
  Tc as O,
  Zn as k,
  _c as A,
  _s as j,
  gc as te,
  gs as ne,
  ic as M,
  kn as re,
  mS as ie,
  oc as ae,
  pS as oe,
  pc as se,
  wc as ce,
  xc as le,
  yc as ue,
} from "./app-initial~app-main~onboarding-page.js";
import {
  $ as de,
  C as N,
  G as fe,
  O as pe,
  S as me,
  T as P,
  W as he,
  ct as ge,
  lt as _e,
  tt as ve,
  w as F,
} from "./app-initial~app-main~remote-conversation-page~new-thread-panel-page~onboarding-page~project~okyna5mq.js";
import {
  $h as ye,
  Di as be,
  Ei as xe,
  Gd as Se,
  Hb as Ce,
  Ib as we,
  Lm as Te,
  Ln as Ee,
  Qs as De,
  Rb as Oe,
  Rs as ke,
  Sx as I,
  Vb as Ae,
  Yn as je,
  Zs as Me,
  b as Ne,
  fv as Pe,
  m as Fe,
  nx as L,
  p as Ie,
  pv as Le,
  qd as Re,
  rx as R,
  v as ze,
  wx as Be,
  x as Ve,
  y as z,
  yx as He,
} from "./app-initial~app-main~new-thread-panel-page.js";
import {
  Al as Ue,
  Cu as We,
  Fi as Ge,
  Il as Ke,
  Li as qe,
  Ri as Je,
  Su as Ye,
  Tl as Xe,
  Ul as Ze,
  at as Qe,
  bl as $e,
  ci as B,
  d as et,
  fi as tt,
  gi as nt,
  h as rt,
  jl as it,
  mi as at,
  oi as ot,
  ot as st,
  si as ct,
  xl as lt,
  yl as ut,
} from "./app-initial~app-main~remote-conversation-page~new-thread-panel-page~onboarding-page~appgen-~o4yhvtva.js";
import {
  $ as dt,
  L as ft,
  Q as pt,
  Z as mt,
  et as ht,
  v as gt,
  y as _t,
  z as vt,
} from "./app-initial~app-main~worktree-init-v2-page~remote-conversation-page~pull-requests-page~new-~djgpfzje.js";
import {
  dn as yt,
  fn as bt,
} from "./app-initial~app-main~worktree-init-v2-page~remote-conversation-page~pull-requests-page~new-~mam8fk3d.js";
import {
  n as xt,
  t as St,
} from "./app-initial~app-main~remote-conversation-page~new-thread-panel-page~projects-index-page~app~edtlcz4n.js";
import {
  g as Ct,
  h as wt,
  m as Tt,
} from "./app-initial~app-main~worktree-init-v2-page~appgen-page~remote-conversation-page~pull-reques~filfqv6y.js";
import {
  i as Et,
  n as Dt,
  s as V,
  t as Ot,
} from "./app-initial~app-main~remote-conversation-page~settings-page~hotkey-window-thread-page~mcp-s~lv2pzabw.js";
import {
  C as kt,
  I as At,
  L as jt,
  M as Mt,
  P as Nt,
  S as Pt,
  Z as Ft,
  b as It,
  f as Lt,
  g as Rt,
  h as zt,
  k as H,
  l as Bt,
  lt as Vt,
  m as Ht,
  n as Ut,
  s as Wt,
  t as Gt,
  v as Kt,
  x as qt,
} from "./app-initial~app-main~remote-conversation-page~new-thread-panel-page~appgen-library-page~hot~fjhbmao5.js";
import {
  n as Jt,
  t as Yt,
} from "./app-initial~app-main~worktree-init-v2-page~appgen-publication-terms-route~appgen-page~remot~l0r2eqtb.js";
import {
  S as Xt,
  a as Zt,
} from "./app-initial~app-main~first-run~page~remote-conversation-page~plugin-detail-page~new-thread-~o67ur2ib.js";
import {
  a as Qt,
  i as $t,
  n as en,
  r as tn,
  t as nn,
} from "./app-initial~app-main~remote-conversation-page~new-thread-panel-page~hotkey-window-thread-pa~f79pop3z.js";
import {
  n as rn,
  t as an,
} from "./app-initial~app-main~remote-conversation-page~hotkey-window-thread-page~remote-connections-~j2v767qq-ZHfm-ldt.js";
import {
  i as on,
  r as sn,
} from "./app-initial~app-main~remote-conversation-page~new-thread-panel-page~settings-page~appgen-li~hx3rzwx8.js";
function cn(e) {
  let t = (0, ln.c)(5),
    { className: n } = e,
    r;
  t[0] === n ? (r = t[1]) : ((r = L(`last:hidden`, n)), (t[0] = n), (t[1] = r));
  let i;
  t[2] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((i = (0, un.jsx)(I, {
        id: `codex.ui.bulletSeparator`,
        defaultMessage: `·`,
        description: `Middle dot separator used between inline items`,
      })),
      (t[2] = i))
    : (i = t[2]);
  let a;
  return (
    t[3] === r
      ? (a = t[4])
      : ((a = (0, un.jsx)(`span`, { "aria-hidden": !0, className: r, children: i })),
        (t[3] = r),
        (t[4] = a)),
    a
  );
}
var ln,
  un,
  dn = e(() => {
    ((ln = u()), R(), He(), (un = C()));
  }),
  fn,
  pn,
  mn = e(() => {
    (Ye(),
      (fn = We(`has-seen-gift-credits-home-banner`, !1)),
      (pn = We(`gift-credits-profile-coachmark-pending`, !1)));
  }),
  hn,
  gn,
  _n = e(() => {
    (t(a()),
      (hn = C()),
      (gn = (e) =>
        (0, hn.jsxs)(`svg`, {
          width: 24,
          height: 24,
          viewBox: `0 0 24 24`,
          fill: `none`,
          xmlns: `http://www.w3.org/2000/svg`,
          ...e,
          children: [
            (0, hn.jsx)(`rect`, {
              x: 3.75,
              y: 3.75,
              width: 16.5,
              height: 16.5,
              rx: 3,
              stroke: `currentColor`,
              strokeWidth: 2,
            }),
            (0, hn.jsx)(`path`, {
              d: `M8.5835 12.7043L12.0316 8.0023C12.3544 7.56216 13.0483 7.8838 12.9211 8.41456L12.4211 10.5H15.0133C15.4228 10.5 15.6587 10.9654 15.4165 11.2957L11.9684 15.9977C11.6456 16.4378 10.9517 16.1162 11.0789 15.5854L11.5789 13.5H8.9867C8.57715 13.5 8.34131 13.0346 8.5835 12.7043Z`,
              fill: `currentColor`,
            }),
          ],
        })));
  }),
  vn,
  yn,
  bn = e(() => {
    (t(a()),
      (vn = C()),
      (yn = (e) =>
        (0, vn.jsx)(`svg`, {
          width: 16,
          height: 16,
          viewBox: `0 0 16 16`,
          fill: `none`,
          xmlns: `http://www.w3.org/2000/svg`,
          ...e,
          children: (0, vn.jsx)(`path`, {
            fillRule: `evenodd`,
            clipRule: `evenodd`,
            d: `M12.0368 1.69459C13.4379 1.22769 14.7711 2.56142 14.304 3.96256L11.2337 13.1735C10.7213 14.7106 8.59383 14.8331 7.90871 13.3649L6.3423 10.0079C6.28804 9.89173 6.20464 9.79237 6.1009 9.71959L5.99074 9.65631L2.63449 8.08913C1.16665 7.40386 1.28853 5.27746 2.82512 4.76491L12.0368 1.69459ZM13.3884 2.9985C12.9661 3.64157 12.2113 4.53879 11.997 4.75319L7.2673 9.48288C7.28051 9.50799 7.2951 9.53284 7.30715 9.55866L8.87199 12.9149C9.15031 13.5114 10.0151 13.4618 10.2236 12.8376L13.0814 4.26334L13.3501 3.40084C13.4001 3.25067 13.41 3.11459 13.3884 2.9985ZM12.9978 2.60944C12.8823 2.58865 12.7466 2.5989 12.5978 2.6485L11.7353 2.91725L3.16183 5.77506C2.53754 5.98334 2.48745 6.84731 3.08371 7.12584L6.44074 8.69225L6.51496 8.73053L11.2446 4.00084C11.4589 3.78664 12.3548 3.0319 12.9978 2.60944Z`,
            fill: `currentColor`,
          }),
        })));
  }),
  U,
  xn,
  Sn = e(() => {
    (t(a()),
      (U = C()),
      (xn = (e) =>
        (0, U.jsx)(`svg`, {
          width: 20,
          height: 20,
          viewBox: `0 0 20 20`,
          fill: `none`,
          xmlns: `http://www.w3.org/2000/svg`,
          ...e,
          children: (0, U.jsx)(`path`, {
            d: `M17.665 10C17.665 10.6877 17.1785 11.2454 16.5488 11.3945L16.4219 11.4189C14.7098 11.6665 13.6129 12.1305 12.877 12.8623C12.1414 13.5938 11.6742 14.6843 11.4238 16.3887C11.3197 17.0973 10.7182 17.665 9.96484 17.665C9.27085 17.665 8.68836 17.1772 8.53613 16.5215C8.12392 14.7459 7.6623 13.619 6.95703 12.8652C6.31314 12.1772 5.39414 11.7268 3.88672 11.4688L3.57715 11.4199C2.88869 11.319 2.33496 10.734 2.33496 10C2.33496 9.26603 2.88869 8.681 3.57715 8.58008L3.88672 8.53125C5.39414 8.27321 6.31314 7.82277 6.95703 7.13477C7.6623 6.38104 8.12392 5.25413 8.53613 3.47852L8.56934 3.35742C8.76133 2.76356 9.31424 2.33496 9.96484 2.33496C10.7182 2.33497 11.3197 2.9027 11.4238 3.61133L11.5283 4.22266C11.7954 5.58295 12.2334 6.49773 12.877 7.1377C13.6129 7.86952 14.7098 8.33351 16.4219 8.58105C17.1119 8.68101 17.665 9.26667 17.665 10Z`,
            fill: `currentColor`,
          }),
        })));
  });
function Cn(e) {
  let t = (0, wn.c)(6),
    { minutes: n, variant: r } = e,
    i = Be(),
    a;
  t[0] !== i || t[1] !== n || t[2] !== r
    ? ((a = A({ intl: i, minutes: n, variant: r })), (t[0] = i), (t[1] = n), (t[2] = r), (t[3] = a))
    : (a = t[3]);
  let o;
  return (
    t[4] === a
      ? (o = t[5])
      : ((o = (0, Tn.jsx)(Tn.Fragment, { children: a })), (t[4] = a), (t[5] = o)),
    o
  );
}
var wn,
  Tn,
  En = e(() => {
    ((wn = u()), He(), ce(), (Tn = C()));
  });
function Dn(e) {
  let t = (0, G.c)(45),
    {
      rateLimits: n,
      activeLimitName: r,
      planType: i,
      suppressUpsell: a,
      selectedModel: o,
      availableRateLimitResetCount: s,
      onRateLimitResetClick: c,
      onPlanUpgradeClick: l,
      planUpgradeOpensNativeSurface: u,
      onRequestLimitIncreaseClick: d,
      isLoading: f,
      showLearnMore: p,
      layout: m,
    } = e,
    h = s === void 0 ? 0 : s,
    g = f === void 0 ? !1 : f,
    _ = p === void 0 ? !0 : p,
    v = m === void 0 ? `default` : m;
  if (!g && n.length === 0 && h === 0) return null;
  let y, b, x, S, C, w, T, E, D, ee;
  if (t[0] !== r || t[1] !== v || t[2] !== n || t[3] !== o || t[4] !== a) {
    let e = Bt(n, { activeLimitName: r, selectedModel: o }),
      i;
    (t[15] !== r || t[16] !== a
      ? ((i = a ?? !Rt(r)), (t[15] = r), (t[16] = a), (t[17] = i))
      : (i = t[17]),
      (S = i),
      (x = v === `compact`),
      (b = N),
      (y = F.Item),
      (D = sn),
      (ee = ge),
      (C = `left`),
      (w = `flex items-center gap-1`),
      t[18] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((T = (0, K.jsx)(I, {
            id: `composer.mode.rateLimit.heading`,
            defaultMessage: `Usage remaining`,
            description: `Rate limit summary heading`,
          })),
          (t[18] = T))
        : (T = t[18]),
      (E =
        e &&
        (0, K.jsx)(`span`, {
          className: `whitespace-nowrap text-token-input-placeholder-foreground opacity-60`,
          children: te(e.remainingPercent),
        })),
      (t[0] = r),
      (t[1] = v),
      (t[2] = n),
      (t[3] = o),
      (t[4] = a),
      (t[5] = y),
      (t[6] = b),
      (t[7] = x),
      (t[8] = S),
      (t[9] = C),
      (t[10] = w),
      (t[11] = T),
      (t[12] = E),
      (t[13] = D),
      (t[14] = ee));
  } else
    ((y = t[5]),
      (b = t[6]),
      (x = t[7]),
      (S = t[8]),
      (C = t[9]),
      (w = t[10]),
      (T = t[11]),
      (E = t[12]),
      (D = t[13]),
      (ee = t[14]));
  let O;
  t[19] !== w || t[20] !== T || t[21] !== E
    ? ((O = (0, K.jsxs)(`span`, { className: w, children: [T, E] })),
      (t[19] = w),
      (t[20] = T),
      (t[21] = E),
      (t[22] = O))
    : (O = t[22]);
  let k;
  t[23] !== y || t[24] !== C || t[25] !== O || t[26] !== D || t[27] !== ee
    ? ((k = (0, K.jsx)(y, { LeftIcon: D, RightIcon: ee, tooltipSide: C, children: O })),
      (t[23] = y),
      (t[24] = C),
      (t[25] = O),
      (t[26] = D),
      (t[27] = ee),
      (t[28] = k))
    : (k = t[28]);
  let A;
  t[29] !== h ||
  t[30] !== x ||
  t[31] !== g ||
  t[32] !== l ||
  t[33] !== c ||
  t[34] !== d ||
  t[35] !== i ||
  t[36] !== u ||
  t[37] !== n ||
  t[38] !== S ||
  t[39] !== _
    ? ((A = (0, K.jsx)(`div`, {
        className: `flex flex-col text-sm`,
        children: g
          ? (0, K.jsx)(F.Item, {
              LeftIcon: dt,
              className: L(
                x && `pl-[calc(var(--padding-row-x)+1.25rem)] pr-[var(--padding-row-x)]`,
              ),
              disabled: !0,
              children: (0, K.jsx)(I, {
                id: `composer.mode.rateLimit.loading`,
                defaultMessage: `Loading usage…`,
                description: `Loading state for the rate limit summary submenu`,
              }),
            })
          : (0, K.jsxs)(K.Fragment, {
              children: [
                (0, K.jsx)(`div`, {
                  className: L(
                    `grid items-center gap-y-1.5 py-1`,
                    x
                      ? `grid-cols-[minmax(0,1fr)_auto] gap-x-3`
                      : `grid-cols-[auto_1fr] gap-x-16 px-[var(--padding-row-x)]`,
                    x && `pl-[calc(var(--padding-row-x)+1.25rem)] pr-[var(--padding-row-x)]`,
                  ),
                  children: n.map((e, t) => {
                    let n = Rt(e.limitName) ? null : e.limitName;
                    return (0, K.jsx)(
                      kn,
                      { rateLimit: e.snapshot, limitName: n, compact: x },
                      `${e.limitName ?? `default`}-${t}`,
                    );
                  }),
                }),
                !S &&
                  (0, K.jsx)(jn, {
                    planType: i,
                    className: L(
                      x && `pl-[calc(var(--padding-row-x)+1.25rem)] pr-[var(--padding-row-x)]`,
                    ),
                    onPlanUpgradeClick: l,
                    opensNativeSurface: u,
                    onRequestLimitIncreaseClick: d,
                  }),
                h > 0
                  ? (0, K.jsx)(F.Item, {
                      RightIcon: ge,
                      className: L(
                        x && `pl-[calc(var(--padding-row-x)+1.25rem)] pr-[var(--padding-row-x)]`,
                      ),
                      onClick: c,
                      children: (0, K.jsx)(I, {
                        id: `composer.mode.rateLimit.resetsAvailable`,
                        defaultMessage: `{availableRateLimitResetCount, plural, one {# reset available} other {# resets available}}`,
                        description: `Menu item for opening available rate limit resets`,
                        values: { availableRateLimitResetCount: h },
                      }),
                    })
                  : null,
                _ &&
                  (0, K.jsx)(F.Item, {
                    rightIcon: (0, K.jsx)(Qe, {
                      href: J,
                      className: L(q, x && `text-token-description-foreground`),
                    }),
                    className: L(
                      x && `pl-[calc(var(--padding-row-x)+1.25rem)] pr-[var(--padding-row-x)]`,
                    ),
                    href: J,
                    onClick: On,
                    children: (0, K.jsx)(`span`, {
                      className: L(
                        x ? `text-token-foreground` : `text-token-description-foreground`,
                      ),
                      children: (0, K.jsx)(I, {
                        id: `composer.mode.local.learnMore`,
                        defaultMessage: `Learn more`,
                        description: `Learn more about rate limits`,
                      }),
                    }),
                  }),
              ],
            }),
      })),
      (t[29] = h),
      (t[30] = x),
      (t[31] = g),
      (t[32] = l),
      (t[33] = c),
      (t[34] = d),
      (t[35] = i),
      (t[36] = u),
      (t[37] = n),
      (t[38] = S),
      (t[39] = _),
      (t[40] = A))
    : (A = t[40]);
  let j;
  return (
    t[41] !== b || t[42] !== k || t[43] !== A
      ? ((j = (0, K.jsx)(b, { trigger: k, children: A })),
        (t[41] = b),
        (t[42] = k),
        (t[43] = A),
        (t[44] = j))
      : (j = t[44]),
    j
  );
}
function On(e) {
  Ct({ event: e, href: J, initiator: `open_in_browser_bridge` });
}
function kn(e) {
  let t = (0, G.c)(19),
    { rateLimit: n, limitName: r, compact: i } = e,
    a = n?.primary,
    o;
  t[0] === a ? (o = t[1]) : ((o = D(a)), (t[0] = a), (t[1] = o));
  let s = o,
    c = n?.secondary,
    l;
  t[2] === c ? (l = t[3]) : ((l = D(c)), (t[2] = c), (t[3] = l));
  let u = l;
  if (!s && !u) return null;
  let d;
  t[4] !== i || t[5] !== r
    ? ((d =
        r != null &&
        (0, K.jsx)(`span`, {
          title: W(r),
          className: L(
            `text-token-foreground col-span-2 min-w-0 truncate font-medium`,
            i ? `mt-1` : `mt-2`,
          ),
          children: (0, K.jsx)(I, {
            id: `composer.mode.rateLimit.modelSectionLabel`,
            defaultMessage: `{modelName} limit:`,
            description: `Section label shown before model-specific rate limit windows in summaries`,
            values: { modelName: W(r) },
          }),
        })),
      (t[4] = i),
      (t[5] = r),
      (t[6] = d))
    : (d = t[6]);
  let f;
  t[7] !== i || t[8] !== n || t[9] !== s
    ? ((f = s && (0, K.jsx)(An, { bucket: n.primary ?? null, compact: i })),
      (t[7] = i),
      (t[8] = n),
      (t[9] = s),
      (t[10] = f))
    : (f = t[10]);
  let p;
  t[11] !== i || t[12] !== n || t[13] !== u
    ? ((p = u && (0, K.jsx)(An, { bucket: n.secondary ?? null, compact: i })),
      (t[11] = i),
      (t[12] = n),
      (t[13] = u),
      (t[14] = p))
    : (p = t[14]);
  let m;
  return (
    t[15] !== d || t[16] !== f || t[17] !== p
      ? ((m = (0, K.jsxs)(K.Fragment, { children: [d, f, p] })),
        (t[15] = d),
        (t[16] = f),
        (t[17] = p),
        (t[18] = m))
      : (m = t[18]),
    m
  );
}
function An(e) {
  let t = (0, G.c)(35),
    { bucket: n, compact: r } = e;
  if (!D(n)) return null;
  let i;
  if (t[0] !== n.resetsAt) {
    let e = O(n.resetsAt ?? null);
    ((i = e == null ? null : ue(e)), (t[0] = n.resetsAt), (t[1] = i));
  } else i = t[1];
  let a = i,
    o,
    s,
    c,
    l;
  if (t[2] !== n.usedPercent || t[3] !== n.windowDurationMins || t[4] !== r) {
    let e = le(n.usedPercent ?? 0),
      i = r && `flex min-w-0 items-center gap-1`,
      a;
    t[9] === i
      ? (a = t[10])
      : ((a = L(`text-token-foreground font-medium`, i)), (t[9] = i), (t[10] = a));
    let u = r && `shrink-0`,
      d;
    t[11] === u ? (d = t[12]) : ((d = L(u)), (t[11] = u), (t[12] = d));
    let f = n.windowDurationMins ?? null,
      p;
    t[13] === f ? (p = t[14]) : ((p = (0, K.jsx)(Cn, { minutes: f })), (t[13] = f), (t[14] = p));
    let m;
    (t[15] !== p || t[16] !== d
      ? ((m = (0, K.jsx)(`span`, { className: d, children: p })),
        (t[15] = p),
        (t[16] = d),
        (t[17] = m))
      : (m = t[17]),
      t[18] !== m || t[19] !== a
        ? ((l = (0, K.jsx)(`span`, { className: a, children: m })),
          (t[18] = m),
          (t[19] = a),
          (t[20] = l))
        : (l = t[20]));
    let h = r
      ? `min-w-0 items-center justify-end`
      : `w-full min-w-0 items-center justify-end overflow-hidden`;
    (t[21] === h
      ? (c = t[22])
      : ((c = L(`text-token-description-foreground flex gap-1 text-end`, h)),
        (t[21] = h),
        (t[22] = c)),
      (o = `shrink-0`),
      (s = te(e)),
      (t[2] = n.usedPercent),
      (t[3] = n.windowDurationMins),
      (t[4] = r),
      (t[5] = o),
      (t[6] = s),
      (t[7] = c),
      (t[8] = l));
  } else ((o = t[5]), (s = t[6]), (c = t[7]), (l = t[8]));
  let u;
  t[23] !== o || t[24] !== s
    ? ((u = (0, K.jsx)(`span`, { className: o, children: s })),
      (t[23] = o),
      (t[24] = s),
      (t[25] = u))
    : (u = t[25]);
  let d;
  t[26] === a
    ? (d = t[27])
    : ((d =
        a &&
        (0, K.jsxs)(K.Fragment, {
          children: [
            (0, K.jsx)(`span`, { className: `shrink-0`, children: (0, K.jsx)(cn, {}) }),
            (0, K.jsx)(`span`, {
              title: a,
              className: L(
                `text-token-description-foreground flex items-center gap-1`,
                `min-w-0 truncate text-right`,
              ),
              children: a,
            }),
          ],
        })),
      (t[26] = a),
      (t[27] = d));
  let f;
  t[28] !== c || t[29] !== u || t[30] !== d
    ? ((f = (0, K.jsxs)(`span`, { className: c, children: [u, d] })),
      (t[28] = c),
      (t[29] = u),
      (t[30] = d),
      (t[31] = f))
    : (f = t[31]);
  let p;
  return (
    t[32] !== l || t[33] !== f
      ? ((p = (0, K.jsxs)(K.Fragment, { children: [l, f] })), (t[32] = l), (t[33] = f), (t[34] = p))
      : (p = t[34]),
    p
  );
}
function W(e) {
  return e.replace(/_/g, `-`);
}
function jn(e) {
  let t = (0, G.c)(26),
    {
      planType: n,
      className: r,
      onPlanUpgradeClick: i,
      opensNativeSurface: a,
      onRequestLimitIncreaseClick: o,
    } = e;
  if (o != null) {
    let e;
    t[0] === Symbol.for(`react.memo_cache_sentinel`)
      ? ((e = (0, K.jsx)(I, {
          id: `settings.usage.limits.requestIncrease`,
          defaultMessage: `Request limit increase`,
          description: `Button to request a workspace monthly usage limit increase`,
        })),
        (t[0] = e))
      : (e = t[0]);
    let n;
    return (
      t[1] !== r || t[2] !== o
        ? ((n = (0, K.jsx)(F.Item, { className: r, onClick: o, children: e })),
          (t[1] = r),
          (t[2] = o),
          (t[3] = n))
        : (n = t[3]),
      n
    );
  }
  let s = a ? ge : void 0,
    c;
  t[4] === s
    ? (c = t[5])
    : ((c = (0, K.jsx)(Qe, { className: q, href: Y, PrimaryIcon: s })), (t[4] = s), (t[5] = c));
  let l = c,
    u;
  t[6] === i
    ? (u = t[7])
    : ((u = (e) => {
        (!wt(e) && (i?.(e), e.defaultPrevented)) ||
          Ct({ event: e, href: Y, initiator: `open_in_browser_bridge` });
      }),
      (t[6] = i),
      (t[7] = u));
  let d = u;
  switch (n) {
    case `free`:
    case `go`: {
      let e;
      t[8] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((e = (0, K.jsx)(I, {
            id: `composer.mode.upgradeToPlus`,
            defaultMessage: `Upgrade to Plus`,
            description: `Upgrade to Plus message for free plan`,
          })),
          (t[8] = e))
        : (e = t[8]);
      let n;
      return (
        t[9] !== r || t[10] !== d || t[11] !== l
          ? ((n = (0, K.jsx)(F.Item, {
              rightIcon: l,
              className: r,
              href: Y,
              onClick: d,
              children: e,
            })),
            (t[9] = r),
            (t[10] = d),
            (t[11] = l),
            (t[12] = n))
          : (n = t[12]),
        n
      );
    }
    case `plus`: {
      let e;
      t[13] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((e = (0, K.jsx)(I, {
            id: `composer.mode.upgradeToPro`,
            defaultMessage: `Upgrade to Pro`,
            description: `Upgrade to Pro`,
          })),
          (t[13] = e))
        : (e = t[13]);
      let n;
      return (
        t[14] !== r || t[15] !== d || t[16] !== l
          ? ((n = (0, K.jsx)(F.Item, {
              rightIcon: l,
              className: r,
              href: Y,
              onClick: d,
              children: e,
            })),
            (t[14] = r),
            (t[15] = d),
            (t[16] = l),
            (t[17] = n))
          : (n = t[17]),
        n
      );
    }
    case `prolite`: {
      let e;
      t[18] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((e = (0, K.jsx)(I, {
            id: `composer.mode.upgradeForMoreUsage`,
            defaultMessage: `Upgrade for more usage`,
            description: `Upgrade for more usage`,
          })),
          (t[18] = e))
        : (e = t[18]);
      let n;
      return (
        t[19] !== r || t[20] !== d || t[21] !== l
          ? ((n = (0, K.jsx)(F.Item, {
              rightIcon: l,
              className: r,
              href: Y,
              onClick: d,
              children: e,
            })),
            (t[19] = r),
            (t[20] = d),
            (t[21] = l),
            (t[22] = n))
          : (n = t[22]),
        n
      );
    }
    case `team`:
    case `self_serve_business_usage_based`:
    case `business`:
    case `enterprise_cbp_automation`:
    case `enterprise_cbp_usage_based`:
    case `education`:
    case `edu_plus`:
    case `edu_pro`:
    case `quorum`:
    case `sci`:
    case `enterprise`:
    case `edu`:
    case `hc`:
    case `finserv`: {
      let e;
      t[23] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((e = (0, K.jsx)(`span`, {
            className: `text-token-description-foreground`,
            children: (0, K.jsx)(I, {
              id: `composer.mode.contactAdmin`,
              defaultMessage: `To get more access, contact your admin`,
              description: `Suggest contacting admin for increased access`,
            }),
          })),
          (t[23] = e))
        : (e = t[23]);
      let n;
      return (
        t[24] === r
          ? (n = t[25])
          : ((n = (0, K.jsx)(F.Item, { className: r, allowWrap: !0, children: e })),
            (t[24] = r),
            (t[25] = n)),
        n
      );
    }
    case `pro`:
    case `free_workspace`:
    case `guest`:
    case `k12`:
    case void 0:
      return null;
  }
}
var G,
  K,
  q,
  J,
  Y,
  X = e(() => {
    ((G = u()),
      R(),
      He(),
      dn(),
      pe(),
      Tt(),
      st(),
      ht(),
      _e(),
      on(),
      zt(),
      ce(),
      En(),
      (K = C()),
      (q = `icon-xs shrink-0 opacity-75 group-focus:opacity-100 group-hover:opacity-100`),
      (J = `https://help.openai.com/en/articles/11369540-using-codex-with-your-chatgpt-plan#h_8dd84c836b`),
      (Y = `https://openai.com/chatgpt/pricing`));
  });
function Mn() {
  let e = (0, Fn.c)(21),
    { authMethod: t, isLoading: n } = Ve(),
    r = t === `chatgpt`,
    i = Ae(In),
    a = Oe(`3909937021`),
    s = Ce(),
    { data: c, isLoading: l, hasEverErrored: u } = Ke(),
    d;
  e[0] === r
    ? (d = e[1])
    : ((d = { queryConfig: { enabled: r, staleTime: o.ONE_MINUTE } }), (e[0] = r), (e[1] = d));
  let { data: f, isLoading: p } = w(`account-info`, d),
    m = c?.plan_type ?? (!l || u ? f?.plan : void 0),
    h;
  e[2] === m ? (h = e[3]) : ((h = Pn(m)), (e[2] = m), (e[3] = h));
  let g = h,
    _,
    v,
    y,
    b;
  if (
    e[4] !== t ||
    e[5] !== u ||
    e[6] !== r ||
    e[7] !== l ||
    e[8] !== g ||
    e[9] !== a ||
    e[10] !== s ||
    e[11] !== m ||
    e[12] !== i
  ) {
    let n = lt(m),
      o = r && g && !s ? i.get(Z, !1) : !1,
      c = r && n && a;
    ((_ = r && l && !u),
      (y = r && g && s),
      (v = r && n && s),
      (b = Nn({
        authMethod: t,
        plan: m,
        isFreeGoUsageSettingsEnabled: o,
        isEnterpriseUsageSettingsEnabled: c,
      })),
      (e[4] = t),
      (e[5] = u),
      (e[6] = r),
      (e[7] = l),
      (e[8] = g),
      (e[9] = a),
      (e[10] = s),
      (e[11] = m),
      (e[12] = i),
      (e[13] = _),
      (e[14] = v),
      (e[15] = y),
      (e[16] = b));
  } else ((_ = e[13]), (v = e[14]), (y = e[15]), (b = e[16]));
  let x = n || (r && p) || _ || y || v,
    S;
  return (
    e[17] !== m || e[18] !== b || e[19] !== x
      ? ((S = { ...b, isUsageSettingsAccessLoading: x, plan: m }),
        (e[17] = m),
        (e[18] = b),
        (e[19] = x),
        (e[20] = S))
      : (S = e[20]),
    S
  );
}
function Nn({
  authMethod: e,
  plan: t,
  isFreeGoUsageSettingsEnabled: n,
  isEnterpriseUsageSettingsEnabled: r = !1,
}) {
  let i = e === `chatgpt`,
    a = i && H(t),
    o = Pn(t),
    s = lt(t);
  return {
    canManageCreditSettings: a,
    isUsageSettingsVisible: a || (i && o && n) || (i && s && r),
  };
}
function Pn(e) {
  return e === ut.FREE || e === ut.GO;
}
var Fn,
  In,
  Z,
  Ln = e(() => {
    ((Fn = u()),
      Ne(),
      Ue(),
      Nt(),
      we(),
      i(),
      $e(),
      m(),
      (In = `1636924626`),
      (Z = `enable_free_go_usage_settings`));
  }),
  Rn,
  zn = e(() => {
    (p(), T(), (Rn = b(l, !1)));
  });
function Bn(e) {
  return e?.structure?.toLowerCase() === `personal`;
}
function Vn({ currentAccount: e, accounts: t }) {
  return Bn(e) ? (t == null ? !0 : t.some((e) => !Bn(e))) : !1;
}
function Hn({ authMethod: e, plan: t, currentAccount: n, accounts: r }) {
  return e !== `chatgpt` || (t !== ut.FREE && t !== ut.GO) || n == null || r == null
    ? !1
    : !Vn({ currentAccount: n, accounts: r });
}
var Q = e(() => {
  $e();
});
function Un(e) {
  let t = (0, Yn.c)(216),
    { triggerButton: r } = e,
    i = x(l),
    a = Oe(tn),
    [o, u] = y(pn),
    d = v(Rn),
    f;
  t[0] !== o || t[1] !== i || t[2] !== u
    ? ((f = (e) => {
        (e && o && u(!1), i.set(Rn, e));
      }),
      (t[0] = o),
      (t[1] = i),
      (t[2] = u),
      (t[3] = f))
    : (f = t[3]);
  let p = f,
    m = De(),
    {
      accountId: g,
      email: _,
      userId: b,
      authMethod: C,
      planAtLogin: T,
      requiresAuth: E,
      isCopilotApiAvailable: D,
      openAIAuth: O,
      setAuthMethod: k,
    } = Ve(),
    { data: A } = it(),
    { data: j, isError: te } = Ke(),
    M = C === `chatgpt`,
    ae = C === `copilot`,
    oe = C === `apikey`,
    se = C === `amazonBedrock`,
    ce = M || oe,
    le;
  t[4] === M ? (le = t[5]) : ((le = { queryConfig: { enabled: M } }), (t[4] = M), (t[5] = le));
  let { data: ue } = w(`account-info`, le),
    N = j?.plan_type ?? ue?.plan ?? T,
    fe;
  t[6] !== M || t[7] !== N ? ((fe = M && lt(N)), (t[6] = M), (t[7] = N), (t[8] = fe)) : (fe = t[8]);
  let pe = fe,
    _e = A?.accounts,
    ve;
  t[9] !== j || t[10] !== C || t[11] !== N || t[12] !== _e
    ? ((ve = Hn({ authMethod: C, plan: N, currentAccount: j, accounts: _e })),
      (t[9] = j),
      (t[10] = C),
      (t[11] = N),
      (t[12] = _e),
      (t[13] = ve))
    : (ve = t[13]);
  let ye = ve,
    xe;
  t[14] === N ? (xe = t[15]) : ((xe = Mt(N)?.pricePlan ?? null), (t[14] = N), (t[15] = xe));
  let Se = xe,
    Ce = !ce && O != null,
    we = !ae && D,
    ke = !ce && O == null && E,
    Ae = E || !1,
    je = Dt(),
    Ne;
  t[16] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((Ne = { logExposure: !1 }), (t[16] = Ne))
    : (Ne = t[16]);
  let Pe = jt(Ne),
    Fe;
  t[17] === Pe ? (Fe = t[18]) : ((Fe = Pe()), (t[17] = Pe), (t[18] = Fe));
  let R = Fe,
    z = ee(),
    He = Me(`/remote/:taskId`),
    Ue;
  t[19] !== z || t[20] !== He
    ? ((Ue = z ?? (He?.params.taskId == null ? null : S(He.params.taskId))),
      (t[19] = z),
      (t[20] = He),
      (t[21] = Ue))
    : (Ue = t[21]);
  let We = Ue,
    Ge = h(Ee, z),
    qe;
  t[22] !== Ge || t[23] !== z
    ? ((qe = z == null ? void 0 : { workspaceRoot: Ge }), (t[22] = Ge), (t[23] = z), (t[24] = qe))
    : (qe = t[24]);
  let Je = qe,
    { activeMode: Ye } = ie(We),
    Xe = Ye?.settings.model ?? null,
    Ze = h(et, `settings`),
    $e = Be(),
    { data: B } = v(be),
    tt = Lt(B),
    rt = Ht(B),
    at;
  t[25] === B?.plan_type
    ? (at = t[26])
    : ((at = Mt(B?.plan_type)?.pricePlan ?? null), (t[25] = B?.plan_type), (t[26] = at));
  let ot = at,
    st = Wt(tt, { activeLimitName: rt, selectedModel: Xe }),
    ct = Kt(B, { activeLimitName: rt, selectedModel: Xe }),
    ut = B?.rate_limit_reset_credits?.available_count ?? 0,
    { isUsageSettingsVisible: dt, isUsageSettingsAccessLoading: ft } = Mn(),
    pt = d && pe && dt && !ft,
    ht = j?.id ?? g ?? ue?.accountId ?? null,
    _t;
  t[27] !== pt || t[28] !== ht
    ? ((_t = { accountId: ht, enabled: pt }), (t[27] = pt), (t[28] = ht), (t[29] = _t))
    : (_t = t[29]);
  let { data: bt } = Vt(_t),
    xt = qn(bt === void 0 ? null : bt),
    St = xt != null,
    wt = st;
  pe && ((wt = []), xt != null && (wt = [xt]));
  let Tt = pe ? null : rt,
    V = pe ? 0 : ut,
    Ot = wt.some(Gn) || V > 0,
    At = qt(),
    Nt = Pt(),
    Ft = kt(),
    It = ot != null && Ft !== `web`,
    Rt = Se != null && Ft !== `web`,
    zt;
  t[30] === j?.structure
    ? (zt = t[31])
    : ((zt = function () {
        return j?.structure?.toLowerCase() === `personal`;
      }),
      (t[30] = j?.structure),
      (t[31] = zt));
  let H = zt,
    Bt;
  t[32] === j?.structure
    ? (Bt = t[33])
    : ((Bt = function () {
        return j?.structure?.toLowerCase() === `workspace`;
      }),
      (t[32] = j?.structure),
      (t[33] = Bt));
  let Ut = Bt,
    Jt = null;
  H() ? (Jt = Zn) : Ut() && (Jt = Qn);
  let Xt;
  t[34] !== j?.name || t[35] !== H
    ? ((Xt = H()
        ? (0, $.jsx)(I, {
            id: `codex.profileDropdown.personalAccountTitle`,
            defaultMessage: `Personal account`,
            description: `Label for a personal account`,
          })
        : (j?.name ??
          (0, $.jsx)(I, {
            id: `codex.profileDropdown.defaultAccountTitle`,
            defaultMessage: `Default account`,
            description: `Fallback label when a non-personal account has no display name`,
          }))),
      (t[34] = j?.name),
      (t[35] = H),
      (t[36] = Xt))
    : (Xt = t[36]);
  let $t = Xt,
    en = j?.profile_picture_url ?? null,
    [rn, on] = (0, Xn.useState)(null),
    sn;
  t[37] !== $e || t[38] !== i || t[39] !== b
    ? ((sn = async () => {
        if (b != null)
          try {
            (await gt(b),
              i
                .get(vt)
                .success(
                  $e.formatMessage({
                    id: `codex.profileDropdown.copyUserIdSuccess`,
                    defaultMessage: `Copied user ID`,
                    description: `Toast shown after copying the ChatGPT user ID from the profile dropdown`,
                  }),
                ));
          } catch {
            i.get(vt).danger(
              $e.formatMessage({
                id: `codex.profileDropdown.copyUserIdError`,
                defaultMessage: `Failed to copy user ID`,
                description: `Toast shown when copying the ChatGPT user ID from the profile dropdown fails`,
              }),
            );
          }
      }),
      (t[37] = $e),
      (t[38] = i),
      (t[39] = b),
      (t[40] = sn))
    : (sn = t[40]);
  let cn = sn,
    ln;
  t[41] === p
    ? (ln = t[42])
    : ((ln = (e) => {
        (p(!1), Ct({ event: e, href: Zt, initiator: `open_in_browser_bridge` }));
      }),
      (t[41] = p),
      (t[42] = ln));
  let un = ln,
    dn;
  t[43] === p
    ? (dn = t[44])
    : ((dn = () => {
        (p(!1), n.dispatchHostMessage({ type: `navigate-to-route`, path: `/settings/usage` }));
      }),
      (t[43] = p),
      (t[44] = dn));
  let fn = dn,
    mn;
  t[45] !== i || t[46] !== p || t[47] !== V
    ? ((mn = () => {
        (p(!1),
          nt(i, Gt, { initialAvailableCount: V, isRateLimitReached: !1, onResetComplete: Wn }));
      }),
      (t[45] = i),
      (t[46] = p),
      (t[47] = V),
      (t[48] = mn))
    : (mn = t[48]);
  let hn = mn,
    _n;
  t[49] !== m || t[50] !== i
    ? ((_n = async () => {
        (await s(i, `use-copilot-auth-if-available`, !1),
          await Le(`logout`, { hostId: c }),
          m(`/login`));
      }),
      (t[49] = m),
      (t[50] = i),
      (t[51] = _n))
    : (_n = t[51]);
  let vn = _n,
    yn;
  t[52] !== en || t[53] !== rn
    ? ((yn = function (e) {
        return en && en !== rn
          ? (0, $.jsx)(`img`, {
              src: en,
              alt: ``,
              className: L(`rounded-full`, e.className),
              onError: () => {
                on(en);
              },
            })
          : (0, $.jsx)(Yt, { className: e.className });
      }),
      (t[52] = en),
      (t[53] = rn),
      (t[54] = yn))
    : (yn = t[54]);
  let bn = yn,
    U;
  if (
    t[55] !== bn ||
    t[56] !== $t ||
    t[57] !== te ||
    t[58] !== R ||
    t[59] !== Se ||
    t[60] !== _ ||
    t[61] !== cn ||
    t[62] !== se ||
    t[63] !== oe ||
    t[64] !== M ||
    t[65] !== ae ||
    t[66] !== m ||
    t[67] !== O ||
    t[68] !== At ||
    t[69] !== Rt ||
    t[70] !== i ||
    t[71] !== k ||
    t[72] !== p ||
    t[73] !== ke ||
    t[74] !== we ||
    t[75] !== Ce ||
    t[76] !== ye
  ) {
    if (((U = []), M)) {
      if (_) {
        let e;
        t[78] === cn
          ? (e = t[79])
          : ((e = () => {
              cn();
            }),
            (t[78] = cn),
            (t[79] = e));
        let n;
        t[80] === _
          ? (n = t[81])
          : ((n = (0, $.jsx)(P, { LeftIcon: re, disabled: !0, children: _ })),
            (t[80] = _),
            (t[81] = n));
        let r;
        (t[82] !== e || t[83] !== n
          ? ((r = (0, $.jsx)(`div`, { onClick: e, children: n }, `email`)),
            (t[82] = e),
            (t[83] = n),
            (t[84] = r))
          : (r = t[84]),
          U.push(r));
      }
      if (!te) {
        let e;
        (t[85] !== bn || t[86] !== $t
          ? ((e = (0, $.jsx)(P, { LeftIcon: bn, disabled: !0, children: $t }, `account`)),
            (t[85] = bn),
            (t[86] = $t),
            (t[87] = e))
          : (e = t[87]),
          U.push(e));
      }
      if (ye) {
        let e = Rt ? ge : void 0,
          n;
        t[88] !== R || t[89] !== e
          ? ((n = (0, $.jsx)(Qe, { className: $n, href: R, PrimaryIcon: e })),
            (t[88] = R),
            (t[89] = e),
            (t[90] = n))
          : (n = t[90]);
        let r;
        t[91] !== R || t[92] !== Se || t[93] !== At || t[94] !== i || t[95] !== p
          ? ((r = (e) => {
              (p(!1),
                At({
                  scope: i,
                  currentPlan: Se,
                  event: e,
                  getPricingUrl: () => R,
                  source: `profile_dropdown_upgrade_cta`,
                }));
            }),
            (t[91] = R),
            (t[92] = Se),
            (t[93] = At),
            (t[94] = i),
            (t[95] = p),
            (t[96] = r))
          : (r = t[96]);
        let a;
        t[97] === Symbol.for(`react.memo_cache_sentinel`)
          ? ((a = (0, $.jsx)(I, {
              id: `codex.profileDropdown.getPlus`,
              defaultMessage: `Upgrade for higher limits`,
              description: `Menu item in the profile dropdown to upgrade a free account for higher limits`,
            })),
            (t[97] = a))
          : (a = t[97]);
        let o;
        (t[98] !== n || t[99] !== r
          ? ((o = (0, $.jsx)(
              P,
              { LeftIcon: xn, rightIcon: n, onClick: r, children: a },
              `upgrade-account`,
            )),
            (t[98] = n),
            (t[99] = r),
            (t[100] = o))
          : (o = t[100]),
          U.push(o));
      }
    } else if (oe) {
      let e;
      (t[101] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((e = (0, $.jsx)(
            P,
            {
              LeftIcon: re,
              disabled: !0,
              children: (0, $.jsx)(I, {
                id: `codex.profileDropdown.apiKeyAuth`,
                defaultMessage: `Logged in with API key`,
                description: `Label indicating the user is authenticated with an API key`,
              }),
            },
            `api-key-auth`,
          )),
          (t[101] = e))
        : (e = t[101]),
        U.push(e));
    } else if (se) {
      let e;
      (t[102] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((e = (0, $.jsx)(
            P,
            {
              LeftIcon: re,
              disabled: !0,
              children: (0, $.jsx)(I, {
                id: `codex.profileDropdown.amazonBedrockAuth`,
                defaultMessage: `Logged in with Amazon Bedrock`,
                description: `Label indicating the user is authenticated with Amazon Bedrock`,
              }),
            },
            `amazon-bedrock-auth`,
          )),
          (t[102] = e))
        : (e = t[102]),
        U.push(e));
    }
    if (ae) {
      let e;
      (t[103] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((e = (0, $.jsx)(
            P,
            {
              LeftIcon: re,
              disabled: !0,
              children: (0, $.jsx)(I, {
                id: `codex.profileDropdown.copilotAuth`,
                defaultMessage: `Logged in with Copilot`,
                description: `Label indicating the user is authenticated with Copilot`,
              }),
            },
            `copilot-auth`,
          )),
          (t[103] = e))
        : (e = t[103]),
        U.push(e));
    }
    if (Ce) {
      let e;
      t[104] !== O || t[105] !== i || t[106] !== k || t[107] !== p
        ? ((e = () => {
            (p(!1), s(i, `use-copilot-auth-if-available`, !1), k(O));
          }),
          (t[104] = O),
          (t[105] = i),
          (t[106] = k),
          (t[107] = p),
          (t[108] = e))
        : (e = t[108]);
      let n;
      t[109] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((n = (0, $.jsx)(I, {
            id: `codex.profileDropdown.switchToOpenAIAccount`,
            defaultMessage: `Use OpenAI account`,
            description: `Label showing the option to switch to OpenAI authentication`,
          })),
          (t[109] = n))
        : (n = t[109]);
      let r;
      (t[110] === e
        ? (r = t[111])
        : ((r = (0, $.jsx)(P, { onClick: e, LeftIcon: an, children: n }, `switch-to-openai`)),
          (t[110] = e),
          (t[111] = r)),
        U.push(r));
    } else if (we) {
      let e;
      t[112] !== i || t[113] !== k || t[114] !== p
        ? ((e = () => {
            (p(!1), s(i, `use-copilot-auth-if-available`, !0), k(`copilot`));
          }),
          (t[112] = i),
          (t[113] = k),
          (t[114] = p),
          (t[115] = e))
        : (e = t[115]);
      let n;
      t[116] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((n = (0, $.jsx)(I, {
            id: `codex.profileDropdown.switchToCopilotAccount`,
            defaultMessage: `Use Copilot account`,
            description: `Label showing the option to switch to Copilot authentication`,
          })),
          (t[116] = n))
        : (n = t[116]);
      let r;
      (t[117] === e
        ? (r = t[118])
        : ((r = (0, $.jsx)(P, { onClick: e, LeftIcon: an, children: n }, `switch-to-copilot`)),
          (t[117] = e),
          (t[118] = r)),
        U.push(r));
    }
    if (ke) {
      let e;
      t[119] !== m || t[120] !== p
        ? ((e = () => {
            (p(!1), m(`/login`));
          }),
          (t[119] = m),
          (t[120] = p),
          (t[121] = e))
        : (e = t[121]);
      let n;
      t[122] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((n = (0, $.jsx)(I, {
            id: `codex.profileDropdown.signInWithOpenAI`,
            defaultMessage: `Sign in with ChatGPT`,
            description: `Profile menu item to sign in with ChatGPT`,
          })),
          (t[122] = n))
        : (n = t[122]);
      let r;
      (t[123] === e
        ? (r = t[124])
        : ((r = (0, $.jsx)(P, { onClick: e, LeftIcon: Ie, children: n }, `sign-in-openai`)),
          (t[123] = e),
          (t[124] = r)),
        U.push(r));
    }
    ((t[55] = bn),
      (t[56] = $t),
      (t[57] = te),
      (t[58] = R),
      (t[59] = Se),
      (t[60] = _),
      (t[61] = cn),
      (t[62] = se),
      (t[63] = oe),
      (t[64] = M),
      (t[65] = ae),
      (t[66] = m),
      (t[67] = O),
      (t[68] = At),
      (t[69] = Rt),
      (t[70] = i),
      (t[71] = k),
      (t[72] = p),
      (t[73] = ke),
      (t[74] = we),
      (t[75] = Ce),
      (t[76] = ye),
      (t[77] = U));
  } else U = t[77];
  let Sn;
  t[125] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((Sn = (0, $.jsx)(mt, {
        color: `ghost`,
        size: `icon`,
        children: (0, $.jsx)(Yt, { className: `icon-xs` }),
      })),
      (t[125] = Sn))
    : (Sn = t[125]);
  let Cn = Sn,
    wn;
  t[126] !== o || t[127] !== a || t[128] !== u || t[129] !== r
    ? ((wn =
        a && o
          ? (0, $.jsx)(de, {
              disableHoverOpen: !0,
              forceMount: !0,
              onOpenChange: (e) => {
                e || u(!1);
              },
              open: !0,
              side: `right`,
              sideOffset: 8,
              tooltipContent: (0, $.jsx)(I, {
                id: `codex.giftCredits.profileCoachmark`,
                defaultMessage: `You can always find Gift credits in this menu`,
                description: `One-time coachmark shown on the Codex account menu after the gift credits home banner is dismissed`,
              }),
              tooltipMaxWidth: `240px`,
              variant: `rich`,
              children: r ?? Cn,
            })
          : (r ??
            (0, $.jsx)(de, {
              tooltipContent: (0, $.jsx)(I, {
                id: `codex.header.settingsTooltip`,
                defaultMessage: `Settings`,
                description: `Tooltip text for opening settings`,
              }),
              children: Cn,
            }))),
      (t[126] = o),
      (t[127] = a),
      (t[128] = u),
      (t[129] = r),
      (t[130] = wn))
    : (wn = t[130]);
  let Tn = wn,
    En;
  t[131] === U.length
    ? (En = t[132])
    : ((En = U.length > 0 && (0, $.jsx)(F.Separator, {})), (t[131] = U.length), (t[132] = En));
  let On;
  t[133] !== je || t[134] !== m || t[135] !== i || t[136] !== p || t[137] !== Je
    ? ((On =
        je &&
        (0, $.jsx)(P, {
          LeftIcon: re,
          onClick: () => {
            (Re(i, Te, { source: `profile_dropdown` }),
              p(!1),
              m(`/settings/profile`, { state: Je }));
          },
          children: (0, $.jsx)(I, {
            id: `codex.profileDropdown.profile`,
            defaultMessage: `Profile`,
            description: `Menu item to open the Codex profile page`,
          }),
        })),
      (t[133] = je),
      (t[134] = m),
      (t[135] = i),
      (t[136] = p),
      (t[137] = Je),
      (t[138] = On))
    : (On = t[138]);
  let kn;
  t[139] !== m || t[140] !== p || t[141] !== Je
    ? ((kn = () => {
        (p(!1), m(`/settings/${Et}`, { state: Je }));
      }),
      (t[139] = m),
      (t[140] = p),
      (t[141] = Je),
      (t[142] = kn))
    : (kn = t[142]);
  let An;
  t[143] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((An = (0, $.jsx)(I, {
        id: `codex.profileDropdown.settingsPage`,
        defaultMessage: `Settings`,
        description: `Menu item to open Codex settings page`,
      })),
      (t[143] = An))
    : (An = t[143]);
  let W;
  t[144] !== Ze || t[145] !== kn
    ? ((W = (0, $.jsx)(P, { LeftIcon: Yt, keyboardShortcut: Ze, onClick: kn, children: An })),
      (t[144] = Ze),
      (t[145] = kn),
      (t[146] = W))
    : (W = t[146]);
  let jn;
  t[147] !== On || t[148] !== W
    ? ((jn = (0, $.jsxs)(he, { browser: !0, electron: !0, children: [On, W] })),
      (t[147] = On),
      (t[148] = W),
      (t[149] = jn))
    : (jn = t[149]);
  let G;
  t[150] === p
    ? (G = t[151])
    : ((G = () => {
        (p(!1), n.dispatchMessage(`show-settings`, { section: Et }));
      }),
      (t[150] = p),
      (t[151] = G));
  let K;
  t[152] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((K = (0, $.jsx)(I, {
        id: `codex.profileDropdown.codexSettings`,
        defaultMessage: `{appName} settings`,
        description: `Menu item to view Codex settings`,
        values: { appName: ze },
      })),
      (t[152] = K))
    : (K = t[152]);
  let q;
  t[153] === G
    ? (q = t[154])
    : ((q = (0, $.jsx)(he, {
        extension: !0,
        children: (0, $.jsx)(P, { LeftIcon: yt, onClick: G, children: K }),
      })),
      (t[153] = G),
      (t[154] = q));
  let J;
  t[155] !== a || t[156] !== p
    ? ((J = a
        ? (0, $.jsx)(P, {
            LeftIcon: nn,
            onClick: () => {
              (p(!1), Qt());
            },
            children: (0, $.jsx)(I, {
              id: `codex.profileDropdown.giftCredits`,
              defaultMessage: `Gift credits`,
              description: `Persistent account-menu item that opens the ChatGPT gift credits purchase flow`,
            }),
          })
        : null),
      (t[155] = a),
      (t[156] = p),
      (t[157] = J))
    : (J = t[157]);
  let Y;
  t[158] !== M || t[159] !== Ut || t[160] !== un
    ? ((Y =
        M && Ut()
          ? (0, $.jsx)(P, {
              LeftIcon: ne,
              rightIcon: (0, $.jsx)(Qe, { href: Zt, className: $n }),
              onClick: un,
              children: (0, $.jsx)(I, {
                id: `codex.profileDropdown.workspaceSettings`,
                defaultMessage: `Workspace settings`,
                description: `Menu item to open ChatGPT workspace settings`,
              }),
            })
          : null),
      (t[158] = M),
      (t[159] = Ut),
      (t[160] = un),
      (t[161] = Y))
    : (Y = t[161]);
  let X;
  t[162] === p
    ? (X = t[163])
    : ((X = () => {
        (p(!1), n.dispatchMessage(`open-keyboard-shortcuts`, {}));
      }),
      (t[162] = p),
      (t[163] = X));
  let Nn;
  t[164] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((Nn = (0, $.jsx)(I, {
        id: `codex.profileDropdown.keyboardShortcuts`,
        defaultMessage: `Keyboard shortcuts`,
        description: `Menu item to open keyboard shortcuts filtered to this extension`,
      })),
      (t[164] = Nn))
    : (Nn = t[164]);
  let Pn;
  t[165] === X
    ? (Pn = t[166])
    : ((Pn = (0, $.jsx)(he, {
        extension: !0,
        children: (0, $.jsx)(P, { LeftIcon: gn, onClick: X, children: Nn }),
      })),
      (t[165] = X),
      (t[166] = Pn));
  let Fn;
  t[167] !== H || t[168] !== M || t[169] !== Jt
    ? ((Fn = M && Jt != null ? (0, $.jsx)(Kn, { isPersonalAccount: H(), layer: Jt }) : null),
      (t[167] = H),
      (t[168] = M),
      (t[169] = Jt),
      (t[170] = Fn))
    : (Fn = t[170]);
  let In;
  t[171] === Fn
    ? (In = t[172])
    : ((In = (0, $.jsx)(he, { electron: !0, children: Fn })), (t[171] = Fn), (t[172] = In));
  let Z;
  t[173] === Ot
    ? (Z = t[174])
    : ((Z = Ot ? (0, $.jsx)(F.Separator, {}) : null), (t[173] = Ot), (t[174] = Z));
  let Ln;
  t[175] === Z
    ? (Ln = t[176])
    : ((Ln = (0, $.jsx)(he, { electron: !0, children: Z })), (t[175] = Z), (t[176] = Ln));
  let zn;
  t[177] !== St ||
  t[178] !== hn ||
  t[179] !== fn ||
  t[180] !== B?.plan_type ||
  t[181] !== ot ||
  t[182] !== It ||
  t[183] !== i ||
  t[184] !== Xe ||
  t[185] !== Ot ||
  t[186] !== ct ||
  t[187] !== Nt ||
  t[188] !== Tt ||
  t[189] !== V ||
  t[190] !== wt
    ? ((zn = Ot
        ? (0, $.jsx)(Dn, {
            rateLimits: wt,
            activeLimitName: Tt,
            planType: B?.plan_type,
            suppressUpsell: St ? !1 : ct,
            selectedModel: Xe,
            availableRateLimitResetCount: V,
            onRateLimitResetClick: hn,
            layout: `compact`,
            planUpgradeOpensNativeSurface: It,
            showLearnMore: !St,
            onRequestLimitIncreaseClick: St ? fn : void 0,
            onPlanUpgradeClick:
              ot == null
                ? void 0
                : (e) => {
                    Nt({
                      scope: i,
                      currentPlan: ot,
                      defaultTab: `personal`,
                      source: `profile_dropdown_rate_limit_summary`,
                    }) && e.preventDefault();
                  },
          })
        : null),
      (t[177] = St),
      (t[178] = hn),
      (t[179] = fn),
      (t[180] = B?.plan_type),
      (t[181] = ot),
      (t[182] = It),
      (t[183] = i),
      (t[184] = Xe),
      (t[185] = Ot),
      (t[186] = ct),
      (t[187] = Nt),
      (t[188] = Tt),
      (t[189] = V),
      (t[190] = wt),
      (t[191] = zn))
    : (zn = t[191]);
  let Bn;
  t[192] === zn
    ? (Bn = t[193])
    : ((Bn = (0, $.jsx)(he, { electron: !0, children: zn })), (t[192] = zn), (t[193] = Bn));
  let Vn;
  t[194] !== vn || t[195] !== i || t[196] !== p || t[197] !== Ae
    ? ((Vn =
        Ae &&
        (0, $.jsx)(P, {
          onClick: () => {
            (p(!1), nt(i, Jn, { onConfirm: vn }));
          },
          LeftIcon: an,
          children: (0, $.jsx)(I, {
            id: `codex.profileDropdown.logOut`,
            defaultMessage: `Log out`,
            description: `Menu item to log out of ChatGPT`,
          }),
        })),
      (t[194] = vn),
      (t[195] = i),
      (t[196] = p),
      (t[197] = Ae),
      (t[198] = Vn))
    : (Vn = t[198]);
  let Q;
  t[199] !== U ||
  t[200] !== En ||
  t[201] !== jn ||
  t[202] !== q ||
  t[203] !== J ||
  t[204] !== Y ||
  t[205] !== Pn ||
  t[206] !== In ||
  t[207] !== Ln ||
  t[208] !== Bn ||
  t[209] !== Vn
    ? ((Q = (0, $.jsxs)(`div`, {
        className: `flex w-full min-w-0 flex-col gap-0`,
        children: [U, En, jn, q, J, Y, Pn, In, Ln, Bn, Vn],
      })),
      (t[199] = U),
      (t[200] = En),
      (t[201] = jn),
      (t[202] = q),
      (t[203] = J),
      (t[204] = Y),
      (t[205] = Pn),
      (t[206] = In),
      (t[207] = Ln),
      (t[208] = Bn),
      (t[209] = Vn),
      (t[210] = Q))
    : (Q = t[210]);
  let Un;
  return (
    t[211] !== d || t[212] !== Tn || t[213] !== p || t[214] !== Q
      ? ((Un = (0, $.jsx)(me, {
          open: d,
          onOpenChange: p,
          contentWidth: `panel`,
          triggerButton: Tn,
          children: Q,
        })),
        (t[211] = d),
        (t[212] = Tn),
        (t[213] = p),
        (t[214] = Q),
        (t[215] = Un))
      : (Un = t[215]),
    Un
  );
}
function Wn() {}
function Gn(e) {
  return !!e.snapshot?.primary?.windowDurationMins || !!e.snapshot?.secondary?.windowDurationMins;
}
function Kn(e) {
  let t = (0, Yn.c)(20),
    { isPersonalAccount: n, layer: r } = e,
    i = x(l),
    a = Oe(`1823918333`),
    o = Oe(`3502353992`),
    s = Ae(r),
    c;
  t[0] === s ? (c = t[1]) : ((c = s.get(`enabled`, !1)), (t[0] = s), (t[1] = c));
  let u = c,
    d = a && u,
    f;
  t[2] === d ? (f = t[3]) : ((f = { enabled: d }), (t[2] = d), (t[3] = f));
  let { data: p } = Ze(f);
  if (!a || !u || p == null) return null;
  let m =
      o &&
      p.remaining_referrals === 0 &&
      (p.ineligible_reason_code === `user_limit_reached` ||
        p.ineligible_reason_code === `workspace_limit_reached`),
    h = m && p.ineligible_reason_code === `workspace_limit_reached`;
  if (!p.should_show && !m) return null;
  let g;
  t[4] !== p.description ||
  t[5] !== p.grant_action ||
  t[6] !== p.grant_amount ||
  t[7] !== p.has_rewards ||
  t[8] !== p.remaining_referrals ||
  t[9] !== p.title ||
  t[10] !== m ||
  t[11] !== h ||
  t[12] !== n ||
  t[13] !== i
    ? ((g = () => {
        (i.set(Rn, !1),
          nt(i, St, {
            grantAmount: p.grant_amount,
            hasRewards: p.has_rewards !== !1,
            hasReachedReferralLimit: m,
            hasReachedWorkspaceReferralLimit: h,
            isPersonalAccount: n,
            offerDescription: p.description,
            offerTitle: p.title,
            referralBeaconType: `rate_limit_reached`,
            referralGrantType: p.grant_action,
            referralKey: Xe,
            remainingReferrals: p.remaining_referrals,
          }));
      }),
      (t[4] = p.description),
      (t[5] = p.grant_action),
      (t[6] = p.grant_amount),
      (t[7] = p.has_rewards),
      (t[8] = p.remaining_referrals),
      (t[9] = p.title),
      (t[10] = m),
      (t[11] = h),
      (t[12] = n),
      (t[13] = i),
      (t[14] = g))
    : (g = t[14]);
  let _;
  t[15] === n
    ? (_ = t[16])
    : ((_ = n
        ? (0, $.jsx)(I, {
            id: `codex.profileDropdown.inviteFriend`,
            defaultMessage: `Invite a friend`,
            description: `Menu item to invite a friend through a persistent referral`,
          })
        : (0, $.jsx)(I, {
            id: `codex.profileDropdown.inviteCoworker`,
            defaultMessage: `Invite a coworker`,
            description: `Menu item to invite a coworker through a persistent referral`,
          })),
      (t[15] = n),
      (t[16] = _));
  let v;
  return (
    t[17] !== g || t[18] !== _
      ? ((v = (0, $.jsx)(P, {
          LeftIcon: yn,
          leftIconClassName: `icon-xs`,
          onClick: g,
          children: _,
        })),
        (t[17] = g),
        (t[18] = _),
        (t[19] = v))
      : (v = t[19]),
    v
  );
}
function qn(e) {
  let t = M(e);
  return t == null
    ? null
    : {
        limitName: null,
        snapshot: {
          limitId: null,
          limitName: null,
          primary: { usedPercent: t.usedPercent, windowDurationMins: se, resetsAt: null },
          secondary: null,
          credits: null,
          individualLimit: null,
          planType: null,
          rateLimitReachedType: null,
        },
      };
}
function Jn(e) {
  let t = (0, Yn.c)(20),
    { onClose: n, onConfirm: r } = e,
    i;
  t[0] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((i = (0, $.jsx)(I, {
        id: `codex.profileDropdown.logOutConfirmation.title`,
        defaultMessage: `Log out?`,
        description: `Title for the confirmation dialog shown before logging out from the profile dropdown`,
      })),
      (t[0] = i))
    : (i = t[0]);
  let a = i,
    o;
  t[1] !== n || t[2] !== r
    ? ((o = () => {
        (n(), r());
      }),
      (t[1] = n),
      (t[2] = r),
      (t[3] = o))
    : (o = t[3]);
  let s = o,
    c;
  t[4] === s
    ? (c = t[5])
    : ((c = (e) => {
        (e.preventDefault(), s());
      }),
      (t[4] = s),
      (t[5] = c));
  let l;
  t[6] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((l = (0, $.jsx)(qe, { asChild: !0, children: (0, $.jsx)(`span`, { children: a }) })),
      (t[6] = l))
    : (l = t[6]);
  let u;
  t[7] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((u = (0, $.jsx)(B, {
        className: `pr-8`,
        title: l,
        subtitle: (0, $.jsx)(I, {
          id: `codex.profileDropdown.logOutConfirmation.subtitle`,
          defaultMessage: `You’ll need to sign in again to keep using {appName}`,
          description: `Subtitle for the confirmation dialog shown before logging out from the profile dropdown`,
          values: { appName: ze },
        }),
      })),
      (t[7] = u))
    : (u = t[7]);
  let d;
  t[8] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((d = (0, $.jsx)(I, {
        id: `codex.profileDropdown.logOutConfirmation.cancel`,
        defaultMessage: `Cancel`,
        description: `Cancel button label for the log out confirmation dialog`,
      })),
      (t[8] = d))
    : (d = t[8]);
  let f;
  t[9] === n
    ? (f = t[10])
    : ((f = (0, $.jsx)(mt, { color: `ghost`, type: `button`, onClick: n, children: d })),
      (t[9] = n),
      (t[10] = f));
  let p;
  t[11] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((p = (0, $.jsx)(mt, {
        color: `danger`,
        type: `submit`,
        children: (0, $.jsx)(I, {
          id: `codex.profileDropdown.logOutConfirmation.confirm`,
          defaultMessage: `Log out`,
          description: `Confirm button label for logging out from the profile dropdown`,
        }),
      })),
      (t[11] = p))
    : (p = t[11]);
  let m;
  t[12] === f
    ? (m = t[13])
    : ((m = (0, $.jsxs)(ct, { className: `gap-2`, children: [f, p] })), (t[12] = f), (t[13] = m));
  let h;
  t[14] !== c || t[15] !== m
    ? ((h = (0, $.jsxs)(ot, {
        as: `form`,
        className: `gap-5 px-6 py-6`,
        onSubmit: c,
        children: [u, m],
      })),
      (t[14] = c),
      (t[15] = m),
      (t[16] = h))
    : (h = t[16]);
  let g;
  return (
    t[17] !== n || t[18] !== h
      ? ((g = (0, $.jsx)(Ge, { open: !0, onOpenChange: n, size: `narrow`, children: h })),
        (t[17] = n),
        (t[18] = h),
        (t[19] = g))
      : (g = t[19]),
    g
  );
}
var Yn,
  Xn,
  $,
  Zn,
  Qn,
  $n,
  er = e(() => {
    ((Yn = u()),
      ye(),
      R(),
      g(),
      p(),
      r(),
      (Xn = t(a(), 1)),
      He(),
      ke(),
      z(),
      je(),
      Pe(),
      Ne(),
      Ue(),
      rt(),
      pt(),
      Je(),
      tt(),
      pe(),
      Tt(),
      st(),
      at(),
      ft(),
      ve(),
      fe(),
      Xt(),
      V(),
      $t(),
      mn(),
      d(),
      k(),
      At(),
      E(),
      j(),
      _e(),
      bt(),
      en(),
      _n(),
      rn(),
      Fe(),
      bn(),
      Jt(),
      Sn(),
      _(),
      Se(),
      xe(),
      Ft(),
      ce(),
      Ut(),
      X(),
      xt(),
      T(),
      Ot(),
      Nt(),
      It(),
      Ln(),
      f(),
      we(),
      _t(),
      ae(),
      $e(),
      zt(),
      m(),
      oe(),
      zn(),
      Q(),
      ($ = C()),
      (Zn = `1038162578`),
      (Qn = `3648137593`),
      ($n = `icon-xs shrink-0 opacity-75 group-focus:opacity-100 group-hover:opacity-100`));
  });
export {
  dn as _,
  Ln as a,
  X as c,
  gn as d,
  _n as f,
  cn as g,
  mn as h,
  Hn as i,
  xn as l,
  fn as m,
  er as n,
  Mn as o,
  pn as p,
  Q as r,
  Dn as s,
  Un as t,
  Sn as u,
};
//# sourceMappingURL=app-initial~app-main~remote-conversation-page~hotkey-window-thread-page~keyboard-shortcuts-~n7jwlpf0.js.map
