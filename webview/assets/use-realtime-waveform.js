...
import { Bt as s, l as c, o as l } from "./use-host-config-C.js";
import { Hi as u, Mi as d } from "./thread-context-inputs.js";
import { h as f } from "./vscode-api.js";
import { Aa as p, Gi as m, ai as h, w as g } from "./src-1.js";
...
async function J({
  ...
  realtimeVoiceToolsDeveloperInstructions: _ = null,
  serviceTier: v,
  setSelectedCollaborationMode: y,
  shouldUseSingletonOrchestratorThread: b = !1,
  scope: x,
  startRealtimeConversationPage: S,
  threadToolsEnabled: C,
  workspaceRootsForLocalExecution: E,
}) {
  let D = T(E);
  try {
    let f = b ? await X(x, r) : null;
    if (f != null) {
    ...
    let h = await w(E, { directoryName: V }),
      T = h.workspaceRoots,
      O = h.cwd ?? n,
      { config: k } = await s(`read-config-for-host`, { hostId: r, includeLayers: !1, cwd: O }),
      A = i ?? d(k),
      j = m(t, T, u(k)),
      M = Z({ activeCollaborationMode: e, newThreadModel: l, newThreadReasoningEffort: p }),
      N = [C ? o : null, _, G].filter((e) => e != null).join(`

`),
      P = await s(`start-conversation`, {
        hostId: r,
        input: [],
        cwd: O,
        workspaceRoots: T,
        ...
        baseInstructions: a,
        ...(D ? { projectlessOutputDirectory: h.projectlessOutputDirectory } : {}),
        additionalDeveloperInstructions: N,
      });
    if (
      (await s(`set-thread-title`, { conversationId: P, title: H }),
      b && x.set(W, r, P),
      y(null),
      g)
    ) {
      await g(P, A, { startRealtime: !0 });
      return;
    }
    S(P);
  } catch (e) {
...
function Y(e) {
  let t = (0, B.c)(18),
    ...
    b = _(),
    C = S(),
    [w] = l(`statsig_default_enable_features`),
    { value: T } = F(),
    E = v(g.realtimeVoiceUseSingletonOrchestratorThread);
  if (!c) return;
  let D;
  t[0] !== b || t[1] !== h
    ? ((D = () => {
