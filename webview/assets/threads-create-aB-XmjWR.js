import { _a as e, ds as t, js as n, vo as r, vr as i } from "./app-server-manager-signals.js";
import { n as a } from "./vscode-api.js";
import { mn as o, yi as s } from "./src-1.js";
import { Br as c } from "./persisted-signal.js";
...
import { t as f } from "./build-worktree-label-from-input-CWcK-Fwd.js";
...
async function g({
  ...
  let v = await y(f, h),
    b = { type: `text`, text: a, text_elements: [] },
    S = x(i, l);
  return {
    threadId: await n(`start-conversation`, {
      hostId: t,
      input: s == null ? [b] : r({ sourceThreadId: s, input: a }),
      cwd: f,
      workspaceRoots: h,
      collaborationMode: S,
      serviceTier: await p(o, t, S?.settings.model ?? null),
...
`)}`,
  ...
  let d = [{ type: `text`, text: i, text_elements: [] }],
    p = s == null ? d : r({ sourceThreadId: s, input: i }),
...
async function v(e, n) {
  let r = e.get(l, t),
    a = await i(`git`).request({
      ...
      params: { cwd: o(n), hostConfig: r, operationSource: `worktree_pending_create` },
    });
  if (a == null) return { type: `branch`, branchName: `main` };
  let { branch: s } = await i(`git`).request({
...
async function y(e, r) {
  let { config: i } = await n(`read-config-for-host`, { hostId: t, includeLayers: !1, cwd: e });
  return s(c(`agent-mode-by-host-id`, {}).local ?? `auto`, r, i);
}
async function b({ input: r, model: i, projectId: a, scope: o, threadSource: s, thinking: l }) {
  let { config: u } = await n(`read-config-for-host`, { hostId: t, includeLayers: !1, cwd: a }),
    d = x(i, l);
  return {
    input: r,
    workspaceRoots: [a],
    ...
    model: null,
    serviceTier: await p(o, t, d?.settings.model ?? null),
...
export { h as t };
//# sourceMappingURL=threads-create-aB-XmjWR.js.map
