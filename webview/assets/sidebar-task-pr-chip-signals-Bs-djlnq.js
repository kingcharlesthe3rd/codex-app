...
import { t as xe } from "./jsx-runtime.js";
import {
  L as y,
  R as b,
  W as x,
...
import { r as Te } from "./lib-2.js";
import { p as Ee } from "./statsig.js";
import { T as De, l as Oe, o as ke } from "./sidebar-signals.js";
import { t as Ae } from "./setting-storage.js";
import { t as je } from "./app-intl-signal-B.js";
import { _ as Me, n as Ne, t as Pe } from "./sidebar-thread-list-signals.js";
import { t as Fe } from "./clock.js";
import { o as T, s as E } from "./thread-actions.js";
import { g as Ie, l as Le, n as D, r as Re } from "./pull-request-status.js";
import { n as ze } from "./pull-request-status-utils.js";
import { n as Be, t as Ve } from "./pull-request-visual-state.js";
import { t as He } from "./gh-cli-status-query.js";
var O = `custom:`;
function Ue(e) {
  return `${O}${e}`;
}
function k(e) {
  return e.startsWith(O) ? e.slice(7) : null;
}
...
function Ke(e) {
  let t = new Set();
  return e.flatMap((e) => (t.has(e) ? [] : (t.add(e), [Ue(e)])));
}
...
function Ye({ sections: e, drop: t }) {
  return t == null
    ...
    : t.targetContainerId === `pinned` ||
      ...
      ? ye({ sections: e, threadId: t.threadId })
...
var A = x(C, null),
  Xe = x(C, null),
  Ze = y(C, ({ get: e }) => e(Ee, `2413345355`)),
  Qe = y(C, ({ get: t }) => t(he, e.SIDEBAR_CUSTOM_SECTIONS)),
  j = y(C, ({ get: t }) => (t(Ze) ? ue(t, e.SIDEBAR_CUSTOM_SECTIONS) : [])),
  $e = y(C, ({ get: e }) => e(Qe).isFetched),
  M = y(C, ({ get: e }) => Ye({ sections: e(j), drop: e(Xe) })),
  et = y(C, ({ get: e }) => {
    let t = e(M),
      n = We({
        sectionOrder: e(De),
  ...
  tt = we(C, ({ sectionId: e, currentThreadKey: t }) => {
    let n = [];
    return y(C, ({ get: r }) => {
      let i = r(M).find((t) => t.id === e);
      if (i == null) return ((n = []), n);
      let { allSidebarThreadKeys: a } = r(r(Ne, { currentConversationId: be(t) })),
        o = ve({ items: r(r(Pe, a)), threadIds: i.threadIds, sortKey: i.sortKey });
      return ((n = nt(n, o)), n);
    });
  });
function nt(e, t) {
...
function ct({ currentCwd: e, targetGroup: t, worktreeWorkspaceRoot: r }) {
  let i = r ?? t.path;
  return {
    projectKind: `local`,
    projectId: t.projectId,
    ...(i == null ? {} : { path: i }),
    pendingCoreUpdate: i != null && n(e ?? ``) !== n(i),
  };
}
async function z(e, n, r) {
  ...
  try {
    return (
      await pe(`git`).request({
        method: `resolve-worktree-for-thread`,
        params: {
          cwd: i,
          conversationId: n,
          hostConfig: fe(e.get, `host_config`) ?? ne(`local`, null),
    ...
    return (
      S.warning(`Failed to resolve an existing worktree for moved thread.`, {
        safe: {},
        sensitive: { error: f(e) },
      }),
      null
...
function K(t, n) {
  let r = async () => {
      let { value: r } = await w(`get-global-state`, {
          params: { key: e.SIDEBAR_CHAT_THREAD_ORDER },
        }),
        i = r ?? null,
        a = n(i);
      a == null || a === i || (await v(t, e.SIDEBAR_CHAT_THREAD_ORDER, a));
    },
    i = B.then(r, r);
  return ((B = i.catch(() => {})), i);
}
function q(t, n) {
  let r = async () => {
      let { value: r } = await w(`get-global-state`, {
          params: { key: e.SIDEBAR_PROJECT_THREAD_ORDERS },
        }),
        i = r ?? a(e.SIDEBAR_PROJECT_THREAD_ORDERS),
        o = n(i);
      o !== i && (await v(t, e.SIDEBAR_PROJECT_THREAD_ORDERS, o));
    },
    i = V.then(r, r);
  return ((V = i.catch(() => {})), i);
}
...
function J(t, n) {
  let r = async () => {
      let { value: r } = await w(`get-global-state`, {
          params: { key: e.THREAD_PROJECT_ASSIGNMENTS },
        }),
        i = u(r);
      c(i[n.threadId], n.assignment) ||
        (await v(
          t,
          e.THREAD_PROJECT_ASSIGNMENTS,
          s({ assignments: i, conversationId: n.threadId, assignment: n.assignment }),
        ));
    },
    i = _t.then(r, r);
  return ((_t = i.catch(() => {})), i);
...
function yt(t, n) {
  let r = async () => {
      let { value: r } = await w(`get-global-state`, { params: { key: e.PROJECTLESS_THREAD_IDS } }),
        i = gt.safeParse(r).data ?? [];
      i.includes(n.threadId) !== n.projectless &&
        (await v(
          t,
          e.PROJECTLESS_THREAD_IDS,
          n.projectless ? [...i, n.threadId] : i.filter((e) => e !== n.threadId),
        ));
    },
    i = vt.then(r, r);
  return ((vt = i.catch(() => {})), i);
...
function xt(e, t) {
  let n = t.projectKind === `remote` ? t.hostId : ie,
...
function St(e, { sectionId: t, threadId: n, included: r, isPinned: i }) {
  ...
  return X(r && i ? T(e, n, a) : a);
}
function Ct(e, { emoji: t, name: n, threadId: r, wasPinned: i }) {
  let a = te(e, { sectionId: le(), emoji: t, name: n, threadId: r });
  return X(r != null && i ? T(e, r, a) : a);
}
...
async function jt(
  ...
  if (c === `chats`)
    return Z(
      Promise.all([
        ...
        h(e, o),
        s === `pinned` ? E(e, o, !1) : Promise.resolve(),
      ]).then(() => void 0),
    );
    ...
    if (u.kind === `remote`)
      return Z(
        Promise.all([
          ...
          H(e, {
            projectId: i.projectId,
            tasks: [...Me(i.threadKeys, new Map(r.map((e) => [e.task.key, e]))), u],
            threadId: o,
            beforeThreadId: l,
          }),
          h(e, o),
  ...
  let d = _e(e, { sectionId: c.slice(7), threadId: o, beforeThreadId: l });
  return X(s === `pinned` ? T(e, o, d) : d);
}
function X(e) {
  return e.catch((e) => {
    S.error(`Failed to save sidebar custom sections`, { safe: {}, sensitive: { error: f(e) } });
  });
}
function Z(e) {
  return e.catch((e) => {
    S.error(`Failed to save thread project assignment`, { safe: {}, sensitive: { error: f(e) } });
  });
}
var Q = Te({
  moveTo: {
...
var $ = xe(),
  It = Se(C, `gh-pr-status`, ({ cwd: e, enabled: n, headBranch: r, hostId: i }) => ({
    enabled: n,
    source: `sidebar_task_pr_chip`,
    params: { cwd: t(e ?? `/`), headBranch: r ?? ``, hostId: i },
    staleTime: Ce.ONE_MINUTE,
  })),
  Lt = y(C, ({ get: e }) => {
    let t = e(Ee, `2553306736`);
    return Ae(e, r.showSidebarPrIcons) ?? t;
  }),
  Rt = b(C, (e, { get: t }) => (!t(Lt) || e == null ? null : Vt(e, t(je)))),
  zt = b(C, (e, { get: t }) => {
    if (!t(Lt)) return null;
    let n =
        e != null && t(g, e) != null && t(p, e) != null && t(He, t(m, e) ?? void 0) === `available`,
      r = t(It, {
        cwd: t(g, e) ?? null,
        enabled: n,
        headBranch: t(p, e) ?? null,
        hostId: t(m, e) ?? void 0,
      });
    if (!n || r.isLoading || r.isError) return null;
    let i = r.data?.status === `success` ? r.data : null;
    if (i == null) return null;
    let a = Ve({ hasOpenPr: i.hasOpenPr, isDraft: i.isDraft, url: i.url });
    return a == null
      ? null
      : Ht(
          a,
          Be({ canMerge: i.canMerge, ciStatus: i.ciStatus, status: a }),
          i.checks,
          i.ciStatus,
          t(je),
        );
...
function Vt(e, t) {
  ...
  return {
    hoverCardSection: {
      id: `pr`,
      rows: [{ id: `pr-status`, icon: (0, $.jsx)(D, { status: e }), label: n }],
    },
    iconBadge: { id: `pr-status`, icon: (0, $.jsx)(D, { status: e }), tooltipContent: null },
  };
}
function Ht(e, t, n, r, i) {
  return {
    hoverCardSection: {
      id: `pr`,
      rows: [
        { id: `pr-status`, icon: (0, $.jsx)(D, { status: e }), label: Ut(e, i) },
        {
          id: `checks-summary`,
          icon: r === `none` ? (0, $.jsx)(Fe, {}) : (0, $.jsx)(Ie, { checks: n }),
          label: Le(r),
        },
      ],
    },
    iconBadge: { id: `pr-status`, icon: (0, $.jsx)(Re, { state: t }), tooltipContent: null },
  };
}
function Ut(e, t) {
  switch (e) {
...
//# sourceMappingURL=sidebar-task-pr-chip-signals-Bs-djlnq.js.map
