import { Hr as e, Mt as t, bt as n, jt as r } from "./src-2.js";
import { Wo as i, aa as a, mo as o, sa as s, uo as c } from "./app-server-manager-signals.js";
import { f as l, n as u } from "./vscode-api.js";
...
async function g({
  ...
  return (
    ...
    await c(e.PROJECT_ORDER, [o, ...(s ?? []).filter((e) => e !== o)]),
    await Promise.all(w(l).map((e) => t({ legacyRoot: null, projectId: o, root: e }))),
...
function _(t, n) {
  let r = C(n);
  return (
    t.query.setData(a, e.ACTIVE_REMOTE_PROJECT_ID, { value: void 0 }),
    t.query.setData(d, { roots: [r.projectId] }),
    f(t, null),
    r.folderPath == null
      ? s(t, e.ACTIVE_WORKSPACE_ROOTS, [r.projectId])
      : l.dispatchMessage(`electron-set-active-workspace-root`, { root: r.folderPath }),
    r.projectId
  );
}
async function v({
    ...
    d =
      u.metadataProjectId == null
        ...
        : [
            s(
              e.LOCAL_PROJECTS,
              r({ localProjects: n ?? {}, projectId: u.metadataProjectId, project: null }),
            ),
...
async function T({ projectId: r, prompt: a, validateProjectId: o = !1 }) {
  let [{ value: s }, { value: c }, l] = await Promise.all([
      u(`get-global-state`, { params: { key: e.LOCAL_PROJECTS } }),
      u(`get-global-state`, { params: { key: e.PROJECT_WRITABLE_ROOTS } }),
      o ? u(`workspace-root-options`, { params: { hostId: i } }) : Promise.resolve(null),
    ]),
    d = t(s),
    f = Object.hasOwn(d, r);
  if (l != null && !l.roots.includes(r) && !f)
    throw Error(
      `Unknown projectId: ${r}\nSaved projectIds:\n${l.roots.join(`
`)}`,
    );
  let m = n(c);
  if (!Object.hasOwn(m, r) && !f) return null;
  let h = await p({ projectId: r, projectWritableRoots: m, legacyRoot: f ? null : r, prompt: a });
  return {
    cwd: h.cwd,
