...
import { _ as r, k as i, m as a, n as o, r as s, u as c } from "./vscode-api.js";
import {
  _r as l,
  ar as u,
  hr as d,
...
import { As as te, No as y, tt as ne } from "./app-server-manager-signals.js";
import { o as re } from "./statsig-DoZ-0xit.js";
import { t as ie } from "./invalidate-queries-and-broadcast.js";
import { d as ae } from "./thread-context-inputs.js";
import { n as oe } from "./use-auth.js";
import { t as se } from "./use-codex-home.js";
import { i as ce, n as le, r as ue, t as de } from "./use-is-plugins-enabled.js";
function fe() {
  return window.electronBridge?.getBuildFlavor?.() || `prod`;
}
var b = `<svg width="192" height="192" viewBox="0 0 192 192" fill="none" xmlns="http://www.w3.org/2000/svg">
...
function D(e) {
  let t = [e.name, e.id, e.interface?.displayName ?? ``].map(pe);
  for (let e of t) {
    let t = k(e);
    if (t != null) return t;
  }
...
function k(e) {
  ...
  return E[e] ?? E[t] ?? E[t.replace(/-mcp-server$/u, ``)] ?? null;
}
function me(e) {
  return e !== `chatgpt`;
}
...
function j(e) {
  let t = 0;
  for (let n = 0; n < e.length; n += 1) t = (t * 31 + e.charCodeAt(n)) % A.length;
  return A[t] ?? ``;
}
var M = e(ee(), 1),
  N = {
  ...
  try {
    let e = { path: r, hostId: t },
      i = await n.fetchQuery({
        queryFn: ({ signal: t }) => o(`read-file-binary`, { params: e, signal: t }),
        queryKey: s(`read-file-binary`, e),
        retry: !1,
        gcTime: 1 / 0,
        staleTime: c.INFINITE,
      });
    return i.contentsBase64 ? `data:${I(r)};base64,${i.contentsBase64}` : null;
  } catch (e) {
    return (
      a.warning(`Failed to inline local image`, {
        safe: {},
...
function I(e) {
  return N[M.default.extname(e).toLowerCase()] ?? `application/octet-stream`;
}
var he = t(),
  L = [`plugins`],
  R = `openai-curated-marketplaces-hidden`,
  z = 4,
  ge = [],
  _e = [],
  ve = [],
  ...
  U = [`datadog`, `statsig`],
  W = new Set([`codex-official`, _, v, m, `openai-primary-runtime`].map(Ue));
function G(e) {
  return {
    ...Ze(e.summary),
...
function we(e, t, a) {
  let o = (0, he.c)(54),
    s;
  o[0] === e ? (s = o[1]) : ((s = { hostId: e }), (o[0] = e), (o[1] = s));
  let l = de(s) && (a?.enabled ?? !0),
    u = a?.additionalMarketplaceKinds ?? ve,
    d = a?.installSuggestionPluginNames ?? null,
    f = re(`4218407052`),
    p = me(oe(e)?.authMethod ?? null),
    m = ye;
  p ? (m = Se) : f && (m = xe);
  let h = Ae({
      additionalMarketplaceKinds: u,
      includeRemoteCatalog: a?.includeRemoteCatalog ?? !0,
      includeVerticalCatalog: !f,
    }),
    g = ie(),
    _ = i(),
    v = n(ae),
    ...
    y = n(ne),
    b;
  o[2] !== e || o[3] !== y ? ((b = y.includes(e)), (o[2] = e), (o[3] = y), (o[4] = b)) : (b = o[4]);
  let x = b,
    S = l && x,
    C;
  o[5] !== e || o[6] !== S
    ? ((C = { enabled: S, hostId: e }), (o[5] = e), (o[6] = S), (o[7] = C))
    : (C = o[7]);
  let w = ce(C),
    T;
  o[8] === e ? (T = o[9]) : ((T = { hostId: e }), (o[8] = e), (o[9] = T));
  let E = le(T),
    D;
  o[10] === e ? (D = o[11]) : ((D = { hostId: e }), (o[10] = e), (o[11] = D));
  let O = ue(D),
    pe = t === void 0,
    k = qe({ codexHome: ee, hostId: e, rootsOverrideCwd: t, workspaceRoots: v.data?.roots }),
    A = l && x && (t !== void 0 || v.isFetched),
    j;
  o[12] === Symbol.for(`react.memo_cache_sentinel`) ? ((j = fe()), (o[12] = j)) : (j = o[12]);
  let M = j,
    N = d == null ? je(e, k, h, f, p) : Oe(e, k, ve, d, f, p),
    P;
  o[13] !== M ||
  o[14] !== m ||
  o[15] !== e ||
  ...
  o[19] !== k
    ? ((P = async () => {
        if (d != null) {
          let t = await te(`send-cli-request-for-host`, {
              hostId: e,
              method: `plugin/installed`,
              params: { ...(k.length > 0 ? { cwds: k } : {}), installSuggestionPluginNames: d },
            }),
            n = Ne(t.marketplaces, m),
            r = Xe(n);
          return {
            featuredPluginIds: ge,
            marketplaceLoadErrors: t.marketplaceLoadErrors,
            marketplaces: Ge(n),
            plugins: await Ke({ hostId: e, plugins: r, queryClient: _ }),
          };
        }
        let t = await te(
            `list-plugins`,
            h == null
              ...
              : { hostId: e, ...(k.length > 0 ? { cwds: k } : {}), marketplaceKinds: h },
          ),
          n = Ne(t.marketplaces, m),
          r = Xe(n),
          i = M == null ? r : He({ buildFlavor: M, plugins: r }),
          a = Ce(t.featuredPluginIds).filter((e) => !m.some((t) => e.endsWith(`@${t}`)));
        return {
          featuredPluginIds: M == null ? a : Ve({ buildFlavor: M, featuredPluginIds: a }),
          marketplaceLoadErrors: t.marketplaceLoadErrors,
          marketplaces: Ge(n),
          plugins: await Ke({ hostId: e, plugins: i, queryClient: _ }),
        };
      }),
      (o[13] = M),
      (o[14] = m),
    ...
    : (P = o[20]);
  let F;
  o[21] !== A || o[22] !== N || o[23] !== P
    ? ((F = { queryKey: N, queryFn: P, enabled: A, staleTime: c.FIVE_MINUTES, gcTime: 1 / 0 }),
      (o[21] = A),
      (o[22] = N),
      (o[23] = P),
      (o[24] = F))
  ...
  if (!l || !x) {
    let e;
    return (
      o[28] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((e = {
            availablePlugins: B,
            featuredPluginIds: ge,
            installedPlugins: B,
            marketplaceLoadErrors: _e,
            marketplaces: be,
            errorMessage: null,
            isLoading: !1,
            isFetching: !1,
  ...
  o[30] !== O.available ||
  o[31] !== E.available ||
  o[32] !== I.data?.featuredPluginIds ||
  o[33] !== I.data?.plugins
    ? ((V = {
        isComputerUseAvailable: w.available,
        isExternalBrowserUseAvailable: O.available,
        isInAppBrowserUseAvailable: E.available,
      }),
      (H = I.data?.plugins ?? B),
      (U = Fe({ plugins: H, ...V })),
      (W = Pe({ featuredPluginIds: I.data?.featuredPluginIds ?? ge, ...V })),
      (o[29] = w.available),
      (o[30] = O.available),
      (o[31] = E.available),
      (o[32] = I.data?.featuredPluginIds),
      ...
      (o[36] = U),
      (o[37] = W))
    : ((V = o[34]), (H = o[35]), (U = o[36]), (W = o[37]));
  let G;
  o[38] === H ? (G = o[39]) : ((G = Ie(H)), (o[38] = H), (o[39] = G));
  let K = I.data?.marketplaceLoadErrors ?? _e,
    q = I.data?.marketplaces ?? be,
    J = I.error ? String(I.error.message) : null,
    Y = (pe && v.isLoading) || I.isLoading || E.isLoading || O.isLoading || w.isLoading,
    X = (pe && v.isFetching) || I.isFetching || w.isFetching,
    Z;
  o[40] !== V || o[41] !== I
    ? ((Z = async () => {
        let e = (await I.refetch()).data?.plugins ?? B;
...
function De(e) {
  ...
  let l = de(s),
    ...
    d;
  t[2] !== i || t[3] !== a
    ? ((u = [...L, `marketplace-kind`, i, a]),
      (d = async () =>
        Xe((await te(`list-plugins`, { hostId: i, marketplaceKinds: [a] })).marketplaces)),
      (t[2] = i),
      (t[3] = a),
      (t[4] = u),
      (t[5] = d))
  ...
  return (
    t[6] !== u || t[7] !== d || t[8] !== f
      ? ((p = { queryKey: u, queryFn: d, enabled: f, staleTime: c.FIVE_MINUTES }),
        (t[6] = u),
        (t[7] = d),
        (t[8] = f),
        (t[9] = p))
...
function Oe(e, t, n = ve, r = null, i = !1, a = !1) {
  return r == null
    ? je(
        e,
        t,
    ...
    : ke([...L, e, t, `installed`, r, `curated-marketplace`, Me(i)], a);
}
function ke(e, t) {
  return t ? [...e, R] : e;
}
...
function je(e, t, n, r, i) {
  ...
  return ke(
    n == null
      ? [...L, e, t, `curated-marketplace`, a]
      : [...L, e, t, `marketplace-kinds`, n, `curated-marketplace`, a],
    i,
  );
}
function Me(e) {
...
function Ve({ buildFlavor: e, featuredPluginIds: t }) {
  ...
  return t.filter((e) => {
    let t = p(e);
    return t == null ? !0 : !h(t) || t === n;
  });
}
function He({ buildFlavor: e, plugins: t }) {
  let n = u(e);
  return t.filter((e) => !h(e.marketplaceName) || e.marketplaceName === n);
}
function Ue(e) {
  return e
    .trim()
...
async function Ke({ hostId: e, plugins: t, queryClient: n }) {
  ...
  async function i(a) {
    let o = t[a];
    if (o == null) return;
    let [s, c] = await Promise.all([P(o.composerIconPath, e, n), P(o.logoPath, e, n)]);
    ((s != null || c != null) &&
      (r[a] = {
        ...o,
  ...
  return (await Promise.all(Array.from({ length: Math.min(z, t.length) }, (e, t) => i(t))), r);
}
function qe({ codexHome: e, hostId: t, rootsOverrideCwd: n, workspaceRoots: r }) {
  let i = t === `local` && e != null ? y(e, H) : null;
...
function Ye(e, t) {
  return t == null ? !0 : f(t) || d(t) ? f(e) || d(e) : e.startsWith(`/`) && !e.startsWith(`//`);
}
function Xe(e) {
  let t = new Map();
  for (let n of e)
