...
import { js as r, nt as i } from "./app-server-manager-signals.js";
import { _ as a, k as o, m as s, n as c, r as l, u as ee } from "./vscode-api.js";
import {
  $n as u,
  Qn as d,
  Xn as f,
...
import { d as ie } from "./thread-context-inputs.js";
import { n as ae } from "./use-auth.js";
import { t as oe } from "./use-codex-home.js";
import { i as se, n as ce, r as le, t as ue } from "./use-is-plugins-enabled.js";
function de() {
  return window.electronBridge?.getBuildFlavor?.() || `prod`;
}
var x = `<svg width="192" height="192" viewBox="0 0 192 192" fill="none" xmlns="http://www.w3.org/2000/svg">
...
function O(e) {
  let t = [e.name, e.id, e.interface?.displayName ?? ``].map(fe);
  for (let e of t) {
    let t = A(e);
    if (t != null) return t;
  }
...
function A(e) {
  ...
  return D[e] ?? D[t] ?? D[t.replace(/-mcp-server$/u, ``)] ?? null;
}
function pe(e) {
  return e !== `chatgpt`;
}
...
function M(e) {
  let t = 0;
  for (let n = 0; n < e.length; n += 1) t = (t * 31 + e.charCodeAt(n)) % j.length;
  return j[t] ?? ``;
}
var N = e(g(), 1),
  P = {
  ...
  try {
    let e = { path: r, hostId: t },
      i = await n.fetchQuery({
        queryFn: ({ signal: t }) => c(`read-file-binary`, { params: e, signal: t }),
        queryKey: l(`read-file-binary`, e),
        retry: !1,
        gcTime: 1 / 0,
        staleTime: ee.INFINITE,
      });
    return i.contentsBase64 ? `data:${L(r)};base64,${i.contentsBase64}` : null;
  } catch (e) {
    return (
      s.warning(`Failed to inline local image`, {
        safe: {},
...
function L(e) {
  return P[N.default.extname(e).toLowerCase()] ?? `application/octet-stream`;
}
var me = t(),
  R = [`plugins`],
  z = `openai-curated-marketplaces-hidden`,
  B = 4,
  he = [],
  ge = [],
  _e = [],
  ...
  xe = [_, m],
  H = `computer-use`,
  U = `.tmp/marketplaces/openai-internal-testing`,
  W = [`datadog`, `statsig`],
  G = new Set([`codex-official`, f, _, m, `openai-primary-runtime`].map(Ue));
function K(e) {
  return {
    ...Ze(e.summary),
    ...
    ...Ce({ marketplaceName: e.marketplaceName, marketplacePath: e.marketplacePath }),
  };
}
function q({ marketplacePath: e, remoteMarketplaceName: t }) {
  if (e != null && t != null)
...
function Se(e) {
  return e.filter((e) => {
    let t = $(e).toLowerCase();
    return !W.some((e) => t === e || t.startsWith(`${e}-`));
  });
}
function Y(e) {
  return e.marketplacePath == null ? Z(e.plugin) : e.plugin.name;
}
function X(e) {
  return e.marketplacePath == null ? Z(e.plugin) : e.plugin.id;
}
function Z(e) {
...
function Q(e) {
  return { ...q(e), pluginName: Y(e) };
}
function Ce({ marketplaceName: e, marketplacePath: t }) {
  return t == null
...
function we(e, t, s) {
  let c = (0, me.c)(53),
    l;
  c[0] === e ? (l = c[1]) : ((l = { hostId: e }), (c[0] = e), (c[1] = l));
  let u = ue(l) && (s?.enabled ?? !0),
    d = s?.additionalMarketplaceKinds ?? _e,
    f = s?.installSuggestionPluginNames ?? null,
    p = ne(`4218407052`),
    m = pe(ae(e)?.authMethod ?? null),
    h = ve;
  m ? (h = xe) : p && (h = be);
  let g = Ae({
      additionalMarketplaceKinds: d,
      includeRemoteCatalog: s?.includeRemoteCatalog ?? !0,
    ...
    b = n(i),
    x;
  c[2] !== e || c[3] !== b ? ((x = b.includes(e)), (c[2] = e), (c[3] = b), (c[4] = x)) : (x = c[4]);
  let S = x,
    C = u && S,
    w;
  c[5] !== e || c[6] !== C
    ? ((w = { enabled: C, hostId: e }), (c[5] = e), (c[6] = C), (c[7] = w))
    : (w = c[7]);
  let T = se(w),
    E;
  c[8] === e ? (E = c[9]) : ((E = { hostId: e }), (c[8] = e), (c[9] = E));
  let D = ce(E),
    O;
  c[10] === e ? (O = c[11]) : ((O = { hostId: e }), (c[10] = e), (c[11] = O));
  let k = le(O),
    fe = t === void 0,
    A = qe({ codexHome: te, hostId: e, rootsOverrideCwd: t, workspaceRoots: y.data?.roots }),
    j = u && S && (t !== void 0 || y.isFetched),
    M;
  c[12] === Symbol.for(`react.memo_cache_sentinel`) ? ((M = de()), (c[12] = M)) : (M = c[12]);
  let N = M,
    P = f == null ? je(e, A, g, p, m) : Oe(e, A, _e, f, p, m),
    F;
  c[13] !== N ||
  c[14] !== h ||
  c[15] !== e ||
  ...
  c[19] !== A
    ? ((F = async () => {
        if (f != null) {
          let t = await r(`send-cli-request-for-host`, {
              hostId: e,
              method: `plugin/installed`,
              params: { ...(A.length > 0 ? { cwds: A } : {}), installSuggestionPluginNames: f },
            }),
            n = Ne(t.marketplaces, h),
            i = Xe(n);
          return {
            featuredPluginIds: he,
            marketplaceLoadErrors: t.marketplaceLoadErrors,
            marketplaces: Ge(n),
            plugins: await Ke({ hostId: e, plugins: i, queryClient: v }),
          };
        }
        let t = await r(
            `list-plugins`,
            g == null
              ? { hostId: e, ...(A.length > 0 ? { cwds: A } : {}) }
              : { hostId: e, ...(A.length > 0 ? { cwds: A } : {}), marketplaceKinds: g },
          ),
          n = Ne(t.marketplaces, h),
          i = Xe(n),
          a = N == null ? i : He({ buildFlavor: N, plugins: i }),
          o = Se(t.featuredPluginIds).filter((e) => !h.some((t) => e.endsWith(`@${t}`)));
        return {
          featuredPluginIds: N == null ? o : Ve({ buildFlavor: N, featuredPluginIds: o }),
          marketplaceLoadErrors: t.marketplaceLoadErrors,
          marketplaces: Ge(n),
          plugins: await Ke({ hostId: e, plugins: a, queryClient: v }),
        };
      }),
      (c[13] = N),
      (c[14] = h),
    ...
    : (F = c[20]);
  let I;
  c[21] !== j || c[22] !== P || c[23] !== F
    ? ((I = { queryKey: P, queryFn: F, enabled: j, staleTime: ee.FIVE_MINUTES, gcTime: 1 / 0 }),
      (c[21] = j),
      (c[22] = P),
      (c[23] = F),
      (c[24] = I))
  ...
  c[27] !== k.available ||
  c[28] !== D.available ||
  c[29] !== L.data?.featuredPluginIds ||
  c[30] !== L.data?.plugins
    ? ((z = {
        isComputerUseAvailable: T.available,
        isExternalBrowserUseAvailable: k.available,
        isInAppBrowserUseAvailable: D.available,
      }),
      (B = L.data?.plugins ?? V),
      (H = Fe({ plugins: B, ...z })),
      (U = Pe({ featuredPluginIds: L.data?.featuredPluginIds ?? he, ...z })),
      (c[26] = T.available),
      (c[27] = k.available),
      (c[28] = D.available),
      (c[29] = L.data?.featuredPluginIds),
      ...
      (c[33] = H),
      (c[34] = U))
    : ((z = c[31]), (B = c[32]), (H = c[33]), (U = c[34]));
  let W;
  c[35] === B ? (W = c[36]) : ((W = Ie(B)), (c[35] = B), (c[36] = W));
  let G = L.data?.marketplaceLoadErrors ?? ge,
    K = L.data?.marketplaces ?? ye,
    q = L.error ? String(L.error.message) : null,
    J = (fe && y.isLoading) || L.isLoading || D.isLoading || k.isLoading || T.isLoading,
    Y = (fe && y.isFetching) || L.isFetching || T.isFetching,
    X;
  c[37] !== z || c[38] !== L
    ? ((X = async () => {
        let e = (await L.refetch()).data?.plugins ?? V;
        return { availablePlugins: Fe({ plugins: e, ...z }), installedPlugins: Ie(e) };
      }),
      (c[37] = z),
      (c[38] = L),
      (c[39] = X))
    : (X = c[39]);
  let Z;
  c[40] === _ ? (Z = c[41]) : ((Z = () => _(R)), (c[40] = _), (c[41] = Z));
  let Q;
  return (
    c[42] !== H ||
    c[43] !== U ||
...
async function Te() {}
async function Ee() {
  return { availablePlugins: V, installedPlugins: V };
}
function De(e) {
  let t = (0, me.c)(10),
    { enabled: n, hostId: i, marketplaceKind: o } = e,
    s = n === void 0 ? !0 : n,
    c;
  ...
  let l = ue(c),
    ...
    d;
  t[2] !== i || t[3] !== o
    ? ((u = [...R, `marketplace-kind`, i, o]),
      (d = async () =>
        Xe((await r(`list-plugins`, { hostId: i, marketplaceKinds: [o] })).marketplaces)),
      (t[2] = i),
      (t[3] = o),
      (t[4] = u),
      (t[5] = d))
  ...
  return (
    t[6] !== u || t[7] !== d || t[8] !== f
      ? ((p = { queryKey: u, queryFn: d, enabled: f, staleTime: ee.FIVE_MINUTES }),
        (t[6] = u),
        (t[7] = d),
        (t[8] = f),
        (t[9] = p))
...
function Oe(e, t, n = _e, r = null, i = !1, a = !1) {
  return r == null
    ? je(
        e,
        t,
    ...
    : ke([...R, e, t, `installed`, r, `curated-marketplace`, Me(i)], a);
}
function ke(e, t) {
  return t ? [...e, z] : e;
}
...
function je(e, t, n, r, i) {
  ...
  return ke(
    n == null
      ? [...R, e, t, `curated-marketplace`, a]
      : [...R, e, t, `marketplace-kinds`, n, `curated-marketplace`, a],
    i,
  );
}
function Me(e) {
...
function Ve({ buildFlavor: e, featuredPluginIds: t }) {
  ...
  return t.filter((e) => {
    let t = p(e);
    return t == null ? !0 : !u(t) || t === n;
  });
}
function He({ buildFlavor: e, plugins: t }) {
  let n = d(e);
  return t.filter((e) => !u(e.marketplaceName) || e.marketplaceName === n);
}
function Ue(e) {
  return e
    .trim()
...
async function Ke({ hostId: e, plugins: t, queryClient: n }) {
  ...
  async function i(a) {
    let o = t[a];
    if (o == null) return;
    let [s, c] = await Promise.all([F(o.composerIconPath, e, n), F(o.logoPath, e, n)]);
    ((s != null || c != null) &&
      (r[a] = {
        ...o,
  ...
  return (await Promise.all(Array.from({ length: Math.min(B, t.length) }, (e, t) => i(t))), r);
}
function qe({ codexHome: e, hostId: t, rootsOverrideCwd: n, workspaceRoots: r }) {
  let i = t === `local` && e != null ? b(e, U) : null;
  return Je([...(typeof n == `string` ? [n] : (n ?? r ?? [])), ...(i == null ? [] : [i])], e);
}
function Je(e, t) {
  return Array.from(new Set(e.map((e) => e.trim()).filter((e) => v(e) && Ye(e, t))));
}
function Ye(e, t) {
  return t == null ? !0 : y(t) || h(t) ? y(e) || h(e) : e.startsWith(`/`) && !e.startsWith(`//`);
}
function Xe(e) {
  let t = new Map();
  for (let n of e)
    for (let e of n.plugins)
      t.has(e.id) ||
        t.set(e.id, {
          ...
          plugin: e,
          keywords: e.keywords,
          ...Ce({ marketplaceName: n.name, marketplacePath: n.path }),
        });
