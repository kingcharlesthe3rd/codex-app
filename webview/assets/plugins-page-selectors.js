import { cr as e, mt as t, nt as n, or as r, rr as i, tt as a } from "./src-4.js";
import { Fo as o } from "./app-server-manager-signals.js";
import { i as s, r as c } from "./use-plugins.js";
import { s as l, t as u } from "./check-plugin-availability.js";
...
function S(e) {
  let t =
    ...
    e.find((e) => r(e.name)) ??
    e.find((e) => e.name === p);
  return t == null ? `remote:${a}` : (t.path ?? `remote:${t.name}`);
}
function C(e, t) {
...
function j({
  ...
  for (let e of o) {
    let t = e.plugin.remotePluginId ?? e.plugin.shareContext?.remotePluginId,
      ...
      r = c.get(n);
    (r == null || (!r.plugin.installed && e.plugin.installed)) && c.set(n, e);
  }
  return M(Array.from(c.values()));
}
function M(e) {
  return e.sort((e, t) => {
    let n = u(e.plugin);
    return n === u(t.plugin) ? 0 : n ? 1 : -1;
  });
}
function N(e) {
  return e.filter((e) => {
    let n = t(e.plugin.id);
    return e.plugin.name !== `browser` || n == null || !r(n);
  });
}
function P(e) {
...
function F({ availablePlugins: e, homeDirectory: t, pluginShares: n, storefrontPlugins: r }) {
  let i = e.find((e) => l(t, e.marketplacePath)),
    a = i == null ? null : s(i),
    o = r.filter((e) => a != null && s(e) === a),
    c = I(n, o);
...
function I(e, n = []) {
  ...
  let r = new Set([
    ...
    i = [];
  for (let { plugin: n } of e) {
    let e = n.remotePluginId;
    if (e == null) throw Error(`remote plugin share ${n.id} is missing remotePluginId`);
    if (r.has(e)) continue;
    let a = t(n.id);
...
function L(e, { mergeOpenAIBundledMarketplace: t }) {
  ...
  let c = i.get(_(a)),
    ...
    u = i.get(_(p)),
    d = R(c);
  if (c != null && d == null) return new Map();
  let f = R(l);
...
function z(e) {
  return e.filter((e) => e.plugin.installed && e.plugin.enabled && !u(e.plugin));
}
function B({ installedPlugins: e, sharedWithYouPlugins: t }) {
