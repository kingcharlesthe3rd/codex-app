...
function u(e, t) {
  let n = e.source,
    r = t.location.origin;
  if (!(n == null || n === t) || (n === t && r !== `` && r !== f && e.origin !== r)) return null;
  let i = e.data;
...
function R(e, t) {
  ...
  return K(
    e?.flatMap((e) =>
      e.hooks.filter(
        (e) => e.source === `plugin` && e.pluginId === t.summary.id && n.has(e.key) && B(e),
      ),
    ) ?? [],
  );
}
...
function ie(e) {
  return { issueCount: e.warnings.length + e.errors.length, needsReview: e.hooks.filter(B).length };
}
function ae(e) {
  let t = [];
  for (let n of q) {
...
function ge({ hooks: e, hostId: t, projectRoot: n }) {
  let r = _e(e, n);
  return X({ hostId: t, pluginId: r === `plugin` ? ve(e) : void 0, projectRoot: n, source: r });
}
function Z(e, t, n) {
  if (
    (e.delete(`hostId`),
...
//# sourceMappingURL=app-initial~app-main~worktree-init-v2-page~remote-conversation-page~plugin-detail-page~new-~sfopfmmp.js.map
