import { n as e, s as t } from "./rolldown-runtime.js";
import {
  At as n,
  Bt as r,
  El as i,
  G as a,
  Ga as o,
  Qt as s,
  Rn as c,
  Rt as l,
  Sr as u,
  Tl as d,
  W as f,
  Xs as p,
  Yt as m,
  _l as h,
  cl as g,
  ec as _,
  ft as v,
  kt as y,
  ot as b,
  st as x,
  tc as ee,
  ul as S,
  v as te,
  vl as C,
  y as ne,
  zn as re,
  zt as ie,
} from "./app-initial~app-main~worktree-init-v2-page~appgen-settings-page~page~appgen-publication-ter~fzo5ij6p.js";
import {
  Dw as ae,
  Eb as oe,
  Ew as se,
  Fu as ce,
  Is as le,
  Ls as ue,
  Pf as de,
  Pu as w,
  Tw as T,
  bl as fe,
  bp as E,
  gh as D,
  hh as pe,
  i_ as me,
  jf as he,
  lo as ge,
  oo as _e,
  r_ as ve,
  so as ye,
  wb as O,
  ww as k,
  yl as be,
} from "./app-initial~app-main~onboarding-page.js";
import {
  $ as A,
  G as j,
  W as M,
  nt as xe,
  rt as N,
  tt as P,
} from "./app-initial~app-main~remote-conversation-page~new-thread-panel-page~onboarding-page~project~okyna5mq.js";
import {
  By as Se,
  Fn as Ce,
  Gn as we,
  Hy as F,
  Jn as Te,
  Kn as Ee,
  Ln as I,
  Qs as De,
  Rr as Oe,
  Rs as ke,
  Sr as Ae,
  Sx as L,
  Tr as je,
  Xr as Me,
  Xy as Ne,
  Yn as Pe,
  Yy as Fe,
  _y as R,
  d_ as Ie,
  ei as z,
  fv as B,
  gy as Le,
  nx as V,
  pv as Re,
  rx as ze,
  u_ as H,
  vx as U,
  wx as Be,
  xr as W,
  yx as G,
} from "./app-initial~app-main~new-thread-panel-page.js";
import {
  Br as Ve,
  Ci as He,
  Gr as K,
  Ir as q,
  Kr as Ue,
  Lr as We,
  Rr as J,
  Vr as Y,
  Xa as Ge,
  Ya as Ke,
  lu as qe,
  uu as Je,
  wi as Ye,
} from "./app-initial~app-main~remote-conversation-page~new-thread-panel-page~onboarding-page~appgen-~o4yhvtva.js";
import {
  $ as Xe,
  H as Ze,
  L as Qe,
  Q as $e,
  U as et,
  Z as tt,
  et as nt,
  z as rt,
} from "./app-initial~app-main~worktree-init-v2-page~remote-conversation-page~pull-requests-page~new-~djgpfzje.js";
import {
  Jn as it,
  M as X,
  N as at,
  Yn as ot,
  h as st,
  m as ct,
} from "./app-initial~app-main~worktree-init-v2-page~remote-conversation-page~pull-requests-page~new-~mam8fk3d.js";
import {
  a as lt,
  o as ut,
} from "./app-initial~app-main~remote-conversation-page~new-thread-panel-page~projects-index-page~app~edtlcz4n.js";
import {
  a as dt,
  i as Z,
  o as ft,
  r as pt,
  s as mt,
} from "./app-initial~app-main~appgen-page~remote-conversation-page~projects-index-page~appgen-librar~i4jkvfhy.js";
function ht(e) {
  let t = (0, yt.c)(4),
    { statusState: n } = e;
  if ((n.unreadCount ?? 0) > 0) {
    let e = n.unreadCount ?? 0,
      r;
    return (
      t[0] === e ? (r = t[1]) : ((r = (0, bt.jsx)(gt, { count: e })), (t[0] = e), (t[1] = r)), r
    );
  }
  if (n.type === `loading`) {
    let e;
    return (
      t[2] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((e = (0, bt.jsx)(vt, {})), (t[2] = e))
        : (e = t[2]),
      e
    );
  }
  if (n.unread === !0) {
    let e;
    return (
      t[3] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((e = (0, bt.jsx)(_t, {})), (t[3] = e))
        : (e = t[3]),
      e
    );
  }
  return null;
}
function gt(e) {
  let t = (0, yt.c)(3),
    { count: n } = e,
    r = n > 99 ? `99+` : n,
    i;
  t[0] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((i = {
        backgroundColor: `color-mix(in srgb, var(--vscode-textLink-foreground) 18%, transparent)`,
        boxShadow: `inset 0 0 0 1px color-mix(in srgb, var(--vscode-textLink-foreground) 72%, transparent)`,
        color: `var(--vscode-textLink-foreground)`,
      }),
      (t[0] = i))
    : (i = t[0]);
  let a;
  return (
    t[1] === r
      ? (a = t[2])
      : ((a = (0, bt.jsx)(`div`, {
          className: `relative flex h-5 min-w-5 shrink-0 items-center justify-center`,
          children: (0, bt.jsx)(`span`, {
            className: `flex h-5 min-w-5 items-center justify-center rounded-full px-1.5 text-[11px] leading-none font-semibold`,
            style: i,
            children: r,
          }),
        })),
        (t[1] = r),
        (t[2] = a)),
    a
  );
}
function _t() {
  let e = (0, yt.c)(1),
    t;
  return (
    e[0] === Symbol.for(`react.memo_cache_sentinel`)
      ? ((t = (0, bt.jsx)(`div`, {
          className: `relative flex size-5 shrink-0 items-center justify-center text-token-description-foreground`,
          children: (0, bt.jsx)(`span`, {
            className: `icon-xs relative scale-50`,
            children: (0, bt.jsx)(`span`, {
              className: `absolute inset-0 rounded-full`,
              style: { backgroundColor: `var(--vscode-textLink-foreground)` },
            }),
          }),
        })),
        (e[0] = t))
      : (t = e[0]),
    t
  );
}
function vt() {
  let e = (0, yt.c)(1),
    t;
  return (
    e[0] === Symbol.for(`react.memo_cache_sentinel`)
      ? ((t = (0, bt.jsx)(`div`, {
          className: `relative flex size-5 shrink-0 items-center justify-center text-token-foreground/70`,
          children: (0, bt.jsx)(Xe, { className: `icon-xs shrink-0`, animationDurationMs: 2e3 }),
        })),
        (e[0] = t))
      : (t = e[0]),
    t
  );
}
var yt,
  bt,
  xt = e(() => {
    ((yt = d()), nt(), (bt = C()));
  });
function St(e) {
  let t = (0, Ct.c)(10),
    { onUndo: n, onOpenSettings: r } = e;
  switch (`electron`) {
    case `chrome-extension`:
    case `extension`: {
      let e;
      return (
        t[0] === Symbol.for(`react.memo_cache_sentinel`)
          ? ((e = (0, wt.jsx)(L, {
              id: `codex.archiveInfo.extension`,
              defaultMessage: `View archived chats in your .codex folder.`,
              description: `Info message shown after archiving a Codex conversation or task in the extension`,
            })),
            (t[0] = e))
          : (e = t[0]),
        e
      );
    }
    case `browser`:
    case `electron`: {
      let e;
      t[1] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((e = (0, wt.jsx)(L, {
            id: `codex.archiveInfo.undoLink`,
            defaultMessage: `Undo`,
            description: `Link label to undo archiving a chat`,
          })),
          (t[1] = e))
        : (e = t[1]);
      let i;
      t[2] === n
        ? (i = t[3])
        : ((i = (0, wt.jsx)(
            `button`,
            {
              className: `cursor-interaction text-token-link underline-offset-2 hover:underline`,
              type: `button`,
              onClick: n,
              children: e,
            },
            `undo`,
          )),
          (t[2] = n),
          (t[3] = i));
      let a;
      t[4] === Symbol.for(`react.memo_cache_sentinel`)
        ? ((a = (0, wt.jsx)(L, {
            id: `codex.archiveInfo.settingsLink`,
            defaultMessage: `Settings`,
            description: `Link label to open the archived chats settings page`,
          })),
          (t[4] = a))
        : (a = t[4]);
      let o;
      t[5] === r
        ? (o = t[6])
        : ((o = (0, wt.jsx)(
            `button`,
            {
              className: `cursor-interaction text-token-link underline-offset-2 hover:underline`,
              type: `button`,
              onClick: r,
              children: a,
            },
            `settings`,
          )),
          (t[5] = r),
          (t[6] = o));
      let s;
      return (
        t[7] !== i || t[8] !== o
          ? ((s = (0, wt.jsx)(L, {
              id: `codex.archiveInfo.electronWithUndo`,
              defaultMessage: `{undoLink} or view archived chats in {settingsLink}`,
              description: `Info message shown after archiving a Codex conversation or task in the electron app`,
              values: { undoLink: i, settingsLink: o },
            })),
            (t[7] = i),
            (t[8] = o),
            (t[9] = s))
          : (s = t[9]),
        s
      );
    }
  }
}
var Ct,
  wt,
  Tt = e(() => {
    ((Ct = d()), G(), (wt = C()));
  });
function Et(e) {
  return e === `worktree` || e === `cloud` || e === `remote` || e === `remote-worktree`;
}
var Dt = e(() => {});
function Ot(e) {
  let t = (0, At.c)(4),
    { envType: n } = e;
  switch (n) {
    case `worktree`: {
      let e;
      return (
        t[0] === Symbol.for(`react.memo_cache_sentinel`)
          ? ((e = (0, jt.jsx)(L, {
              id: `codex.taskRowLayout.hoverCard.worktree`,
              defaultMessage: `Worktree`,
              description: `Environment label shown in a sidebar row hover card`,
            })),
            (t[0] = e))
          : (e = t[0]),
        e
      );
    }
    case `cloud`: {
      let e;
      return (
        t[1] === Symbol.for(`react.memo_cache_sentinel`)
          ? ((e = (0, jt.jsx)(L, {
              id: `codex.taskRowLayout.hoverCard.cloudTask`,
              defaultMessage: `Cloud task`,
              description: `Environment label shown in a sidebar row hover card`,
            })),
            (t[1] = e))
          : (e = t[1]),
        e
      );
    }
    case `remote`: {
      let e;
      return (
        t[2] === Symbol.for(`react.memo_cache_sentinel`)
          ? ((e = (0, jt.jsx)(L, {
              id: `codex.taskRowLayout.hoverCard.remote`,
              defaultMessage: `Remote`,
              description: `Environment label shown in a sidebar row hover card`,
            })),
            (t[2] = e))
          : (e = t[2]),
        e
      );
    }
    case `remote-worktree`: {
      let e;
      return (
        t[3] === Symbol.for(`react.memo_cache_sentinel`)
          ? ((e = (0, jt.jsx)(L, {
              id: `codex.taskRowLayout.hoverCard.remoteWorktree`,
              defaultMessage: `Remote worktree`,
              description: `Environment label shown in a sidebar row hover card`,
            })),
            (t[3] = e))
          : (e = t[3]),
        e
      );
    }
  }
}
function kt(e) {
  let t = (0, At.c)(21),
    { envTooltip: n, envType: r, className: i, hostId: a, disableTooltip: o } = e,
    s = o === void 0 ? !1 : o,
    c = Et(r) ? r : null;
  if (!c) return null;
  let l = n != null && n !== ``,
    u = s || l,
    d;
  bb0: switch (c) {
    case `remote`: {
      if (a == null) return null;
      let e;
      (t[0] !== i || t[1] !== u || t[2] !== a
        ? ((e = (0, jt.jsx)(Z, { className: i, disableTooltip: u, hostId: a })),
          (t[0] = i),
          (t[1] = u),
          (t[2] = a),
          (t[3] = e))
        : (e = t[3]),
        (d = e));
      break bb0;
    }
    case `remote-worktree`: {
      if (a == null) return null;
      let e;
      (t[4] !== i || t[5] !== u || t[6] !== a
        ? ((e = (0, jt.jsx)(dt, { className: i, disableTooltip: u, hostId: a })),
          (t[4] = i),
          (t[5] = u),
          (t[6] = a),
          (t[7] = e))
        : (e = t[7]),
        (d = e));
      break bb0;
    }
    case `worktree`: {
      let e;
      (t[8] !== i || t[9] !== u
        ? ((e = (0, jt.jsx)(ft, { className: i, disableTooltip: u })),
          (t[8] = i),
          (t[9] = u),
          (t[10] = e))
        : (e = t[10]),
        (d = e));
      break bb0;
    }
    case `cloud`: {
      let e;
      (t[11] !== i || t[12] !== u
        ? ((e = (0, jt.jsx)(pt, { className: i, disableTooltip: u })),
          (t[11] = i),
          (t[12] = u),
          (t[13] = e))
        : (e = t[13]),
        (d = e));
    }
  }
  let f;
  t[14] !== s || t[15] !== n || t[16] !== l || t[17] !== d
    ? ((f = l && !s ? (0, jt.jsx)(A, { tooltipContent: n, children: d }) : d),
      (t[14] = s),
      (t[15] = n),
      (t[16] = l),
      (t[17] = d),
      (t[18] = f))
    : (f = t[18]);
  let p = f,
    m;
  return (
    t[19] === p
      ? (m = t[20])
      : ((m = (0, jt.jsx)(`span`, {
          className: `flex shrink-0 items-center align-middle text-sm`,
          children: p,
        })),
        (t[19] = p),
        (t[20] = m)),
    m
  );
}
var At,
  jt,
  Mt = e(() => {
    ((At = d()), G(), P(), mt(), Dt(), (jt = C()));
  }),
  Nt,
  Pt,
  Ft = e(() => {
    (t(i()),
      (Nt = C()),
      (Pt = (e) =>
        (0, Nt.jsxs)(`svg`, {
          width: 14,
          height: 14,
          viewBox: `0 0 14 14`,
          fill: `none`,
          xmlns: `http://www.w3.org/2000/svg`,
          ...e,
          children: [
            (0, Nt.jsx)(`path`, {
              d: `M1.3812 8.27413V4.75577C1.3812 4.32969 1.38061 3.98381 1.40346 3.70408C1.42674 3.41945 1.47636 3.16514 1.59688 2.92852L1.67323 2.79173C1.86352 2.4815 2.13672 2.22875 2.46279 2.06261L2.5525 2.02062C2.76355 1.93013 2.98935 1.88955 3.23835 1.86919C3.51808 1.84634 3.86396 1.84693 4.29004 1.84693H4.74304C4.83633 1.84692 4.90121 1.84681 4.96508 1.85074L5.1305 1.86792C5.51249 1.92388 5.87119 2.09044 6.1612 2.34891L6.32089 2.50479C6.39392 2.57845 6.4179 2.6024 6.44114 2.62313L6.51367 2.6823C6.68734 2.81274 6.89613 2.89025 7.11427 2.9037L7.28351 2.90561H8.91989C9.28374 2.90561 9.57891 2.90537 9.81889 2.92215C10.0629 2.93924 10.2824 2.97524 10.4907 3.06403L10.6625 3.14738C11.0519 3.35977 11.3607 3.69801 11.5361 4.10936L11.5927 4.26715C11.6409 4.42714 11.6644 4.59509 11.6773 4.77677C11.6942 5.01512 11.6945 5.30802 11.6945 5.66813C11.6945 5.78286 11.6489 5.89296 11.5679 5.97415C11.4866 6.0554 11.3761 6.1014 11.2612 6.1014H2.24774V8.27413C2.24774 8.71449 2.24816 9.01893 2.26747 9.25519C2.28636 9.48629 2.32126 9.61398 2.36926 9.70819L2.41189 9.78517C2.51891 9.95962 2.67262 10.1015 2.85598 10.1949L2.93424 10.2286C3.02069 10.2595 3.1358 10.2825 3.30897 10.2967C3.54523 10.316 3.84967 10.3164 4.29004 10.3164H6.13002L6.21782 10.3253C6.41513 10.3658 6.56329 10.5404 6.56329 10.7497C6.56329 10.959 6.41513 11.1336 6.21782 11.1741L6.13002 11.183H4.29004C3.86396 11.183 3.51808 11.1836 3.23835 11.1607C2.98935 11.1403 2.76355 11.0998 2.5525 11.0093L2.46279 10.9673C2.13672 10.8011 1.86352 10.5484 1.67323 10.2382L1.59688 10.1014C1.47636 9.86476 1.42674 9.61044 1.40346 9.32581C1.38061 9.04609 1.3812 8.70021 1.3812 8.27413ZM2.24774 5.23485H10.826C10.8244 5.0787 10.8212 4.94989 10.8133 4.83848C10.8029 4.6911 10.7856 4.59201 10.7631 4.51718L10.7389 4.44911C10.6402 4.21783 10.4666 4.02778 10.2477 3.90831L10.151 3.86123C10.0685 3.82607 9.95714 3.8007 9.75844 3.78679C9.55573 3.77261 9.29595 3.77216 8.91989 3.77216H7.28351L7.06083 3.76834C6.673 3.74442 6.30198 3.60706 5.99323 3.37515L5.86471 3.27017C5.81684 3.2275 5.77151 3.18072 5.70566 3.1143L5.58477 2.99596C5.42152 2.85042 5.2196 2.75635 5.00453 2.72492L4.91228 2.71538C4.88114 2.71346 4.84701 2.71347 4.74304 2.71347H4.29004C3.84967 2.71347 3.54523 2.71389 3.30897 2.73319C3.1358 2.74735 3.02069 2.77037 2.93424 2.80127L2.85598 2.83499C2.67262 2.92843 2.51891 3.07028 2.41189 3.24472L2.36926 3.32171C2.32126 3.41592 2.28636 3.54361 2.26747 3.7747C2.24816 4.01096 2.24774 4.3154 2.24774 4.75577V5.23485Z`,
              fill: `currentColor`,
            }),
            (0, Nt.jsx)(`path`, {
              d: `M9.51822 12.2436C9.51822 12.0609 9.37009 11.9127 9.18736 11.9127C9.00463 11.9127 8.8565 12.0609 8.8565 12.2436C8.8565 12.4263 9.00463 12.5745 9.18736 12.5745C9.37009 12.5745 9.51822 12.4263 9.51822 12.2436ZM9.51822 9.50914C9.51822 9.3264 9.37009 9.17828 9.18736 9.17828C9.00463 9.17828 8.8565 9.3264 8.8565 9.50914C8.8565 9.69187 9.00463 9.84 9.18736 9.84C9.37009 9.84 9.51822 9.69187 9.51822 9.50914ZM12.0249 9.50914C12.0249 9.32643 11.8767 9.17833 11.694 9.17828C11.5113 9.17828 11.3632 9.3264 11.3632 9.50914C11.3632 9.69187 11.5113 9.84 11.694 9.84C11.8767 9.83995 12.0249 9.69184 12.0249 9.50914ZM12.3886 9.50914C12.3886 9.85195 12.14 10.1361 11.8134 10.1928H11.8759V10.4205C11.8759 10.7727 11.5903 11.0582 11.2382 11.0582H9.64319C9.49192 11.0582 9.36921 11.1809 9.36921 11.3322V11.5739C9.6645 11.6539 9.88192 11.923 9.88192 12.2436C9.88192 12.6272 9.57095 12.9382 9.18736 12.9382C8.80377 12.9382 8.4928 12.6272 8.4928 12.2436C8.4928 11.923 8.71022 11.6539 9.00551 11.5739V10.1786C8.71025 10.0986 8.4928 9.82971 8.4928 9.50914C8.4928 9.12555 8.80377 8.81458 9.18736 8.81458C9.57095 8.81458 9.88192 9.12555 9.88192 9.50914C9.88192 9.82971 9.66447 10.0986 9.36921 10.1786V10.757C9.45227 10.7174 9.54503 10.6945 9.64319 10.6945H11.2382C11.3895 10.6945 11.5122 10.5718 11.5122 10.4205V10.1928H11.5747C11.248 10.1361 10.9995 9.85201 10.9995 9.50914C10.9995 9.12555 11.3104 8.81458 11.694 8.81458C12.0776 8.81462 12.3886 9.12558 12.3886 9.50914Z`,
              fill: `currentColor`,
            }),
            (0, Nt.jsx)(`path`, {
              d: `M11.694 8.75989C12.1078 8.75994 12.4438 9.09507 12.444 9.50891C12.444 9.8398 12.2286 10.1186 11.9313 10.2179V10.421C11.931 10.8033 11.6203 11.1134 11.2379 11.1134H9.64319C9.52231 11.1134 9.42446 11.2113 9.42444 11.3322V11.5353C9.72159 11.6346 9.937 11.9125 9.93713 12.2433C9.93713 12.6573 9.6011 12.9933 9.18713 12.9933C8.77327 12.9932 8.43811 12.6572 8.43811 12.2433C8.43824 11.9122 8.65326 11.6334 8.95081 11.5343V10.2169C8.65342 10.1177 8.43811 9.84 8.43811 9.50891C8.43823 9.09512 8.77335 8.76001 9.18713 8.75989C9.60102 8.75989 9.93701 9.09505 9.93713 9.50891C9.93713 9.83972 9.72143 10.1165 9.42444 10.2159V10.6759C9.49314 10.6528 9.56654 10.6398 9.64319 10.6398H11.2379C11.3586 10.6398 11.4574 10.5417 11.4576 10.421V10.2179C11.1599 10.1188 10.9449 9.84013 10.9449 9.50891C10.9451 9.09507 11.2801 8.75992 11.694 8.75989ZM9.18713 11.9679C9.03498 11.968 8.91191 12.0911 8.91174 12.2433C8.91174 12.3956 9.03488 12.5195 9.18713 12.5197C9.33949 12.5197 9.4635 12.3956 9.4635 12.2433C9.46334 12.0911 9.33939 11.9679 9.18713 11.9679ZM9.18713 9.23352C9.03496 9.23364 8.91187 9.35673 8.91174 9.50891C8.91174 9.66119 9.03488 9.78516 9.18713 9.78528C9.33949 9.78528 9.4635 9.66127 9.4635 9.50891C9.46338 9.35666 9.33942 9.23352 9.18713 9.23352ZM11.694 9.23352C11.5417 9.23356 11.4187 9.35668 11.4186 9.50891C11.4186 9.66125 11.5416 9.78524 11.694 9.78528C11.8463 9.78524 11.9703 9.66123 11.9703 9.50891C11.9702 9.3567 11.8462 9.23356 11.694 9.23352Z`,
              stroke: `currentColor`,
              strokeWidth: 0.11,
              strokeLinejoin: `round`,
            }),
          ],
        })));
  });
function It(e) {
  let t = (0, Ht.c)(18),
    {
      projectIcon: n,
      projectLabel: r,
      repositoryLabel: i,
      sections: a,
      showUnreadDot: o,
      timestampDateString: s,
      threadTitle: c,
      threadTitleValue: l,
      onRenameThreadTitle: u,
    } = e,
    d = o === void 0 ? !1 : o,
    f;
  t[0] !== u || t[1] !== d || t[2] !== c || t[3] !== l || t[4] !== s
    ? ((f =
        c != null && c !== ``
          ? (0, Q.jsxs)(`div`, {
              className: `flex w-full min-w-0 items-center gap-3 pb-0.5`,
              children: [
                (0, Q.jsx)(zt, { onRenameThreadTitle: u, title: c, titleValue: l }),
                s != null && s !== ``
                  ? (0, Q.jsxs)(`div`, {
                      className: `flex shrink-0 items-center gap-1 text-xs leading-5 text-token-description-foreground`,
                      children: [
                        (0, Q.jsx)(he, { dateString: s }),
                        d
                          ? (0, Q.jsx)(`span`, {
                              "aria-hidden": !0,
                              className: `h-1.5 w-1.5 rounded-full bg-token-charts-blue`,
                            })
                          : null,
                      ],
                    })
                  : null,
              ],
            })
          : null),
      (t[0] = u),
      (t[1] = d),
      (t[2] = c),
      (t[3] = l),
      (t[4] = s),
      (t[5] = f))
    : (f = t[5]);
  let p;
  t[6] !== n || t[7] !== r
    ? ((p =
        r != null && r !== ``
          ? (0, Q.jsx)(Vt, { row: { id: `project`, icon: n ?? (0, Q.jsx)(He, {}), label: r } })
          : null),
      (t[6] = n),
      (t[7] = r),
      (t[8] = p))
    : (p = t[8]);
  let m;
  t[9] === i
    ? (m = t[10])
    : ((m =
        i != null && i !== ``
          ? (0, Q.jsx)(Vt, { row: { id: `repository`, icon: (0, Q.jsx)(Pt, {}), label: i } })
          : null),
      (t[9] = i),
      (t[10] = m));
  let h;
  t[11] === a ? (h = t[12]) : ((h = a.map(Lt)), (t[11] = a), (t[12] = h));
  let g;
  return (
    t[13] !== f || t[14] !== p || t[15] !== m || t[16] !== h
      ? ((g = (0, Q.jsxs)(`div`, {
          className: `flex w-fit max-w-[min(20rem,calc(100vw-16px))] min-w-56 flex-col gap-1 px-row-x py-1.5 text-token-foreground`,
          children: [f, p, m, h],
        })),
        (t[13] = f),
        (t[14] = p),
        (t[15] = m),
        (t[16] = h),
        (t[17] = g))
      : (g = t[17]),
    g
  );
}
function Lt(e) {
  return (0, Q.jsx)(
    `div`,
    { className: `flex min-w-0 flex-col gap-1`, children: e.rows.map(Rt) },
    e.id,
  );
}
function Rt(e) {
  return (0, Q.jsx)(Vt, { row: e }, e.id);
}
function zt(e) {
  let t = (0, Ht.c)(22),
    { onRenameThreadTitle: n, title: r, titleValue: i } = e,
    a = Be(),
    [o, s] = (0, Ut.useState)(!1),
    [c, l] = (0, Ut.useState)(i ?? ``),
    u = n != null && i != null,
    d;
  t[0] !== n || t[1] !== i
    ? ((d = (e) => {
        let t = e.trim();
        if ((s(!1), t.length === 0 || t === i)) {
          l(i ?? ``);
          return;
        }
        n?.(t);
      }),
      (t[0] = n),
      (t[1] = i),
      (t[2] = d))
    : (d = t[2]);
  let f = d;
  if (!u) {
    let e;
    return (
      t[3] === r
        ? (e = t[4])
        : ((e = (0, Q.jsx)(`div`, {
            className: `w-0 min-w-0 flex-1 truncate text-base leading-6 font-medium text-token-foreground`,
            children: r,
          })),
          (t[3] = r),
          (t[4] = e)),
      e
    );
  }
  if (o) {
    let e;
    t[5] === f
      ? (e = t[6])
      : ((e = (e) => {
          e.currentTarget.dataset.cancelRename !== `true` && f(e.currentTarget.value);
        }),
        (t[5] = f),
        (t[6] = e));
    let n;
    t[7] === Symbol.for(`react.memo_cache_sentinel`)
      ? ((n = (e) => {
          l(e.target.value);
        }),
        (t[7] = n))
      : (n = t[7]);
    let r;
    t[8] === i
      ? (r = t[9])
      : ((r = (e) => {
          if (e.key === `Enter`) {
            (e.preventDefault(), e.currentTarget.blur());
            return;
          }
          e.key === `Escape` &&
            (e.preventDefault(),
            (e.currentTarget.dataset.cancelRename = `true`),
            l(i ?? ``),
            s(!1));
        }),
        (t[8] = i),
        (t[9] = r));
    let o;
    t[10] === a
      ? (o = t[11])
      : ((o = a.formatMessage({
          id: `codex.taskRowHoverCard.threadTitleAriaLabel`,
          defaultMessage: `Chat title`,
          description: `Accessible label for the inline chat title editor in the thread hover card`,
        })),
        (t[10] = a),
        (t[11] = o));
    let u;
    return (
      t[12] !== c || t[13] !== e || t[14] !== r || t[15] !== o
        ? ((u = (0, Q.jsx)(`input`, {
            autoFocus: !0,
            size: 1,
            className: `h-6 w-0 min-w-0 flex-1 rounded-md border border-token-focus-border bg-token-input-background px-1.5 text-base leading-6 font-medium text-token-input-foreground outline-none`,
            value: c,
            onBlur: e,
            onChange: n,
            onFocus: Bt,
            onKeyDown: r,
            "aria-label": o,
          })),
          (t[12] = c),
          (t[13] = e),
          (t[14] = r),
          (t[15] = o),
          (t[16] = u))
        : (u = t[16]),
      u
    );
  }
  let p;
  t[17] === i
    ? (p = t[18])
    : ((p = () => {
        (l(i ?? ``), s(!0));
      }),
      (t[17] = i),
      (t[18] = p));
  let m;
  return (
    t[19] !== p || t[20] !== r
      ? ((m = (0, Q.jsx)(`button`, {
          type: `button`,
          className: `w-0 min-w-0 flex-1 cursor-interaction truncate rounded-md text-left text-base leading-6 font-medium text-token-foreground hover:bg-token-list-hover-background focus-visible:bg-token-list-hover-background focus-visible:outline-none`,
          onClick: p,
          children: r,
        })),
        (t[19] = p),
        (t[20] = r),
        (t[21] = m))
      : (m = t[21]),
    m
  );
}
function Bt(e) {
  e.currentTarget.select();
}
function Vt(e) {
  let t = (0, Ht.c)(29),
    { row: n } = e,
    r;
  t[0] === n.icon
    ? (r = t[1])
    : ((r = (0, Ut.cloneElement)(n.icon, { className: V(`icon-xs`, n.icon.props.className) })),
      (t[0] = n.icon),
      (t[1] = r));
  let i = r,
    a,
    o,
    s,
    c;
  if (
    t[2] !== i ||
    t[3] !== n.allowWrap ||
    t[4] !== n.label ||
    t[5] !== n.onClick ||
    t[6] !== n.tone
  ) {
    c = Symbol.for(`react.early_return_sentinel`);
    bb0: {
      let e = V(
          `flex min-w-0 gap-1.5 text-sm leading-5`,
          n.allowWrap ? `items-start` : `h-5 items-center`,
        ),
        r;
      t[11] === i
        ? (r = t[12])
        : ((r = (0, Q.jsx)(`span`, {
            className: `flex h-5 w-4 shrink-0 items-center justify-center text-token-description-foreground`,
            children: i,
          })),
          (t[11] = i),
          (t[12] = r));
      let l = n.allowWrap ? `whitespace-normal` : `overflow-hidden text-ellipsis whitespace-nowrap`,
        u = n.tone === `danger` ? `text-token-editor-warning-foreground` : `text-token-foreground`,
        d;
      t[13] !== l || t[14] !== u
        ? ((d = V(`block min-w-0 flex-1 leading-5`, l, u)), (t[13] = l), (t[14] = u), (t[15] = d))
        : (d = t[15]);
      let f;
      t[16] !== n.label || t[17] !== d
        ? ((f = (0, Q.jsx)(`span`, { className: d, children: n.label })),
          (t[16] = n.label),
          (t[17] = d),
          (t[18] = f))
        : (f = t[18]);
      let p;
      if (
        (t[19] !== r || t[20] !== f
          ? ((p = (0, Q.jsxs)(Q.Fragment, { children: [r, f] })),
            (t[19] = r),
            (t[20] = f),
            (t[21] = p))
          : (p = t[21]),
        (a = p),
        n.onClick == null)
      ) {
        c = (0, Q.jsx)(`div`, { className: e, children: a });
        break bb0;
      }
      ((o = `button`),
        (s = V(
          e,
          `w-full cursor-interaction rounded-md text-left hover:bg-token-list-hover-background focus-visible:bg-token-list-hover-background focus-visible:outline-none`,
        )));
    }
    ((t[2] = i),
      (t[3] = n.allowWrap),
      (t[4] = n.label),
      (t[5] = n.onClick),
      (t[6] = n.tone),
      (t[7] = a),
      (t[8] = o),
      (t[9] = s),
      (t[10] = c));
  } else ((a = t[7]), (o = t[8]), (s = t[9]), (c = t[10]));
  if (c !== Symbol.for(`react.early_return_sentinel`)) return c;
  let l;
  t[22] === n
    ? (l = t[23])
    : ((l = (e) => {
        (e.stopPropagation(), n.onClick?.(e));
      }),
      (t[22] = n),
      (t[23] = l));
  let u;
  return (
    t[24] !== a || t[25] !== o || t[26] !== s || t[27] !== l
      ? ((u = (0, Q.jsx)(`button`, { type: o, className: s, onClick: l, children: a })),
        (t[24] = a),
        (t[25] = o),
        (t[26] = s),
        (t[27] = l),
        (t[28] = u))
      : (u = t[28]),
    u
  );
}
var Ht,
  Ut,
  Q,
  Wt = e(() => {
    ((Ht = d()), ze(), (Ut = t(i(), 1)), G(), de(), Ft(), Ye(), (Q = C()));
  }),
  Gt,
  Kt,
  qt = e(() => {
    (t(i()),
      (Gt = C()),
      (Kt = (e) =>
        (0, Gt.jsxs)(`svg`, {
          width: 20,
          height: 20,
          viewBox: `0 0 20 20`,
          fill: `none`,
          xmlns: `http://www.w3.org/2000/svg`,
          ...e,
          children: [
            (0, Gt.jsx)(`path`, {
              d: `M11.8008 10.1816C12.1035 10.2438 12.3309 10.5119 12.3311 10.833C12.3311 11.1542 12.1036 11.4222 11.8008 11.4844L11.666 11.498H8.33301C7.96589 11.4979 7.66797 11.2002 7.66797 10.833C7.66814 10.466 7.966 10.1682 8.33301 10.168H11.666L11.8008 10.1816Z`,
              fill: `currentColor`,
            }),
            (0, Gt.jsx)(`path`, {
              fillRule: `evenodd`,
              clipRule: `evenodd`,
              d: `M15.417 2.66797C16.7045 2.66815 17.7489 3.71251 17.749 5V5.83301C17.749 6.33171 17.59 6.79271 17.3232 7.17188C17.3263 7.19763 17.3311 7.22343 17.3311 7.25V12.667C17.3311 13.3559 17.3317 13.9131 17.2949 14.3633C17.2622 14.7639 17.197 15.1246 17.0527 15.4609L16.9863 15.6035C16.7209 16.1245 16.3169 16.5602 15.8213 16.8643L15.6035 16.9863C15.2268 17.1782 14.8202 17.2575 14.3623 17.2949C13.9121 17.3317 13.3549 17.332 12.666 17.332H7.33301C6.64407 17.332 6.08689 17.3317 5.63672 17.2949C5.23627 17.2622 4.87521 17.1979 4.53906 17.0537L4.39648 16.9863C3.8754 16.7208 3.43882 16.3171 3.13477 15.8213L3.0127 15.6035C2.82089 15.227 2.74153 14.821 2.7041 14.3633C2.66732 13.9131 2.66797 13.3559 2.66797 12.667V7.25C2.66797 7.22312 2.67268 7.19694 2.67578 7.1709C2.4096 6.79197 2.25195 6.33115 2.25195 5.83301V5C2.25212 3.7124 3.29634 2.66797 4.58398 2.66797H15.417ZM16.001 8.08789C15.8141 8.13621 15.619 8.16501 15.417 8.16504H4.58398C4.38146 8.16504 4.18541 8.13644 3.99805 8.08789V12.667C3.99805 13.3778 3.99895 13.8714 4.03027 14.2549C4.06097 14.6303 4.11779 14.8421 4.19824 15L4.26855 15.126C4.44482 15.4134 4.69792 15.6478 5 15.8018L5.12988 15.8574C5.27361 15.9089 5.4633 15.9467 5.74512 15.9697C6.12858 16.0011 6.62215 16.002 7.33301 16.002H12.666C13.3767 16.002 13.8705 16.001 14.2539 15.9697C14.6292 15.9391 14.8411 15.8821 14.999 15.8018L15.126 15.7305C15.4132 15.5542 15.6479 15.3019 15.8018 15L15.8574 14.8691C15.9088 14.7255 15.9467 14.5363 15.9697 14.2549C16.0011 13.8714 16.001 13.3779 16.001 12.667V8.08789ZM4.58398 3.99805C4.03088 3.99805 3.5822 4.44693 3.58203 5V5.83301C3.58203 6.38621 4.03078 6.83496 4.58398 6.83496H15.417C15.97 6.83478 16.4189 6.3861 16.4189 5.83301V5C16.4188 4.44705 15.9699 3.99823 15.417 3.99805H4.58398Z`,
              fill: `currentColor`,
            }),
          ],
        })));
  }),
  Jt,
  Yt,
  Xt = e(() => {
    ((Jt = `_statusPillProgress_1a6wl_1`), (Yt = { statusPillProgress: Jt }));
  });
function Zt(e) {
  let t = (0, fn.c)(199),
    {
      title: n,
      titleSuffix: r,
      hoverCardContent: i,
      chips: a,
      statusPill: o,
      iconBadges: s,
      floatStatusIconsRight: c,
      metaContent: l,
      overlayMetaContent: u,
      metaState: d,
      hideInlineBadgesOnHover: f,
      isActive: p,
      onClick: m,
      onDoubleClick: h,
      onArchive: g,
      archiveAriaLabel: _,
      archiveConfirmLabel: v,
      statusState: y,
      statusIndicatorReplacesMeta: b,
      hideStatusIndicator: x,
      useStableTrailingRail: ee,
      envType: S,
      envTooltip: te,
      disableEnvTooltip: C,
      pulseEnvIcon: ne,
      envIconLocation: re,
      disabled: ie,
      allowActionsWhenDisabled: ae,
      blurOnClick: oe,
      idleIndicatorNode: se,
      idleStatusIndicatorNode: ce,
      indicatorRestNode: le,
      indicatorHoverNode: ue,
      reserveLeadingSlot: de,
      additionalHoverActionCount: w,
      priorityIndicatorNode: T,
      onContextMenu: fe,
      renderActions: E,
      onHoverCardOpenChange: D,
      hoverCardSide: pe,
      hoverCardAlign: me,
      hoverCardSideOffset: he,
      hoverCardAlignOffset: ge,
      metaHidden: _e,
      variant: ve,
      hostId: ye,
      dataAttributes: O,
    } = e,
    k;
  t[0] === a ? (k = t[1]) : ((k = a === void 0 ? [] : a), (t[0] = a), (t[1] = k));
  let be = k,
    j;
  t[2] === s ? (j = t[3]) : ((j = s === void 0 ? [] : s), (t[2] = s), (t[3] = j));
  let M = j,
    N = c === void 0 ? !1 : c,
    P = d === void 0 ? `default` : d,
    Se = f === void 0 ? !1 : f,
    Ce = p === void 0 ? !1 : p,
    we = b === void 0 ? !1 : b,
    F = x === void 0 ? !1 : x,
    Te = ee === void 0 ? !1 : ee,
    Ee = re === void 0 ? `end` : re,
    I = ie === void 0 ? !1 : ie,
    De = ae === void 0 ? !1 : ae,
    Oe = oe === void 0 ? !1 : oe,
    ke = de === void 0 ? !1 : de,
    Ae = w === void 0 ? 0 : w,
    L = pe === void 0 ? `right` : pe,
    je = me === void 0 ? `start` : me,
    Me = he === void 0 ? 2 : he,
    Ne = ge === void 0 ? 0 : ge,
    Pe = _e === void 0 ? !1 : _e,
    Fe = ve === void 0 ? `default` : ve,
    { type: R, unread: Ie } = y,
    z = Ie === void 0 ? !1 : Ie,
    [B, Le] = (0, pn.useState)(!1),
    Re;
  t[4] !== N || t[5] !== M ? ((Re = N ? [] : M), (t[4] = N), (t[5] = M), (t[6] = Re)) : (Re = t[6]);
  let ze = Re,
    H;
  t[7] !== N || t[8] !== M ? ((H = N ? M : []), (t[7] = N), (t[8] = M), (t[9] = H)) : (H = t[9]);
  let U = H,
    Be = N ? se : null,
    W = I ? null : (g ?? null),
    G;
  t[10] === W ? (G = t[11]) : ((G = W == null ? null : () => Le(!0)), (t[10] = W), (t[11] = G));
  let Ve = G,
    He = W != null,
    K = u != null,
    q = B && He,
    Ue = !q,
    We = Ae + (He ? 1 : 0),
    J =
      E == null
        ? `group-focus-within:hidden group-hover:hidden`
        : `group-hover:hidden group-has-[:focus-visible]:hidden`,
    Y = o != null || (we && (R === `loading` || z)),
    Ge = (N ? null : se) ?? le,
    Ke;
  t[12] !== T || t[13] !== q || t[14] !== Y || t[15] !== R
    ? ((Ke =
        R === `error` && !q && T == null && !Y
          ? (0, $.jsx)(`div`, {
              className: `relative flex size-5 shrink-0 items-center justify-center text-token-description-foreground`,
              children: (0, $.jsx)(it, {
                className: `icon-xs shrink-0 text-token-error-foreground`,
              }),
            })
          : null),
      (t[12] = T),
      (t[13] = q),
      (t[14] = Y),
      (t[15] = R),
      (t[16] = Ke))
    : (Ke = t[16]);
  let qe = Ke,
    Je = null;
  if (Ue && (!Y || (Te && !K)) && (!B || E != null) && l != null) {
    let e = Te && !K && `shrink-0`,
      n;
    t[17] !== J || t[18] !== e
      ? ((n = V(J, e)), (t[17] = J), (t[18] = e), (t[19] = n))
      : (n = t[19]);
    let r;
    (t[20] !== l || t[21] !== P || t[22] !== n
      ? ((r = (0, $.jsx)(ln, { className: n, content: l, metaState: P })),
        (t[20] = l),
        (t[21] = P),
        (t[22] = n),
        (t[23] = r))
      : (r = t[23]),
      (Je = r));
  }
  let Ye = T ?? (N ? null : qe) ?? Ge,
    Xe = ke || Ye != null || ue != null,
    Ze = Ye,
    Qe = T != null || (!N && qe != null) ? null : ue,
    $e = R !== `loading` && !z,
    et;
  t[24] !== _ || t[25] !== B || t[26] !== Ve || t[27] !== Te
    ? ((et =
        Ve != null && !B
          ? (0, $.jsx)(dn, { archiveAriaLabel: _, onArchive: Ve, useStableTrailingRail: Te })
          : null),
      (t[24] = _),
      (t[25] = B),
      (t[26] = Ve),
      (t[27] = Te),
      (t[28] = et))
    : (et = t[28]);
  let tt = et,
    nt;
  t[29] !== I || t[30] !== Xe || t[31] !== Qe || t[32] !== Ze || t[33] !== $e
    ? ((nt = Xe
        ? (0, $.jsx)(`div`, {
            className: V(`w-4`, I && `pointer-events-none`),
            children: (0, $.jsxs)(`div`, {
              className: `relative flex items-center justify-center`,
              children: [
                Ze
                  ? (0, $.jsx)(`span`, {
                      className: V(
                        `flex items-center justify-center`,
                        Qe &&
                          ($e
                            ? `group-focus-within:opacity-0 group-hover:opacity-0`
                            : `group-hover:opacity-0`),
                      ),
                      children: Ze,
                    })
                  : null,
                Qe
                  ? (0, $.jsx)(`span`, {
                      className: V(
                        `flex items-center justify-center opacity-0 pointer-events-none`,
                        $e
                          ? `group-focus-within:opacity-100 group-hover:opacity-100 group-focus-within:pointer-events-auto group-hover:pointer-events-auto`
                          : `group-hover:opacity-100 group-hover:pointer-events-auto`,
                        Ze ? `absolute inset-0` : `relative`,
                      ),
                      children: Qe,
                    })
                  : null,
              ],
            }),
          })
        : null),
      (t[29] = I),
      (t[30] = Xe),
      (t[31] = Qe),
      (t[32] = Ze),
      (t[33] = $e),
      (t[34] = nt))
    : (nt = t[34]);
  let rt = nt,
    X;
  t[35] !== I ||
  t[36] !== F ||
  t[37] !== ce ||
  t[38] !== q ||
  t[39] !== o ||
  t[40] !== y ||
  t[41] !== R ||
  t[42] !== z
    ? ((X = q
        ? null
        : o == null
          ? !F && (R === `loading` || z)
            ? (0, $.jsx)(ht, { statusState: y })
            : !F && R === `idle`
              ? ce
              : null
          : (0, $.jsx)(nn, { ...o, onClick: I ? void 0 : o.onClick }, o.progress?.deadlineMs)),
      (t[35] = I),
      (t[36] = F),
      (t[37] = ce),
      (t[38] = q),
      (t[39] = o),
      (t[40] = y),
      (t[41] = R),
      (t[42] = z),
      (t[43] = X))
    : (X = t[43]);
  let at = X,
    ot;
  t[44] !== De ||
  t[45] !== W ||
  t[46] !== tt ||
  t[47] !== I ||
  t[48] !== K ||
  t[49] !== E ||
  t[50] !== q ||
  t[51] !== Ve ||
  t[52] !== o?.onClick
    ? ((ot =
        (I && !De) || K || q || (o?.onClick != null && !I)
          ? null
          : E
            ? E({ archive: W, requestArchive: Ve })
            : tt),
      (t[44] = De),
      (t[45] = W),
      (t[46] = tt),
      (t[47] = I),
      (t[48] = K),
      (t[49] = E),
      (t[50] = q),
      (t[51] = Ve),
      (t[52] = o?.onClick),
      (t[53] = ot))
    : (ot = t[53]);
  let st = ot,
    ct = N && o == null,
    lt;
  t[54] !== st ||
  t[55] !== ct ||
  t[56] !== K ||
  t[57] !== Je ||
  t[58] !== J ||
  t[59] !== at ||
  t[60] !== o ||
  t[61] !== Y ||
  t[62] !== Te
    ? ((lt =
        at != null && !ct
          ? (0, $.jsx)(`span`, {
              "data-hover-card-open-immediately": !0,
              className: V(
                `flex shrink-0 items-center justify-center`,
                o == null ? `size-5 -mr-1` : `-mr-0.5`,
                Te && Y && Je != null && `absolute right-0`,
                K && `invisible`,
                st != null && J,
              ),
              children: at,
            })
          : null),
      (t[54] = st),
      (t[55] = ct),
      (t[56] = K),
      (t[57] = Je),
      (t[58] = J),
      (t[59] = at),
      (t[60] = o),
      (t[61] = Y),
      (t[62] = Te),
      (t[63] = lt))
    : (lt = t[63]);
  let ut = lt,
    dt;
  t[64] !== C || t[65] !== te || t[66] !== S || t[67] !== ye || t[68] !== ne
    ? ((dt =
        S === `worktree` || S === `cloud` || S === `remote` || S === `remote-worktree`
          ? (0, $.jsx)(kt, {
              className: ne ? `animate-pulse` : void 0,
              disableTooltip: C,
              envTooltip: te,
              envType: S,
              hostId: ye,
            })
          : null),
      (t[64] = C),
      (t[65] = te),
      (t[66] = S),
      (t[67] = ye),
      (t[68] = ne),
      (t[69] = dt))
    : (dt = t[69]);
  let Z = dt,
    ft;
  t[70] === Symbol.for(`react.memo_cache_sentinel`) ? ((ft = []), (t[70] = ft)) : (ft = t[70]);
  let pt = ft;
  if (N && Ee === `end`) {
    if (S === `remote-worktree`) {
      let e = ne ? `animate-pulse` : void 0,
        n;
      t[71] !== C || t[72] !== ye || t[73] !== e
        ? ((n = (0, $.jsx)(
            kt,
            { className: e, disableTooltip: C, envType: `remote`, hostId: ye },
            `remote`,
          )),
          (t[71] = C),
          (t[72] = ye),
          (t[73] = e),
          (t[74] = n))
        : (n = t[74]);
      let r = ne ? `animate-pulse` : void 0,
        i;
      t[75] !== C || t[76] !== r
        ? ((i = (0, $.jsx)(
            kt,
            { className: r, disableTooltip: C, envType: `worktree` },
            `worktree`,
          )),
          (t[75] = C),
          (t[76] = r),
          (t[77] = i))
        : (i = t[77]);
      let a;
      (t[78] !== n || t[79] !== i
        ? ((a = [n, i]), (t[78] = n), (t[79] = i), (t[80] = a))
        : (a = t[80]),
        (pt = a));
    } else if (Z != null) {
      let e;
      (t[81] === Z ? (e = t[82]) : ((e = [Z]), (t[81] = Z), (t[82] = e)), (pt = e));
    }
  }
  let mt = N ? qe : null,
    gt = N && o == null ? at : null,
    _t;
  t[83] !== mt || t[84] !== U || t[85] !== Be || t[86] !== gt
    ? ((_t = [...U, mt, Be, gt].filter($t)),
      (t[83] = mt),
      (t[84] = U),
      (t[85] = Be),
      (t[86] = gt),
      (t[87] = _t))
    : (_t = t[87]);
  let vt = _t.length + pt.length,
    yt = vt === 0 ? 0 : vt * vn + (vt - 1) * yn + bn,
    bt;
  t[88] !== st ||
  t[89] !== I ||
  t[90] !== K ||
  t[91] !== q ||
  t[92] !== pt ||
  t[93] !== mt ||
  t[94] !== U ||
  t[95] !== Be ||
  t[96] !== vt ||
  t[97] !== gt ||
  t[98] !== J
    ? ((bt =
        !q && vt > 0
          ? (0, $.jsx)(rn, {
              iconBadges: U,
              leadingIconNodes: pt,
              trailingIconNodes: [mt, Be, gt],
              disabled: I,
              className: V(_n, K && `invisible`, st != null && J),
            })
          : null),
      (t[88] = st),
      (t[89] = I),
      (t[90] = K),
      (t[91] = q),
      (t[92] = pt),
      (t[93] = mt),
      (t[94] = U),
      (t[95] = Be),
      (t[96] = vt),
      (t[97] = gt),
      (t[98] = J),
      (t[99] = bt))
    : (bt = t[99]);
  let xt = bt,
    St = Ue && (K || Je || (!ct && at) || q),
    Ct;
  st != null &&
    (E != null || !Te || !St) &&
    (We >= 3
      ? (Ct =
          E == null
            ? `group-focus-within:min-w-20 group-hover:min-w-20`
            : `group-hover:min-w-20 group-has-[:focus-visible]:min-w-20`)
      : We === 2
        ? (Ct =
            E == null
              ? `group-focus-within:min-w-12 group-hover:min-w-12`
              : `group-hover:min-w-12 group-has-[:focus-visible]:min-w-12`)
        : We === 1 &&
          (Ct =
            E == null
              ? `group-focus-within:min-w-5 group-hover:min-w-5`
              : `group-hover:min-w-5 group-has-[:focus-visible]:min-w-5`));
  let wt;
  t[100] !== B || t[101] !== I || t[102] !== m
    ? ((wt = (e) => {
        I ||
          B ||
          e.defaultPrevented ||
          (e.currentTarget === e.target &&
            (e.key === `Enter` || e.key === ` `) &&
            (e.preventDefault(), xe(), m()));
      }),
      (t[100] = B),
      (t[101] = I),
      (t[102] = m),
      (t[103] = wt))
    : (wt = t[103]);
  let Tt = wt,
    Et;
  t[104] !== Oe || t[105] !== B || t[106] !== I || t[107] !== m
    ? ((Et = (e) => {
        if (!I) {
          if (B) {
            (e.preventDefault(), e.stopPropagation());
            return;
          }
          e.defaultPrevented || (xe(), m(), Oe && e.currentTarget.blur());
        }
      }),
      (t[104] = Oe),
      (t[105] = B),
      (t[106] = I),
      (t[107] = m),
      (t[108] = Et))
    : (Et = t[108]);
  let Dt = Et,
    Ot;
  t[109] !== B || t[110] !== I || t[111] !== h
    ? ((Ot = (e) => {
        if (!I) {
          if (B) {
            (e.preventDefault(), e.stopPropagation());
            return;
          }
          e.defaultPrevented || h?.(e);
        }
      }),
      (t[109] = B),
      (t[110] = I),
      (t[111] = h),
      (t[112] = Ot))
    : (Ot = t[112]);
  let At = Ot,
    jt = N
      ? `pr-1 pl-[var(--padding-row-cell-x,var(--padding-row-x))]`
      : `px-[var(--padding-row-cell-x,var(--padding-row-x))]`,
    Mt = I && `opacity-60`,
    Nt = Ce && `bg-token-list-hover-background`,
    Pt = Fe === `tableRow` && `!h-9 bg-transparent !px-0 !py-0 hover:bg-transparent`,
    Ft;
  t[113] !== jt || t[114] !== Mt || t[115] !== Nt || t[116] !== Pt
    ? ((Ft = V(
        `group relative h-[var(--height-token-row)] cursor-interaction rounded-[var(--radius-token-row)] py-row-y text-sm hover:bg-token-list-hover-background focus-visible:outline-offset-[-2px]`,
        jt,
        Mt,
        Nt,
        Pt,
      )),
      (t[113] = jt),
      (t[114] = Mt),
      (t[115] = Nt),
      (t[116] = Pt),
      (t[117] = Ft))
    : (Ft = t[117]);
  let It = Ft,
    Lt = i != null && i !== !1 && i !== ``,
    Rt = typeof n == `string` || typeof n == `number` ? String(n) : void 0,
    zt = r == null && `flex-1`,
    Bt;
  t[118] === zt
    ? (Bt = t[119])
    : ((Bt = V(`min-w-0 truncate select-none`, zt)), (t[118] = zt), (t[119] = Bt));
  let Vt;
  t[120] !== Rt || t[121] !== Bt || t[122] !== n
    ? ((Vt = (0, $.jsx)(
        `span`,
        { className: Bt, "data-thread-title": !0, draggable: !1, children: n },
        Rt,
      )),
      (t[120] = Rt),
      (t[121] = Bt),
      (t[122] = n),
      (t[123] = Vt))
    : (Vt = t[123]);
  let Ht = Vt,
    Ut = I ? void 0 : fe,
    Q;
  t[124] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((Q = () => {
        Le(!1);
      }),
      (t[124] = Q))
    : (Q = t[124]);
  let Wt = I ? -1 : 0,
    Gt = I || void 0,
    Kt = Ce ? `page` : void 0,
    qt;
  t[125] === st
    ? (qt = t[126])
    : ((qt =
        st == null
          ? null
          : (0, $.jsx)(`div`, {
              className: `contents`,
              "data-hover-card-open-immediately": !0,
              children: st,
            })),
      (t[125] = st),
      (t[126] = qt));
  let Jt = rt != null && `ml-1.5`,
    Yt;
  t[127] === Jt
    ? (Yt = t[128])
    : ((Yt = V(`flex min-w-0 flex-1 items-center gap-2 pl-0.5`, Jt)), (t[127] = Jt), (t[128] = Yt));
  let Xt = Ee === `start` && Z,
    Zt = Ce ? `text-[var(--vscode-foreground)]` : `text-token-foreground`,
    tn = Fe === `tableRow` && `-ml-0.5 h-9 items-center self-center leading-9`,
    an;
  t[129] !== Zt || t[130] !== tn
    ? ((an = V(`flex min-w-0 flex-1 self-stretch items-center gap-2 text-base leading-5`, Zt, tn)),
      (t[129] = Zt),
      (t[130] = tn),
      (t[131] = an))
    : (an = t[131]);
  let on = Ee === `middle` && Z,
    sn;
  t[132] !== an || t[133] !== on || t[134] !== Ht || t[135] !== r
    ? ((sn = (0, $.jsxs)(`div`, {
        className: an,
        "data-thread-title-trigger": !0,
        children: [Ht, r, on],
      })),
      (t[132] = an),
      (t[133] = on),
      (t[134] = Ht),
      (t[135] = r),
      (t[136] = sn))
    : (sn = t[136]);
  let cn = Ee === `end` && !N && !q ? Z : null,
    mn = Se ? `group-hover:hidden` : void 0,
    hn;
  t[137] !== be || t[138] !== I || t[139] !== ze || t[140] !== q || t[141] !== cn || t[142] !== mn
    ? ((hn = (0, $.jsx)(en, {
        chips: be,
        envIconNode: cn,
        iconBadges: ze,
        hideInlineBadges: q,
        disabled: I,
        className: mn,
      })),
      (t[137] = be),
      (t[138] = I),
      (t[139] = ze),
      (t[140] = q),
      (t[141] = cn),
      (t[142] = mn),
      (t[143] = hn))
    : (hn = t[143]);
  let gn;
  t[144] !== Yt || t[145] !== Xt || t[146] !== sn || t[147] !== hn
    ? ((gn = (0, $.jsxs)(`div`, { className: Yt, children: [Xt, sn, hn] })),
      (t[144] = Yt),
      (t[145] = Xt),
      (t[146] = sn),
      (t[147] = hn),
      (t[148] = gn))
    : (gn = t[148]);
  let xn = Te && `relative mr-[var(--task-row-trailing-inset)]`,
    Sn = St && `min-w-[26px]`,
    Cn;
  t[149] !== Ct || t[150] !== xn || t[151] !== Sn
    ? ((Cn = V(`ml-[3px] flex items-center justify-end gap-1`, xn, Sn, Ct)),
      (t[149] = Ct),
      (t[150] = xn),
      (t[151] = Sn),
      (t[152] = Cn))
    : (Cn = t[152]);
  let wn;
  t[153] !== W ||
  t[154] !== v ||
  t[155] !== K ||
  t[156] !== Pe ||
  t[157] !== Je ||
  t[158] !== u ||
  t[159] !== q ||
  t[160] !== ut ||
  t[161] !== Y ||
  t[162] !== Te
    ? ((wn = q
        ? (0, $.jsx)(un, {
            label: v,
            onConfirm: (e) => {
              (e.stopPropagation(), Le(!1), W?.());
            },
          })
        : (0, $.jsxs)($.Fragment, {
            children: [
              u,
              Je &&
                (0, $.jsx)(`div`, {
                  className: V(((Te && Y) || Pe || K) && `invisible`),
                  children: Je,
                }),
              ut,
            ],
          })),
      (t[153] = W),
      (t[154] = v),
      (t[155] = K),
      (t[156] = Pe),
      (t[157] = Je),
      (t[158] = u),
      (t[159] = q),
      (t[160] = ut),
      (t[161] = Y),
      (t[162] = Te),
      (t[163] = wn))
    : (wn = t[163]);
  let Tn;
  t[164] !== Cn || t[165] !== wn
    ? ((Tn = (0, $.jsx)(`div`, { className: Cn, children: wn })),
      (t[164] = Cn),
      (t[165] = wn),
      (t[166] = Tn))
    : (Tn = t[166]);
  let En;
  t[167] !== st || t[168] !== yt || t[169] !== J
    ? ((En =
        yt > 0
          ? (0, $.jsx)(`div`, {
              className: V(`shrink-0`, st != null && J),
              style: { width: `${yt}px` },
            })
          : null),
      (t[167] = st),
      (t[168] = yt),
      (t[169] = J),
      (t[170] = En))
    : (En = t[170]);
  let Dn;
  t[171] !== rt || t[172] !== gn || t[173] !== Tn || t[174] !== En
    ? ((Dn = (0, $.jsxs)(`div`, {
        className: `flex h-full w-full items-center text-sm leading-4`,
        children: [rt, gn, Tn, En],
      })),
      (t[171] = rt),
      (t[172] = gn),
      (t[173] = Tn),
      (t[174] = En),
      (t[175] = Dn))
    : (Dn = t[175]);
  let On;
  t[176] !== It ||
  t[177] !== O ||
  t[178] !== Tt ||
  t[179] !== Dt ||
  t[180] !== At ||
  t[181] !== xt ||
  t[182] !== Ut ||
  t[183] !== Wt ||
  t[184] !== Gt ||
  t[185] !== Kt ||
  t[186] !== qt ||
  t[187] !== Dn
    ? ((On = (0, $.jsxs)(`div`, {
        ...O,
        className: It,
        onClick: Dt,
        onDoubleClick: At,
        onContextMenu: Ut,
        onPointerLeave: Q,
        onKeyDown: Tt,
        role: `button`,
        tabIndex: Wt,
        "aria-disabled": Gt,
        "aria-current": Kt,
        children: [qt, xt, Dn],
      })),
      (t[176] = It),
      (t[177] = O),
      (t[178] = Tt),
      (t[179] = Dt),
      (t[180] = At),
      (t[181] = xt),
      (t[182] = Ut),
      (t[183] = Wt),
      (t[184] = Gt),
      (t[185] = Kt),
      (t[186] = qt),
      (t[187] = Dn),
      (t[188] = On))
    : (On = t[188]);
  let kn = On;
  if (!Lt) return kn;
  let An;
  t[189] === D
    ? (An = t[190])
    : ((An = (e) => {
        D?.(e);
      }),
      (t[189] = D),
      (t[190] = An));
  let jn;
  return (
    t[191] !== je ||
    t[192] !== Ne ||
    t[193] !== i ||
    t[194] !== L ||
    t[195] !== Me ||
    t[196] !== kn ||
    t[197] !== An
      ? ((jn = (0, $.jsx)(A, {
          getDelayDuration: Qt,
          onOpenChange: An,
          side: L,
          align: je,
          alignOffset: Ne,
          sideOffset: Me,
          interactive: !0,
          variant: `rich`,
          tooltipContent: i,
          children: kn,
        })),
        (t[191] = je),
        (t[192] = Ne),
        (t[193] = i),
        (t[194] = L),
        (t[195] = Me),
        (t[196] = kn),
        (t[197] = An),
        (t[198] = jn))
      : (jn = t[198]),
    jn
  );
}
function Qt(e, t) {
  return cn(e.target) ? 0 : t;
}
function $t(e) {
  return e != null;
}
function en(e) {
  let t = (0, fn.c)(25),
    { className: n, chips: r, envIconNode: i, iconBadges: a, hideInlineBadges: o, disabled: s } = e,
    c;
  t[0] === r ? (c = t[1]) : ((c = r === void 0 ? [] : r), (t[0] = r), (t[1] = c));
  let l = c,
    u;
  t[2] === a ? (u = t[3]) : ((u = a === void 0 ? [] : a), (t[2] = a), (t[3] = u));
  let d = u,
    f = o === void 0 ? !1 : o,
    p = s === void 0 ? !1 : s,
    m;
  t[4] === l ? (m = t[5]) : ((m = l.flatMap(tn)), (t[4] = l), (t[5] = m));
  let h = m,
    g;
  if (t[6] !== p || t[7] !== d) {
    let e;
    (t[9] === p
      ? (e = t[10])
      : ((e = (e) => sn({ badge: e, disabled: p })), (t[9] = p), (t[10] = e)),
      (g = d.flatMap(e)),
      (t[6] = p),
      (t[7] = d),
      (t[8] = g));
  } else g = t[8];
  let _ = g,
    v;
  t[11] !== h || t[12] !== f
    ? ((v = f ? [] : h), (t[11] = h), (t[12] = f), (t[13] = v))
    : (v = t[13]);
  let y = v;
  if (y.length === 0 && i == null && _.length === 0) return null;
  let b;
  t[14] === n
    ? (b = t[15])
    : ((b = V(`flex min-w-[24px] items-center justify-end gap-2`, n)), (t[14] = n), (t[15] = b));
  let x;
  t[16] === y
    ? (x = t[17])
    : ((x =
        y.length > 0
          ? (0, $.jsx)(`div`, { className: `flex items-center gap-1`, children: y })
          : null),
      (t[16] = y),
      (t[17] = x));
  let ee;
  t[18] !== i || t[19] !== _
    ? ((ee =
        i != null || _.length > 0
          ? (0, $.jsxs)(`div`, {
              "data-hover-card-open-immediately": !0,
              className: `flex items-center gap-1.5`,
              children: [_, i],
            })
          : null),
      (t[18] = i),
      (t[19] = _),
      (t[20] = ee))
    : (ee = t[20]);
  let S;
  return (
    t[21] !== b || t[22] !== x || t[23] !== ee
      ? ((S = (0, $.jsxs)(`div`, { className: b, children: [x, ee] })),
        (t[21] = b),
        (t[22] = x),
        (t[23] = ee),
        (t[24] = S))
      : (S = t[24]),
    S
  );
}
function tn(e) {
  let { id: t, label: n } = e;
  return n == null
    ? []
    : [
        (0, $.jsx)(
          `span`,
          {
            className: `inline-flex max-w-[150px] items-center truncate rounded-full bg-token-charts-green/20 py-0.5 pr-2.5 pl-2 text-sm text-token-charts-green`,
            children: n,
          },
          t,
        ),
      ];
}
function nn(e) {
  let t = (0, fn.c)(31),
    { ariaLabel: n, label: r, hoverLabel: i, onClick: a, progress: o } = e,
    s = a != null,
    c;
  t[0] === o?.nowMs
    ? (c = t[1])
    : ((c = () => o?.nowMs ?? Date.now()), (t[0] = o?.nowMs), (t[1] = c));
  let [l] = (0, pn.useState)(c),
    u;
  t[2] !== s || t[3] !== o || t[4] !== l
    ? ((u =
        o == null
          ? null
          : (0, $.jsx)(`span`, {
              className: V(
                `pointer-events-none absolute inset-0 bg-token-charts-blue/25`,
                s &&
                  `group-hover:bg-token-foreground/20 group-focus-visible/status-pill:bg-token-foreground/20`,
                Yt.statusPillProgress,
              ),
              style: {
                animationDelay: `${o.deadlineMs - l - o.durationMs}ms`,
                animationDuration: `${o.durationMs}ms`,
                animationPlayState: o.nowMs == null ? void 0 : `paused`,
              },
              "aria-hidden": !0,
            })),
      (t[2] = s),
      (t[3] = o),
      (t[4] = l),
      (t[5] = u))
    : (u = t[5]);
  let d = s && `group-hover:invisible group-focus-visible/status-pill:invisible`,
    f;
  t[6] === d
    ? (f = t[7])
    : ((f = V(`relative col-start-1 row-start-1 min-w-0 truncate`, d)), (t[6] = d), (t[7] = f));
  let p;
  t[8] !== r || t[9] !== f
    ? ((p = (0, $.jsx)(`span`, { className: f, children: r })), (t[8] = r), (t[9] = f), (t[10] = p))
    : (p = t[10]);
  let m;
  t[11] !== i || t[12] !== s || t[13] !== r
    ? ((m = s
        ? (0, $.jsx)(`span`, {
            className: `invisible relative col-start-1 row-start-1 min-w-0 truncate group-hover:visible group-focus-visible/status-pill:visible`,
            children: i ?? r,
          })
        : null),
      (t[11] = i),
      (t[12] = s),
      (t[13] = r),
      (t[14] = m))
    : (m = t[14]);
  let h;
  t[15] !== u || t[16] !== p || t[17] !== m
    ? ((h = (0, $.jsxs)($.Fragment, { children: [u, p, m] })),
      (t[15] = u),
      (t[16] = p),
      (t[17] = m),
      (t[18] = h))
    : (h = t[18]);
  let g = h,
    _ =
      s &&
      `group/status-pill cursor-interaction group-hover:bg-token-foreground/10 group-hover:text-token-foreground focus-visible:bg-token-foreground/10 focus-visible:text-token-foreground focus-visible:outline-token-focus-ring focus-visible:outline-2 focus-visible:outline-offset-2`,
    v;
  t[19] === _
    ? (v = t[20])
    : ((v = V(
        `relative inline-grid max-w-[150px] shrink-0 items-center overflow-hidden rounded-full bg-token-charts-blue/15 py-0.5 pr-2.5 pl-2 text-sm text-token-charts-blue`,
        _,
      )),
      (t[19] = _),
      (t[20] = v));
  let y = v;
  if (!s) {
    let e;
    return (
      t[21] !== y || t[22] !== g
        ? ((e = (0, $.jsx)(`span`, { className: y, children: g })),
          (t[21] = y),
          (t[22] = g),
          (t[23] = e))
        : (e = t[23]),
      e
    );
  }
  let b;
  t[24] === a
    ? (b = t[25])
    : ((b = (e) => {
        (e.stopPropagation(), a());
      }),
      (t[24] = a),
      (t[25] = b));
  let x;
  return (
    t[26] !== n || t[27] !== y || t[28] !== g || t[29] !== b
      ? ((x = (0, $.jsx)(`button`, {
          className: y,
          type: `button`,
          "aria-label": n,
          onClick: b,
          children: g,
        })),
        (t[26] = n),
        (t[27] = y),
        (t[28] = g),
        (t[29] = b),
        (t[30] = x))
      : (x = t[30]),
    x
  );
}
function rn(e) {
  let t = (0, fn.c)(22),
    { className: n, disabled: r, iconBadges: i, leadingIconNodes: a, trailingIconNodes: o } = e,
    s = r === void 0 ? !1 : r,
    c,
    l;
  if (t[0] !== s || t[1] !== i || t[2] !== a || t[3] !== o) {
    let e = i === void 0 ? [] : i,
      n;
    (t[6] === a ? (n = t[7]) : ((n = a === void 0 ? [] : a), (t[6] = a), (t[7] = n)), (c = n));
    let r;
    t[8] === s ? (r = t[9]) : ((r = (e) => sn({ badge: e, disabled: s })), (t[8] = s), (t[9] = r));
    let u;
    (t[10] === o ? (u = t[11]) : ((u = o ?? []), (t[10] = o), (t[11] = u)),
      (l = [...e.flatMap(r), ...u].filter(on)),
      (t[0] = s),
      (t[1] = i),
      (t[2] = a),
      (t[3] = o),
      (t[4] = c),
      (t[5] = l));
  } else ((c = t[4]), (l = t[5]));
  let u = l,
    d;
  t[12] !== c || t[13] !== u
    ? ((d = [...c, ...u]), (t[12] = c), (t[13] = u), (t[14] = d))
    : (d = t[14]);
  let f = d;
  if (f.length === 0) return null;
  let p;
  t[15] === n
    ? (p = t[16])
    : ((p = V(`flex shrink-0 items-center justify-end`, n)), (t[15] = n), (t[16] = p));
  let m;
  t[17] === f ? (m = t[18]) : ((m = f.map(an)), (t[17] = f), (t[18] = m));
  let h;
  return (
    t[19] !== p || t[20] !== m
      ? ((h = (0, $.jsx)(`div`, {
          "data-hover-card-open-immediately": !0,
          className: p,
          children: m,
        })),
        (t[19] = p),
        (t[20] = m),
        (t[21] = h))
      : (h = t[21]),
    h
  );
}
function an(e, t) {
  return (0, $.jsx)(
    `span`,
    { className: `flex h-5 min-w-5 items-center justify-center`, children: e },
    t,
  );
}
function on(e) {
  return e != null;
}
function sn({ badge: e, disabled: t }) {
  let { id: n, icon: r, ariaLabel: i, className: a, onClick: o, tooltipContent: s } = e;
  if (!(0, pn.isValidElement)(r)) return [];
  let c = (0, pn.cloneElement)(r, { className: V(mn, r.props.className) }),
    l =
      o == null || t
        ? (0, $.jsx)(
            `span`,
            {
              className: V(`flex shrink-0 items-center text-token-description-foreground`, a),
              children: c,
            },
            n,
          )
        : (0, $.jsx)(
            `button`,
            {
              type: `button`,
              className: V(
                `focus-visible:outline-token-focus-ring flex shrink-0 items-center justify-center rounded-md text-token-description-foreground hover:text-token-foreground focus-visible:outline-2 focus-visible:outline-offset-2`,
                a,
              ),
              "aria-label": i,
              onClick: (e) => {
                (e.stopPropagation(), o());
              },
              children: c,
            },
            n,
          );
  return s != null && s !== ``
    ? [
        (0, $.jsx)(
          A,
          {
            tooltipContent: s,
            children: (0, $.jsx)(`span`, {
              className: V(`flex shrink-0 items-center`, a),
              children: l,
            }),
          },
          n,
        ),
      ]
    : [l];
}
function cn(e) {
  return e instanceof Element && e.closest(xn) != null;
}
function ln(e) {
  let t = (0, fn.c)(6),
    { className: n, content: r, metaState: i } = e;
  if (r == null) return null;
  let a = i === `loading` ? `loading-shimmer-pure-text min-w-[72px]` : `truncate`,
    o;
  t[0] !== n || t[1] !== a
    ? ((o = V(
        `text-token-description-foreground text-sm leading-4 empty:hidden tabular-nums overflow-visible`,
        a,
        `text-right`,
        n,
      )),
      (t[0] = n),
      (t[1] = a),
      (t[2] = o))
    : (o = t[2]);
  let s;
  return (
    t[3] !== r || t[4] !== o
      ? ((s = (0, $.jsx)(`div`, { className: o, children: r })), (t[3] = r), (t[4] = o), (t[5] = s))
      : (s = t[5]),
    s
  );
}
function un(e) {
  let t = (0, fn.c)(5),
    { label: n, onConfirm: r } = e,
    i;
  t[0] === n
    ? (i = t[1])
    : ((i =
        n ??
        (0, $.jsx)(L, {
          id: `codex.taskRowLayout.confirm`,
          defaultMessage: `Confirm`,
          description: `Confirmation button for archiving a task`,
        })),
      (t[0] = n),
      (t[1] = i));
  let a;
  return (
    t[2] !== r || t[3] !== i
      ? ((a = (0, $.jsx)(tt, {
          size: `composerSm`,
          color: `danger`,
          className: `!h-auto !border-none !py-0`,
          onClick: r,
          children: i,
        })),
        (t[2] = r),
        (t[3] = i),
        (t[4] = a))
      : (a = t[4]),
    a
  );
}
function dn(e) {
  let t = (0, fn.c)(11),
    { onArchive: n, archiveAriaLabel: r, useStableTrailingRail: i } = e,
    a;
  t[0] === i
    ? (a = t[1])
    : ((a = i ? { right: `var(--task-row-trailing-inset)` } : void 0), (t[0] = i), (t[1] = a));
  let o;
  t[2] === n
    ? (o = t[3])
    : ((o = (e) => {
        (e.stopPropagation(), n());
      }),
      (t[2] = n),
      (t[3] = o));
  let s;
  t[4] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((s = (0, $.jsx)(Kt, { className: `icon-xs` })), (t[4] = s))
    : (s = t[4]);
  let c;
  t[5] !== r || t[6] !== o
    ? ((c = (0, $.jsx)(`button`, {
        type: `button`,
        className: `focus-visible:outline-token-focus-ring pointer-events-none flex h-5 w-5 items-center justify-center rounded-md opacity-0 group-focus-within:pointer-events-auto group-focus-within:opacity-100 group-hover:pointer-events-auto group-hover:opacity-50 hover:opacity-100 focus-visible:pointer-events-auto focus-visible:opacity-100 focus-visible:outline-2 focus-visible:outline-offset-2`,
        onClick: o,
        "aria-label": r,
        children: s,
      })),
      (t[5] = r),
      (t[6] = o),
      (t[7] = c))
    : (c = t[7]);
  let l;
  return (
    t[8] !== a || t[9] !== c
      ? ((l = (0, $.jsx)(`div`, { className: gn, style: a, children: c })),
        (t[8] = a),
        (t[9] = c),
        (t[10] = l))
      : (l = t[10]),
    l
  );
}
var fn,
  pn,
  $,
  mn,
  hn,
  gn,
  _n,
  vn,
  yn,
  bn,
  xn,
  Sn = e(() => {
    ((fn = d()),
      ze(),
      (pn = t(i(), 1)),
      G(),
      $e(),
      P(),
      N(),
      qt(),
      ot(),
      Mt(),
      xt(),
      Xt(),
      ($ = C()),
      (mn = `icon-2xs`),
      (hn = `absolute right-0 top-0 z-10 flex h-full items-center justify-end gap-2 pr-0.5`),
      (gn = `${hn} mr-0.5 w-[52px]`),
      (_n = `absolute right-0 top-0 z-10 flex h-full min-w-[52px] items-center justify-end gap-2 pr-1`),
      (vn = 20),
      (yn = 8),
      (bn = 4),
      (xn = `[data-hover-card-open-immediately]`));
  });
function Cn(e) {
  let t = (0, wn.c)(16),
    { onActiveArchiveStart: n, onArchiveStart: r, onArchiveSuccess: i, onArchiveError: a } = e,
    [o, s] = (0, Tn.useState)(!1),
    c = r != null || i != null || a != null,
    l;
  t[0] !== n || t[1] !== r || t[2] !== c
    ? ((l = () => {
        (c ? r?.() : s(!0), n?.());
      }),
      (t[0] = n),
      (t[1] = r),
      (t[2] = c),
      (t[3] = l))
    : (l = t[3]);
  let u = l,
    d;
  t[4] === i
    ? (d = t[5])
    : ((d = () => {
        i?.();
      }),
      (t[4] = i),
      (t[5] = d));
  let f = d,
    p;
  t[6] !== a || t[7] !== c
    ? ((p = () => {
        c ? a?.() : s(!1);
      }),
      (t[6] = a),
      (t[7] = c),
      (t[8] = p))
    : (p = t[8]);
  let m = p,
    h = m,
    g;
  return (
    t[9] !== o || t[10] !== u || t[11] !== h || t[12] !== f || t[13] !== m || t[14] !== c
      ? ((g = {
          archived: o,
          usesExternalArchiveHandling: c,
          beginArchive: u,
          handleArchiveSuccess: f,
          handleArchiveError: h,
          restoreArchivedRow: m,
        }),
        (t[9] = o),
        (t[10] = u),
        (t[11] = h),
        (t[12] = f),
        (t[13] = m),
        (t[14] = c),
        (t[15] = g))
      : (g = t[15]),
    g
  );
}
var wn,
  Tn,
  En = e(() => {
    ((wn = d()), (Tn = t(i(), 1)));
  });
function Dn(e) {
  let t = (0, On.c)(77),
    {
      task: n,
      onClose: r,
      onSelect: i,
      isActive: a,
      envIconLocation: o,
      useStableTrailingRail: s,
      statusIndicatorReplacesMeta: c,
      hideStatusIndicator: u,
      idleIndicatorNode: d,
      indicatorRestNode: f,
      indicatorHoverNode: p,
      reserveLeadingSlot: m,
      additionalHoverActionCount: g,
      renderActions: _,
      hideInlineBadgesOnHover: v,
      metaHidden: y,
      metaContent: b,
      overlayMetaContent: x,
      extraIconBadges: te,
      floatStatusIconsRight: C,
      hoverCardProjectLabel: ne,
      hoverCardProjectIcon: re,
      hoverCardRepositoryLabel: ie,
      hoverCardSections: ae,
      contextMenuItems: oe,
      variant: se,
      onArchiveStart: ce,
      onArchiveSuccess: le,
      onArchiveError: ue,
      dataAttributes: de,
    } = e,
    w = a === void 0 ? !1 : a,
    T = o === void 0 ? `end` : o,
    fe = s === void 0 ? !1 : s,
    E = c === void 0 ? !1 : c,
    D = u === void 0 ? !1 : u,
    pe = m === void 0 ? !1 : m,
    me = g === void 0 ? 0 : g,
    ge = v === void 0 ? !1 : v,
    _e = y === void 0 ? !1 : y,
    ve = C === void 0 ? !1 : C,
    ye = ee(l),
    O = Be(),
    k = De(),
    be = h(),
    A,
    j,
    M;
  if (t[0] !== k || t[1] !== r || t[2] !== i || t[3] !== n) {
    j = () => {
      (i?.(), r(), k(`/remote/${A}`));
    };
    let { id: e, title: a } = n;
    ((M = a),
      (A = e),
      (t[0] = k),
      (t[1] = r),
      (t[2] = i),
      (t[3] = n),
      (t[4] = A),
      (t[5] = j),
      (t[6] = M));
  } else ((A = t[4]), (j = t[5]), (M = t[6]));
  let xe = n.task_status_display?.latest_turn_status_display?.turn_status,
    N = xe === `in_progress` || xe === `pending`,
    P = xe === `failed`,
    Ce = n.updated_at ?? n.created_at ?? null,
    we = Ce ? new Date(Ce * 1e3) : null,
    F = N ? `loading` : P ? `error` : `idle`,
    Te = b ?? (we ? (0, kn.jsx)(he, { dateString: we.toISOString() }) : null),
    Ee = we ? `default` : N ? `loading` : `default`,
    I = !N && !!we,
    Oe;
  t[7] !== ue || t[8] !== ce || t[9] !== le
    ? ((Oe = { onArchiveStart: ce, onArchiveSuccess: le, onArchiveError: ue }),
      (t[7] = ue),
      (t[8] = ce),
      (t[9] = le),
      (t[10] = Oe))
    : (Oe = t[10]);
  let {
      archived: ke,
      usesExternalArchiveHandling: Ae,
      beginArchive: je,
      handleArchiveSuccess: Me,
      handleArchiveError: Ne,
      restoreArchivedRow: Pe,
    } = Cn(Oe),
    Fe;
  t[11] === A
    ? (Fe = t[12])
    : ((Fe = () =>
        Se.safePost(`/wham/tasks/{task_id}/archive`, { parameters: { path: { task_id: A } } })),
      (t[11] = A),
      (t[12] = Fe));
  let R;
  t[13] !== Me ||
  t[14] !== A ||
  t[15] !== O ||
  t[16] !== k ||
  t[17] !== be ||
  t[18] !== Pe ||
  t[19] !== ye
    ? ((R = () => {
        Me();
        let e = ye.get(rt).info(
          (0, kn.jsx)(St, {
            onUndo: () => {
              (e.close(),
                Se.safePost(`/wham/tasks/{task_id}/recover`, {
                  parameters: { path: { task_id: A } },
                })
                  .then(() => {
                    (Pe(),
                      be.invalidateQueries({ queryKey: [`tasks`] }),
                      be.invalidateQueries({ queryKey: [`task`, A] }));
                  })
                  .catch(() => {
                    ye.get(rt).danger(
                      O.formatMessage({
                        id: `cloudTaskRow.unarchiveError`,
                        defaultMessage: `Failed to unarchive task`,
                        description: `Error message when undoing a cloud task archive`,
                      }),
                    );
                  }));
            },
            onOpenSettings: () => {
              k(`/settings/data-controls`);
            },
          }),
          { id: `archive-task` },
        );
        (be.invalidateQueries({ queryKey: [`tasks`] }),
          be.invalidateQueries({ queryKey: [`task`, A] }));
      }),
      (t[13] = Me),
      (t[14] = A),
      (t[15] = O),
      (t[16] = k),
      (t[17] = be),
      (t[18] = Pe),
      (t[19] = ye),
      (t[20] = R))
    : (R = t[20]);
  let z;
  t[21] !== Ne || t[22] !== O || t[23] !== ye
    ? ((z = () => {
        (Ne(),
          ye
            .get(rt)
            .danger(
              O.formatMessage({
                id: `cloudTaskRow.archiveError`,
                defaultMessage: `Failed to archive task`,
                description: `Error message when archiving a Codex task`,
              }),
            ));
      }),
      (t[21] = Ne),
      (t[22] = O),
      (t[23] = ye),
      (t[24] = z))
    : (z = t[24]);
  let B;
  t[25] !== Fe || t[26] !== R || t[27] !== z
    ? ((B = { mutationFn: Fe, onSuccess: R, onError: z }),
      (t[25] = Fe),
      (t[26] = R),
      (t[27] = z),
      (t[28] = B))
    : (B = t[28]);
  let Le = S(B),
    V;
  t[29] !== Le || t[30] !== je
    ? ((V = () => {
        (je(), Le.mutate());
      }),
      (t[29] = Le),
      (t[30] = je),
      (t[31] = V))
    : (V = t[31]);
  let Re = V;
  if (ke && !Ae) return null;
  let ze;
  t[32] === M
    ? (ze = t[33])
    : ((ze =
        Ie(M ?? ``) ||
        (0, kn.jsx)(L, {
          id: `codex.taskRow.title`,
          defaultMessage: `New chat`,
          description: `Default title for a Codex thread that doesn't have a title`,
        })),
      (t[32] = M),
      (t[33] = ze));
  let H = ze,
    U =
      ne != null && ne !== ``
        ? (0, kn.jsx)(It, {
            projectLabel: ne,
            projectIcon: re,
            repositoryLabel: ie,
            sections: [
              {
                id: `environment`,
                rows: [
                  {
                    id: `environment`,
                    icon: (0, kn.jsx)(kt, { envType: `cloud`, disableTooltip: !0 }),
                    label: (0, kn.jsx)(Ot, { envType: `cloud` }),
                  },
                ],
              },
              ...(ae ?? []),
            ],
            showUnreadDot: n.has_unread_turn,
            timestampDateString: we?.toISOString(),
            threadTitle: H,
          })
        : null,
    W;
  t[34] !== F || t[35] !== n.has_unread_turn
    ? ((W = { type: F, unread: n.has_unread_turn }),
      (t[34] = F),
      (t[35] = n.has_unread_turn),
      (t[36] = W))
    : (W = t[36]);
  let G = (E || N) && b == null,
    Ve = I ? Re : null,
    He;
  t[37] === O ? (He = t[38]) : ((He = O.formatMessage(An.archiveTask)), (t[37] = O), (t[38] = He));
  let K;
  t[39] === Symbol.for(`react.memo_cache_sentinel`)
    ? ((K = (0, kn.jsx)(L, {
        id: `codex.cloudTaskRow.confirmArchiveTask`,
        defaultMessage: `Confirm`,
        description: `Confirmation button for archiving a cloud task`,
      })),
      (t[39] = K))
    : (K = t[39]);
  let q = se === `tableRow` ? `bottom` : void 0,
    Ue = se === `tableRow` ? `end` : void 0,
    We = se === `tableRow` ? -36 : void 0,
    J;
  t[40] !== me ||
  t[41] !== de ||
  t[42] !== T ||
  t[43] !== te ||
  t[44] !== ve ||
  t[45] !== ge ||
  t[46] !== D ||
  t[47] !== U ||
  t[48] !== d ||
  t[49] !== p ||
  t[50] !== f ||
  t[51] !== w ||
  t[52] !== _e ||
  t[53] !== Ee ||
  t[54] !== j ||
  t[55] !== x ||
  t[56] !== _ ||
  t[57] !== pe ||
  t[58] !== W ||
  t[59] !== G ||
  t[60] !== Ve ||
  t[61] !== He ||
  t[62] !== q ||
  t[63] !== Ue ||
  t[64] !== We ||
  t[65] !== Te ||
  t[66] !== H ||
  t[67] !== fe ||
  t[68] !== se
    ? ((J = (0, kn.jsx)(Zt, {
        isActive: w,
        onClick: j,
        idleIndicatorNode: d,
        indicatorRestNode: f,
        indicatorHoverNode: p,
        reserveLeadingSlot: pe,
        additionalHoverActionCount: me,
        variant: se,
        statusState: W,
        statusIndicatorReplacesMeta: G,
        hideStatusIndicator: D,
        useStableTrailingRail: fe,
        envType: `cloud`,
        envIconLocation: T,
        title: H,
        hoverCardContent: U,
        iconBadges: te,
        floatStatusIconsRight: ve,
        hideInlineBadgesOnHover: ge,
        metaHidden: _e,
        metaContent: Te,
        overlayMetaContent: x,
        metaState: Ee,
        onArchive: Ve,
        archiveAriaLabel: He,
        archiveConfirmLabel: K,
        renderActions: _,
        hoverCardSide: q,
        hoverCardAlign: Ue,
        hoverCardSideOffset: We,
        dataAttributes: de,
      })),
      (t[40] = me),
      (t[41] = de),
      (t[42] = T),
      (t[43] = te),
      (t[44] = ve),
      (t[45] = ge),
      (t[46] = D),
      (t[47] = U),
      (t[48] = d),
      (t[49] = p),
      (t[50] = f),
      (t[51] = w),
      (t[52] = _e),
      (t[53] = Ee),
      (t[54] = j),
      (t[55] = x),
      (t[56] = _),
      (t[57] = pe),
      (t[58] = W),
      (t[59] = G),
      (t[60] = Ve),
      (t[61] = He),
      (t[62] = q),
      (t[63] = Ue),
      (t[64] = We),
      (t[65] = Te),
      (t[66] = H),
      (t[67] = fe),
      (t[68] = se),
      (t[69] = J))
    : (J = t[69]);
  let Y = J;
  if (oe === void 0 || (oe.length === 0 && !I)) return Y;
  let Ge;
  t[70] !== I || t[71] !== oe || t[72] !== Re
    ? ((Ge = () => {
        let e = [...oe];
        return (I && e.push({ id: `archive-task`, message: An.archiveTask, onSelect: Re }), e);
      }),
      (t[70] = I),
      (t[71] = oe),
      (t[72] = Re),
      (t[73] = Ge))
    : (Ge = t[73]);
  let Ke = Ge,
    qe;
  return (
    t[74] !== Ke || t[75] !== Y
      ? ((qe = (0, kn.jsx)(Ze, { getItems: Ke, children: Y })),
        (t[74] = Ke),
        (t[75] = Y),
        (t[76] = qe))
      : (qe = t[76]),
    qe
  );
}
var On,
  kn,
  An,
  jn = e(() => {
    ((On = d()),
      g(),
      p(),
      G(),
      ke(),
      Tt(),
      et(),
      de(),
      Qe(),
      H(),
      ie(),
      F(),
      Mt(),
      Wt(),
      Sn(),
      En(),
      (kn = C()),
      (An = U({
        archiveTask: {
          id: `codex.cloudTaskRow.archiveTask`,
          defaultMessage: `Archive task`,
          description: `Label for archiving a cloud task`,
        },
      })));
  });
function Mn({ codexHome: e, worktreePath: t }) {
  let n = Nn(t, e);
  return n == null ? b(t) : (n[Pn(n) - 1] ?? b(t));
}
function Nn(e, t) {
  let n = v(e).replace(/\/+$/, ``),
    r = n.toLowerCase();
  if (t != null) {
    let e = v(c(t)).replace(/\/+$/, ``).toLowerCase();
    if (r === e || r.startsWith(`${e}/`)) return n.slice(e.length).split(`/`).filter(Boolean);
  }
  let i = r.lastIndexOf(Fn);
  return i === -1
    ? null
    : n
        .slice(i + 17)
        .split(`/`)
        .filter(Boolean);
}
function Pn(e) {
  return e.length >= 2 && /^[0-9a-f]{4,}$/i.test(e[0]) ? 2 : 1;
}
var Fn,
  In = e(() => {
    (r(), x(), (Fn = `.codex/worktrees/`));
  });
function Ln({
  branchMismatchWarning: e,
  branchName: t,
  branchTooltipContent: n,
  codexHome: r,
  workspaceIcon: i,
  workspacePath: a,
}) {
  let o = t?.trim() || null,
    s = a != null && re(a, r) ? Mn({ codexHome: r, worktreePath: a }) : null,
    c = [];
  return (
    o != null &&
      c.push({
        id: `branch`,
        icon: (0, Rn.jsx)(Ke, {}),
        label:
          n == null
            ? o
            : (0, Rn.jsx)(A, { tooltipContent: n, children: (0, Rn.jsx)(`span`, { children: o }) }),
      }),
    e != null &&
      c.push({
        allowWrap: !0,
        id: `branch-mismatch`,
        icon: (0, Rn.jsx)(k, { className: `text-token-editor-warning-foreground` }),
        label: e,
        tone: `danger`,
      }),
    s != null && c.push({ id: `worktree`, icon: i ?? (0, Rn.jsx)(w, {}), label: s }),
    c.length > 0 ? c : void 0
  );
}
var Rn,
  zn = e(() => {
    (r(), P(), Ge(), T(), ce(), In(), (Rn = C()));
  });
function Bn(e) {
  let t = (0, Vn.c)(55),
    {
      task: n,
      envIconLocation: r,
      useStableTrailingRail: i,
      statusIndicatorReplacesMeta: a,
      hideStatusIndicator: o,
      isActive: s,
      hasAttention: c,
      indicatorRestNode: l,
      indicatorHoverNode: u,
      reserveLeadingSlot: d,
      additionalHoverActionCount: f,
      renderActions: p,
      variant: m,
      hoverCardProjectLabel: h,
      floatStatusIconsRight: g,
      metaContent: _,
      overlayMetaContent: v,
      onClick: y,
      onDoubleClick: b,
      onArchive: x,
      onContextMenu: ee,
      dataAttributes: S,
    } = e,
    te = r === void 0 ? `end` : r,
    C = i === void 0 ? !1 : i,
    ne = a === void 0 ? !1 : a,
    re = o === void 0 ? !1 : o,
    ie = s === void 0 ? !1 : s,
    ae = c === void 0 ? !1 : c,
    oe = d === void 0 ? !1 : d,
    se = f === void 0 ? 0 : f,
    ce = g === void 0 ? !1 : g,
    le = Be(),
    ue = De(),
    de = Je(n.hostId),
    w = n.phase === `failed`,
    T;
  t[0] === n.phase ? (T = t[1]) : ((T = me(n.phase)), (t[0] = n.phase), (t[1] = T));
  let fe = T,
    E;
  t[2] !== _ || t[3] !== n.createdAt
    ? ((E = _ ?? (0, Hn.jsx)(he, { dateString: new Date(n.createdAt).toISOString() })),
      (t[2] = _),
      (t[3] = n.createdAt),
      (t[4] = E))
    : (E = t[4]);
  let D = E,
    pe;
  t[5] !== ae || t[6] !== w || t[7] !== fe
    ? ((pe = fe
        ? { type: `loading`, unread: ae }
        : w
          ? { type: `error`, unread: ae }
          : { type: `idle`, unread: ae }),
      (t[5] = ae),
      (t[6] = w),
      (t[7] = fe),
      (t[8] = pe))
    : (pe = t[8]);
  let ge = pe,
    _e = n.startingState.type === `branch` ? n.startingState.branchName : null,
    ve = n.worktreeGitRoot ?? n.worktreeWorkspaceRoot,
    O;
  t[9] !== de || t[10] !== _e || t[11] !== ve
    ? ((O = Ln({ branchName: _e, codexHome: de, workspacePath: ve })),
      (t[9] = de),
      (t[10] = _e),
      (t[11] = ve),
      (t[12] = O))
    : (O = t[12]);
  let k = O,
    be;
  t[13] !== w || t[14] !== n.label
    ? ((be = w
        ? (0, Hn.jsx)(L, {
            id: `recentTasks.worktreeInitFailedTitle`,
            defaultMessage: `Worktree init failed`,
            description: `Worktree row title when the init script fails`,
          })
        : Ie(n.label)),
      (t[13] = w),
      (t[14] = n.label),
      (t[15] = be))
    : (be = t[15]);
  let A = be,
    j;
  t[16] !== k || t[17] !== h || t[18] !== A
    ? ((j =
        h != null && h !== ``
          ? (0, Hn.jsx)(It, {
              projectLabel: h,
              sections: [{ id: `environment`, rows: k ?? [] }],
              threadTitle: A,
            })
          : null),
      (t[16] = k),
      (t[17] = h),
      (t[18] = A),
      (t[19] = j))
    : (j = t[19]);
  let M = j,
    xe;
  t[20] !== ue || t[21] !== y || t[22] !== n.id
    ? ((xe = () => {
        if (y) {
          y();
          return;
        }
        ue(`/worktree-init-v2/${n.id}`);
      }),
      (t[20] = ue),
      (t[21] = y),
      (t[22] = n.id),
      (t[23] = xe))
    : (xe = t[23]);
  let N = xe,
    P = ne && _ == null,
    Se;
  t[24] !== le || t[25] !== x
    ? ((Se = x ? le.formatMessage(ye.archiveThread) : void 0),
      (t[24] = le),
      (t[25] = x),
      (t[26] = Se))
    : (Se = t[26]);
  let Ce;
  t[27] === x
    ? (Ce = t[28])
    : ((Ce = x
        ? (0, Hn.jsx)(L, {
            id: `codex.localTaskRow.confirmArchiveTask`,
            defaultMessage: `Confirm`,
            description: `Confirmation button for archiving a local task`,
          })
        : void 0),
      (t[27] = x),
      (t[28] = Ce));
  let we;
  return (
    t[29] !== se ||
    t[30] !== S ||
    t[31] !== te ||
    t[32] !== ce ||
    t[33] !== N ||
    t[34] !== re ||
    t[35] !== M ||
    t[36] !== u ||
    t[37] !== l ||
    t[38] !== ie ||
    t[39] !== fe ||
    t[40] !== x ||
    t[41] !== ee ||
    t[42] !== b ||
    t[43] !== v ||
    t[44] !== p ||
    t[45] !== oe ||
    t[46] !== ge ||
    t[47] !== P ||
    t[48] !== Se ||
    t[49] !== Ce ||
    t[50] !== D ||
    t[51] !== A ||
    t[52] !== C ||
    t[53] !== m
      ? ((we = (0, Hn.jsx)(Zt, {
          isActive: ie,
          title: A,
          hoverCardContent: M,
          metaContent: D,
          overlayMetaContent: v,
          envType: `worktree`,
          envIconLocation: te,
          floatStatusIconsRight: ce,
          pulseEnvIcon: fe,
          statusIndicatorReplacesMeta: P,
          hideStatusIndicator: re,
          useStableTrailingRail: C,
          indicatorRestNode: l,
          indicatorHoverNode: u,
          reserveLeadingSlot: oe,
          additionalHoverActionCount: se,
          variant: m,
          renderActions: p,
          statusState: ge,
          onArchive: x,
          archiveAriaLabel: Se,
          archiveConfirmLabel: Ce,
          onClick: N,
          onDoubleClick: b,
          onContextMenu: ee,
          dataAttributes: S,
        })),
        (t[29] = se),
        (t[30] = S),
        (t[31] = te),
        (t[32] = ce),
        (t[33] = N),
        (t[34] = re),
        (t[35] = M),
        (t[36] = u),
        (t[37] = l),
        (t[38] = ie),
        (t[39] = fe),
        (t[40] = x),
        (t[41] = ee),
        (t[42] = b),
        (t[43] = v),
        (t[44] = p),
        (t[45] = oe),
        (t[46] = ge),
        (t[47] = P),
        (t[48] = Se),
        (t[49] = Ce),
        (t[50] = D),
        (t[51] = A),
        (t[52] = C),
        (t[53] = m),
        (t[54] = we))
      : (we = t[54]),
    we
  );
}
var Vn,
  Hn,
  Un = e(() => {
    ((Vn = d()), G(), ke(), de(), qe(), H(), _e(), ve(), Wt(), Sn(), zn(), (Hn = C()));
  });
function Wn(e) {
  let t = (0, Gn.c)(3),
    { data: n } = ne(o.PROJECT_APPEARANCES),
    r;
  return (
    t[0] !== n || t[1] !== e
      ? ((r = u(n)[e] ?? null), (t[0] = n), (t[1] = e), (t[2] = r))
      : (r = t[2]),
    r
  );
}
var Gn,
  Kn = e(() => {
    ((Gn = d()), r(), te());
  }),
  qn,
  Jn = e(() => {
    qn = 6e4;
  });
function Yn({ codexHome: e, cwd: t, hideRemoteHostEnvIcon: n, hostId: r, isGrouped: i }) {
  let a = !n && r != null && r !== `local`,
    o = re(t, e);
  return a ? (o ? `remote-worktree` : `remote`) : o ? `worktree` : i ? `localGrouped` : void 0;
}
var Xn = e(() => {
  (r(), f());
});
function Zn(e) {
  let t = (0, $n.c)(48),
    {
      conversationId: n,
      displayCwd: r,
      hasAttachedHeartbeatAutomation: i,
      heartbeatAutomationTooltipContent: s,
      hideProjectMetadata: c,
      hideRemoteHostEnvIcon: l,
      hoverCardSections: u,
      hoverCardHostConfig: d,
      hoverCardProjectId: f,
      hoverCardProjectLabel: p,
      hoverCardRepositoryLabel: h,
      hasUnreadTurn: g,
      isAutomationRun: v,
      isGrouped: y,
      isProjectlessHoverCard: b,
      projectFallbackIcon: x,
      shouldFetchHoverBranch: ee,
      threadSummary: S,
      threadTitle: te,
    } = e,
    C = i === void 0 ? !1 : i,
    re = c === void 0 ? !1 : c,
    ie = l === void 0 ? !1 : l,
    ae = g === void 0 ? !1 : g,
    ce = v === void 0 ? !1 : v,
    le = y === void 0 ? !1 : y,
    ue = b === void 0 ? !1 : b,
    de = ee === void 0 ? !1 : ee,
    w = S === void 0 ? null : S,
    T = Be(),
    E = _(I, n),
    D = r ?? E ?? w?.cwd ?? null,
    pe = _(oe, n) ?? w?.title,
    me = _(Ce, n) ?? w?.createdAt,
    he = _(je, n) ?? w?.recencyAt ?? w?.updatedAt ?? me,
    _e = _(we, n) ?? w?.gitInfo?.branch,
    ve = _(Te, n) ?? w?.hostId ?? null,
    ye = a(ve ?? `local`),
    O = d ?? ye,
    k = d?.id ?? ve,
    be = Je(k ?? `local`),
    A = _(z, n) ?? w?.workspaceKind,
    j = ue || A === `projectless`,
    { data: M } = ne(o.THREAD_PROJECT_ASSIGNMENTS),
    xe;
  t[0] !== n || t[1] !== M
    ? ((xe = m(M)[n] ?? null), (t[0] = n), (t[1] = M), (t[2] = xe))
    : (xe = t[2]);
  let N = re || ue ? null : (f ?? (j ? null : (xe?.projectId ?? D))),
    P = Wn(N ?? ``),
    { renameThread: Se } = ge(),
    F = Yn({ codexHome: be, cwd: D, hideRemoteHostEnvIcon: ie, hostId: k, isGrouped: le }),
    Ee =
      !j &&
      D != null &&
      (F == null || F === `worktree` || F === `remote-worktree` || F === `localGrouped`),
    De;
  t[3] === T
    ? (De = t[4])
    : ((De = T.formatMessage({
        id: `sidebarElectron.threadHoverCardChatProject`,
        defaultMessage: `Chat`,
        description: `Fallback project label for a sidebar thread hover card when it does not belong to a project or workspace`,
      })),
      (t[3] = T),
      (t[4] = De));
  let Oe = De,
    ke;
  t[5] !== D || t[6] !== re || t[7] !== Oe || t[8] !== p || t[9] !== j
    ? ((ke = re ? Oe : (lt(null, p) ?? (j ? Oe : (lt(D) ?? D ?? Oe)))),
      (t[5] = D),
      (t[6] = re),
      (t[7] = Oe),
      (t[8] = p),
      (t[9] = j),
      (t[10] = ke))
    : (ke = t[10]);
  let Ae = ke,
    L = Ee && de,
    Me;
  t[11] === L ? (Me = t[12]) : ((Me = { enabled: L, staleTime: 0 }), (t[11] = L), (t[12] = Me));
  let Ne = fe(D, O, `local_thread_hover_card`, Me),
    Pe =
      F === `remote` || F === `remote-worktree`
        ? O.id === k
          ? O.display_name
          : null
        : (D ?? null),
    Fe =
      _e != null && Ne.data != null && _e !== Ne.data
        ? T.formatMessage({
            id: `codex.localTaskRow.hoverCardBranchMismatch`,
            defaultMessage: `Chat branch is based on what branch was active when last worked on this chat`,
            description: `Tooltip shown when the checked out branch differs from the chat branch in the task hover card`,
          })
        : void 0,
    R =
      Fe == null
        ? void 0
        : T.formatMessage({
            id: `codex.localTaskRow.hoverCardBranchMismatchWarning`,
            defaultMessage: `Chat branch reflects active branch when last used; sending a message will update chat branch`,
            description: `Warning text shown when the checked out branch differs from the chat branch in the task hover card`,
          }),
    Ie = Ee
      ? Ln({
          branchMismatchWarning: R,
          branchName: _e ?? Ne.data ?? null,
          branchTooltipContent: Fe,
          codexHome: be,
          workspaceIcon: F === `worktree` || F === `remote-worktree` ? void 0 : (0, er.jsx)(He, {}),
          workspacePath: D,
        })
      : void 0,
    B;
  t[13] !== C || t[14] !== s || t[15] !== T || t[16] !== ce
    ? ((B =
        ce || C
          ? [
              {
                id: `automation`,
                icon: (0, er.jsx)(se, { className: `shrink-0` }),
                label:
                  s ??
                  T.formatMessage({
                    id: `codex.localTaskRow.automation`,
                    defaultMessage: `Scheduled task chat`,
                    description: `Accessible label for the scheduled task thread icon`,
                  }),
              },
            ]
          : []),
      (t[13] = C),
      (t[14] = s),
      (t[15] = T),
      (t[16] = ce),
      (t[17] = B))
    : (B = t[17]);
  let Le = B,
    V =
      (F === `remote` || F === `remote-worktree`) && O.id === k
        ? [
            {
              id: `remote-host`,
              icon: (0, er.jsx)(kt, { envType: `remote`, hostId: k ?? void 0, disableTooltip: !0 }),
              label: O.display_name,
            },
          ]
        : [],
    Re =
      V.length > 0
        ? [...V, ...(Ie ?? [])]
        : (Ie ??
          (Et(F)
            ? [
                {
                  id: `environment`,
                  icon: (0, er.jsx)(kt, { envType: F, hostId: k ?? void 0, disableTooltip: !0 }),
                  label: Pe ?? (0, er.jsx)(Ot, { envType: F }),
                },
              ]
            : [])),
    ze;
  t[18] === Re
    ? (ze = t[19])
    : ((ze = { id: `environment`, rows: Re }), (t[18] = Re), (t[19] = ze));
  let H;
  t[20] === u ? (H = t[21]) : ((H = u ?? []), (t[20] = u), (t[21] = H));
  let U;
  t[22] === Le ? (U = t[23]) : ((U = { id: `automation`, rows: Le }), (t[22] = Le), (t[23] = U));
  let W;
  t[24] !== ze || t[25] !== H || t[26] !== U
    ? ((W = [ze, ...H, U].filter(Qn)), (t[24] = ze), (t[25] = H), (t[26] = U), (t[27] = W))
    : (W = t[27]);
  let G = W,
    Ve;
  t[28] !== P || t[29] !== x || t[30] !== N
    ? ((Ve = N == null ? (0, er.jsx)(ct, {}) : P == null ? x : (0, er.jsx)(K, { appearance: P })),
      (t[28] = P),
      (t[29] = x),
      (t[30] = N),
      (t[31] = Ve))
    : (Ve = t[31]);
  let q = ue ? null : h,
    Ue;
  t[32] === he
    ? (Ue = t[33])
    : ((Ue = he == null ? void 0 : new Date(he).toISOString()), (t[32] = he), (t[33] = Ue));
  let We = pe ?? void 0,
    J;
  t[34] !== n || t[35] !== k || t[36] !== Se
    ? ((J = (e) => {
        Se({ conversationId: n, hostId: k ?? void 0, title: e });
      }),
      (t[34] = n),
      (t[35] = k),
      (t[36] = Se),
      (t[37] = J))
    : (J = t[37]);
  let Y;
  return (
    t[38] !== ae ||
    t[39] !== Ae ||
    t[40] !== G ||
    t[41] !== Ve ||
    t[42] !== q ||
    t[43] !== Ue ||
    t[44] !== We ||
    t[45] !== J ||
    t[46] !== te
      ? ((Y = (0, er.jsx)(It, {
          projectLabel: Ae,
          projectIcon: Ve,
          repositoryLabel: q,
          sections: G,
          showUnreadDot: ae,
          timestampDateString: Ue,
          threadTitle: te,
          threadTitleValue: We,
          onRenameThreadTitle: J,
        })),
        (t[38] = ae),
        (t[39] = Ae),
        (t[40] = G),
        (t[41] = Ve),
        (t[42] = q),
        (t[43] = Ue),
        (t[44] = We),
        (t[45] = J),
        (t[46] = te),
        (t[47] = Y))
      : (Y = t[47]),
    Y
  );
}
function Qn(e) {
  return e.rows.length > 0;
}
var $n,
  er,
  tr = e(() => {
    (($n = d()),
      p(),
      r(),
      G(),
      Pe(),
      P(),
      be(),
      qe(),
      te(),
      st(),
      ae(),
      Ye(),
      O(),
      _e(),
      Ue(),
      Kn(),
      f(),
      ut(),
      Xn(),
      Mt(),
      Dt(),
      Wt(),
      zn(),
      (er = C()));
  });
function nr(e) {
  return (0, rr.jsx)(M, { browser: !0, electron: !0, children: (0, rr.jsx)(Zn, { ...e }) });
}
var rr,
  ir = e(() => {
    (j(), tr(), (rr = C()));
  });
function ar(e) {
  let t = (0, sr.c)(134),
    {
      conversationId: r,
      displayCwd: i,
      onSelect: a,
      onClick: o,
      onBeforeNavigate: c,
      onDoubleClick: u,
      isActive: d,
      isGrouped: f,
      envIconLocation: p,
      useStableTrailingRail: m,
      statusIndicatorReplacesMeta: h,
      hideStatusIndicator: g,
      hideInlineBadgesOnHover: v,
      idleIndicatorNode: y,
      idleStatusIndicatorNode: b,
      indicatorRestNode: x,
      indicatorHoverNode: S,
      reserveLeadingSlot: te,
      additionalHoverActionCount: C,
      priorityIndicatorNode: ne,
      onContextMenu: re,
      renderActions: ie,
      metaHidden: ae,
      extraIconBadges: ce,
      floatStatusIconsRight: le,
      onArchiveStart: de,
      onArchiveSuccess: w,
      onArchiveError: T,
      onActiveArchiveStart: fe,
      variant: E,
      titleOverride: pe,
      titleSuffix: me,
      hoverCardSections: he,
      hoverCardHostConfig: ge,
      disableHoverCard: _e,
      forceLoadingIndicator: ve,
      attentionIndicatorOnly: ye,
      blurOnClick: O,
      hasPendingChildApproval: k,
      hideRemoteHostEnvIcon: be,
      disableEnvTooltip: A,
      disabled: j,
      allowActionsWhenDisabled: M,
      isAutomationRun: xe,
      isProjectlessHoverCard: N,
      isUnread: P,
      hasAttachedHeartbeatAutomation: Se,
      heartbeatAutomationTooltipContent: we,
      metaContent: F,
      overlayMetaContent: De,
      hoverCardProjectFallbackIcon: ke,
      hoverCardProjectId: je,
      hoverCardProjectLabel: Ne,
      hoverCardRepositoryLabel: Pe,
      threadSummary: R,
      dataAttributes: Ie,
    } = e,
    z = d === void 0 ? !1 : d,
    B = f === void 0 ? !1 : f,
    V = m === void 0 ? !1 : m,
    ze = h === void 0 ? !1 : h,
    H = g === void 0 ? !1 : g,
    U = v === void 0 ? !1 : v,
    G = te === void 0 ? !1 : te,
    He = C === void 0 ? 0 : C,
    K = ae === void 0 ? !1 : ae,
    q = le === void 0 ? !1 : le,
    Ue = _e === void 0 ? !1 : _e,
    Y = ve === void 0 ? !1 : ve,
    Ge = ye === void 0 ? !1 : ye,
    Ke = O === void 0 ? !1 : O,
    qe = k === void 0 ? !1 : k,
    Ye = be === void 0 ? !1 : be,
    Xe = j === void 0 ? !1 : j,
    Ze = M === void 0 ? !1 : M,
    Qe = xe === void 0 ? !1 : xe,
    $e = N === void 0 ? !1 : N,
    et = Se === void 0 ? !1 : Se,
    tt = R === void 0 ? null : R,
    nt = ee(l),
    it = at(),
    X = Be(),
    ot = _(Ee, r),
    st = _(Ce, r) ?? tt?.createdAt,
    ct = _(I, r),
    lt = i ?? ct ?? tt?.cwd ?? null,
    ut = _(Ve, r) ?? tt?.hasUnreadTurn,
    dt = _(Me, r),
    Z = _(Te, r) ?? tt?.hostId ?? null;
  _(W, r) ?? tt?.modelProvider;
  let ft = Je(Z ?? `local`),
    pt = _(We, r),
    mt = _(Ae, r),
    ht = Z ?? `local`,
    gt;
  t[0] !== r || t[1] !== ht
    ? ((gt = { conversationId: r, hostId: ht }), (t[0] = r), (t[1] = ht), (t[2] = gt))
    : (gt = t[2]);
  let _t = _(ue, gt),
    vt = _(J, r),
    yt = _(Oe, r) ?? tt?.source,
    bt = _(oe, r) ?? tt?.title ?? null,
    xt = z ? fe : void 0,
    Ct;
  t[3] !== T || t[4] !== de || t[5] !== w || t[6] !== xt
    ? ((Ct = {
        onActiveArchiveStart: xt,
        onArchiveStart: de,
        onArchiveSuccess: w,
        onArchiveError: T,
      }),
      (t[3] = T),
      (t[4] = de),
      (t[5] = w),
      (t[6] = xt),
      (t[7] = Ct))
    : (Ct = t[7]);
  let {
      archived: wt,
      usesExternalArchiveHandling: Tt,
      beginArchive: Et,
      handleArchiveSuccess: Dt,
      handleArchiveError: Ot,
      restoreArchivedRow: kt,
    } = Cn(Ct),
    At = D(),
    jt;
  t[8] === yt ? (jt = t[9]) : ((jt = Le(yt)), (t[8] = yt), (t[9] = jt));
  let Mt = At && jt?.parentThreadId != null,
    Nt = vt;
  (pt === `response` || (Ge && Nt === `loading`)) && (Nt = `idle`);
  let Pt;
  t[10] !== bt || t[11] !== pe
    ? ((Pt = (0, lr.jsx)(or, { title: bt, titleOverride: pe })),
      (t[10] = bt),
      (t[11] = pe),
      (t[12] = Pt))
    : (Pt = t[12]);
  let Ft = Pt,
    [It, Lt] = (0, cr.useState)(!1),
    Rt;
  t[13] !== Ge ||
  t[14] !== Y ||
  t[15] !== qe ||
  t[16] !== ut ||
  t[17] !== Mt ||
  t[18] !== P ||
  t[19] !== pt ||
  t[20] !== Nt ||
  t[21] !== dt
    ? ((Rt = Y
        ? { type: `loading` }
        : {
            type: Nt,
            unread: Mt ? !1 : (P ?? ut === !0) || (Ge && ((dt ?? 0) > 0 || pt != null || qe)),
            unreadCount: Mt || Ge ? 0 : (dt ?? 0),
          }),
      (t[13] = Ge),
      (t[14] = Y),
      (t[15] = qe),
      (t[16] = ut),
      (t[17] = Mt),
      (t[18] = P),
      (t[19] = pt),
      (t[20] = Nt),
      (t[21] = dt),
      (t[22] = Rt))
    : (Rt = t[22]);
  let zt = Rt,
    Bt;
  t[23] !== Et ||
  t[24] !== r ||
  t[25] !== Ot ||
  t[26] !== Dt ||
  t[27] !== Z ||
  t[28] !== X ||
  t[29] !== Mt ||
  t[30] !== kt ||
  t[31] !== nt
    ? ((Bt = () => {
        (Et(),
          Re(`archive-conversation`, {
            conversationId: r,
            hostId: Z ?? void 0,
            source: `recent_tasks_menu`,
          })
            .then(() => {
              Dt();
              let e = () => {
                n.dispatchHostMessage({
                  type: `navigate-to-route`,
                  path: `/settings/data-controls`,
                });
              };
              if (!Mt) {
                let t = nt.get(rt).info(
                  (0, lr.jsx)(St, {
                    onUndo: () => {
                      (t.close(),
                        Re(`unarchive-conversation`, { hostId: Z ?? `local`, conversationId: r })
                          .then(() => {
                            kt();
                          })
                          .catch(() => {
                            nt.get(rt).danger(
                              X.formatMessage({
                                id: `localTaskRow.unarchiveError`,
                                defaultMessage: `Failed to unarchive conversation`,
                                description: `Error message when undoing a local conversation archive`,
                              }),
                            );
                          }));
                    },
                    onOpenSettings: e,
                  }),
                  { duration: s / 1e3, id: `archive-thread` },
                );
              }
            })
            .catch(() => {
              (Ot(),
                nt
                  .get(rt)
                  .danger(
                    X.formatMessage({
                      id: `localTaskRow.archiveError`,
                      defaultMessage: `Failed to archive conversation`,
                      description: `Error message when archiving a local Codex conversation`,
                    }),
                  ));
            }));
      }),
      (t[23] = Et),
      (t[24] = r),
      (t[25] = Ot),
      (t[26] = Dt),
      (t[27] = Z),
      (t[28] = X),
      (t[29] = Mt),
      (t[30] = kt),
      (t[31] = nt),
      (t[32] = Bt))
    : (Bt = t[32]);
  let Vt = Bt,
    Ht;
  t[33] !== qe || t[34] !== pt
    ? ((Ht =
        pt === `approval` || qe
          ? {
              id: `awaiting-approval`,
              label: (0, lr.jsx)(L, {
                id: `codex.localTaskRow.awaitingApproval`,
                defaultMessage: `Awaiting approval`,
                description: `Chip indicating a local task is waiting for user approval`,
              }),
            }
          : null),
      (t[33] = qe),
      (t[34] = pt),
      (t[35] = Ht))
    : (Ht = t[35]);
  let Ut = Ht,
    Q;
  t[36] === Ut ? (Q = t[37]) : ((Q = Ut == null ? [] : [Ut]), (t[36] = Ut), (t[37] = Q));
  let Wt = Q,
    Gt = mt?.type === `userInput` ? mt : null,
    Kt = Gt != null && _t?.requestId === Gt.item.requestId ? _t : null,
    qt = Kt?.resolutionState.status === `scheduled` ? Kt.resolutionState : null,
    Jt = Kt?.resolutionState.status === `scheduled` ? Kt.requestId : null,
    Yt;
  t[38] !== Ge ||
  t[39] !== r ||
  t[40] !== Z ||
  t[41] !== X ||
  t[42] !== z ||
  t[43] !== pt ||
  t[44] !== qt ||
  t[45] !== Jt
    ? ((Yt =
        pt === `response` && !z && !Ge
          ? {
              ariaLabel: qt == null ? void 0 : X.formatMessage(ur.snoozeInputTimeout),
              label: (0, lr.jsx)(L, {
                id: `codex.localTaskRow.needsInput`,
                defaultMessage: `Needs input`,
                description: `Status pill indicating an inactive local task needs user input`,
              }),
              hoverLabel: qt == null ? void 0 : (0, lr.jsx)(L, { ...ur.snoozeInputTimeout }),
              onClick:
                Jt == null
                  ? void 0
                  : () => {
                      Fe.requestUserInputAutoResolution.snooze({
                        conversationId: r,
                        hostId: Z ?? `local`,
                        requestId: Jt,
                      });
                    },
              progress: qt == null ? void 0 : { deadlineMs: qt.deadlineMs, durationMs: qn },
            }
          : void 0),
      (t[38] = Ge),
      (t[39] = r),
      (t[40] = Z),
      (t[41] = X),
      (t[42] = z),
      (t[43] = pt),
      (t[44] = qt),
      (t[45] = Jt),
      (t[46] = Yt))
    : (Yt = t[46]);
  let Xt = Yt,
    Qt;
  t[47] !== et || t[48] !== we || t[49] !== X || t[50] !== Qe
    ? ((Qt =
        Qe || et
          ? {
              id: `automation`,
              icon: (0, lr.jsx)(se, { className: `shrink-0` }),
              ariaLabel: Qe
                ? X.formatMessage({
                    id: `codex.localTaskRow.automation`,
                    defaultMessage: `Scheduled task chat`,
                    description: `Accessible label for the scheduled task thread icon`,
                  })
                : X.formatMessage({
                    id: `codex.localTaskRow.attachedHeartbeatAutomation`,
                    defaultMessage: `Scheduled task attached`,
                    description: `Accessible label for the scheduled task icon shown on a chat with an attached scheduled task`,
                  }),
              tooltipContent: et ? we : null,
            }
          : null),
      (t[47] = et),
      (t[48] = we),
      (t[49] = X),
      (t[50] = Qe),
      (t[51] = Qt))
    : (Qt = t[51]);
  let $t = Qt,
    en = ce;
  if ($t != null) {
    let e;
    (t[52] !== $t || t[53] !== ce
      ? ((e = ce == null ? [$t] : [...ce, $t]), (t[52] = $t), (t[53] = ce), (t[54] = e))
      : (e = t[54]),
      (en = e));
  }
  let tn;
  t[55] !== ft || t[56] !== lt || t[57] !== Ye || t[58] !== Z || t[59] !== B
    ? ((tn = Yn({ codexHome: ft, cwd: lt, hideRemoteHostEnvIcon: Ye, hostId: Z, isGrouped: B })),
      (t[55] = ft),
      (t[56] = lt),
      (t[57] = Ye),
      (t[58] = Z),
      (t[59] = B),
      (t[60] = tn))
    : (tn = t[60]);
  let nn = tn,
    rn = nn === `remote` || nn === `remote-worktree` ? null : lt,
    an;
  t[61] !== r ||
  t[62] !== Ue ||
  t[63] !== i ||
  t[64] !== et ||
  t[65] !== ut ||
  t[66] !== we ||
  t[67] !== Ye ||
  t[68] !== ge ||
  t[69] !== ke ||
  t[70] !== je ||
  t[71] !== Ne ||
  t[72] !== Pe ||
  t[73] !== he ||
  t[74] !== Qe ||
  t[75] !== B ||
  t[76] !== $e ||
  t[77] !== Mt ||
  t[78] !== It ||
  t[79] !== tt ||
  t[80] !== Ft
    ? ((an = Ue
        ? null
        : nr({
            conversationId: r,
            displayCwd: i,
            hasAttachedHeartbeatAutomation: et,
            heartbeatAutomationTooltipContent: we,
            hideRemoteHostEnvIcon: Ye,
            hoverCardSections: he,
            hoverCardHostConfig: ge,
            hoverCardProjectId: je,
            hoverCardProjectLabel: Ne,
            hoverCardRepositoryLabel: Pe,
            threadSummary: tt,
            hasUnreadTurn: Mt ? !1 : ut === !0,
            isAutomationRun: Qe,
            isGrouped: B,
            isProjectlessHoverCard: $e,
            projectFallbackIcon: ke,
            shouldFetchHoverBranch: It,
            threadTitle: Ft,
          })),
      (t[61] = r),
      (t[62] = Ue),
      (t[63] = i),
      (t[64] = et),
      (t[65] = ut),
      (t[66] = we),
      (t[67] = Ye),
      (t[68] = ge),
      (t[69] = ke),
      (t[70] = je),
      (t[71] = Ne),
      (t[72] = Pe),
      (t[73] = he),
      (t[74] = Qe),
      (t[75] = B),
      (t[76] = $e),
      (t[77] = Mt),
      (t[78] = It),
      (t[79] = tt),
      (t[80] = Ft),
      (t[81] = an))
    : (an = t[81]);
  let on = an;
  if ((wt && !Tt) || (ot !== !0 && tt == null) || st == null) return null;
  let sn = Z ?? void 0,
    cn;
  t[82] !== r || t[83] !== it || t[84] !== c || t[85] !== o || t[86] !== a
    ? ((cn = () => {
        (c?.(),
          it(r),
          (0, cr.startTransition)(() => {
            (a?.(), o?.());
          }));
      }),
      (t[82] = r),
      (t[83] = it),
      (t[84] = c),
      (t[85] = o),
      (t[86] = a),
      (t[87] = cn))
    : (cn = t[87]);
  let ln = en,
    un = E === `tableRow` ? `bottom` : void 0,
    dn = E === `tableRow` ? `end` : void 0,
    fn = E === `tableRow` ? -36 : void 0,
    pn;
  t[88] === X
    ? (pn = t[89])
    : ((pn = X.formatMessage({
        id: `codex.localTaskRow.archiveTask`,
        defaultMessage: `Archive chat`,
        description: `Label for archiving a local thread`,
      })),
      (t[88] = X),
      (t[89] = pn));
  let $;
  t[90] === Symbol.for(`react.memo_cache_sentinel`)
    ? (($ = (0, lr.jsx)(L, {
        id: `codex.localTaskRow.confirmArchiveTask`,
        defaultMessage: `Confirm`,
        description: `Confirmation button for archiving a local task`,
      })),
      (t[90] = $))
    : ($ = t[90]);
  let mn;
  return (
    t[91] !== He ||
    t[92] !== Ze ||
    t[93] !== Ke ||
    t[94] !== Wt ||
    t[95] !== Ie ||
    t[96] !== A ||
    t[97] !== Xe ||
    t[98] !== p ||
    t[99] !== rn ||
    t[100] !== nn ||
    t[101] !== q ||
    t[102] !== Vt ||
    t[103] !== U ||
    t[104] !== H ||
    t[105] !== on ||
    t[106] !== en ||
    t[107] !== y ||
    t[108] !== b ||
    t[109] !== S ||
    t[110] !== x ||
    t[111] !== z ||
    t[112] !== F ||
    t[113] !== K ||
    t[114] !== re ||
    t[115] !== u ||
    t[116] !== De ||
    t[117] !== ne ||
    t[118] !== ie ||
    t[119] !== G ||
    t[120] !== ze ||
    t[121] !== Xt ||
    t[122] !== zt ||
    t[123] !== sn ||
    t[124] !== cn ||
    t[125] !== un ||
    t[126] !== dn ||
    t[127] !== fn ||
    t[128] !== pn ||
    t[129] !== Ft ||
    t[130] !== me ||
    t[131] !== V ||
    t[132] !== E
      ? ((mn = (0, lr.jsx)(Zt, {
          hostId: sn,
          isActive: z,
          idleIndicatorNode: y,
          idleStatusIndicatorNode: b,
          indicatorRestNode: x,
          indicatorHoverNode: S,
          reserveLeadingSlot: G,
          additionalHoverActionCount: He,
          priorityIndicatorNode: ne,
          variant: E,
          onContextMenu: re,
          onClick: cn,
          onDoubleClick: u,
          disabled: Xe,
          allowActionsWhenDisabled: Ze,
          statusState: zt,
          statusIndicatorReplacesMeta: ze,
          hideStatusIndicator: H,
          useStableTrailingRail: V,
          envType: nn,
          envIconLocation: p,
          disableEnvTooltip: A,
          envTooltip: rn,
          hoverCardContent: on,
          blurOnClick: Ke,
          title: Ft,
          titleSuffix: me,
          chips: Wt,
          statusPill: Xt,
          iconBadges: ln,
          floatStatusIconsRight: q,
          hideInlineBadgesOnHover: U,
          metaHidden: K,
          metaContent: F,
          overlayMetaContent: De,
          onArchive: Vt,
          onHoverCardOpenChange: Lt,
          hoverCardSide: un,
          hoverCardAlign: dn,
          hoverCardSideOffset: fn,
          dataAttributes: Ie,
          archiveAriaLabel: pn,
          archiveConfirmLabel: $,
          renderActions: ie,
        })),
        (t[91] = He),
        (t[92] = Ze),
        (t[93] = Ke),
        (t[94] = Wt),
        (t[95] = Ie),
        (t[96] = A),
        (t[97] = Xe),
        (t[98] = p),
        (t[99] = rn),
        (t[100] = nn),
        (t[101] = q),
        (t[102] = Vt),
        (t[103] = U),
        (t[104] = H),
        (t[105] = on),
        (t[106] = en),
        (t[107] = y),
        (t[108] = b),
        (t[109] = S),
        (t[110] = x),
        (t[111] = z),
        (t[112] = F),
        (t[113] = K),
        (t[114] = re),
        (t[115] = u),
        (t[116] = De),
        (t[117] = ne),
        (t[118] = ie),
        (t[119] = G),
        (t[120] = ze),
        (t[121] = Xt),
        (t[122] = zt),
        (t[123] = sn),
        (t[124] = cn),
        (t[125] = un),
        (t[126] = dn),
        (t[127] = fn),
        (t[128] = pn),
        (t[129] = Ft),
        (t[130] = me),
        (t[131] = V),
        (t[132] = E),
        (t[133] = mn))
      : (mn = t[133]),
    mn
  );
}
function or(e) {
  let t = (0, sr.c)(3),
    { title: n, titleOverride: r } = e;
  if (typeof r != `string` && r != null) return r;
  if (n != null) return n;
  let i;
  t[0] === r ? (i = t[1]) : ((i = r?.trim() ?? ``), (t[0] = r), (t[1] = i));
  let a = i;
  if (a.length > 0) return a;
  let o;
  return (
    t[2] === Symbol.for(`react.memo_cache_sentinel`)
      ? ((o = (0, lr.jsx)(L, {
          id: `codex.taskRow.title`,
          defaultMessage: `New chat`,
          description: `Default title for a Codex thread that doesn't have a title`,
        })),
        (t[2] = o))
      : (o = t[2]),
    o
  );
}
var sr,
  cr,
  lr,
  ur,
  dr = e(() => {
    ((sr = d()),
      p(),
      r(),
      (cr = t(i(), 1)),
      G(),
      Pe(),
      le(),
      B(),
      Jn(),
      R(),
      Tt(),
      Qe(),
      qe(),
      pe(),
      ae(),
      Y(),
      O(),
      y(),
      Ne(),
      ie(),
      E(),
      f(),
      X(),
      Xn(),
      ir(),
      q(),
      Sn(),
      En(),
      (lr = C()),
      (ur = U({
        snoozeInputTimeout: {
          id: `codex.localTaskRow.snoozeInputTimeout`,
          defaultMessage: `Snooze`,
          description: `Button label for permanently disabling an input request timeout`,
        },
      })));
  });
export {
  kt as _,
  Kn as a,
  xt as b,
  Un as c,
  En as d,
  Cn as f,
  qt as g,
  Kt as h,
  Jn as i,
  Dn as l,
  Sn as m,
  dr as n,
  Wn as o,
  gn as p,
  qn as r,
  Bn as s,
  ar as t,
  jn as u,
  Mt as v,
  ht as y,
};
//# sourceMappingURL=app-initial~app-main~remote-conversation-page~projects-index-page~hotkey-window-thread-page~hc7acb17.js.map
