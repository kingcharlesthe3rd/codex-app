import { h as e } from "./vscode-api.js";
import { Cn as t, Dn as n, En as r, On as i, Tn as a } from "./product-logger.js";
...
function u({ diagnostics: e, durationMs: n }) {
  return {
    durationMs: n,
...
function d({ durationMs: e }) {
  return { durationMs: e, status: t.CODEX_PRIMARY_RUNTIME_DEPENDENCIES_DIAGNOSE_STATUS_FAILED };
}
...
function p(e) {
  switch (e) {
    case `latest`:
      return i.CODEX_PRIMARY_RUNTIME_RELEASE_LATEST;
    case `latest-alpha`:
      return i.CODEX_PRIMARY_RUNTIME_RELEASE_LATEST_ALPHA;
  }
}
function m(e) {
  switch (e) {
    case `already-current`:
      return n.CODEX_PRIMARY_RUNTIME_INSTALL_RESULT_STATUS_ALREADY_CURRENT;
    case `canceled`:
      return n.CODEX_PRIMARY_RUNTIME_INSTALL_RESULT_STATUS_CANCELED;
    case `failed`:
      return n.CODEX_PRIMARY_RUNTIME_INSTALL_RESULT_STATUS_FAILED;
    case `installed`:
      return n.CODEX_PRIMARY_RUNTIME_INSTALL_RESULT_STATUS_INSTALLED;
  }
}
function h(e) {
  switch (e) {
    case `already-current`:
      return a.CODEX_PRIMARY_RUNTIME_DEPENDENCIES_RESET_STATUS_ALREADY_CURRENT;
    case `canceled`:
      return a.CODEX_PRIMARY_RUNTIME_DEPENDENCIES_RESET_STATUS_CANCELED;
    case `failed`:
      return a.CODEX_PRIMARY_RUNTIME_DEPENDENCIES_RESET_STATUS_FAILED;
    case `installed`:
      return a.CODEX_PRIMARY_RUNTIME_DEPENDENCIES_RESET_STATUS_INSTALLED;
  }
}
function g(e) {
  return e == null || e.length === 0 ? {} : { bundleVersion: e };
}
async function _({ formatMessage: t, hostId: n, productLogger: i, release: a, toast: s }) {
  let c = Date.now(),
    u = s.info(
      t({
  ...
  try {
    let e = await o({ hostId: n, release: a, request: `install` });
    if (
      (i.logProductEvent(
        r,
        l({
    ...
    (e.error(`Error installing primary runtime`, { safe: { release: a }, sensitive: { error: n } }),
      i.logProductEvent(
        r,
