import { $s as e, ic as t, nc as n, rc as r, tc as i } from "./app-server-manager-signals.js";
...
function u({ diagnostics: t, durationMs: n }) {
  return {
    durationMs: n,
    problemCount: t.problems.length,
    status: t.installed
      ? e.CODEX_PRIMARY_RUNTIME_DEPENDENCIES_DIAGNOSE_STATUS_OK
      : e.CODEX_PRIMARY_RUNTIME_DEPENDENCIES_DIAGNOSE_STATUS_PROBLEM,
    ...g(t.bundleVersion),
  };
}
function d({ durationMs: t }) {
  return { durationMs: t, status: e.CODEX_PRIMARY_RUNTIME_DEPENDENCIES_DIAGNOSE_STATUS_FAILED };
}
...
function p(e) {
  switch (e) {
    case `latest`:
      return t.CODEX_PRIMARY_RUNTIME_RELEASE_LATEST;
    case `latest-alpha`:
      return t.CODEX_PRIMARY_RUNTIME_RELEASE_LATEST_ALPHA;
  }
}
function m(e) {
  switch (e) {
...
async function _({ formatMessage: e, hostId: t, productLogger: r, release: i, toast: s }) {
  let c = Date.now(),
    u = s.info(
      e({
  ...
  try {
    let a = await o({ hostId: t, release: i, request: `install` });
    if (
      (r.logProductEvent(
        n,
        l({
    ...
    if (v(t)) {
      (r.logProductEvent(
        n,
        l({ bundleVersion: null, durationMs: Date.now() - c, release: i, status: `canceled` }),
      ),
        s.info(
          e({
    ...
    (a.error(`Error installing primary runtime`, { safe: { release: i }, sensitive: { error: t } }),
      r.logProductEvent(
        n,
        l({ bundleVersion: null, durationMs: Date.now() - c, release: i, status: `failed` }),
      ),
      s.danger(
        e({
