  ...
  ce = class e {
    ...
    insert(t, n, r, i, a, o) {
      ...
      (this._children.hasOwnProperty(c)
        ? (u = this._children[c])
        : ((u = new e(this._mainRule.clone(), se.cloneArr(this._rulesWithParentScopes))),
          (this._children[c] = u)),
...
`;
    ...
    return (
      he(a), { lineLength: o, lineTokens: s, ruleStack: c.stack, stoppedEarly: c.stoppedEarly }
    );
  }
};
function ft(e, t) {
...
function or(e, t, n) {
  ...
  let c = vn(
    r.attribute,
    Object.assign({}, e.settings.characterReferences, { subset: rr.name[i][a] }),
  );
  return n === !0 ||
    ((n = Array.isArray(n)
  ...
  return {
    ...
    code(e) {
      ...
      function a(e, t, r, a) {
        ...
        if (l === -1)
          throw new n(`Failed to find start index for decoration ${JSON.stringify(a.start)}`);
        if (u === -1)
          throw new n(`Failed to find end index for decoration ${JSON.stringify(a.end)}`);
  ...
  for (; e.length > 0; ) {
    ...
    if (!Number.isNaN(r))
      if (r === 0) t.push({ type: `resetAll` });
        ...
        t.push({ type: `setForegroundColor`, value: { type: `named`, name: k[r - 30] } });
      else if (r === 38) {
        let n = Zr(e);
        n && t.push({ type: `setForegroundColor`, value: n });
      } else if (r === 39) t.push({ type: `resetForegroundColor` });
      else if (r <= 47)
        t.push({ type: `setBackgroundColor`, value: { type: `named`, name: k[r - 40] } });
      else if (r === 48) {
        let n = Zr(e);
    ...
    new Set(
      (
        await Promise.all(
          e
            .filter((e) => !wr(e))
            .map(async (e) => await Sr(e).then((e) => (Array.isArray(e) ? e : [e]))),
        )
      ).flat(),
    ),
  );
...
async function Ai(e) {
  return (await Promise.all(e.map(async (e) => (Er(e) ? null : Oi(await Sr(e)))))).filter(
    (e) => !!e,
  );
}
function ji(e, t = 3) {
...
function Ni(e, t) {
  ...
  if (t[e]) {
    ...
    for (; t[e]; ) {
      if (((e = t[e]), n.has(e)))
        throw new Mi(`Circular alias \`${Array.from(n).join(` -> `)} -> ${e}\``);
      n.add(e);
    }
  }
  return e;
...
function Xi(e, t, n, r) {
  ...
  if (i === `(`) {
    ...
    if (n === `(?=` || n === `(?!` || n === `(?<=` || n === `(?<!`)
      return {
        token: ua(n[2] === `<` ? `lookbehind` : `lookahead`, n, { negate: n.endsWith(`!`) }),
      };
    if (
      (n === `(` && e.captureGroup) ||
...
function _a(e, t) {
  ...
  if (e.endsWith(`:`))
    return (t.pushModX(i), t.numOpenGroups++, ua(`group`, e, { ...((a || o) && { flags: s }) }));
...
function Ia(e, t, n) {
  let {
    ...
    d = u && c.negate;
  if (
    (c.type === `CapturingGroup` && (i.push(c), c.name && Vi(a, c.name, []).push(c)),
...
function Mo(e, t) {
  ...
  for (R.lastIndex = 0; (f = R.exec(e)); ) {
    ...
    if (t === `[`) u++;
      ...
      (d++, s.set(String(d), R.lastIndex), s.set(p, R.lastIndex), c.push({ num: d, name: p }));
    else if (t[0] === `(`) {
      let e = t === `(`;
...
var rs = {
    ...
    Assertion({ node: e, parent: t, key: n, container: r, root: i, remove: a, replaceWith: o }, s) {
      ...
      if (c === `line_end`)
        o(K(F({ body: [N({ body: [Ba(`string_end`)] }), N({ body: [Ua(10)] })] }), t));
...
\v\f\u2028\u2029])`,
    ...
    Directive({
      ...
      if (s === `flags`)
        ...
        let e = n.body[0],
          r =
            n.body.length === 1 && Da(e, { type: `Group` }) && e.body[0].body.length === 1
              ? e.body[0]
              : n;
...
var tc = Object.defineProperty,
  nc = (e, t, n) =>
    t in e ? tc(e, t, { enumerable: !0, configurable: !0, writable: !0, value: n }) : (e[t] = n),
...
const Bc = { dark: `pierre-dark`, light: `pierre-light` };
...
function Uc(e, t) {
  ...
  for (let n of e) {
    if (Hc.has(n.name)) continue;
    let e = Vc.get(n.name);
    (e ?? ((e = n), Vc.set(n.name, e)), Hc.add(e.name), t.loadLanguageSync(e.data));
  }
}
const Wc = new Map(),
  Gc = new Set();
function Kc(e, t) {
  ...
  for (let n of e) {
    let e;
    if (typeof n == `string`) {
      if (((e = Wc.get(n)), e == null))
        throw Error(
          `loadResolvedThemes: ${n} is not resolved, you must resolve it before calling loadResolvedThemes`,
        );
    } else ((e = n), (n = n.name), Wc.has(n) || Wc.set(n, e));
    Gc.has(n) || (Gc.add(n), t.loadThemeSync(e));
  }
}
const Z = new Map();
let qc = 0;
...
function Yc(e) {
  ...
  if (Jc[e] != null) return Jc[e];
  let t = e.match(/\.([^/\\]+\.[^/\\]+)$/);
  if (t != null) {
    if (Z.has(t[1])) return Z.get(t[1]) ?? `text`;
    if (Jc[t[1]] != null) return Jc[t[1]] ?? `text`;
  }
  let n = e.match(/\.([^.]+)$/)?.[1] ?? ``;
  return Z.has(n) ? (Z.get(n) ?? `text`) : (Jc[n] ?? `text`);
}
function Xc(e, t) {
  if (e <= qc) return !1;
  Z.clear();
...
function $c({ tagName: e, children: t = [], properties: n = {} }) {
...
const nl = Symbol(`no-token`),
  rl = Symbol(`multiple-tokens`);
function il(e) {
  let t = al(e);
  if (t != null) return t;
  let n = nl,
    ...
    o = () => {
      ...
      if (i.length === 1) {
        ...
        if (e?.type === `element`) {
          sl(e, a);
          for (let t of e.children) ol(t);
        } else ol(e);
        (r.push(e), (i = []), (a = void 0));
        return;
      }
      for (let e of i) ol(e);
      (r.push($c({ tagName: `span`, properties: { "data-char": a }, children: i })),
        (i = []),
        (a = void 0));
    },
    s = (e) => {
...
function cl(e = !1, t = !1) {
        ...
        pre(t) {
          ...
          if (r != null) {
            ...
            for (let a of r.children)
              a.type === `element` && (e && il(a), i.push(tl(a, t, n)), t++);
            r.children = i;
          }
          return t;
        },
  ...
  return (
    ...
    { state: n, transformers: r, toClass: ll }
  );
}
const ll = Rc({ classPrefix: `hl-` }),
  ul = {
...
function dl({ theme: e = Bc, highlighter: t, prefix: n }) {
  let r = ``;
  if (typeof e == `string`) {
    let i = t.getTheme(e);
    ((r += `color:${i.fg};`),
...
function hl({ fileDiff: e, errorPrefix: t }) {
...
function gl({
  ...
  let a = hl({ fileDiff: e, errorPrefix: i });
  if (a <= 0) return;
  if (n === !0 || a <= r)
    return { fromStart: a, fromEnd: 0, rangeSize: a, collapsedLines: 0, renderAll: !0 };
...
function vl({
  ...
  let a = yl({ diff: e, expandedHunks: r, collapsedContextThreshold: i }),
    o = 0,
    s = e.hunks.length - 1,
    c = e.hunks.length;
  for (; o <= s; ) {
...
function Ol({
  ...
  if (
    ...
    t == null ||
    o === `none` ||
    ((e = Zc(e)), (t = Zc(t)), e.length > s || t.length > s)
  )
    return;
  ...
  let p = 0;
  for (let e of l)
    (e[0] === 1 && i.push(wl({ line: n, spanStart: p, spanLength: e[1].length })),
      (p += e[1].length));
  p = 0;
  for (let e of u)
    (e[0] === 1 && a.push(wl({ line: r, spanStart: p, spanLength: e[1].length })),
      (p += e[1].length));
}
function kl() {
  return {
...
function Al({
  ...
  languageOverride: s,
  options: { theme: c = Bc, ...l },
}) {
  let u = s ?? Yc(e.name),
    d = s ?? Yc(t.name),
    { state: f, transformers: p } = cl(l.useTokenTransformer),
    m =
      typeof c == `string`
        ? {
            ...l,
    ...
    deletionLines:
      e.contents === ``
        ...
        : ((m.lang = u),
          (f.lineInfo = n),
          (m.decorations = a),
          pl(i.codeToHast(Zc(e.contents), m))),
    additionLines:
      t.contents === ``
        ? []
        : ((m.lang = d),
          (m.decorations = o),
          (f.lineInfo = r),
          pl(i.codeToHast(Zc(t.contents), m))),
  };
}
function jl(e) {
  let t = [0];
  for (let n = 0; n < e.length; n++) {
    ...
    (r === 10 || r === 13) &&
      (r === 13 && n + 1 < e.length && e.charCodeAt(n + 1) === 10 && n++, t.push(n + 1));
  }
  return t;
}
function Ml(e) {
...
function Pl(
  e,
  t,
  { theme: n = Bc, tokenizeMaxLineLength: r, useTokenTransformer: i },
  { forcePlainText: a, startingLine: o, totalLines: s, lines: c } = Nl,
) {
  a ? ((o ??= 0), (s ??= 1 / 0)) : ((o = 0), (s = 1 / 0));
  let l = o > 0 || s < 1 / 0,
    { state: u, transformers: d } = cl(i),
    f = a ? `text` : (e.lang ?? Yc(e.name)),
  ...
  let h =
    ...
    g = pl(t.codeToHast(l ? Fl(c ?? Ml(e.contents), o, s) : e.contents, h)),
    _ = l ? Array(o) : g;
...
function Ul(e = `shiki-js`) {
  return (
    (Il ??= zi({
      themes: [],
      langs: [],
      engine: e === `shiki-wasm` ? vc(import(`./wasm-1.js`)) : Ys(),
    })),
    Il
  );
}
