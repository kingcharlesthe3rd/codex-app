...
import { wn as u } from "./app-server-manager-signals.js";
import { B as d, V as f, h as p, l as m, lt as h, u as g, v as _ } from "./vscode-api.js";
import { n as v } from "./fuse.js";
...
function b(e) {
  return (0, y.default)(e.flatMap((e) => (e == null ? [] : [e])));
}
function x(e, t) {
  return {
    access_mode: e.access_mode,
    allowed_user_emails: b(
      e.allowed_users.filter((e) => e.account_user_id !== t).map((e) => e.email),
    ),
  };
}
function S({ accessGroups: e, accessPolicy: t, group: n, knownAccessGroups: r }) {
  return {
    access_mode: t.access_mode,
    allowed_user_emails: b(t.allowed_users.map((e) => e.email)),
    ...C({
      accessGroups: e,
      knownAccessGroups: r,
...
function C({
  ...
  return {
    ...(u ? { allowed_workspace_group_ids: w(c.filter((e) => e.source === `workspace`)) } : {}),
    ...(d ? { allowed_tenant_group_ids: w(c.filter((e) => e.source === `tenant`)) } : {}),
  };
}
function w(e) {
  return Array.from(new Set(e.map((e) => e.id)));
}
var T = t([`admins_only`, `workspace_all`, `custom`, `public`]),
  E = h(),
  D = [`appgen`, `projects`],
  O = [`appgen`, `project`],
  k = [`appgen`, `screenshot`],
  ...
  M = s({ id: c(), name: c(), size: l() }),
  N = s({
    project_id: c(),
    access_mode: T,
    allowed_account_user_ids: i(c()),
    allowed_workspace_group_ids: i(c()),
    allowed_tenant_group_ids: i(c()),
    revision: l(),
    updated_at: c(),
  }),
  P = N.extend({
    allowed_groups: i(M),
    allowed_users: i(s({ account_user_id: c(), email: c().nullable(), name: c().nullable() })),
  }),
  F = s({
  ...
  I = F.extend({ access_mode: T.nullable(), access_policy: N.nullable() }),
  L = s({ cursor: c().nullable().optional(), items: i(I) }),
  R = F.extend({ access_mode: T.nullable(), access_policy: P.nullable() }),
  z = s({
    entries: i(s({ is_secret: n(), key: c(), type: r(`envvar`), value: c().nullable() })),
    project_id: c(),
    revision: l(),
  ...
  B = s({
    ...
    workspace_groups: i(M),
  }),
  V = a([
    s({ error: s({}).passthrough(), id: l(), jsonrpc: r(`2.0`) }),
    s({
      id: l(),
      jsonrpc: r(`2.0`),
      result: s({ isError: n().optional(), structuredContent: o().optional() }),
    }),
  ]);
function H() {
  let e = (0, E.c)(1),
...
var J = f(p, (e) => ({
  ...
  re = d(p, (e) => ({
    mutationKey: [...O, e, `delete`],
    mutationFn: () =>
      ee.safeDelete(`/wham/sites/projects/{project_id}`, {
    ...
    onSuccess: async (t, n, r, { client: i }) => {
      (i.setQueryData(D, (t) =>
        ...
        await Promise.all([
          i.invalidateQueries({ queryKey: [...O, e], refetchType: `none` }),
...
//# sourceMappingURL=appgen-url.js.map
