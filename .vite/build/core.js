...
var r;
function i() {
  if (globalThis.TextDecoder !== void 0) return (r ??= new globalThis.TextDecoder(`utf-8`));
}
var a = 32 * 1024,
  o = 65533;
...
function ee(e) {
  ...
  for (e.length >= 3 && e[0] === 239 && e[1] === 187 && e[2] === 191 && (r = 3); r < e.length; ) {
    ...
    (u(t, n, ((i & 7) << 18) | ((a & 63) << 12) | ((s & 63) << 6) | (c & 63)), (r += 4));
  }
  return (c(t, n), t.join(``));
}
function te(e) {
  ...
  for (; i + 1 < r; ) {
    ...
    if (((i += 2), a >= 55296 && a <= 56319)) {
      if (i + 1 < r) {
        let r = e[i] | (e[i + 1] << 8);
        r >= 56320 && r <= 57343 ? (l(t, n, a), l(t, n, r), (i += 2)) : l(t, n, o);
      } else l(t, n, o);
      continue;
    }
    if (a >= 56320 && a <= 57343) {
...
function ie(e) {
  ...
  for (let i = 0; i < e.length; i++) {
    ...
    ((r += s ?? String.fromCharCode(o)), r.length >= a && (n.push(r), (r = ``)));
  }
...
var ae = {
  ...
  de = class extends ue {
    async readFromStream(e, t) {
      ...
      return (
        n.done && (this.endOfStream = n.done), n.value ? (e.set(n.value), n.value.length) : 0
      );
    }
  },
  y = class extends v {
...
var b = class {
    ...
    async readNumber(e) {
      if ((await this.readBuffer(this.numBuffer, { length: e.len })) < e.len) throw new _();
      return e.get(this.numBuffer, 0);
    }
    async peekNumber(e) {
      if ((await this.peekBuffer(this.numBuffer, { length: e.len })) < e.len) throw new _();
      return e.get(this.numBuffer, 0);
    }
    async ignore(e) {
  ...
  me = class extends b {
    ...
    async readBuffer(e, t) {
      ...
      if (((this.position += i), (!t || !t.mayBeLess) && i < n.length)) throw new _();
      return i;
    }
    async peekBuffer(e, t) {
      ...
      if (n.position) {
        ...
        if (t > 0) {
          ...
          return (
            (r = await this.peekBuffer(i, { mayBeLess: n.mayBeLess })), e.set(i.subarray(t)), r - t
          );
        }
  ...
  D = class e {
    ...
    async findEndOfCentralDirectoryLocator() {
      ...
      for (let r = n.length - 4; r >= 0; r--)
        if (n[r] === E[0] && n[r + 1] === E[1] && n[r + 2] === E[2] && n[r + 3] === E[3])
    ...
    async readCentralDirectory() {
      ...
      if (t > 0) {
        ...
        for (let e = 0; e < n.nrOfEntriesOfSize; ++e) {
          ...
          ((t.filename = await this.tokenizer.readToken(new g(t.filenameLength, `utf-8`))),
    ...
    async unzip(e) {
      ...
      do {
        ...
        if (
          ...
          let e = [],
            n = T;
          w(`Compressed-file-size unknown, scanning for next data-descriptor-signature....`);
          let i = -1;
          for (; i < 0 && n === T; ) {
          ...
          (w(`Found data-descriptor-signature at pos=${this.tokenizer.position}`),
        ...
        if (
          ...
          t.dataDescriptor && (await this.tokenizer.readToken(S)).signature !== 134695760)
        )
          throw Error(
    ...
    async inflate(t, n, r) {
      ...
      if (t.compressedMethod !== 8)
        throw Error(`Unsupported ZIP compression method: ${t.compressedMethod}`);
      return (
    ...
    async readLocalFileHeader() {
      ...
      if (e === x.LocalFileHeader) {
        ...
        return ((e.filename = await this.tokenizer.readToken(new g(e.filenameLength, `utf-8`))), e);
      }
...
var Ae = {
  ...
  He = N,
  H = M,
  Ue = new Set([`Unexpected signature`, `Encrypted ZIP`, `Expected Central-File-Header signature`]),
  We = [
...
async function K(e, t, n, { maximumLength: r = t.length, reason: i = `read` } = {}) {
...
async function qe(e, { maximumLength: t = A } = {}) {
  ...
  try {
    for (;;) {
      ...
      if (e) break;
      if (((i += a.length), i > t))
        throw (await n.cancel(), Error(`ZIP entry decompressed data exceeds ${t} bytes`));
      r.push(a);
    }
  } finally {
    n.releaseLock();
...
function Ye(e, t) {
  ...
  let n = e.length - J;
  for (let r = 0; r <= n; r++) if (m.get(e, r) === q && m.get(e, r + 8) === t + r) return r;
  return -1;
}
function Xe(e) {
...
async function Qe(e, { shouldBuffer: t, maximumLength: n = A } = {}) {
  ...
  if ((Z(e.tokenizer) || (e.knownSizeDescriptorScannedBytes += o), t)) return Ze(a, o);
}
function $e(e, t) {
...
async function et(e, t, { shouldBuffer: n, maximumDescriptorLength: r = A } = {}) {
...
((D.prototype.inflate = async function (e, t, n) {
  ...
  (D.prototype.unzip = async function (e) {
    ...
    do {
      if (Q(this.tokenizer, r, M)) throw new U(`ZIP stream probing exceeds ${M} bytes`);
...
function dt(e, t) {
  (t.startsWith(`word/`) && (e.hasWordDirectory = !0),
    ...
    t.startsWith(`3D/`) && t.endsWith(`.model`) && (e.hasThreeDimensionalModelEntry = !0));
}
function $(e) {
  if (
    !(
    ...
    if (e.hasThreeDimensionalModelEntry) return { ext: `3mf`, mime: `model/3mf` };
  }
}
function ft(e) {
...
var pt = class e {
  ...
  async fromBuffer(e) {
    ...
    if (t?.length > 1) return this.fromTokenizer(ve(t, this.getTokenizerOptions()));
  }
  async fromBlob(e) {
  ...
  async toDetectionStream(e, t) {
    ...
    try {
      let { value: e, done: t } = await it(a, new Uint8Array(n), this.options.signal);
      if (((i = e), !t && e))
        try {
  ...
  async detectGzip(t) {
    if (this.gzipProbeDepth >= Ie) return { ext: `gz`, mime: `application/gzip` };
    ...
    if (r) {
      ...
      ((i = setTimeout(() => {
        t.abort(new DOMException(`Operation timed out after ${F} ms`, `TimeoutError`));
      }, F)),
        (a =
  ...
  detectConfident = async (e) => {
    ...
    if (this.checkString(`ID3`)) {
      ...
      let t = await e.readToken(Ae),
        n = Z(e);
      if (!Number.isFinite(t) || t < 0 || (n && (t > I || e.position + t > I))) return;
      if (e.position + t > e.fileInfo.size) return n ? void 0 : { ext: `mp3`, mime: `audio/mpeg` };
      try {
        await G(e, t, { maximumLength: n ? I : e.fileInfo.size, reason: `ID3 payload` });
      } catch (e) {
    ...
    if (this.check([26, 69, 223, 163])) {
      async function t() {
        ...
        return (await K(e, i, void 0, { maximumLength: i.length, reason: `EBML field` }), i);
      }
      async function n() {
        let e = await t(),
          n = await t();
    ...
    if (this.check([202, 254, 186, 190])) {
      ...
      if (e > 0 && e <= 30) return { ext: `macho`, mime: `application/x-mach-binary` };
...
`,
    ...
    if (this.check([137, 80, 78, 71, 13, 10, 26, 10])) {
      ...
      await e.ignore(8);
      async function r() {
        return { length: await e.readToken(oe), type: await e.readToken(new g(4, `latin1`)) };
      }
      let i = Z(e),
        a = e.position,
        o = 0,
    ...
    if (this.check([48, 38, 178, 117, 142, 102, 207, 17, 166, 217])) {
      ...
      try {
        async function n() {
          ...
          return (
            await K(e, t, void 0, { maximumLength: t.length, reason: `ASF header GUID` }),
        ...
        let r = Z(e),
          i = e.position,
          a = 0;
        for (; e.position + 24 < e.fileInfo.size && (a++, !(a > Be || Q(e, i, M))); ) {
          let i = e.position,
            a = await n(),
            o = a.size - 24;
          ...
          if (X(a.id, [145, 7, 220, 183, 183, 169, 207, 17, 142, 230, 0, 192, 12, 32, 83, 101])) {
            let t = new Uint8Array(16);
            if (
              ...
              X(t, [64, 158, 105, 248, 77, 91, 207, 17, 168, 253, 0, 128, 95, 92, 68, 43]))
            )
              return { ext: `asf`, mime: `audio/x-ms-asf` };
            if (X(t, [192, 239, 25, 188, 77, 91, 207, 17, 168, 253, 0, 128, 95, 92, 68, 43]))
...
`,
  ...
  async readTiffHeader(e) {
    ...
    if (n === 42) {
      ...
      try {
        await G(this.tokenizer, r, { maximumLength: n, reason: `TIFF IFD offset` });
      } catch (e) {
        if (e instanceof _) return;
        throw e;
      }
