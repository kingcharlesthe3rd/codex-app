...
async function b() {
  try {
    ...
    try {
      return await r.R.load(``);
    } catch {
      return r.R.createDefault();
    }
  }
}
...
var S = `pending-source-dmg-cleanup.json`,
  ...
  w = t
    .Oi({
      images: t
        .bi(
          t
            .Oi({
              "image-path": t.ji().optional(),
              "system-entities": t
                .bi(t.Oi({ "mount-point": t.ji().optional() }).passthrough())
                .optional(),
            })
            .passthrough(),
        )
    ...
    .passthrough(),
  T = t.Oi({ sourceDmgPath: t.ji() }).passthrough();
async function E({
  clearPendingSourceDmgPath: e = z,
  copyAppBundleToApplicationsFolder: t = te,
  detachSourceDmg: a = W,
  getCurrentAppBundlePath: o = r.w,
  getPendingSourceDmgPath: c = L,
  getSourceDmgPath: l = V,
  isApplicationsFolderWritable: u = ee,
  isPackaged: d = i.app.isPackaged,
  openInstalledAppBundle: f = k,
  platform: p = process.platform,
  quitCurrentApp: m = () => i.app.quit(),
  setPendingSourceDmgPath: h = R,
  showInstallerWindow: g = A,
  sourceDmgExists: _ = s.existsSync,
  trashItem: v = (e) => i.shell.trashItem(e),
  isInApplicationsFolder: y = () => D({ getCurrentAppBundlePath: o }),
  ...
  if (x == null) return !1;
  let S = await g();
  F({ setPendingSourceDmgPath: h, sourceDmgPath: x });
  try {
...
function D({ getCurrentAppBundlePath: e }) {
  try {
    if (`isInApplicationsFolder` in i.app) return i.app.isInApplicationsFolder();
  } catch (e) {
    n.Lr().warning(`Failed to check app Applications folder status`, {
...
async function k(e) {
  try {
    return (i.app.releaseSingleInstanceLock(), await C(`open`, [`-n`, e]), !0);
  } catch (e) {
    return (
      n
        .Lr()
...
async function A() {
  ...
  return (
    ...
    await t.loadURL(`data:text/html;charset=utf-8,${encodeURIComponent(K(i.app.getName()))}`),
    t.isDestroyed() || (j(t), t.show(), t.focus()),
    {
      allowClose: n,
      setStatus: async (e) => {
        t.isDestroyed() ||
...
function M(e) {
  try {
    ...
    n.Lr().warning(`Failed to remove staging app bundle`, {
      safe: { errorType: e instanceof Error ? e.name : typeof e },
    });
  }
}
async function N({
  ...
  let a = n();
  a != null &&
    (await I({ detachSourceDmg: t, sourceDmgExists: r, sourceDmgPath: a, trashItem: i })) &&
    e();
}
function P(e) {
  try {
...
function F({ setPendingSourceDmgPath: e, sourceDmgPath: t }) {
  try {
    e(t);
  } catch (e) {
    n.Lr().warning(`Failed to remember app source DMG for cleanup`, {
      safe: { errorType: e instanceof Error ? e.name : typeof e },
    });
  }
}
async function I({ detachSourceDmg: e, sourceDmgExists: t, sourceDmgPath: r, trashItem: i }) {
  if (!t(r)) return !0;
  try {
    if (!e(r)) return !1;
  } catch (e) {
    return (
      n
        .Lr()
        .warning(`Failed to detach app source DMG`, {
          safe: { errorType: e instanceof Error ? e.name : typeof e },
        }),
      !1
    );
...
function L() {
  ...
  try {
    return T.parse(JSON.parse(s.readFileSync(e, `utf8`))).sourceDmgPath;
  } catch (e) {
    return (
      n
        .Lr()
...
function B() {
  return a.join(i.app.getPath(`userData`), S);
}
function V() {
  let e = r.w();
  return e.startsWith(`/Volumes/`) ? H(e, U()) : null;
}
function H(e, t) {
  let n = null,
    r = ``;
  for (let i of t)
    if (a.extname(i.imagePath).toLowerCase() === `.dmg`)
      for (let t of i.mountPoints) J(e, t) && t.length > r.length && ((n = i), (r = t));
...
function U() {
  ...
  return (w.parse(JSON.parse(e)).images ?? []).flatMap((e) => {
...
function G(e) {
  try {
    ...
    try {
      return ((0, c.execFileSync)(`hdiutil`, [`detach`, `-force`, e]), !0);
    } catch (e) {
      return (
        n
          .Lr()
...
(r.v(),
  r.n(Z),
  i.app.setName(t.ft(Q)),
  i.app.setPath(
    `userData`,
    x({ appDataPath: i.app.getPath(`appData`), buildFlavor: Q, env: process.env }),
  ),
  r.C(),
  process.platform === `win32` && i.app.setAppUserModelId(r.D(Q)));
var $ = r.E({ isMacOS: Z, isPackaged: i.app.isPackaged });
if (!(!$ || i.app.requestSingleInstanceLock()))
  ...
  ($ &&
    ...
    i.app.whenReady().then(async () => {
      let { desktopSentry: t, sparkleManager: a } = e;
      if (
        !(await v({
      ...
      if (!(await E()) && (await r.g())) {
        await a.initialize();
        try {
          let { runMainAppStartup: e } = await Promise.resolve().then(() => require(`./main.js`));
          await e();
        } catch (e) {
