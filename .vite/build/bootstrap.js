    ...
    .$s({
      images: t
        .Gs(
          t
            .$s({
              "image-path": t.nc().optional(),
              "system-entities": t
                .Gs(t.$s({ "mount-point": t.nc().optional() }).passthrough())
                .optional(),
            })
            .passthrough(),
        )
...
function te({ getCurrentAppBundlePath: e }) {
  try {
    ...
    t.Ur().warning(`Failed to check app Applications folder status`, {
...
function P(e) {
  try {
    ...
    t.Ur().warning(`Failed to remove staging app bundle`, {
      safe: { errorType: e instanceof Error ? e.name : typeof e },
    });
  }
}
...
function L({ setPendingSourceDmgPath: e, sourceDmgPath: n }) {
  try {
    ...
    t.Ur().warning(`Failed to remember app source DMG for cleanup`, {
      safe: { errorType: e instanceof Error ? e.name : typeof e },
    });
  }
}
async function R({ detachSourceDmg: e, sourceDmgExists: n, sourceDmgPath: r, trashItem: i }) {
  ...
  if (!a) return (t.Ur().warning(`Failed to detach app source DMG after retries`), !1);
  try {
    return (await i(r), !0);
  } catch (e) {
    return (
...
var $ = r.O({ isMacOS: Z, isPackaged: i.app.isPackaged });
