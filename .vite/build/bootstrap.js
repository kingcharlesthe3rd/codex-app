...
function S({ buildFlavor: e, env: t }) {
  ...
  return Object.entries(i).map(([e, t]) => {
    if (e.length === 0)
      throw Error(`CODEX_ELECTRON_CHROMIUM_SWITCHES contains an empty switch name`);
    if (t != null && typeof t != `string`)
      throw Error(`CODEX_ELECTRON_CHROMIUM_SWITCHES value for ${e} must be a string or null`);
...
async function R({ detachSourceDmg: e, sourceDmgExists: n, sourceDmgPath: r, trashItem: i }) {
  ...
  if (!a) return (t.ti().warning(`Failed to detach app source DMG after retries`), !1);
  try {
    return (await i(r), !0);
  } catch (e) {
    return (
...
(r.b(),
  ...
  i.app.setPath(
    `userData`,
    C({ appDataPath: i.app.getPath(`appData`), buildFlavor: Q, env: process.env }),
  ),
