...
</html>`}function J(e){return e.replaceAll(`&`,`&amp;`).replaceAll(`<`,`&lt;`).replaceAll(`>`,`&gt;`)}function Y(e,t){let n=i.relative(t,e);return n===``||!!n&&!n.startsWith(`..`)&&!i.isAbsolute(n)}var X={"install-update":`Install Update`,"check-for-updates":`Check for Updates`,quit:`Quit`};async function re(e){let{sparkleManager:t}=n.A(),i=t.getIsUpdateReady()?[`install-update`,`quit`]:t.hasUpdater()?[`check-for-updates`,`quit`]:[`quit`];switch(i[(await r.dialog.showMessageBox({type:`error`,buttons:i.map(e=>X[e]),defaultId:0,cancelId:i.length-1,message:`${r.app.getName()} failed to start.`,detail:e instanceof Error?e.message:`The main desktop app failed during startup.`,noLink:!0})).response]??`quit`){case`install-update`:await t.installUpdatesIfAvailable();return;case`check-for-updates`:await t.checkForUpdates();return;case`quit`:r.app.quit();return}}var Z=process.platform===`darwin`,Q=n.F.resolve(),ie=n.G();n.y(),n.n(Z),r.app.setName(t.Xi(Q,ie)),r.app.setPath(`userData`,x({appDataPath:r.app.getPath(`appData`),buildFlavor:Q,env:process.env})),n.w(),process.platform===`win32`&&r.app.setAppUserModelId(n.k(Q));var $=n.O({isMacOS:Z,isPackaged:r.app.isPackaged});if(!(!$||r.app.requestSingleInstanceLock()))t.zr().info(`Exiting second desktop instance`,{safe:{packaged:r.app.isPackaged,platform:process.platform}}),r.app.exit(0);else{let e=n.A(Q);$&&r.app.on(`second-instance`,(t,n)=>{e.queueSecondInstanceArgs(n)}),r.app.whenReady().then(async()=>{let{desktopSentry:i,sparkleManager:a}=e;if(!await v({appName:r.app.getName(),environment:{arch:process.arch,isPackaged:r.app.isPackaged,platform:process.platform}})){r.app.quit();return}if(!await O()&&await n._()){await a.initialize();try{let{runMainAppStartup:e}=await Promise.resolve().then(()=>require(`./main-DS-bu3Xr.js`));await e()}catch(e){for(let e of r.BrowserWindow.getAllWindows())e.isDestroyed()||e.destroy();t.zr().error(`Desktop bootstrap failed to start the main app`,{safe:{phase:`bootstrap-import-main`}}),i.captureException(e,{tags:{phase:`bootstrap-import-main`}}),await re(e)}}})}
