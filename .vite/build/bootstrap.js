...
async function b() {
  try {
    ...
    try {
      return await n.U.load(``);
    } catch {
      return n.U.createDefault();
    }
  }
}
...
var S = `pending-source-dmg-cleanup.json`,
  ...
  E = t
    .zs({
      images: t
        .Ms(
          t
            .zs({
              "image-path": t.Hs().optional(),
              "system-entities": t
                .Ms(t.zs({ "mount-point": t.Hs().optional() }).passthrough())
                .optional(),
            })
            .passthrough(),
        )
    ...
    .passthrough(),
  D = t.zs({ sourceDmgPath: t.Hs() }).passthrough();
async function O({
  clearPendingSourceDmgPath: e = B,
  copyAppBundleToApplicationsFolder: i = k,
  detachSourceDmg: a = G,
  getCurrentAppBundlePath: s = n.D,
...
function ee({ getCurrentAppBundlePath: e }) {
  try {
    ...
    t.Vr().warning(`Failed to check app Applications folder status`, {
...
function N(e) {
  try {
    ...
    t.Vr().warning(`Failed to remove staging app bundle`, {
      safe: { errorType: e instanceof Error ? e.name : typeof e },
    });
  }
}
...
function I({ setPendingSourceDmgPath: e, sourceDmgPath: n }) {
  try {
    ...
    t.Vr().warning(`Failed to remember app source DMG for cleanup`, {
      safe: { errorType: e instanceof Error ? e.name : typeof e },
    });
  }
}
async function L({ detachSourceDmg: e, sourceDmgExists: n, sourceDmgPath: r, trashItem: i }) {
  ...
  if (!a) return (t.Vr().warning(`Failed to detach app source DMG after retries`), !1);
  try {
    return (await i(r), !0);
  } catch (e) {
    return (
...
(n.b(),
  n.n(Z),
  r.app.setName(t.Qi(Q, ie)),
  r.app.setPath(
    `userData`,
    x({ appDataPath: r.app.getPath(`appData`), buildFlavor: Q, env: process.env }),
  ),
  n.T(),
  process.platform === `win32` && r.app.setAppUserModelId(n.A(Q)));
var $ = n.k({ isMacOS: Z, isPackaged: r.app.isPackaged });
