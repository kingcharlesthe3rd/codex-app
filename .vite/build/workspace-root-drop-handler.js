...
let c = require(`node:crypto`),
  l = require(`node:fs/promises`);
l = e.o(l);
let u = require(`node:stream`),
  d = require(`node:url`),
  f = require(`module`),
  p = require(`node:worker_threads`);
var m = t.Ni(t.Pi(), t.Pi()),
  h = null;
async function g(e) {
  if (h?.repoRoot !== e)
    try {
      h = { repoRoot: e, nativeIntl: await v.load(e) };
    } catch {
      h = { repoRoot: null, nativeIntl: v.createDefault() };
    }
}
function _() {
  if (h == null) throw Error(`NativeIntl has not been configured.`);
  return h.nativeIntl;
}
var v = class e {
    constructor(e) {
    ...
    static async load(t) {
      let n = i.app.getLocale(),
        [r] = n.replace(/_/g, `-`).split(`-`),
        a = i.app.isPackaged
          ? (0, o.join)(i.app.getAppPath(), `native-menu-locales`)
          : (0, o.join)(t, `webview`, `src`, `locales`),
        s = [(0, o.join)(a, `${n}.json`), ...(r == null ? [] : [(0, o.join)(a, `${r}.json`)])];
      return new e(
        (
          await Promise.all(
            s.map(async (e) => {
              try {
                return m.parse(JSON.parse(await (0, l.readFile)(e, `utf-8`)));
              } catch (e) {
    ...
    formatMessage({ messageId: e, defaultMessage: t, values: n }) {
      return (this.messages?.[e] ?? t).replace(/\{(\w+)\}/g, (e, t) => n?.[t] ?? e);
    }
  },
  y = e.i({ default: () => b }),
  b,
...
function te() {
  i.protocol.registerSchemesAsPrivileged([
    { scheme: `app`, privileges: { standard: !0, secure: !0, stream: !0, supportFetchAPI: !0 } },
  ]);
}
function ne(e) {
  return (0, o.join)(e, `..`, `..`, `webview`);
}
function re(e, t) {
  let n = new URL(`${O}/${E}`);
  return (
    e && n.searchParams.set(`initialRoute`, e),
    t?.mcpAppSandboxDevtools === !0 && n.searchParams.set(`mcpAppSandboxDevtools`, `1`),
    n.toString()
  );
}
function ie(e, n) {
  ...
  if (!r) return null;
  try {
    if (pe(t.vr(decodeURIComponent(r)))) return null;
  } catch {
    return null;
  }
  let i = new URL(e);
  if (i.protocol !== `app:`) return null;
  if (i.pathname.startsWith(D)) return i.host === T ? me(i.pathname) : null;
  if (i.host && i.host !== w) return null;
  let a = t.vr(i.pathname ? i.pathname : `/`),
    s = a.startsWith(`/`) ? a.slice(1) : a,
    c = o.posix.normalize(s);
  if (c === `.` || c === ``) return (0, o.join)(n, E);
  if (c.startsWith(`..`) || c.includes(`/..`)) return null;
  let l = (0, o.join)(n, ...c.split(`/`)),
    u = (0, o.relative)(n, l);
  return u.startsWith(`..`) || (0, o.isAbsolute)(u) ? null : l;
}
function ae(e) {
  let t;
  try {
  ...
  return t.protocol !== `app:` || t.host !== T || !t.pathname.startsWith(D) ? null : me(t.pathname);
}
function oe(e) {
  (le(),
    i.protocol.handle(`app`, async (t) => {
      let n = ie(t.url, e);
      return n
        ? he(n)
          ? ge(t, n)
          : i.net.fetch((0, d.pathToFileURL)(n).toString())
        : new Response(null, { status: 404, statusText: `Not Found` });
    }));
}
function se(e) {
  let t = ue(e?.url);
  return t == null ? !1 : t === O || t === de();
}
function ce(e) {
  return !se(e.frame);
}
function le() {
  i.session.defaultSession.webRequest.onBeforeRequest({ urls: [`${ee}/*`] }, (e, t) => {
    t({ cancel: ce(e) });
  });
}
function ue(e) {
...
function de() {
  if (i.app.isPackaged) return null;
...
function me(e, n = o.resolve) {
  ...
  let a = t.vr(i);
  if (!t.hr(a)) return null;
  let s = n(t.yr(a)),
    c = (0, C.lookup)(s);
  return typeof c != `string` || (!c.startsWith(`image/`) && !c.startsWith(`video/`)) ? null : s;
}
function he(e) {
  let t = (0, C.lookup)(e);
...
async function ge(e, t) {
  ...
  if (!a)
    return (
      ...
      new Response(u.Readable.toWeb((0, s.createReadStream)(t)), { headers: i })
    );
  let o = _e(a, n.size);
  return o
    ? (i.set(`Content-Length`, String(o.end - o.start + 1)),
      i.set(`Content-Range`, `bytes ${o.start}-${o.end}/${n.size}`),
      new Response(u.Readable.toWeb((0, s.createReadStream)(t, { start: o.start, end: o.end })), {
        status: 206,
        statusText: `Partial Content`,
        headers: i,
      }))
    : (i.set(`Content-Range`, `bytes */${n.size}`),
      new Response(null, { status: 416, statusText: `Range Not Satisfiable`, headers: i }));
}
function _e(e, t) {
  ...
  let a = Number(r),
    o = i === `` ? t - 1 : Number(i);
  return a < 0 || o < a || a >= t ? null : { start: a, end: Math.min(o, t - 1) };
}
var ve = () => {
  let e = process.cwd(),
    t = [];
  return (
    process.resourcesPath &&
      t.push(o.default.join(process.resourcesPath, `app.asar`, `package.json`)),
    t.push(o.default.join(e, `package.json`)),
    t.push(o.default.join(e, `electron`, `package.json`)),
    t
  );
};
function k(e, { candidates: t = ve(), parse: n } = {}) {
  for (let r of t) {
    if (!(0, s.existsSync)(r)) continue;
    let t = (0, s.readFileSync)(r, `utf8`),
      i = JSON.parse(t),
...
var A = n.Rr(`build-flavor`),
  ye = [t.Sr.Nightly, t.Sr.InternalAlpha, t.Sr.PublicBeta, t.Sr.Prod],
  be = {
    [t.Sr.Nightly]: { kind: `msix` },
    [t.Sr.InternalAlpha]: { kind: `msix` },
    [t.Sr.PublicBeta]: {
      kind: `store`,
  ...
  Se = (e, t, n, r) => !xe(r) && ye.includes(e) && t === n,
  Ce = (e, t, n) => Se(e, t, `win32`, n),
  we = (e) => be[e] ?? null,
  j = {
    ...t.Sr,
    readFromPackageMetadata() {
      let e = k(`codexBuildFlavor`, { parse: (e) => (j.isValid(e) ? e : null) });
      return e
    ...
    resolve() {
      let e = process.env.BUILD_FLAVOR,
        t = j.parse(e);
      return t
        ? (A().info(`Resolved build flavor from env`, { safe: { value: t }, sensitive: {} }), t)
        : (e?.trim() &&
            A().warning(`Invalid BUILD_FLAVOR`, {
              safe: { value: e, expected: j.help },
              sensitive: {},
            }),
          j.readFromPackageMetadata() ||
            (process.env.NODE_ENV === `production`
              ? (A().warning(`BUILD_FLAVOR missing; defaulting to prod in production runtime`),
                j.Prod)
              : (A().warning(`BUILD_FLAVOR missing; defaulting to dev in non-production runtime`),
                j.Dev)));
    },
    ...
    shouldIncludeWindowsUpdater(e, t, n = process.env) {
      return Ce(e, t, n) && we(e) != null;
    },
    shouldIncludeWindowsMsixUpdater(e, t, n = process.env) {
      return Ce(e, t, n) && we(e)?.kind === `msix`;
    },
    shouldIncludeBrowserUsePeerAuthorization(e, t) {
      return t === `darwin` && ye.includes(e);
    },
    shouldIncludeUpdater(e, t, n = process.env) {
      return j.shouldIncludeSparkle(e, t, n) || j.shouldIncludeWindowsUpdater(e, t, n);
    },
    getWindowsStoreConfig(e) {
      let t = we(e);
      return t?.kind === `store`
        ? { storeProductId: t.storeProductId, storeUpdateManifestUrl: t.storeUpdateManifestUrl }
        : null;
    },
    allowDevtools(e) {
...
function Te(e = process.platform, t = process.env) {
  return e !== `linux` && t.CODEX_USE_OWL_APP_SHELL !== `0`;
}
var Ee = void 0,
  De = n.Rr(`codex-build-number`);
function Oe() {
  return (
    process.env.CODEX_BUILD_NUMBER?.trim() ||
    (i.app.isPackaged
      ? k(`codexBuildNumber`) ||
        (De().error(`CodexBuildNumber missing from package metadata.`), null)
      : null)
  );
}
var ke = {
    get value() {
      return (Ee === void 0 && (Ee = Oe()), Ee);
    },
  },
  M = {
    codexAppSessionId: r.t,
    buildFlavor: j.resolve(),
    buildNumber: ke.value,
    appVersion: i.app.getVersion(),
  };
function Ae() {
  let e = t.qr(M.appVersion),
    a = M.buildFlavor !== `prod`,
    o = t.A(e.version);
  return (
    r.r({
      dsn: t.k,
      environment: M.buildFlavor,
      release: o,
      dist: M.buildNumber ?? void 0,
      tracesSampleRate: 0,
      integrations: (e) => [
        ...e,
        r.a({ root: i.app.isPackaged ? i.app.getAppPath() : process.cwd(), prefix: `app:///` }),
      ],
    }),
    r.s(`sessionId`, r.t),
    r.s(`preRelease`, a),
    r.s(`buildFlavor`, M.buildFlavor),
    r.s(`bundle`, `electron`),
    r.s(`host`, `app`),
    n.kn({ startSpan: (e, t) => t() }),
    { desktopSentry: r.n, release: o, tracesSampleRate: 0 }
  );
}
var je = 10080 * 60 * 1e3,
  Me = 7200 * 1e3,
  ...
  Ie = t.ji({
    relaunchNotification: N,
    relaunchNotificationPeriodMs: N,
    relaunchWindow: t
      .Ii()
      .optional()
      .transform((e) => e ?? null),
    relaunchFastIfOutdatedDays: N,
  }),
  Le = t.ji({ entries: t.Ci(t.ji({ duration_mins: N, start: t.ji({ hour: N, minute: N }) })) }),
  Re = t.ji({
    owlUpdatePolicies: t.ji({
      getRelaunchNotificationPolicy: t.Ti((e) => typeof e == `function`),
      on: t.Ti((e) => typeof e == `function`),
    }),
  });
function ze() {
  if (!Te()) return null;
  let e = process._linkedBinding;
  if (typeof e != `function`) return null;
  let t;
  try {
    t = e.call(process, `electron_browser_owl_update_policies`);
  } catch {
    return null;
  }
  return Ue(t) ? t.owlUpdatePolicies : null;
}
function Be(e) {
  return He(e.getRelaunchNotificationPolicy());
}
function Ve(e, t) {
  return Ke(e + We(t), t.relaunchWindow);
}
function He(e) {
...
function Ue(e) {
  return Re.safeParse(e).success;
}
function We(e) {
  let { relaunchNotificationPeriodMs: t } = e,
    n;
  return (
    (n = t == null || t < Ne ? je : t), Ge(e.relaunchFastIfOutdatedDays) ? Math.min(n, Me) : n
  );
}
function Ge(e) {
  if (e == null || e < Pe) return !1;
  let n = t.Kr(i.app.getVersion());
  return n ? Date.now() - n.getTime() > e * Fe : !1;
}
function Ke(e, t) {
  let n = qe(t);
...
function Ze(e) {
  let t = e.trim().split(`.`);
  if (t.some((e) => !/^\d+$/.test(e))) throw Error(`Invalid build version '${e}'.`);
  return t.map((e) => Number.parseInt(e, 10));
}
var I = n.Rr(`windows-updater`),
  Qe = (0, f.createRequire)(__filename),
  $e = class {
    stateRoot = ``;
    isUpdateReady = !1;
    ...
    async initialize() {
      ...
      ((this.stateRoot = o.default.join(i.app.getPath(`userData`), `windows-msix-updater`, e)),
        await l.default.mkdir(this.stateRoot, { recursive: !0 }),
        (this.stateRoot = await l.default.realpath(this.stateRoot)),
    ...
    async installUpdatesIfAvailable() {
      ...
      try {
        ...
        (this.setInstallProgressPercent(null),
          I().error(`Failed to install prepared Windows update`, {
            safe: {},
            sensitive: { error: e },
          }),
          this.showActionError({
            message: `Unable to install the ${i.app.getName()} update.`,
    ...
    async runCheck({ manual: e }) {
      ...
      try {
        ...
        throw (
          ...
          I().error(`Failed to check for Windows updates`, { safe: {}, sensitive: { error: t } }),
          e &&
            (await this.showActionError({
              message: `Unable to check for ${i.app.getName()} updates.`,
    ...
    async fetchManifest() {
      let e = await i.net.fetch(this.options.manifestUrl);
      if (!e.ok)
        throw Error(`Windows update manifest request failed (${e.status} ${e.statusText})`);
    ...
    async downloadUpdatePackage(e) {
      let t = e.packageUrl,
        n = decodeURIComponent(o.default.posix.basename(new URL(t).pathname)).trim();
      if (n === `` || !n.toLowerCase().endsWith(`.msix`))
        throw Error(`Invalid Windows update package URL '${t}'.`);
      let r = o.default.join(this.stateRoot, n),
        a = `${r}.download`;
      await this.cleanupDownloadedPackages(a);
      let s = await i.net.fetch(t);
      if (!s.ok || s.body == null)
        throw Error(`Windows update download failed (${s.status} ${s.statusText})`);
      this.assertFetchedHttpsUrl(s.url, t, `package`);
      let d = (0, c.createHash)(`sha256`),
        f = 0,
        p = u.Readable.fromWeb(s.body),
        m = await l.default.open(a, `w`);
      try {
      ...
      if (f !== e.length)
        throw (
          await l.default.rm(a, { force: !0 }),
          Error(`Downloaded MSIX length mismatch (expected ${e.length}, got ${f}).`)
        );
      if (h !== e.sha256)
        throw (
          await l.default.rm(a, { force: !0 }),
          Error(`Downloaded MSIX checksum mismatch (expected ${e.sha256}, got ${h}).`)
        );
      return (await l.default.rename(a, r), r);
    }
    async installPreparedUpdate() {
      let e = await this.readStageState();
      if (e == null) throw Error(`Install requested without a staged Windows update.`);
      (this.setUpdateReady(!1), this.setInstallProgressPercent(0));
      let t = !1,
        n = () => {
          t || ((t = !0), this.options.onInstallUpdatesRequested?.());
        },
        r = this.options.nativeAddon.activateStagedPackage(e.packagePath, (e) => {
      ...
      try {
        ...
        throw (
          await this.runCheck({ manual: !1 }).catch((e) => {
            I().error(`Failed to resync Windows updater state`, {
              safe: {},
              sensitive: { error: e },
            });
          }),
    ...
    async showManualCheckResult(e) {
      if (e === `update-ready`) {
        await i.dialog.showMessageBox({
          ...
          defaultId: 0,
          noLink: !0,
          message: `${i.app.getName()} update is ready to install.`,
      ...
      await i.dialog.showMessageBox({
        ...
        defaultId: 0,
        noLink: !0,
        message: `${i.app.getName()} is up to date.`,
      });
    }
    async reconcileLocalPreparedState() {
      ...
      if (P(e.buildVersion, this.options.buildVersion) <= 0) {
    ...
    async readStageState() {
      ...
      try {
        let t = await l.default.readFile(e, `utf8`),
          n = JSON.parse(t);
        if (
        ...
        let i = o.default.join(this.stateRoot, decodeURIComponent(o.default.basename(r)));
        return {
        ...
        return t?.code === `ENOENT`
          ? null
          : (I().warning(`Discarding invalid Windows updater staged state.`, {
              safe: {},
              sensitive: { error: t, statePath: e },
            }),
            await l.default.rm(e, { force: !0 }),
            null);
      }
    }
    async cleanupDownloadedPackages(e) {
      let t = await l.default.readdir(this.stateRoot, { withFileTypes: !0 });
      await Promise.all(
        t
          .filter((e) => {
          ...
          .map((t) => {
            let n = o.default.join(this.stateRoot, t.name);
            return e != null && n === e ? Promise.resolve() : l.default.rm(n, { force: !0 });
          }),
      );
    }
    ...
    async performCheck() {
      ...
      if (P(e.buildVersion, this.options.buildVersion) <= 0)
        return (
          await this.discardPreparedUpdate(), this.setUpdateLifecycleState(`idle`), `up-to-date`
        );
      ...
      if (
        ...
        return (this.setUpdateReady(!0), this.setUpdateLifecycleState(`ready`), `update-ready`);
      ...
      return (
        await l.default.writeFile(
          `${r}.tmp`,
          JSON.stringify(
            { buildVersion: e.buildVersion, length: e.length, packagePath: n, sha256: e.sha256 },
            null,
            2,
          ),
        ),
        await l.default.rename(`${r}.tmp`, r),
    ...
    getStageStatePath() {
      return o.default.join(this.stateRoot, `staged-msix.json`);
    }
    async discardPreparedUpdate() {
      (await l.default.rm(this.getStageStatePath(), { force: !0 }),
    ...
    setInstallProgressPercent(e) {
      this.installProgressPercent !== e &&
        ((this.installProgressPercent = e), this.options.onInstallProgressChanged?.(e));
    }
...
function et() {
  let e;
  try {
    return Qe((0, o.join)(process.resourcesPath, `native`, `windows-updater.node`));
  } catch (t) {
    e = t;
  }
  return (
    I().error(`Failed to load native Windows updater addon`, { safe: {}, sensitive: { error: e } }),
    null
  );
}
var tt = 1,
  nt = 1800 * 1e3,
  rt = t.ji({
    schemaVersion: t.Ai().int().min(tt),
    buildVersion: t.Pi().trim().min(1),
    storeProductId: t.Pi().trim().min(1),
    packageIdentity: t.Pi().trim().min(1),
  });
function it(e) {
...
async function at(e) {
  ...
  if (!n.ok)
    throw Error(`Windows Store update manifest request failed (${n.status} ${n.statusText})`);
  if (new URL(n.url.trim() || t).protocol !== `https:`)
    throw Error(`Windows Store update manifest must use https transport.`);
  return rt.parse(JSON.parse(await n.text()));
}
function ot(e) {
  let { manifest: t } = e,
    n = F(e.buildVersion),
    r = e.lastStoreDetectionAttemptAtMs != null && e.nowMs - e.lastStoreDetectionAttemptAtMs < nt;
  if (t == null)
    return n
      ? n
        ? `up-to-date`
  ...
  if (!F(t.buildVersion))
    throw Error(`Windows Store update manifest has invalid build version '${t.buildVersion}'.`);
  return n && P(t.buildVersion, e.buildVersion) <= 0
    ? `up-to-date`
    : e.isUpdateReady
      ? `update-ready`
      : r
...
var L = n.Rr(`windows-store-updater`),
  st = class {
    isUpdateReady = !1;
    installProgressPercent = null;
    lastUnavailableReason = null;
    ...
    async installUpdatesIfAvailable() {
      ...
      try {
        ...
        (this.setInstallProgressPercent(null),
          L().error(`Failed to install Windows Store update`, {
            safe: {},
            sensitive: { error: e },
          }),
          await this.showActionError({
    ...
    async runCheck() {
      ...
      try {
        ...
        throw (
          this.setUpdateLifecycleState(this.isUpdateReady ? `ready` : `idle`),
          L().error(`Failed to check for Windows Store updates`, {
            safe: {},
            sensitive: { error: e },
          }),
          e
    ...
    async performCheck() {
      this.isUpdateReady || this.setUpdateLifecycleState(`checking`);
      let e = await at({ fetch: (e) => i.net.fetch(e), ...this.options });
      F(this.options.buildVersion) ||
        L().warning(
      ...
      ((this.lastStoreDetectionAttemptAtMs = Date.now()),
        L().info(`Checking Windows Store for package updates`, {
          safe: {
            buildVersion: this.options.buildVersion,
            manifestBuildVersion: e?.buildVersion ?? null,
            packageIdentity: this.options.packageIdentity,
      ...
      if (
        (L().info(`Windows Store package update check completed`, {
          safe: {
      ...
      if (!n.completed)
        throw (
          ...
          Error(`Windows Store update download failed with state '${n.overallState}'.`)
        );
      (this.setUpdateReady(!0), this.setUpdateLifecycleState(`ready`));
    }
    async performInstall() {
      ...
      let e = !1,
        ...
        n = await this.options.nativeAddon.trySilentDownloadAndInstallStoreUpdates((e, n) => {
      ...
      throw (
        this.setInstallProgressPercent(null),
        Error(`Windows Store update install failed with state '${n.overallState}'.`)
      );
    }
    setUpdateReady(e) {
      this.isUpdateReady !== e &&
  ...
  R = n.Rr(`sparkle`),
  ct = (0, f.createRequire)(__filename),
  lt = 15,
  ut = 300 * 60 * 1e3,
  dt = 60 * 1e3,
  ft = 2 ** 31 - 1;
function pt() {
  ...
  if (!e) return lt * 60 * 1e3;
  let t = Number(e);
  return Number.isFinite(t) ? (t <= 0 ? 0 : t * 60 * 1e3) : lt * 60 * 1e3;
}
var mt = class {
    updater = null;
    isUpdateReady = !1;
    ...
    async initialize() {
      ...
      try {
        if (
          ...
          i.ipcMain.handle(`codex_desktop:check-for-updates`, async (e) => {
        ...
        R().warning(`Updater unavailable after init.`, {
          safe: {
        ...
        (R().error(`Failed to set up updater`, { safe: {}, sensitive: { error: e } }),
    ...
    showRelaunchNoticeForDebug() {
      this.setRelaunchNotice(Date.now() + ut);
    }
    async checkForUpdates() {
      if (!this.updater) {
        R().warning(`Update check request ignored (updater unavailable).`);
        return;
      }
      try {
        ...
        (this.updateLifecycleState === `checking` &&
          !this.isUpdateReady &&
          this.setUpdateLifecycleState(`idle`),
          R().error(`Failed to check for updates`, { safe: {}, sensitive: { error: e } }));
      }
    }
    async installUpdatesIfAvailable() {
      if (!this.updater)
        return (R().warning(`Update install request ignored (updater unavailable).`), !1);
      try {
        return (
          this.isUpdateReady &&
        ...
        return (
          this.setUpdateLifecycleState(this.isUpdateReady ? `ready` : `idle`),
          R().error(`Failed to install update`, { safe: {}, sensitive: { error: e } }),
          !1
        );
      }
    }
    ...
    async initializeWindowsUpdater() {
      ...
      let t = {
        ...
        n = j.getWindowsStoreConfig(this.options.buildFlavor),
        r;
      if (n != null)
        r = new st({
    ...
    async initializeMacSparkle() {
      ...
      let t = null,
        n;
      try {
        t = ct((0, o.join)(process.resourcesPath, `native`, `sparkle.node`));
      } catch (e) {
        n = e;
      }
      if (!t) {
        (R().error(`Failed to load native Sparkle addon`, { safe: {}, sensitive: { error: n } }),
      ...
      try {
        (typeof r.setLogSink == `function` &&
          r.setLogSink((e, t) => {
            R().log(this.decodeNativeLogLevel(e) ?? `info`, t);
          }),
          ...
          typeof r.setDownloadProgressSink == `function` &&
            r.setDownloadProgressSink((e) => {
              this.setDownloadProgressPercent(e);
            }),
        ...
        (R().error(`Failed to initialize Sparkle addon`, { safe: {}, sensitive: { error: e } }),
      ...
      let s = () => {
        try {
          ...
          (this.isUpdateReady || this.setUpdateLifecycleState(`idle`),
            R().error(`Failed to check for updates in background`, {
              safe: {},
              sensitive: { error: e },
            }));
        }
    ...
    resolveMacSparkleFeedUrl() {
      return (
        ...
        ((this.lastUnavailableReason = `missing packaged sparkle feed url`), null)
      );
    }
    resolveWindowsUpdateUrl() {
      let e = k(`codexWindowsUpdateUrl`)?.trim() ?? ``;
      if (!e) return ((this.lastUnavailableReason = `missing packaged windows update url`), null);
      let t;
      try {
        t = new URL(e);
      } catch {
        return ((this.lastUnavailableReason = `invalid packaged windows update url`), null);
      }
      return t.protocol === `https:`
        ? t.toString()
        : ((this.lastUnavailableReason = `windows update url must use https transport`), null);
    }
    resolveBuildVersion() {
      return k(`codexBuildNumber`)?.trim() ?? i.app.getVersion();
    }
    resolveWindowsStoreBuildVersion() {
      let e = i.app.getVersion().trim();
      try {
        return t.Jr(e);
      } catch (t) {
        return (
          R().warning(
            ...
            { safe: { appVersion: e }, sensitive: { error: t } },
          ),
          e
        );
      }
    ...
    setUpdateReady(e) {
      (e
        ? this.setUpdateLifecycleState(`ready`)
        : (this.updateLifecycleState === `ready` || this.updateLifecycleState === `installing`) &&
          this.setUpdateLifecycleState(`idle`),
        this.isUpdateReady !== e &&
          ((this.isUpdateReady = e),
          e
    ...
    scheduleForcedUpdateInstall() {
      ...
      let t = Ve(this.updateDownloadedAtMs, e);
      (this.setRelaunchNotice(t),
        this.scheduleForcedUpdateInstallAt(Math.max(t, this.forcedUpdateRetryAtMs ?? t)));
    }
    ...
    getRelaunchNotificationPolicy() {
      return this.owlUpdatePolicies ? Be(this.owlUpdatePolicies) : null;
    }
    async installForcedUpdate() {
      this.forcedUpdateInstallStarted ||
        ...
        this.setRelaunchNotice(null),
        R().info(`Installing update required by managed relaunch policy.`),
        !((await this.installUpdatesIfAvailable()) || !this.isUpdateReady) &&
          ((this.forcedUpdateInstallStarted = !1),
          (this.forcedUpdateRetryAtMs = Date.now() + dt),
          this.scheduleForcedUpdateInstall()));
    }
    setInstallProgressPercent(e) {
      (e == null
    ...
    setDownloadProgressPercent(e) {
      this.downloadProgressPercent !== e &&
        ((this.downloadProgressPercent = e), this.options.onDownloadProgressChanged?.(e));
    }
    createRelaunchNotice(e) {
      return { deadlineAtMs: e, reportedAtMs: Date.now() };
    }
    setRelaunchNotice(e) {
      if ((this.relaunchNotice?.deadlineAtMs ?? null) === e) return;
      let t = e == null ? null : this.createRelaunchNotice(e);
      ((this.relaunchNotice = t), this.options.onRelaunchNoticeChanged?.(t));
    }
    setUpdateLifecycleState(e) {
...
function ht(e) {
  ...
  let a = [],
    o = null,
    s = () => {
      if (process.platform === `win32` && i.app.isPackaged) {
        i.app.exit(0);
        return;
      }
      i.app.quit();
  ...
  return (
    (z = {
      ...
      buildFlavor: r,
      desktopSentry: n,
      sparkleManager: new mt({
        enableUpdater: j.shouldIncludeUpdater(r, process.platform, process.env),
        buildFlavor: r,
        ...
        onRelaunchNoticeChanged: (e) => {
          l(e);
        },
        isTrustedIpcEvent: (e) => p(e),
      }),
      setSparkleBridgeHandlers: (e) => {
        (e.onInstallUpdatesRequested != null && (s = e.onInstallUpdatesRequested),
          e.onUpdateLifecycleStateChanged != null && (c = e.onUpdateLifecycleStateChanged),
          e.onRelaunchNoticeChanged != null && (l = e.onRelaunchNoticeChanged),
          e.onUpdateReadyChanged != null && (u = e.onUpdateReadyChanged),
          e.onInstallProgressChanged != null && (d = e.onInstallProgressChanged),
          e.onDownloadProgressChanged != null && (f = e.onDownloadProgressChanged),
          e.isTrustedIpcEvent != null && (p = e.isTrustedIpcEvent));
      },
...
var _t = `codex:`,
  vt = `--open-project`,
  yt = new Set([`marketplace`]),
  bt = new Set([`marketplacePath`, `mode`]),
  xt = [`browser-use`, `computer-use/google-chrome`, `connections`],
  St = new Set([`name`, `description`, `imageUrl`]);
function Ct(e) {
...
function wt(e) {
  ...
  if (n.protocol !== _t) return null;
  switch (n.host) {
    case `plugins`:
      return kt(n) ?? At(n) ?? Mt(n);
    case `pets`:
      return jt(n);
    case `automations`:
    ...
    case `settings`: {
      let e = B(n).join(`/`),
        t = xt.find((t) => t === e);
      return t == null ? { kind: `settings` } : { kind: `settings`, section: t };
    }
    case `skills`:
      return { kind: `skills` };
    case `threads`: {
      ...
      return e
        ? e === `new`
          ? (Ot(n) ?? { kind: `newThread` })
          : Ct(e)
            ? { kind: `localConversation`, conversationId: t.Dr(e) }
            : null
        : null;
    }
    default:
...
function Tt(e, { allowBareWindowsProjectPathArg: t = !1 } = {}) {
  let n = [];
  for (let r = 0; r < e.length; r += 1) {
    ...
    t && r > 0 && Et(i) && n.push({ kind: `newThread`, path: i });
  }
  return n;
}
function Et(e) {
  let t = e.trim();
  return t.length === 0 ? !1 : /^[a-zA-Z]:[\\/]/.test(t) || t.startsWith(`\\\\`);
}
function Dt(e) {
  let [t] = B(e);
...
function kt(e) {
  ...
  let a = U(r);
  if (a == null || !H(e, yt)) return null;
  let o = V(e, `marketplace`)?.trim();
  return t.at(o) ? { kind: `pluginInstall`, marketplaceName: o, pluginName: a } : null;
}
function At(e) {
  let [n, r] = B(e);
  if (r != null || !H(e, bt)) return null;
  let i = U(n),
    a = V(e, `marketplacePath`)?.trim(),
    o = V(e, `mode`)?.trim();
  return i == null || a == null || (o != null && o !== `share`) || !t.hr(a) || t._r(a)
    ? null
    : {
...
function jt(e) {
  let [t, n] = B(e);
  if (t !== `install` || n != null || !H(e, St)) return null;
  let r = V(e, `name`)?.trim(),
    i = V(e, `description`)?.trim() ?? null,
    a = V(e, `imageUrl`);
...
function Nt(e, t) {
  ...
  return n === `oauth_callback`
    ? { kind: `connectorOAuthCallback`, fullRedirectUrl: t, returnTo: V(e, `returnTo`) }
    : null;
}
function Pt({ isMacOS: e, isPackaged: t }) {
  return t ? !e : !1;
}
function Ft({
  app: e,
  ...
  let c = [],
    l = n.Ir();
  async function u() {
    if (c.length === 0 || !e.isReady()) return;
    let t = await i();
    if (!t) return;
    let n = c.at(-1);
    ((c.length = 0), n && (await a(t, n)));
  }
  function d(n) {
    (c.push(n),
      e.isReady() &&
        u().catch((e) => {
          s.reportNonFatal(e instanceof Error ? e : t.Bt(e), { kind: `flush-pending-deep-links` });
        }));
  }
  function f(e, t) {
    let n = wt(e);
    return n ? (d(n.kind === `localPluginDetail` && t != null ? { ...n, hostId: t } : n), !0) : !1;
  }
  let p = e.isPackaged && !r;
  function m(e) {
    for (let t of Tt(e)) d(t);
  }
  function h(e) {
    let t = Tt(e, { allowBareWindowsProjectPathArg: p });
    for (let e of t) d(e);
    return t.length > 0;
  }
  function g() {
    if (!(!e.isPackaged || process.platform === `win32`))
      try {
        e.setAsDefaultProtocolClient(`codex`) ||
          l.warning(`Failed to register codex:// protocol handler`, {
            safe: { isPackaged: e.isPackaged },
            sensitive: {},
          });
      } catch (e) {
        l.warning(`Failed to register codex:// protocol handler`, {
...
function It() {
  ...
  for (;;) {
    if (o.extname(e) === `.app`) return e;
    let t = o.dirname(e);
    if (t === e) return process.execPath;
    e = t;
  }
}
var Lt = e.n((e, t) => {
  ...
  Rt = e.o(t.St()),
  zt = e.o(Lt()),
  Bt = `owl-feature-bootstrap-cache.json`,
  Vt = t.Ci(t.Pi()),
  Ht = t.ji({ enabledOwlFeatureNames: Vt });
function Ut() {
  if (!Te()) return;
  let e = Jt();
  if (e.length > 0) {
    let t = i.app.commandLine.getSwitchValue(`enable-features`),
      n = t.length > 0 ? `${t},${e.join(`,`)}` : e.join(`,`);
    (i.app.commandLine.removeSwitch(`enable-features`),
      i.app.commandLine.appendSwitch(`enable-features`, n));
  }
}
function Wt(e) {
  let t = W(Vt.parse(e)),
    n = Yt();
  (s.default.mkdirSync(o.default.dirname(n), { recursive: !0 }),
    Xt(n, JSON.stringify({ enabledOwlFeatureNames: t })));
}
function Gt() {
  return {
    getState: async () => Kt(),
    isOwlFeatureEnabled: async (e) => qt(e),
    setEnabledFeatureNames: async (e) => (Wt(e), Kt()),
  };
}
function Kt() {
  let e = W(i.app.commandLine.getSwitchValue(`enable-features`).split(`,`)),
    t = Jt();
  return {
    activeFeatureNames: e,
    pendingFeatureNames: t,
    restartRequired: !(0, Rt.default)((0, zt.default)(e).sort(), (0, zt.default)(t).sort()),
  };
}
function qt(e) {
  return i.app.commandLine.getSwitchValue(`enable-features`).split(`,`).includes(e);
}
function Jt() {
  let e = Yt();
  try {
    let { enabledOwlFeatureNames: t } = Ht.parse(JSON.parse(s.default.readFileSync(e, `utf8`)));
    return W(t);
  } catch {
    return [];
  }
...
function W(e) {
  return e.filter((e) => t.B.some((t) => t === e));
}
function Yt() {
  return o.default.join(i.app.getPath(`userData`), Bt);
}
function Xt(e, t) {
  let n = o.default.join(
    o.default.dirname(e),
    `.${o.default.basename(e)}.tmp-${Date.now()}-${(0, c.randomUUID)()}`,
  );
  try {
    (s.default.writeFileSync(n, t, `utf8`), s.default.renameSync(n, e));
  } catch (e) {
    throw (s.default.rmSync(n, { force: !0 }), e);
  }
}
function Zt(e, t = process.platform) {
  let n = process.env,
    r = (0, a.homedir)();
  return t === `darwin`
    ? (0, o.join)(r, `Library`, `Logs`, gt(e ?? j.resolve()))
    : t === `win32`
      ? (0, o.join)(n.LOCALAPPDATA ?? (0, o.join)(r, `AppData`, `Local`), `Codex`, `Logs`)
      : t === `linux`
        ? (0, o.join)(n.XDG_STATE_HOME ?? (0, o.join)(r, `.local`, `state`), `codex`, `logs`)
        : (0, o.join)(r, `.codex`, `logs`);
}
var Qt = 10 * 1024 * 1024,
  $t = 5,
  en = 1e4,
...
function rn(e, t) {
  return (0, o.join)(e, t.getUTCFullYear().toString(), G(t.getUTCMonth() + 1), G(t.getUTCDate()));
}
function K(e, t, n, r) {
...
function cn(e, t, n, r) {
  try {
    let r = new Date(Date.UTC(t.getUTCFullYear(), t.getUTCMonth(), t.getUTCDate())),
      i = new Date(r);
    i.setUTCDate(i.getUTCDate() - (n - 1));
    for (let t of (0, s.readdirSync)(e)) {
      if (!/^\d{4}$/.test(t)) continue;
      let n = Number(t);
      if (!Number.isFinite(n)) continue;
      let r = (0, o.join)(e, t);
      for (let e of (0, s.readdirSync)(r)) {
        if (!/^\d{2}$/.test(e)) continue;
        let t = Number(e);
        if (!Number.isFinite(t) || t < 1 || t > 12) continue;
        let a = (0, o.join)(r, e);
        for (let e of (0, s.readdirSync)(a)) {
          if (!/^\d{2}$/.test(e)) continue;
          let r = Number(e);
          !Number.isFinite(r) ||
            r < 1 ||
            ...
            (new Date(Date.UTC(n, t - 1, r)) < i &&
              (0, s.rmSync)((0, o.join)(a, e), { recursive: !0, force: !0 }));
        }
        (0, s.readdirSync)(a).length === 0 && (0, s.rmSync)(a, { recursive: !0, force: !0 });
      }
      (0, s.readdirSync)(r).length === 0 && (0, s.rmSync)(r, { recursive: !0, force: !0 });
    }
  } catch (t) {
...
function ln(e, t = {}) {
  let n = (t, n) => e.nonFatalReporter.reportNonFatal(t, { kind: `file-based-logger`, extra: n }),
    r = e.rootDir ?? Zt(),
    i = t.processId ?? process.pid,
    a = t.threadId ?? p.threadId ?? 0,
    c = t.instanceId ?? sn(),
    l = e.appSessionId,
    u = t.now ?? (() => new Date()),
    d = e.maxSegmentBytes ?? Qt,
    f = e.maxSegments ?? $t,
    m = e.pendingLineLimit ?? en,
    h = e.highWaterMarkBytes ?? tn,
    g = nn,
    _ = t.createStream ?? ((e, t) => (0, s.createWriteStream)(e, { flags: `w`, highWaterMark: t })),
    v = K(f, $t, n, `maxSegments`),
    y = K(d, Qt, n, `maxSegmentBytes`),
    b = K(m, en, n, `pendingLineLimit`),
    x = { logLine: () => {} };
  try {
    let e = u(),
      t = rn(r, e);
    ((0, s.mkdirSync)(t, { recursive: !0 }), cn(r, e, g, n));
    let d = an(l, i, a, c, e),
      f = (e) => (0, o.join)(t, `${d}-${e}.log`),
      p = !1,
      m = (e) => {
        ((p = !0),
      ...
      ne = () => {
        if (O === 0) return;
        let e = `[file-logger] dropped ${O} lines due to backpressure\n`;
        (T.push({ text: e, bytes: Buffer.byteLength(e) }), (O = 0));
      },
      re = () => {
        if (p) {
          ((T = []), (E = 0), (O = 0));
      ...
      logLine: (e) => {
        if (!p)
          try {
            ...
            (T.push({ text: t, bytes: Buffer.byteLength(t) }), re());
          } catch (e) {
...
function un(e) {
  return (
    ...
    !!e &&
    `code` in e &&
    (e.code === `EPIPE` || e.code === `ERR_STREAM_DESTROYED` || e.code === `EBADF`)
  );
}
function dn(e = [process.stdout, process.stderr]) {
  for (let t of e)
...
function pn(e, t, r, i) {
  let a = (e) => e ?? { safe: {}, sensitive: {} },
    o = fn(),
    s = ln({ appSessionId: r, nonFatalReporter: e }),
    c = (e) => {
    ...
    l = (e) => (r, s) => {
      ...
      let l = a(s),
        ...
        d = Object.keys(u).length === 0 ? r : `${r} ${n.Fr(u)}`;
      (o(e, d), c(`${e} ${d}`), t.log(e, r, l.safe));
    },
    u = {
      trace: l(`trace`),
      debug: l(`debug`),
...
var mn = { retry: `Retry`, quit: `Quit` };
async function hn() {
  return _n({
    databasePath: n.dr(),
    open: () => {
      let e = n.fr();
      if (e == null) throw Error(`SQLite is only available in the Electron app.`);
      yn(e);
    },
  });
}
...
async function _n({ databasePath: e, open: t, shouldHandleError: n = () => !0 }) {
  for (;;)
    try {
      return (await t(), !0);
    } catch (t) {
      if (!n(t)) throw t;
      if ((await bn(vn(e, t), t)) === `retry`) continue;
      return (i.app.quit(), !1);
    }
}
function vn(e, t) {
...
async function bn(e, t) {
  return (
    await i.dialog.showMessageBox({
      type: `error`,
      buttons: [mn.retry, mn.quit],
      defaultId: 0,
      cancelId: 1,
      message: `${i.app.getName()} cannot access its local database.`,
      detail: [
...
var Sn = [
    ...
    { displayName: `XLS`, extension: `xls`, mimeType: `application/vnd.ms-excel` },
    {
      displayName: `XLSM`,
      extension: `xlsm`,
  ...
  Cn = Sn.map(({ extension: e }) => `.${e}`);
Sn.map(({ mimeType: e }) => e);
var wn = new Set(Cn);
function Tn(e) {
  return wn.has(o.extname(e).toLowerCase());
}
function En(e, t = process.platform) {
  let n = [];
  for (let r = 1; r < e.length; r += 1) {
    ...
    if (i.startsWith(`--open-project=`) || i.startsWith(`-`) || On(i)) continue;
    let a = kn(i);
    a == null || !An(a, t) || !Tn(a) || n.push(a);
  }
  return n;
}
function Dn(e) {
  let t = [],
    n = new Set();
  for (let r of e) n.has(r) || (n.add(r), t.push({ label: o.basename(r), path: r, fsPath: r }));
  return t;
}
function On(e) {
...
function kn(e) {
  ...
  try {
    return (0, d.fileURLToPath)(e);
  } catch {
    return null;
  }
}
function An(e, t) {
  return t === `win32` ? o.win32.isAbsolute(e) : o.posix.isAbsolute(e);
}
function q(e) {
...
function J(e) {
  return {
    activeRoots: q(e.get(t.br.ACTIVE_WORKSPACE_ROOTS)),
    roots: q(e.get(t.br.WORKSPACE_ROOT_OPTIONS)),
    labels: jn(e.get(t.br.WORKSPACE_ROOT_LABELS)),
  };
}
function Y(e) {
...
function Nn(e, n) {
  let r = e.get(t.br.REMOTE_PROJECTS) ?? [],
    i = [],
    a = {},
    o = new Set();
  for (let e of r) {
...
function Z(e, n) {
  e.set(t.br.ACTIVE_WORKSPACE_ROOTS, n);
}
function Pn(e, n) {
  e.set(t.br.WORKSPACE_ROOT_OPTIONS, n);
}
function Fn(e, n) {
  let r = [n, ...q(e.get(t.br.PROJECT_ORDER)).filter((e) => e !== n)];
  e.set(t.br.PROJECT_ORDER, r);
}
function In(e, n) {
  let r = n(e.get(t.br.WORKSPACE_ROOT_OPTIONS) ?? null);
  return (e.set(t.br.WORKSPACE_ROOT_OPTIONS, r), r);
}
function Ln(e, n) {
  let r = n(e.get(t.br.WORKSPACE_ROOT_LABELS) ?? null);
  return (e.set(t.br.WORKSPACE_ROOT_LABELS, r), r);
}
function Rn(e, t) {
...
var zn = [],
  ...
  $ = null;
function Bn(e) {
  return e.length === 0 ? !1 : Q == null ? (zn.push(...e), !0) : (Q(e), !0);
}
function Vn(e) {
  if (!e || $ != null) return () => {};
  let t = (e, t) => {
    (e.preventDefault(), Bn([t]));
  };
  return (
    ($ = t),
    i.app.on(`open-file`, t),
    () => {
      $ === t && (i.app.removeListener(`open-file`, t), ($ = null));
    }
  );
}
function Hn(e) {
  let r = Vn(e.isMacOS),
    ...
    s = async (t) => {
      ...
      let r = X(e.globalState),
        i = [];
      for (let t of n) r.includes(t) || ((r = [t, ...r]), Fn(e.globalState, t), i.push(t));
      let a = n[n.length - 1];
      if (i.length > 0) {
        (Pn(e.globalState, r),
          e.windowManager.sendMessageToAllWindows({ type: `workspace-root-options-updated` }));
        for (let t of i)
          e.windowManager.sendMessageToAllWindows({ type: `workspace-root-option-added`, root: t });
      }
      let o = Y(e.globalState);
      ((o.length !== 1 || o[0] !== a) &&
        (Z(e.globalState, [a]),
        e.windowManager.sendMessageToAllWindows({ type: `active-workspace-roots-updated` })),
        e.windowManager.showPrimaryWindow({ stealFocus: !0 }),
        e.windowManager.sendMessageToAllWindows({
    ...
    c = async (t) => {
      ...
      let i = await e.ensureWindow();
      i == null ||
        i.isDestroyed() ||
        (e.windowManager.showPrimaryWindow({ stealFocus: !0 }),
        e.windowManager.sendMessageToWindow(i, {
          type: `new-quick-chat`,
          files: r,
          previewFiles: r,
        }),
        n
          .Ir()
          .info(`Opened desktop files in quick chat`, {
            safe: { fileCount: r.length, fileExtensions: Un(r) },
          }));
    },
    u = async (e) => {
      let t = o.resolve(e);
      try {
        let e = await (0, l.stat)(t);
        return e.isDirectory()
          ? { kind: `directory`, path: t }
          : e.isFile() && Tn(t)
            ? { kind: `file`, path: t }
            : null;
      } catch (e) {
        return (
          n.Ir().warning(`Failed to stat desktop open path`, { safe: {}, sensitive: { error: e } }),
          null
        );
      }
    },
    d = async () => {
      ...
      (await s(n), await c(r), i.length > 0 && (await d()));
    },
    f = async () => {
      if (a) return a;
      a = d();
    ...
    p = (n) => {
      (i.push(...n),
        f().catch((n) => {
          e.errorReporter.reportNonFatal(n instanceof Error ? n : t.Bt(n), {
            kind: `desktop-open-paths`,
          });
        }));
    };
  Q = p;
  let m = zn.splice(0, zn.length);
  return (
    m.length > 0 && p(m),
    () => {
      (Q === p && (Q = null), r());
...
function Un(e) {
  return Array.from(new Set(e.map((e) => o.extname(e.path).slice(1).toLowerCase()))).sort();
}
