...
const e = require(`./src.js`),
  t = require(`./file-based-logger.js`);
...
var c = (0, i.promisify)(s.gzip),
  l = e.qr(`feedback-desktop-log-archive`),
  u = 512,
  d = 2,
  f = new Map();
async function p(e = new Date(), i, s = []) {
  let u = new Date(Date.UTC(e.getUTCFullYear(), e.getUTCMonth(), e.getUTCDate())),
    d = Math.floor(e.getTime() / 1e3),
    p = x(t.n(), u),
    m = i
      ?.readMostRecentSessionLog(e)
      .catch(
        (e) => (
          l().warning(`Failed to read realtime session log for feedback.`, {
            safe: {},
            sensitive: { error: e },
          }),
          null
    ...
    [_, v] = await Promise.all([h(p), m]),
    y = [
      ...(await Promise.all(
        _.map(async (e) => ({
          name: e,
          contents: await (0, o.readFile)((0, r.join)(p, e)),
          mtimeSeconds: d,
          mode: 420,
        })),
      )),
      ...s.map(({ contents: e, name: t }) => ({
        contents: e,
        mode: 420,
        mtimeSeconds: d,
        name: t,
  ...
  if (
    (v != null &&
      y.push({ name: `realtime-sessions/latest.jsonl`, contents: v, mtimeSeconds: d, mode: 420 }),
    y.length === 0)
  )
    return null;
  let b = await (0, o.mkdtemp)((0, r.join)((0, n.tmpdir)(), `codex-feedback-desktop-log-archive-`)),
    C = (0, r.join)(b, `codex-desktop-app-logs-${S(u)}.tar.gz`);
  await (0, o.writeFile)(C, await c(g(y)));
  let w = (0, a.randomUUID)();
  return (f.set(w, b), { archiveId: w, archivePath: C });
}
async function m(e) {
  let t = f.get(e);
  if (t == null) return !1;
...
//# sourceMappingURL=feedback-desktop-log-archive.js.map
