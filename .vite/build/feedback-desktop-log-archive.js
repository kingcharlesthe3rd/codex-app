...
var c = (0, i.promisify)(s.gzip),
  l = e.Gr(`feedback-desktop-log-archive`),
  u = 512,
  d = 2,
  f = new Map();
async function p(e = new Date(), i) {
  let s = new Date(Date.UTC(e.getUTCFullYear(), e.getUTCMonth(), e.getUTCDate())),
    ...
    [m, _] = await Promise.all([h(d), p]),
    v = await Promise.all(
      m.map(async (e) => ({
        name: e,
        contents: await (0, o.readFile)((0, r.join)(d, e)),
        mtimeSeconds: u,
        mode: 420,
      })),
    );
  if (
    (_ != null &&
      v.push({ name: `realtime-sessions/latest.jsonl`, contents: _, mtimeSeconds: u, mode: 420 }),
    v.length === 0)
  )
    return null;
  let y = await (0, o.mkdtemp)((0, r.join)((0, n.tmpdir)(), `codex-feedback-desktop-log-archive-`)),
    b = (0, r.join)(y, `codex-desktop-app-logs-${S(s)}.tar.gz`);
  await (0, o.writeFile)(b, await c(g(v)));
  let C = (0, a.randomUUID)();
  return (f.set(C, y), { archiveId: C, archivePath: b });
}
async function m(e) {
  let t = f.get(e);
  if (t == null) return !1;
