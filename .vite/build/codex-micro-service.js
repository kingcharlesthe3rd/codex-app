...
var n = e.ni(`CodexMicroService`),
  ...
  _ = class {
    ...
    async updateLighting(e) {
      if (((this.lastLightingModel = e), this.lifecycleState === `stopped`)) return !1;
    ...
    async stop() {
      ...
      (this.reconnectTimer != null &&
        (this.clearScheduled(this.reconnectTimer), (this.reconnectTimer = null)),
        this.batteryRefreshTimer != null &&
          (this.clearScheduled(this.batteryRefreshTimer), (this.batteryRefreshTimer = null)),
        this.clearSelectionLightingTimer(),
      ...
      ((this.comm = null),
        ...
        !(this.lifecycleState !== `stopped` || this.connectionAttemptId !== e) &&
          ...
          this.setDeviceState({ status: `not-detected`, error: null, battery: null })));
    }
    dispose() {
      return this.stop();
    }
    async ensureConnected() {
      if (this.lifecycleState !== `started` || this.api != null || this.reconnectTimer != null)
        return;
    ...
    async connect(e) {
      ...
      try {
        ...
        (n.warning(`Codex Micro discovery failed`, { safe: {}, sensitive: { error: t } }),
          this.setDeviceState({ status: `error`, error: `Discovery failed`, battery: null }),
      ...
      this.unsubscribeConnectionEvent = a.onConnectionEvent((t) => {
        if (this.isCurrentConnectionAttempt(e))
          switch (t.type) {
            case r.CONNECTED:
              this.setDeviceState({ status: `connected`, error: null, battery: null });
              break;
            case r.DISCONNECTED:
      ...
      try {
        ...
        (n.warning(`Codex Micro connection failed`, { safe: {}, sensitive: { error: t } }),
          this.unsubscribeConnectionEvent?.(),
          (this.unsubscribeConnectionEvent = null),
          await a.disconnect(),
          this.setDeviceState({ status: `error`, error: `Connection failed`, battery: null }),
          this.scheduleReconnect());
      }
    }
    applyLighting(e) {
      return this.enqueueLightingWrite(async () => {
        ...
        let n = await t.sendLightingConfig(y(e.slots, e.voiceState, this.selectionLightingVisible));
        if (this.lifecycleState !== `started` || this.api !== t) return !1;
        let r = await t.sendThreadsLighting(v(e.slots));
        return (r && this.api === t && (this.displayedLightingModel = e), n && r);
      });
    }
    handleHidEvent(e) {
      ...
      this.options.onHidEvent({
        ...
        agent: e.agent ?? null,
        slot: n,
        threadKey: n == null ? null : (this.displayedLightingModel.slots[n]?.threadKey ?? null),
      });
    }
    ...
    clearSelectionLightingTimer() {
      this.selectionLightingTimer != null &&
        (this.clearScheduled(this.selectionLightingTimer), (this.selectionLightingTimer = null));
    }
          ...
          this.setDeviceState({
            ...
            battery:
              t.batteryPercentage == null
                ? null
                : { percentage: t.batteryPercentage, isCharging: t.isCharging ?? null },
          });
        } catch {}
        this.lifecycleState === `started` &&
          this.api === e &&
    ...
    isCurrentConnectionAttempt(e) {
      return this.lifecycleState === `started` && this.connectionAttemptId === e;
    }
  };
function v(t) {
  return t.map((t) =>
...
function y(t, n, r) {
  let i = t.find((e) => e.selected),
    o = b(n);
  if (i == null || i.status === `off`) return { keys: h.keys, ambient: o ?? h.ambient };
  let s = {
  ...
  return {
    keys: r ? { effect: a.solid, brightness: 1, speed: 0, magic: 0, color: c.color } : h.keys,
    ambient: c,
  };
}
function b(e) {
  switch (e) {
    ...
    case `completed`:
      return { effect: a.solid, brightness: 1, speed: 0, magic: 0, color: 16777215 };
  }
}
function x() {
